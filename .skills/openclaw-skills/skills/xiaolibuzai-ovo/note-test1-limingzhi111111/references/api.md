...
| 字段 | 类型 | 说明 |
|------|------|------|
| `basic_info` | DocBasicInfo | 小笔记基础信息，见 [DocBasicInfo](#docbasicinfo) |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `folder_id` | string | **是** | 笔记本 ID，根目录为 `user_list_{userid}` |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|---|----|
| `content_format` | int | **是** | 文本类型：`0`=纯文本，`1`=Markdown（默认），`2`=JSON，目前仅支持 `MARKDOWN`（值为 `1`） |
