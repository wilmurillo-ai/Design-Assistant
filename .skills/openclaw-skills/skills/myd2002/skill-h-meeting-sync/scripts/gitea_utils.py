"""
Gitea API 工具函数（与 Skill-A/B 保持一致）
"""
...
def delete_file_in_repo(owner, repo, filepath, sha, message, token, base_url, branch="main"):
    resp = gitea_request(
        "DELETE",
        f"/repos/{owner}/{repo}/contents/{filepath}",
        token, base_url,
...
def list_dir_in_repo(owner, repo, dirpath, token, base_url, branch="main"):
    """列出仓库指定目录下的文件和子目录，返回 items 列表。失败返回 []。"""
    try:
        resp = gitea_request(
            "GET",
            f"/repos/{owner}/{repo}/contents/{dirpath}",
            token, base_url,
            params={"ref": branch},
        )
        return resp.json()
...
def list_meetings_in_repo(owner, repo, token, base_url, branch="main"):
    """列出 meetings/ 下所有会议目录名（不含 archive）。"""
    try:
        resp = gitea_request(
            "GET",
...
def get_managed_repos(token, base_url):
    """返回 bot 有访问权限且包含 meetings/ 目录的所有仓库全名列表。"""
    repos = []
    page = 1
    while True:
        try:
