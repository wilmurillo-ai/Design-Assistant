...
> ⚠️ **Critical:** The `range` MUST be in the request body (inside `valueRange`), NOT in the URL path. Putting range in the URL path causes HTTP 404.
> 
> ✅ Correct: `PUT /values` with `{"valueRange": {"range": "sheetId!A1", "values": [[...]]}}`
> ❌ Wrong: `PUT /values/sheetId!A1` (returns 404)
...
> Uses the same PUT endpoint as Write Values. To append, set range to just the sheet ID (e.g. `"0bxxxx"`) — data will be written starting from the first empty row.

## OAuth Scopes

When constructing an authorization URL, scopes are separated by `+`:

```
scope=sheets%3Aspreadsheet%3Acreate+sheets%3Aspreadsheet%3Aread+sheets%3Aspreadsheet%3Awrite_only
```

| Permission | Scope Name |
|---|---|
| Create spreadsheet | `sheets:spreadsheet:create` |
| Read spreadsheet | `sheets:spreadsheet:read` |
| Write spreadsheet | `sheets:spreadsheet:write_only` ⚠️ NOT `write` |
| Read-only | `sheets:spreadsheet:readonly` |

> ⚠️ Common mistake: using `sheets:spreadsheet:write` (wrong) → correct is `sheets:spreadsheet:write_only`
...
**Endpoint:** `POST /sheets/v3/spreadsheets/{spreadsheet_token}/sheets`
...
> ⚠️ Note: Use v3 API (not v2), and use `/sheets` endpoint (not `sheets_batch_update`)
