# WordPress Expert (`wordpress-site-ops`)

**Skill id / folder / ClawHub slug:** `wordpress-site-ops` — ClawHub display name: **WordPress Expert** (`clawhub publish --name "WordPress Expert"`). The **URL path** stays `…/wordpress-site-ops` (technical id); the **page title** is set by `--name` or ClawHub settings.
...
- `metadata.openclaw.requires.env: ["WORDPRESS_SITE_URL","WORDPRESS_USER","WORDPRESS_APPLICATION_PASSWORD"]` — must be set (host env or `skills.entries["wordpress-site-ops"].env`) for the skill to be **eligible**, aligned with typical REST use.
