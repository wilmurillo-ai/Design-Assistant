...
Skill **`wordpress-site-ops`** must **not** build a parallel OpenClaw. Prefer WordPress-specific, policy-aware steps via the plugin; everything else via **built-in** OpenClaw tools (names depend on gateway version—see [OpenClaw Tools](https://docs.openclaw.ai/tools)).
