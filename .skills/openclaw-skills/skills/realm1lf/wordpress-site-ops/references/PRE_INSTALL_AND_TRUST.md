...
Read this **before** enabling the companion OpenClaw plugin, setting WordPress credentials, or deploying optional code to a site. Skill id: **`wordpress-site-ops`** (ClawHub display name **WordPress Expert**).
...
- Store values in **`openclaw.json`** `skills.entries["wordpress-site-ops"].env` or host env—**never** in chat, Git, or the skill bundle.
...
Registries and security scanners may flag skills that mention credentials, shell install steps, or third-party repos. This skill declares **expected** REST configuration via `metadata.openclaw.requires.env` in `SKILL.md` so metadata and docs stay aligned. If you believe a flag is a **false positive**, contact ClawHub maintainers with links to this file and the public plugin repo.
