# User expectations: WordPress Expert (skill `wordpress-site-ops` + tools)

From the perspective of someone using skill **`wordpress-site-ops`** (ClawHub display name **WordPress Expert**) and plugin **`wordpress-site-tools`** for a WordPress project: what they **expect**, how they **use** it, and what should be possible **later**.
...
1. **Skill**—copy folder `openclaw-wordpress-skill` to `~/.openclaw/workspace/skills/wordpress-site-ops` (folder name = skill `name`).
...
4. **Config**—in `skills.entries["wordpress-site-ops"].env` (and optionally `plugins.entries.wordpress-site-tools.config`):
...
*Alignment: skill `wordpress-site-ops` + plugin `wordpress-site-tools` (connection check, REST, WP-CLI, optional media upload, optional plugin files); long-term goal: broad WordPress operation and developer role per section 2.1.*
