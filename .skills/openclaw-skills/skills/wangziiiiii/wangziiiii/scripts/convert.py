...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Convert image format (single file or directory).")
    p.add_argument("--input", required=True, help="Input image file or directory")
    p.add_argument("--format", required=True, help="Target format: jpg/png/webp/bmp/tiff/gif")
    ...
    p.add_argument("--quality", type=int, default=90, help="Quality for lossy formats (1-100)")
    ...
    p.add_argument("--suffix", default="_converted", help="Output filename suffix")
    p.add_argument("--overwrite", action="store_true", help="Overwrite existing files")
    p.add_argument("--include-ext", default=None, help="Only process these extensions, comma-separated (e.g. jpg,png)")
...
def main() -> int:
    ...
    target_format = normalize_format(args.format)
    ...
    for src in files:
        try:
            ...
            save_image(img, out_path, target_format, quality=args.quality, optimize=args.optimize)
            ok += 1
