...
# Broad search
flyai fliggy-fast-search --query "{city} hotels"
```
...
```bash
# Relax 30%
flyai search-hotels ... --max-price {budget*1.3}
# Try homestays
...
```bash
# Shift ±1 day
flyai search-hotels ... --check-in-date "{in+1}" --check-out-date "{out+1}"
# City-wide
flyai search-hotels --dest-name "{city}" --sort price_asc
```
