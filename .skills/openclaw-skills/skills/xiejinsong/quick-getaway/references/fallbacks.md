...
```bash
flyai search-hotels --dest-name "{city}" --sort rate_desc  # Remove filters
flyai search-hotels --dest-name "{nearby}" --sort rate_desc
```
...
```bash
flyai fliggy-fast-search --query "{user request}"
```
