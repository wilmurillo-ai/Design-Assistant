...
def baidu_search(api_key: str, client: str, environment: str, api_base: str, request_body: dict) -> dict:
    ...
    headers = {
        ...
        "User-Agent": "openclaw-baidu-web-search",
    }
    ...
    keys_to_remove = {"icon", "web_anchor"}
    results = []
    for item in references:
        result = {k: v for k, v in item.items() if k not in keys_to_remove and v}
...
if __name__ == "__main__":
    ...
    if "q" not in parse_data:
        print("Error: 'q' must be present in request body.")
        sys.exit(1)
    ...
    request_body: dict = {
        "q":     parse_data["q"],
        "count": parse_data.get("count", 10),
    }
    if "search_recency_filter" in parse_data:
        request_body["search_recency_filter"] = parse_data["search_recency_filter"]

    try:
        result = baidu_search(api_key, client, environment, api_base, request_body)
