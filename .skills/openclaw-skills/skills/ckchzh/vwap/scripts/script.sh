#!/usr/bin/env bash
# vwap — Vwap reference tool. Use when working with vwap in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
vwap v$VERSION — Vwap Reference Tool
...
## What is Vwap?
Vwap (vwap) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with vwap.

## Key Concepts
- Core vwap principles and fundamentals
- How vwap fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Vwap Matters
Understanding vwap is critical for:
...
## Getting Started
1. Understand the basic vwap concepts
...
cmd_formulas() {
    cat << 'EOF'
# Vwap — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Vwap — Instruments & Tools Overview

## Primary Instruments
- Core tools used in vwap operations
...
cmd_glossary() {
    cat << 'EOF'
# Vwap — Key Terms & Definitions

## Core Terminology
- **Vwap**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "vwap v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: vwap help"; exit 1 ;;
esac
