...
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${TEST_GENERATOR_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/test-generator}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
test-generator v$VERSION

Multi-purpose utility tool

Usage: test-generator <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "test-generator v$VERSION" ;;
