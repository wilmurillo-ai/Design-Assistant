#!/usr/bin/env bash
# test-publish-check - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${TEST_PUBLISH_CHECK_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/test-publish-check}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
test-publish-check v$VERSION

Multi-purpose utility tool

Usage: test-publish-check <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "test-publish-check v$VERSION" ;;
