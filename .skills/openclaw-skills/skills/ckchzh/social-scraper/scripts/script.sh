#!/usr/bin/env bash
# social-scraper — Social Scraper reference tool. Use when working with social scraper in data contexts.
...
show_help() {
    cat << 'HELPEOF'
social-scraper v$VERSION — Social Scraper Reference Tool

Usage: social-scraper <command>
...
cmd_intro() {
    cat << 'EOF'
# Social Scraper — Overview

## What is Social Scraper?
Social Scraper (social-scraper) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with social scraper.

## Key Concepts
- Core social scraper principles and fundamentals
- How social scraper fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Social Scraper Matters
Understanding social scraper is critical for:
...
## Getting Started
1. Understand the basic social scraper concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Social Scraper — Quick Start Guide
...
## Installation
1. Download or clone the social scraper package
...
cmd_patterns() {
    cat << 'EOF'
# Social Scraper — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for social scraper
...
cmd_debugging() {
    cat << 'EOF'
# Social Scraper — Debugging Guide
...
cmd_performance() {
    cat << 'EOF'
# Social Scraper — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Social Scraper — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Social Scraper — Migration & Upgrade Guide
...
cmd_cheatsheet() {
    cat << 'EOF'
# Social Scraper — Quick Reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "social-scraper v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: social-scraper help"; exit 1 ;;
esac
