#!/usr/bin/env bash
# sandwich -- Analyze sandwich operations. Use when you need to understand sandwich mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SANDWICH_DIR:-$HOME/.sandwich}"
...
show_help() {
    cat << EOF
sandwich v$VERSION -- Analyze sandwich operations. Use when you need to understand sandwich mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: sandwich <command> [args]
...
cmd_add() {
    local value="${1:?Usage: sandwich add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: sandwich search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: sandwich remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "sandwich v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: sandwich help"; exit 1 ;;
esac
