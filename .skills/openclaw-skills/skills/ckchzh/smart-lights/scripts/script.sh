#!/usr/bin/env bash
# smart-lights — Smart Lights reference tool. Use when working with smart lights in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
smart-lights v$VERSION — Smart Lights Reference Tool

Usage: smart-lights <command>
...
cmd_intro() {
    cat << 'EOF'
# Smart Lights — Overview

## What is Smart Lights?
Smart Lights (smart-lights) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with smart lights.

## Key Concepts
- Core smart lights principles and fundamentals
- How smart lights fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Smart Lights Matters
Understanding smart lights is critical for:
...
## Getting Started
1. Understand the basic smart lights concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Smart Lights — Quick Start Guide
...
## Installation
1. Download or clone the smart lights package
...
cmd_patterns() {
    cat << 'EOF'
# Smart Lights — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for smart lights
...
cmd_performance() {
    cat << 'EOF'
# Smart Lights — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Smart Lights — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Smart Lights — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "smart-lights v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: smart-lights help"; exit 1 ;;
esac
