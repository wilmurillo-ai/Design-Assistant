#!/usr/bin/env bash
# cashflow - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${CASHFLOW_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/cashflow}"
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "cashflow v$VERSION" ;;
