#!/usr/bin/env bash
# finality -- Analyze finality operations. Use when you need to understand finality mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FINALITY_DIR:-$HOME/.finality}"
...
show_help() {
    cat << EOF
finality v$VERSION -- Analyze finality operations. Use when you need to understand finality mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: finality <command> [args]
...
cmd_add() {
    local value="${1:?Usage: finality add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: finality search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: finality remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "finality v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: finality help"; exit 1 ;;
esac
