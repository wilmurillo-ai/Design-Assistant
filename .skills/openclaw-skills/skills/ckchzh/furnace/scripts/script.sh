#!/usr/bin/env bash
# furnace -- Furnace temperature and control manager
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${FURNACE_DIR:-$HOME/.furnace}"
...
show_help() {
    cat << EOF
furnace v$VERSION -- Furnace temperature and control manager

Usage: furnace <command> [args]
...
cmd_add() {
    local value="${1:?Usage: furnace add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: furnace search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: furnace remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "furnace v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: furnace help"; exit 1 ;;
esac
