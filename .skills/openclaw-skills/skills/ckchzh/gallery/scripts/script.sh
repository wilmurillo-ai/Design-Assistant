#!/usr/bin/env bash
# gallery — Gallery reference tool. Use when working with gallery in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
gallery v$VERSION — Gallery Reference Tool
...
## What is Gallery?
Gallery (gallery) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with gallery.

## Key Concepts
- Core gallery principles and fundamentals
- How gallery fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Gallery Matters
Understanding gallery is critical for:
...
## Getting Started
1. Understand the basic gallery concepts
...
## Installation
1. Download or clone the gallery package
...
cmd_patterns() {
    cat << 'EOF'
# Gallery — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for gallery
...
cmd_security() {
    cat << 'EOF'
# Gallery — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Gallery — Migration & Upgrade Guide
