#!/usr/bin/env bash
# loan-calculator - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${LOAN_CALCULATOR_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/loan-calculator}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
loan-calculator v$VERSION

Multi-purpose utility tool

Usage: loan-calculator <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "loan-calculator v$VERSION" ;;
