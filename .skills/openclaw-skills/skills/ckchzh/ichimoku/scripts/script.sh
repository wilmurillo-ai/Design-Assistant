#!/usr/bin/env bash
# ichimoku — Ichimoku reference tool. Use when working with ichimoku in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
ichimoku v$VERSION — Ichimoku Reference Tool

Usage: ichimoku <command>
...
## What is Ichimoku?
Ichimoku (ichimoku) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with ichimoku.

## Key Concepts
- Core ichimoku principles and fundamentals
- How ichimoku fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Ichimoku Matters
Understanding ichimoku is critical for:
...
## Getting Started
1. Understand the basic ichimoku concepts
...
cmd_formulas() {
    cat << 'EOF'
# Ichimoku — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Ichimoku — Instruments & Tools Overview

## Primary Instruments
- Core tools used in ichimoku operations
...
cmd_glossary() {
    cat << 'EOF'
# Ichimoku — Key Terms & Definitions

## Core Terminology
- **Ichimoku**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "ichimoku v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: ichimoku help"; exit 1 ;;
esac
