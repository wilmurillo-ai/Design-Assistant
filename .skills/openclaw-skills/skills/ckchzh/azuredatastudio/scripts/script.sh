#!/usr/bin/env bash
# azuredatastudio - Data processing and analysis toolkit
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${AZUREDATASTUDIO_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/azuredatastudio}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
azuredatastudio v$VERSION

Data processing and analysis toolkit

Usage: azuredatastudio <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "azuredatastudio v$VERSION" ;;
