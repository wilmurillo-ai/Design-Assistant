#!/usr/bin/env bash
# perf-tools — Perf Tools reference tool. Use when working with perf tools in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
perf-tools v$VERSION — Perf Tools Reference Tool

Usage: perf-tools <command>
...
## What is Perf Tools?
Perf Tools (perf-tools) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with perf tools.

## Key Concepts
- Core perf tools principles and fundamentals
- How perf tools fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Perf Tools Matters
Understanding perf tools is critical for:
...
## Getting Started
1. Understand the basic perf tools concepts
...
## Installation
1. Download or clone the perf tools package
...
cmd_patterns() {
    cat << 'EOF'
# Perf Tools — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for perf tools
...
cmd_performance() {
    cat << 'EOF'
# Perf Tools — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Perf Tools — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Perf Tools — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "perf-tools v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: perf-tools help"; exit 1 ;;
esac
