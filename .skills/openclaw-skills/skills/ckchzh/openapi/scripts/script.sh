#!/usr/bin/env bash
# openapi — Openapi reference tool. Use when working with openapi in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
openapi v$VERSION — Openapi Reference Tool

Usage: openapi <command>
...
## What is Openapi?
Openapi (openapi) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with openapi.

## Key Concepts
- Core openapi principles and fundamentals
- How openapi fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Openapi Matters
Understanding openapi is critical for:
...
## Getting Started
1. Understand the basic openapi concepts
...
## Installation
1. Download or clone the openapi package
...
cmd_patterns() {
    cat << 'EOF'
# Openapi — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for openapi
...
cmd_migration() {
    cat << 'EOF'
# Openapi — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "openapi v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: openapi help"; exit 1 ;;
esac
