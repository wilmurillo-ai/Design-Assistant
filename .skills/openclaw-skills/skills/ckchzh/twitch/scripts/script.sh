...
DATA_DIR="${HOME}/.local/share/twitch"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "twitch v2.0.0"; }

_help() {
    echo "Twitch v2.0.0 — gaming toolkit"
    echo ""
    echo "Usage: twitch <command> [args]"
    echo ""
    echo "Commands:"
    echo "  roll               Roll"
    echo "  score              Score"
...
case "${1:-help}" in
    roll)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/roll.log" 2>/dev/null || echo "  No entries yet. Use: twitch roll <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/roll.log"
            local total=$(wc -l < "$DATA_DIR/roll.log")
            echo "  [Twitch] roll: $input"
    ...
    score)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/score.log" 2>/dev/null || echo "  No entries yet. Use: twitch score <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/score.log"
            local total=$(wc -l < "$DATA_DIR/score.log")
            echo "  [Twitch] score: $input"
    ...
    rank)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/rank.log" 2>/dev/null || echo "  No entries yet. Use: twitch rank <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/rank.log"
            local total=$(wc -l < "$DATA_DIR/rank.log")
            echo "  [Twitch] rank: $input"
    ...
    history)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/history.log" 2>/dev/null || echo "  No entries yet. Use: twitch history <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/history.log"
            local total=$(wc -l < "$DATA_DIR/history.log")
            echo "  [Twitch] history: $input"
    ...
    stats)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/stats.log" 2>/dev/null || echo "  No entries yet. Use: twitch stats <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/stats.log"
            local total=$(wc -l < "$DATA_DIR/stats.log")
            echo "  [Twitch] stats: $input"
    ...
    challenge)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/challenge.log" 2>/dev/null || echo "  No entries yet. Use: twitch challenge <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/challenge.log"
            local total=$(wc -l < "$DATA_DIR/challenge.log")
            echo "  [Twitch] challenge: $input"
    ...
    create)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/create.log" 2>/dev/null || echo "  No entries yet. Use: twitch create <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/create.log"
            local total=$(wc -l < "$DATA_DIR/create.log")
            echo "  [Twitch] create: $input"
    ...
    join)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/join.log" 2>/dev/null || echo "  No entries yet. Use: twitch join <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/join.log"
            local total=$(wc -l < "$DATA_DIR/join.log")
            echo "  [Twitch] join: $input"
    ...
    track)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/track.log" 2>/dev/null || echo "  No entries yet. Use: twitch track <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/track.log"
            local total=$(wc -l < "$DATA_DIR/track.log")
            echo "  [Twitch] track: $input"
    ...
    leaderboard)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/leaderboard.log" 2>/dev/null || echo "  No entries yet. Use: twitch leaderboard <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/leaderboard.log"
            local total=$(wc -l < "$DATA_DIR/leaderboard.log")
            echo "  [Twitch] leaderboard: $input"
    ...
    reward)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/reward.log" 2>/dev/null || echo "  No entries yet. Use: twitch reward <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/reward.log"
            local total=$(wc -l < "$DATA_DIR/reward.log")
            echo "  [Twitch] reward: $input"
    ...
    reset)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/reset.log" 2>/dev/null || echo "  No entries yet. Use: twitch reset <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/reset.log"
            local total=$(wc -l < "$DATA_DIR/reset.log")
            echo "  [Twitch] reset: $input"
    ...
    *)
        echo "Unknown: $1 — run 'twitch help'"
        exit 1
        ;;
esac