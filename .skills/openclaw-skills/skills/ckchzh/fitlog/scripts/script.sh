...
_version() { echo "fitlog v2.0.0"; }

_help() {
    echo "Fitlog v2.0.0 — productivity toolkit"
    echo ""
    echo "Usage: fitlog <command> [args]"
    echo ""
    echo "Commands:"
    echo "  add                Add"
    echo "  plan               Plan"
...
_search() {
    local term="${1:?Usage: fitlog search <term>}"
...
case "${1:-help}" in
    ...
    plan)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/plan.log" 2>/dev/null || echo "  No entries yet. Use: fitlog plan <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/plan.log"
            local total=$(wc -l < "$DATA_DIR/plan.log")
            echo "  [Fitlog] plan: $input"
    ...
    track)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/track.log" 2>/dev/null || echo "  No entries yet. Use: fitlog track <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/track.log"
            local total=$(wc -l < "$DATA_DIR/track.log")
            echo "  [Fitlog] track: $input"
    ...
    review)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/review.log" 2>/dev/null || echo "  No entries yet. Use: fitlog review <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/review.log"
            local total=$(wc -l < "$DATA_DIR/review.log")
            echo "  [Fitlog] review: $input"
    ...
    streak)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/streak.log" 2>/dev/null || echo "  No entries yet. Use: fitlog streak <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/streak.log"
            local total=$(wc -l < "$DATA_DIR/streak.log")
            echo "  [Fitlog] streak: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: fitlog remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Fitlog] remind: $input"
    ...
    prioritize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/prioritize.log" 2>/dev/null || echo "  No entries yet. Use: fitlog prioritize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/prioritize.log"
            local total=$(wc -l < "$DATA_DIR/prioritize.log")
            echo "  [Fitlog] prioritize: $input"
    ...
    archive)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/archive.log" 2>/dev/null || echo "  No entries yet. Use: fitlog archive <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/archive.log"
            local total=$(wc -l < "$DATA_DIR/archive.log")
            echo "  [Fitlog] archive: $input"
    ...
    tag)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tag.log" 2>/dev/null || echo "  No entries yet. Use: fitlog tag <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tag.log"
            local total=$(wc -l < "$DATA_DIR/tag.log")
            echo "  [Fitlog] tag: $input"
    ...
    timeline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/timeline.log" 2>/dev/null || echo "  No entries yet. Use: fitlog timeline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/timeline.log"
            local total=$(wc -l < "$DATA_DIR/timeline.log")
            echo "  [Fitlog] timeline: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: fitlog report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Fitlog] report: $input"
    ...
    weekly-review)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/weekly-review.log" 2>/dev/null || echo "  No entries yet. Use: fitlog weekly-review <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/weekly-review.log"
            local total=$(wc -l < "$DATA_DIR/weekly-review.log")
            echo "  [Fitlog] weekly-review: $input"
    ...
    *)
        ...
        echo "Run 'fitlog help' for available commands."
        exit 1
        ;;
esac