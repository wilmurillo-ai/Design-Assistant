#!/usr/bin/env bash
# Dailylog — productivity tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

DATA_DIR="${HOME}/.local/share/dailylog"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }

_version() { echo "dailylog v2.0.0"; }

_help() {
    echo "Dailylog v2.0.0 — productivity toolkit"
    echo ""
    echo "Usage: dailylog <command> [args]"
    echo ""
    echo "Commands:"
    echo "  add                Add"
    echo "  plan               Plan"
...
_search() {
    local term="${1:?Usage: dailylog search <term>}"
...
case "${1:-help}" in
    add)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/add.log" 2>/dev/null || echo "  No entries yet. Use: dailylog add <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/add.log"
            local total=$(wc -l < "$DATA_DIR/add.log")
            echo "  [Dailylog] add: $input"
    ...
    plan)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/plan.log" 2>/dev/null || echo "  No entries yet. Use: dailylog plan <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/plan.log"
            local total=$(wc -l < "$DATA_DIR/plan.log")
            echo "  [Dailylog] plan: $input"
    ...
    track)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/track.log" 2>/dev/null || echo "  No entries yet. Use: dailylog track <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/track.log"
            local total=$(wc -l < "$DATA_DIR/track.log")
            echo "  [Dailylog] track: $input"
    ...
    review)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/review.log" 2>/dev/null || echo "  No entries yet. Use: dailylog review <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/review.log"
            local total=$(wc -l < "$DATA_DIR/review.log")
            echo "  [Dailylog] review: $input"
    ...
    streak)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/streak.log" 2>/dev/null || echo "  No entries yet. Use: dailylog streak <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/streak.log"
            local total=$(wc -l < "$DATA_DIR/streak.log")
            echo "  [Dailylog] streak: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: dailylog remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Dailylog] remind: $input"
    ...
    prioritize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/prioritize.log" 2>/dev/null || echo "  No entries yet. Use: dailylog prioritize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/prioritize.log"
            local total=$(wc -l < "$DATA_DIR/prioritize.log")
            echo "  [Dailylog] prioritize: $input"
    ...
    archive)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/archive.log" 2>/dev/null || echo "  No entries yet. Use: dailylog archive <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/archive.log"
            local total=$(wc -l < "$DATA_DIR/archive.log")
            echo "  [Dailylog] archive: $input"
    ...
    tag)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tag.log" 2>/dev/null || echo "  No entries yet. Use: dailylog tag <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tag.log"
            local total=$(wc -l < "$DATA_DIR/tag.log")
            echo "  [Dailylog] tag: $input"
    ...
    timeline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/timeline.log" 2>/dev/null || echo "  No entries yet. Use: dailylog timeline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/timeline.log"
            local total=$(wc -l < "$DATA_DIR/timeline.log")
            echo "  [Dailylog] timeline: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: dailylog report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Dailylog] report: $input"
    ...
    weekly-review)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/weekly-review.log" 2>/dev/null || echo "  No entries yet. Use: dailylog weekly-review <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/weekly-review.log"
            local total=$(wc -l < "$DATA_DIR/weekly-review.log")
            echo "  [Dailylog] weekly-review: $input"
    ...
    *)
        ...
        echo "Run 'dailylog help' for available commands."
        exit 1
        ;;
esac