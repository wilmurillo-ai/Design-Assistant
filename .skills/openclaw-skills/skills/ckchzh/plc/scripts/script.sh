#!/usr/bin/env bash
# plc -- Programmable logic controller programming helper
...
show_help() {
    cat << EOF
plc v$VERSION -- Programmable logic controller programming helper
...
cmd_add() {
    local value="${1:?Usage: plc add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: plc search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: plc remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: plc help"; exit 1 ;;
esac
