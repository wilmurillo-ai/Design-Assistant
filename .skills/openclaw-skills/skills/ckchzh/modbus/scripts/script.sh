#!/usr/bin/env bash
# modbus -- Modbus communication protocol tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MODBUS_DIR:-$HOME/.modbus}"
...
show_help() {
    cat << EOF
modbus v$VERSION -- Modbus communication protocol tool
...
cmd_add() {
    local value="${1:?Usage: modbus add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: modbus search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: modbus remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "modbus v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: modbus help"; exit 1 ;;
esac
