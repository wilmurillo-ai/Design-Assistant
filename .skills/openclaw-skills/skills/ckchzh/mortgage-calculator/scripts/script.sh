#!/usr/bin/env bash
# mortgage-calculator - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${MORTGAGE_CALCULATOR_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/mortgage-calculator}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
mortgage-calculator v$VERSION

Multi-purpose utility tool

Usage: mortgage-calculator <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "mortgage-calculator v$VERSION" ;;
