#!/usr/bin/env bash
# prophet — Prophet reference tool. Use when working with prophet in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
prophet v$VERSION — Prophet Reference Tool

Usage: prophet <command>
...
## What is Prophet?
Prophet (prophet) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with prophet.

## Key Concepts
- Core prophet principles and fundamentals
- How prophet fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Prophet Matters
Understanding prophet is critical for:
...
## Getting Started
1. Understand the basic prophet concepts
...
## Installation
1. Download or clone the prophet package
...
cmd_patterns() {
    cat << 'EOF'
# Prophet — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for prophet
...
cmd_performance() {
    cat << 'EOF'
# Prophet — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Prophet — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Prophet — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "prophet v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: prophet help"; exit 1 ;;
esac
