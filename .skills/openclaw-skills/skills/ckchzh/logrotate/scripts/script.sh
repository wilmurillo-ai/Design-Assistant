#!/usr/bin/env bash
# logrotate — Logrotate reference tool. Use when working with logrotate in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
logrotate v$VERSION — Logrotate Reference Tool

Usage: logrotate <command>
...
## What is Logrotate?
Logrotate (logrotate) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with logrotate.

## Key Concepts
- Core logrotate principles and fundamentals
- How logrotate fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Logrotate Matters
Understanding logrotate is critical for:
...
## Getting Started
1. Understand the basic logrotate concepts
...
## Installation
1. Download or clone the logrotate package
...
cmd_patterns() {
    cat << 'EOF'
# Logrotate — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for logrotate
...
cmd_performance() {
    cat << 'EOF'
# Logrotate — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Logrotate — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Logrotate — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "logrotate v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: logrotate help"; exit 1 ;;
esac
