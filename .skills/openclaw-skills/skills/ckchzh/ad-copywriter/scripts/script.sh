#!/usr/bin/env bash
# ad-copywriter - Content creation and optimization assistant
set -euo pipefail
VERSION="2.0.0"
...
show_help() {
    cat << EOF
ad-copywriter v$VERSION

Content creation and optimization assistant

Usage: ad-copywriter <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "ad-copywriter v$VERSION" ;;
