#!/usr/bin/env bash
# linea — Linea reference tool. Use when working with linea in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
linea v$VERSION — Linea Reference Tool
...
## What is Linea?
Linea (linea) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with linea.

## Key Concepts
- Core linea principles and fundamentals
- How linea fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Linea Matters
Understanding linea is critical for:
...
## Getting Started
1. Understand the basic linea concepts
...
cmd_formulas() {
    cat << 'EOF'
# Linea — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Linea — Instruments & Tools Overview

## Primary Instruments
- Core tools used in linea operations
...
cmd_glossary() {
    cat << 'EOF'
# Linea — Key Terms & Definitions

## Core Terminology
- **Linea**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "linea v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: linea help"; exit 1 ;;
esac
