#!/usr/bin/env bash
# bearing -- Bearing selection and life calculator
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BEARING_DIR:-$HOME/.bearing}"
...
show_help() {
    cat << EOF
bearing v$VERSION -- Bearing selection and life calculator

Usage: bearing <command> [args]
...
cmd_add() {
    local value="${1:?Usage: bearing add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: bearing search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: bearing remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "bearing v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: bearing help"; exit 1 ;;
esac
