#!/usr/bin/env bash
# substrate — Substrate reference tool. Use when working with substrate in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
substrate v$VERSION — Substrate Reference Tool

Usage: substrate <command>
...
## What is Substrate?
Substrate (substrate) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with substrate.

## Key Concepts
- Core substrate principles and fundamentals
- How substrate fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Substrate Matters
Understanding substrate is critical for:
...
## Getting Started
1. Understand the basic substrate concepts
...
cmd_formulas() {
    cat << 'EOF'
# Substrate — Key Formulas & Calculations
...
cmd_regulations() {
    cat << 'EOF'
# Substrate — Regulatory Framework
...
cmd_instruments() {
    cat << 'EOF'
# Substrate — Instruments & Tools Overview

## Primary Instruments
- Core tools used in substrate operations
...
cmd_glossary() {
    cat << 'EOF'
# Substrate — Key Terms & Definitions

## Core Terminology
- **Substrate**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "substrate v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: substrate help"; exit 1 ;;
esac
