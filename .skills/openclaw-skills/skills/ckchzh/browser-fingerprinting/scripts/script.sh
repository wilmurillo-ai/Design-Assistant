#!/usr/bin/env bash
# browser-fingerprinting - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${BROWSER_FINGERPRINTING_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/browser-fingerprinting}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
browser-fingerprinting v$VERSION

Multi-purpose utility tool

Usage: browser-fingerprinting <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "browser-fingerprinting v$VERSION" ;;
