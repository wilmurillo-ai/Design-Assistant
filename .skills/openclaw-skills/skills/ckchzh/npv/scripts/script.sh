#!/usr/bin/env bash
# npv — Npv reference tool. Use when working with npv in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
npv v$VERSION — Npv Reference Tool
...
## What is Npv?
Npv (npv) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with npv.

## Key Concepts
- Core npv principles and fundamentals
- How npv fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Npv Matters
Understanding npv is critical for:
...
## Getting Started
1. Understand the basic npv concepts
...
cmd_instruments() {
    cat << 'EOF'
# Npv — Instruments & Tools Overview

## Primary Instruments
- Core tools used in npv operations
...
cmd_glossary() {
    cat << 'EOF'
# Npv — Key Terms & Definitions

## Core Terminology
- **Npv**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "npv v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: npv help"; exit 1 ;;
esac
