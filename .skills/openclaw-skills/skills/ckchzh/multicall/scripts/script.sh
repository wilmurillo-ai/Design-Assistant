#!/usr/bin/env bash
# multicall -- Analyze multicall operations. Use when you need to understand multicall mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MULTICALL_DIR:-$HOME/.multicall}"
...
show_help() {
    cat << EOF
multicall v$VERSION -- Analyze multicall operations. Use when you need to understand multicall mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: multicall <command> [args]
...
cmd_add() {
    local value="${1:?Usage: multicall add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: multicall search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: multicall remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="multicall-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "multicall v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: multicall help"; exit 1 ;;
esac
