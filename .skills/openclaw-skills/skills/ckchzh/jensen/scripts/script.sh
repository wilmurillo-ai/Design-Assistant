#!/usr/bin/env bash
# jensen — Jensen reference tool. Use when working with jensen in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
jensen v$VERSION — Jensen Reference Tool

Usage: jensen <command>
...
## What is Jensen?
Jensen (jensen) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with jensen.

## Key Concepts
- Core jensen principles and fundamentals
- How jensen fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Jensen Matters
Understanding jensen is critical for:
...
## Getting Started
1. Understand the basic jensen concepts
...
cmd_formulas() {
    cat << 'EOF'
# Jensen — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Jensen — Instruments & Tools Overview

## Primary Instruments
- Core tools used in jensen operations
...
cmd_glossary() {
    cat << 'EOF'
# Jensen — Key Terms & Definitions

## Core Terminology
- **Jensen**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "jensen v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: jensen help"; exit 1 ;;
esac
