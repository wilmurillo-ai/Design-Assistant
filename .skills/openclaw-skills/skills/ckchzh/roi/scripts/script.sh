#!/usr/bin/env bash
# roi — Roi reference tool. Use when working with roi in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
roi v$VERSION — Roi Reference Tool
...
## What is Roi?
Roi (roi) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with roi.

## Key Concepts
- Core roi principles and fundamentals
- How roi fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Roi Matters
Understanding roi is critical for:
...
## Getting Started
1. Understand the basic roi concepts
...
## Installation
1. Download or clone the roi package
...
cmd_patterns() {
    cat << 'EOF'
# Roi — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for roi
