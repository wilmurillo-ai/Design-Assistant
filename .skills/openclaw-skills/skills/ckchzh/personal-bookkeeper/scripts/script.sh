#!/usr/bin/env bash
# Personal Bookkeeper — finance tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

DATA_DIR="${HOME}/.local/share/personal-bookkeeper"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }

_version() { echo "personal-bookkeeper v2.0.0"; }

_help() {
    echo "Personal Bookkeeper v2.0.0 — finance toolkit"
    echo ""
    echo "Usage: personal-bookkeeper <command> [args]"
    echo ""
    echo "Commands:"
    echo "  record             Record"
...
_stats() {
    echo "=== Personal Bookkeeper Stats ==="
    local total=0
...
_export() {
    ...
    case "$fmt" in
        ...
        csv)
            ...
            ;;
        txt)
            echo "=== Personal Bookkeeper Export ===" > "$out"
            for f in "$DATA_DIR"/*.log; do
                [ -f "$f" ] || continue
...
_status() {
    echo "=== Personal Bookkeeper Status ==="
    echo "  Version: v2.0.0"
...
_search() {
    local term="${1:?Usage: personal-bookkeeper search <term>}"
...
case "${1:-help}" in
    record)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/record.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper record <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/record.log"
            local total=$(wc -l < "$DATA_DIR/record.log")
            echo "  [Personal Bookkeeper] record: $input"
    ...
    categorize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/categorize.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper categorize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/categorize.log"
            local total=$(wc -l < "$DATA_DIR/categorize.log")
            echo "  [Personal Bookkeeper] categorize: $input"
    ...
    balance)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/balance.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper balance <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/balance.log"
            local total=$(wc -l < "$DATA_DIR/balance.log")
            echo "  [Personal Bookkeeper] balance: $input"
    ...
    trend)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/trend.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper trend <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/trend.log"
            local total=$(wc -l < "$DATA_DIR/trend.log")
            echo "  [Personal Bookkeeper] trend: $input"
    ...
    forecast)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/forecast.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper forecast <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/forecast.log"
            local total=$(wc -l < "$DATA_DIR/forecast.log")
            echo "  [Personal Bookkeeper] forecast: $input"
    ...
    export-report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export-report.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper export-report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export-report.log"
            local total=$(wc -l < "$DATA_DIR/export-report.log")
            echo "  [Personal Bookkeeper] export-report: $input"
    ...
    budget-check)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/budget-check.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper budget-check <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/budget-check.log"
            local total=$(wc -l < "$DATA_DIR/budget-check.log")
            echo "  [Personal Bookkeeper] budget-check: $input"
    ...
    summary)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/summary.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper summary <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/summary.log"
            local total=$(wc -l < "$DATA_DIR/summary.log")
            echo "  [Personal Bookkeeper] summary: $input"
    ...
    alert)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/alert.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper alert <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/alert.log"
            local total=$(wc -l < "$DATA_DIR/alert.log")
            echo "  [Personal Bookkeeper] alert: $input"
    ...
    history)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/history.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper history <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/history.log"
            local total=$(wc -l < "$DATA_DIR/history.log")
            echo "  [Personal Bookkeeper] history: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Personal Bookkeeper] compare: $input"
    ...
    tax-note)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tax-note.log" 2>/dev/null || echo "  No entries yet. Use: personal-bookkeeper tax-note <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tax-note.log"
            local total=$(wc -l < "$DATA_DIR/tax-note.log")
            echo "  [Personal Bookkeeper] tax-note: $input"
    ...
    *)
        ...
        echo "Run 'personal-bookkeeper help' for available commands."
        exit 1
        ;;
esac