#!/usr/bin/env bash
# cnc -- CNC machining program manager
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CNC_DIR:-$HOME/.cnc}"
...
show_help() {
    cat << EOF
cnc v$VERSION -- CNC machining program manager

Usage: cnc <command> [args]
...
cmd_add() {
    local value="${1:?Usage: cnc add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: cnc search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: cnc remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "cnc v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: cnc help"; exit 1 ;;
esac
