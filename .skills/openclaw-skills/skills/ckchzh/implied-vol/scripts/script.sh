#!/usr/bin/env bash
# implied-vol — Implied Vol reference tool. Use when working with implied vol in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
implied-vol v$VERSION — Implied Vol Reference Tool

Usage: implied-vol <command>
...
cmd_intro() {
    cat << 'EOF'
# Implied Vol — Overview

## What is Implied Vol?
Implied Vol (implied-vol) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with implied vol.

## Key Concepts
- Core implied vol principles and fundamentals
- How implied vol fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Implied Vol Matters
Understanding implied vol is critical for:
...
## Getting Started
1. Understand the basic implied vol concepts
...
cmd_formulas() {
    cat << 'EOF'
# Implied Vol — Key Formulas & Calculations
...
cmd_regulations() {
    cat << 'EOF'
# Implied Vol — Regulatory Framework
...
cmd_instruments() {
    cat << 'EOF'
# Implied Vol — Instruments & Tools Overview

## Primary Instruments
- Core tools used in implied vol operations
...
cmd_glossary() {
    cat << 'EOF'
# Implied Vol — Key Terms & Definitions

## Core Terminology
- **Implied Vol**: The primary subject of this reference
...
cmd_checklist() {
    cat << 'EOF'
# Implied Vol — Inspection Checklist
...
case "$CMD" in
    ...
    version|--version|-v) echo "implied-vol v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: implied-vol help"; exit 1 ;;
esac
