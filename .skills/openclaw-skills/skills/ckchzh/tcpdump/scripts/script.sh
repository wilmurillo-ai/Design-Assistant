#!/usr/bin/env bash
# tcpdump — Tcpdump reference tool. Use when working with tcpdump in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
tcpdump v$VERSION — Tcpdump Reference Tool

Usage: tcpdump <command>
...
## What is Tcpdump?
Tcpdump (tcpdump) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with tcpdump.

## Key Concepts
- Core tcpdump principles and fundamentals
- How tcpdump fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Tcpdump Matters
Understanding tcpdump is critical for:
...
## Getting Started
1. Understand the basic tcpdump concepts
...
## Installation
1. Download or clone the tcpdump package
...
cmd_patterns() {
    cat << 'EOF'
# Tcpdump — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for tcpdump
...
cmd_performance() {
    cat << 'EOF'
# Tcpdump — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Tcpdump — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Tcpdump — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "tcpdump v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: tcpdump help"; exit 1 ;;
esac
