#!/usr/bin/env bash
# performance-review - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
...
show_help() {
    cat << EOF
performance-review v$VERSION

Multi-purpose utility tool

Usage: performance-review <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "performance-review v$VERSION" ;;
