#!/usr/bin/env bash
# perf-analyzer — Perf Analyzer reference tool. Use when working with perf analyzer in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
perf-analyzer v$VERSION — Perf Analyzer Reference Tool

Usage: perf-analyzer <command>
...
cmd_intro() {
    cat << 'EOF'
# Perf Analyzer — Overview

## What is Perf Analyzer?
Perf Analyzer (perf-analyzer) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with perf analyzer.

## Key Concepts
- Core perf analyzer principles and fundamentals
- How perf analyzer fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Perf Analyzer Matters
Understanding perf analyzer is critical for:
...
## Getting Started
1. Understand the basic perf analyzer concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Perf Analyzer — Quick Start Guide
...
## Installation
1. Download or clone the perf analyzer package
...
cmd_patterns() {
    cat << 'EOF'
# Perf Analyzer — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for perf analyzer
...
cmd_security() {
    cat << 'EOF'
# Perf Analyzer — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Perf Analyzer — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "perf-analyzer v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: perf-analyzer help"; exit 1 ;;
esac
