#!/usr/bin/env bash
# cobot -- Collaborative robot task planner
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${COBOT_DIR:-$HOME/.cobot}"
...
show_help() {
    cat << EOF
cobot v$VERSION -- Collaborative robot task planner

Usage: cobot <command> [args]
...
cmd_add() {
    local value="${1:?Usage: cobot add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: cobot search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: cobot remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "cobot v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: cobot help"; exit 1 ;;
esac
