#!/usr/bin/env bash
# actuator -- Actuator selection and sizing calculator
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ACTUATOR_DIR:-$HOME/.actuator}"
...
show_help() {
    cat << EOF
actuator v$VERSION -- Actuator selection and sizing calculator

Usage: actuator <command> [args]
...
cmd_add() {
    local value="${1:?Usage: actuator add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: actuator search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: actuator remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "actuator v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: actuator help"; exit 1 ;;
esac
