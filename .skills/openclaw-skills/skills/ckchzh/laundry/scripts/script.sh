...
DATA_DIR="${HOME}/.local/share/laundry"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "laundry v2.0.0"; }

_help() {
    echo "Laundry v2.0.0 — home toolkit"
    echo ""
    echo "Usage: laundry <command> [args]"
    echo ""
    echo "Commands:"
    echo "  add                Add"
...
case "${1:-help}" in
    add)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/add.log" 2>/dev/null || echo "  No entries yet. Use: laundry add <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/add.log"
            local total=$(wc -l < "$DATA_DIR/add.log")
            echo "  [Laundry] add: $input"
    ...
    inventory)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/inventory.log" 2>/dev/null || echo "  No entries yet. Use: laundry inventory <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/inventory.log"
            local total=$(wc -l < "$DATA_DIR/inventory.log")
            echo "  [Laundry] inventory: $input"
    ...
    schedule)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/schedule.log" 2>/dev/null || echo "  No entries yet. Use: laundry schedule <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/schedule.log"
            local total=$(wc -l < "$DATA_DIR/schedule.log")
            echo "  [Laundry] schedule: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: laundry remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Laundry] remind: $input"
    ...
    checklist)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/checklist.log" 2>/dev/null || echo "  No entries yet. Use: laundry checklist <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/checklist.log"
            local total=$(wc -l < "$DATA_DIR/checklist.log")
            echo "  [Laundry] checklist: $input"
    ...
    usage)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/usage.log" 2>/dev/null || echo "  No entries yet. Use: laundry usage <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/usage.log"
            local total=$(wc -l < "$DATA_DIR/usage.log")
            echo "  [Laundry] usage: $input"
    ...
    cost)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cost.log" 2>/dev/null || echo "  No entries yet. Use: laundry cost <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/cost.log"
            local total=$(wc -l < "$DATA_DIR/cost.log")
            echo "  [Laundry] cost: $input"
    ...
    maintain)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/maintain.log" 2>/dev/null || echo "  No entries yet. Use: laundry maintain <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/maintain.log"
            local total=$(wc -l < "$DATA_DIR/maintain.log")
            echo "  [Laundry] maintain: $input"
    ...
    log)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/log.log" 2>/dev/null || echo "  No entries yet. Use: laundry log <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/log.log"
            local total=$(wc -l < "$DATA_DIR/log.log")
            echo "  [Laundry] log: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: laundry report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Laundry] report: $input"
    ...
    seasonal)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/seasonal.log" 2>/dev/null || echo "  No entries yet. Use: laundry seasonal <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/seasonal.log"
            local total=$(wc -l < "$DATA_DIR/seasonal.log")
            echo "  [Laundry] seasonal: $input"
    ...
    tips)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tips.log" 2>/dev/null || echo "  No entries yet. Use: laundry tips <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tips.log"
            local total=$(wc -l < "$DATA_DIR/tips.log")
            echo "  [Laundry] tips: $input"
    ...
    *)
        echo "Unknown: $1 — run 'laundry help'"
        exit 1
        ;;
esac