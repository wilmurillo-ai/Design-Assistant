#!/usr/bin/env bash
# terms — Terms reference tool. Use when working with terms in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
terms v$VERSION — Terms Reference Tool
...
## What is Terms?
Terms (terms) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with terms.

## Key Concepts
- Core terms principles and fundamentals
- How terms fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Terms Matters
Understanding terms is critical for:
...
## Getting Started
1. Understand the basic terms concepts
...
## Installation
1. Download or clone the terms package
...
cmd_patterns() {
    cat << 'EOF'
# Terms — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for terms
...
case "$CMD" in
    ...
    version|--version|-v) echo "terms v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: terms help"; exit 1 ;;
esac
