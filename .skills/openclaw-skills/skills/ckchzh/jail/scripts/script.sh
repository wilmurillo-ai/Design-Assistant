#!/usr/bin/env bash
# jail — Jail reference tool. Use when working with jail in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
jail v$VERSION — Jail Reference Tool
...
## What is Jail?
Jail (jail) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with jail.

## Key Concepts
- Core jail principles and fundamentals
- How jail fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Jail Matters
Understanding jail is critical for:
...
## Getting Started
1. Understand the basic jail concepts
...
## Installation
1. Download or clone the jail package
...
cmd_patterns() {
    cat << 'EOF'
# Jail — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for jail
...
case "$CMD" in
    ...
    version|--version|-v) echo "jail v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: jail help"; exit 1 ;;
esac
