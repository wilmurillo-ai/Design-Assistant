#!/usr/bin/env bash
# swagger — Swagger reference tool. Use when working with swagger in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
swagger v$VERSION — Swagger Reference Tool

Usage: swagger <command>
...
## What is Swagger?
Swagger (swagger) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with swagger.

## Key Concepts
- Core swagger principles and fundamentals
- How swagger fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Swagger Matters
Understanding swagger is critical for:
...
## Getting Started
1. Understand the basic swagger concepts
...
## Installation
1. Download or clone the swagger package
...
cmd_patterns() {
    cat << 'EOF'
# Swagger — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for swagger
...
cmd_performance() {
    cat << 'EOF'
# Swagger — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Swagger — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Swagger — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "swagger v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: swagger help"; exit 1 ;;
esac
