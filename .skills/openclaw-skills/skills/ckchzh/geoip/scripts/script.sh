#!/usr/bin/env bash
# geoip — Geoip reference tool. Use when working with geoip in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
geoip v$VERSION — Geoip Reference Tool
...
## What is Geoip?
Geoip (geoip) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with geoip.

## Key Concepts
- Core geoip principles and fundamentals
- How geoip fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Geoip Matters
Understanding geoip is critical for:
...
## Getting Started
1. Understand the basic geoip concepts
...
## Installation
1. Download or clone the geoip package
...
cmd_patterns() {
    cat << 'EOF'
# Geoip — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for geoip
...
case "$CMD" in
    ...
    version|--version|-v) echo "geoip v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: geoip help"; exit 1 ;;
esac
