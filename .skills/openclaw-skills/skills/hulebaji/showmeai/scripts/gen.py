#!/usr/bin/env python3
"""
Showmeai Image Generation & Editing Script
...
def request_edit(
    ...
    boundary = "----ShowmeaiBoundary" + os.urandom(8).hex()
    parts = []
...
def main():
    ap = argparse.ArgumentParser(description="Generate or edit images via Showmeai API.")
    ...
    if not api_key:
        print("Error: Showmeai_API_KEY not set.", file=sys.stderr); sys.exit(1)
    if not base_url:
