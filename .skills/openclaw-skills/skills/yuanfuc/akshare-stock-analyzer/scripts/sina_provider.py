...
def _normalize_date_ymd(date_str: str) -> str:
	"""接受 YYYYMMDD 或 YYYY-MM-DD，统一转成 YYYYMMDD 字符串（新浪接口需要）。"""
...
def _to_sina_symbol(symbol_or_name: str) -> str:
	"""将代码或名称统一转换为新浪接口需要的 sh/sz/bj 前缀代码。"""
...
def fetch_stock_daily_sina(
	...
	"""通用新浪日线原始数据接口（不含任何技术指标）。

	对 ak.stock_zh_a_daily 进行轻量封装，统一输入和输出格式，便于和腾讯版本互换。

	:param symbol_or_name: 股票代码(可带不带市场前缀)或股票名称，例如 "600036"、"sh600036"、"招商银行"
	...
	"""
	symbol = _to_sina_symbol(symbol_or_name)
	start_str = _normalize_date_ymd(start_date)
	end_str = _normalize_date_ymd(end_date)

	print(f"调用新浪接口获取数据: {symbol}, {start_str} -> {end_str} ...")
	...
	# 新浪接口返回列通常为: ["date", "open", "high", "low", "close", "volume", ...]
	# 为了与我们的脚本保持一致，只抽取并重命名成统一的中文 OHLCV 列
	col_map = {}
	...
	if "low" in df.columns:
		col_map["low"] = "最低"
	# 成交量字段有的叫 volume，有的可能叫成交量，这里做个兼容
	vol_col = None
	...
	base_cols = [c for c in ["date", "open", "close", "high", "low"] if c in df.columns]
	...
	# 确保列顺序为我们习惯的顺序
	ordered_cols = ["日期", "开盘", "收盘", "最高", "最低", "成交量"]
	result = result[[c for c in ordered_cols if c in result.columns]]
	return result


def fetch_a500_daily_sina(
	symbol: str = "sh600036",
	days: int = 120,
	adjust: str = "",
) -> pd.DataFrame:
	"""基于新浪的 A 股日线原始数据接口示例（默认示例用招商银行 600036）。"""
...
def main():
	"""命令行入口。
	 ...
	 用法示例：
		1）通用拉取：
			python xinlangdatafetch.py -s sh600036 -b 20250101 -e 20250201 -a qfq

		2）仅指定代码，回溯 N 天（默认最近 120 天）：
			python xinlangdatafetch.py -s sh600036 -d 60
	"""

	import argparse

	parser = argparse.ArgumentParser(description="新浪日线原始数据获取脚本")
	parser.add_argument(
		"-s",
		"--symbol",
		help="股票代码或名称，例如 600036、sh600036、招商银行，必填",
	)
	parser.add_argument(
		"-b",
		"--start",
	...
	if args.symbol and args.start and args.end:
		...
		print(
			f"正在获取 {symbol_label} 日线原始数据(新浪): {args.start} -> {args.end}, adjust={args.adjust} ..."
		)
		try:
	...
	# 情况二：未提供完整起止日期，走“最近 N 天”逻辑
		...
		symbol_label = args.symbol
		print(
			f"正在获取 {symbol_label} 最近 {args.days} 天日线原始数据(新浪), adjust={args.adjust} ..."
		)
		try:
			df = fetch_a500_daily_sina(symbol=symbol_label, days=args.days, adjust=args.adjust)
		except Exception as e:
			print(f"获取数据时出错: {e}")
			import traceback
	...
	print("\n" + "=" * 70)
	print(f"日线原始数据预览 (新浪, {symbol_label})")
	print("=" * 70)
