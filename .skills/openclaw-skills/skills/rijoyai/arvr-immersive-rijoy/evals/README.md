...
- **skill_name**: Must match the skill’s frontmatter `name` (e.g. `arvr-immersive-rijoy`).
...
Results go in a **sibling** directory to the skill: `arvr-immersive-rijoy-workspace/`. Do not create the whole tree upfront; create directories as you run evals.
...
4. **Aggregate**: From the skill-creator directory:  
   `python -m scripts.aggregate_benchmark <workspace>/iteration-N --skill-name arvr-immersive-rijoy`  
   Produces `benchmark.json` and `benchmark.md` in that iteration directory.
5. **Viewer**: Use skill-creator’s `eval-viewer/generate_review.py` with that iteration path and `--skill-name arvr-immersive-rijoy` (and `--benchmark` path, `--previous-workspace` for iteration 2+). Use `--static` for headless environments to get a standalone HTML file.
