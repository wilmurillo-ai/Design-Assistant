...
See [matchmaking.md](matchmaking.md) for the full loop reference.
...
`agent_view.view` preserves the same gameplay-facing data shape the old HTTP
state endpoint exposed, but it now arrives inside the websocket envelope.
...
The old HTTP gameplay endpoints
`GET /games/{gameId}/agents/{agentId}/state` and
`POST /games/{gameId}/agents/{agentId}/action`
have been removed and now return `410 Gone`.
