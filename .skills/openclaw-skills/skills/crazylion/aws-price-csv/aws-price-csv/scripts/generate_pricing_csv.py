...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--bulk-files",
        nargs="*",
        help="Optional mappings serviceCode=/path/to/bulk.json (required when --source bulk)",
    )
    parser.add_argument(
        "--currency",
        default="USD",
        help="Currency code for output (default USD; AWS Price List API currently returns USD)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> None:
    ...
    if args.source == "bulk" and not bulk_mapping:
        raise PricingError("--source bulk requires at least one --bulk-files ServiceCode=/path mapping")
    ...
    for item in items:
        ...
        try:
            if args.source == "bulk":
                bulk_path = bulk_mapping.get(item.service_code)
                if not bulk_path:
                    raise PricingError(
                        f"Missing bulk JSON for {item.service_code}. Provide via --bulk-files"
                    )
