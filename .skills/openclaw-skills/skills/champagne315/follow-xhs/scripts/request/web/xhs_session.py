...
async def create_xhs_session(
    ...
    Args:
        web_session: 小红书web_session cookie值。如果未提供，将自动从config.json读取
        proxy: 代理地址
        did: 设备ID
        sid: 会话ID
    ...
    if web_session is None:
        ...
        if web_session is None:
            logger.warning("未配置web_session！请在config.json中配置web_session.value，或手动传入web_session参数")
        else:
            logger.info("从配置文件加载web_session成功")
