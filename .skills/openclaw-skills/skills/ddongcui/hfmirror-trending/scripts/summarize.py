...
def fetch_trending_data(url=TRENDING_API_URL):
    try:
        ...
        headers = {'User-Agent': 'hfmirror-trending-skill/1.0'}
...
def summarize_trending(data, md_path=None):
    ...
    output = []
    output.append("# Hugging Face 热门资源深度解析报告\n")
    output.append("---")
    
    # Models Table
    output.append("\n## 一、 热门模型 (Trending Models)\n")
    output.append("| 模型 ID | 下载量 | 点赞数 | 参数量 | 任务类型 | 更新日期 |")
    output.append("| :--- | :--- | :--- | :--- | :--- | :--- |")
    ...
    # Datasets Table
    output.append("\n## 二、 热门数据集 (Trending Datasets)\n")
    output.append("| 数据集 ID | 下载量 | 点赞数 | 行数 | 更新日期 |")
    output.append("| :--- | :--- | :--- | :--- | :--- |")
    ...
    # Spaces Table
    output.append("\n## 三、 热门 Space (Trending Spaces)\n")
    output.append("| 应用标题 | ID | 点赞数 | AI 类别 | 运行硬件 | 更新日期 |")
    output.append("| :--- | :--- | :--- | :--- | :--- | :--- |")
