...
```python
await text2image(client, *, prompt, model_id, aspect_ratio="auto", resolution="auto", on_poll=None, **extra)
```
...
| aspect_ratio | str | No | `"auto"` | `1:1`, `16:9`, `9:16`, `4:3`, `3:4`, `3:2`, `2:3`, `21:9`, `auto` |
| resolution | str | No | `"auto"` | `1k`, `2k`, `4k`, `auto` |
...
Multipliers: `2k`=2×, `4k`=4×. When `aspect_ratio="auto"`, defaults to 1024×1024.

### image2image

```python
await image2image(client, *, prompt, model_id, image_urls, aspect_ratio="auto", resolution="auto", on_poll=None, **extra)
```
...
| aspect_ratio | str | No | `"auto"` | Use `"auto"` to preserve original dimensions |
| resolution | str | No | `"auto"` | `1k`, `2k`, `4k`, `auto` |
...
- `aspect_ratio="auto"` is sent as empty string `""` to the API
...
| aspect_ratio | str | No | `"16:9"` | Note: defaults to `16:9` (not `auto`) for T2V |
...
```python
await single_image2video(client, *, prompt, model_id, image_url, duration=5, resolution="1080p", aspect_ratio="auto", audio_enable=False, on_poll=None, **extra)
```
...
| aspect_ratio | str | No | `"auto"` | `auto` preserves source image ratio |
...
```python
await start_end_frame2video(client, *, prompt, model_id, start_image_url, end_image_url, duration=5, resolution="1080p", aspect_ratio="auto", audio_enable=False, on_poll=None, **extra)
```
...
```python
await multiple_image2video(client, *, prompt, model_id, image_urls, duration=5, resolution="1080p", aspect_ratio="auto", audio_enable=False, on_poll=None, **extra)
```
