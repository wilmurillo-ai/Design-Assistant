...
STATE_DIR = WORKSPACE / "skills" / "novel-scraper-pro" / "state"
...
def check_memory(limit_mb=2500):
    """检查内存使用情况（单位：MB），返回是否超过阈值"""
    try:
        with open("/proc/meminfo", "r") as f:
            ...
            if used > limit_mb:
                logger.info(f"🔄 内存过高 ({used:.0f}MB/{limit_mb}MB)，等待 GC...")
                import gc
                gc.collect()
                time.sleep(1)
                return True
...
def load_progress():
    ...
    if PROGRESS_FILE.exists():
        try:
            ...
            logger.debug(f"进度加载失败：{e}")
    return {"chapters": [], "book_url": None}


def save_progress(chapters, book_url):
    """保存进度文件"""
    STATE_DIR.mkdir(parents=True, exist_ok=True)
    data = {"chapters": chapters, "book_url": book_url}
    with open(PROGRESS_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2, ensure_ascii=False)


def get_resume_point(urls, book_url):
    """获取续抓起点"""
    progress = load_progress()
    
    # 新书检测
    if progress.get("book_url") and progress["book_url"] != book_url:
        logger.info("🔄 检测到新书，清空进度")
        return 0, urls
    
    completed = {c["url"] for c in progress.get("chapters", [])}
    
    for i, (url, ch) in enumerate(urls):
        if url not in completed:
            if i > 0:
                logger.info(f"✅ 跳过已完成 {i} 章")
            return i, urls[i:]
...
def fetch_html(url, retries=3, force_spa=False):
    """获取 HTML，带重试（V6：支持 SPA 检测）"""
    parsed = urlparse(url)
    ...
    use_browser = force_spa or domain in SPA_DOMAINS
        ...
        try:
            if use_browser:
                ...
                if result.returncode == 0:
                    ...
                    # 检测是否为 SPA
                    if detect_spa(html):
                        logger.info(f"检测到 SPA 特征，切换到浏览器模式：{domain}")
                        SPA_DOMAINS.add(domain)
                        use_browser = True
                        continue  # 重新用浏览器抓取
                    return html
            ...
            if attempt < retries - 1:
                ...
                # 最后一次失败，尝试用浏览器兜底
                if not use_browser:
                    logger.info(f"curl 失败，尝试浏览器兜底：{url}")
                    html = fetch_with_browser(url)
                    if html:
                        return html
                raise
...
def scrape_urls(urls, book_name=None, merge_interval=10, strict_quality=False,
                memory_monitor=True, resume=True, force_spa=False):
    """
    智能抓取（V6 增强版）
    ...
    Args:
        ...
        memory_monitor: 是否启用内存监控（默认 True）
        resume: 是否启用中断续抓（默认 True）
        force_spa: 是否强制 SPA 模式（默认 False）
    
    Returns:
        saved_files: 保存的文件列表
    """
    logger.info("=" * 60)
    logger.info("📖 Novel Scraper Pro V6 - 智能增强版")
    logger.info("=" * 60)
    ...
    if resume:
        logger.info("💾 中断续抓：已启用")
    if force_spa:
        logger.info("🌐 SPA 模式：已启用")
    ...
    if resume:
        ...
        skip_count, urls = get_resume_point(urls, book_url)
        if skip_count > 0:
            logger.info(f"✅ 跳过已完成 {skip_count} 章")
    ...
    for idx, (url, expected_ch_num) in enumerate(urls, 1):
        # V6 新增：内存检查
        if memory_monitor and check_memory():
            logger.info("🔄 内存过高，等待 GC...")
            import gc
            gc.collect()
            time.sleep(1)
        ...
        if cache_file.exists():
            try:
                ...
                if len(cached_data) == 2:
                    ...
                    # V6 修复：检查缓存内容是否完整（段数<50 可能是未补全的分页）
                    if len(cached_content) < 50:
                        logger.info(f"  ⚠️ 缓存内容过少 ({len(cached_content)}段)，重新抓取...")
                    else:
        ...
        for retry in range(RATE_LIMIT["max_retries"]):
            try:
                ...
                if resume:
                    # 追加当前章节到进度
                    progress = load_progress()
                    progress["chapters"].append({"url": url, "title": title})
                    progress["book_url"] = book_url
                    save_progress(progress["chapters"], book_url)
    ...
    if incomplete_chapters:
        ...
        max_runtime = 10 * 60  # 分页补全最多 10 分钟
        
        for i, ch_info in enumerate(incomplete_chapters, 1):
            # 超时检查
            if time.time() - start_time > max_runtime:
                logger.warning(f"⏰ 分页补全超时，已补全{i-1}/{len(incomplete_chapters)}章")
                break
            ...
            while next_url and page_count < 5:
                ...
                try:
                    ...
                    # 单页超时检查
                    if time.time() - page_start > 15:
                        logger.warning("    单页抓取超时 (>15 秒)")
                        break
            ...
            # V6 修复：每 5 章保存一次，防止超时丢失
            if i % 5 == 0:
                logger.info(f"  💾 保存进度（{i}/{len(incomplete_chapters)}）...")
                save_chapters(chapters_data, book_name, merge_interval)
...
def main():
    parser = argparse.ArgumentParser(description="📖 Novel Scraper Pro V6 - 智能增强版")
    ...
    parser.add_argument("--no-resume", action="store_true",
                        help="关闭中断续抓（默认开启）")
    parser.add_argument("--force-spa", action="store_true",
                        help="强制使用浏览器抓取 SPA 网站")
    ...
    # V6 参数处理（默认开启，用 --no-xxx 关闭）
    memory_monitor = not args.no_memory_monitor
    resume = not args.no_resume
    force_spa = args.force_spa
