...
function usage() {
  console.error(`Usage: sports-news.mjs "query" [--top_k 9] [--algorithm most_recent|semantic|most_recent_semantic|trending]`);
  console.error(`Example: sports-news.mjs "NFL playoff results"`);
  console.error(`Example: sports-news.mjs "NBA trade rumors" --algorithm trending`);
  console.error(`Example: sports-news.mjs "MLB scores today" --top_k 5`);
  process.exit(2);
}
...
if (!resp.ok) {
  ...
  throw new Error(`Dappier Sports News failed (${resp.status}): ${text}`);
}
...
if (items.length === 0) {
  console.log("No sports news found for this query.");
  process.exit(0);
}
