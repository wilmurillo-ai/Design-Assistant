# Recurring Payments - Alipay

Use this file when checkout includes recurring billing or renewals.

## Scope Clarification

Alipay improves initial payment entry, but recurring charges depend on backend and PSP contracts that differ from one-time checkout.

## Required Decisions

- Is Alipay used for initial payment method capture only?
...
1. Capture initial payment method through Alipay flow.
2. Store reusable payment method reference via PSP.
3. Run renewals from backend scheduler or billing engine.
4. Reconcile renewals through webhook or callback events.

## Validation Checklist

- Initial Alipay checkout and first invoice succeed.
