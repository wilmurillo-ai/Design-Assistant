...
def search(query, num=10, engines=None, lang=None, categories=None):
    ...
    # Combine results, answers, and infoboxes for comprehensive coverage
    results = list(data.get("results", []))
    answers = data.get("answers", [])
    infoboxes = data.get("infoboxes", [])
    
    # Convert answers to result format
    for ans in answers:
        if isinstance(ans, str):
            results.append({"title": "Answer", "content": ans, "url": "", "engines": ["answer"]})
    
    # Convert infoboxes to result format
    for inf in infoboxes:
        results.append({
            "title": inf.get("infobox", "Info"),
            "content": inf.get("content", ""),
            "url": inf.get("urls", [{}])[0].get("url", "") if inf.get("urls") else "",
            "engines": inf.get("engines", [])
        })
