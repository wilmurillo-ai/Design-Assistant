...
try:
    ...
    print(f"[致命错误] 缺少必要模块: {e}")
    print("请运行: pip install python-docx pywin32")
    sys.exit(1)
...
if __name__ == "__main__":
    ...
    if not is_ready:
        ...
        print("请确保已安装 Microsoft Word 且系统已激活。")
        sys.exit(1)
...
def create_robust_word_doc(
    ...
    result["success"] = True
    result["message"] = f"文档生成成功 (大小: {size} 字节)"
    return result
...
if __name__ == "__main__":
    ...
    output_file = os.path.join(desktop_path, '规划报告_终结版.docx')
