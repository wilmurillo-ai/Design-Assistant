...
const PD_ROUTER_BASE_URL = 'https://platform.paodingai.com';
...
function buildOpenApiUrl(serviceCode, endpoint) {
  const normalizedEndpoint = endpoint.replace(/^\/+/, '');
  return new URL(`/openapi/${serviceCode}/${normalizedEndpoint}`, `${PD_ROUTER_BASE_URL}/`).toString();
}
...
async function uploadFile({ serviceCode, apiKey, filePath, forceUpdate, forceOcr }) {
  ...
  const response = await fetch(buildOpenApiUrl(serviceCode, 'upload'), {
    method: 'POST',
...
async function pollParsed({ serviceCode, apiKey, uuid, pollIntervalMs, timeoutMs }) {
  const startTime = Date.now();

  while (Date.now() - startTime < timeoutMs) {
    const response = await fetch(buildOpenApiUrl(serviceCode, `document/${uuid}`), {
      method: 'GET',
      headers: buildAuthHeaders(apiKey),
    });
...
async function downloadMarkdown({ serviceCode, apiKey, uuid, outputPath, includeImages }) {
  const endpoint = includeImages ? `document/${uuid}/markdown?include_images=true` : `document/${uuid}/markdown`;
  const response = await fetch(buildOpenApiUrl(serviceCode, endpoint), {
    method: 'GET',
    headers: buildAuthHeaders(apiKey),
  });
...
async function main() {
  ...
  const serviceCode = normalizeServiceCode(DEFAULT_SERVICE_CODE);
  ...
  stderr.write(`[pd-router-pdflux-markdown] Uploading ${path.basename(filePath)} via ${PD_ROUTER_BASE_URL}\n`);
  const uuid = await uploadFile({ serviceCode, apiKey, filePath, forceUpdate, forceOcr });
