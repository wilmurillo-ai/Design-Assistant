#!/usr/bin/env python3
"""
VVMAI Image Generation & Editing Script
...
def request_generate(
    ...
    try:
        ...
        raise RuntimeError(f"VVMAI API error ({e.code}): {e.read().decode('utf-8', errors='replace')}") from e
...
def request_edit(
    ...
    boundary = "----VVMAIBoundary" + os.urandom(8).hex()
    parts = []
...
def main():
    ap = argparse.ArgumentParser(description="Generate or edit images via VVMAI API.")
