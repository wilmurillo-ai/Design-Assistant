...
"""
豆包视觉模型集成脚本
为tianji-fengshui技能添加豆包大模型图片分析功能
"""
...
class DoubaoVisionAnalyzer:
    """豆包视觉模型分析器"""
    
    def __init__(self, config_path=None):
        """初始化分析器"""
        self.config = self.load_config(config_path)
        self.api_config = self.config.get("model_routing", {}).get("image_analysis", {})
    ...
    def analyze_image(self, image_path, prompt, max_tokens=2000, temperature=0.7):
        ...
        if not api_key:
            return {
                "success": False,
                "error": "未配置豆包API密钥",
                "suggestion": "请使用 set_api_key() 方法设置API密钥"
            }
...
def main():
    ...
    if len(sys.argv) < 3:
        print("使用方法:")
        print("  python3 doubao_vision_integration.py <API密钥> <图片路径> [分析类型]")
        print("")
        print("分析类型:")
        print("  palm       掌纹分析（默认）")
    ...
    analysis_type = sys.argv[3] if len(sys.argv) > 3 else "palm"
    
    print("🧭 玄机子·豆包视觉分析")
    print("=" * 50)
    
    # 初始化分析器
    analyzer = DoubaoVisionAnalyzer()
    ...
    elif analysis_type == "custom" and len(sys.argv) > 4:
