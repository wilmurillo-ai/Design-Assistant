#!/usr/bin/env bash
# Cross-platform installer for film-location-scout skill
# Supports: Claude Code, Cursor, GitHub Copilot, Windsurf, Cline, and more

set -e

SKILL_NAME="film-location-scout"
...
install_skill() {
    ...
    # Copy skill files (excluding install.sh and hidden files)
    for file in "$SCRIPT_DIR"/*; do
        if [ -f "$file" ]; then
            ...
            if [ "$dirname" != ".git" ] && [ "$dirname" != "scripts" ]; then
    ...
    if [ -d "$SCRIPT_DIR/scripts" ]; then
        cp -R "$SCRIPT_DIR/scripts" "$install_path/"
    fi
...
Install film-location-scout skill to your AI coding assistant.
...
main() {
    ...
    log_info "Film Location Scout Skill Installer"
    echo ""
    ...
    echo "Examples:"
    echo "  /film-location-scout"
    echo "  /film-location-scout Find film locations in Paris"
    echo "  /film-location-scout 帮我找附近的电影取景地"
}
