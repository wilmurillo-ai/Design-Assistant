...
def _redact_json_like(data: Any) -> Any:
    if isinstance(data, dict):
        ...
        for k, v in data.items():
            ...
            if key in _SENSITIVE_FIELD_NAMES:
                ...
                out[k] = _redact_json_like(v)
        return out
    if isinstance(data, list):
        return [_redact_json_like(x) for x in data]
    return data
...
def _request_log_response_json(
    ...
    try:
        ...
        if http_code is not None:
            ...
            envelope = {"_raw": text}
    envelope = _redact_json_like(envelope)
