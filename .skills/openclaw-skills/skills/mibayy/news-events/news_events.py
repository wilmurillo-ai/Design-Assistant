...
def run(live: bool = False, quiet: bool = False):
    ...
    for story in filtered:
        ...
        # Gradient sizing based on impact score (0.0-1.0)
        if impact >= 0.25:
            trade_size = min(TRADE_SIZE_USD * 1.5, 30.0)   # high impact: $30
        elif impact >= 0.18:
            trade_size = TRADE_SIZE_USD                     # normal: $20
        elif impact >= 0.12:
            trade_size = TRADE_SIZE_USD * 0.6              # marginal: $12
        else:
            trade_size = TRADE_SIZE_USD * 0.4              # weak: $8
        ...
        if live:
            ...
            log.info("  [DRY RUN] Would %s $%.0f on %s", side, trade_size, question[:50])
            trades_made += 1
