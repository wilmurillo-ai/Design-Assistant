...
export function OfficeCanvas({
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent) => {
      unlockAudio();
      // Middle mouse button (button 1) OR left-click when not in edit mode starts panning
  ...
  // Wheel: plain wheel zooms, Ctrl+wheel/trackpad two-finger pans
  const handleWheel = useCallback(
    (e: React.WheelEvent) => {
      e.preventDefault();
      if (e.ctrlKey || e.metaKey) {
        // Pan via Ctrl+wheel or trackpad pinch (browsers send ctrlKey for pinch)
        const dpr = window.devicePixelRatio || 1;
        officeState.cameraFollowId = null;
        ...
        // Zoom via plain mouse wheel scroll
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      if (e.touches.length === 1 && touchStartRef.current) {
        ...
        const t = e.touches[0];
        const dpr = window.devicePixelRatio || 1;
        const dx = (t.clientX - touchStartRef.current.x) * dpr;
        const dy = (t.clientY - touchStartRef.current.y) * dpr;
        panRef.current = clampPan(
        ...
        // Map scale to zoom levels
        const newZoom = Math.round(pinchStartRef.current.zoom * scale);
