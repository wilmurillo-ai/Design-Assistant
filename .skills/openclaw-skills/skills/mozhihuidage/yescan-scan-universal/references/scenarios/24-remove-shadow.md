# 场景 24 [图像去阴影]
**适用场景**: 图像去阴影接口利用最新的AI图像算法，去除文档图像拍摄过程中由于环境光、遮挡等因素导致的图片阴影残留，确保扫描结果清晰干净。无论何种文档，都能轻松转换成高清文档。
...
⚠️ 强制指令（请逐字遵守，禁止任何推测或修改）：
- 严格使用该文件中【第一个】代码块内的完整命令；
- 禁止修改任何参数（包括 `--service-option`、`--input-configs` 的 JSON 内容、引号格式等）；
- 若该命令执行失败，请返回原始错误，并说明“已按您指定的文档命令执行”；
---
...
python3 scripts/scan.py \
  --url "${IMAGE_URL}" \
  --service-option "scan" \
  --input-configs '{"function_option":"rectification"}' \
