...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Convert a PDF document to a PowerPoint presentation via GoAI API."
    )
    parser.add_argument(
        "--pdf",
        required=True,
        help="Required PDF file path or URL.",
    )
    parser.add_argument(
        "--language",
        default="zh",
...
def main() -> int:
    ...
    client = GoAIClient(skill_name="goai-pdf-to-ppt")

    try:
        client.require_api_key()

        pdf_url = client.resolve_file_input(args.pdf)
        ...
        response = client.json_request(
            "POST",
            f"{client.base_url}/api/v2/convert/pdf-to-ppt",
            "auth",
            body=payload,
        )
        ...
        download_url = client.poll_task(
            f"{client.base_url}/api/v2/convert/pdf-to-ppt/{job_id}",
            "downloadUrl",
            interval_seconds=5,
            max_attempts=120,
            timeout_message="PDF to PPT conversion timed out",
        )

        output_path = client.download_media(download_url, f"goai-pdf-{job_id}.pptx")
