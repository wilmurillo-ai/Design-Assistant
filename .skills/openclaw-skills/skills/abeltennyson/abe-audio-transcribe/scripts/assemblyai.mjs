...
export async function main(argv = process.argv.slice(2)) {
  ...
  const sttBaseUrl = normaliseBaseUrl(flags['base-url'] ?? DEFAULT_STT_BASE_URL);
  const llmBaseUrl = normaliseBaseUrl(flags['llm-base-url'] ?? DEFAULT_LLM_BASE_URL_US);
  ...
  const apiKey = String(flags['api-key'] ?? process.env.SKILLBOSS_API_KEY ?? '');
  ...
  if (command === 'transcribe') {
    ...
    if (!apiKey) throw new Error('Missing API key. Set SKILLBOSS_API_KEY or pass --api-key.');
...
Agent-friendly speech transcription and LLM workflow helper powered by SkillBoss API Hub.
- Non-interactive.
...
General flags:
  --base-url <url>              SkillBoss API base URL. Default: ${SKILLBOSS_API_BASE}
  --llm-base-url <url>          SkillBoss LLM base URL. Default: ${SKILLBOSS_API_BASE}
  --api-key <key>               Override SKILLBOSS_API_KEY
...
Environment:
  SKILLBOSS_API_KEY             Required — SkillBoss API Hub key
...
async function uploadFile({ baseUrl, apiKey, filePath, quiet = false }) {
  ...
  stderr(`Reading ${abs} (${stat.size} bytes) for SkillBoss API Hub STT`, quiet);
  const data = await fsp.readFile(abs);
  const audioData = data.toString('base64');
  const filename = path.basename(abs);
  // Return a marker object; createTranscript will send this directly to SkillBoss STT
  return { __skillboss_local: true, audio_data: audioData, filename };
}

async function createTranscript({ baseUrl, apiKey, audioUrl, config, quiet = false }) {
  const sttInputs = typeof audioUrl === 'object' && audioUrl?.__skillboss_local
    ? { audio_data: audioUrl.audio_data, filename: audioUrl.filename }
    : { audio_url: audioUrl };
  stderr(`Calling SkillBoss API Hub STT`, quiet);
  const json = await requestJson(baseUrl, apiKey, '/pilot', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ type: 'stt', inputs: sttInputs }),
    quiet,
  });
  // SkillBoss STT: data.result.text — shape into transcript-compatible object
  const text = json?.data?.result?.text ?? '';
  return {
    id: json?.data?.request_id ?? `stt-${Date.now()}`,
    status: 'completed',
    text,
    audio_url: typeof audioUrl === 'string' ? audioUrl : (sttInputs.filename ?? ''),
  };
}
...
async function createSpeechUnderstanding({ baseUrl, apiKey, transcriptId, requestObject, quiet = false }) {
  stderr(`Calling SkillBoss API Hub for speech understanding`, quiet);
  const taskDesc = JSON.stringify(requestObject, null, 2);
  const json = await requestJson(baseUrl, apiKey, '/pilot', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({
      ...
      inputs: {
        messages: [
          {
            role: 'user',
            content: `Process the following speech understanding tasks for transcript id "${transcriptId}":\n${taskDesc}\n\nReturn a JSON object with results for each requested task.`,
          },
        ],
      },
      prefer: 'balanced',
  ...
  // Return shaped object compatible with downstream mergeTranscriptWithUnderstanding
  const text = json?.data?.result?.choices?.[0]?.message?.content ?? '';
  const parsed = safeJsonParse(text);
  return parsed ?? { response: { raw_text: text } };
}

async function createChatCompletion({ baseUrl, apiKey, requestBody, quiet = false }) {
  stderr(`Calling SkillBoss API Hub LLM`, quiet);
  const inputs = {
    ...
    ...(requestBody.response_format !== undefined ? { response_format: requestBody.response_format } : {}),
  };
  const json = await requestJson(baseUrl, apiKey, '/pilot', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ type: 'chat', inputs, prefer: 'balanced' }),
    quiet,
  });
  // SkillBoss: data.result contains OpenAI-compatible choices array
...
async function resolveTranscriptLikeInput({
  ...
  if (allowFetchById) {
    if (!apiKey) {
      throw new Error('Resolving a transcript id requires SKILLBOSS_API_KEY. For offline formatting, pass a local raw JSON or agent JSON file instead.');
    }
...
async function materialiseOutputBundleFromAgentJson({
  ...
  if ((needsParagraphs || needsSentences || needsSrt || needsVtt) && !apiKey) {
    throw new Error('Paragraph/sentence/subtitle exports require SKILLBOSS_API_KEY because they call SkillBoss API Hub endpoints.');
  }
