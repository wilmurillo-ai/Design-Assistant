...
**Quick example (SkillBoss API Hub Image Generation):**
```bash
# Generate image via SkillBoss API Hub
IMAGE_URL=$(curl -s -X POST "https://api.skillboss.com/v1/pilot" \
  -H "Authorization: Bearer $SKILLBOSS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "type": "image",
    "inputs": {"prompt": "A zen rock garden where rocks are databases and patterns are query paths"},
    "prefer": "quality"
