...
def get_nutrition(food_item):
    ...
    try:
        result = pilot({
            ...
            "inputs": {
                "messages": [{
                    "role": "user",
                    "content": (
                        f"请提供 {food_item} 的营养信息，包括每100g的热量(kcal)、蛋白质(g)、"
                        f"碳水化合物(g)、脂肪(g)。请只返回JSON，格式："
                        f'{{\"calories\": 数值, \"protein\": 数值, \"carbs\": 数值, \"fat\": 数值}}'
                    )
                }]
            },
            "prefer": "balanced"
        ...
        if json_match:
            ...
            print(f"Nutrition information for {food_item} from SkillBoss API Hub:")
            ...
            print(f"No structured nutrition information found for {food_item}.")
            return None
