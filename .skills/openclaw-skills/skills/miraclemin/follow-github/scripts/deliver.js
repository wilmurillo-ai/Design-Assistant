...
//
// The script reads delivery config from ~/.follow-github/config.json
// and API keys from ~/.follow-github/.env
// ============================================================================
...
async function sendEmail(text, apiKey, toEmail) {
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'GitHub Digest <digest@resend.dev>',
      to: [toEmail],
      subject: `GitHub Digest — ${new Date().toLocaleDateString('en-US', {
  ...
  try {
    switch (delivery.method) {
      ...
      case 'email': {
        ...
        console.log(JSON.stringify({ status: 'ok', method: 'email', to: toEmail }));
        break;
      }
