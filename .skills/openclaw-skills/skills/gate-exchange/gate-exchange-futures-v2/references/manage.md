...
|-----------|----------|-------------|
| List open orders | `cex_fx_list_price_triggered_orders` | "Show my TP/SL orders", "List conditional orders" |
...
After cancel or amend, call `cex_fx_get_fx_price_triggered_order` or `cex_fx_list_price_triggered_orders` to confirm the result.
...
1. Call `cex_fx_list_price_triggered_orders(settle="usdt", status="open")`.
...
1. **Locate order**: if user provides an order ID, use it directly. If not, call `cex_fx_list_price_triggered_orders` to show open orders and let user identify the one to cancel.
...
4. **Verify**: call `cex_fx_list_price_triggered_orders(settle, status="open")` → confirm list is empty (or reduced).
...
**Expected Behavior**:
1. Call `cex_fx_list_price_triggered_orders(settle="usdt", status="open", contract="BTC_USDT")`.
...
**Expected Behavior**:
1. Call `cex_fx_list_price_triggered_orders(settle="usdt", status="open", contract="BTC_USDT")` to find the TP order (trigger `>=` for long TP).
...
3. Verify via `cex_fx_list_price_triggered_orders(settle="usdt", status="open")` → confirm list is empty.
...
**Expected Behavior**:
1. Call `cex_fx_list_price_triggered_orders` to find the SL order.
2. Get position mode via `cex_fx_get_fx_accounts(settle)` to determine `close`/`auto_size` for amend.
