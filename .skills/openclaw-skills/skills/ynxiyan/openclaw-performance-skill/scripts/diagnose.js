...
class PerformanceDiagnoser {
        ...
        check: async () => {
          try {
            ...
            if (stats) {
              ...
              return {
                passed,
                message: `${freeGB}GB 可用`,
                details: passed ? '磁盘空间充足' : '磁盘空间不足，建议清理',
                score: passed ? 10 : 5
              };
            }
            return { passed: true, message: '未知', details: '无法获取磁盘信息', score: 5 };
          } catch {
            return { passed: true, message: '未知', details: '无法获取磁盘信息', score: 5 };
          }
        }
  ...
  async checkConfigurations() {
    const configChecks = [
      ...
      {
        name: '上下文压缩配置',
        description: '检查是否配置自动上下文压缩',
        check: async () => {
          const configPath = path.join(process.env.HOME || process.env.USERPROFILE, '.openclaw', 'config.json');
          if (fs.existsSync(configPath)) {
            try {
              const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
              const hasCompaction = config.agents?.defaults?.compaction?.triggerAtPercent === 75;
              return {
                passed: hasCompaction,
                message: hasCompaction ? '已配置自动压缩' : '未配置自动压缩',
                details: hasCompaction ? '压缩阈值: 75%' : '建议设置triggerAtPercent: 75',
                score: hasCompaction ? 15 : 0
              };
            } catch {
              return { passed: false, message: '配置文件解析失败', details: '配置文件格式错误', score: 0 };
            }
          }
          return { passed: false, message: '无配置文件', details: '需要创建配置文件', score: 0 };
        }
      },
      {
        name: '流式模式配置',
        description: '检查消息流式模式',
        check: async () => {
          const configPath = path.join(process.env.HOME || process.env.USERPROFILE, '.openclaw', 'config.json');
          if (fs.existsSync(configPath)) {
            try {
              const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
              const telegramMode = config.channels?.telegram?.streamMode;
              const discordMode = config.channels?.discord?.streamMode;
              
              const hasOptimizedMode = telegramMode === 'chunked' || telegramMode === 'full' || 
                                      discordMode === 'chunked' || discordMode === 'full';
              
              return {
                passed: hasOptimizedMode,
                message: hasOptimizedMode ? '已优化流式模式' : '未优化流式模式',
                details: hasOptimizedMode ? `当前模式: ${telegramMode || discordMode}` : '建议使用chunked或full模式',
                score: hasOptimizedMode ? 10 : 0
              };
            } catch {
              return { passed: false, message: '无法检查', details: '配置文件解析失败', score: 0 };
            }
          }
          return { passed: false, message: '无配置文件', details: '需要创建配置文件', score: 0 };
        }
  ...
  async testMemoryGrowth() {
    ...
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        ...
        const passed = growthMB < 10; // 增长小于10MB
        resolve({
          passed,
  ...
  async identifyProblems() {
    ...
    const commonProblems = [
      ...
      {
        condition: () => {
          const configResults = this.results.filter(r => r.category === 'config');
          return configResults.length > 0 && configResults.every(r => !r.passed);
        },
        issue: '配置缺失',
        description: '缺少关键性能优化配置',
        severity: 'medium',
        category: 'config'
      }
    ];
    ...
    this.problems?.forEach(problem => {
      switch (problem.issue) {
        case 'OpenClaw版本':
          ...
          break;
        case '上下文压缩配置':
          recommendations.push('配置自动上下文压缩，设置triggerAtPercent: 75');
          break;
        case '流式模式配置':
          recommendations.push('优化流式模式，改为chunked或full模式');
          break;
        case '内存管理问题':
        ...
        case '整体性能较差':
          ...
          break;
        case '配置缺失':
          recommendations.push('创建并配置OpenClaw配置文件');
          break;
      }
    });
