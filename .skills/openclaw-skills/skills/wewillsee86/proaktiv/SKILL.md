...
- **Budget**: max 6 pings per day (resets at midnight)
...
3. **Prompt for your Telegram chat ID** (required for pings to reach you)
...
### Daily Ping Budget
Default: 6 pings/day. Resets at midnight. Hard cap — Proactive won't exceed it.
...
Runs in an isolated session — no human attention required.
...
- Lower budget: edit `DAILY_BUDGET_MAX` in `proaktiv_check.py`
...
- **v1.0.18**: Fix routing — `inject_trigger()` now sends to MAIN session instead of Telegram. SOUL.md processes the trigger, generates researched content, sends proper message. No raw trigger text in chat.
- **v1.0.9**: Topic Templates Module — all topic rules in TOPIC_TEMPLATES.md (F1, KI_NEWS, SUNO, N8N, SERIEN, etc.). Generic fallback: any custom interest researched automatically.
- **v1.0.8**: Fix .env loading — `os.environ[]` assignment ensures .env always overrides CHANGE_ME.
- **v1.0.7**: Remove deceptive onboarding text. Add permission transparency for chat history.
- **v1.0.6**: Full English rebrand. PROAKTIV → Proactive throughout.
- **v1.0.5**: Privacy fix — `user_reacted` field names. No personal data in repo.
- **v1.0.4**: Nightshift Module — Quiet Hours trigger autonomous memory compaction.
- **v1.0.3**: Interactive install — prompts for Telegram chat ID.
- **v1.0.2**: F1 Sunday suppression.
