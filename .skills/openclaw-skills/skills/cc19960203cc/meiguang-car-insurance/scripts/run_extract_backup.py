...
def parse_jiaoqiang(text, company="unknown"):
    ...
    data["实收保费"] = safe_extract(text, [
        ...
        # 华海交强险：中文大写金额后面直接跟数字（如叁佰陆拾元整\n360.00 / 柒佰柒拾元整\n770.00）
        # 用 last=True 参数取最后一个匹配（避免车船税360先于保费770匹配）
        r"([0-9,]+\.00)\s*\n\s*柒佰柒拾元整",  # 华海交强险实收保费（770.00 在柒佰柒拾元整前面）
        r"([0-9,]+\.00)\s*\n\s*叁佰陆拾元整",  # 华海交强用车船税（360.00 在叁佰陆拾元整前面）
        r"保险费合计[\s\S]*?[￥¥][：:\s]*([0-9,]+\.?\d*)",
        r"（￥：\s*([0-9,]+\.?\d*)元）",
        r"实收保费[：:\s]*[￥¥]?\s*([0-9,]+\.?\d*)",
        r"保险费合计[（(][^)]*)[）)]\s*[￥¥]?\s*([0-9,]+\.?\d*)",
        ...
        r"含税总保险费[^0-9]*([0-9,]+\.?\d*)",
        r"总保险费[^\d]*([0-9,]+\.?\d*)",
        r"(\d+\.00)\n[^\n]*\n[一二三四五六七八九十零〇两整角分]+",  # 华海交强险：数字行 → 内容行 → 中文大写行（如770.00\n1\n7.70\n烟台中心支公司）
...
def parse_shangye(text, company="unknown"):
    ...
    data["实收保费"] = safe_extract(text, [
        ...
        r"含税总保险费[^0-9]*([0-9,]+\.?\d*)",
        r"总保险费[^\d]*([0-9,]+\.?\d*)",
        r"[一二三四五六七八九十零〇两整角分元角]+\s*\n\s*(\d+\.\d+)",  # 华海商业险：中文大写 → 数字（伍佰捌拾伍元肆角壹分\n585.41）
        r"(\d+\.00)\n[^\n]*\n[一二三四五六七八九十零〇两整角分元角]+",  # 华海：数字 → 内容行 → 中文大写
    ])
...
def parse_dadi_anyang(pymupdf_text, plumber_text):
    ...
    fee = safe_extract(text, [
        r"Total premium[^0-9]*?([0-9,]+\.?\d*)",  # 英文标签：CID乱码导致中文标签损坏，非贪婪跳过所有非数字字符
        r"总保险费[：:\s]*[￥¥]?\s*([0-9,]+\.?\d*)",
        r"总保费[：:\s]*[￥¥]?\s*([0-9,]+\.?\d*)",
        r"保险费.*?([0-9,]+\.?\d*)\s*(?:元|RMB)",
        r"[（(][￥¥]?\s*([0-9]{3,5}\.00)[）)]\s*元",
...
def route_type(text):
    ...
    has_shang_hdr = ("机动车商业保险保险单" in header) or ("机动车辆商业保险保险单" in header) or ("神行车保机动车保险单" in header) or ("机动车综合商业保险" in header)
    ...
    has_shang_full = ("机动车商业保险保险单" in text) or ("机动车辆商业保险保险单" in text) or ("机动车综合商业保险" in text)
