...
#@IF openclaw
- treat this as the expected OpenClaw delivery handoff, not a Telegram setup problem
- if this run was started from an interactive channel, deliver formatted results through the invocation channel using the returned `preview`
- if this run was started by OpenClaw cron, return formatted results for the cron runner's configured announce target using the returned `preview`
- proceed to served marking
#@ENDIF
#@IF !openclaw
- do **not** mark served
...
#@ENDIF
#@IF codex
- Codex sandbox resend policy is defined in `references/platform-codex.md`.
...
- if the approved retry returns `{"sent": true, "reason": "sent", ...}`, show the returned `preview` to the user, print pre-send audit debug in the CLI if needed, and proceed to served marking
- if approval is denied, or the retry still returns `{"sent": false, ...}`, do **not** mark served, show the returned `preview` to the user, print pre-send audit debug in the CLI if needed, and treat the second result as final
- for any other `send_failed` `error_code`, do **not** request elevated resend; do **not** mark served; show the returned `preview` to the user; print pre-send audit debug in the CLI if needed; then report the failure briefly using `reason` plus any short returned `error` / `status_code`
#@ENDIF
...
#@IF openclaw
If send result was `telegram_not_configured` and the returned `preview` was
delivered through OpenClaw:
...
For OpenClaw's internal-delivery path, keep `sent = false` to match the raw
`send` result, but use `send_reason = "telegram_internal"` after the returned
`preview` is delivered through the invocation channel or scheduled user chat.
#@ENDIF
...
python -m weekend_scout run-complete --run-id "<run_id>" \
  ...
  --events-sent <city_count + trip_count> \
  --sent <true|false> \
  --send-reason <%%RUN_COMPLETE_SEND_REASONS%%> \
