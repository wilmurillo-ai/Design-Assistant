...
#@IF claude-code
argument-hint: [city] [radius-km] [--cached-only] [--research-only] [--reset]
...
- If the user invoked `%%INVOKE_CMD%% --reset`, do **not** run `init-skill`.
...
- Do **not** send that message for `%%INVOKE_CMD%% --cached-only`.
...
- If the user invoked `%%INVOKE_CMD%% --research-only`, report the `score_summary`
