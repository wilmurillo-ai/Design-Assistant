...
import { buildMarketPositionPayload, getExceedsWithdrawableError, getWithdrawAllHasDebtError, getWithdrawNoCollateralError, } from "./shared/market.js";
export async function cmdMarketWithdraw(args) {
    ...
    try {
        ...
        const { marketInfo, userData } = await getMarketRuntimeData(chainId, marketId, walletAddress);
        ...
        if (args.amount) {
            ...
            if (userData.withdrawable.lt(requestedWithdrawAmount)) {
                printJson(getExceedsWithdrawableError(args.amount, userData, collateralInfo.symbol));
                exitWithCode(1);
            }
        }
        ...
        const steps = ensureStepsGenerated(await sdk.buildWithdrawParams({
            chainId,
            marketId,
            assets,
