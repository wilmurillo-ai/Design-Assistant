...
export function buildSdkErrorOutput(message, options) {
    const targetLabel = options.targetType === TargetType.Vault ? "Vault" : "Market";
    const invalidPhrase = options.targetType === TargetType.Vault
    ...
    if (invalidPhrase) {
        return {
            status: "error",
            reason: options.targetType === TargetType.Vault ? "invalid_vault" : "invalid_market",
    ...
    const isInsufficient = insufficientKeywords.some((keyword) => message.toLowerCase().includes(keyword.toLowerCase()));
