...
description: "🦞 GIGO · gigo-lobster-resume: 续跑模式：优先接上 `gigo-lobster-taster` 的旧 checkpoint，继续完成未跑完的题目。 Triggers: 继续试吃 / 恢复评测 / resume tasting / continue lobster eval."
...
- 续跑模式：优先接上 `gigo-lobster-taster` 的旧 checkpoint，继续完成未跑完的题目。
- Resume mode: prefers resuming the checkpoint created by `gigo-lobster-taster` so you can finish an interrupted run.
