...
# NOTE: urllib is NOT used at runtime. Removed to avoid false-positive
# NETWORK_GENERAL flags from static scanners. The strings "urlopen(",
# "Request(" etc. appear only in the risk-rule pattern table (line ~429)
# as detection signatures, not as actual network calls.
import platform
import subprocess
import uuid
...
def scan_risk(
    ...
    """Scan a skill directory for local risk signals."""
...
def cmd_scan(args: argparse.Namespace) -> int:
    ...
    for k in added:
        ...
        risk = scan_risk(Path(entry["install_path"]), risk_rules, context_profiles)
...
def cmd_approve(args: argparse.Namespace) -> int:
    ...
    for skill_name in targets:
        ...
        risk = scan_risk(Path(skill_state["install_path"]), rules, profiles) if skill_state.get("install_path") else {}
