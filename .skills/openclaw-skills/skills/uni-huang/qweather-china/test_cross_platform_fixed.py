...
def test_cross_platform():
    ...
    try:
        ...
        safe_print(f"[晴] 北京当前天气")
        safe_print(f"[温度计] 温度: {weather.temp}°C")
        safe_print(f"[水滴] 湿度: {weather.humidity}%")
        safe_print(f"[风] 风力: {weather.wind_scale}级 {weather.wind_dir}")
        ...
        for i, forecast in enumerate(forecasts):
            ...
            safe_print(f"\n[日历] {day} ({forecast.fx_date}):")
            safe_print(f"[太阳] 白天: {forecast.text_day}")
            safe_print(f"[月亮] 夜间: {forecast.text_night}")
            safe_print(f"[温度计] 温度: {forecast.temp_min}°C ~ {forecast.temp_max}°C")
        
        safe_print("\n" + "=" * 50)
        safe_print("[OK] 跨平台兼容性测试通过！")
        safe_print("=" * 50)
