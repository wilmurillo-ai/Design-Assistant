...
class ProperGradingWeatherFixed:
    """修复版：确保所有含义相同的简单问题都得到简洁回答"""
    ...
    def __init__(self, user_city="beijing"):
        ...
        self.simple_patterns = {
            ...
            'humidity': ['湿度', '湿度怎么样', '湿度如何', '湿度情况'],
            'temperature': ['温度', '多少度', '温度多少', '现在温度', '当前温度'],
        }
        
        # 详细问题模式
        self.detailed_patterns = ['详细', '完整', '全面', '报告', '天气预报']
    ...
    def _get_city(self, question):
        ...
        all_keywords.extend(['现在', '今天', '明天', '外面', '吗', '？', '?', '呢', '啊', '呀'])
    ...
    def _simple_answer(self, question, location, city_name, weather, today):
        ...
        # 根据关键词分类（按优先级）
        if any(pattern in q for pattern in self.simple_patterns['umbrella']):
            return self._answer_umbrella_simple(location_note, weather, today)
        elif any(pattern in q for pattern in self.simple_patterns['cold']):
            return self._answer_cold_simple(location_note, weather, today)
        elif any(pattern in q for pattern in self.simple_patterns['hot']):
            return self._answer_hot_simple(location_note, weather, today)
        elif any(pattern in q for pattern in self.simple_patterns['rain']):
            return self._answer_rain_simple(location_note, weather, today)
        elif any(pattern in q for pattern in self.simple_patterns['wind']):
            return self._answer_wind_simple(location_note, weather)
        elif any(pattern in q for pattern in self.simple_patterns['humidity']):
            return self._answer_humidity_simple(location_note, weather)
        elif any(pattern in q for pattern in self.simple_patterns['temperature']):
...
def test_fixed_grading():
    """测试修复后的分级响应"""
    weather = ProperGradingWeatherFixed(user_city="beijing")
    ...
    safe_print("=" * 60)
    safe_print("修复测试：含义相同但问法不同的问题")
    safe_print("确保所有简单问题都得到