...
function parsePhaseHeader(line) {
  // "## 阶段 1：拆解与发散（Task Deconstruction）"
...
function parseStepCheck(line) {
  // "[ ] step text" → unchecked, "[x] step text" → checked
...
/**
 * 1. SOP文件生成器
 * generateSOP(goal, subTasks, directories)
 *
 * @param {string}  goal        — 研究目标描述
 * @param {string[]|object} subTasks — 子任务列表（字符串数组或 {key, description} 数组）
 * @param {object}  directories — 目录配置（覆盖 DEFAULT_DIRS）
 * @returns {{ sopContent: string, sopPath: string, decisionLogContent: string, decisionLogPath: string }}
 */
...
/**
 * 2. SOP状态读取器
 * readSOPState(sopPath)
 *
 * @param {string} sopPath — SOP文件路径
 * @returns {{
 *   currentPhase: number|null,
 *   phases: Array<{ number, label, totalSteps, completedSteps, steps: Array<{checked, text}> }>,
...
export function readSOPState(sopPath) {
  ...
  for (const p of phases) {
    if (p.completedSteps === p.totalSteps && p.totalSteps > 0) {
      ...
      if (!foundCurrent) continue; // 已完成的阶段
    } else {
      if (!foundCurrent) {
        foundCurrent = true;
  ...
  // 也检查待执行阶段的序号完整性
  // 如果阶段3在待执行但阶段2不在已完成也不在待执行，说明阶段2缺失
...
/**
 * 3. SOP状态推进器
 * advanceSOPState(sopPath, phaseName, resultSummary)
 *
 * @param {string} sopPath      — SOP文件路径
 * @param {string} phaseName    — 阶段标识（key如'deconstruction'或数字如'1'或'阶段1'）
 * @param {object} resultSummary — { completedSteps?: string[], decision?: string, reason?: string, alternatives?: string, context?: string }
...
export function advanceSOPState(sopPath, phaseName, resultSummary = {}) {
  ...
  let updated = content;
  const phaseDef = PHASES.find(p => p.number === phaseNum);
  const completedSteps = resultSummary.completedSteps || []; // 要打钩的步骤文本（部分匹配即可）
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inTargetPhase) {
      ...
      if (step && !step.checked) {
        // 如果 completedSteps 为空，打钩所有步骤（标记整个阶段完成）
        // 如果 completedSteps 有值，只打钩匹配的步骤
...
/**
 * 5. 阶段转换验证器
 * validatePhaseTransition(sopPath, fromPhase, toPhase)
 *
 * @param {string}  sopPath   — SOP文件路径
 * @param {number|null} fromPhase — 来源阶段编号（null=自动推断当前阶段）
 * @param {number}  toPhase   — 目标阶段编号
 * @returns {{ valid: boolean, reason?: string, missingFiles?: string[] }}
 */
export function validatePhaseTransition(sopPath, fromPhase, toPhase) {
  ...
  if (!PHASES.find(p => p.number === toPhase)) {
    return { valid: false, message: \`目标阶段\${toPhase}不存在（合法范围1-5）\` };
  }
...
/**
 * 6. 决策日志系统
 * logDecision(sopPathOrLogPath, decision, reason, context)
 *
 * @param {string} logPath   — 决策日志文件路径（或 sopPath 自动推断）
 * @param {string} decision  — 决策内容
 * @param {string} reason    — 决策原因
 * @param {object} context   — { phase?, alternatives?, ... }
 * @returns {{ success: boolean, message: string }}
 */
export function logDecision(logPath, decision, reason, context = {}) {
  // 如果传入的是 SOP 路径而非决策日志路径，自动推断
