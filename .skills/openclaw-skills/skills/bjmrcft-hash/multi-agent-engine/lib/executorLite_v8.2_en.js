// executorLite_v8.2_en.js - English only, clean ES module
// Fixed version without Chinese characters that may cause encoding issues

import fs from 'node:fs';
import path from 'node:path';
import { generateAgentPrompt } from './communication.js';
import { validateOutput, selectSchema } from './outputSchema.js';
...
import { validateEnhanced, StateVersionManager } from './stateManager.js';
...
function estimateComplexityLite(task) {
  ...
  const simpleSignals = ['list', 'summarize', 'format', 'simple', 'quick'];
  if (simpleSignals.some(s => desc.includes(s))) return 'simple';
  const complexSignals = ['deep', 'comprehensive', 'multidimensional', 'analysis'];
...
export function buildLiteSpawnParams(agentProfile, task, workflow, options = {}) {
  ...
  if (modelResult.error || !modelResult.model) {
    ...
    if (fallbackModel) {
      ...
      modelResult.reason = `[fallback] use first model in pool: ${fallbackModel.id}`;
    }
  }
  ...
  const outputInstruction = `Output file: ${outputFile} (${outputMode === 'json' ? 'JSON format' : 'Markdown format'})\nAfter completion output: EXECUTION_COMPLETE`;
...
export function validateLiteOutput(agentName, outputDir, options = {}) {
  ...
  try {
    ...
    return {
      valid: false,
      errors: [`Read failed: ${error.message}`],
      parsed: null,
      action: 'retry',
      strategy: 'retry',
      score: 0
...
export function buildBatchLiteSpawnParams(agentNames, workflow, profiles, options = {}) {
  ...
  return agentNames.map(agentName => {
    ...
    if (!profile) {
      return {
        error: `Agent config missing: ${agentName}`,
        agentName,
        skip: true
      };
    }
    ...
    const task = {
      description: options.tasks?.[agentName] || `Execute ${agentName} research task`,
