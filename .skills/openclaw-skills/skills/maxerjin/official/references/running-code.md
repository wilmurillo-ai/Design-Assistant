...
> **Remote browser caveat:** `recordVideo` and `tracing` save artifacts on the **server side**. When using `chromium.connect()` to a remote AgentGo browser, these files may not transfer to your local machine automatically. Use `page.screenshot()` for reliable visual capture instead.
