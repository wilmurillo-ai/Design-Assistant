...
## 项目简介
`deep-search-mpro` 是一个用于深度研究与报告生成的技能模板，核心目标是：
...
说明：`scripts/data_collector.py` 默认是 dry-run（不发起网络请求）。如需真实网络采集，请显式启用网络请求并设置请求上限。
...
> 专业深度研究与报告生成技能：支持企业/产品/行业/市场/AI 厂商分析，使用 Agent 内置 `web_search` + `web_fetch`（无需 API key），输出咨询级 Markdown + HTML 报告。
...
git init
git add .
git commit -m "init: add deep-search-mpro skill project"
git branch -M main
