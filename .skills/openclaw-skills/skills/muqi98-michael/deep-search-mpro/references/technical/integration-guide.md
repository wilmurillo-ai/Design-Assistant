...
# 或者使用其搜索能力（示例命令，根据你本地实际脚本调整）
bash search.sh "skincare market size China growth rate" --max 8
```
...
```bash
# 备选抓取（使用你选择的任意公开检索入口 URL）
web_fetch \
    "https://example.com/search?q=skincare+market+size+2024" \
    --maxChars 8000

# 区域/参数过滤（按站点实际参数为准）
web_fetch \
    "https://example.com/search?q=best+coffee+melbourne&region=us-en" \
    --maxChars 8000
```
...
1. **混合搜索**：
   - 中文数据：优先 web_fetch（百度、微信）
   - 国际数据：优先 deep-research-pro + Google
   - 快速补充：web_fetch（可选任意公开检索入口 URL）
