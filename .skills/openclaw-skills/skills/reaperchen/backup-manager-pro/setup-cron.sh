...
CRON_FILE="/tmp/openclaw-backup-cron"

echo -e "${BLUE}🦞 设置备份Cron自动任务...${NC}"
...
# 每周一凌晨3:30 - 自开发Skills备份（较小，保留4周）
30 3 * * 1 "$BACKUP_MANAGER_DIR/backup-weekly-skills.sh" >> "$HOME/.openclaw/workspace/memory/backup-cron.log" 2>&1
...
# 询问用户确认
echo ""
echo -e "${YELLOW}⚠️  即将添加以上Cron任务到你的crontab${NC}"
echo -e "${YELLOW}📅 执行时间:${NC}"
echo -e "  • 每日配置备份: 02:00 (凌晨2点)"
echo -e "  • 每周完整备份: 03:00 每周一 (凌晨3点)"
echo -e "  • 每周Skills备份: 03:30 每周一 (凌晨3点30分)"
...
# 测试第一个任务（可选）
echo ""
echo -e "${YELLOW}🧪 是否立即测试每日备份任务? (y/N): ${NC}" -n
read -p "" -n 1 -r
echo ""
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo -e "${BLUE}🚀 测试每日备份任务...${NC}"
    "$BACKUP_MANAGER_DIR/backup-now.sh"
    echo -e "${GREEN}✅ 测试完成，请检查备份日志${NC}"
fi
