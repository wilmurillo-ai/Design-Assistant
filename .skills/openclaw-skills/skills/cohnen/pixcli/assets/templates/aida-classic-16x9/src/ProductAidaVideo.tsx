...
export const ProductAidaVideo: React.FC<Props> = ({plan}) => {
  ...
  return (
    <AbsoluteFill style={{backgroundColor: "#020617"}}>
      {plan.musicFile ? <Audio src={staticFile(plan.musicFile)} volume={0.2} /> : null}
      {timeline.map(({scene, from}) => (
        <Sequence key={scene.id} from={from} durationInFrames={scene.durationInFrames}>
          {scene.voiceoverFile ? <Audio src={staticFile(scene.voiceoverFile)} volume={1} /> : null}
          {scene.sfxFile ? <Audio src={staticFile(scene.sfxFile)} volume={0.35} /> : null}
