...
logger = logging.getLogger("ok-bridge")
...
class BridgeServer:
    ...
    async def _handle_cli(self, ws: ServerConnection, msg: dict) -> None:
        ...
        if not self._extension_ws:
            await ws.send(json.dumps({
                "error": "Extension 未连接，请确认浏览器已安装并启用 OK Bridge 扩展"
            }))
            return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="OK Extension Bridge Server")
