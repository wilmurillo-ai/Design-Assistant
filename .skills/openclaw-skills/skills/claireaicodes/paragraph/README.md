...
This skill integrates seamlessly with OpenClaw agents. Below are example prompts and expected tool usage.
...
from memory/2026-03-10.md, do not send newsletter.
```
The agent will automatically call `paragraph_createPost` with extracted title and markdown.

**Import new subscribers from a CSV file**
```
Add all subscribers from /path/to/subscribers.csv to my Paragraph publication.
```
The agent uses `paragraph_importSubscribers` with the CSV file content.

**List recent posts and summarize coin activity**
```
Show me the 5 most recent posts and identify which ones have associated coins.
```
The agent calls `paragraph_listPosts` with limit=5, then `paragraph_getCoin` for each post that has a coinId.
...
graph LR
    ...
    B --> C[paragraph_createPost]
    C --> D[Paragraph API]
    D --> E[Post Created]
    E --> B --> F[Success Response]
```

*Simplified flow for creating a post.*

### Tips for Best Results

- Always set `PARAGRAPH_PUBLICATION_SLUG` in your OpenClaw environment.
- For batch operations (e.g., importing subscribers), consider rate limits and use retry logic.
- Use `paragraph_testConnection` first to verify credentials.
- Post creation is onchain; allow a few seconds for the transaction to confirm before relying on the slug or URL.
