...
_SSL_CONTEXT_UNVERIFIED = ssl._create_unverified_context()
...
def fetch_xml(url: str, timeout: int = 30) -> str:
    ...
    for attempt in range(4):
        try:
            ...
            # Some local Python environments miss CA certificates; retry once without verification.
            if "CERTIFICATE_VERIFY_FAILED" in str(err):
                print("WARN: SSL verify failed, retrying without certificate verification.", file=sys.stderr)
                with urllib.request.urlopen(
                    req, timeout=timeout, context=_SSL_CONTEXT_UNVERIFIED
                ) as resp:
                    return resp.read().decode("utf-8", errors="replace")
            raise
...
def download_file(url: str, destination: Path, timeout: int = 60) -> None:
    ...
    try:
        ...
        if "CERTIFICATE_VERIFY_FAILED" not in str(err):
            raise
        print("WARN: SSL verify failed during PDF download, retrying insecurely.", file=sys.stderr)
        with urllib.request.urlopen(req, timeout=timeout, context=_SSL_CONTEXT_UNVERIFIED) as resp:
            data = resp.read()
