...
Implements the four-phase retrieval workflow for AI agent memory.

Usage:
    loader = MemoryLoader("./memory_root")
    context = loader.load_session_context(user_message="Let's continue on the schemas")
    # context.text contains the assembled memory to inject into the system prompt
    # context.metadata contains loading decisions for debugging
"""
...
class MemoryLoader:
    ...
    def _daily_log_dir(self) -> Optional[Path]:
        """Return the daily log directory, checking new layout (memory/) then legacy (memory/daily/)."""
        # New layout: daily logs live directly in memory/
        new_dir = self.root / "memory"
        # Legacy layout: memory/daily/
        legacy_dir = self.root / "memory" / "daily"
        # Prefer new if it contains date-named .md files, otherwise fall back to legacy
        if any(new_dir.glob("[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9].md")):
            return new_dir
        if legacy_dir.exists() and any(legacy_dir.glob("*.md")):
            return legacy_dir
        # No logs yet — return new layout as default
        return new_dir

    def _daily_log_relative(self, filename: str, source_dir: Path) -> str:
        """Return the relative path string for a daily log file."""
        if source_dir == self.root / "memory" / "daily":
    ...
    def _recent_daily_summaries(self, count: int = 5) -> list[LoadedContent]:
        ...
        if not daily_dir or not daily_dir.exists():
            return []
        logs = sorted(daily_dir.glob("[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9].md"), reverse=True)[:count]
        results = []
        for log_path in logs:
            text = log_path.read_text(encoding="utf-8")
            # Extract just the summary + decisions sections
            summary = self._extract_log_sections(text, ["Session Summary", "Decisions"])
            if summary:
                rel = self._daily_log_relative(log_path.name, daily_dir)
        ...
        for line in index_text.split("\n"):
            if "|" in line and query.lower() in line.lower():
                ...
                if len(parts) > 1 and re.match(r"\d{4}-\d{2}-\d{2}", parts[1]):
                    matches.append(self._daily_log_relative(f"{parts[1]}.md", daily_dir))
        return matches
    ...
    def _scan_daily_logs_by_filename(self, query: str) -> list[str]:
        """Fallback: just return recent logs (no index available)."""
        daily_dir = self._daily_log_dir()
        if not daily_dir or not daily_dir.exists():
            return []
        return [
            self._daily_log_relative(p.name, daily_dir)
            for p in sorted(daily_dir.glob("[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9].md"), reverse=True)[:10]
        ]
