...
_ENCAPSULATION_EMBEDDED: Optional[Dict[str, Any]] = {"public_skill_key": "test_txt_public", "original_skill_key": "test_txt", "encapsulation_target": "claw_hub"}
...
def _credentials_from_environment() -> Optional[Tuple[str, str]]:
    """调用 /api/claw/agent-run 前：仅从环境变量解析 pk/sk（不发起网络请求）。"""
...
def load_prana_credentials(prana_base_url: Optional[str] = None) -> Tuple[str, str]:
    """
    调用 agent-run 前解析 x-api-key 所用凭证：**先环境变量**，完整则**不**请求 GET；否则再 GET /api/v1/api-keys（见 _build_api_keys_fetch_url）。
    优先级：
      PRANA_SKILL_PUBLIC_KEY + PRANA_SKILL_SECRET_KEY
      PRANA_SKILL_API_KEY（整段 JSON，或单行 public_key:secret_key）
      GET /api/v1/api-keys（需可达 base_url；成功则注入当前进程环境）
    """
    epk = os.environ.get("PRANA_SKILL_PUBLIC_KEY", "").strip()
    esk = os.environ.get("PRANA_SKILL_SECRET_KEY", "").strip()
    if (epk and not esk) or (esk and not epk):
        print(
            "提示: PRANA_SKILL_PUBLIC_KEY 与 PRANA_SKILL_SECRET_KEY 须成对出现；当前不完整，将尝试 GET /api/v1/api-keys 获取新密钥对。",
            file=sys.stderr,
        )
