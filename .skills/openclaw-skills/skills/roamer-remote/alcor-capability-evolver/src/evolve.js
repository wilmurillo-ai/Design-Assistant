...
`.trim();
  ...
  const executorTask = [
    '#!/bin/bash',
    '# Bridge sub-agent: Apply GEP protocol patch and run solidify',
    '# Environment variables for solidify fallback logic:',
    'export EVOLVER_MUTATION_ID="' + (mutation && mutation.id || '') + '"',
    'export EVOLVER_GENE_ID="' + (selectedGene && selectedGene.id || '') + '"',
    'export EVOLVER_PERSONALITY_KEY="' + (personalitySelection && personalitySelection.personality_key || '') + '"',
    'export EVOLVER_INTENT="' + (mutation && mutation.category || 'innovate') + '"',
    'export EVOLVER_SIGNALS=' + JSON.stringify((Array.isArray(signals) ? signals : []).slice(0, 12)),
    '',
    'echo "[Bridge] Environment variables set for solidify:"',
    'echo "  EVOLVER_MUTATION_ID=$EVOLVER_MUTATION_ID"',
    'echo "  EVOLVER_GENE_ID=$EVOLVER_GENE_ID"',
    'echo "  EVOLVER_INTENT=$EVOLVER_INTENT"',
    'echo "  EVOLVER_SIGNALS=$EVOLVER_SIGNALS"',
    '',
    'echo "[Bridge] Running solidify..."',
