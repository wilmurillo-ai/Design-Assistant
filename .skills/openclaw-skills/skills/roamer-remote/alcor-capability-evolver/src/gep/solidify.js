...
function solidify({ intent, summary, dryRun = false, rollbackOnFailure = true } = {}) {
  ...
  // Fallback logic: when lastRun is missing (e.g., bridge sub-agent in isolated session),
  // try to read mutation/personality from environment variables or prompt artifacts.
  let mutationRaw = lastRun && lastRun.mutation && typeof lastRun.mutation === 'object' ? lastRun.mutation : null;
  let personalityRaw =
    lastRun && lastRun.personality_state && typeof lastRun.personality_state === 'object' ? lastRun.personality_state : null;
  
  // Initialize protocolViolations: when lastRun is missing, start with protocol violation markers
  // (Bridge sub-agent without env vars will fail here; with env vars, we'll clear them below)
  // Note: We set this AFTER env var checks to ensure env vars can clear violations
  let protocolViolations = [];

  // If lastRun state is unavailable (isolated bridge session), try to reconstruct from env vars or artifacts.
  if (process.env.EVOLVER_MUTATION_ID && process.env.EVOLVER_GENE_ID) {
    // Reconstruct mutation object from environment variables
    const mutationId = process.env.EVOLVER_MUTATION_ID;
    const geneId = process.env.EVOLVER_GENE_ID;
    const intent = process.env.EVOLVER_INTENT || 'innovate';
    const signalsRaw = process.env.EVOLVER_SIGNALS ? JSON.parse(process.env.EVOLVER_SIGNALS) : [];
    const signals = Array.isArray(signalsRaw) ? signalsRaw.slice(0, 12) : [];
    ...
    mutationRaw = {
      ...
      target: geneId,
      expected_effect: 'apply GEP protocol patch',
      risk_level: 'medium',
      rationale: 'Reconstructed from bridge sub-agent environment variables',
    };
    
    // Reconstruct personality state with fixed values (from Bridge prompt injection)
    personalityRaw = {
    ...
    // Update signals for signalKey computation
    if (Array.isArray(signals) && signals.length > 0) {
      signals.splice(0, signals.length, ...signals);
    }
    
    // Environment variables available - clear protocol violations
    protocolViolations.splice(0, protocolViolations.length);
  }
  else if (!mutationRaw || !personalityRaw) {
    // Check if we're running in a bridge sub-agent (no last_run) but have mutation_id and personality_key in env
    const mutationId = process.env.EVOLVER_MUTATION_ID || null;
    const personalityKeyEnv = process.env.EVOLVER_PERSONALITY_KEY || null;
    const geneIdEnv = process.env.EVOLVER_GENE_ID || null;
    const signalsEnv = process.env.EVOLVER_SIGNALS ? JSON.parse(process.env.EVOLVER_SIGNALS) : null;
    const intentEnv = process.env.EVOLVER_INTENT || null;

    if (mutationId && signalsEnv && geneIdEnv && personalityKeyEnv) {
      // Reconstruct mutation object from available metadata
      mutationRaw = {
        type: 'Mutation',
        id: mutationId,
        category: intentEnv || 'innovate',
        trigger_signals: signalsEnv.slice(0, 8),
        target: geneIdEnv,
        expected_effect: 'apply GEP protocol patch',
        risk_level: 'medium',
        rationale: 'Reconstructed from bridge sub-agent environment variables',
      };
      // Reconstruct personality state
      personalityRaw = {
        type: 'PersonalityState',
        rigor: 0.7,
        creativity: 0.35,
      ...
      // Update signals for signalKey computation
      if (Array.isArray(signalsEnv) && signalsEnv.length > 0) {
        signals.splice(0, signals.length, ...signalsEnv);
      }
  ...
  // Protocol checks logic:
  // - If lastRun exists (main session): perform full protocol checks
  // - If lastRun is missing (bridge sub-agent): fallback logic already attempted env vars
  //   and cleared protocolViolations if successful; otherwise, protocolViolations remains empty
  //   (Bridge sub-agent without env vars will still have empty array, skipping protocol checks)
  if (lastRun) {
  ...
  // --- Bounty task answer detection ---
  // For external bounty tasks, the bridge agent writes an answer file instead of
  // modifying local code. If blast_radius is 0 and signals include bounty_task/external_task,
  // check for an answer file and use it to create a synthetic capsule.
  const hasBountySignal = Array.isArray(signals) && signals.some(s => s === 'bounty_task' || s === 'external_task');
  const isExternalAnswer = hasBountySignal && blast.files === 0 && blast.lines === 0;
  let bountyAnswerFile = null;
  let bountyAnswerContent = null;
  if (isExternalAnswer) {
    const answerDir = path.join(getEvolutionDir(), 'bounty_answers');
    try {
      if (fs.existsSync(answerDir)) {
        const answerFiles = fs.readdirSync(answerDir).filter(f => f.endsWith('.md') || f.endsWith('.json') || f.endsWith('.txt')).sort();
        // Find the most recent answer file matching the active task
        for (const af of answerFiles) {
          const absPath = path.join(answerDir, af);
          try {
            const content = fs.readFileSync(absPath, 'utf8');
            if (content.trim().length > 50) {
              bountyAnswerFile = absPath;
              bountyAnswerContent = content;
              console.log(`[Solidify] Found bounty answer file: ${af}`);
              break;
            }
      ...
      console.log('[Solidify] Bounty answer scan failed (non-fatal): ' + (e && e.message || e));
    }
  }
  ...
  // For external bounty answers with no local code changes, synthesize a capsule
  // from the answer file. The capsule content is the answer itself, not a git diff.
  let externalAnswerCapsule = false;
  if (success && isExternalAnswer && bountyAnswerContent) {
    // Override blast radius with the answer content size as a proxy metric
    const answerLines = bountyAnswerContent.split('\n').length;
    blast.files = 1;
    blast.lines = answerLines;
    blast.changed_files = ['bounty_answer.md'];
    blast.all_changed_files = ['bounty_answer.md'];
    blast.ignored_files = [];
    externalAnswerCapsule = true;
    console.log(`[Solidify] External bounty answer detected (${answerLines} lines). Synthesizing capsule from answer content.`);
  }
  ...
  if (success) {
    ...
    const autoSummary = externalAnswerCapsule
      ? `固化：外部赏金任务回答 — ${lastRun && lastRun.active_task_title ? lastRun.active_task_title.slice(0, 80) : 'unknown task'}`
      : geneUsed
    ...
    const capsuleDiff = externalAnswerCapsule ? bountyAnswerContent : captureDiffSnapshot(repoRoot);
    const capsuleContent = externalAnswerCapsule
      ? { type: 'bounty_answer', answer: bountyAnswerContent, task_id: (lastRun && lastRun.active_task_id) || null, task_title: (lastRun && lastRun.active_task_title) || null }
      : buildCapsuleContent({ intent, gene: geneUsed, signals, blast, mutation, score });
