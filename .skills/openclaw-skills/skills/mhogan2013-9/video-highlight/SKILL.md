...
version: "1.0.0"
displayName: "Video Highlight — Extract and Export Key Moments"
description: >
...
# Video Highlight — Extract and Export Key Moments
...
Say you have a 2-hour sports game recording and want to extract the best moments and compile them into a 60-second highlight reel — the backend processes it in about 1-2 minutes and hands you a 1080p MP4.

Tip: trimming your source video to the relevant section before uploading speeds up highlight detection.

## Matching Input to Actions

User prompts referencing video highlight, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `video-highlight`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
