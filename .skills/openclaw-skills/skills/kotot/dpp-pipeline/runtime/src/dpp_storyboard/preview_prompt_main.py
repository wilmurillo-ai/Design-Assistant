...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="dpp-preview-prompt",
        description="Generate the composition prompt only (no video generation task). Useful for reviewing prompt quality.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional reference clip duration override in seconds. Use -1 to let the model decide. "
            "Otherwise the value must be between 2 and 12. "
            "If omitted, uses the extracted reference clip duration."
        ),
    )
    parser.add_argument(
        "--retry",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        logger.info("Starting prompt preview (no video generation).")
    ...
    generation_prompt = result.get("prompt_plan", {}).get("generation_prompt", "")

    logger.info("Prompt preview written to %s", output_path)
    print("\n=== generation_prompt ===")
