...
调用大华云平台大模型进行图像分析。**完整的抓图→AI 分析流程，无需设备密码！**
...
**基本格式：**
```
/dahua-ai-device-image-analysis 从 <设备SN> 抓图并分析 <你的问题>
```

或

```
使用 dahua-ai-device-image-analysis 技能，从 <设备SN> 抓图并分析 <你的问题>
```

**示例：**
```
/dahua-ai-device-image-analysis 从 BA5918431 抓一张图，并分析有没有人
```

```
使用 dahua-ai-device-image-analysis 技能，从 BA5918431 抓图，判断图中是否有白色车辆
```
...
```bash
python dahua_ai_device_image_analysis.py --device-sn <设备SN> --prompt "<你的问题>"
```

**示例：**
```bash
python dahua_ai_device_image_analysis.py \
...
**重要**: 本技能不需要设备密码和 Product ID，配置更加简洁安全！
...
```
skills/dahua-ai-device-image-analysis/
├── README.md                         # 本文件
├── SKILL.md                          # Skill 描述文件（含 GUI 设置指南）
├── FAQ.md                            # 常见问题解答
├── dahua_ai_device_image_analysis.py # 主程序
