---
name: picwish-segmentation
description: >-
  Remove background from any image (person, object, stamp) and output transparent PNG.
  Use when user asks to remove background, extract foreground, generate transparent PNG, or cutout.
  Input: image URL or local file (jpg/png/webp/bmp/tiff/raw, max 4096×4096, 20MB).
  Output: transparent PNG image URL.
...
- Resolve output directory:
  ```bash
  node $OC_SCRIPT route-output --skill picwish-segmentation --name tmp --ext tmp
  ```
  - Script not found → 3-level fallback:
    1. cwd has `openclaw.yaml` → `./output/`
    2. `$VISUAL` exists → `$VISUAL/output/picwish-segmentation/`
    3. None → `~/Downloads/`
  - `mkdir -p {output_dir}`
...
- Infer `type` from user intent:
  - Mentions "person" / "people" → `type=person`
  - Mentions "object" / "product" → `type=object`
  - Mentions "stamp" / "seal" → `type=stamp`
  - Not specified → leave empty (auto-detect)
- `output_type`: default `2` (image only); set to `1` or `3` if user asks for mask
- `crop`: default `0`; set to `1` if user asks to "crop to edges"

### 3. Execute

```bash
node scripts/run_task.mjs --skill picwish-segmentation --input-json '{...}' --output-dir '{output_dir}'
```
...
- Rename via workspace helper:
  ```bash
  node $OC_SCRIPT rename --file {path} --name segmentation
  ```
