---
name: picwish-id-photo
description: >-
  Generate standard ID photos (passport, visa, 1-inch, 2-inch) with custom size and background color.
  Use when user needs ID photo, passport photo, visa photo, or solid-background portrait.
  Input: image with face (max 4096×4096, 20MB) + optional size/format/bg_color.
  Output: standard ID photo image URL.
  Requires env PICWISH_API_KEY or openclaw config skills.entries.picwish.apiKey.
  China mainland users set PICWISH_REGION=cn via env or openclaw config.
requirements:
  credentials:
    - name: PICWISH_API_KEY
      source: env | openclaw-config:skills.entries.picwish.apiKey
    - name: PICWISH_REGION
      source: env | openclaw-config:skills.entries.picwish.region
      required: false
      default: global
  permissions:
    - type: file_read
      paths:
        - ~/.openclaw/config.json
        - ~/.openclaw/workspace/scripts/
        - ~/.openclaw/workspace/visual/
        - ./
    - type: file_write
      paths:
        - ~/.openclaw/workspace/visual/output/
        - ./output/
    - type: exec
      commands:
        - node
---

## Workflow

### 1. Preflight

- Resolve image input (must contain face):
  - URL in message → `image_url` | path in message → `image_file` | file attachment → use temp path/URL from platform | none → ask user
  - See root skill "Image Input Resolution" for full priority
- Resolve context:
  ```bash
  node $OC_SCRIPT resolve
  ```
  - Script not found → check cwd for `openclaw.yaml` → determine mode; check `$VISUAL` dir → determine capabilities
- Resolve output directory:
  ```bash
  node $OC_SCRIPT route-output --skill picwish-id-photo --name tmp --ext tmp
  ```
  - Script not found → 3-level fallback:
    1. cwd has `openclaw.yaml` → `./output/`
    2. `$VISUAL` exists → `$VISUAL/output/picwish-id-photo/`
    3. None → `~/Downloads/`
  - `mkdir -p {output_dir}`

> Path aliases: `$VISUAL` = `{OPENCLAW_HOME}/workspace/visual/`, `$OC_SCRIPT` = `{OPENCLAW_HOME}/workspace/scripts/oc-workspace.mjs`

### 2. Build Parameters

- Required: `image_url` or `image_file` (from URL, path, or attachment — see root skill)
- size inference:
  - "1-inch" / "一寸" → size=295x413
  - "2-inch" / "二寸" → size=413x579
  - "passport" / "护照" → size=354x472
  - Specific dimensions → use as given (width/height 100–2000)
  - Not specified → leave empty (keep original size)
- format: transparent background → png; with background color → jpg
- bg_color (only for format=jpg):
  - "white" → FFFFFF
  - "blue" → 438EDB
  - "red" → FF0000
  - User specifies hex → use as given

### 3. Execute

```bash
node scripts/run_task.mjs --skill picwish-id-photo --input-json '{...}' --output-dir '{output_dir}'
```

⚠️ Poll timeout set to 300 seconds (advanced ID photo processing takes longer)

### 4. Deliver

- Rename via workspace helper:
  ```bash
  node $OC_SCRIPT rename --file {path} --name idphoto
  ```
  - Script not found → file already named `{output_dir}/{date}_{skill}.{ext}` by run_task.mjs
- Show `saved_path` as primary result; `result_url` as backup (valid ~1 hour)
- ⚠️ Always copy the **complete** `result_url` verbatim from script output — never strip query parameters (they contain required auth tokens)
