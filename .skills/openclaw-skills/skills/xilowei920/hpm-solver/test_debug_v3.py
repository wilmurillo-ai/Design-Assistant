...
class DataPreprocessor:
    ...
    def filter_by_divisibility(groups: List[EquivalenceGroup], target: int) -> List[EquivalenceGroup]:
        ...
        关键修复：
        1. 考虑 maxqty 限制：低价物品能否买够数量来替代高价物品
        2. 考虑总价值限制：低价物品的总价值是否足够达到目标
        
        如果低价物品无法满足以上任一条件，则保留高价物品
        """
        ...
        for group in groups:
            ...
            for kept_group in kept:
                ...
                if group.actual_price % kept_group.actual_price == 0:
                    ...
                    can_buy_enough = kept_group.maxqty >= ratio
                    
                    # 条件2：低价物品的总价值是否足够达到目标？
                    # 如果目标 > 低价物品的总价值，则需要保留高价物品
                    can_reach_target = kept_group.max_value >= target
                    
                    if can_buy_enough and can_reach_target:
                        can_replace = True
                        print(f"  [DEBUG] 过滤: {group.actual_price} 是 {kept_group.actual_price} 的 {ratio} 倍")
                        print(f"           低价物品 maxqty={kept_group.maxqty} >= ratio={ratio} ✓")
                        print(f"           低价物品 max_value={kept_group.max_value} >= target={target} ✓")
                        print(f"           可以完全替代，跳过")
                        break
                    else:
                        print(f"  [DEBUG] 检查: {group.actual_price} 是 {kept_group.actual_price} 的 {ratio} 倍")
                        if not can_buy_enough:
                            print(f"           低价物品 maxqty={kept_group.maxqty} < ratio={ratio} ✗ 无法买够数量")
                        if not can_reach_target:
                            print(f"           低价物品 max_value={kept_group.max_value} < target={target} ✗ 无法达到目标")
                        print(f"           不能完全替代，保留")
