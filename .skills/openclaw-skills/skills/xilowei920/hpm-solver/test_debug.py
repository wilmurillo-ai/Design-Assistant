...
class DataPreprocessor:
    ...
    @staticmethod
    def filter_by_divisibility(groups: List[EquivalenceGroup]) -> List[EquivalenceGroup]:
        """约数过滤：如果价格B是价格A的整数倍，过滤B"""
        print(f"\n[DEBUG] filter_by_divisibility 输入: {len(groups)} 个等价组")
        ...
        for group in groups:
            ...
            for kept_group in kept:
                if group.actual_price % kept_group.actual_price == 0:
                    can_replace = True
                    print(f"  [DEBUG] 过滤: {group.actual_price} 是 {kept_group.actual_price} 的倍数，跳过")
                    break
            if not can_replace:
                kept.append(group)
...
class HPMSolverDebug:
    ...
    def solve(self, target_diamond: int, items: List[Item]) -> Optional[Solution]:
        ...
        filtered_groups = DataPreprocessor.filter_by_divisibility(groups)
