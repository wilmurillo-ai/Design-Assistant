...
def like_tweet(user_id, tweet_id):
    ...
    if me_resp.status_code != 200:
        raise Exception(f"Failed to get user: {me_resp.text}")
    ...
    if response.status_code == 200:
        ...
        raise Exception(f"Failed to like: {response.status_code} {response.text}")
...
def main():
    ...
    try:
        result = like_tweet(args.user_id, args.tweet_id)
