...
# 卡通漫画风格基础 prompt（参考 xhs/cover-template.md）
CARTOON_STYLE="卡通漫画风格,flat design,明亮色彩,可爱简约,no text,no watermark"

# 自动追加风格关键词（如果用户没有明确指定风格）
if [ "$USER_IMAGE_MODE" = false ]; then
  # 检查 prompt 中是否已包含风格关键词
  if [[ "$PROMPT" != *"卡通"* && "$PROMPT" != *"flat"* && "$PROMPT" != *"漫画"* ]]; then
    PROMPT="${PROMPT},${CARTOON_STYLE}"
  fi
fi
...
# 检查实际渲染高度，超出则缩小字号重试
TB_H=$(identify -format "%h" "$TEXT_BLOCK" 2>/dev/null)
...
# 同样检查宽度，超出则缩小
TB_W=$(identify -format "%w" "$TEXT_BLOCK" 2>/dev/null)
