...
def main():
    parser = argparse.ArgumentParser(description="查询月升月落和月相数据")
    ...
    result = api.request("v7/astronomy/moon", params)

    if not result:
        print("错误: 获取月相数据失败", file=sys.stderr)
        sys.exit(1)
    ...
    if args.raw:
        ...
        print(f"\n📍 {basic.get('location', '')} - {args.date} 月相数据")
        print("=" * 60)

        if moon:
            m = moon[0]
            print(f"🌙 月升: {m.get('moonrise', '--')}")
            print(f"🌑 月落: {m.get('moonset', '--')}")
            print(f"🔆 照明度: {m.get('illumination', '--')}%")
            print(f"🌗 月相: {m.get('moonPhase', '--')}")

            # 显示24小时月相数据
            hourly = m.get('hourly', [])
            if hourly:
                print(f"\n⏰ 24小时月相变化:")
                for h in hourly[:12]:  # 显示前12小时
                    print(f"   {h.get('fxTime', '')} | 照明度: {h.get('illumination', '--')}% | {h.get('moonPhase', '--')}")
