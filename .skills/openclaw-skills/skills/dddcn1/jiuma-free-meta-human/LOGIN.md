...
# 第三步：验证登录状态并获取API密钥
python3 login.py --check --access_token "abc123xyz456"
# 输出示例：
# {
#   "status": "success",
...
```bash
# 获取登录信息
exec python3 login.py --login

# 检查登录状态
exec python3 login.py --check --access_token "your_token_here"
```
...
# 数字人生成技能
python3 jiuma_free_meta_human/generate_video.py  --action "create" --text "{{.text}}" --human_id {{.human_id}} --voice_id {{.voice_id}}
```
...
1. **API密钥安全**：获取的API密钥会自动保存在本地，请确保文件系统安全
2. **登录时效**：登录二维码和access_token通常有有效期限制，建议尽快完成登录
3. **网络要求**：需要能够访问九马AI API服务器（api.jiuma.com）
4. **兼容性**：此技能需要Python 3.6+和requests库