---
name: video-generator-free-best
version: "1.0.0"
displayName: "Video Generator Free Best — Generate Videos from Text or Images"
description: >
  Skip the learning curve of professional editing software. Describe what you want — create a 30-second promotional video from my text and images — and get ready-to-share videos back in 1-2 minutes. Upload MP4, MOV, JPG, PNG files up to 200MB, and the AI handles AI video creation automatically. Ideal for content creators and marketers who want to create videos quickly without expensive tools or editing skills.
...
# Video Generator Free Best — Generate Videos from Text or Images
...
One thing worth knowing — shorter scripts and fewer images produce faster results with cleaner output.

## Matching Input to Actions

User prompts referencing video generator free best, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `video-generator-free-best`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
