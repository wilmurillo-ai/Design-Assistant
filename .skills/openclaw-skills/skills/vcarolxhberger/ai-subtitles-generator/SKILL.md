...
version: "1.0.0"
displayName: "AI Subtitles Generator — Generate and Burn In Captions"
description: >
  add video files into captioned video files with this skill. Works with MP4, MOV, AVI, WebM files up to 500MB. YouTubers and content creators use it for adding subtitles to YouTube videos — processing takes 30-60 seconds on cloud GPUs and you get 1080p MP4 files.
...
# AI Subtitles Generator — Generate and Burn In Captions
...
User prompts referencing ai subtitles generator, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `ai-subtitles-generator`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
