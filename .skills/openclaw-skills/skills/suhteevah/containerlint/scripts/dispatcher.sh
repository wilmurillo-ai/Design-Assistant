#!/usr/bin/env bash
# ContainerLint -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: DF, SC, HC, RS, NW, OR
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[ContainerLint]${NC} $*"; }
log_success() { echo -e "${GREEN}[ContainerLint]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[ContainerLint]${NC} $*"; }
log_error()   { echo -e "${RED}[ContainerLint]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}ContainerLint${NC} v${VERSION}"
  echo "  Docker & container security anti-pattern analyzer"
  echo "  https://containerlint.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: DF, SC, HC, RS, NW, OR${NC}"
  ...
  echo -e "  ${GREEN}FREE${NC}        30 patterns  DF, SC categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  DF, SC, HC, RS"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  DF, SC, HC, RS, NW, OR"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  DF  Dockerfile Best Practices  -- latest tag, ADD vs COPY, chmod 777, stale apt cache"
  echo "  SC  Security Context           -- privileged mode, root user, hardcoded secrets, host PID/IPC"
  echo "  HC  Health & Readiness         -- missing HEALTHCHECK, no graceful shutdown, probe config"
  echo "  RS  Resource Management        -- no memory/CPU limits, unlimited PIDs, missing ulimits"
  echo "  NW  Networking & Exposure      -- exposed database ports, host networking, missing TLS"
  echo "  OR  Orchestration & Compose    -- no restart policy, :latest in compose, plaintext secrets"
  echo ""
  ...
  echo "  dispatcher.sh --path . --category DF --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://containerlint.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export CONTAINERLINT_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_containerlint_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_containerlint_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- ContainerLint Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(containerlint_pattern_count) patterns across 6 categories${NC}"
  echo ""

  containerlint_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "containerlint" "$config" 2>/dev/null; then
...
# --- ContainerLint hooks ----------------------------------------
    containerlint-check:
      run: |
        CONTAINERLINT_SKILL_DIR="${CONTAINERLINT_SKILL_DIR:-$HOME/.openclaw/skills/containerlint}"
        if [[ -f "$CONTAINERLINT_SKILL_DIR/scripts/analyzer.sh" ]]; then
          source "$CONTAINERLINT_SKILL_DIR/scripts/patterns.sh"
          source "$CONTAINERLINT_SKILL_DIR/scripts/analyzer.sh"
          hook_containerlint_check
        else
          echo "[ContainerLint] Skill not found at $CONTAINERLINT_SKILL_DIR -- skipping scan"
        fi
      fail_text: |
        Container security issues detected in staged files!
        Run 'containerlint scan' to see details.
        Or 'git commit --no-verify' to skip (NOT recommended).
HOOKS
    echo -e "${GREEN}+${NC} Appended ContainerLint hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for container security issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "containerlint" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- ContainerLint hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "containerlint" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed ContainerLint hooks"
    else
      log_warn "No ContainerLint hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_containerlint_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_containerlint_tier)")
        local categories
        categories=$(get_containerlint_categories_for_tier "$tier_num")
