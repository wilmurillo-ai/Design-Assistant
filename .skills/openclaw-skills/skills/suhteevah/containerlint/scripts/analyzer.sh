#!/usr/bin/env bash
# ContainerLint -- Core Analysis Engine
...
CONTAINERLINT_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "DF SC HC RS")
...
scan_file() {
  ...
  for category in $categories; do
    ...
    local patterns_name
    patterns_name=$(get_containerlint_patterns_for_category "$category")
...
# Calculate a container security score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in DF SC HC RS NW OR; do
    local cat_count
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Container Security Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>ContainerLint Report -- ${target}</title>
  <style>
    ...
    h1 { color: #0f172a; border-bottom: 3px solid #0891b2; padding-bottom: 0.5rem; }
    ...
    th { background: #0e7490; color: white; padding: 0.75rem; text-align: left; font-size: 0.875rem; }
...
<body>
  <h1>ContainerLint Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${CONTAINERLINT_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Dockerfile Best Practices</td><td>DF</td><td>$(count_by_category "DF")</td></tr>
    <tr><td>Security Context</td><td>SC</td><td>$(count_by_category "SC")</td></tr>
    <tr><td>Health & Readiness</td><td>HC</td><td>$(count_by_category "HC")</td></tr>
    <tr><td>Resource Management</td><td>RS</td><td>$(count_by_category "RS")</td></tr>
    <tr><td>Networking & Exposure</td><td>NW</td><td>$(count_by_category "NW")</td></tr>
    <tr><td>Orchestration & Compose</td><td>OR</td><td>$(count_by_category "OR")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://containerlint.pages.dev">ContainerLint</a> v${CONTAINERLINT_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (container escape, secret exposure)"$'\n'
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- fix privileged containers, exposed ports, missing user directives"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- harden security context and add health checks"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[ContainerLint]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$CONTAINERLINT_VERSION}"
...
# do_containerlint_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "DF")
do_containerlint_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_containerlint_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_containerlint_category "$filter_upper"; then
      echo -e "${RED}[ContainerLint]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: DF, SC, HC, RS, NW, OR" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[ContainerLint]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://containerlint.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[ContainerLint]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(containerlint_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[ContainerLint]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No container security issues detected!${NC}"
        echo ""
        echo -e "Container Security Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_containerlint_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[ContainerLint]${NC} No container security issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[ContainerLint] Container security issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix container security issues before committing."
    echo -e "  Run: ${CYAN}containerlint scan${NC} for details"
