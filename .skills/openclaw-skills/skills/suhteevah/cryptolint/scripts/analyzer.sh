#!/usr/bin/env bash
# CryptoLint -- Core Analysis Engine
...
CRYPTOLINT_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "WA KM EM RN")
...
# Calculate a cryptography quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in WA KM EM RN TC CP; do
    ...
    if [[ $cat_count -gt 0 ]]; then
      local label
      label=$(get_cryptolint_category_label "$cat")
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Cryptographic Security Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>CryptoLint Report -- ${target}</title>
  <style>
...
<body>
  <h1>CryptoLint Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${CRYPTOLINT_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Weak Algorithms</td><td>WA</td><td>$(count_by_category "WA")</td></tr>
    <tr><td>Key Management</td><td>KM</td><td>$(count_by_category "KM")</td></tr>
    <tr><td>Encryption Modes</td><td>EM</td><td>$(count_by_category "EM")</td></tr>
    <tr><td>Random Number Generation</td><td>RN</td><td>$(count_by_category "RN")</td></tr>
    <tr><td>Timing & Comparison</td><td>TC</td><td>$(count_by_category "TC")</td></tr>
    <tr><td>Certificate & Protocol</td><td>CP</td><td>$(count_by_category "CP")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://cryptolint.pages.dev">CryptoLint</a> v${CRYPTOLINT_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (broken algorithms / key exposure)${NEWLINE:-$'\n'}"
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- replace weak algorithms and fix key management"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- upgrade encryption modes and strengthen randomness"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[CryptoLint]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$CRYPTOLINT_VERSION}"
...
# do_cryptolint_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "WA")
do_cryptolint_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_cryptolint_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_cryptolint_category "$filter_upper"; then
      echo -e "${RED}[CryptoLint]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: WA, KM, EM, RN, TC, CP" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[CryptoLint]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://cryptolint.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[CryptoLint]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(cryptolint_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[CryptoLint]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No cryptography issues detected!${NC}"
        echo ""
        echo -e "Cryptographic Security Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_cryptolint_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[CryptoLint]${NC} No cryptography issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[CryptoLint] Cryptography issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix cryptography issues before committing."
    echo -e "  Run: ${CYAN}cryptolint scan${NC} for details"
