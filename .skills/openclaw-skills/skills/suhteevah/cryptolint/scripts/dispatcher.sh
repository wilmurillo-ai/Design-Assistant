#!/usr/bin/env bash
# CryptoLint -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: WA, KM, EM, RN, TC, CP
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[CryptoLint]${NC} $*"; }
log_success() { echo -e "${GREEN}[CryptoLint]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[CryptoLint]${NC} $*"; }
log_error()   { echo -e "${RED}[CryptoLint]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}CryptoLint${NC} v${VERSION}"
  echo "  Cryptography misuse & weak algorithm detector"
  echo "  https://cryptolint.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: WA, KM, EM, RN, TC, CP${NC}"
  ...
  echo -e "  ${GREEN}FREE${NC}        30 patterns  WA, KM categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  WA, KM, EM, RN"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  WA, KM, EM, RN, TC, CP"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  WA  Weak Algorithms         -- MD5, SHA1, DES, RC4, small RSA keys, deprecated ciphers"
  echo "  KM  Key Management          -- hardcoded keys, weak derivation, missing rotation, insecure storage"
  echo "  EM  Encryption Modes        -- ECB mode, missing IV/nonce, nonce reuse, insecure padding"
  echo "  RN  Random Number Gen       -- Math.random, weak PRNGs, predictable seeds, insufficient entropy"
  echo "  TC  Timing & Comparison     -- non-constant-time comparison, timing side channels, unsafe equality"
  echo "  CP  Certificate & Protocol  -- disabled TLS verification, weak protocols, missing cert pinning"
  echo ""
  ...
  echo "  dispatcher.sh --path . --category WA --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://cryptolint.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export CRYPTOLINT_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_cryptolint_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_cryptolint_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- CryptoLint Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(cryptolint_pattern_count) patterns across 6 categories${NC}"
  echo ""

  cryptolint_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "cryptolint" "$config" 2>/dev/null; then
...
# --- CryptoLint hooks ----------------------------------------
    cryptolint-check:
      run: |
        CRYPTOLINT_SKILL_DIR="${CRYPTOLINT_SKILL_DIR:-$HOME/.openclaw/skills/cryptolint}"
        if [[ -f "$CRYPTOLINT_SKILL_DIR/scripts/analyzer.sh" ]]; then
          source "$CRYPTOLINT_SKILL_DIR/scripts/patterns.sh"
          source "$CRYPTOLINT_SKILL_DIR/scripts/analyzer.sh"
          hook_cryptolint_check
        else
          echo "[CryptoLint] Skill not found at $CRYPTOLINT_SKILL_DIR -- skipping scan"
        fi
      fail_text: |
        Cryptography issues detected in staged files!
        Run 'cryptolint scan' to see details.
        Or 'git commit --no-verify' to skip (NOT recommended).
HOOKS
    echo -e "${GREEN}+${NC} Appended CryptoLint hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for cryptography issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "cryptolint" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- CryptoLint hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "cryptolint" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed CryptoLint hooks"
    else
      log_warn "No CryptoLint hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_cryptolint_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_cryptolint_tier)")
        local categories
        categories=$(get_cryptolint_categories_for_tier "$tier_num")
