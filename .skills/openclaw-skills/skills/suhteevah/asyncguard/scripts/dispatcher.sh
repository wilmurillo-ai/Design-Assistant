#!/usr/bin/env bash
# AsyncGuard -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: PM, AR, EL, CA, AE, AC
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[AsyncGuard]${NC} $*"; }
log_success() { echo -e "${GREEN}[AsyncGuard]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[AsyncGuard]${NC} $*"; }
log_error()   { echo -e "${RED}[AsyncGuard]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}AsyncGuard${NC} v${VERSION}"
  echo "  Async/await anti-pattern analyzer"
  echo "  https://asyncguard.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: PM, AR, EL, CA, AE, AC${NC}"
  echo -e "  --help, -h             ${DIM}Show this help${NC}"
  echo -e "  --version, -v          ${DIM}Show version${NC}"
  echo ""
  echo -e "${BOLD}COMMANDS${NC}"
  echo -e "  scan                   ${DIM}Run async anti-pattern scan (default)${NC}"
  echo -e "  status                 ${DIM}Show license and configuration info${NC}"
  echo -e "  patterns               ${DIM}List all detection patterns${NC}"
  echo -e "  hooks [install|uninstall]  ${DIM}Manage git hooks (Pro+)${NC}"
  echo -e "  report [path]          ${DIM}Generate markdown report (Pro+)${NC}"
  echo ""
  echo -e "${BOLD}TIER SYSTEM${NC}"
  echo -e "  ${GREEN}FREE${NC}        30 patterns  PM, AR categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  PM, AR, EL, CA"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  PM, AR, EL, CA, AE, AC"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  PM  Promise/Future Misuse    -- unhandled promises, async executor, nested .then chains"
  echo "  AR  Async Resource Leaks     -- unclosed connections, missing dispose, dangling streams"
  echo "  EL  Event Loop Blocking      -- sync I/O, sync crypto, CPU-intensive loops"
  echo "  CA  Cancellation & Abortion  -- missing AbortSignal, no cleanup, orphaned tasks"
  echo "  AE  Async Error Patterns     -- swallowed rejections, empty .catch, async forEach"
  echo "  AC  Async Coordination       -- unbounded Promise.all, missing semaphore, no backpressure"
  echo ""
  ...
  echo "  dispatcher.sh --path . --category EL --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://asyncguard.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export ASYNCGUARD_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_asyncguard_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_asyncguard_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- AsyncGuard Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(asyncguard_pattern_count) patterns across 6 categories${NC}"
  echo ""

  asyncguard_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "asyncguard" "$config" 2>/dev/null; then
...
HOOKS
    echo -e "${GREEN}+${NC} Appended AsyncGuard hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for async issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "asyncguard" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- AsyncGuard hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "asyncguard" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed AsyncGuard hooks"
    else
      log_warn "No AsyncGuard hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_asyncguard_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_asyncguard_tier)")
        local categories
        categories=$(get_asyncguard_categories_for_tier "$tier_num")
