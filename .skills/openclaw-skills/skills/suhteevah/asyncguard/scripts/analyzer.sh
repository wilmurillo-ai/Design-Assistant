#!/usr/bin/env bash
# AsyncGuard -- Core Analysis Engine
...
ASYNCGUARD_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "PM AR EL CA")
...
scan_file() {
  ...
  for category in $categories; do
    ...
    local patterns_name
    patterns_name=$(get_asyncguard_patterns_for_category "$category")
...
# Calculate an async safety score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in PM AR EL CA AE AC; do
    local cat_count
    cat_count=$(count_by_category "$cat")
    if [[ $cat_count -gt 0 ]]; then
      local label
      label=$(get_asyncguard_category_label "$cat")
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Async Safety Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>AsyncGuard Report -- ${target}</title>
  <style>
    ...
    h1 { color: #0f172a; border-bottom: 3px solid #fdcb6e; padding-bottom: 0.5rem; }
...
<body>
  <h1>AsyncGuard Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${ASYNCGUARD_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Promise/Future Misuse</td><td>PM</td><td>$(count_by_category "PM")</td></tr>
    <tr><td>Async Resource Leaks</td><td>AR</td><td>$(count_by_category "AR")</td></tr>
    <tr><td>Event Loop Blocking</td><td>EL</td><td>$(count_by_category "EL")</td></tr>
    <tr><td>Cancellation & Abortion</td><td>CA</td><td>$(count_by_category "CA")</td></tr>
    <tr><td>Async Error Patterns</td><td>AE</td><td>$(count_by_category "AE")</td></tr>
    <tr><td>Async Coordination</td><td>AC</td><td>$(count_by_category "AC")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://asyncguard.pages.dev">AsyncGuard</a> v${ASYNCGUARD_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- fix immediately (resource leaks / unhandled rejections)"$'\n'
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- add cancellation handling and cleanup"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- improve async coordination and error handling"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[AsyncGuard]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$ASYNCGUARD_VERSION}"
...
# do_asyncguard_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "EL")
do_asyncguard_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_asyncguard_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_asyncguard_category "$filter_upper"; then
      echo -e "${RED}[AsyncGuard]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: PM, AR, EL, CA, AE, AC" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[AsyncGuard]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://asyncguard.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[AsyncGuard]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(asyncguard_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[AsyncGuard]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No async issues detected!${NC}"
        echo ""
        echo -e "Async Safety Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_asyncguard_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[AsyncGuard]${NC} No async issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[AsyncGuard] Async issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix async issues before committing."
    echo -e "  Run: ${CYAN}asyncguard scan${NC} for details"
