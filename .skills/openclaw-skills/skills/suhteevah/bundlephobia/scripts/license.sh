#!/usr/bin/env bash
# BundlePhobia — License Validation Module
# JWT-based offline validation — no phone-home, no telemetry
...
BUNDLEPHOBIA_LICENSE_KEY="${BUNDLEPHOBIA_LICENSE_KEY:-}"
...
get_bundlephobia_key() {
  if [[ -n "$BUNDLEPHOBIA_LICENSE_KEY" ]]; then
    echo "$BUNDLEPHOBIA_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('bundlephobia', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.bundlephobia?.apiKey || '');
} catch(e) {}
" 2>/dev/null)
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.bundlephobia.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
validate_license() {
  ...
  if [[ "$product" != "bundlephobia" && -n "$product" ]]; then
    return 1
  fi
...
check_bundlephobia_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  local key
  if ! key=$(get_bundlephobia_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}━━━ BundlePhobia License Required ━━━${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://bundlephobia.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"bundlephobia\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export BUNDLEPHOBIA_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi

  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[BundlePhobia]${NC} Invalid license key."
    echo -e "Get a valid license at: ${CYAN}https://bundlephobia.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  # Validate product
  if [[ -n "$product" && "$product" != "bundlephobia" ]]; then
    echo -e "${RED}[BundlePhobia]${NC} License is for product '${BOLD}$product${NC}', not bundlephobia."
    echo -e "Get a BundlePhobia license at: ${CYAN}https://bundlephobia.pages.dev/pricing${NC}"
    return 1
  fi

  local actual_level="${TIER_LEVELS[$tier]:-0}"
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[BundlePhobia]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://bundlephobia.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[BundlePhobia]${NC} License expired. Renew at: ${CYAN}https://bundlephobia.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
show_bundlephobia_status() {
  ...
  if ! key=$(get_bundlephobia_key) || [[ -z "$key" ]]; then
    ...
    echo -e "  Available: scan (5 file limit), patterns"
    echo -e "  Upgrade at: ${CYAN}https://bundlephobia.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "bundlephobia" ]]; then
    ...
    echo -e "  This key is for $product, not BundlePhobia."
    echo -e "  Get a BundlePhobia license at: ${CYAN}https://bundlephobia.pages.dev/pricing${NC}"
    return
  fi
  ...
  case "$tier" in
    pro)
      echo -e "  Available: scan (unlimited), hooks, report, audit"
      echo -e "  Upgrade to Team for: budget, sarif, ci"
      ;;
    team)
      echo -e "  Available: scan, hooks, report, audit, budget, sarif, ci"
      ;;
    enterprise)
