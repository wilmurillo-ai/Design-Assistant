#!/usr/bin/env bash
# AccessLint — License Validation Module
...
ACCESSLINT_LICENSE_KEY="${ACCESSLINT_LICENSE_KEY:-}"
...
get_accesslint_key() {
  if [[ -n "$ACCESSLINT_LICENSE_KEY" ]]; then
    echo "$ACCESSLINT_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('accesslint', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.accesslint?.apiKey || '');
} catch(e) {}
" 2>/dev/null)
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.accesslint.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
validate_license() {
  ...
  if [[ "$product" != "accesslint" && -n "$product" ]]; then
    return 1
  fi
...
check_accesslint_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  local key
  if ! key=$(get_accesslint_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}--- AccessLint License Required ---${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://accesslint.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"accesslint\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export ACCESSLINT_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi

  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[AccessLint]${NC} Invalid license key."
    echo -e "Get a valid license at: ${CYAN}https://accesslint.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  # Validate product
  if [[ -n "$product" && "$product" != "accesslint" ]]; then
    echo -e "${RED}[AccessLint]${NC} License is for product '${BOLD}$product${NC}', not accesslint."
    echo -e "Get an AccessLint license at: ${CYAN}https://accesslint.pages.dev/pricing${NC}"
    return 1
  fi

  local actual_level="${TIER_LEVELS[$tier]:-0}"
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[AccessLint]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://accesslint.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[AccessLint]${NC} License expired. Renew at: ${CYAN}https://accesslint.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
show_accesslint_status() {
  ...
  if ! key=$(get_accesslint_key) || [[ -z "$key" ]]; then
    ...
    echo -e "  Upgrade at: ${CYAN}https://accesslint.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "accesslint" ]]; then
    ...
    echo -e "  This key is for $product, not AccessLint."
    echo -e "  Get an AccessLint license at: ${CYAN}https://accesslint.pages.dev/pricing${NC}"
    return
  fi
  ...
  case "$tier" in
    pro)
      ...
      echo -e "  Upgrade to Team for: policy, sarif, wcag"
      ;;
    team)
      echo -e "  Available: scan, hooks, report, audit, policy, sarif, wcag"
      ;;
    enterprise)
