#!/usr/bin/env bash
# EventLint -- Core Analysis Engine
...
EVENTLINT_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "PP CP MS ED")
...
# Calculate an event quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in PP CP MS ED OD EO; do
    ...
    if [[ $cat_count -gt 0 ]]; then
      local label
      label=$(get_eventlint_category_label "$cat")
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Event Architecture Quality Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>EventLint Report -- ${target}</title>
  <style>
    ...
    h1 { color: #0f172a; border-bottom: 3px solid #00b894; padding-bottom: 0.5rem; }
...
<body>
  <h1>EventLint Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${EVENTLINT_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Producer Patterns</td><td>PP</td><td>$(count_by_category "PP")</td></tr>
    <tr><td>Consumer Patterns</td><td>CP</td><td>$(count_by_category "CP")</td></tr>
    <tr><td>Message Schema</td><td>MS</td><td>$(count_by_category "MS")</td></tr>
    <tr><td>Error & Dead Letter</td><td>ED</td><td>$(count_by_category "ED")</td></tr>
    <tr><td>Ordering & Delivery</td><td>OD</td><td>$(count_by_category "OD")</td></tr>
    <tr><td>Event Observability</td><td>EO</td><td>$(count_by_category "EO")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://eventlint.pages.dev">EventLint</a> v${EVENTLINT_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (message loss / infinite redelivery / dual-write)"$'\n'
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- add DLQ, idempotency, and schema validation"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- improve message contracts and consumer resilience"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
    recommendations="${recommendations}4. **LOW:** ${low_count} low findings -- review for observability and documentation gaps"$'\n'
  fi
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[EventLint]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$EVENTLINT_VERSION}"
...
# do_eventlint_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "MS")
do_eventlint_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_eventlint_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_eventlint_category "$filter_upper"; then
      echo -e "${RED}[EventLint]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: PP, CP, MS, ED, OD, EO" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[EventLint]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://eventlint.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[EventLint]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(eventlint_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[EventLint]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No event architecture issues detected!${NC}"
        echo ""
        echo -e "Event Architecture Quality Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_eventlint_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[EventLint]${NC} No event architecture issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[EventLint] Event architecture issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix event architecture issues before committing."
    echo -e "  Run: ${CYAN}eventlint scan${NC} for details"
