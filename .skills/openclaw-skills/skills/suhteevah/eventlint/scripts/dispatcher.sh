#!/usr/bin/env bash
# EventLint -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: PP, CP, MS, ED, OD, EO
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[EventLint]${NC} $*"; }
log_success() { echo -e "${GREEN}[EventLint]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[EventLint]${NC} $*"; }
log_error()   { echo -e "${RED}[EventLint]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}EventLint${NC} v${VERSION}"
  echo "  Event & message queue anti-pattern analyzer"
  echo "  https://eventlint.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: PP, CP, MS, ED, OD, EO${NC}"
  ...
  echo -e "  ${GREEN}FREE${NC}        30 patterns  PP, CP categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  PP, CP, MS, ED"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  PP, CP, MS, ED, OD, EO"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  PP  Producer Patterns    -- fire-and-forget, missing keys, no schema validation"
  echo "  CP  Consumer Patterns    -- no idempotency, auto-ack, unbounded batch, blocking"
  echo "  MS  Message Schema       -- no registry, unversioned, breaking changes, loose types"
  echo "  ED  Error & Dead Letter  -- missing DLQ, infinite redelivery, swallowed exceptions"
  echo "  OD  Ordering & Delivery  -- dual-write, no outbox, missing dedup, race conditions"
  echo "  EO  Event Observability  -- no tracing, missing metrics, no audit, no alerting"
  echo ""
  ...
  echo "  dispatcher.sh --path . --category ED --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://eventlint.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export EVENTLINT_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_eventlint_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_eventlint_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- EventLint Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(eventlint_pattern_count) patterns across 6 categories${NC}"
  echo ""

  eventlint_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "eventlint" "$config" 2>/dev/null; then
...
# --- EventLint hooks ----------------------------------------
    eventlint-check:
      run: |
        EVENTLINT_SKILL_DIR="${EVENTLINT_SKILL_DIR:-$HOME/.openclaw/skills/eventlint}"
        if [[ -f "$EVENTLINT_SKILL_DIR/scripts/analyzer.sh" ]]; then
          source "$EVENTLINT_SKILL_DIR/scripts/patterns.sh"
          source "$EVENTLINT_SKILL_DIR/scripts/analyzer.sh"
          hook_eventlint_check
        else
          echo "[EventLint] Skill not found at $EVENTLINT_SKILL_DIR -- skipping scan"
        fi
      fail_text: |
        Event architecture issues detected in staged files!
        Run 'eventlint scan' to see details.
        Or 'git commit --no-verify' to skip (NOT recommended).
HOOKS
    echo -e "${GREEN}+${NC} Appended EventLint hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for event architecture issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "eventlint" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- EventLint hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "eventlint" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed EventLint hooks"
    else
      log_warn "No EventLint hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_eventlint_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_eventlint_tier)")
        local categories
        categories=$(get_eventlint_categories_for_tier "$tier_num")
