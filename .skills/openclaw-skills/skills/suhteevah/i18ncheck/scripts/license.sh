#!/usr/bin/env bash
# I18nCheck -- License Validation Module
...
I18NCHECK_LICENSE_KEY="${I18NCHECK_LICENSE_KEY:-}"
...
# ---------------------------------------------------------------------------
# Key retrieval -- check env var first, then openclaw.json config
# ---------------------------------------------------------------------------

get_i18ncheck_key() {
  # 1. Check environment variable
  if [[ -n "$I18NCHECK_LICENSE_KEY" ]]; then
    echo "$I18NCHECK_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('i18ncheck', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.i18ncheck?.apiKey || '');
} catch(e) {}
" 2>/dev/null)
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.i18ncheck.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
# ---------------------------------------------------------------------------
# JWT payload decoding -- 3 fallbacks (base64 -d, base64 -D, openssl)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# JSON field extraction -- 3 fallbacks (python3, node, jq)
# ---------------------------------------------------------------------------
...
validate_license() {
  ...
  if [[ "$product" != "i18ncheck" && -n "$product" ]]; then
    return 1
  fi
...
# Main license gate -- validates key, tier, product, and expiry
# ---------------------------------------------------------------------------

check_i18ncheck_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  # Get the license key
  local key
  if ! key=$(get_i18ncheck_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}--- I18nCheck License Required ---${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://i18ncheck.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"i18ncheck\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export I18NCHECK_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi
  ...
  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[I18nCheck]${NC} Invalid license key."
    echo -e "Get a valid license at: ${CYAN}https://i18ncheck.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  # Validate product
  if [[ -n "$product" && "$product" != "i18ncheck" ]]; then
    echo -e "${RED}[I18nCheck]${NC} License is for product '${BOLD}$product${NC}', not i18ncheck."
    echo -e "Get an I18nCheck license at: ${CYAN}https://i18ncheck.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[I18nCheck]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://i18ncheck.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[I18nCheck]${NC} License expired. Renew at: ${CYAN}https://i18ncheck.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
# ---------------------------------------------------------------------------
# get_i18ncheck_tier -- returns current tier as string
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# show_i18ncheck_status -- display license info and available features
# ---------------------------------------------------------------------------
...
show_i18ncheck_status() {
  ...
  if ! key=$(get_i18ncheck_key) || [[ -z "$key" ]]; then
    ...
    echo -e "  Upgrade at: ${CYAN}https://i18ncheck.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "i18ncheck" ]]; then
    ...
    echo -e "  This key is for $product, not I18nCheck."
    echo -e "  Get an I18nCheck license at: ${CYAN}https://i18ncheck.pages.dev/pricing${NC}"
    return
  fi
  ...
  case "$tier" in
    pro)
      ...
      echo -e "  Upgrade to Team for: team-report, baseline"
      ;;
    team)
