#!/usr/bin/env bash
# LogSentry -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: SL, LL, SD, LI, CI, OB
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[LogSentry]${NC} $*"; }
log_success() { echo -e "${GREEN}[LogSentry]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[LogSentry]${NC} $*"; }
log_error()   { echo -e "${RED}[LogSentry]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}LogSentry${NC} v${VERSION}"
  echo "  Logging quality & observability analyzer"
  echo "  https://logsentry.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: SL, LL, SD, LI, CI, OB${NC}"
  ...
  echo -e "  ${GREEN}FREE${NC}        30 patterns  SL, LL categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  SL, LL, SD, LI"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  SL, LL, SD, LI, CI, OB"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  SL  Structured Logging    -- missing loggers, print statements, concatenation"
  echo "  LL  Log Levels            -- incorrect levels, debug for errors, inconsistency"
  echo "  SD  Sensitive Data        -- passwords, tokens, PII, credit cards in logs"
  echo "  LI  Log Injection         -- unsanitized input, CRLF, format string attacks"
  echo "  CI  Correlation & Context -- missing request IDs, trace IDs, timestamps"
  echo "  OB  Observability         -- missing metrics, health checks, audit trails"
  echo ""
  ...
  echo "  dispatcher.sh --path . --category SD --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://logsentry.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export LOGSENTRY_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_logsentry_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_logsentry_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- LogSentry Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(logsentry_pattern_count) patterns across 6 categories${NC}"
  echo ""

  logsentry_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "logsentry" "$config" 2>/dev/null; then
...
HOOKS
    echo -e "${GREEN}+${NC} Appended LogSentry hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for logging issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "logsentry" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- LogSentry hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "logsentry" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed LogSentry hooks"
    else
      log_warn "No LogSentry hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_logsentry_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_logsentry_tier)")
        local categories
        categories=$(get_logsentry_categories_for_tier "$tier_num")
