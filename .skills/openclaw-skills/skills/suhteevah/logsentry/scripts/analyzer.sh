#!/usr/bin/env bash
# LogSentry -- Core Analysis Engine
...
LOGSENTRY_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "SL LL SD LI")
...
# Calculate a logging quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in SL LL SD LI CI OB; do
    ...
    if [[ $cat_count -gt 0 ]]; then
      local label
      label=$(get_logsentry_category_label "$cat")
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Logging Quality Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>LogSentry Report -- ${target}</title>
  <style>
...
<body>
  ...
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${LOGSENTRY_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Structured Logging</td><td>SL</td><td>$(count_by_category "SL")</td></tr>
    <tr><td>Log Levels</td><td>LL</td><td>$(count_by_category "LL")</td></tr>
    <tr><td>Sensitive Data</td><td>SD</td><td>$(count_by_category "SD")</td></tr>
    <tr><td>Log Injection</td><td>LI</td><td>$(count_by_category "LI")</td></tr>
    <tr><td>Correlation & Context</td><td>CI</td><td>$(count_by_category "CI")</td></tr>
  ...
  <div class="footer">
    Generated by <a href="https://logsentry.pages.dev">LogSentry</a> v${LOGSENTRY_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (sensitive data / injection)${NEWLINE:-$'\n'}"
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- replace print statements with structured loggers"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- fix log levels and add context fields"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[LogSentry]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$LOGSENTRY_VERSION}"
...
# do_logsentry_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "SD")
do_logsentry_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_logsentry_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_logsentry_category "$filter_upper"; then
      echo -e "${RED}[LogSentry]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: SL, LL, SD, LI, CI, OB" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[LogSentry]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://logsentry.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[LogSentry]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(logsentry_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[LogSentry]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No logging issues detected!${NC}"
        echo ""
        echo -e "Logging Quality Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_logsentry_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[LogSentry]${NC} No logging issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[LogSentry] Logging issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix logging issues before committing."
    echo -e "  Run: ${CYAN}logsentry scan${NC} for details"
