#!/usr/bin/env bash
# RegexGuard -- Core Analysis Engine
...
REGEXGUARD_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "CB PE CE MA")
...
# Calculate a regex quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in CB PE CE MA AN PI; do
    ...
    if [[ $cat_count -gt 0 ]]; then
      local label
      label=$(get_regexguard_category_label "$cat")
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Regex Quality Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>RegexGuard Report -- ${target}</title>
  <style>
    ...
    h1 { color: #0f172a; border-bottom: 3px solid #f87171; padding-bottom: 0.5rem; }
...
<body>
  <h1>RegexGuard Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${REGEXGUARD_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Catastrophic Backtracking</td><td>CB</td><td>$(count_by_category "CB")</td></tr>
    <tr><td>Portability Errors</td><td>PE</td><td>$(count_by_category "PE")</td></tr>
    <tr><td>Correctness Errors</td><td>CE</td><td>$(count_by_category "CE")</td></tr>
    <tr><td>Maintainability Issues</td><td>MA</td><td>$(count_by_category "MA")</td></tr>
    <tr><td>Anchoring & Boundaries</td><td>AN</td><td>$(count_by_category "AN")</td></tr>
    <tr><td>Pattern Injection</td><td>PI</td><td>$(count_by_category "PI")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://regexguard.pages.dev">RegexGuard</a> v${REGEXGUARD_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- fix immediately (ReDoS, injection vectors, missing validation anchors)"$'\n'
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- fix portability issues, correctness bugs, and anchoring gaps"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- address maintainability, engine compatibility, and boundary issues"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
    recommendations="${recommendations}4. **LOW:** ${low_count} low findings -- review best practices, simplify verbose quantifiers, add comments"$'\n'
  fi
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[RegexGuard]${NC} Report template not found: $template_file" >&2
    return 1
  fi

  local tier_name="unknown"
  local tier_level="${CURRENT_TIER_LEVEL:-0}"
  case "$tier_level" in
    0) tier_name="Free" ;;
    1) tier_name="Pro" ;;
    2) tier_name="Team" ;;
    3) tier_name="Enterprise" ;;
  esac

  local report
  report=$(cat "$template_file")
  report="${report//\{\{TOOL_NAME\}\}/RegexGuard}"
  report="${report//\{\{VERSION\}\}/$REGEXGUARD_VERSION}"
  report="${report//\{\{SCAN_DATE\}\}/$date_str}"
  report="${report//\{\{TARGET_PATH\}\}/$target}"
  report="${report//\{\{TIER\}\}/$tier_name}"
  report="${report//\{\{TOTAL_FILES\}\}/$files_scanned}"
  report="${report//\{\{TOTAL_FINDINGS\}\}/$total_issues}"
  report="${report//\{\{OVERALL_SCORE\}\}/$score}"
  report="${report//\{\{OVERALL_GRADE\}\}/$grade}"
  report="${report//\{\{PASS_FAIL\}\}/$pass_fail}"
  ...
  report="${report//\{\{CATEGORY_SUMMARY\}\}/$cat_breakdown}"
...
# do_regexguard_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "CB")
do_regexguard_scan() {
  local target="${1:-.}"
  ...
  # Store tier level for report generation
  ...
  local categories
  categories=$(get_regexguard_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_regexguard_category "$filter_upper"; then
      echo -e "${RED}[RegexGuard]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: CB, PE, CE, MA, AN, PI" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[RegexGuard]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://regexguard.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[RegexGuard]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(regexguard_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[RegexGuard]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No regex issues detected!${NC}"
        echo ""
        echo -e "Regex Quality Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_regexguard_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[RegexGuard]${NC} No regex issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[RegexGuard] Regex issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix regex issues before committing."
    echo -e "  Run: ${CYAN}regexguard scan${NC} for details"
