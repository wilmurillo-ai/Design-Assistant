#!/usr/bin/env bash
# RegexGuard -- License Validation Module
...
REGEXGUARD_LICENSE_KEY="${REGEXGUARD_LICENSE_KEY:-}"
...
# Retrieve the RegexGuard license key from environment variable or config file.
# Priority: ENV var > openclaw.json (python3 > node > jq fallbacks)
get_regexguard_key() {
  # 1. Check environment variable
  if [[ -n "$REGEXGUARD_LICENSE_KEY" ]]; then
    echo "$REGEXGUARD_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('regexguard', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.regexguard?.apiKey || '');
} catch(e) {}
" 2>/dev/null)

    # Try jq third
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.regexguard.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
validate_license() {
  ...
  # Validate product matches regexguard
  if [[ "$product" != "regexguard" && -n "$product" ]]; then
    return 1
  fi
...
check_regexguard_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  # Step 1: Retrieve the license key
  local key
  if ! key=$(get_regexguard_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}--- RegexGuard License Required ---${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://regexguard.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"regexguard\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export REGEXGUARD_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi
  ...
  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[RegexGuard]${NC} Invalid license key format."
    echo -e "Get a valid license at: ${CYAN}https://regexguard.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  if [[ -n "$product" && "$product" != "regexguard" ]]; then
    echo -e "${RED}[RegexGuard]${NC} License is for product '${BOLD}$product${NC}', not regexguard."
    echo -e "Get a RegexGuard license at: ${CYAN}https://regexguard.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[RegexGuard]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://regexguard.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[RegexGuard]${NC} License expired. Renew at: ${CYAN}https://regexguard.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
show_regexguard_status() {
  ...
  if ! key=$(get_regexguard_key) || [[ -z "$key" ]]; then
    ...
    echo -e "  Patterns: 30 (CB, PE categories)"
    echo -e "  Upgrade at: ${CYAN}https://regexguard.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "regexguard" ]]; then
    ...
    echo -e "  This key is for $product, not RegexGuard."
    echo -e "  Get a RegexGuard license at: ${CYAN}https://regexguard.pages.dev/pricing${NC}"
    return
  fi
  ...
  case "$tier" in
    pro)
      echo -e "  Patterns: 60 (CB, PE, CE, MA categories)"
      echo -e "  Formats: text, json, html"
      echo -e "  Upgrade to Team for: AN, PI categories + full 90 patterns"
      ;;
    team)
