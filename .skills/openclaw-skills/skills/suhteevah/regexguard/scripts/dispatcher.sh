#!/usr/bin/env bash
# RegexGuard -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: CB, PE, CE, MA, AN, PI
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[RegexGuard]${NC} $*"; }
log_success() { echo -e "${GREEN}[RegexGuard]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[RegexGuard]${NC} $*"; }
log_error()   { echo -e "${RED}[RegexGuard]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}RegexGuard${NC} v${VERSION}"
  echo "  Regex safety & correctness analyzer"
  echo "  https://regexguard.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: CB, PE, CE, MA, AN, PI${NC}"
  echo -e "  --help, -h             ${DIM}Show this help${NC}"
  echo -e "  --version, -v          ${DIM}Show version${NC}"
  echo ""
  echo -e "${BOLD}COMMANDS${NC}"
  echo -e "  scan                   ${DIM}Run regex safety & correctness scan (default)${NC}"
  ...
  echo -e "  ${GREEN}FREE${NC}        30 patterns  CB, PE categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  CB, PE, CE, MA"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  CB, PE, CE, MA, AN, PI"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  CB  Catastrophic Backtracking -- nested quantifiers, exponential patterns, ReDoS"
  echo "  PE  Portability Errors        -- non-POSIX features, engine-specific syntax"
  echo "  CE  Correctness Errors        -- unescaped dots, bad ranges, greedy vs lazy"
  echo "  MA  Maintainability Issues    -- overly complex patterns, no comments, duplication"
  echo "  AN  Anchoring & Boundaries    -- missing ^/$, \\b misuse, multiline flag gaps"
  echo "  PI  Pattern Injection         -- unsanitized user input in regex, DoS vectors"
  echo ""
  ...
  echo "  dispatcher.sh --path . --category CB --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://regexguard.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export REGEXGUARD_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_regexguard_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_regexguard_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- RegexGuard Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(regexguard_pattern_count) patterns across 6 categories${NC}"
  echo ""

  regexguard_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "regexguard" "$config" 2>/dev/null; then
...
# --- RegexGuard hooks ----------------------------------------
    regexguard-check:
      run: |
        REGEXGUARD_SKILL_DIR="${REGEXGUARD_SKILL_DIR:-$HOME/.openclaw/skills/regexguard}"
        if [[ -f "$REGEXGUARD_SKILL_DIR/scripts/analyzer.sh" ]]; then
          source "$REGEXGUARD_SKILL_DIR/scripts/patterns.sh"
          source "$REGEXGUARD_SKILL_DIR/scripts/analyzer.sh"
          hook_regexguard_check
        else
          echo "[RegexGuard] Skill not found at $REGEXGUARD_SKILL_DIR -- skipping scan"
        fi
      fail_text: |
        Regex issues detected in staged files!
        Run 'regexguard scan' to see details.
        Or 'git commit --no-verify' to skip (NOT recommended).
HOOKS
    echo -e "${GREEN}+${NC} Appended RegexGuard hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for regex issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "regexguard" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- RegexGuard hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "regexguard" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed RegexGuard hooks"
    else
      log_warn "No RegexGuard hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_regexguard_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_regexguard_tier)")
        local categories
        categories=$(get_regexguard_categories_for_tier "$tier_num")
