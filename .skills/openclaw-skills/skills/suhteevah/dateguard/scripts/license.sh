#!/usr/bin/env bash
# DateGuard -- License Validation Module
...
DATEGUARD_LICENSE_KEY="${DATEGUARD_LICENSE_KEY:-}"
...
# Retrieve the DateGuard license key from environment variable or config file.
# Priority: ENV var > openclaw.json (python3 > node > jq fallbacks)
get_dateguard_key() {
  # 1. Check environment variable
  if [[ -n "$DATEGUARD_LICENSE_KEY" ]]; then
    echo "$DATEGUARD_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('dateguard', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.dateguard?.apiKey || '');
} catch(e) {}
" 2>/dev/null)

    # Try jq third
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.dateguard.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
validate_license() {
  ...
  # Validate product matches dateguard
  if [[ "$product" != "dateguard" && -n "$product" ]]; then
    return 1
  fi
...
check_dateguard_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  # Step 1: Retrieve the license key
  local key
  if ! key=$(get_dateguard_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}--- DateGuard License Required ---${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://dateguard.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"dateguard\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export DATEGUARD_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi
  ...
  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[DateGuard]${NC} Invalid license key format."
    echo -e "Get a valid license at: ${CYAN}https://dateguard.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  if [[ -n "$product" && "$product" != "dateguard" ]]; then
    echo -e "${RED}[DateGuard]${NC} License is for product '${BOLD}$product${NC}', not dateguard."
    echo -e "Get a DateGuard license at: ${CYAN}https://dateguard.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[DateGuard]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://dateguard.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[DateGuard]${NC} License expired. Renew at: ${CYAN}https://dateguard.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
show_dateguard_status() {
  ...
  if ! key=$(get_dateguard_key) || [[ -z "$key" ]]; then
    ...
    echo -e "  Patterns: 30 (TZ, NF categories)"
    echo -e "  Upgrade at: ${CYAN}https://dateguard.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "dateguard" ]]; then
    ...
    echo -e "  This key is for $product, not DateGuard."
    echo -e "  Get a DateGuard license at: ${CYAN}https://dateguard.pages.dev/pricing${NC}"
    return
  fi
  ...
  case "$tier" in
    pro)
      echo -e "  Patterns: 60 (TZ, NF, EP, DA categories)"
      echo -e "  Formats: text, json, html"
      echo -e "  Upgrade to Team for: CP, ST categories + full 90 patterns"
      ;;
    team)
