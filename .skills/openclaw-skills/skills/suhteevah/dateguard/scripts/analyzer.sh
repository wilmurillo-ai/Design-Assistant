#!/usr/bin/env bash
# DateGuard -- Core Analysis Engine
...
DATEGUARD_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "TZ NF EP DA")
...
scan_file() {
  ...
  for category in $categories; do
    ...
    local patterns_name
    patterns_name=$(get_dateguard_patterns_for_category "$category")
...
# Calculate a date/time quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in TZ NF EP DA CP ST; do
    local cat_count
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Date/Time Quality Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>DateGuard Report -- ${target}</title>
  <style>
...
<body>
  <h1>DateGuard Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${DATEGUARD_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Timezone Handling</td><td>TZ</td><td>$(count_by_category "TZ")</td></tr>
    <tr><td>Naive Formatting</td><td>NF</td><td>$(count_by_category "NF")</td></tr>
    <tr><td>Epoch & Precision</td><td>EP</td><td>$(count_by_category "EP")</td></tr>
    <tr><td>Date Arithmetic</td><td>DA</td><td>$(count_by_category "DA")</td></tr>
    <tr><td>Comparison & Parsing</td><td>CP</td><td>$(count_by_category "CP")</td></tr>
    <tr><td>Storage & Serialization</td><td>ST</td><td>$(count_by_category "ST")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://dateguard.pages.dev">DateGuard</a> v${DATEGUARD_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (Y2038, data loss)${NEWLINE:-$'\n'}"
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- fix naive datetime and DST-unsafe arithmetic"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- add timezone handling and use ISO formats"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[DateGuard]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$DATEGUARD_VERSION}"
...
#   category_filter -- optional category code to filter (e.g., "TZ")
do_dateguard_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_dateguard_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_dateguard_category "$filter_upper"; then
      echo -e "${RED}[DateGuard]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: TZ, NF, EP, DA, CP, ST" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[DateGuard]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://dateguard.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[DateGuard]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(dateguard_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[DateGuard]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No date/time issues detected!${NC}"
        echo ""
        echo -e "Date/Time Quality Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_dateguard_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[DateGuard]${NC} No date/time issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[DateGuard] Date/time issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix date/time issues before committing."
    echo -e "  Run: ${CYAN}dateguard scan${NC} for details"
