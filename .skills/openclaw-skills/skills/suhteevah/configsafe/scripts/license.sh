#!/usr/bin/env bash
# ConfigSafe — License Validation Module
# Same JWT-based offline validation as DocSync/DepGuard/EnvGuard
...
CONFIGSAFE_LICENSE_KEY="${CONFIGSAFE_LICENSE_KEY:-}"
...
get_configsafe_key() {
  if [[ -n "$CONFIGSAFE_LICENSE_KEY" ]]; then
    echo "$CONFIGSAFE_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('configsafe', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.configsafe?.apiKey || '');
} catch(e) {}
" 2>/dev/null)
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.configsafe.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
validate_license() {
  ...
  if [[ "$product" != "configsafe" && -n "$product" ]]; then
    return 1
  fi
...
check_configsafe_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  local key
  if ! key=$(get_configsafe_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}━━━ ConfigSafe License Required ━━━${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://configsafe.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"configsafe\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export CONFIGSAFE_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi

  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[ConfigSafe]${NC} Invalid license key."
    echo -e "Get a valid license at: ${CYAN}https://configsafe.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  # Validate product
  if [[ -n "$product" && "$product" != "configsafe" ]]; then
    echo -e "${RED}[ConfigSafe]${NC} License is for product '${BOLD}$product${NC}', not configsafe."
    echo -e "Get a ConfigSafe license at: ${CYAN}https://configsafe.pages.dev/pricing${NC}"
    return 1
  fi

  local actual_level="${TIER_LEVELS[$tier]:-0}"
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[ConfigSafe]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://configsafe.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[ConfigSafe]${NC} License expired. Renew at: ${CYAN}https://configsafe.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
show_configsafe_status() {
  ...
  if ! key=$(get_configsafe_key) || [[ -z "$key" ]]; then
    ...
    echo -e "  Upgrade at: ${CYAN}https://configsafe.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "configsafe" ]]; then
    ...
    echo -e "  This key is for $product, not ConfigSafe."
    echo -e "  Get a ConfigSafe license at: ${CYAN}https://configsafe.pages.dev/pricing${NC}"
    return
  fi
  ...
  case "$tier" in
    pro)
      echo -e "  Available: scan (unlimited), hooks, report, benchmark"
      echo -e "  Upgrade to Team for: policy, compliance"
      ;;
    team)
      echo -e "  Available: scan, hooks, report, benchmark, policy, compliance"
      ;;
    enterprise)
