...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "QD RN OF RL")
...
# Calculate a GraphQL quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in QD RN OF RL SD CQ; do
    ...
    if [[ $cat_count -gt 0 ]]; then
      local label
      label=$(get_gqllint_category_label "$cat")
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}GraphQL Quality Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>GQLLint Report -- ${target}</title>
  <style>
...
<body>
  <h1>GQLLint Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${GQLLINT_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Query Depth & Complexity</td><td>QD</td><td>$(count_by_category "QD")</td></tr>
    <tr><td>Resolver N+1</td><td>RN</td><td>$(count_by_category "RN")</td></tr>
    <tr><td>Over/Under Fetching</td><td>OF</td><td>$(count_by_category "OF")</td></tr>
    <tr><td>Rate Limiting & Auth</td><td>RL</td><td>$(count_by_category "RL")</td></tr>
    <tr><td>Schema Design</td><td>SD</td><td>$(count_by_category "SD")</td></tr>
    <tr><td>Client Query Safety</td><td>CQ</td><td>$(count_by_category "CQ")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://gqllint.pages.dev">GQLLint</a> v${GQLLINT_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (query injection, N+1 queries, introspection leak)"$'\n'
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- fix auth gaps, unbounded queries, and resolver performance"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- improve schema design, add rate limiting, fix client patterns"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
    recommendations="${recommendations}4. **LOW:** ${low_count} low findings -- review best practices, add types, improve documentation"$'\n'
  fi
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[GQLLint]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$GQLLINT_VERSION}"
...
# do_gqllint_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "RN")
do_gqllint_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_gqllint_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_gqllint_category "$filter_upper"; then
      echo -e "${RED}[GQLLint]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: QD, RN, OF, RL, SD, CQ" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[GQLLint]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://gqllint.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[GQLLint]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(gqllint_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[GQLLint]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No GraphQL issues detected!${NC}"
        echo ""
        echo -e "GraphQL Quality Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_gqllint_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[GQLLint]${NC} No GraphQL issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[GQLLint] GraphQL issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix GraphQL issues before committing."
    echo -e "  Run: ${CYAN}gqllint scan${NC} for details"
