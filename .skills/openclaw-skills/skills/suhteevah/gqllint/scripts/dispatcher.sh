#!/usr/bin/env bash
# GQLLint -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: QD, RN, OF, RL, SD, CQ
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[GQLLint]${NC} $*"; }
log_success() { echo -e "${GREEN}[GQLLint]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[GQLLint]${NC} $*"; }
log_error()   { echo -e "${RED}[GQLLint]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}GQLLint${NC} v${VERSION}"
  echo "  GraphQL anti-pattern & security analyzer"
  echo "  https://gqllint.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: QD, RN, OF, RL, SD, CQ${NC}"
  echo -e "  --help, -h             ${DIM}Show this help${NC}"
  echo -e "  --version, -v          ${DIM}Show version${NC}"
  echo ""
  echo -e "${BOLD}COMMANDS${NC}"
  echo -e "  scan                   ${DIM}Run GraphQL anti-pattern scan (default)${NC}"
  ...
  echo -e "  ${GREEN}FREE${NC}        30 patterns  QD, RN categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  QD, RN, OF, RL"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  QD, RN, OF, RL, SD, CQ"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  QD  Query Depth & Complexity  -- unbounded depth, no complexity limit, introspection leak"
  echo "  RN  Resolver N+1              -- database calls in loops, no DataLoader, sequential awaits"
  echo "  OF  Over/Under Fetching       -- SELECT *, no pagination, eager loading, missing projections"
  echo "  RL  Rate Limiting & Auth      -- no auth on mutations, open introspection, missing rate limit"
  echo "  SD  Schema Design             -- raw scalars in mutations, no input types, naming issues"
  echo "  CQ  Client Query Safety       -- string concatenation, no error handling, missing variables"
  echo ""
  ...
  echo "  dispatcher.sh --path . --category RN --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://gqllint.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export GQLLINT_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_gqllint_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_gqllint_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- GQLLint Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(gqllint_pattern_count) patterns across 6 categories${NC}"
  echo ""

  gqllint_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "gqllint" "$config" 2>/dev/null; then
...
# --- GQLLint hooks ------------------------------------------
    gqllint-check:
      run: |
        GQLLINT_SKILL_DIR="${GQLLINT_SKILL_DIR:-$HOME/.openclaw/skills/gqllint}"
        if [[ -f "$GQLLINT_SKILL_DIR/scripts/analyzer.sh" ]]; then
          source "$GQLLINT_SKILL_DIR/scripts/patterns.sh"
          source "$GQLLINT_SKILL_DIR/scripts/analyzer.sh"
          hook_gqllint_check
        else
          echo "[GQLLint] Skill not found at $GQLLINT_SKILL_DIR -- skipping scan"
        fi
      fail_text: |
        GraphQL issues detected in staged files!
        Run 'gqllint scan' to see details.
        Or 'git commit --no-verify' to skip (NOT recommended).
HOOKS
    echo -e "${GREEN}+${NC} Appended GQLLint hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for GraphQL issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "gqllint" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- GQLLint hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "gqllint" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed GQLLint hooks"
    else
      log_warn "No GQLLint hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_gqllint_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_gqllint_tier)")
        local categories
        categories=$(get_gqllint_categories_for_tier "$tier_num")
