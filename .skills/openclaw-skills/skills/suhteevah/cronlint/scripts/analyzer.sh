...
CRONLINT_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "OE TZ ER RC")
...
# Calculate a scheduling quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in OE TZ ER RC LM OB; do
    ...
    if [[ $cat_count -gt 0 ]]; then
      local label
      label=$(get_cronlint_category_label "$cat")
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Scheduling Quality Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>CronLint Report -- ${target}</title>
  <style>
    ...
    h1 { color: #0f172a; border-bottom: 3px solid #f59e0b; padding-bottom: 0.5rem; }
...
<body>
  <h1>CronLint Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${CRONLINT_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Overlapping Execution</td><td>OE</td><td>$(count_by_category "OE")</td></tr>
    <tr><td>Timezone & Scheduling</td><td>TZ</td><td>$(count_by_category "TZ")</td></tr>
    <tr><td>Error & Recovery</td><td>ER</td><td>$(count_by_category "ER")</td></tr>
    <tr><td>Resource Contention</td><td>RC</td><td>$(count_by_category "RC")</td></tr>
    <tr><td>Lifecycle Management</td><td>LM</td><td>$(count_by_category "LM")</td></tr>
    <tr><td>Observability</td><td>OB</td><td>$(count_by_category "OB")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://cronlint.pages.dev">CronLint</a> v${CRONLINT_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (overlapping execution, silent failures, resource abuse)"$'\n'
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- fix scheduling gaps and error handling"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- add timezone safety, retry logic, resource limits"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
    recommendations="${recommendations}4. **LOW:** ${low_count} low findings -- review best practices and observability gaps"$'\n'
  fi
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[CronLint]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$CRONLINT_VERSION}"
...
# do_cronlint_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "OE")
do_cronlint_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_cronlint_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_cronlint_category "$filter_upper"; then
      echo -e "${RED}[CronLint]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: OE, TZ, ER, RC, LM, OB" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[CronLint]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://cronlint.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[CronLint]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(cronlint_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[CronLint]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No scheduling issues detected!${NC}"
        echo ""
        echo -e "Scheduling Quality Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_cronlint_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[CronLint]${NC} No scheduling issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[CronLint] Scheduling issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix scheduling issues before committing."
    echo -e "  Run: ${CYAN}cronlint scan${NC} for details"
