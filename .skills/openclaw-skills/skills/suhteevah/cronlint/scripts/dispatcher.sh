#!/usr/bin/env bash
# CronLint -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: OE, TZ, ER, RC, LM, OB
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[CronLint]${NC} $*"; }
log_success() { echo -e "${GREEN}[CronLint]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[CronLint]${NC} $*"; }
log_error()   { echo -e "${RED}[CronLint]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}CronLint${NC} v${VERSION}"
  echo "  Scheduled task & cron job anti-pattern analyzer"
  echo "  https://cronlint.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: OE, TZ, ER, RC, LM, OB${NC}"
  echo -e "  --help, -h             ${DIM}Show this help${NC}"
  echo -e "  --version, -v          ${DIM}Show version${NC}"
  echo ""
  echo -e "${BOLD}COMMANDS${NC}"
  echo -e "  scan                   ${DIM}Run scheduling anti-pattern scan (default)${NC}"
  ...
  echo -e "  ${GREEN}FREE${NC}        30 patterns  OE, TZ categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  OE, TZ, ER, RC"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  OE, TZ, ER, RC, LM, OB"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  OE  Overlapping Execution -- missing locks, concurrent runs, no mutex"
  echo "  TZ  Timezone & Scheduling -- hardcoded TZ, DST risks, UTC confusion"
  echo "  ER  Error & Recovery      -- no try/catch, missing retry, silent failure"
  echo "  RC  Resource Contention   -- every-minute cron, no rate limit, unbounded"
  echo "  LM  Lifecycle Management  -- no graceful shutdown, orphaned tasks, stale"
  echo "  OB  Observability         -- no duration logging, no metrics, no alerting"
  echo ""
  ...
  echo "  dispatcher.sh --path . --category OE --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://cronlint.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export CRONLINT_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_cronlint_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_cronlint_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- CronLint Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(cronlint_pattern_count) patterns across 6 categories${NC}"
  echo ""

  cronlint_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "cronlint" "$config" 2>/dev/null; then
...
# --- CronLint hooks ----------------------------------------
    cronlint-check:
      run: |
        CRONLINT_SKILL_DIR="${CRONLINT_SKILL_DIR:-$HOME/.openclaw/skills/cronlint}"
        if [[ -f "$CRONLINT_SKILL_DIR/scripts/analyzer.sh" ]]; then
          source "$CRONLINT_SKILL_DIR/scripts/patterns.sh"
          source "$CRONLINT_SKILL_DIR/scripts/analyzer.sh"
          hook_cronlint_check
        else
          echo "[CronLint] Skill not found at $CRONLINT_SKILL_DIR -- skipping scan"
        fi
      fail_text: |
        Scheduling issues detected in staged files!
        Run 'cronlint scan' to see details.
        Or 'git commit --no-verify' to skip (NOT recommended).
HOOKS
    echo -e "${GREEN}+${NC} Appended CronLint hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for scheduling issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "cronlint" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- CronLint hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "cronlint" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed CronLint hooks"
    else
      log_warn "No CronLint hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_cronlint_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_cronlint_tier)")
        local categories
        categories=$(get_cronlint_categories_for_tier "$tier_num")
