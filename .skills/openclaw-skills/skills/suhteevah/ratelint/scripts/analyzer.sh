...
RATELINT_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "RL BF TH BP")
...
scan_file() {
  ...
  for category in $categories; do
    ...
    local patterns_name
    patterns_name=$(get_ratelint_patterns_for_category "$category")
...
# Calculate a rate limiting quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in RL BF TH BP QO DD; do
    local cat_count
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Rate Limiting Quality Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>RateLint Report -- ${target}</title>
  <style>
...
<body>
  <h1>RateLint Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${RATELINT_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Rate Limit Configuration</td><td>RL</td><td>$(count_by_category "RL")</td></tr>
    <tr><td>Brute Force Protection</td><td>BF</td><td>$(count_by_category "BF")</td></tr>
    <tr><td>Throttling & Backpressure</td><td>TH</td><td>$(count_by_category "TH")</td></tr>
    <tr><td>Backoff & Retry</td><td>BP</td><td>$(count_by_category "BP")</td></tr>
    <tr><td>Queue & Buffer Overflow</td><td>QO</td><td>$(count_by_category "QO")</td></tr>
    <tr><td>DDoS & Abuse Prevention</td><td>DD</td><td>$(count_by_category "DD")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://ratelint.pages.dev">RateLint</a> v${RATELINT_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (brute force exposure, retry storms)${NEWLINE:-$'\n'}"
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- fix missing rate limits and unprotected endpoints"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- add throttling, debounce, and backpressure handling"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[RateLint]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$RATELINT_VERSION}"
...
# do_ratelint_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "RL")
do_ratelint_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_ratelint_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_ratelint_category "$filter_upper"; then
      echo -e "${RED}[RateLint]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: RL, BF, TH, BP, QO, DD" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[RateLint]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://ratelint.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[RateLint]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(ratelint_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[RateLint]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No rate limiting issues detected!${NC}"
        echo ""
        echo -e "Rate Limiting Quality Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_ratelint_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[RateLint]${NC} No rate limiting issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[RateLint] Rate limiting issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix rate limiting issues before committing."
    echo -e "  Run: ${CYAN}ratelint scan${NC} for details"
