#!/usr/bin/env bash
# RateLint -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: RL, BF, TH, BP, QO, DD
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[RateLint]${NC} $*"; }
log_success() { echo -e "${GREEN}[RateLint]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[RateLint]${NC} $*"; }
log_error()   { echo -e "${RED}[RateLint]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}RateLint${NC} v${VERSION}"
  echo "  Rate limiting & API throttling anti-pattern analyzer"
  echo "  https://ratelint.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: RL, BF, TH, BP, QO, DD${NC}"
  ...
  echo -e "  ${GREEN}FREE${NC}        30 patterns  RL, BF categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  RL, BF, TH, BP"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  RL, BF, TH, BP, QO, DD"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  RL  Rate Limit Configuration   -- missing rate limiter, no limit headers, unbounded request acceptance"
  echo "  BF  Brute Force Protection     -- no login attempt limits, missing lockout, OTP brute force"
  echo "  TH  Throttling & Backpressure  -- no request throttling, missing queue bounds, no load shedding"
  echo "  BP  Backpressure & Flow Control -- no backpressure signaling, unbounded worker pools, missing flow control"
  echo "  QO  Queue & Buffer Overflow    -- unbounded queues, no max queue size, missing overflow handling"
  echo "  RE  Retry & Backoff            -- no exponential backoff, infinite retry loops, missing jitter"
  echo ""
  ...
  echo "  dispatcher.sh --path . --category RL --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://ratelint.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export RATELINT_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_ratelint_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_ratelint_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- RateLint Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(ratelint_pattern_count) patterns across 6 categories${NC}"
  echo ""

  ratelint_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "ratelint" "$config" 2>/dev/null; then
...
# --- RateLint hooks ----------------------------------------
    ratelint-check:
      run: |
        RATELINT_SKILL_DIR="${RATELINT_SKILL_DIR:-$HOME/.openclaw/skills/ratelint}"
        if [[ -f "$RATELINT_SKILL_DIR/scripts/analyzer.sh" ]]; then
          source "$RATELINT_SKILL_DIR/scripts/patterns.sh"
          source "$RATELINT_SKILL_DIR/scripts/analyzer.sh"
          hook_ratelint_check
        else
          echo "[RateLint] Skill not found at $RATELINT_SKILL_DIR -- skipping scan"
        fi
      fail_text: |
        Rate limiting issues detected in staged files!
        Run 'ratelint scan' to see details.
        Or 'git commit --no-verify' to skip (NOT recommended).
HOOKS
    echo -e "${GREEN}+${NC} Appended RateLint hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for rate limiting issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "ratelint" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- RateLint hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "ratelint" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed RateLint hooks"
    else
      log_warn "No RateLint hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_ratelint_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_ratelint_tier)")
        local categories
        categories=$(get_ratelint_categories_for_tier "$tier_num")
