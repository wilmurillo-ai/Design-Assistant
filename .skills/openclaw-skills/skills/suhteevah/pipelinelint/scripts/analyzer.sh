#!/usr/bin/env bash
# PipelineLint -- Core Analysis Engine
...
PIPELINELINT_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "SE CF TS AR")
...
scan_file() {
  ...
  for category in $categories; do
    ...
    local patterns_name
    patterns_name=$(get_pipelinelint_patterns_for_category "$category")
    ...
    for entry in "${_patterns_ref[@]}"; do
      ...
      # The -- separator prevents patterns starting with - from being treated as flags
      local matches
...
# Calculate a pipeline quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in SE CF TS AR DP EN; do
    local cat_count
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Pipeline Quality Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>PipelineLint Report -- ${target}</title>
  <style>
...
<body>
  <h1>PipelineLint Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${PIPELINELINT_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Secrets & Security</td><td>SE</td><td>$(count_by_category "SE")</td></tr>
    <tr><td>Caching & Performance</td><td>CF</td><td>$(count_by_category "CF")</td></tr>
    <tr><td>Testing & Quality</td><td>TS</td><td>$(count_by_category "TS")</td></tr>
    <tr><td>Artifacts & Dependencies</td><td>AR</td><td>$(count_by_category "AR")</td></tr>
    <tr><td>Deployment Safety</td><td>DP</td><td>$(count_by_category "DP")</td></tr>
    <tr><td>Environment & Configuration</td><td>EN</td><td>$(count_by_category "EN")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://pipelinelint.pages.dev">PipelineLint</a> v${PIPELINELINT_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (secrets exposure, deployment safety)${NEWLINE:-$'\n'}"
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- fix unsafe dependencies and skipped tests"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- add caching, fix environment configs"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[PipelineLint]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$PIPELINELINT_VERSION}"
...
# do_pipelinelint_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "SE")
do_pipelinelint_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_pipelinelint_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_pipelinelint_category "$filter_upper"; then
      echo -e "${RED}[PipelineLint]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: SE, CF, TS, AR, DP, EN" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[PipelineLint]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://pipelinelint.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[PipelineLint]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(pipelinelint_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[PipelineLint]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No pipeline issues detected!${NC}"
        echo ""
        echo -e "Pipeline Quality Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_pipelinelint_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[PipelineLint]${NC} No pipeline issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[PipelineLint] Pipeline issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix pipeline issues before committing."
    echo -e "  Run: ${CYAN}pipelinelint scan${NC} for details"
