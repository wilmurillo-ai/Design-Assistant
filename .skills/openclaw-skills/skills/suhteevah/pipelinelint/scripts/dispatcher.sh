#!/usr/bin/env bash
# PipelineLint -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: SE, CF, TS, AR, DP, EN
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[PipelineLint]${NC} $*"; }
log_success() { echo -e "${GREEN}[PipelineLint]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[PipelineLint]${NC} $*"; }
log_error()   { echo -e "${RED}[PipelineLint]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}PipelineLint${NC} v${VERSION}"
  echo "  CI/CD pipeline anti-pattern analyzer"
  echo "  https://pipelinelint.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: SE, CF, TS, AR, DP, EN${NC}"
  ...
  echo -e "  ${GREEN}FREE${NC}        30 patterns  SE, CF categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  SE, CF, TS, AR"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  SE, CF, TS, AR, DP, EN"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  SE  Secrets & Security       -- hardcoded passwords, API keys, tokens, secret leakage"
  echo "  CF  Caching & Performance    -- missing caches, redundant installs, slow builds"
  echo "  TS  Testing & Quality        -- skipped tests, disabled linting, no coverage"
  echo "  AR  Artifacts & Dependencies -- unpinned deps, unverified downloads, insecure installs"
  echo "  DP  Deployment Safety        -- no approval gates, force push, destructive operations"
  echo "  EN  Environment & Config     -- hardcoded values, no timeouts, no retries, plain HTTP"
  echo ""
  ...
  echo "  dispatcher.sh --path .github/workflows/ --format json"
  echo "  dispatcher.sh --path . --category SE --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://pipelinelint.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export PIPELINELINT_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_pipelinelint_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_pipelinelint_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- PipelineLint Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(pipelinelint_pattern_count) patterns across 6 categories${NC}"
  echo ""

  pipelinelint_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "pipelinelint" "$config" 2>/dev/null; then
...
# --- PipelineLint hooks ----------------------------------------
    pipelinelint-check:
      run: |
        PIPELINELINT_SKILL_DIR="${PIPELINELINT_SKILL_DIR:-$HOME/.openclaw/skills/pipelinelint}"
        if [[ -f "$PIPELINELINT_SKILL_DIR/scripts/analyzer.sh" ]]; then
          source "$PIPELINELINT_SKILL_DIR/scripts/patterns.sh"
          source "$PIPELINELINT_SKILL_DIR/scripts/analyzer.sh"
          hook_pipelinelint_check
        else
          echo "[PipelineLint] Skill not found at $PIPELINELINT_SKILL_DIR -- skipping scan"
        fi
      fail_text: |
        Pipeline issues detected in staged files!
        Run 'pipelinelint scan' to see details.
        Or 'git commit --no-verify' to skip (NOT recommended).
HOOKS
    echo -e "${GREEN}+${NC} Appended PipelineLint hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for pipeline issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "pipelinelint" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- PipelineLint hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "pipelinelint" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed PipelineLint hooks"
    else
      log_warn "No PipelineLint hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_pipelinelint_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_pipelinelint_tier)")
        local categories
        categories=$(get_pipelinelint_categories_for_tier "$tier_num")
