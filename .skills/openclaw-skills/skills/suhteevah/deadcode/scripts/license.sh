#!/usr/bin/env bash
# DeadCode -- License Validation Module
# Same JWT-based offline validation as ConfigSafe/DocSync/DepGuard/EnvGuard
...
DEADCODE_LICENSE_KEY="${DEADCODE_LICENSE_KEY:-}"
...
get_deadcode_key() {
  if [[ -n "$DEADCODE_LICENSE_KEY" ]]; then
    echo "$DEADCODE_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('deadcode', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.deadcode?.apiKey || '');
} catch(e) {}
" 2>/dev/null)
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.deadcode.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
validate_license() {
  ...
  if [[ "$product" != "deadcode" && -n "$product" ]]; then
    return 1
  fi
...
check_deadcode_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  local key
  if ! key=$(get_deadcode_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}--- DeadCode License Required ---${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://deadcode.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"deadcode\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export DEADCODE_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi

  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[DeadCode]${NC} Invalid license key."
    echo -e "Get a valid license at: ${CYAN}https://deadcode.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  # Validate product
  if [[ -n "$product" && "$product" != "deadcode" ]]; then
    echo -e "${RED}[DeadCode]${NC} License is for product '${BOLD}$product${NC}', not deadcode."
    echo -e "Get a DeadCode license at: ${CYAN}https://deadcode.pages.dev/pricing${NC}"
    return 1
  fi

  local actual_level="${TIER_LEVELS[$tier]:-0}"
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[DeadCode]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://deadcode.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[DeadCode]${NC} License expired. Renew at: ${CYAN}https://deadcode.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
show_deadcode_status() {
  ...
  if ! key=$(get_deadcode_key) || [[ -z "$key" ]]; then
    ...
    echo -e "  Upgrade at: ${CYAN}https://deadcode.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "deadcode" ]]; then
    ...
    echo -e "  This key is for $product, not DeadCode."
    echo -e "  Get a DeadCode license at: ${CYAN}https://deadcode.pages.dev/pricing${NC}"
    return
  fi
  ...
  case "$tier" in
    pro)
      echo -e "  Available: scan (unlimited), hooks, report, orphans"
      echo -e "  Upgrade to Team for: ignore rules, sarif, CI/CD integration"
      ;;
    team)
      echo -e "  Available: scan, hooks, report, orphans, ignore, sarif"
      ;;
    enterprise)
