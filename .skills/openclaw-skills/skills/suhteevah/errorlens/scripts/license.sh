#!/usr/bin/env bash
# ErrorLens -- License Validation Module
...
ERRORLENS_LICENSE_KEY="${ERRORLENS_LICENSE_KEY:-}"
...
get_errorlens_key() {
  ...
  if [[ -n "$ERRORLENS_LICENSE_KEY" ]]; then
    echo "$ERRORLENS_LICENSE_KEY"
    return 0
  fi

  # Second: check openclaw.json config file
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('errorlens', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.errorlens?.apiKey || '');
} catch(e) {}
" 2>/dev/null)

    # Fallback to jq
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.errorlens.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
validate_license() {
  ...
  if [[ "$product" != "errorlens" && -n "$product" ]]; then
    return 1
  fi
...
# --- License Check (main entry point) --------------------------------------

check_errorlens_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  # Get the license key
  local key
  if ! key=$(get_errorlens_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}--- ErrorLens License Required ---${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://errorlens.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"errorlens\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export ERRORLENS_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi
  ...
  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[ErrorLens]${NC} Invalid license key."
    echo -e "Get a valid license at: ${CYAN}https://errorlens.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  # Validate product
  if [[ -n "$product" && "$product" != "errorlens" ]]; then
    echo -e "${RED}[ErrorLens]${NC} License is for product '${BOLD}$product${NC}', not errorlens."
    echo -e "Get an ErrorLens license at: ${CYAN}https://errorlens.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[ErrorLens]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://errorlens.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[ErrorLens]${NC} License expired. Renew at: ${CYAN}https://errorlens.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
show_errorlens_status() {
  ...
  if ! key=$(get_errorlens_key) || [[ -z "$key" ]]; then
    ...
    echo -e "  Upgrade at: ${CYAN}https://errorlens.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "errorlens" ]]; then
    ...
    echo -e "  This key is for $product, not ErrorLens."
    echo -e "  Get an ErrorLens license at: ${CYAN}https://errorlens.pages.dev/pricing${NC}"
    return
  fi
