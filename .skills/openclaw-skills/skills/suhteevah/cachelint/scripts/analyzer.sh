#!/usr/bin/env bash
# CacheLint -- Core Analysis Engine
...
CACHELINT_VERSION="${VERSION:-1.0.0}"
...
# Arguments:
#   $1 -- filepath to scan
#   $2 -- space-separated list of category codes (e.g., "CI TE CS RM")
...
# Calculate a caching quality score based on findings.
...
print_category_breakdown_text() {
  ...
  for cat in CI TE CS RM CA SH; do
    ...
    if [[ $cat_count -gt 0 ]]; then
      local label
      label=$(get_cachelint_category_label "$cat")
...
print_score_text() {
  ...
  echo ""
  echo -e "${BOLD}Caching Quality Score: ${gc}${score}/100${NC} (Grade: ${gc}${BOLD}${grade}${NC})"
...
<head>
  ...
  <title>CacheLint Report -- ${target}</title>
  <style>
    ...
    h1 { color: #0f172a; border-bottom: 3px solid #fd79a8; padding-bottom: 0.5rem; }
...
<body>
  <h1>CacheLint Report</h1>
  <p>Target: <code>${target}</code> | Date: ${date_str} | Version: ${CACHELINT_VERSION}</p>
  ...
  <table>
    ...
    <tr><td>Cache Invalidation</td><td>CI</td><td>$(count_by_category "CI")</td></tr>
    <tr><td>TTL & Expiry</td><td>TE</td><td>$(count_by_category "TE")</td></tr>
    <tr><td>Cache Stampede</td><td>CS</td><td>$(count_by_category "CS")</td></tr>
    <tr><td>Redis/Store Misuse</td><td>RM</td><td>$(count_by_category "RM")</td></tr>
    <tr><td>Cache Architecture</td><td>CA</td><td>$(count_by_category "CA")</td></tr>
    <tr><td>Security & Hygiene</td><td>SH</td><td>$(count_by_category "SH")</td></tr>
  </table>
  ...
  <div class="footer">
    Generated by <a href="https://cachelint.pages.dev">CacheLint</a> v${CACHELINT_VERSION} on ${date_str}.
  </div>
</body>
</html>
HTML_EOF
...
generate_markdown_report() {
  ...
  if [[ $crit_count -gt 0 ]]; then
    recommendations="${recommendations}1. **CRITICAL:** ${crit_count} critical findings -- address immediately (stampede risk / data loss / KEYS * usage)"$'\n'
  fi
  if [[ $high_count -gt 0 ]]; then
    recommendations="${recommendations}2. **HIGH:** ${high_count} high-severity findings -- fix cache invalidation gaps and TTL issues"$'\n'
  fi
  if [[ $med_count -gt 0 ]]; then
    recommendations="${recommendations}3. **MEDIUM:** ${med_count} medium findings -- add TTL jitter, fix Redis misuse, review architecture"$'\n'
  fi
  if [[ $low_count -gt 0 ]]; then
    recommendations="${recommendations}4. **LOW:** ${low_count} low findings -- review best practices and namespace conventions"$'\n'
  fi
  ...
  if [[ ! -f "$template_file" ]]; then
    echo -e "${RED}[CacheLint]${NC} Report template not found: $template_file" >&2
    return 1
  fi
  ...
  report="${report//\{\{VERSION\}\}/$CACHELINT_VERSION}"
...
# do_cachelint_scan <target> <tier_level> <format> <verbose> <category_filter>
#
# Arguments:
...
#   category_filter -- optional category code to filter (e.g., "CS")
do_cachelint_scan() {
  local target="${1:-.}"
  ...
  local categories
  categories=$(get_cachelint_categories_for_tier "$tier_level")

  # Validate category filter if provided
  if [[ -n "$category_filter" ]]; then
    local filter_upper
    filter_upper=$(echo "$category_filter" | tr '[:lower:]' '[:upper:]')
    if ! is_valid_cachelint_category "$filter_upper"; then
      echo -e "${RED}[CacheLint]${NC} Invalid category: ${BOLD}$category_filter${NC}" >&2
      echo -e "  Valid categories: CI, TE, CS, RM, CA, SH" >&2
      return 1
    fi
    ...
    if [[ "$cat_available" == false ]]; then
      echo -e "${RED}[CacheLint]${NC} Category ${BOLD}$filter_upper${NC} is not available on your current tier." >&2
      echo -e "  Available categories for your tier: ${BOLD}$categories${NC}" >&2
      echo -e "  Upgrade at: ${CYAN}https://cachelint.pages.dev/pricing${NC}" >&2
      return 1
    fi
  fi
  ...
  if [[ $total_files -eq 0 ]]; then
    if [[ "$format" == "json" ]]; then
      ...
      echo -e "${GREEN}[CacheLint]${NC} No scannable files found in ${BOLD}$target${NC}"
  ...
  if [[ "$format" == "text" ]]; then
    ...
    for cat in $categories; do
      local cnt
      cnt=$(cachelint_category_count "$cat")
      pattern_count=$((pattern_count + cnt))
    done

    echo -e "${BLUE}[CacheLint]${NC} Scanning ${BOLD}$total_files${NC} files with ${BOLD}$pattern_count${NC} patterns..."
    if [[ -n "$category_filter" ]]; then
  ...
  case "$format" in
    ...
    text|*)
      if [[ ${#FINDINGS[@]} -eq 0 ]]; then
        echo -e "${GREEN}${BOLD}No caching issues detected!${NC}"
        echo ""
        echo -e "Caching Quality Score: ${GREEN}${BOLD}100/100${NC} (Grade: ${GREEN}${BOLD}A${NC})"
...
hook_cachelint_check() {
  ...
  if [[ ${#FINDINGS[@]} -eq 0 ]]; then
    echo -e "${GREEN}[CacheLint]${NC} No caching issues in $file_count staged files."
    return 0
  fi
  ...
  echo -e "${RED}${BOLD}[CacheLint] Caching issues detected in staged files!${NC}"
  echo ""
  ...
  if [[ "$has_blocking" == true ]]; then
    echo ""
    echo -e "${RED}Commit blocked.${NC} Fix caching issues before committing."
    echo -e "  Run: ${CYAN}cachelint scan${NC} for details"
