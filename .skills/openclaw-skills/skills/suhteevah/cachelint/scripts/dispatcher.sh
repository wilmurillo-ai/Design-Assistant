#!/usr/bin/env bash
# CacheLint -- Main CLI Dispatcher (Entry Point)
...
#   --category <cat>       Filter to specific category: CI, TE, CS, RM, CA, SH
#   --help, -h             Show help
...
log_info()    { echo -e "${BLUE}[CacheLint]${NC} $*"; }
log_success() { echo -e "${GREEN}[CacheLint]${NC} $*"; }
log_warn()    { echo -e "${YELLOW}[CacheLint]${NC} $*"; }
log_error()   { echo -e "${RED}[CacheLint]${NC} $*" >&2; }

# --- Version & Help -----------------------------------------------------------

show_version() {
  echo -e "${BOLD}CacheLint${NC} v${VERSION}"
  echo "  Caching anti-pattern analyzer"
  echo "  https://cachelint.pages.dev"
}
...
show_help() {
  ...
  echo -e "  --category <cat>       ${DIM}Filter: CI, TE, CS, RM, CA, SH${NC}"
  echo -e "  --help, -h             ${DIM}Show this help${NC}"
  echo -e "  --version, -v          ${DIM}Show version${NC}"
  echo ""
  echo -e "${BOLD}COMMANDS${NC}"
  echo -e "  scan                   ${DIM}Run caching anti-pattern scan (default)${NC}"
  ...
  echo -e "  ${GREEN}FREE${NC}        30 patterns  CI, TE categories"
  echo -e "  ${CYAN}PRO${NC}  \$19/mo  60 patterns  CI, TE, CS, RM"
  echo -e "  ${MAGENTA}TEAM${NC} \$39/mo  90 patterns  CI, TE, CS, RM, CA, SH"
  echo -e "  ${BOLD}ENTERPRISE${NC}   90 patterns  All + priority support"
  echo ""
  echo -e "${BOLD}CHECK CATEGORIES${NC}"
  echo "  CI  Cache Invalidation   -- stale data, missing invalidation after writes"
  echo "  TE  TTL & Expiry         -- missing TTL, infinite cache, no jitter"
  echo "  CS  Cache Stampede       -- thundering herd, no locking on rebuild"
  echo "  RM  Redis/Store Misuse   -- KEYS *, no pooling, FLUSHALL, no pipeline"
  echo "  CA  Cache Architecture   -- N+1 gets, no abstraction, mixed strategies"
  echo "  SH  Security & Hygiene   -- PII in keys, no TLS, missing encryption"
  echo ""
  ...
  echo "  dispatcher.sh --path . --category CS --verbose"
  echo "  dispatcher.sh --path . --format html > report.html"
  echo ""
  echo -e "Get a license at ${CYAN}https://cachelint.pages.dev/pricing${NC}"
}
...
resolve_tier() {
  ...
  if [[ -n "$tier_override" ]]; then
    ...
    if [[ $required_level -gt 0 ]]; then
      ...
      if [[ -n "$license_key_flag" ]]; then
        export CACHELINT_LICENSE_KEY="$license_key_flag"
      fi

      if ! check_cachelint_license "$tier_override"; then
        exit 1
      fi
    fi
    echo "$required_level"
  ...
  local detected_tier
  detected_tier=$(get_cachelint_tier)
...
do_list_patterns() {
  ...
  echo -e "${BOLD}--- CacheLint Detection Patterns ---${NC}"
  echo ""
  echo -e "${BOLD}$(cachelint_pattern_count) patterns across 6 categories${NC}"
  echo ""

  cachelint_list_patterns "all"
}
...
do_hooks_install() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "cachelint" "$config" 2>/dev/null; then
...
# --- CacheLint hooks ----------------------------------------
    cachelint-check:
      run: |
        CACHELINT_SKILL_DIR="${CACHELINT_SKILL_DIR:-$HOME/.openclaw/skills/cachelint}"
        if [[ -f "$CACHELINT_SKILL_DIR/scripts/analyzer.sh" ]]; then
          source "$CACHELINT_SKILL_DIR/scripts/patterns.sh"
          source "$CACHELINT_SKILL_DIR/scripts/analyzer.sh"
          hook_cachelint_check
        else
          echo "[CacheLint] Skill not found at $CACHELINT_SKILL_DIR -- skipping scan"
        fi
      fail_text: |
        Caching issues detected in staged files!
        Run 'cachelint scan' to see details.
        Or 'git commit --no-verify' to skip (NOT recommended).
HOOKS
    echo -e "${GREEN}+${NC} Appended CacheLint hooks to existing lefthook.yml"
  else
  ...
  echo ""
  log_success "Hooks installed! Staged files will be scanned for caching issues on every commit."
}
...
do_hooks_uninstall() {
  ...
  if [[ -f "$config" ]]; then
    if grep -q "cachelint" "$config" 2>/dev/null; then
      local tmp
      tmp=$(mktemp)
      sed '/# --- CacheLint hooks/,/fail_text:.*$/d' "$config" > "$tmp"
      grep -v "cachelint" "$tmp" > "$config" 2>/dev/null || mv "$tmp" "$config"
      rm -f "$tmp"
      echo -e "${GREEN}+${NC} Removed CacheLint hooks"
    else
      log_warn "No CacheLint hooks found"
    fi
  else
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      hooks)
        ...
        case "$subcmd" in
          install)
            ...
            check_cachelint_license "pro" || exit 1
            do_hooks_install
            exit $?
            ;;
          uninstall)
      ...
      report)
        ...
        source "$SCRIPT_DIR/analyzer.sh"
        local tier_num
        tier_num=$(get_tier_level "$(get_cachelint_tier)")
        local categories
        categories=$(get_cachelint_categories_for_tier "$tier_num")
