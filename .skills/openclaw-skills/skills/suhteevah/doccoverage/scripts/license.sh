#!/usr/bin/env bash
# DocCoverage -- License Validation Module
# Same JWT-based offline validation as ConfigSafe/DocSync/DepGuard/EnvGuard/SQLGuard
...
DOCCOVERAGE_LICENSE_KEY="${DOCCOVERAGE_LICENSE_KEY:-}"
...
get_doccoverage_key() {
  if [[ -n "$DOCCOVERAGE_LICENSE_KEY" ]]; then
    echo "$DOCCOVERAGE_LICENSE_KEY"
    return 0
  fi
...
try:
    with open('$OPENCLAW_CONFIG') as f:
        cfg = json.load(f)
    print(cfg.get('skills', {}).get('entries', {}).get('doccoverage', {}).get('apiKey', ''))
except: pass
" 2>/dev/null)
...
try {
  ...
  console.log(cfg?.skills?.entries?.doccoverage?.apiKey || '');
} catch(e) {}
" 2>/dev/null)
    elif command -v jq &>/dev/null; then
      key=$(jq -r '.skills.entries.doccoverage.apiKey // ""' "$OPENCLAW_CONFIG" 2>/dev/null)
    fi
...
validate_license() {
  ...
  if [[ "$product" != "doccoverage" && -n "$product" ]]; then
    return 1
  fi
...
check_doccoverage_license() {
  local required_tier="${1:-pro}"
  local required_level="${TIER_LEVELS[$required_tier]:-1}"

  local key
  if ! key=$(get_doccoverage_key) || [[ -z "$key" ]]; then
    echo -e "${RED}${BOLD}--- DocCoverage License Required ---${NC}"
    echo ""
    echo -e "This feature requires a ${BOLD}${required_tier}${NC} license."
    echo ""
    echo -e "Get your license at: ${CYAN}${BOLD}https://doccoverage.pages.dev/pricing${NC}"
    echo ""
    echo "Then configure it:"
    echo -e "  ${DIM}# In ~/.openclaw/openclaw.json:${NC}"
    echo -e "  ${CYAN}\"skills\": { \"entries\": { \"doccoverage\": { \"apiKey\": \"YOUR_KEY\" } } }${NC}"
    echo ""
    echo -e "  ${DIM}# Or set the environment variable:${NC}"
    echo -e "  ${CYAN}export DOCCOVERAGE_LICENSE_KEY=\"YOUR_KEY\"${NC}"
    return 1
  fi

  local payload
  if ! payload=$(decode_jwt_payload "$key"); then
    echo -e "${RED}[DocCoverage]${NC} Invalid license key."
    echo -e "Get a valid license at: ${CYAN}https://doccoverage.pages.dev/pricing${NC}"
    return 1
  fi
  ...
  # Validate product
  if [[ -n "$product" && "$product" != "doccoverage" ]]; then
    echo -e "${RED}[DocCoverage]${NC} License is for product '${BOLD}$product${NC}', not doccoverage."
    echo -e "Get a DocCoverage license at: ${CYAN}https://doccoverage.pages.dev/pricing${NC}"
    return 1
  fi

  local actual_level="${TIER_LEVELS[$tier]:-0}"
  if [[ "$actual_level" -lt "$required_level" ]]; then
    echo -e "${RED}[DocCoverage]${NC} This feature requires ${BOLD}$required_tier${NC} tier. You have: ${BOLD}$tier${NC}"
    echo -e "Upgrade at: ${CYAN}https://doccoverage.pages.dev/upgrade${NC}"
    return 1
  fi
  ...
  if [[ -n "$expiry" ]]; then
    ...
    if [[ "$now" -gt "$expiry" ]]; then
      echo -e "${RED}[DocCoverage]${NC} License expired. Renew at: ${CYAN}https://doccoverage.pages.dev/renew${NC}"
      return 1
    fi
  fi
...
show_doccoverage_status() {
  ...
  if ! key=$(get_doccoverage_key) || [[ -z "$key" ]]; then
    ...
    echo -e "  Upgrade at: ${CYAN}https://doccoverage.pages.dev/pricing${NC}"
    return
  fi
  ...
  if [[ -n "$product" && "$product" != "doccoverage" ]]; then
    ...
    echo -e "  This key is for $product, not DocCoverage."
    echo -e "  Get a DocCoverage license at: ${CYAN}https://doccoverage.pages.dev/pricing${NC}"
    return
  fi
  ...
  case "$tier" in
    pro)
      echo -e "  Available: scan (unlimited), hooks, report, coverage"
      echo -e "  Upgrade to Team for: policy, sarif, changelog"
      ;;
    team)
      echo -e "  Available: scan, hooks, report, coverage, policy, sarif, changelog"
      ;;
    enterprise)
