...
struct FanEntry: Codable {
    let id: Int
    let name: String?
    let actual_rpm: Double?
    let minimum_rpm: Double?
    let maximum_rpm: Double?
    let target_rpm: Double?
    let mode: String?
}

struct FanPayload: Codable {
    let kind: String
    ...
    init(supported: Bool, fan_count: Int, fans: [FanEntry], notes: [String]) {
        self.kind = "fanstat"
...
extension UInt16 {
    init(bytes: (UInt8, UInt8)) { self = UInt16(bytes.0) << 8 | UInt16(bytes.1) }
}

extension UInt32 {
    init(bytes: (UInt8, UInt8, UInt8, UInt8)) { self = UInt32(bytes.0) << 24 | UInt32(bytes.1) << 16 | UInt32(bytes.2) << 8 | UInt32(bytes.3) }
}

extension Int {
    init(fromFPE2 bytes: (UInt8, UInt8)) { self = (Int(bytes.0) << 6) + (Int(bytes.1) >> 2) }
}
...
final class SMC {
    ...
    func getValue(_ key: String) -> Double? {
        ...
        if value.dataSize == 0 { return nil }
        let zeroIsValid = key == "FS! " || key.hasPrefix("F") && (key.hasSuffix("Md") || key.hasSuffix("Ac") || key.hasSuffix("Tg"))
        if value.bytes.first(where: { $0 != 0 }) == nil && !zeroIsValid { return nil }
    ...
    func getString(_ key: String) -> String? {
        ...
        guard value.dataType == SMCDataType.fds.rawValue else { return nil }
        let chars = (4..<16).map { String(UnicodeScalar(value.bytes[$0])) }
...
func emit(_ payload: FanPayload) -> Never {
...
guard let smc = SMC() else {
    emit(FanPayload(supported: false, fan_count: 0, fans: [], notes: ["AppleSMC service could not be opened."]))
}

guard let rawCount = smc.getValue("FNum") else {
    emit(FanPayload(supported: false, fan_count: 0, fans: [], notes: ["FNum key was not available on this host."]))
}

let fanCount = Int(rawCount)
var fans: [FanEntry] = []
for id in 0..<fanCount {
    let modeValue = Int(smc.getValue("F\(id)Md") ?? -1)
    let mode: String?
    switch modeValue {
    case 0, 3: mode = "automatic"
    case 1: mode = "forced"
    default: mode = nil
    }
    fans.append(FanEntry(
        id: id,
        name: smc.getString("F\(id)ID"),
        actual_rpm: smc.getValue("F\(id)Ac"),
        minimum_rpm: smc.getValue("F\(id)Mn"),
        maximum_rpm: smc.getValue("F\(id)Mx"),
        target_rpm: smc.getValue("F\(id)Tg"),
        mode: mode
    ))
}

var notes = ["Non-privileged AppleSMC fan readout."]
if fans.allSatisfy({ ($0.actual_rpm ?? 0) == 0 }) {
    notes.append("All reported actual RPM values are 0 at sample time; on Apple Silicon this can mean the fans are currently stopped rather than unreadable.")
}

emit(FanPayload(supported: true, fan_count: fanCount, fans: fans, notes: notes))
