/**
 * VEROQ — OpenClaw Skill
 *
 * Intelligence briefs from VEROQ.
 * Hundreds of curated sources, 18 categories, bias scored, confidence rated.
 *
 * All requests go to api.veroq.ai (read-only, public endpoints).
 * No credentials, tokens, or user data are sent or stored.
 */

const API_BASE = "https://api.veroq.ai";
const SITE_URL = "https://veroq.ai";
...
async function handleAsk(args) {
  ...
  if (!question) {
    return "Usage: /ask [question]\nExample: /ask What's happening with NVDA?\nExample: /ask How is the market doing today?";
  }
  ...
  if (data.trade_signal) {
    ...
    const action = (ts.action || "").toUpperCase();
    lines.push(`Trade Signal: ${action} (${ts.score}/100)`);
    if (ts.factors?.length) {
      for (const f of ts.factors) lines.push(`  - ${f}`);
    }
    lines.push("");
  }
  ...
  if (data.credits_used != null) meta.push(`Credits: ${data.credits_used}`);
  if (data.response_time_ms != null) meta.push(`${data.response_time_ms}ms`);
  ...
  if (data.endpoints_called?.length) {
    lines.push(`Endpoints: ${data.endpoints_called.join(", ")}`);
  }

  if (data.follow_ups?.length) {
    lines.push("", `Follow up: ${data.follow_ups.join(" | ")}`);
  }
...
async function handleVerify(args) {
  ...
  if (!claim) {
    return "Usage: /verify [claim]\nExample: /verify NVIDIA revenue grew 200% last quarter\nExample: /verify Bitcoin hit an all-time high in March 2026";
  }
  ...
  const lines = [
    `Verdict: ${verdictLabel[data.verdict] || (data.verdict || "UNKNOWN").toUpperCase()}`,
  ...
  if (data.evidence?.length) {
    lines.push("", "Evidence:");
    for (const e of data.evidence.slice(0, 5)) {
      lines.push(`  ▸ ${e.headline || e.title || e.text || ""}`);
      if (e.relevance_score != null) {
        lines.push(`    Relevance: ${Math.round(e.relevance_score * 100)}%`);
      }
    }
  }

  const meta = [];
  if (data.sources_analyzed != null) meta.push(`Sources analyzed: ${data.sources_analyzed}`);
  if (data.briefs_matched != null) meta.push(`Briefs matched: ${data.briefs_matched}`);
  if (meta.length > 0) lines.push("", meta.join(" | "));
...
module.exports = {
  ...
  commands: {
    ask: {
      description: "Ask any question — get a verified, sourced answer",
      execute: async (args) => {
        try {
          return await handleAsk(args);
        } catch (err) {
          return `Error processing question: ${err.message}`;
        }
      },
    },

    verify: {
      description: "Fact-check any claim against verified intelligence",
      execute: async (args) => {
        try {
          return await handleVerify(args);
        } catch (err) {
          return `Error verifying claim: ${err.message}`;
        }
      },
    },
    ...
    "web-search": {
      description: "Search the web with optional VEROQ trust scoring",
      execute: async (args) => {
        try {
