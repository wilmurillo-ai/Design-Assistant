---
name: ids_admin_app_tool_login
description: "관리자 APP 도구 로그인, 절차는 admin_login과 동일.
【호출 절차】
...
관리자 APP 도구 로그인, 절차는 admin_login과 동일.
【호출 절차】
1. 먼저 captcha_img를 호출하여 그래픽 인증 코드 획득
2. sign 계산: MD5(account + login_pwd + 'sjpOkkmhm9ds').toUpperCase()
3. POST: { account, login_pwd, captcha_code_key, captcha_code }
...
| ----- | ----- |
| Name | `ids_admin_app_tool_login` |
| Display Name | 관리자 APP 도구 로그인 |
| Method | `POST` |
| Endpoint | `/ids/admin_app_tool_login` |
