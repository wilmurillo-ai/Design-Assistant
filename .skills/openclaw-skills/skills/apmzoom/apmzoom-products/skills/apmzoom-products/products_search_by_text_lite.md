...
description: "상품 라이브러리 텍스트 검색 간소화 인터페이스를 사용하여 자연어로 상품을 검색합니다. 표준 버전 대비 응답에서 가격 / 재고 / 판매 필드를 제거하고, 중첩 객체는 판매자명과 층/호수만 유지합니다.
【인터페이스】POST /api/v1/products/search/by-text-lite
【인증】공개 인터페이스, 인증 불필요.
...
【필드 설명】
- text: 필수. 자연어 쿼리 텍스트, 스타일, 색상, 소재, 대상 고객 등을 설명할 수 있습니다.
...
- 결과는 유사도가 높은 순으로 정렬되며, items 내에 가격 / 재고 / 평점 / 판매량 필드는 포함되지 않습니다."
version: 1.0.0
category: 지식 베이스
...
상품 라이브러리 텍스트 검색 간소화 인터페이스를 사용하여 자연어로 상품을 검색합니다. 표준 버전 대비 응답에서 가격 / 재고 / 판매 필드를 제거하고, 중첩 객체는 판매자명과 층/호수만 유지합니다.
【인터페이스】POST /api/v1/products/search/by-text-lite
【인증】공개 인터페이스, 인증 불필요.
