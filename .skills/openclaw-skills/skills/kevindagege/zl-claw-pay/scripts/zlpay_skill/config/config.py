...
class Config:
    ...
    def get_log_level(cls) -> str:
        ...
        if cls._LOG_LEVEL is None:
            cls._LOG_LEVEL = os.getenv("ZLPAY_LOG_LEVEL", "INFO")
    ...
    def get_api_key(cls) -> str:
        ...
        if not api_key:
            raise ConfigError("ZLPAY_API_KEY is required in environment variables")
        return api_key
    
    @classmethod
    def get_api_key_from_config(cls) -> str:
        """Get API Key from config/.env file (loaded by dotenv)"""
    ...
    def get_gm_client_private_key(cls) -> str:
        ...
        raise ConfigError(
            "GM client private key required. "
            "Set ZLPAY_GM_CLIENT_PRIVATE_KEY or ZLPAY_GM_CLIENT_PRIVATE_KEY_PATH in environment"
        )
    
    @classmethod
    def get_gm_server_public_key(cls) -> str:
        """Get GM server public key from environment or file"""
        ...
        raise ConfigError(
            "GM server public key required. "
            "Set ZLPAY_GM_SERVER_PUBLIC_KEY or ZLPAY_GM_SERVER_PUBLIC_KEY_PATH in environment"
        )
    
    @classmethod
    def get_app_id(cls) -> str:
        """Get App ID from environment (required)"""
        app_id = os.getenv("ZLPAY_APP_ID")
        if app_id:
            return app_id
        
        raise ConfigError("ZLPAY_APP_ID is required in environment variables")
