...
安全声明:
  - 本脚本仅连接腾讯内部 iWiki MCP Server (prod.mcp.it.woa.com)
  - 所有操作需要用户通过 TAI_PAT_TOKEN 环境变量提供授权凭证
  - 支持的操作范围: iWiki 文档的读取、创建、更新和文件导入
  - 不收集、不传输任何用户个人数据
  - 凭证仅用于 HTTP Authorization header，不会持久化或转发到第三方
...
示例:
  ...
  # 文件上传导入（默认不覆盖，安全优先）:
  python connect_mcp.py upload ./doc.md 4017403457                     # 上传 Markdown 文件到指定父目录（不覆盖）
  python connect_mcp.py upload ./doc.docx 4017403457 --task-type md_import  # 指定任务类型
  python connect_mcp.py upload ./doc.md 4017403457 --cover             # 显式覆盖同名文档
  python connect_mcp.py upload ./doc.md 4017403457 --no-cover          # 不覆盖同名文档（默认行为）
...
class MCPClient:
    ...
    def upload_file(
        ...
        ⚠️ 安全说明: cover 默认为 False（不覆盖），需要覆盖时必须显式传入 cover=True。
        
        Args:
            file_path:   本地文件路径
            parent_id:   父文档/目录 ID，文件将导入到该目录下
            task_type:   导入任务类型，默认 'md_import'
            cover:       是否覆盖同名文档，默认 False（安全优先，不覆盖）
...
""")
            ...
            if user_input.lower().startswith("upload "):
                ...
                while i < len(upload_parts):
                    ...
                    print(f"   覆盖同名: {'是' if up_cover else '否（默认安全模式）'}")
