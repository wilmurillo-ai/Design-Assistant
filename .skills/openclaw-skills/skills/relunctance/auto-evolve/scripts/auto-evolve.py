...
class FourPerspectiveScanner:
    ...
    def scan(self) -> list[PerspectiveFinding]:
        ...
        The scan simulates receiving a Feishu message:
        ...
        for repo in self.repos:
            ...
            for perspective in ["USER", "PRODUCT", "PROJECT", "TECH"]:
                ...
                findings = self._scan_by_perspective(repo, perspective)
                if findings:
    ...
    def _scan_by_perspective(self, repo: Repository, perspective: str) -> list[PerspectiveFinding]:
        """Dispatch to the right perspective scanner."""
        if perspective == "USER":
            return self._scan_user(repo)
        elif perspective == "PRODUCT":
            return self._scan_product(repo)
        elif perspective == "PROJECT":
            return self._scan_project(repo)
        elif perspective == "TECH":
            return self._scan_tech(repo)
        return []

    def _scan_user(self, repo: Repository) -> list[PerspectiveFinding]:
        """
        👤 用户视角: Is the tool easy and pleasant to use?
        Asks: CLI design, error messages, learning curve, fault tolerance.
        """
        config = get_openclaw_llm_config()
        if not config.get("api_key") or not config.get("base_url"):
            return []
    ...
    def _scan_product(self, repo: Repository) -> list[PerspectiveFinding]:
        """
        📦 产品视角: Does it actually solve what it claims to solve?
        Asks: README promises vs reality, documented pain points still broken.
        """
        config = get_openclaw_llm_config()
        if not config.get("api_key") or not config.get("base_url"):
            return []
    ...
    def _scan_project(self, repo: Repository) -> list[PerspectiveFinding]:
        """
        🏗 项目视角: Is the project运作得好？
        Asks: learnings闭环, 巡检节奏, 配置合理性, 依赖健康.
        """
        config = get_openclaw_llm_config()
        if not config.get("api_key") or not config.get("base_url"):
            return []
    ...
    def _scan_tech(self, repo: Repository) -> list[PerspectiveFinding]:
        """
        ⚙️ 技术视角: Is the code healthy?
        This is the lowest priority — code quality is secondary to product truth.
