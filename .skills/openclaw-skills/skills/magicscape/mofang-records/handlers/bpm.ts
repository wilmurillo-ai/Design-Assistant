...
function buildHttpConfig(context: { config: Record<string, string> }): HttpClientConfig {
  return {
...
function activitiPrincipal(): string | undefined {
...
function findFirstEndEventId(xml: string): string | null {
  const m = xml.match(/<endEvent\b[^>]*\bid="([^"]+)"/);
  return m ? m[1] : null;
}

function findUserTaskIdByName(xml: string, taskName: string): string | null {
 ...
 */
async function tryQueryTasksRuntimeFallback(
  ...
  filterRecordId: string | undefined,
  page: number,
  pageSize: number
): Promise<{ success: boolean; message: string; data?: any }> {
  const hasVarFilter = !!filterFormId;
  const hasTaskShapeFilter = !!(
    filterParams.taskName ||
  ...
  if (simplePage) {
    let qs = `assignee=${encodeURIComponent(assignee)}&sort=createTime&order=desc&start=${
      (page - 1) * pageSize
    }&size=${pageSize}`;
    ...
    return {
      ...
      data: {
        ...
        queryFallbackNote:
          '当前环境 POST /activitirest/service/query/tasks 不可用（如 415），已改用 GET /runtime/tasks。',
      },
    };
  }
  ...
  while (scanned < QUERY_FALLBACK_MAX_SCAN) {
    ...
    const payload = res.data as { data?: any[] };
    const batch = payload?.data ?? [];
    if (batch.length === 0) break;

    for (const task of batch) {
      scanned++;
      if (filterParams.taskName && task.name !== filterParams.taskName) continue;
      ...
      if (hasVarFilter) {
        ...
        if (!vr.success || !Array.isArray(vr.data)) continue;
        const map = new Map(
          (vr.data as any[]).map((x: any) => [x.name, x.value != null ? String(x.value) : ''])
        );
        if (map.get('formId') !== filterFormId) continue;
        if (
          filterRecordId != null &&
          filterRecordId !== '' &&
  ...
  return {
    ...
    data: {
      ...
      queryFallbackUsed: true,
      queryFallbackNote:
        'POST /query/tasks 不可用，已 GET 分页拉取并在内存中按任务名/定义 key/formId/recordId 过滤。',
      scannedRuntimeTasks: scanned,
      ...(filterParams.processDefinitionName
        ? {
            queryFallbackWarning:
              'fallback 模式下未按 processDefinitionName 过滤（仅能用 processDefinitionKey 等）。',
          }
        : {}),
    },
  };
...
export async function listBpmTasksHandler(
  params: {
  ...
  const payload = res.data as { data?: unknown[]; total?: number };
...
export async function queryBpmTasksHandler(
  params: {
    formHint?: string;
    spaceHint?: string;
    recordId?: string;
  ...
  if (params.processDefinitionKey) body.processDefinitionKey = params.processDefinitionKey;
  if (params.processDefinitionName) body.processDefinitionName = params.processDefinitionName;
  if (params.taskName) body.name = params.taskName;
  if (params.taskDefinitionKey) body.taskDefinitionKey = params.taskDefinitionKey;

  const pvars: { name: string; value: string; operation: string }[] = [];
  ...
  if (res.success) {
    const d = res.data as { data?: unknown[]; total?: number };
    return {
      success: true,
      message: 'ok',
      data: {
  ...
  if (res.message.includes('415') || res.message.includes('405')) {
    ...
    return tryQueryTasksRuntimeFallback(
      config,
      {
...
/** 任务详情 + 变量（定位办理上下文） */
export async function getBpmTaskHandler(
  ...
  if (Array.isArray(variables)) {
    for (const v of variables as { name?: string; value?: string }[]) {
      if (v.name === 'recordId') recordId = v.value;
      if (v.name === 'spaceId') spaceId = v.value;
      if (v.name === 'formId') formId = v.value;
    }
  }
...
function buildKeepVariablesForTask(raw: any[]): any[] {
  const keep: any[] = [];
  for (const v of raw) {
    if (!v || typeof v !== 'object') continue;
    const name = v.name as string;
    if (!KEEP_COMPLETE_VAR_NAMES.has(name)) continue;
 ...
 */
export async function completeBpmTaskHandler(
  params: {
    taskId: string;
  ...
  if (!getV.success) return getV;

  let list: any[] = Array.isArray(getV.data) ? getV.data : [];
  ...
  if (Array.isArray(params.variables) && params.variables.length > 0) {
    keep = keep.concat(params.variables as any[]);
  }
...
export async function delegateBpmTaskHandler(
...
export async function claimBpmTaskHandler(
  params: { taskId: string; assignee?: string },
  context: { config: Record<string, string> }
): Promise<{ success: boolean; message: string; data?: any }> {
  const taskId = params.taskId?.trim();
...
export async function resolveBpmTaskHandler(
  params: { taskId: string },
  context: { config: Record<string, string> }
): Promise<{ success: boolean; message: string; data?: any }> {
  const taskId = params.taskId?.trim();
...
export async function jumpBpmTaskHandler(
  params: {
    taskId: string;
  ...
  const loadBpmnXml = async (): Promise<{ ok: false; message: string } | { ok: true; xml: string }> => {
    const tres = await bpmFetch(config, 'GET', `${ACT}/runtime/tasks/${encodeURIComponent(taskId)}`);
    if (!tres.success) return { ok: false, message: tres.message };
    const defId = tres.data?.processDefinitionId as string | undefined;
    if (!defId) return { ok: false, message: '无法从任务解析 processDefinitionId。' };
    const xmlres = await bpmFetch(
      config,
      'GET',
      `${ACT}/repository/process-definitions/${encodeURIComponent(defId)}/resourcedata`
    );
    if (!xmlres.success) return { ok: false, message: xmlres.message };
    const xml = xmlres.data?.xml as string | undefined;
    if (!xml) return { ok: false, message: '未获取到流程 BPMN XML。' };
    return { ok: true, xml };
  };
  ...
  if (kind === 'abort') {
    if (!targetId) {
      ...
      if (!('xml' in b)) return { success: false, message: b.message };
  ...
  return bpmFetch(
    config,
    'POST',
    `${ACT}/runtime/tasks/${encodeURIComponent(taskId)}/jump/${encodeURIComponent(targetId!)}/${kind}`,
    {}
  );
}
 ...
 */
export async function openBpmTransactionHandler(
  params: {
    processKey?: string;
...
export async function closeBpmTransactionHandler(
