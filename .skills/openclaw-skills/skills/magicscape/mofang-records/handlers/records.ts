...
const ERROR_PATTERNS: Array<{ pattern: RegExp; hint: string }> = [
  {
...
function diagnoseServerError(msg: string): string {
...
function coalesceQueryFilters(params: {
  filters?: BqFilter[];
  filter?: { items?: LegacyFilterItem[] };
}): { effective?: BqFilter[]; error?: string } {
  ...
  if (typeof legacy !== 'object' || !Array.isArray(legacy.items) || legacy.items.length === 0) {
    return {
      effective: undefined,
      error:
        '参数错误：本命令只接受 filters（数组），不要使用魔方前端的 filter / filterType / value2 / filterGroup。日期区间示例：{"filters":[{"fieldName":"填报日期","operator":"between","value":"2026-03-01,2026-04-01"}]}',
    };
  }
  ...
  for (const item of legacy.items) {
    ...
    if (!fieldName) {
      return {
        effective: undefined,
        error:
          'filter.items 中存在缺少 fieldName 的项。请改用手写 filters 数组，每项含 fieldName、operator、value。',
      };
    }
    ...
    if (ft === 'between') {
      ...
      if (!v1 || !v2) {
        return {
          effective: undefined,
          error:
            'filter 中 BETWEEN 缺少 value 或 value2。CLI 请使用：{"filters":[{"fieldName":"…","operator":"between","value":"开始日期,结束日期"}]}（两个日期用英文逗号放在一个 value 里）。',
        };
      }
