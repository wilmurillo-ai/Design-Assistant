...
function extractSpaces(data: any): { label: string; id: string }[] {
  const items = data?.items;
  if (!Array.isArray(items)) return [];
  return items.map((item: any) => ({ label: item.label, id: item.id }));
}

function extractForms(data: any): { label: string; id: string }[] {
...
async function querySpaces(
  ...
  if (spaces.length === 0) {
    ...
    if (likeResult.success) spaces = extractSpaces(likeResult.data);
  }
  return spaces;
}
...
async function queryForm(
  ...
  if (forms.length === 0) {
    ...
    const likeResult = await apiRequest(config, 'GET', likePath);
    if (likeResult.success) forms = extractForms(likeResult.data);
  }
  if (forms.length === 0) {
    ...
    if (listResult.success) {
      ...
      const fuzzy = allForms.filter(
        (f) => f.label.includes(formLabel) || formLabel.includes(f.label)
      );
...
async function listAllSpaces(config: HttpClientConfig): Promise<{ label: string; id: string }[]> {
 ...
 */
export async function resolveSpaceForm(
  config: HttpClientConfig,
  formHint: string,
  spaceHint?: string
): Promise<ResolveResult> {
