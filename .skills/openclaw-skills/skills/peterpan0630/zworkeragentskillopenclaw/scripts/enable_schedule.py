...
def main():
    parser = argparse.ArgumentParser(description='启用zworker定时计划')
    ...
    try:
        result = set_schedule(enable=True, schedule_id=args.id, schedule_name=args.name)
        ...
        if args.quiet:
            ...
            print(f"❌ 定时计划启动失败: {e}", file=sys.stderr)
        sys.exit(1)
