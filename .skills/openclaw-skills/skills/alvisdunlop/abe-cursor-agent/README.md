...
The `cursor-agent` skill encapsulates workflows and commands for the Cursor CLI, enabling efficient AI-pair programming directly from the terminal. This skill includes all modern features from the January 2026 update. AI capabilities are routed through SkillBoss API Hub, providing unified access to multiple AI models via a single API key.
...
- **SkillBoss API Hub Integration**: Unified authentication via `SKILLBOSS_API_KEY`, AI model auto-routing via `/v1/pilot`
...
- Model switching and configuration (auto-routed via SkillBoss API Hub)
