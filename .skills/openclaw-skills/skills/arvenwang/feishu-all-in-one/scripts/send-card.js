...
// 发送卡片到指定 chat_id 或 open_id
async function sendCardToChatId(appId, appSecret, chatId, card, receiveIdType = 'chat_id') {
...
async function main() {
  ...
  let templatePath = '';
  let options = [];
  let receiveIdType = 'chat_id'; // 默认使用 chat_id
  
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--chat-id') {
      chatId = args[++i];
    } else if (args[i] === '--receive-id-type') {
  ...
  // 发送卡片
  try {
    const result = await sendCardToChatId(config.appId, config.appSecret, chatId, card, receiveIdType);
    console.log('✅ 卡片发送成功！');
