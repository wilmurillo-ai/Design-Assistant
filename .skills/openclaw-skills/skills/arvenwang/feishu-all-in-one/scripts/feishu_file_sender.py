...
def resolve_agent_id(config: Dict[str, Any]) -> str:
    ...
    # Support both agents.list and agents.defaults formats
    agents_list = config.get("agents", {}).get("list", [])
    defaults = config.get("agents", {}).get("defaults", {})
    
    # Add defaults as a pseudo-agent if workspace exists
    if defaults.get("workspace"):
        agents_list = agents_list + [{"id": "main", **defaults}]
...
def resolve_feishu_account(
    ...
    # First try bindings (standard way)
    ...
    # If no binding, try to find from feishu.accounts directly
    if not account_id:
        feishu_accounts = config.get("channels", {}).get("feishu", {}).get("accounts", {})
        # Try agent_id first, then "main", then first available
        for key in [agent_id, "main", "default"]:
            if key in feishu_accounts:
                account_id = key
                break
