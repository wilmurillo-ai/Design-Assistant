...
def train_model():
    ...
    model_file = f"{CONFIG['model_dir']}/stock_lightgbm_v23.txt"
    ...
    model_info = {
        ...
        'metrics': {'accuracy': round(accuracy, 3), 'precision': round(precision, 3), 'recall': round(recall, 3), 'f1': round(f1, 3), 'auc': round(auc, 3)},
        'training_stocks': sample_size
    }
    
    with open(f"{CONFIG['model_dir']}/model_info_v23.json", 'w') as f:
...
def run_prediction(model, feature_cols):
    ...
    for i, stock in enumerate(stocks):
        ...
        results.append({
            'stock_code': ts_code, 'stock_name': name,
    ...
    with open(f'{out_dir}/ml_prediction_nextday_{date_str}.json', 'w') as f:
        json.dump({
            'timestamp': ts, 'model': 'LightGBM v23.0',
            'target': f'次日上涨{CONFIG["target_gain"]}%+',
            'prob_threshold': CONFIG['prob_threshold'],
            'total_screened': len(results), 'qualified': len(qualified),
            'top_20': qualified[:20]
        }, f, indent=2, ensure_ascii=False)
    
    print(f'\n✅ 推荐已保存：{out_dir}/ml_prediction_nextday_{date_str}.json')
    print("\n" + "="*80)
...
if __name__ == '__main__':
    ...
    run_prediction(model, feature_cols)
