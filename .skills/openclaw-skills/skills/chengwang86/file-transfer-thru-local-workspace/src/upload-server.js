...
║  Workspace: ${WORKSPACE}                          ║
║  上传目录：${UPLOADS_DIR}                        ║
║  Token 认证：${process.env.GATEWAY_AUTH_VALUE ? '已启用' : '未启用'}                        ║
║  技能包浏览下载：支持                                 ║
╚═══════════════════════════════════════════════════════╝
`);
...
function zipSkill(skillName, res) {
    ...
    // 使用 zip 命令打包，排除 node_modules 目录（太大且可通过 npm install 重新安装）
    // -r: 递归打包
    // -x: 排除指定模式
...
const server = http.createServer((req, res) => {
    ...
    // 下载文件 API
    if (req.method === 'GET' && parsedUrl.pathname.startsWith('/api/files/')) {
        const tokenParam = parsedUrl.searchParams.get('token');
        ...
        try {
            ...
            res.end(fileData);
            console.log(`[${new Date().toISOString()}] 📥 文件下载：${safeFilename}`);
        } catch (error) {
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🎁 技能包浏览下载：http://${ip}:${PORT}/ (查看技能列表)`);
