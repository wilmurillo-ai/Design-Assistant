# ValueSider Superinvestor Data (OpenClaw Skill)

动态采集 [ValueSider](https://valuesider.com) 上 Superinvestors 的 13F 持仓与买卖活动，供 OpenClaw 调用。

## 安装依赖

```bash
cd valuesider-superinvestor-data
...
```bash
clawhub publish ./valuesider-superinvestor-data --slug valuesider-superinvestor-data --name "ValueSider Superinvestor Data" --version 1.0.0 --tags latest
```
...
```bash
cd valuesider-superinvestor-data
clawhub publish . --slug valuesider-superinvestor-data --name "ValueSider Superinvestor Data" --version 1.0.0 --tags latest
```

其他人可通过 `clawhub install valuesider-superinvestor-data` 安装。
