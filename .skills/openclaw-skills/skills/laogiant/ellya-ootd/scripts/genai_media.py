"""
Generate and analyze images using various AI providers (Gemini, Minimax).
Requires API keys in environment: GEMINI_API_KEY, MINIMAX_API_KEY
...
from image_provider import create_provider, resolve_provider, available_providers


# genai_media.py now delegates provider selection and image generation to image_provider.py
...
# Image saving and provider-specific helpers are implemented in image_provider.py



def generate_image_with_provider(prompt: str, input_images: list[str] | None, provider: str) -> list[str]:
    provider = resolve_provider(provider)
    provider_obj = create_provider(provider)
    final_prompt = build_generation_prompt(prompt)
    return provider_obj.generate_images(final_prompt, input_images)
...
def do_generate_series(
    ...
    if gemini_key:
        analysis_client = genai.Client(api_key=gemini_key)
    ...
    if analysis_client:
        scene, character = extract_scene_and_character(image_part, gemini_key)
        if scene:
            print(f"Scene   : {scene}")
        if character:
        ...
        mode, context, variations = classify_scene_and_generate_context(scene, character, count, gemini_key)
    else:
        print("Gemini API key not configured; skipping scene extraction/classification.")
        scene, character = "", ""
        mode, context = "pose", ""
        variations = generate_pose_variations(scene, character, context, count, "")
    print(f"Mode: {mode}")
    ...
    for idx, variation in enumerate(variations):
        ...
        try:
            saved = provider_obj.generate_images(variation_prompt, [input_image])
        except Exception as exc:
...
def do_generate(prompt: str, input_images: list[str] | None, provider: str = "minimax") -> None:
    provider = resolve_provider(provider)
    generate_image_with_provider(prompt, input_images, provider)
...
def generate_main() -> None:
    ...
    parser.add_argument("--provider", choices=["gemini", "minimax"], default=None, help="AI provider to use")

    args = parser.parse_args()
    provider = resolve_provider(args.provider)
    provider_obj = create_provider(provider)
    final_prompt = resolve_final_prompt(args.prompt, args.styles, provider_obj.api_key)
    do_generate(final_prompt, args.input_images, provider)
...
def analyze_main() -> None:
    ...
    if not api_key:
        sys.exit("Error: GEMINI_API_KEY is missing for analysis.")
...
def series_main() -> None:
    ...
    provider = resolve_provider(args.provider)
    do_generate_series(
        args.input_image, args.count, args.variations, provider,
    )
