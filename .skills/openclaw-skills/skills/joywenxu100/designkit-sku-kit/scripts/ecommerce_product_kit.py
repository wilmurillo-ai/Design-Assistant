...
TARGET_PRESETS = {
    "walmart": {
        ...
        "language": "English",
        "aspect_ratio": "1:1",
        "channel_fit": "按 Walmart 主图规范优先：纯白背景(#FFFFFF)、单品居中、主体占比 80-90%、禁止文字/水印/logo/拼图。",
    },
    "mercadolibre": {
        "platform": "amazon",
        "market": "ES",
        "language": "Spanish",
        "aspect_ratio": "1:1",
        "channel_fit": "按 MercadoLibre 风格优先：主图白底高对比、信息直给；文案与卖点偏西语电商表达，避免冗长。",
    },
}
...
def _normalize_preset(raw: str) -> str:
    ...
    if text in ("walmart", "沃尔玛"):
        return "walmart"
    if text in ("mercadolibre", "meli", "mercado", "美客多", "美克多", "美客多平台"):
        return "mercadolibre"
    return ""


def resolve_target_profile(inp: Dict[str, Any]) -> Dict[str, str]:
    preset_raw = str(inp.get("target_preset", "") or inp.get("target_market", "") or inp.get("channel", ""))
    preset = _normalize_preset(preset_raw)

    base_platform = str(inp.get("platform", "amazon")).strip() or "amazon"
    base_market = str(inp.get("market", "US")).strip() or "US"
    base_language = str(inp.get("language", "English")).strip() or "English"
    base_ratio = str(inp.get("aspect_ratio", inp.get("ratio", "1:1"))).strip() or "1:1"

    if preset and preset in TARGET_PRESETS:
        cfg = TARGET_PRESETS[preset]
        return {
            "preset": preset,
            "platform": str(inp.get("platform") or cfg["platform"]),
            "market": str(inp.get("market") or cfg["market"]),
            "language": str(inp.get("language") or cfg["language"]),
            "aspect_ratio": str(inp.get("aspect_ratio", inp.get("ratio")) or cfg["aspect_ratio"]),
            "channel_fit": cfg["channel_fit"],
        }
    ...
    return {
        ...
        "aspect_ratio": base_ratio,
        "channel_fit": "按目标平台通用规范：主图清晰、信息可读、避免夸张效果。",
    }
...
def cmd_style_create(inp: Dict[str, Any]) -> None:
    ...
    channel_fit = inp.get("channel_fit", "按目标平台通用规范：主图清晰、信息可读、避免夸张效果。")
