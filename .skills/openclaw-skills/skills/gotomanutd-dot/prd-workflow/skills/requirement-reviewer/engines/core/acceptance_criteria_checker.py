...
class AcceptanceCriteriaChecker:
    ...
    def extract_acceptance_section(self, prd_content: str) -> str:
        """提取验收标准章节"""
        # 优先匹配"第 5 章 验收标准"格式（硬编码第 5 章到第 6 章）
        chapter_pattern = r'(#+\s*第 5 章[^\n]*\n.*?)(?=#+\s*第 6 章|$)'
        match = re.search(chapter_pattern, prd_content, re.DOTALL)
        if match:
            return match.group(1)

        # 备用模式：匹配 Markdown 标题格式
        patterns = [
            r'(#{1,2}\s*验收标准 [^\n]*\n.*?)(?=#{1,2}\s+|$)',
            r'(验收标准.*?)(?=#{1,2}\s+|$)',
            r'(测试用例.*?)(?=#{1,2}\s+|$)',
            r'(Acceptance Criteria.*?)(?=#{1,2}\s+|$)',
        ]
    ...
    def check_required_elements(self, section: str) -> List[Dict]:
        ...
        for element in self.REQUIRED_ELEMENTS:
            if element not in section:
                # 检查是否有等价替代（如 GWT 格式可以替代前置条件/操作步骤/预期结果）
                has_equivalent = self.check_equivalent_elements(section, element)
    ...
    def check_equivalent_elements(self, section: str, element: str) -> bool:
        """检查是否有等价替代元素"""
        # GWT 格式可以替代前置条件/操作步骤/预期结果
        if element in ["前置条件", "操作步骤", "预期结果"]:
            # 检查单行 GWT 格式
            gwt_matches = self.GWT_PATTERN.findall(section)

            # 检查表格分行 GWT 格式（| Given | ... 单独行）
            given_count = len(re.findall(r'^\|\s*Given\s*\|', section, re.MULTILINE))
            when_count = len(re.findall(r'^\|\s*When\s*\|', section, re.MULTILINE))
            then_count = len(re.findall(r'^\|\s*Then\s*\|', section, re.MULTILINE))
            table_gwt_groups = min(given_count, when_count, then_count)

            # 有 3 组以上 GWT 格式（单行或表格），认为等价
            if len(gwt_matches) >= 3 or table_gwt_groups >= 3:
                return True
