import { StardotsClient } from './stardots-client';
...
export default class StardotsBackupSkill {
  ...
  description = '自动将图像备份到 Stardots.io 平台';
  version = '1.0.0';

  private client: StardotsClient | null = null;
  private config: StardotsConfig | null = null;
  ...
  async initialize(config: Record<string, any>): Promise<void> {
    this.config = {
      ...
      space: config.space as string || 'default',
    };
    this.client = new StardotsClient(this.config);
  }
  ...
  private matchUploadIntent(message: string): boolean {
  ...
  private matchListIntent(message: string): boolean {
    const lowerMsg = message.toLowerCase();
  ...
  private async handleUpload(message: string): Promise<SkillResult> {
  ...
  private async handleList(): Promise<SkillResult> {
    if (!this.client) {
      throw new Error('技能未初始化');
    }
    ...
    try {
      ...
      const fileList = result.files.map(file => 
        `- ${file.name} (${(file.size / 1024).toFixed(2)} KB) [${new Date(file.createdAt).toLocaleString()}]`
      ).join('\n');
