...
try { require('dotenv').config({ path: path.resolve(__dirname, './.env') }); } catch (e) { console.warn('[LSHML] Warning: dotenv not found or failed to load .env'); }
const fs = require('fs');
  ...
  try {
    if (fs.existsSync(lockFile)) {
      ...
      if (!Number.isFinite(pid) || pid <= 0) {
        ...
        try {
          process.kill(pid, 0);
          console.log(`[Singleton] LSHML loop already running (PID ${pid}). Exiting.`);
          return false;
        } catch (e) {
...
async function main() {
  ...
  // Dashboard mode — launch the web UI
  if (args.includes('--dashboard')) {
    const { startServer } = require('./dashboard/server');
    const port = parseInt(process.env.LSHML_DASHBOARD_PORT, 10) || 8420;
    startServer(port);
    return; // keep server running
  }

  if (command === 'run' || command === '/evolve' || isLoop) {
    console.log('Starting self-healing ML engine...');
    ...
    console.log(`Usage: node index.js [run|/evolve|solidify|distill|asset-log] [--loop] [--dashboard]
  - --dashboard       Launch web dashboard (port 8420, or LSHML_DASHBOARD_PORT)
  - solidify flags:
    - --dry-run
    - --no-rollback
