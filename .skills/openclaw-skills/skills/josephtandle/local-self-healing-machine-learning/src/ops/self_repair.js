...
function repair(gitRoot) {
    ...
    // Only enabled if EVOLVE_GIT_RESET=true AND there are no uncommitted changes.
    if (process.env.EVOLVE_GIT_RESET === 'true') {
        try {
            // Safety: refuse to hard reset if there are uncommitted changes
            var status = '';
            try {
                status = execSync('git status --porcelain', { cwd: root, encoding: 'utf8', timeout: 10000 }).trim();
            } catch (e) {}
            if (status) {
                console.warn('[SelfRepair] SKIPPING hard reset — uncommitted changes detected (' + status.split('\n').length + ' files). Stash or commit first.');
                repaired.push('hard_reset_skipped_dirty_tree');
            } else {
                console.log('[SelfRepair] Resetting local branch to origin/main (HARD reset)...');
                execSync('git fetch origin main', { cwd: root, stdio: 'ignore', timeout: 30000 });
