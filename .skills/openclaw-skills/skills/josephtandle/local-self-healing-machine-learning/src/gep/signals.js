...
// --- ML Feedback Loop integration (Phase 1) ---
...
function extractSignals({ recentSessionTranscript, todayLog, memorySnippet, userSnippet, recentEvents }) {
  ...
  var deduped = Array.from(new Set(signals));

  // ML Feedback Loop: check outcomes against final signals
  if (_feedbackLoop) {
    try { _feedbackLoop.checkOutcomes(deduped); } catch (e) {}
  }
