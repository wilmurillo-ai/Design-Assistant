...
function readRecentCandidates(limit = 20) {
  try {
    ...
    // Deduplicate by id, keeping the one with latest created_at
    var byId = {};
    all.forEach(function (c) {
      if (!c || !c.id) return;
      var existing = byId[c.id];
      if (!existing || (c.created_at && (!existing.created_at || c.created_at > existing.created_at))) {
        byId[c.id] = c;
      }
    });
...
function writeAllCandidates(candidates) {
  var dir = getGepAssetsDir(); ensureDir(dir);
  var lines = candidates.map(function (c) { return JSON.stringify(c); }).join('\n');
  if (lines) lines += '\n';
  var tmp = candidatesPath() + '.tmp';
  fs.writeFileSync(tmp, lines, 'utf8');
  fs.renameSync(tmp, candidatesPath());
}

function updateCandidateStatus(candidateId, status, meta) {
  var all = readAllCandidates();
  var updated = false;
  all.forEach(function (c) {
    if (c && c.id === candidateId) {
      c.status = status;
  ...
  if (updated) writeAllCandidates(all);
  return updated;
}

function reconcileAllCandidates(event) {
  var { reconcileCandidates } = require('./candidates');
  var all = readAllCandidates();
  var reconciled = reconcileCandidates(all, event);
  writeAllCandidates(reconciled);
  return reconciled;
}
...
module.exports = {
  ...
  updateCandidateStatus, reconcileAllCandidates,
};
