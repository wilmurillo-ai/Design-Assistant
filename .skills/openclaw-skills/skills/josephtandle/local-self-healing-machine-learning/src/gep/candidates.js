...
function reconcileCandidates(candidates, event) {
  if (!Array.isArray(candidates)) return [];
  var eventSignals = event && Array.isArray(event.signals) ? event.signals : [];
  var eventId = event && event.id ? String(event.id) : '';
  var now = Date.now();
  var staleMs = STALE_DAYS * 24 * 60 * 60 * 1000;

  return candidates.map(function (c) {
    if (!c || typeof c !== 'object') return c;
    // Already implemented — leave as-is
    if (c.status === 'implemented') return c;

    // Check signal overlap with the event
    var candidateSignals = Array.isArray(c.signals) ? c.signals : [];
    var matched = eventId && candidateSignals.some(function (s) {
      return eventSignals.indexOf(s) !== -1;
    });

    if (matched) {
      c.status = 'implemented';
      c.implementedBy = eventId;
      c.implementedAt = new Date().toISOString();
      return c;
    }

    // Mark stale if older than 30 days
    var createdAt = c.created_at ? new Date(c.created_at).getTime() : 0;
    if (createdAt > 0 && (now - createdAt) > staleMs) {
      if (c.status !== 'stale') c.status = 'stale';
      return c;
    }
