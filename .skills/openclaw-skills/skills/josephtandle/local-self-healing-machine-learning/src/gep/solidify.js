...
const { loadGenes, upsertGene, appendEventJsonl, appendCapsule, upsertCapsule, getLastEventId, appendFailedCapsule, reconcileAllCandidates } = require('./assetStore');
...
// ML modules (Phase 1 + Phase 4)
let _feedbackLoop = null;
let _knowledgeBase = null;
try { _feedbackLoop = require('../ml/feedbackLoop'); } catch (e) {}
try { _knowledgeBase = require('../ml/knowledgeBase'); } catch (e) {}
...
function applyEpigeneticMarks(gene, envFingerprint, outcomeStatus) {
  ...
  // Keep max 50 marks — no time-based decay. Knowledge compounds forever.
  // (Previously: 90-day decay + max 10. Replaced in Phase 4 of ML enhancement.)
  gene.epigenetic_marks = gene.epigenetic_marks
    .filter((m) => m && m.context)
    .slice(-50);
...
function solidify({ intent, summary, dryRun = false, rollbackOnFailure = true } = {}) {
  ...
  const event = {
    ...
    meta: {
      ...
      ml_stats: (function() {
        try {
          var stats = {};
          if (_feedbackLoop) stats.feedback = _feedbackLoop.getFeedbackStats();
          if (_knowledgeBase) stats.knowledge = _knowledgeBase.getKnowledgeStats();
          return stats;
        } catch (e) { return null; }
      })(),
    },
  ...
  // ML Feedback Loop: record that this gene was applied (Phase 1)
  if (!dryRun && _feedbackLoop && geneUsed && geneUsed.id) {
    try {
      const envLabel = envFp ? [envFp.platform, envFp.arch, envFp.node_version].filter(Boolean).join('/') : 'unknown';
      _feedbackLoop.recordApplication({ geneId: geneUsed.id, signals, environment: envLabel });
    } catch (e) {}
  }

  // ML Knowledge Base: record outcome for persistent compounding knowledge (Phase 4)
  if (!dryRun && _knowledgeBase && geneUsed && geneUsed.id) {
    try {
      const envLabel = envFp ? [envFp.platform, envFp.arch, envFp.node_version].filter(Boolean).join('/') : 'unknown';
      const errorHash = _feedbackLoop ? _feedbackLoop.hashError(signals.filter(s => String(s).startsWith('errsig:')).join('|')) : null;
      if (errorHash) {
        _knowledgeBase.recordOutcome({ errorHash, geneId: geneUsed.id, success, environment: envLabel });
      }
    } catch (e) {}
  }
  ...
  if (!dryRun) {
    ...
    try { reconcileAllCandidates(event); } catch (e) { /* non-blocking */ }
    if (capsule) {
  ...
  // Auto-publish disabled (local-only build).
  let publishResult = null;
  // Anti-pattern auto-publish disabled (local-only build).
