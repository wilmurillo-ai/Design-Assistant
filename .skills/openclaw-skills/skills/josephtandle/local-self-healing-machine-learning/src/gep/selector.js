...
// ML Predictor integration (Phase 3) — sync wrapper for async predictor
let _predictor = null;
let _knowledgeBase = null;
try { _predictor = require('../ml/predictor'); } catch (e) {}
...
function selectGeneAndCapsule({ genes, capsules, signals, memoryAdvice, driftEnabled, failedCapsules }) {
  ...
  // Check ML knowledge base for recommendation before standard selection
  let mlPreferredGeneId = null;
  let mlSelectionMethod = 'signal_matching';
  if (_knowledgeBase) {
    try {
      const errorSigs = (signals || []).filter(s => String(s).startsWith('errsig:'));
      if (errorSigs.length > 0) {
        const errorHash = require('crypto').createHash('sha256')
          .update(errorSigs.join('|')).digest('hex').slice(0, 16);
        const rec = _knowledgeBase.recommend(errorHash);
        if (rec && rec.gene_id && rec.confidence >= 0.5) {
          mlPreferredGeneId = rec.gene_id;
          mlSelectionMethod = 'knowledge_base';
        }
      }
    } catch (e) {}
  }

  // ML predictor status (async prediction happens in evolve.js before selectGeneAndCapsule)
  if (!mlPreferredGeneId && _predictor && _predictor.isModelReady()) {
    mlSelectionMethod = 'predictor_available';
  }

  const effectivePreferred = mlPreferredGeneId || preferredGeneId;

  const { selected, alternatives, driftIntensity } = selectGene(genes, signals, {
    bannedGeneIds: effectiveBans,
    preferredGeneId: effectivePreferred,
    driftEnabled: !!driftEnabled,
  });
  ...
  if (mlSelectionMethod !== 'signal_matching') {
    selector.reason.push('ml_selection_method: ' + mlSelectionMethod);
    if (mlPreferredGeneId) selector.reason.push('ml_preferred_gene: ' + mlPreferredGeneId);
  }
  return {
