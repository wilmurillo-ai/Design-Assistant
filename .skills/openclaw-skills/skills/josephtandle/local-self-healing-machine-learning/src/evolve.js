...
function checkSystemHealth() {
  ...
  try {
    ...
    // INTEGRATION_STATUS_CMD removed (security hardening).
    // Running arbitrary shell commands from env vars is a command injection vector.
    // Integration health should be checked via structured APIs, not exec().
...
function performMaintenance() {
  ...
  try {
    ...
    if (evolverFiles.length > 0) {
      console.log(`[Maintenance] Cleaned ${evolverFiles.length} LSHML hand session(s).`);
    }
...
// --- Auto-update: DISABLED by default (security hardening) ---
// Auto-update silently downloads and installs code from remote registry.
// This is a supply chain risk — if the registry is compromised, arbitrary
// code gets pulled onto the machine. Require explicit opt-in via config.
function checkAndAutoUpdate() {
  try {
    const configPath = path.join(os.homedir(), '.openclaw', 'openclaw.json');
    let autoUpdate = false; // CHANGED: default OFF, require explicit opt-in
    let intervalHours = 6;
    try {
      if (fs.existsSync(configPath)) {
        const cfg = JSON.parse(fs.readFileSync(configPath, 'utf8'));
        if (cfg.evolver && cfg.evolver.autoUpdate === true) autoUpdate = true;
    ...
    console.log('[AutoUpdate] Auto-update is enabled via openclaw.json config.');
...
async function run() {
  ...
  if (process.platform !== 'win32') {
    try {
      ...
      if (_psRace && _psRace.length > 0) {
        console.log('[LSHML] Another Hand Agent is already running. Yielding this cycle.');
        return;
      }
    } catch (_) {
  ...
  if (activeUserSessions > QUEUE_MAX) {
    console.log(`[LSHML] Agent has ${activeUserSessions} active user sessions (max ${QUEUE_MAX}). Backing off ${QUEUE_BACKOFF_MS}ms to avoid starving user conversations.`);
  ...
  if (sysLoad.load1m > LOAD_MAX) {
    console.log(`[LSHML] System load ${sysLoad.load1m.toFixed(2)} exceeds max ${LOAD_MAX.toFixed(1)} (auto-calculated for ${os.cpus().length} cores). Backing off ${QUEUE_BACKOFF_MS}ms.`);
    writeDormantHypothesis({
  ...
  try {
    ...
    if (e && e.code === 'ENOENT') {
      console.warn('[LSHML] CWD lost (ENOENT). Recovering to REPO_ROOT: ' + REPO_ROOT);
      try { process.chdir(REPO_ROOT); } catch (e2) {
        console.error('[LSHML] CWD recovery failed: ' + (e2 && e2.message ? e2.message : e2));
        throw e;
      }
    } else {
      throw e;
  ...
  try {
    ...
    console.error('[LSHML] FATAL: Not a git repository (' + REPO_ROOT + ').');
    console.error('[LSHML] Self-healing ML requires git for rollback, blast radius calculation, and solidify.');
    console.error('[LSHML] Run "git init && git add -A && git commit -m init" in your project root, then try again.');
    process.exitCode = 1;
    return;
  }
  ...
  // ML: Enhance signals with embedding-based error clusters (Phase 2)
  try {
    const { enhanceSignalsWithClusters } = require('./ml/errorClassifier');
    const enhanced = await enhanceSignalsWithClusters(signals);
    if (enhanced && enhanced.length > signals.length) {
      for (const s of enhanced) {
        if (!signals.includes(s)) signals.push(s);
      }
      console.log('[ML] Enhanced signals with ' + (signals.length - enhanced.length + (enhanced.length - signals.length)) + ' cluster signal(s).');
    }
  } catch (e) {
    // ML enhancement is optional — fallback to regex signals
  }

  // ML: Trigger predictor retraining if needed (Phase 3)
  try {
    const { retrainIfNeeded } = require('./ml/trainer');
    const trainResult = await retrainIfNeeded();
    if (trainResult && trainResult.retrained) {
      console.log('[ML] Predictor retrained: ' + trainResult.sample_count + ' samples, ' + trainResult.gene_count + ' genes.');
    }
  } catch (e) {
    // Non-blocking
  }
  ...
  // SECURITY: Hub task claiming is opt-in only. An external hub server should not
  // be able to push arbitrary tasks that get auto-executed on this machine.
  // Set EVOLVE_HUB_TASKS=true to enable.
  const hubTasksEnabled = String(process.env.EVOLVE_HUB_TASKS || '').toLowerCase() === 'true';
  let activeTask = null;
  ...
  // ML: Get async predictions from trained model (Phase 3)
  let mlPredictions = null;
  try {
    const { isModelReady, predict } = require('./ml/predictor');
    if (isModelReady()) {
      const predResult = await predict(signals);
      if (predResult && predResult.ok && predResult.predictions.length > 0) {
        mlPredictions = predResult.predictions;
        console.log('[ML] Predictor suggests: ' + predResult.predictions.slice(0, 3).map(p => p.gene_id + '(' + p.confidence.toFixed(2) + ')').join(', '));
        // Feed the top prediction to memoryAdvice as preferred gene
        if (!memoryAdvice) memoryAdvice = {};
        if (!memoryAdvice.preferredGeneId && predResult.predictions[0].confidence >= 0.6) {
          memoryAdvice.preferredGeneId = predResult.predictions[0].gene_id;
          memoryAdvice.explanation = memoryAdvice.explanation || [];
          memoryAdvice.explanation.push('ml_predictor: ' + predResult.predictions[0].gene_id + ' (confidence=' + predResult.predictions[0].confidence.toFixed(2) + ')');
        }
      }
    }
  } catch (e) {
