    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        try {
          ...
          // DELETE returns empty body or just {success: true}
          if (res.statusCode >= 200 && res.statusCode < 300) {
...
module.exports = async (postId) => {
  if (!postId) {
    ...
    console.error('Usage: openclaw-facebook-posting fb-post-delete <post_id>');
    process.exit(1);
  }
  ...
  console.log('📝 Note: Facebook deprecated direct deletion via API (v2.4+)');
  ...
  try {
    ...
    // Step 2: Try to delete the post directly first (may work for some pages)
    console.log(`🗑️  Attempting to delete post: ${postId}`);
    console.log(`📝 Note: Facebook deprecated direct deletion in Graph API v2.4`);
    console.log(`📝 If this fails, we'll hide the post instead.\n`);
    ...
    try {
      ...
      console.log(`⚠️  Deletion failed (expected for most pages): ${deleteError.message}`);
      console.log(`🔄 Falling back to hiding the post...\n`);
      
      // Step 3: Hide (unpublish) the post as fallback
    ...
    console.error('❌ Failed to delete/hide post:', error.message);
...
if (require.main === module) {
  ...
  if (!postId) {
    console.error('❌ Error: post_id is required');
    console.error('Usage: openclaw-facebook-posting fb-post-delete <post_id>');
    console.error('\nNote: This command attempts to delete a post, but falls back to hiding it');
    console.error('      if deletion fails (which is expected for most pages due to API deprecation).');
    console.error('      For a hide-only command, use: fb-post-hide <post_id>');
    process.exit(1);
  }
