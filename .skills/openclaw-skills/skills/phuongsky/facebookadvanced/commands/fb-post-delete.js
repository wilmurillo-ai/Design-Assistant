#!/usr/bin/env node
/**
 * OpenClaw Command: fb-post-delete
...
program
  .name('fb-post-delete')
  .description('Delete a post from Facebook Page')
  .argument('<post_id>', 'The ID of the post to delete')
  ...
  .action(async (postId, options) => {
    ...
    // Confirm deletion
    if (!options.confirm) {
      const confirm = await question('⚠️  This will permanently delete the post. Continue? (y/N): ');
