...
def convert_image(src: str) -> str:
    ...
    if p.suffix.lower() in (".jpg", ".jpeg", ".png"):
        return src
    ff = _find_ffmpeg()
    dst = tempfile.mktemp(suffix=".jpg", prefix="aa_img_")
...
def convert_video(src: str) -> str:
    ...
    ff = _find_ffmpeg()
    dst = tempfile.mktemp(suffix=".mp4", prefix="aa_vid_")
    # ensure ≥24fps, H.264
...
def main():
    ...
    try:
        ...
        if args.download:
            ...
            urllib.request.urlretrieve(final_url, args.output)
            size = Path(args.output).stat().st_size
            print(f"Saved {args.output} ({size//1024}KB)")
