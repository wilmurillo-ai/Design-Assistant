...
function pasteWindows(retries: number, delayMs: number, targetApp?: string): boolean {
  const activateCmd = targetApp
    ? `
    $procs = Get-Process -Name "${targetApp.replace(/"/g, '`"')}" -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowHandle -ne 0 }
    if ($procs) {
...
"@
  ...
  for (let i = 0; i < retries; i++) {
    const result = spawnSync('powershell.exe', ['-NoProfile', '-Sta', '-Command', ps], { stdio: 'pipe' });
    if (result.status === 0) {
      return true;
    }
    const stderr = result.stderr?.toString().trim();
