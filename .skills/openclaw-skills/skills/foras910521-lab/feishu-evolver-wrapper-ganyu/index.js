...
async function run() {
    ...
    // 2. Resolve Core Evolver Path via config (fixed: use absolute path from config)
    const config = require('./config');
    const evolverDir = config.resolveEvolverCore().path;
    if (!evolverDir || !fs.existsSync(evolverDir)) {
        console.error("Critical Error: Core evolver not found at: " + evolverDir);
        process.exit(1);
    }
