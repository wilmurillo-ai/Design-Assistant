...
- **本文件（`chanjing-tts-voice-clone-SKILL.md`）**：业务逻辑、追问与异常语义；据此调度 [`scripts/cli_capabilities.py`](scripts/cli_capabilities.py) 与 [`scripts/`](scripts/) 下脚本。
...
**前置条件（权限验证）**：凭证由顶层入口统一校验；本 Skill 不重复执行凭证校验步骤。脚本使用同一套凭证。
...
|---|---|---|---|---|---|
| 1 | `create_voice.py` | `name`、公开可访问 `url` | `voice_id` | Step 2 | `10400`→`auth_required`；`40000`→`need_param`；`50000`→`upstream_error` |
| 2 | `poll_voice.py` | `voice_id` | 可用 `voice_id` | Step 3 | 超时/失败：`timeout` 或 `upstream_error` |
| 3 | `create_task.py` | `voice_id`、`text` | `task_id` | Step 4 | `10400`→`auth_required`；`40000`→`need_param`；`50000`→`upstream_error` |
| 4 | `poll_task.py` | `task_id` | 远端音频 URL | Step 5 | 超时/失败：`timeout` 或 `upstream_error` |
| 5 | 下载动作（仅显式下载） | 用户明确要求保存到本地 | 本地路径 | 结束 | 下载失败：返回远端 URL + 错误信息 |
...
本 Skill 提供脚本（`skills/chanjing-content-creation-skill/products/chanjing-tts-voice-clone/scripts/`），使用同一配置文件获取访问凭据。
...
VOICE_ID=$(python skills/chanjing-content-creation-skill/products/chanjing-tts-voice-clone/scripts/create_voice.py --name "我的声音" --url "https://example.com/ref.mp3")

# 2. 轮询直到声音就绪
python skills/chanjing-content-creation-skill/products/chanjing-tts-voice-clone/scripts/poll_voice.py --voice-id "$VOICE_ID"

# 3. 创建 TTS 任务
TASK_ID=$(python skills/chanjing-content-creation-skill/products/chanjing-tts-voice-clone/scripts/create_task.py --audio-man "$VOICE_ID" --text "Hello, I am your AI assistant.")

# 4. 轮询到完成，得到音频下载链接
python skills/chanjing-content-creation-skill/products/chanjing-tts-voice-clone/scripts/poll_task.py --task-id "$TASK_ID"
```
