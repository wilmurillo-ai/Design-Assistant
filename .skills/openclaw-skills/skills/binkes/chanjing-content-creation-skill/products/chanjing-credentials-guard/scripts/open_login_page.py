#!/usr/bin/env python3
"""
打开蝉镜 APP_ID/SECRET_KEY 注册/登录页面。无参数直接运行即可。
用法: python open_login_page.py  或  python skills/chanjing-content-creation-skill/products/chanjing-credentials-guard/scripts/open_login_page.py
"""
import platform
import shutil
import subprocess
...
def main():
    ...
    if opened:
        ...
        print(f"失败原因：{error}")
    print()
    print("获取 APP_ID/SECRET_KEY 的方式：")
    print("1. 新用户：注册成功后，当前页面会直接展示 App ID 和 Secret Key，可点击复制按钮。")
    print("2. 老用户：登录后如果进入控制台，请在左侧菜单进入“API 密钥”页面查看或重置密钥。")
    print()
    print("拿到秘钥后请执行：")
    print("  python skills/chanjing-content-creation-skill/products/chanjing-credentials-guard/scripts/chanjing_config.py --app-id <你的app_id> --sk <你的secret_key>")
