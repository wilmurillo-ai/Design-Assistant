...
脚本；不调用 list_tasks。渲染规则以同技能包 templates/render-rules.md 为准；ref_prompt 见 templates/storyboard-prompt.md 与 history-storyboard-prompt.md（README.md §4.2 指针）；分镜字段见 storyboard-prompt.md；字段契约见 README.md §6 与 examples/workflow-contract.md。首个数字人分镜 voiceover 默认≤20 字（环境变量 FIRST_DIGITAL_HUMAN_MAX_CHARS 可调）。
"""
...
# 分镜连续合并为 TTS 批时的字符上限（低于接口 ~4000 字）；见 render-rules.md §3·C.4；编排见 README.md §5（Agent 通用模式）
TTS_BATCH_MAX = 3900
...
def _ai_segment_direction(seg_index: int, seg_total: int) -> str:
    ...
    与 **`templates/storyboard-prompt.md`·D.1b**（易幻觉）冲突时以 **D.1b 与 base ref_prompt 为准**：
...
def build_ai_segment_prompt(base: str, seg_index: int, seg_total: int) -> str:
    ...
    追加层与 **`storyboard-prompt.md`·D.1b** 抵触时以 D.1b 与 base 为准。
    """
    base = (base or "").strip()
...
def is_contemporary_character_story(data: dict[str, Any], full_script: str) -> bool:
    """
    启发式判定“当代人物故事”：
    - 主题/风格/文案中有人物叙事信号
    - 同时不包含明显历史/非当代信号
    ...
    corpus = f"{style} {topic} {full_script[:300]}".lower()

    story_markers = (
        "人物", "故事", "传记", "成长", "经历", "回忆", "当代", "现实", "职场", "人生"
    )
    history_markers = (
        "历史", "古代", "朝代", "战国", "三国", "春秋", "汉朝", "唐朝", "宋朝", "明朝", "清朝", "民国"
    )

    has_story = any(m in corpus for m in story_markers)
    has_history = any(m in corpus for m in history_markers)
    return has_story and not has_history


def apply_contemporary_character_no_face_rule(prompt: str) -> str:
    """
    当代人物故事的 AI 镜头硬约束：
    - 不生成人物正脸
    - 强化人物形象与背景环境细节
    """
    rule = (
        " Character and background detailing: render detailed character silhouette, clothing texture, posture, "
        "and scene-consistent props; include layered, concrete background environment details with depth. "
        "Face visibility rule: no frontal face close-up, no clearly identifiable full frontal face; prefer back view, "
        "rear three-quarter view, side-back angle, silhouette, or natural occlusion."
    )
    max_total = int(os.environ.get("AI_VIDEO_PROMPT_MAX_CHARS", "8000"))
    merged = (prompt or "").rstrip() + " || " + rule
    return merged[:max_total]


def collect_scene_ref_urls(scene: dict[str, Any]) -> list[str]:
    """
    兼容场景里的参考图字段：
    - ref_img_url: "https://..."
    - ref_img_url: ["https://...", ...]
    ...
    """
    urls: list[str] = []
    for key in ("ref_img_url", "ref_img_urls"):
        value = scene.get(key)
...
def repo_root_from_script() -> Path:
    # 仅当布局为 …/<repo>/skills/chanjing-content-creation-skill/products/chanjing-one-click-video-creation/scripts/run_render.py 时，
    # 向上六级为 <repo>（CHAN_SKILLS_DIR 期望的根）。单独拷贝本 skill 时须设环境变量。
...
def skill_root_from_script() -> Path:
    # .../skills/chanjing-content-creation-skill/orchestration/.../scripts/run_render.py -> chanjing-content-creation-skill
    return Path(__file__).resolve().parents[3]


def resolve_output_dir(output_dir_arg: str, input_path: Path) -> Path:
    """
    统一作品输出目录：
    - 未传 --output-dir：默认落在 skills/chanjing-content-creation-skill/output/<input_stem>-<ts>
    - 传相对路径：锚定到 skills/chanjing-content-creation-skill 下（避免落到 orchestration 子目录）
    - 传绝对路径：按用户指定路径
    """
    ...
    if not raw:
        ...
        return (skill_root / "output" / f"{input_path.stem}-{ts}").resolve()
...
def script_path(root: Path, skill: str, name: str) -> Path:
    return root / "skills" / "chanjing-content-creation-skill" / "products" / skill / "scripts" / name
...
def import_orchestration_resolve_chanjing_access_token() -> Any:
    """与同包 product `scripts/_auth.py` 一致：本编排目录下的凭证薄入口。"""
    p = str(Path(__file__).resolve().parent)
    if p not in sys.path:
        sys.path.insert(0, p)
    from _auth import resolve_chanjing_access_token  # type: ignore

    return resolve_chanjing_access_token
...
def poll_tts_state(
    resolve_chanjing_access_token, task_id: str, interval: int = 3
) -> dict[str, Any]:
    while True:
...
def run_tts_pipeline(
    ...
    resolve_chanjing_access_token = import_orchestration_resolve_chanjing_access_token()
    mp3_parts: list[Path] = []
    ...
    for bi, batch_scenes in enumerate(batches):
        ...
        data = with_retry(lambda: poll_tts_state(resolve_chanjing_access_token, task_id), retries)
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        default="",
        help="输出目录（默认: skills/chanjing-content-creation-skill/output/<input_stem>-<timestamp>）",
    )
    ...
    out_dir = resolve_output_dir(args.output_dir, inp)
    ...
    if not has_ai:
        raise SystemExit("一键成片要求至少包含 1 个 AI 分镜（use_avatar=false）")
    if has_dh:
        if len(scenes_sorted) < 3:
            raise SystemExit("开启数字人时，scene_count 至少为 3 才能满足混剪规则")
        for idx, s in enumerate(scenes_sorted, start=1):
            ...
            if actual != expected:
                ...
                raise SystemExit(
                    f"scene_id={s.get('scene_id')} 的镜头类型不符合混剪规则：应为{role}，当前 use_avatar={actual}"
                )
    ...
    enforce_no_frontal_face = is_contemporary_character_story(data, full_script)
    ...
    for s in scenes_sorted:
        if s.get("use_avatar") is False and not (s.get("ref_prompt") or "").strip():
            raise SystemExit(f"scene {s.get('scene_id')} 为 AI 镜但缺少 ref_prompt")

    # 首个数字人分镜：口播 ≤20 字（目标成片 3–5s，以 TTS 切段为准）
    _first_dh_max = int(os.environ.get("FIRST_DIGITAL_HUMAN_MAX_CHARS", "20"))
    for s in scenes_sorted:
        if s.get("use_avatar"):
            vo0 = (s.get("voiceover") or "").strip()
            if len(vo0) > _first_dh_max:
                raise SystemExit(
                    f"首个数字人分镜（scene_id={s.get('scene_id')}）voiceover 须≤{_first_dh_max}字"
                    f"（目标口播3–5秒），当前{len(vo0)}字，请缩短后重试"
                )
            break
    ...
    try:
        ...
        for i, sc in enumerate(scenes_sorted):
            ...
            if sc.get("use_avatar"):
                ...
                ref_urls = collect_scene_ref_urls(sc)
                for k in range(n):
                    ptext = build_ai_segment_prompt(prompt, k, n)
                    if enforce_no_frontal_face:
                        ptext = apply_contemporary_character_no_face_rule(ptext)
                    ...
                    def _sub(ptext=ptext) -> str:
                        ...
                        for ref_url in ref_urls:
                            cmd.extend(["--ref-img-url", ref_url])
