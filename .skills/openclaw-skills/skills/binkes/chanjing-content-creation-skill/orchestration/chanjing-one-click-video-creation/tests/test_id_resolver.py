...
def _repo_root() -> Path:
    # 与 run_render.repo_root_from_script 一致：本 skill 根 → products → 包根 → skills → 仓库根
    return _skill_root().parent.parent.parent.parent
...
def _fetch_from_api(repo: Path) -> dict[str, Any]:
    ...
    lv = base / "chanjing-tts" / "scripts" / "list_voices.py"
    lf = base / "chanjing-video-compose" / "scripts" / "list_figures.py"
...
class TestIdResolver(unittest.TestCase):
    ...
    def test_resolve_from_api_does_not_write_cache_file(self):
        cred = Path(__file__).resolve().parents[3] / ".env"
        if not cred.is_file():
            self.skipTest("无蝉镜凭证文件，跳过自动拉取 ID")
        raw = cred.read_text(encoding="utf-8")
        has_user = "CHANJING_APP_ID=" in raw and "CHANJING_APP_ID=\n" not in raw
        has_secret = "CHANJING_SECRET_KEY=" in raw and "CHANJING_SECRET_KEY=\n" not in raw
        if not (has_user and has_secret):
            self.skipTest("凭证文件缺少有效 app_id/secret_key，跳过自动拉取 ID")
