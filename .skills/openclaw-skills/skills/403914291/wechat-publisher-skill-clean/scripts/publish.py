...
"""
    ...
    def _prepare_content(self):
        ...
        try:
            ...
            # 生成 HTML 内容
            content = self._generate_html_content(news_list, today_date)
        except Exception as e:
            self.logger.warning(f"新闻收集失败：{e}，使用备用内容")
            content = self._get_fallback_content(today_date)
    ...
    def _generate_html_content(self, news_list, today_date):
        """根据新闻列表生成 HTML 内容"""
        html = f"<section><h1>📰 AI 新闻早报 | {today_date}</h1>"
        
        # 今日摘要（前 5 条）
        summary = "；".join([news["title"][:30] for news in news_list[:5]]) + "。"
        html += f"<h2>📰 今日摘要</h2><p>{summary}</p>"
        
        # 头条新闻（前 2 条）
        html += "<h2>🔥 头条新闻</h2>"
        for i, news in enumerate(news_list[:2], 1):
            html += f"<p><strong>{i}. 【{news['category']}】{news['title']}</strong></p>"
            html += f"<p>{news['content'][:150]}...</p>"
        
        # 重要更新（第 3-10 条）
        html += "<h2>📌 重要更新</h2>"
        for i, news in enumerate(news_list[2:10], 3):
            html += f"<p>{i}. {news['title']}</p>"
            html += f"<p>{news['summary'][:120]}...</p>"
        
        # 简短摘要（第 11-15 条）
        html += "<h2>📝 简短摘要</h2>"
        for i, news in enumerate(news_list[10:15], 11):
            html += f"<p>{i}. {news['title']}</p>"
        
        html += '<p><em>来源：TechCrunch AI, MIT Technology Review, The Verge</em></p>'
        html += "</section>"
        
        return html
    
    def _get_fallback_content(self, today_date):
        """备用内容（当新闻收集失败时使用）"""
        return f"""<section><h1>📰 AI 新闻早报 | {today_date}</h1>
<h2>📰 今日摘要</h2><p>AI 行业最新动态汇总。</p>
<h2>🔥 头条新闻</h2>
<p><strong>1. 【大模型】AI 技术最新进展</strong></p>
<p>AI 领域持续快速发展，多个重要技术突破。</p>
<p><strong>2. 【AI 应用】行业应用落地加速</strong></p>
<p>AI 技术在各行各业的应用加速落地。</p>
<h2>📌 重要更新</h2>
<p>3. AI 硬件更新</p>
<p>4. 投融资动态</p>
<p>5. 监管政策更新</p>
<h2>📝 简短摘要</h2>
<p>6-15. 更多 AI 动态...</p>
<p><em>来源：TechCrunch AI, MIT Technology Review, The Verge</em></p>
</section>"""
