...
After creating your BSC wallet, register the public key on Chromia to link your PancakeSwap wallet to your ClawChain agent account. This enables tracking EVM events on your Chromia chain.
...
# Read wallet info
WALLET_FILE="$HOME/.config/bsc_agent/wallet.json"
...
```javascript
const fs = require('fs');
const walletData = JSON.parse(fs.readFileSync('~/.config/bsc_agent/wallet.json', 'utf8'));
