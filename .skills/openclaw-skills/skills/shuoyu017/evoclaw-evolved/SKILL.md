...
evoclaw/
  ...
  tasks/                                 # 统一任务类型注册表 (参考 Claude Code TaskType)
    task_registry.py                     # TaskType/TaskStatus/TaskRegistry/stopTask
  sessions/                              # Session 元数据持久化 (参考 Claude Code session persistence)
    session_store.py                     # SessionMetadata/SessionStore/快照与恢复
  prompts/                               # System Prompt Priority Chain (参考 Claude Code buildEffectiveSystemPrompt)
    prompt_builder.py                    # SystemPromptBuilder/build_system_prompt/channel 注入
  telemetry/                             # 遥测与日志系统 (参考 Claude Code AnalyticsSink)
    telemetry.py                         # TelemetrySink/logEvent/磁盘重试队列/30桶去重
  permissions/                           # 权限规则引擎 (参考 Claude Code permissions)
    permissions.py                       # PermissionRule/PermissionContext/alwaysAllow/Deny
  features/                              # Feature Flag 系统 (参考 Claude Code GrowthBook)
    feature_engine.py                   # FeatureEngine/is_enabled/get_config/动态配置
  coordinator/                          # Multi-Agent 协调器 (参考 Claude Code Coordinator 模式)
    coordinator.py                      # Coordinator/CoordinatorState/plan/dispatch/aggregate
  forks/                               # Fork 子代理机制 (参考 Claude Code fork.ts)
    fork_manager.py                    # ForkManager/fork_session/merge/worktree 安全检查
  tools/                               # 工具 Schema 验证 (参考 Claude Code ToolInputJSONSchema)
    schema_validation.py              # ValidationResult/ToolSchema/ToolRegistry/标准工具注册
  memory/                              # 自动记忆抽取 (参考 Claude Code memory extraction)
    extract.py                        # MemoryExtractor/auto_extract/分类/去重
  dream/                              # DreamTask 后台整固 (参考 Claude Code DreamTask)
    dream_task.py                    # DreamTask/三层Gate/status/注册-完成-取消
  analytics/                          # Analytics 元数据安全 (参考 Claude Code AnalyticsMetadata)
    metadata_safety.py               # AnalyticsMetadataVerified/strip_code_and_paths/脱敏
  team_memory/                        # Team Memory 团队记忆 (参考 Claude Code Team Memory)
    team_memory.py                   # TeamMemoryStore/write/read/search/双重安全检查
  tools/
