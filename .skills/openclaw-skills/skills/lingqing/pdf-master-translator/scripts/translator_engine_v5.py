...
if __name__ == "__main__":
    ...
    os.makedirs("translator_tmp_v5", exist_ok=True)
    all_html = []
    
    for p in range(args.start, args.end + 1):
        try:
            all_html.append(process_page(client, doc, p, "translator_tmp_v5"))
        except Exception as e:
            all_html.append(f"<div class='page-container'><h2>Page {p} Failed</h2><p>{e}</p></div>")

    build_pdf(all_html, f"NASA_Chapter2_V5_P{args.start}-{args.end}.pdf")
    shutil.rmtree("translator_tmp_v5", ignore_errors=True)
