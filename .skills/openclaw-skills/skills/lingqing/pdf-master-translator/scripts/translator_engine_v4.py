...
def agent_table_worker(client, img_path):
    uploaded = client.files.upload(file=img_path)
    # 1. 提取翻译
    res1 = client.models.generate_content(model='gemini-2.5-pro', contents=["将此表格翻译为中文，输出严格的 Markdown 表格格式，严禁改动公式/变量大小写。", uploaded])
    ...
    if not qa_data["is_accurate"]:
        raise QualityCheckFailed("Table accuracy check failed.")
    return md_table
...
def agent_2_translate(client, masked_img_path, p_num, ctx_prompt):
    ...
    2. 即使页面因抠图只剩下少量文字（如仅剩图注），也请结合【全息上下文】理解其准确语义并格式化输出，不要丢弃。
    3. 严格使用 XML 标签：
...
def process_page(client, doc, p_num, work_dir):
    ...
    with Image.open(main_img) as im:
        ...
        for idx, box in enumerate(layout.get("tables", [])):
            ...
            try:
                print("  -> Table Guard: Evaluating table reconstruction...")
                md_tbl = agent_table_worker(client, crop_fn)
                table_mds.append(md_tbl)
                draw.rectangle([l, t, r, b], fill="white") # 验证通过，涂白原图
                print("  -> Table Guard: Passed. Using Markdown.")
            except QualityCheckFailed:
                print("  -> Table Guard: Failed. Falling back to image crop.")
    ...
    ctx_prompt = f"【本页完整原文参考】(用于辅助理解被涂白后剩余的零碎文本)：\n{full_text_ctx}\n"
    if p_num > 1:
...
if __name__ == "__main__":
    ...
    os.makedirs("translator_tmp_v4", exist_ok=True)
    all_html = []
    
    for p in range(args.start, args.end + 1):
        try:
            all_html.append(process_page(client, doc, p, "translator_tmp_v4"))
        except Exception as e:
            all_html.append(f"<div class='page-container'><h2>Page {p} Failed</h2><p>{e}</p></div>")

    build_pdf(all_html, f"NASA_Chapter2_V4_P{args.start}-{args.end}.pdf")
    shutil.rmtree("translator_tmp_v4", ignore_errors=True)
    print("✅ V4 Processing Complete!")
