...
class AIAnalyzer:
    """AI 分析器 - 支持 Gemini 和 OpenAI (含 DeepSeek)"""
    ...
    def analyze(self, code: str, name: str, technical_data: Dict[str, Any], 
        ...
        if not self.api_key:
            ...
            return self._default_analysis(technical_data)
        
        try:
            if self.provider == 'gemini':
                return self._analyze_with_gemini(code, name, technical_data, fundamental_data)
            else:
                return self._analyze_with_openai(code, name, technical_data, fundamental_data)
        except Exception as e:
            logger.error(f"AI 分析失败: {e}")
            return self._default_analysis(technical_data)
    
    def _analyze_with_gemini(self, code: str, name: str, tech: Dict[str, Any], 
                            fundamental: Dict[str, Any] = None) -> Dict[str, Any]:
        """使用 Gemini API"""
        import requests
        import os
        
        prompt = self._build_prompt(code, name, tech, fundamental)
    ...
    def _analyze_with_openai(self, code: str, name: str, tech: Dict[str, Any],
                            fundamental: Dict[str, Any] = None) -> Dict[str, Any]:
        """使用 OpenAI API (含 DeepSeek)"""
        if not HAS_OPENAI or not self.client:
            return self._default_analysis(tech)
    ...
    def _build_prompt(self, code: str, name: str, tech: Dict[str, Any], 
                     fundamental: Dict[str, Any] = None) -> str:
        """构建 AI 提示词 (含基本面)"""
        
        # 技术面部分
        tech_part = f"""技术指标数据:
- 当前价格: {tech.get('current_price', 'N/A')}
- MA5: {tech.get('ma5', 'N/A')} (乖离率: {tech.get('bias_ma5', 0):+.2f}%)
- MA10: {tech.get('ma10', 'N/A')} (乖离率: {tech.get('bias_ma10', 0):+.2f}%)
...
- 风险因素: {', '.join(tech.get('risk_factors', []))}"""
        ...
        if fundamental and not fundamental.get('error'):
            ...
            cf = fundamental.get('capital_flow', {})
            
            if val or cf:
                fundamental_part = f"""
基本面数据:
- 市盈率 (PE): {val.get('pe', 'N/A')}
- 市净率 (PB): {val.get('pb', 'N/A')}
- 主力净流入: {cf.get('main_inflow', 'N/A')}
- 主力净流入占比: {cf.get('main_inflow_rate', 'N/A')}"""
        
        prompt = f"""你是一位专业的股票分析师，请根据以下信息给出投资建议。
...
只输出 JSON，不要其他内容。"""
    ...
    def _default_analysis(self, tech: Dict[str, Any] = None) -> Dict[str, Any]:
        """默认分析结果"""
        return {
            'sentiment_score': 50,
            'trend_prediction': '震荡',
