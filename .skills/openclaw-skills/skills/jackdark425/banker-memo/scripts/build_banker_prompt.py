#!/usr/bin/env python3
"""Parameterise the banker prompt for any CN listed company.

Usage: python3 build_banker_prompt.py <ts_code> <name_cn> <industry> <raw_data_dir> <output_dir>
Writes the full prompt to stdout."""
import sys, json, pathlib, glob

ts_code, name_cn, industry, raw_dir, out_dir = sys.argv[1:6]

# Discover raw-data files + uscc
files = sorted(glob.glob(f"{raw_dir}/*.json"))
file_list = "\n".join(f"- `{pathlib.Path(f).name}`" for f in files)
# Find PM uscc from filename
...
**不可引用此目录之外的数据**。统一社会信用代码：{uscc}（从 PrimeMatrix 文件名推断）。
...
### 2. Company Profile
沿革（成立 + 上市）、主业拆解（行业 `{industry}` 含义展开）、股权与资本结构表，数据均来自 basic_info + daily_basic + primematrix
