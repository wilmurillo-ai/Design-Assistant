...
Skill para gerenciar as finanças pessoais do Evandro, incluindo registro diário de gastos em linguagem natural, processamento de extratos bancários e faturas de cartão, e geração de resumos mensais e anuais.

## Como Funciona

**Importante:** Esta skill NÃO é um processo automático que roda em background. Ela é um **conjunto de instruções** que guia a agente (Donna) quando você faz perguntas ou comandos relacionados a finanças.

**Fluxo de execução:**
1. Você fala algo que ativa um trigger (ex: "gastei 50 no iFood")
2. A Donna detecta que é sobre finanças
3. A Donna carrega esta skill como guia
4. A Donna **segue as instruções**:
...
- ❌ Não monitora arquivos por conta própria
- ❌ Não envia alertas sem ser perguntada
- ❌ Não processa faturas automaticamente

**O que a skill FAZ quando ativada:**
- ✅ Lê os arquivos financeiros quando você pede
- ✅ Registra gastos quando você informa
- ✅ Calcula resumos quando você consulta
- ✅ Processa extratos quando você envia
...
### Contexto Pessoal do Evandro
- **Empresa própria / Autônomo** → receitas vêm direto dos clientes (ex: Avanto)
- **NÃO registrar "Retirada Empresa"** → os pagamentos dos clientes JÁ SÃO a receita
- Clientes podem ter % de repasse (ex: 35%) → registrar apenas a parte do Evandro
- **Namorada:** Ana Vitoria Nogueira Vian → PIX enviados = gastos reais, PIX recebidos = contribuições/reembolsos
- **Sogro:** Alexandre Placido Vianna → contribui com parte do aluguel
- **Aluguel na planilha:** registrar apenas o valor LÍQUIDO (aluguel total - contribuição Alexandre). NÃO registrar Alexandre como receita separada
- **Mãe:** Vanessa Cristina da Costa → PIX recebido = reembolso (comprou pra ela), NÃO é receita. PIX enviado = gasto real.
- **Irmã:** Evelin Thomazini Castilho → mesmo padrão da mãe
- **PIX de/para EVANDRO THOMAZINI** = transferência entre contas próprias da empresa. Só contabilizar o RECEBIDO como "Salário / Renda Principal"
- **Pagamentos de cartão de crédito no extrato** = NÃO registrar (já estão nas faturas)
- **Resgates CDB/RDB** = NÃO registrar (movimentação de investimentos)
- **Aluguel:** R$ 2.202,16/mês - Invest Imoveis (categoria Moradia)
- **IPVA:** R$ 276,71/mês (categoria Outras Fixas)

## Lições Aprendidas com Erros Reais

### Duplicatas (Fev/2026 — R$ 849,51 perdidos)
3 entradas manuais com descrições genéricas ("Mercado", "Barbearia", "Coisas pra casa") foram registradas SEM indicar o cartão. Depois, ao processar as faturas C6, as mesmas compras foram registradas com os nomes reais (Havan Jacareí, MP Milgrau, Sam's Club SJC), criando duplicatas.

**Sinais de alerta:** entrada sem cartão na coluna Obs, descrição genérica, mesmo valor + data próxima.

### PIX vs Cartão (Fev/2026 — 4 transações erradas)
4 transações do cartão Santander tinham nomes de pessoas na descrição e foram erroneamente registradas como "PIX Vânia Matos", "PIX Juliana Pimenta", etc.

**Regra:** Transações com nomes de pessoas em faturas são cobranças via link/maquininha, NUNCA PIX.
