...
**Auditing eval quality** — if every new artifact hits 100% within 2–3 sessions, your evals are probably too easy. Concrete checks: (a) run the pre-loop BASELINE artifact against the eval — if it already scores 70%+, the eval wasn't discriminating enough; a well-designed eval should baseline at 30–60% on a reasonably good artifact; (b) have someone else try to break the "converged" artifact with new prompts not in your eval — if they find failures easily, the eval missed real scenarios; (c) count how many prompts test the happy path vs. adversarial/edge cases — if >60% are happy path, rebalance; (d) use real-world failure cases as eval inputs — they're unbiased by definition.
...
**Guarding against eval designer bias**: When the same person designs the eval AND runs the loop, there's a subtle conflict of interest — you may unconsciously write prompts you know the artifact can handle, avoiding truly hard edges. Structural safeguards: (a) write eval prompts BEFORE looking at the current artifact version — test what the artifact *should* do, not what it *does*; (b) have a second person review or contribute prompts — fresh eyes find gaps you've normalized; (c) include "red team" prompts — deliberately adversarial inputs designed to break the artifact; (d) use real-world failure cases from actual usage as eval inputs — they're unbiased by definition. If your baselines are consistently above 50%, that's evidence of unconscious ease bias.
