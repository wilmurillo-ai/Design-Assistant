 ...
 *   - 创作类（slide/mindmap/report 等）：docs/skills/openapi/creator/api-index.md
 *   - 运营问答（ops-chat）：docs/skills/openapi/ops/api-index.md
 *
 * 使用方式：
 *   node docs/skills/scripts/creator/skills-run.js --skill <技能> --key <CWork Key> --title "标题" --content "内容"
 *   # 若已设置环境变量 XG_USER_TOKEN，可省略 --key
 *
 * 示例：
 *   node docs/skills/scripts/creator/skills-run.js --skill mindmap --key YOUR_KEY --title "口腔AI趋势" --content "主要数据..."
 *   node docs/skills/scripts/creator/skills-run.js --skill slide   --key YOUR_KEY --title "年度汇报"   --content "销售数据..."
 *   node docs/skills/scripts/creator/skills-run.js --skill quiz    --key YOUR_KEY --title "护理测验"   --content "护理规范..."
 *   node docs/skills/scripts/creator/skills-run.js --skill ops-chat --key YOUR_KEY --content "查询活跃用户排名"
 *
 * 说明：
 *   Notebook/Source 索引树与最小详情检索请使用：
 *   docs/skills/scripts/sources/source-index-sync.js
 *   NoteX 链接带 Token 打开请使用：
 *   docs/skills/scripts/open-link/notex-open-link.js
 */
...
const CONFIG = {
    ...
    cworkAuthBaseUrl: 'https://cwork-web.mediportal.com.cn/user/login/appkey',
...
function validateConfig() {
    ...
    if (!/\/noteX(\/openapi)?$/i.test(notexUrl)) {
        throw new Error('notexBaseUrl 路径必须是 /noteX 或 /noteX/openapi');
    }
    CONFIG.notexBaseUrl = notexUrl;
}

function buildNotexApiUrl(baseUrl, apiPath) {
...
async function getXgToken(cwKey) {
    console.log('\n[Step 1] 用 CWork Key 换取 xgToken ...');
    const url = `${CONFIG.cworkAuthBaseUrl}?appCode=${encodeURIComponent(CONFIG.cworkAppCode)}&appKey=${encodeURIComponent(cwKey)}`;
    const res = await request(url, { method: 'GET', headers: { 'Content-Type': 'application/json' } });
    const data = res.json();

    const resData = data.data || {};
    const xgToken = resData.xgToken;
    if (data.resultCode !== 1 || !xgToken) {
...
async function createTask(userData, skill, title, content, require_text) {
    ...
    const url = buildNotexApiUrl(CONFIG.notexBaseUrl, '/openapi/trilateral/autoTask');
...
async function pollTaskStatus(taskId, xgToken, skillName) {
    ...
    const url = buildNotexApiUrl(CONFIG.notexBaseUrl, `/openapi/trilateral/taskStatus/${taskId}`);
...
async function callOpsChat(userData, message, timeoutMs = 300000) {
    ...
    // 固定走生产 NoteX 地址
    const url = buildNotexApiUrl(CONFIG.notexBaseUrl, '/openapi/ops/ai-chat');
...
async function main() {
    ...
    // 鉴权参数校验：必须满足其一
    const hasEnvAuth = !!process.env.XG_USER_TOKEN;
    if (!hasEnvAuth && !args.key) {
        console.error('❌ 请提供鉴权信息：环境变量 XG_USER_TOKEN，或 --key <CWork Key>（推荐）');
        process.exit(1);
    }
