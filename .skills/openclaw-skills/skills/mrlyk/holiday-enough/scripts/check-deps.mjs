...
async function main() {
  ...
  if (!chromePort) {
    console.log('chrome: not connected — 正在打开 Chrome 远程调试设置页面...');
    try {
      if (os.platform() === 'darwin') {
        execSync('open "chrome://inspect/#remote-debugging"');
      } else if (os.platform() === 'win32') {
        execSync('start chrome://inspect/#remote-debugging');
      } else {
        execSync('xdg-open "chrome://inspect/#remote-debugging"');
      }
    } catch (_) {}
    console.log('请在打开的页面中勾选 "Allow remote debugging for this browser instance"，然后重新运行此检查。');
    process.exit(1);
  }
