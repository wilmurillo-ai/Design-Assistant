...
@click.option("--json", "output_json", is_flag=True, help="以 JSON 格式输出")
def history_orders_cmd(start, end, output_json):
    """查看历史订单
    ...
    try:
        ...
        ctx = TradeContext(get_config())
        resp = ctx.history_orders(start_at=start_dt, end_at=end_dt)
    except ValueError:
...
@click.option("--json", "output_json", is_flag=True, help="以 JSON 格式输出")
def buy_cmd(symbol, qty, price, output_json):
    """限价买入
...
@click.option("--price", required=True, type=float, help="限价价格")
@click.option("--json", "output_json", is_flag=True, help="以 JSON 格式输出")
def sell_cmd(symbol, qty, price, output_json):
    """限价卖出
...
@click.option("--json", "output_json", is_flag=True, help="以 JSON 格式输出")
def cancel_cmd(order_id, output_json):
    """撤销订单
