...
1.  **多条件后置表达式**（如 `continue:` `q:` `b:` 后的条件）：`&&` `||` 两侧**不加空格**，所有运算符（`=` `<=` `>=` `'=` 等）两侧**不加空格**，避免IRIS编译报错。

反例：

```java
continue:(hospId '= "") && (hospId '= ($p(^CTLOC(locId), "^", 22)))
continue:(qOnlyProInStock) && (proQty <= 0)
```
...
**注意**：只有后置表达式（跟在命令后的条件）需要这样处理，普通 `if` 语句中的条件运算符两侧仍需加空格。
