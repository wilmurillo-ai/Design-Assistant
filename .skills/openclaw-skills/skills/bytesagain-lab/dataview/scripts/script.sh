...
DATA_DIR="${HOME}/.local/share/dataview"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "dataview v2.0.0"; }

_help() {
    echo "Dataview v2.0.0 — data toolkit"
    echo ""
    echo "Usage: dataview <command> [args]"
    echo ""
    echo "Commands:"
    echo "  ingest             Ingest"
...
_search() {
    local term="${1:?Usage: dataview search <term>}"
...
case "${1:-help}" in
    ingest)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/ingest.log" 2>/dev/null || echo "  No entries yet. Use: dataview ingest <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/ingest.log"
            local total=$(wc -l < "$DATA_DIR/ingest.log")
            echo "  [Dataview] ingest: $input"
    ...
    transform)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/transform.log" 2>/dev/null || echo "  No entries yet. Use: dataview transform <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/transform.log"
            local total=$(wc -l < "$DATA_DIR/transform.log")
            echo "  [Dataview] transform: $input"
    ...
    query)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/query.log" 2>/dev/null || echo "  No entries yet. Use: dataview query <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/query.log"
            local total=$(wc -l < "$DATA_DIR/query.log")
            echo "  [Dataview] query: $input"
    ...
    filter)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/filter.log" 2>/dev/null || echo "  No entries yet. Use: dataview filter <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/filter.log"
            local total=$(wc -l < "$DATA_DIR/filter.log")
            echo "  [Dataview] filter: $input"
    ...
    aggregate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/aggregate.log" 2>/dev/null || echo "  No entries yet. Use: dataview aggregate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/aggregate.log"
            local total=$(wc -l < "$DATA_DIR/aggregate.log")
            echo "  [Dataview] aggregate: $input"
    ...
    visualize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/visualize.log" 2>/dev/null || echo "  No entries yet. Use: dataview visualize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/visualize.log"
            local total=$(wc -l < "$DATA_DIR/visualize.log")
            echo "  [Dataview] visualize: $input"
    ...
    export)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export.log" 2>/dev/null || echo "  No entries yet. Use: dataview export <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export.log"
            local total=$(wc -l < "$DATA_DIR/export.log")
            echo "  [Dataview] export: $input"
    ...
    sample)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/sample.log" 2>/dev/null || echo "  No entries yet. Use: dataview sample <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/sample.log"
            local total=$(wc -l < "$DATA_DIR/sample.log")
            echo "  [Dataview] sample: $input"
    ...
    schema)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/schema.log" 2>/dev/null || echo "  No entries yet. Use: dataview schema <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/schema.log"
            local total=$(wc -l < "$DATA_DIR/schema.log")
            echo "  [Dataview] schema: $input"
    ...
    validate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/validate.log" 2>/dev/null || echo "  No entries yet. Use: dataview validate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/validate.log"
            local total=$(wc -l < "$DATA_DIR/validate.log")
            echo "  [Dataview] validate: $input"
    ...
    pipeline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/pipeline.log" 2>/dev/null || echo "  No entries yet. Use: dataview pipeline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/pipeline.log"
            local total=$(wc -l < "$DATA_DIR/pipeline.log")
            echo "  [Dataview] pipeline: $input"
    ...
    profile)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/profile.log" 2>/dev/null || echo "  No entries yet. Use: dataview profile <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/profile.log"
            local total=$(wc -l < "$DATA_DIR/profile.log")
            echo "  [Dataview] profile: $input"
    ...
    *)
        echo "Unknown: $1 — run 'dataview help'"
        exit 1
        ;;
esac