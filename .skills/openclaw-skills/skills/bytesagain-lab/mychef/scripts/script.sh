#!/usr/bin/env bash
# mychef — Mychef reference tool. Use when working with mychef in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
mychef v$VERSION — Mychef Reference Tool

Usage: mychef <command>
...
## What is Mychef?
Mychef (mychef) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with mychef.

## Key Concepts
- Core mychef principles and fundamentals
- How mychef fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Mychef Matters
Understanding mychef is critical for:
...
## Getting Started
1. Understand the basic mychef concepts
...
## Installation
1. Download or clone the mychef package
...
cmd_patterns() {
    cat << 'EOF'
# Mychef — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for mychef
...
cmd_performance() {
    cat << 'EOF'
# Mychef — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Mychef — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Mychef — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "mychef v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: mychef help"; exit 1 ;;
esac
