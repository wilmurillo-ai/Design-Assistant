#!/usr/bin/env bash
# sleep-tracker - Health and wellness tracker
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${SLEEP_TRACKER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/sleep-tracker}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
sleep-tracker v$VERSION

Health and wellness tracker

Usage: sleep-tracker <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "sleep-tracker v$VERSION" ;;
