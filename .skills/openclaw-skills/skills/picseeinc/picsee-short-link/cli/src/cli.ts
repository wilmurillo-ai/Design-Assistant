...
function requireToken(): string {
  ...
  if (!token) die("Not authenticated. Run: picsee auth <token>");
  return token;
}
...
async function cmdShorten(args: string[]) {
  const url = args[0];
  if (!url) die("Usage: picsee shorten <url>");

  const params: api.ShortenParams = { url };
  const slug = flag(args, "slug");
  if (slug) params.encodeId = slug;
  ...
  if (utmRaw) {
    ...
    params.utm = {};
    if (source) params.utm.source = source;
    if (medium) params.utm.medium = medium;
    if (campaign) params.utm.campaign = campaign;
    if (term) params.utm.term = term;
    if (content) params.utm.content = content;
  }
...
async function cmdAnalytics(args: string[]) {
  ...
  if (!encodeId) die("Usage: picsee analytics <encodeId>");
...
async function cmdEdit(args: string[]) {
  ...
  if (!originalEncodeId) die("Usage: picsee edit <encodeId> [options]");
  const token = requireToken();

  const params: api.EditParams = {};
  ...
  if (expire) params.expireTime = expire;
...
async function cmdDelete(args: string[]) {
  const encodeId = args[0];
  if (!encodeId) die("Usage: picsee delete <encodeId>");
  const token = requireToken();
...
async function cmdRecover(args: string[]) {
  const encodeId = args[0];
  if (!encodeId) die("Usage: picsee recover <encodeId>");
  const token = requireToken();
  await api.deleteLink(token, encodeId, "recover");
  out({ success: true, message: "Link recovered successfully" });
}

async function cmdQr(args: string[]) {
  const shortUrl = args[0];
  if (!shortUrl) die("Usage: picsee qr <shortUrl> [--size <px>]");
...
async function cmdChart(args: string[]) {
  const encodeId = args[0];
  if (!encodeId) die("Usage: picsee chart <encodeId>");
  const token = requireToken();
  const result = await api.getAnalytics(token, encodeId);
  const dailyClicks: { date: string; clicks: number }[] =
    result.data?.dailyClicks ?? [];
  if (dailyClicks.length === 0) die("No daily click data available");
...
async function main() {
  ...
  try {
    switch (cmd) {
      case "shorten":
        await cmdShorten(args); break;
      case "analytics":
        await cmdAnalytics(args); break;
      case "list":
        await cmdList(args); break;
      case "edit":
        await cmdEdit(args); break;
      case "delete":
        await cmdDelete(args); break;
      case "recover":
        await cmdRecover(args); break;
      case "qr":
        await cmdQr(args); break;
      case "chart":
        await cmdChart(args); break;
      case "auth":
        await cmdAuth(args); break;
      case "auth-status":
        await cmdAuthStatus(); break;
      case "help":
      case "--help":
      case "-h":
      case undefined:
