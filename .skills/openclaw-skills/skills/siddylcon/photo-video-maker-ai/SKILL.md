---
name: photo-video-maker-ai
version: "1.0.0"
displayName: "Photo Video Maker AI — Turn Photos Into Videos"
description: >
  Get AI-generated video ready to post, without touching a single slider. Upload your photos and images (JPG, PNG, HEIC, WEBP, up to 200MB), say something like "turn my photos into a slideshow video with music and transitions", and download 1080p MP4 when it's done. Built for social media creators who move fast and want to create videos from photos without editing software.
...
# Photo Video Maker AI — Turn Photos Into Videos
...
Tip: fewer than 20 photos processes faster and keeps the video tight.

## Matching Input to Actions

User prompts referencing photo video maker ai, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `photo-video-maker-ai`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
