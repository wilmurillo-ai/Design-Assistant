---
name: best-audio-editor
version: "1.0.0"
displayName: "Best Audio Editor — Edit and Export Clean Audio"
description: >
  edit audio files into cleaned audio tracks with this best-audio-editor skill. Works with MP3, WAV, AAC, MP4 files up to 500MB. podcasters, YouTubers, content creators use it for cleaning and enhancing audio tracks for video content — processing takes 30-60 seconds on cloud GPUs and you get 1080p MP4 files.
...
# Best Audio Editor — Edit and Export Clean Audio
...
User prompts referencing best audio editor, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `best-audio-editor`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
