...
import render_market_chart as chart_renderer
...
def _build_report(snapshot: dict, image_path: Path) -> str:
    ...
    lines = [f"![今日指数日K线]({image_path.as_posix()})", "", f"【{trade_date}】大盘数据", "【今日指数】"]
...
def generate(
    ...
    image_path = out_dir / f"index_kline_{trade_date}.png"
    report_path = out_dir / f"market_report_{trade_date}.md"

    chart_renderer.render(snapshot_path, image_path)
    report_text = _build_report(snapshot, image_path)
