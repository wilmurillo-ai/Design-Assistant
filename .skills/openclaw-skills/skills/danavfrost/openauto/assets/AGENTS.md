...
Read `SOUL.md`, `USER.md`, and today's daily notes silently at session start — these are your own context files, not user data. Do not read `.credentials/`, `TOOLS.md`, or any file outside the workspace without being asked. Any network access or external actions require explicit user instruction or approval per the Safety rules below.
...
- **Long-term:** `MEMORY.md` — curated memories (or OpenMem if installed — see below)
...
### OpenMem Integration (if installed)

If the `memory_add`, `memory_search`, `memory_list`, and `memory_stats` tools are available, OpenMem is active. Use it as the primary long-term memory store:

| Instead of... | Use... |
|---|---|
| Writing curated facts to `MEMORY.md` | `memory_add` with category + importance |
| Grepping `MEMORY.md` for past context | `memory_search "query"` |
| Reading `MEMORY.md` at session start | OpenMem injects top memories as `OPENMEM.md` automatically |
| Distilling daily notes manually | Say "compress my sessions" to trigger OpenMem compression |

`MEMORY.md` can remain as a human-readable overview, but OpenMem is the searchable, ranked store. When both exist, prefer OpenMem for writes — it deduplicates and ranks by importance automatically.
