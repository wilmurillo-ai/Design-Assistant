...
name: openauto
version: 1.0.7
description: "Proactive, self-improving agent architecture with hardened security. WAL Protocol, Working Buffer, Compaction Recovery, Autonomous Crons, and battle-tested patterns. Fork of proactive-agent with security audit applied."
author: halthelobster
---

# OpenAuto

**A hardened fork of [proactive-agent](https://clawhub.ai/halthelobster/proactive-agent) v1.0.7.**

Security audit applied: removed unsafe principles, fixed broken config paths, scrubbed fabricated statistics, added Working Buffer privacy guardrails.

**A proactive, self-improving architecture for your AI agent.**

Most agents just wait. This one anticipates your needs — and gets better at it over time.

## Filesystem Access & Trust Model

This skill reads and writes the following local paths at startup and during operation. The agent may also make network calls (web search, calendar, email) when instructed to do so by the user — those are task-driven, not automatic.

**Trust note:** The safety rules in this skill (no external actions without approval, don't log secrets, etc.) are instructions to the agent — they are advisory, not technically enforced. Compliance depends on the underlying model following its instructions. Do not store credentials or highly sensitive data in workspace files you would not want the agent to read.

| Path | Access | Purpose |
|---|---|---|
| `<workspace>/SOUL.md` | read | Agent identity loaded at session start |
| `<workspace>/USER.md` | read/write | Human context, updated from conversation |
| `<workspace>/AGENTS.md` | read/write | Operating rules and learned lessons |
| `<workspace>/MEMORY.md` | read/write | Curated long-term memory |
| `<workspace>/SESSION-STATE.md` | read/write | Active working memory (WAL target) |
| `<workspace>/TOOLS.md` | read/write | Tool config and gotchas |
| `<workspace>/ONBOARDING.md` | read/write | First-run setup progress |
| `<workspace>/memory/YYYY-MM-DD.md` | read/write | Daily session logs |
| `<workspace>/memory/working-buffer.md` | read/write | Danger zone exchange log |
| `<workspace>/.credentials/` | read (on request only) | Credential storage location documented in TOOLS.md — never read automatically at startup |
| `~/.openclaw/openclaw.json` | read (on request only) | Gateway config — only read when explicitly running `security-audit.sh` |

**Automatic startup reads are limited to:** `SOUL.md`, `USER.md`, and today's daily notes. All other file access is on-demand or user-initiated.

**Note:** The OpenClaw skill metadata spec does not support declaring filesystem paths as requirements. The table above is the disclosure substitute. The `.credentials/` directory is user-managed — this skill does not create it or write to it.

## What's New in v1.0.7 (OpenAuto)
...
**Privacy note:** Do not copy sensitive content (API keys, passwords, tokens, private personal information) verbatim into the buffer. Summarize generically instead — e.g. "credentials discussed" rather than the actual value.
...
| Skill | Purpose |
|-------|---------|
| **OpenAuto** (this) | Act without being asked, survive context loss |
...
**Original:** [proactive-agent](https://clawhub.ai/halthelobster/proactive-agent) by Hal 9001 ([@halthelobster](https://x.com/halthelobster)) — battle-tested from thousands of conversations.

**This fork:** Security audit and hardening applied to v1.0.7.

**OpenAuto v1.0.0 — Security changes from proactive-agent v1.0.7:**
- Removed "Ask forgiveness, not permission" principle from SOUL.md — too broad a license for autonomous action
- Fixed `security-audit.sh` gateway config path (`~/.clawdbot/` → `~/.openclaw/`) — check was silently broken for all OpenClaw users
- Removed hardcoded `/tmp/clawdbot/*.log` path from HEARTBEAT.md log review
- Removed fabricated "~26% of community skills contain vulnerabilities" statistic
- Added Working Buffer privacy warning — do not log API keys, passwords, or tokens verbatim to disk
- Removed internal dev artifacts (`SKILL-v2.3-backup.md`, `SKILL-v3-draft.md`, `_meta.json`) from published package

**Upstream v1.0.7 Changelog:**
