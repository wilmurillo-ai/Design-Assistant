// 卖出下单
// 用法: bun run scripts/sell.ts --token <TOKEN_ID> --price <PRICE> --size <SIZE> [--type market|limit]
//
// --type 默认: limit
...
async function sellOrder(
    ...
    if (orderType === "market") {
        ...
        // 限价卖出
        console.log(`限价卖出: ${size} shares @ $${price} (总计 ~$${(price * size).toFixed(2)}) ...`);
...
if (!tokenId || !size) {
    console.error("用法: bun run scripts/sell.ts --token <TOKEN_ID> --price <PRICE> --size <SIZE> [--type market|limit]");
    console.error("\n参数说明:");
...
sellOrder(tokenId, price, size, orderType).catch(console.error);
