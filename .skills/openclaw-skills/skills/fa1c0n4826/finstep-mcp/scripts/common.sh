...
BASE_URL="https://fintool-mcp.finstep.cn/common"
...
case "$TYPE" in
    ...
    trade_date)
        ...
        END="${3:-$(date +%Y-%m-%d)}"
        MARKET="${4:-83}"
        call_mcp "get_trade_date" "$(jq -n \
            --arg start_date "$START" --arg end_date "$END" --argjson security_market "$MARKET" \
            '{"start_date":$start_date,"end_date":$end_date,"security_market":$security_market}')"
        ;;
    url)
        # 解析网页内容
        URL="$2"
