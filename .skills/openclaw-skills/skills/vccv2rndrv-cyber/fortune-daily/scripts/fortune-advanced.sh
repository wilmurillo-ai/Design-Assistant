...
show_help() {
    ...
    echo ""
    echo "示例:"
    echo "  fortune-advanced --type weekly --name 老郑 --zodiac 鼠"
    echo "  fortune-advanced --type all --name 小明 --zodiac 龙"
}
...
generate_weekly_fortune() {
    ...
    # 显示当前周（第几周）
    local week_num=$(date +%V 2>/dev/null || echo "当前")
...
${WHITE}📊 基本信息${NC}
   周期：${GREEN}$current_year年第${week_num}周${NC}
   姓名：${GREEN}${name:-未提供}${NC}
   生肖：${GREEN}${zodiac:-未提供}${NC}
