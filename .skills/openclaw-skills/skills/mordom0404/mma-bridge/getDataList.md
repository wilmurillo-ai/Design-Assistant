...
```bash
mma post --method getDataList --data '<json>'
```
...
```bash
# 使用默认端口查询流星数据
mma post --method getDataList --data '{"type": [0], "orderBy": "start_time", "sort": "DESC", "pageSize": 10, "pageIndex": 1}'

# 指定端口查询双子座流星雨的数据
mma post --method getDataList --port 9000 --data '{"meteorShower": ["4"], "orderBy": "start_time", "sort": "DESC", "pageSize": 20, "pageIndex": 1}'

# 查询收藏的流星数据
mma post --method getDataList --data '{"isCollect": 1, "type": [0], "orderBy": "start_time", "sort": "DESC", "pageSize": 10, "pageIndex": 1}'
```
...
```bash
# Windows CMD - 使用转义双引号
mma post --method getDataList --data "{\"pageIndex\":1,\"pageSize\":10,\"orderBy\":2,\"sort\":\"DESC\",\"date\":\"2025-12-31\"}" --port 9000
```
