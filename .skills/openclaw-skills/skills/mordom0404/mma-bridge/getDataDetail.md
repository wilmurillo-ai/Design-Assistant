...
```bash
mma post --method getDataDetail --data '<json>'
```
...
```bash
# 查询ID为12345的数据详情
mma post --method getDataDetail --data '{"id": "12345"}'

# 指定端口查询数据详情
mma post --method getDataDetail --port 9000 --data '{"id": "12345"}'
```
...
```bash
# Windows CMD - 使用转义双引号
mma post --method getDataDetail --data "{\"id\":\"12345\"}" --port 9000
```
