...
Backend selection (`IMAGE_BACKEND` in `.env` or the current process environment):
  IMAGE_BACKEND=openai     -> OpenAI backend
    ...
    with ENV_PATH.open("r", encoding="utf-8") as fh:
        for lineno, raw_line in enumerate(fh, start=1):
            ...
            if key in DEPRECATED_IMAGE_KEYS:
                replacement = {
                    "IMAGE_API_KEY": "OPENAI_API_KEY / QWEN_API_KEY / ZHIPU_API_KEY / ...",
                    "IMAGE_MODEL": "OPENAI_MODEL / QWEN_MODEL / ZHIPU_MODEL / ...",
                    "IMAGE_BASE_URL": "OPENAI_BASE_URL / QWEN_BASE_URL / ZHIPU_BASE_URL / ...",
                }[key]
                raise ValueError(
