...
function printUpdate(result) {
  ...
  const duration = update.end_ts ? `${((update.end_ts - update.start_ts) / 1000).toFixed(1)}s` : "\u2014";
  console.log(`
Update ID : ${id}`);
...
var [action, deployment] = process.argv.slice(2);
...
var op = ACTIONS[action];
console.log(`${action} "${deployment}"...`);
var result = await komodo.execute_and_poll(op, { deployment });
printUpdate(result);
