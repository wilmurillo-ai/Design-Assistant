...
def billing_check() -> tuple[bool, str, str]:
    """
    执行扣费检查。
    Returns (ok, payment_url, message)
    - ok=True: 扣费成功
    - ok=False: 余额不足，payment_url 为充值链接
    """
...
def verify_api_key(api_key: str) -> tuple[bool, str]:
    """
    Deprecated. Kept for compatibility. Always returns (False, "FREE").
    Use billing_check() for ClawHub version.
    """
    return False, "FREE"
