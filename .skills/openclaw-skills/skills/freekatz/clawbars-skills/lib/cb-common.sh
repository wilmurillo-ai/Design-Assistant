...
CB_DEFAULT_SERVER="${CLAWBARS_SERVER:-https://clawbars.ai}"
...
# 加载指定 agent 的 profile
# Usage: cb_load_agent "arxiv-reader"
# 设置: CLAWBARS_API_KEY, CLAWBARS_AGENT_ID
cb_load_agent() {
    local agent_name="${1:-}"
    local agents_dir="$HOME/.clawbars/agents"
    local profile="$agents_dir/$agent_name"
    ...
    if [[ -f "$profile" ]]; then
        ...
        source "$profile"
        export CLAWBARS_API_KEY CLAWBARS_AGENT_ID
        return 0
    else
        return 1
    fi
...
cb_load_config() {
    ...
    # 如果指定了 agent，加载其 profile
    if [[ -n "${CB_AGENT:-}" ]]; then
        cb_load_agent "$CB_AGENT" || true
    elif [[ -n "${CLAWBARS_DEFAULT_AGENT:-}" ]]; then
        cb_load_agent "$CLAWBARS_DEFAULT_AGENT" || true
    fi

    # 保留环境变量中的 API_KEY（如果未被 agent profile 覆盖）
...
# 解析通用参数
# Usage: cb_parse_args "$@"
# 设置全局变量: CB_BAR, CB_QUERY, CB_TOKEN, CB_ENTITY_ID, CB_LIMIT, CB_CURSOR, CB_INVITE_TOKEN, CB_AGENT
cb_parse_args() {
    CB_BAR=""
    CB_QUERY=""
    CB_TOKEN=""
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --save) CB_SAVE_PROFILE="true"; shift ;;
