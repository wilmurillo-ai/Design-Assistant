...
def fetch_github(limit=5, keyword=None):
    ...
    for article in soup.select('article.Box-row'):
        try:
            ...
            repo_name = h2['href'].strip('/')  # 添加 repo_name 用于缓存识别
            ...
            items.append({
                ...
                "url": link,
                "repo_name": repo_name,  # 添加 repo_name 字段
                "heat": f"{stars} stars",
                "time": "Today"
            })
        except: continue
...
def fetch_huggingface_papers(limit=5, keyword=None):
    ...
    # We use Playwright for RSS sources that require JavaScript rendering.
    # For sources requiring JS rendering, we use a dedicated Playwright script
...
AI_NEWSLETTER_SOURCES = [
    # Note: Bens Bites may require JS rendering for some content
...
def fetch_rss_with_playwright(url, source_name, limit=5):
    """Fallback fetcher using Playwright for RSS sources requiring JS rendering"""
    try:
...
# User requested sources (2026-03-16)
GLOBAL_SOURCES = [
    ("Social Europe", "https://www.socialeurope.eu/feed/"),
    ("China Digital Times", "http://feeds.feedburner.com/chinadigitaltimes/IyPt"),
    ("Jacobin", "https://jacobin.com/feed/"),
    ("The Next Recession", "http://thenextrecession.wordpress.com/feed/"),
    ("Making Sense of Cents", "http://www.makingsenseofcents.com/feed"),
    ("Clever Girl Finance", "https://www.clevergirlfinance.com/feed/"),
]
...
def fetch_global(limit=5, keyword=None):
    """User requested global sources"""
    all_items = []
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        futures = {executor.submit(fetch_rss_feed, url, name, 3): name for name, url in GLOBAL_SOURCES}
        for future in concurrent.futures.as_completed(futures):
            all_items.extend(future.result())
    return filter_items(all_items, keyword)[:limit]
...
def main():
    ...
    sources_map = {
        ...
        'essays': fetch_essays, 'global': fetch_global,
