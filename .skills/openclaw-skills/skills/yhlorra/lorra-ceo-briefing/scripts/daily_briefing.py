...
from fetch_news import (
    ...
    fetch_global, enrich_items_with_content
)
from github_cache import process_github_trending
...
PROFILES = {
    # 1. 高价值简报 (只选高质量源，无需关键词过滤)
    "insights": {
        "global_scan": {
            "sources": [
                (fetch_hackernews, 10, None),
                (fetch_producthunt, 5, None),
                (fetch_github, 10, None),
                (fetch_v2ex, 5, None)
            ],
            "enrich": True  # 抓取正文
        },
        "hn_ai": {
            "sources": [(fetch_hackernews, 15, "AI,LLM,Agent,model,startup")],
            "enrich": True
        },
        "github_trending": {
            "sources": [(fetch_github, 10, None)],
            "enrich": True
        }
    },

    # 2. 综合早报 (General Morning Routine) - 已添加新RSS源 + 左翼媒体源
    "general": {
        "global_scan": {
            "sources": [
                ...
                (fetch_producthunt, 5, None),
                (fetch_github, 5, None),
                (fetch_v2ex, 5, "AI,Python,Java,编程,代码,程序员,技术,软件"),
                (fetch_ai_newsletters, 3, "AI,LLM,Agent"),
                (fetch_essays, 2, None),
                (fetch_global, 8, None),  # 左翼/社会媒体源: Jacobin, Social Europe, China Digital Times 等
            ],
            "enrich": True
        },
        "hn_ai": {
...
def fetch_section(section_name, config):
    ...
    # 对 GitHub Trending 进行缓存处理和深度分析
    if "github" in section_name.lower():
        print(f"[{section_name}] Processing GitHub trending cache...", file=sys.stderr)
        new_projects = process_github_trending(results, do_deep_analysis=True)
        if not new_projects:
            print(f"[{section_name}] No new projects, skipping...", file=sys.stderr)
            return []
