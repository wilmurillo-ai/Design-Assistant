...
BOARD_WEB_BASE = "https://tekan.cn/board"
...
def cmd_create(args, parser):
    ...
    if not args.quiet:
        board_url = client.shorten_url(f"{BOARD_WEB_BASE}/{board_id}")
        print(f"Board created: {name}", file=sys.stderr)
        print(f"  url: {board_url}", file=sys.stderr)
    print(board_id)
...
def cmd_detail(args, parser):
    ...
    print(f"  url:      {client.shorten_url(f'{BOARD_WEB_BASE}/{args.board_id}')}")
    if members:
...
def _print_task_detail(result: dict, client=None) -> None:
    ...
    if res:
        for k, v in res.items():
            print(f"  {k}: {v}")
    if btid and bid:
        edit_url = f"{BOARD_WEB_BASE}/{bid}?boardResultId={btid}"
        if client:
            edit_url = client.shorten_url(edit_url)
...
def cmd_task_detail(args, parser):
    ...
    if args.json:
        ...
        _print_task_detail(result, client=client)
