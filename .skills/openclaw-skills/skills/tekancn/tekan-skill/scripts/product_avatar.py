...
from shared.client import TopviewClient, TopviewError, to_tekan_credit, convert_result_credits, shorten_url
...
def print_result(result: dict, args) -> None:
    ...
    if args.json:
        ...
        if result_url:
            print(f"  [查看结果]({result_url})")
            print(f"  {shorten_url(result_url)}")
