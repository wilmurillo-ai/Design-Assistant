...
def test_live_articles_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    urls = _get_e2e_urls()
    if not urls:
    ...
    monkeypatch.setattr(wtm, "OUTPUT_DIR", out_dir)

    for url in urls:
        asyncio.run(asyncio.wait_for(wtm.fetch_article(url), timeout=timeout))
