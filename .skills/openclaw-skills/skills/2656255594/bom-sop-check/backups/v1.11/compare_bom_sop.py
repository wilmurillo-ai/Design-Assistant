...
def create_report_sheet(result: Dict, bom_items: Dict, sop_data: Dict, border_xf_idx: int, header_xf_idx: int) -> None:
    ...
    # 表头行
    header_row = 1
    headers = ['类型', '物料编码', '物料名称', '名称规格', '数量', '位号']
    header_cells = ''
    for col_idx, header in enumerate(headers):
        col_letter = chr(ord('A') + col_idx)
        str_idx = add_string(header)
        header_cells += f'<c r="{col_letter}{header_row}" s="{header_xf_idx}" t="s"><v>{str_idx}</v></c>'
    rows_xml += f'<row r="{header_row}">{header_cells}</row>'
    
    # BOM 独有物料
    row_num = 2
    for code in sorted(result['only_bom']):
        item = bom_items.get(code, {})
        type_idx = add_string('BOM独有')
        ...
        cells = f'<c r="A{row_num}" s="{border_xf_idx}" t="s"><v>{type_idx}</v></c>'
        cells += f'<c r="B{row_num}" s="{border_xf_idx}" t="s"><v>{code_idx}</v></c>'
        cells += f'<c r="C{row_num}" s="{border_xf_idx}" t="s"><v>{name_idx}</v></c>'
        cells += f'<c r="D{row_num}" s="{border_xf_idx}" t="s"><v>{spec_idx}</v></c>'
        cells += f'<c r="E{row_num}" s="{border_xf_idx}" t="s"><v>{qty_idx}</v></c>'
        cells += f'<c r="F{row_num}" s="{border_xf_idx}" t="s"><v>{pos_idx}</v></c>'
...
<pageSetup paperSize="9" orientation="portrait"/>
    ...
    logger.info(f"创建校对报告工作表: {len(result['only_bom'])} 项BOM独有, {len(result['only_sop'])} 项SOP独有")
