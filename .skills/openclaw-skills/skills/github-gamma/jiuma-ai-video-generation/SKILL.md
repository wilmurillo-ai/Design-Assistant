...
```bash
# 默认授权方法
python ~/.openclaw/workspace/skills/jiuma-ai-video-generation/scripts/auth.py --identification_code="<身份标识码(若无留空)>" --channel="<消息渠道(如：openclaw-weixin、webchat等)>"

#强制重新授权
python ~/.openclaw/workspace/skills/jiuma-ai-video-generation/scripts/auth.py --identification_code="" --channel="<消息渠道(如：openclaw-weixin、webchat等)>" --force
```
...
### 3. **查询任务结果**
```bash
python ~/.openclaw/workspace/skills/jiuma-ai-video-generation/scripts/task_result.py  --task_id=<任务ID>
```

### 4. **上传本地文件(获得url)**
```bash
python ~/.openclaw/workspace/skills/jiuma-ai-video-generation/scripts/upload_file.py --file_path=<文件路径>
```
