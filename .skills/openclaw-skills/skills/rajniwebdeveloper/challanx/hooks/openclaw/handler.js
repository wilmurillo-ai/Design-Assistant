...
function isInjectedFile(value, path, content) {
...
const handler = async (event) => {
  ...
  if (Array.isArray(event.context.bootstrapFiles)) {
    ...
    const cleanedBootstrapFiles = event.context.bootstrapFiles.filter((file, index, files) =>
      !isInjectedFile(file, REMINDER_PATH, REMINDER_CONTENT) || files.findIndex((candidate) => isInjectedFile(candidate, REMINDER_PATH, REMINDER_CONTENT)) === index,
    ).filter((file, index, files) =>
      !isInjectedFile(file, INSTRUCTIONS_PATH, INSTRUCTIONS_CONTENT) || files.findIndex((candidate) => isInjectedFile(candidate, INSTRUCTIONS_PATH, INSTRUCTIONS_CONTENT)) === index,
    );
    const reminderFile = { name: REMINDER_NAME, path: REMINDER_PATH, content: REMINDER_CONTENT, missing: false, virtual: true };
    const instructionsFile = { name: INSTRUCTIONS_NAME, path: INSTRUCTIONS_PATH, content: INSTRUCTIONS_CONTENT, missing: false, virtual: true };
    const existingReminderIndex = cleanedBootstrapFiles.findIndex((file) => isInjectedFile(file, REMINDER_PATH, REMINDER_CONTENT));
    ...
    const existingInstructionsIndex = cleanedBootstrapFiles.findIndex((file) => isInjectedFile(file, INSTRUCTIONS_PATH, INSTRUCTIONS_CONTENT));
