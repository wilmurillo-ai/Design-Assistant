...
```bash
find . -path "*/skills/tutor-buddy-pro/SKILL.md" -exec dirname {} \;
```
...
```bash
SKILL_DIR=$(find . -path "*/skills/tutor-buddy-pro/config" -exec dirname {} \; | head -1)/config
...
```bash
SKILL_DIR=$(find . -path "*/skills/tutor-buddy-pro/SKILL.md" -exec dirname {} \;)
...
test -f "$(find . -path '*/skills/tutor-buddy-pro/SKILL.md')" && echo "✅ SKILL.md found" || echo "❌ SKILL.md missing"
