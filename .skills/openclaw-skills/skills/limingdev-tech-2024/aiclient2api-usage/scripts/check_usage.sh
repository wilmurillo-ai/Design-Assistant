...
PWD_FILE="$AICLIENT_DIR/configs/pwd"
...
if [ -f "$PWD_FILE" ]; then
    ...
    if [ -n "$TOKEN" ]; then
        # Use token to refresh usage
        REFRESH_RESPONSE=$(curl -s -X GET "http://127.0.0.1:16825/api/usage?refresh=true" \
            -H "Authorization: Bearer $TOKEN" 2>/dev/null)
        
        if echo "$REFRESH_RESPONSE" | grep -q "timestamp"; then
            echo "✅ Usage data refreshed successfully"
        else
            echo "⚠️  Refresh failed, using cached data"
        fi
    else
        echo "⚠️  Authentication failed, using cached data"
    fi
else
    echo "⚠️  Password file not found, using cached data"
fi
