#!/usr/bin/env python3
"""Generate and edit images via SkillBoss API Hub.

Supports two engines (both routed through SkillBoss API Hub /v1/pilot):
  - gemini: Standard image generation with editing support.
...
def _pilot(api_key: str, body: dict) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f"SkillBoss API error ({e.code}): {err[:500]}") from e
...
def generate_image(api_key: str, prompt: str) -> bytes:
    """Generate an image via SkillBoss API Hub (auto-routed)."""
    result = _pilot(api_key, {
        "type": "image",
...
def edit_image(api_key: str, prompt: str, image_path: str) -> bytes:
    """Edit an image via SkillBoss API Hub with inline image data."""
...
def generate_image_hq(api_key: str, prompt: str, aspect: str) -> bytes:
    """Generate a high-quality image via SkillBoss API Hub with aspect ratio support."""
    inputs = {"prompt": prompt}
...
<p class="footer">Generated via <a href="https://api.heybossai.com" style="color:#3b82f6">SkillBoss API Hub</a></p>
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="Generate and edit images via SkillBoss API Hub.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s                                    # 4 random images
...
""",
    ...
    ap.add_argument("--engine", choices=["gemini", "imagen"], default="gemini",
                    help="Engine: gemini (supports edit) or imagen (high-quality). Both routed via SkillBoss API Hub.")
    ap.add_argument("--model", default="",
                    help="Model hint (informational; SkillBoss auto-routes to the best model).")
    ...
    if not api_key:
        ...
        print('Set your SkillBoss API Hub key: export SKILLBOSS_API_KEY="your-key-here"', file=sys.stderr)
        return 2
    ...
    print(f"Engine: {args.engine} (routed via SkillBoss API Hub)")
    ...
    for idx, prompt in enumerate(prompts, start=1):
        ...
        try:
            if args.edit:
                img_bytes = edit_image(api_key, prompt, args.edit)
            elif args.engine == "imagen":
                img_bytes = generate_image_hq(api_key, prompt, args.aspect)
            else:
