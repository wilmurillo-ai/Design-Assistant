...
输出：本地视频文件路径，例如：
`/root/.openclaw/workspace/seedance_result_1770864513.mp4`
...
message(
  ...
  filePath="/root/.openclaw/workspace/seedance_result_1770864513.mp4",  # 本地视频路径
...
请求参数：
- `file_type`: video/mp4
- `file_name`: seedance_result_xxx.mp4
- `file_size`: [文件大小]
...
| 环节 | 所需权限 | 配置位置 |
|------|---------|---------|
| 生成视频 | SkillBoss API Hub SKILLBOSS_API_KEY | 环境变量 |
...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│ seedance生成视频 │ → API调用SkillBoss API Hub
└────────┬────────┘
         │
         ▼
┌─────────────────┐
