...
def choose_script(item):
    ...
    if item.get("consistent") or item.get("reference_image") or item.get("reference_images"):
        return SCRIPT_DIR / "reference_media.py"
    if mode == "video":
...
def item_to_command(item):
    ...
    if mode == "video" and item.get("image") is not None:
        raise SystemExit("video manifest items should use `input_reference` or `reference_image`, not `image`")

    cmd = [sys.executable, str(script), "--prompt", str(prompt)]

    if script.name in {"generate_consistent_media.py", "reference_media.py"}:
        cmd.extend(["--mode", str(mode)])
        video_ref = item.get("input_reference") if mode == "video" else None
        add_repeatable(cmd, "--reference-image", item.get("reference_images") or item.get("reference_image") or video_ref)
    ...
    if script.name == "generate_video.py" and item.get("input_reference") is not None:
        cmd.extend(["--input-reference", str(item["input_reference"])])

    simple_flags = [
        "provider", "model", "endpoint", "out_dir", "prefix", "size", "quality", "style", "background",
        "n", "seed", "nsfw", "duration", "seconds", "fps", "preset", "config", "status_endpoint_template",
