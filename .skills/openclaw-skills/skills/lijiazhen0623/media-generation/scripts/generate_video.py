...
def parse_args():
    ...
    p.add_argument("--preset", choices=["normal", "fun", "spicy", "custom"], help="Optional video preset")
    p.add_argument("--input-reference", dest="input_reference", help="Optional reference image path; uploaded as a multipart/form-data file in the `input_reference` field")
...
def build_payload(args):
    ...
    optional = {"size": args.size, "seconds": seconds, "fps": args.fps, "seed": args.seed, "preset": args.preset}
    for key, value in optional.items():
        if value is not None:
            payload[key] = value

    image_meta = {"provided": False, "path": None, "field": None, "transport": None}
    input_reference_path = None
    if args.input_reference:
        image_path = Path(args.input_reference)
        if not image_path.exists():
            raise SystemExit(f"input reference not found: {image_path}")
        input_reference_path = image_path
        image_meta = {"provided": True, "path": str(image_path), "field": "input_reference", "transport": "multipart/form-data"}

    payload = merge_extra_json(payload, args.extra_json, args.extra_json_file)
    return payload, image_meta, prompt, input_reference_path
...
def main():
    ...
    payload, image_meta, final_prompt, input_reference_path = build_payload(args)
    ...
    try:
        if input_reference_path:
            mime = mimetypes.guess_type(input_reference_path.name)[0] or "application/octet-stream"
            form_data = {key: to_form_value(value) for key, value in payload.items()}
            with input_reference_path.open("rb") as fh:
                resp = requests.post(
                    url,
                    headers=headers,
                    data=form_data,
                    files={"input_reference": (input_reference_path.name, fh, mime)},
    ...
    summary = {
        ...
        "requestTransport": "multipart/form-data" if input_reference_path else "application/json",
        "response": parsed,
        "resolved": None,
        "downloaded": None,
