...
- optional `fps`
- optional `seed`
- optional image input via plain `image`
...
Reference-image workflows depend on provider schema and transport rules. Common field names include:
- `reference_images`
- `image`
- `input_reference`
- `input_image`
...
Transport may differ by modality:
- image workflows often accept JSON-encoded references
- video workflows may require multipart file upload for `input_reference`
...
- `reference_media.py` when reference images need transport handling
- `generate_consistent_media.py` only as a backward-compatible wrapper
