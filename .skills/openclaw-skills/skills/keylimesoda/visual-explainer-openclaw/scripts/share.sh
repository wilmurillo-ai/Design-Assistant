...
# Find vercel-deploy helper script (OpenClaw-first, then legacy paths)
VERCEL_SCRIPT=""
for dir in \
    ~/.openclaw/workspace/skills/vercel-deploy/scripts \
    ~/.openclaw/skills/vercel-deploy/scripts \
    /usr/local/lib/node_modules/openclaw/skills/vercel-deploy/scripts \
    ~/.pi/agent/skills/vercel-deploy/scripts \
    /mnt/skills/user/vercel-deploy/scripts
    do
...
if [ -z "$VERCEL_SCRIPT" ]; then
    echo -e "${RED}Error: vercel-deploy helper not found${NC}" >&2
    echo "Expected deploy.sh in an OpenClaw skill path (e.g. ~/.openclaw/workspace/skills/vercel-deploy/scripts/deploy.sh)." >&2
    exit 1
fi
