...
export async function ocrImage(filePath) {
    ...
    return await client.call(() => client.getClient().optical_char_recognition.image.basicRecognize({
        data: {
 ...
 */
export async function speechToText(filePath, format = 'opus') {
    const { appId, appSecret } = getAuth();
    const client = createClient({ appId, appSecret });
    if (!fs.existsSync(filePath)) {
        return { ok: false, error: `文件不存在: ${filePath}` };
    ...
    return await client.call(() => client.getClient().speech_to_text.speech.fileRecognize({
        data: {
            speech: {
