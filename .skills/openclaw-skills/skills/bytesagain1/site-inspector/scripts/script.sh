#!/usr/bin/env bash
# site-inspector — Site Inspector reference tool. Use when working with site inspector in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
site-inspector v$VERSION — Site Inspector Reference Tool

Usage: site-inspector <command>
...
cmd_intro() {
    cat << 'EOF'
# Site Inspector — Overview

## What is Site Inspector?
Site Inspector (site-inspector) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with site inspector.

## Key Concepts
- Core site inspector principles and fundamentals
- How site inspector fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Site Inspector Matters
Understanding site inspector is critical for:
...
## Getting Started
1. Understand the basic site inspector concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Site Inspector — Quick Start Guide
...
## Installation
1. Download or clone the site inspector package
...
cmd_patterns() {
    cat << 'EOF'
# Site Inspector — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for site inspector
...
cmd_debugging() {
    cat << 'EOF'
# Site Inspector — Debugging Guide
...
cmd_performance() {
    cat << 'EOF'
# Site Inspector — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Site Inspector — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Site Inspector — Migration & Upgrade Guide
...
cmd_cheatsheet() {
    cat << 'EOF'
# Site Inspector — Quick Reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "site-inspector v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: site-inspector help"; exit 1 ;;
esac
