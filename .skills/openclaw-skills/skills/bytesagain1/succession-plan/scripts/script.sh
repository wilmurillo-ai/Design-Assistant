#!/usr/bin/env bash
# succession-plan - Productivity and task management tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${SUCCESSION_PLAN_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/succession-plan}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
succession-plan v$VERSION

Productivity and task management tool

Usage: succession-plan <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "succession-plan v$VERSION" ;;
