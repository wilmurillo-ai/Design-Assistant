#!/usr/bin/env bash
# spc — Spc reference tool. Use when working with spc in data contexts.
...
show_help() {
    cat << 'HELPEOF'
spc v$VERSION — Spc Reference Tool
...
## What is Spc?
Spc (spc) is a specialized tool/concept in the data domain.
It provides essential capabilities for professionals working with spc.

## Key Concepts
- Core spc principles and fundamentals
- How spc fits into the broader data ecosystem  
- Essential terminology every practitioner should know

## Why Spc Matters
Understanding spc is critical for:
...
## Getting Started
1. Understand the basic spc concepts
...
## Installation
1. Download or clone the spc package
...
cmd_patterns() {
    cat << 'EOF'
# Spc — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for spc
