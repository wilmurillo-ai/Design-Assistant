#!/usr/bin/env bash
# tagout -- Tagout safety compliance tracker
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${TAGOUT_DIR:-$HOME/.tagout}"
...
show_help() {
    cat << EOF
tagout v$VERSION -- Tagout safety compliance tracker

Usage: tagout <command> [args]
...
cmd_add() {
    local value="${1:?Usage: tagout add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: tagout search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: tagout remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "tagout v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: tagout help"; exit 1 ;;
esac
