#!/usr/bin/env bash
# payback — Payback reference tool. Use when working with payback in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
payback v$VERSION — Payback Reference Tool

Usage: payback <command>
...
## What is Payback?
Payback (payback) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with payback.

## Key Concepts
- Core payback principles and fundamentals
- How payback fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Payback Matters
Understanding payback is critical for:
...
## Getting Started
1. Understand the basic payback concepts
...
cmd_formulas() {
    cat << 'EOF'
# Payback — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Payback — Instruments & Tools Overview

## Primary Instruments
- Core tools used in payback operations
...
cmd_glossary() {
    cat << 'EOF'
# Payback — Key Terms & Definitions

## Core Terminology
- **Payback**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "payback v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: payback help"; exit 1 ;;
esac
