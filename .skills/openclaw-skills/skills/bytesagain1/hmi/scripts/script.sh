#!/usr/bin/env bash
# hmi -- Human-machine interface design tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${HMI_DIR:-$HOME/.hmi}"
...
show_help() {
    cat << EOF
hmi v$VERSION -- Human-machine interface design tool

Usage: hmi <command> [args]
...
cmd_add() {
    local value="${1:?Usage: hmi add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: hmi search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: hmi remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "hmi v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: hmi help"; exit 1 ;;
esac
