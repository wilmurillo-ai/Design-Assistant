#!/usr/bin/env bash
# notion-template - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${NOTION_TEMPLATE_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/notion-template}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
notion-template v$VERSION

Multi-purpose utility tool

Usage: notion-template <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "notion-template v$VERSION" ;;
