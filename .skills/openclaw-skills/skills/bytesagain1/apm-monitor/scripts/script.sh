#!/usr/bin/env bash
# apm-monitor - Data processing and analysis toolkit
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${APM_MONITOR_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/apm-monitor}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
apm-monitor v$VERSION

Data processing and analysis toolkit

Usage: apm-monitor <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "apm-monitor v$VERSION" ;;
