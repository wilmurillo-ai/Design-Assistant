#!/usr/bin/env bash
# csrf — Csrf reference tool. Use when working with csrf in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
csrf v$VERSION — Csrf Reference Tool
...
## What is Csrf?
Csrf (csrf) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with csrf.

## Key Concepts
- Core csrf principles and fundamentals
- How csrf fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Csrf Matters
Understanding csrf is critical for:
...
## Getting Started
1. Understand the basic csrf concepts
...
## Installation
1. Download or clone the csrf package
...
cmd_patterns() {
    cat << 'EOF'
# Csrf — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for csrf
...
case "$CMD" in
    ...
    version|--version|-v) echo "csrf v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: csrf help"; exit 1 ;;
esac
