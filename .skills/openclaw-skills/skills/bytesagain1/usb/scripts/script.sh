#!/usr/bin/env bash
# usb — Usb reference tool. Use when working with usb in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
usb v$VERSION — Usb Reference Tool
...
## What is Usb?
Usb (usb) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with usb.

## Key Concepts
- Core usb principles and fundamentals
- How usb fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Usb Matters
Understanding usb is critical for:
...
## Getting Started
1. Understand the basic usb concepts
...
## Installation
1. Download or clone the usb package
...
cmd_patterns() {
    cat << 'EOF'
# Usb — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for usb
...
case "$CMD" in
    ...
    version|--version|-v) echo "usb v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: usb help"; exit 1 ;;
esac
