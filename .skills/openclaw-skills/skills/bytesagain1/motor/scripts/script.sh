#!/usr/bin/env bash
# motor -- Motor specification and selection tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${MOTOR_DIR:-$HOME/.motor}"
...
show_help() {
    cat << EOF
motor v$VERSION -- Motor specification and selection tool

Usage: motor <command> [args]
...
cmd_add() {
    local value="${1:?Usage: motor add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: motor search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: motor remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "motor v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: motor help"; exit 1 ;;
esac
