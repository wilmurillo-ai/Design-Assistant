#!/usr/bin/env bash
# fontpreview — Fontpreview reference tool. Use when working with fontpreview in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
fontpreview v$VERSION — Fontpreview Reference Tool

Usage: fontpreview <command>
...
cmd_intro() {
    cat << 'EOF'
# Fontpreview — Overview

## What is Fontpreview?
Fontpreview (fontpreview) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with fontpreview.

## Key Concepts
- Core fontpreview principles and fundamentals
- How fontpreview fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Fontpreview Matters
Understanding fontpreview is critical for:
...
## Getting Started
1. Understand the basic fontpreview concepts
...
## Installation
1. Download or clone the fontpreview package
...
cmd_patterns() {
    cat << 'EOF'
# Fontpreview — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for fontpreview
...
cmd_performance() {
    cat << 'EOF'
# Fontpreview — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Fontpreview — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Fontpreview — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "fontpreview v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: fontpreview help"; exit 1 ;;
esac
