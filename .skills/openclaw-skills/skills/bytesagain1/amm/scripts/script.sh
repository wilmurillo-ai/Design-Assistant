#!/usr/bin/env bash
# amm -- Analyze amm operations. Use when you need to understand amm mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${AMM_DIR:-$HOME/.amm}"
...
show_help() {
    cat << EOF
amm v$VERSION -- Analyze amm operations. Use when you need to understand amm mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: amm <command> [args]
...
cmd_add() {
    local value="${1:?Usage: amm add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: amm search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: amm remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "amm v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: amm help"; exit 1 ;;
esac
