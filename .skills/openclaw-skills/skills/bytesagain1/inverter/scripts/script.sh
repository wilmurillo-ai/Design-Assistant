...
VERSION="1.0.0"
DATA_DIR="${INVERTER_DIR:-$HOME/.inverter}"
...
cmd_add() {
    local value="${1:?Usage: inverter add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: inverter search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: inverter remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
