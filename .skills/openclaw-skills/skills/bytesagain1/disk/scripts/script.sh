#!/usr/bin/env bash
# disk — Disk reference tool. Use when working with disk in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
disk v$VERSION — Disk Reference Tool
...
## What is Disk?
Disk (disk) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with disk.

## Key Concepts
- Core disk principles and fundamentals
- How disk fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Disk Matters
Understanding disk is critical for:
...
## Getting Started
1. Understand the basic disk concepts
...
## Installation
1. Download or clone the disk package
...
cmd_patterns() {
    cat << 'EOF'
# Disk — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for disk
