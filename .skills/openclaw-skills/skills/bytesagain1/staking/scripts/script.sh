#!/usr/bin/env bash
# staking -- Analyze staking operations. Use when you need to understand staking mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${STAKING_DIR:-$HOME/.staking}"
...
show_help() {
    cat << EOF
staking v$VERSION -- Analyze staking operations. Use when you need to understand staking mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: staking <command> [args]
...
cmd_add() {
    local value="${1:?Usage: staking add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: staking search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: staking remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "staking v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: staking help"; exit 1 ;;
esac
