#!/usr/bin/env bash
# gitignore — Gitignore reference tool. Use when working with gitignore in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
gitignore v$VERSION — Gitignore Reference Tool

Usage: gitignore <command>
...
## What is Gitignore?
Gitignore (gitignore) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with gitignore.

## Key Concepts
- Core gitignore principles and fundamentals
- How gitignore fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Gitignore Matters
Understanding gitignore is critical for:
...
## Getting Started
1. Understand the basic gitignore concepts
...
## Installation
1. Download or clone the gitignore package
...
cmd_patterns() {
    cat << 'EOF'
# Gitignore — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for gitignore
...
cmd_performance() {
    cat << 'EOF'
# Gitignore — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Gitignore — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Gitignore — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "gitignore v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: gitignore help"; exit 1 ;;
esac
