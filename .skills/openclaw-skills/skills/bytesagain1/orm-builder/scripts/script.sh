#!/usr/bin/env bash
# orm-builder — Orm Builder reference tool. Use when working with orm builder in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
orm-builder v$VERSION — Orm Builder Reference Tool

Usage: orm-builder <command>
...
cmd_intro() {
    cat << 'EOF'
# Orm Builder — Overview

## What is Orm Builder?
Orm Builder (orm-builder) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with orm builder.

## Key Concepts
- Core orm builder principles and fundamentals
- How orm builder fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Orm Builder Matters
Understanding orm builder is critical for:
...
## Getting Started
1. Understand the basic orm builder concepts
...
## Installation
1. Download or clone the orm builder package
...
cmd_patterns() {
    cat << 'EOF'
# Orm Builder — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for orm builder
...
cmd_performance() {
    cat << 'EOF'
# Orm Builder — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Orm Builder — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Orm Builder — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "orm-builder v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: orm-builder help"; exit 1 ;;
esac
