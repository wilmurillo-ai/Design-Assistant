#!/usr/bin/env bash
# attestation -- Analyze attestation operations. Use when you need to understand attestation mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ATTESTATION_DIR:-$HOME/.attestation}"
...
show_help() {
    cat << EOF
attestation v$VERSION -- Analyze attestation operations. Use when you need to understand attestation mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: attestation <command> [args]
...
cmd_add() {
    local value="${1:?Usage: attestation add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: attestation search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: attestation remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="attestation-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "attestation v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: attestation help"; exit 1 ;;
esac
