#!/usr/bin/env bash
# http-client — Http Client reference tool. Use when working with http client in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
http-client v$VERSION — Http Client Reference Tool

Usage: http-client <command>
...
## What is Http Client?
Http Client (http-client) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with http client.

## Key Concepts
- Core http client principles and fundamentals
- How http client fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Http Client Matters
Understanding http client is critical for:
...
## Getting Started
1. Understand the basic http client concepts
...
## Installation
1. Download or clone the http client package
...
cmd_patterns() {
    cat << 'EOF'
# Http Client — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for http client
...
cmd_security() {
    cat << 'EOF'
# Http Client — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Http Client — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "http-client v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: http-client help"; exit 1 ;;
esac
