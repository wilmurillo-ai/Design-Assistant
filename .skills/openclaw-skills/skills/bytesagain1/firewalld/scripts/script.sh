#!/usr/bin/env bash
# firewalld — Firewalld reference tool. Use when working with firewalld in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
firewalld v$VERSION — Firewalld Reference Tool

Usage: firewalld <command>
...
## What is Firewalld?
Firewalld (firewalld) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with firewalld.

## Key Concepts
- Core firewalld principles and fundamentals
- How firewalld fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Firewalld Matters
Understanding firewalld is critical for:
...
## Getting Started
1. Understand the basic firewalld concepts
...
## Installation
1. Download or clone the firewalld package
...
cmd_patterns() {
    cat << 'EOF'
# Firewalld — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for firewalld
...
cmd_performance() {
    cat << 'EOF'
# Firewalld — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Firewalld — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Firewalld — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "firewalld v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: firewalld help"; exit 1 ;;
esac
