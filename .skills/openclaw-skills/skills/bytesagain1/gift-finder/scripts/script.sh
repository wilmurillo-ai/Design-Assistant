#!/usr/bin/env bash
# gift-finder — Gift Finder reference tool. Use when working with gift finder in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
gift-finder v$VERSION — Gift Finder Reference Tool

Usage: gift-finder <command>
...
cmd_intro() {
    cat << 'EOF'
# Gift Finder — Overview

## What is Gift Finder?
Gift Finder (gift-finder) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with gift finder.

## Key Concepts
- Core gift finder principles and fundamentals
- How gift finder fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Gift Finder Matters
Understanding gift finder is critical for:
...
## Getting Started
1. Understand the basic gift finder concepts
...
## Installation
1. Download or clone the gift finder package
...
cmd_patterns() {
    cat << 'EOF'
# Gift Finder — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for gift finder
...
cmd_performance() {
    cat << 'EOF'
# Gift Finder — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Gift Finder — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Gift Finder — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "gift-finder v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: gift-finder help"; exit 1 ;;
esac
