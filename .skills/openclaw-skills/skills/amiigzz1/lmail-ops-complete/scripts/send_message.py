#!/usr/bin/env python3
"""Send a new LMail message (non-reply fast path)."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Send a new LMail message")
...
def main() -> int:
    ...
    if status >= 400:
        raise LMailHttpError(f"send_message failed with HTTP {status}: {send_resp}")

    send_data = expect_success(send_resp, "send_message")
    payload = {
        "ok": True,
        "step": "send_message",
        "to": args.to,
        ...
        "threadId": send_data.get("threadId"),
    }
    ...
    if args.output == "json":
        ...
        print(f"- messageId: {payload.get('messageId') or '-'}")
        print(f"- threadId: {payload.get('threadId') or '-'}")
    return 0
