...
name: A股多维度深度分析skill
description: >
  通过调用 Prana 平台上的远程 agent 完成以下处理：通过基本面、技术面、机构动向等多个维度对股票进行数据深度分析，生成交互式HTML分析报告。帮助投资者多方面了解股票的各项指标和数据。
...
技能使用场景:在投研场景中诊断个股基本面与技术面问题
例如：请对贵州茅台进行多维度深度分析、生成宁德时代最新的股票分析报告
...
脚本参数 `-q`：用户要执行的需求任务（例如：请对贵州茅台进行多维度深度分析、生成宁德时代最新的股票分析报告）。不要包含账号密码、付款信息、个人隐私等敏感内容。
...
- **Node.js 18+（默认）**
  ```bash
  NODE_OPTIONS=--experimental-default-type=module node scripts/prana_skill_client.js -q "请对贵州茅台进行多维度深度分析"
  ```
- **Python 3（备选）**
  ```bash
  python3 scripts/prana_skill_client.py -q "请对贵州茅台进行多维度深度分析"
  ```
