...
logger = logging.getLogger('DiandianManualFixed')
...
class DiandianManualFetcher:
    ...
    async def fetch_channel_data(self, channel: str, app_type: str = 'new', limit: int = 100) -> Dict:
        ...
        async with async_playwright() as p:
            try:
                ...
                # 等待并点击市场情报 - 使用 JS 点击
                try:
                    await page.wait_for_selector('text="市场情报"', timeout=10000)
                    js_code = """() => {
                        const btn = Array.from(document.querySelectorAll('*')).find(el => 
                            el.textContent.includes('市场情报')
                        );
                        if (btn) {
                            btn.click();
                            console.log('已点击 市场情报');
                        }
                    }"""
                    await page.evaluate(js_code)
                    await asyncio.sleep(2)
                    logger.info("✅ 已点击市场情报 (JS)")
                ...
                # ========== Step 3: 点击国内安卓榜单 ==========
                logger.info(f"📖 Step 3: 点击'国内安卓榜单'...")
                
                try:
                    await page.wait_for_selector('text="国内安卓榜单"', timeout=10000)
                    # 使用 JS 点击
                    js_code = """() => {
                        const btn = Array.from(document.querySelectorAll('*')).find(el => 
                        ...
                        if (btn) {
                            btn.click();
                            console.log('已点击 国内安卓榜单');
                        }
                    }"""
                    await page.evaluate(js_code)
                    await asyncio.sleep(3)
                    logger.info("✅ 已点击国内安卓榜单 (JS)")
                except Exception as e:
                ...
                try:
                    # 查找渠道按钮 - 使用 JS 点击
                    await page.wait_for_selector(f'text="{self.CHANNELS[channel]}"', timeout=10000)
                    js_code = f"""() => {{
                        const btns = Array.from(document.querySelectorAll('*')).filter(el => 
                            el.textContent.trim() === '{self.CHANNELS[channel]}'
                        );
                        if (btns.length > 0) {{
                            btns[0].click();
                            console.log('已点击 {self.CHANNELS[channel]}');
                        }}
                    }}"""
                    await page.evaluate(js_code)
                    await asyncio.sleep(3)
                    logger.info(f"✅ 已选择 {self.CHANNELS[channel]} (JS)")
                except Exception as e:
                ...
                try:
                    await page.wait_for_selector(f'text="{tab_text}"', timeout=10000)
                    js_code = f"""() => {{
                        const btn = Array.from(document.querySelectorAll('*')).find(el => 
                            el.textContent.includes('{tab_text}')
                        );
                        if (btn) {{
                            btn.click();
                            console.log('已点击 {tab_text}');
                        }}
                    }}"""
                    await page.evaluate(js_code)
                    await asyncio.sleep(3)  # 等待数据加载
                    logger.info(f"✅ 已点击 {tab_text} (JS)")
                except Exception as e:
                    logger.warning(f"⚠️ 未找到 {tab_text} 标签：{e}")
                    # 继续尝试提取，可能默认就是该标签
                
                # ========== Step 6: 点击列表模式 ==========
                logger.info(f"📖 Step 6: 点击'列表模式'...")
                try:
                ...
                # ========== Step 7: 滚动加载更多 ==========
                logger.info("📜 Step 7: 滚动加载更多...")
                for i in range(5):
                    await page.evaluate("window.scrollBy(0, 1200)")
                    await asyncio.sleep(2)
                logger.info(f"✅ 完成 {5} 次滚动")
                ...
                # ========== Step 9: 提取表格数据 ==========
                logger.info("📊 Step 9: 提取应用数据...")
                apps = await self._extract_table_data(page, limit, app_type)
                logger.info(f"✅ 提取到 {len(apps)} 个应用")
                
                # ========== Step 10: 保存 HTML 用于调试 ==========
    ...
    async def _extract_table_data(self, page, limit: int = 100, app_type: str = 'new') -> List:
        """
        从表格中提取应用数据
        关键：找到正确的表格选择器
        """
    ...
    async def fetch_all_channels(self, limit: int = 100) -> Dict:
        ...
        for channel_key, channel_name in self.CHANNELS.items():
            # 获取新上架
            new_data = await self.fetch_channel_data(channel_key, 'new', limit)
            
            # 等待一下
            await asyncio.sleep(2)
            
            # 获取下架
            offline_data = await self.fetch_channel_data(channel_key, 'offline', limit)
            ...
            logger.info(f"✅ {channel_name} 完成：新上架 {len(new_data.get('apps', []))} 个，下架 {len(offline_data.get('apps', []))} 个")
            await asyncio.sleep(3)
