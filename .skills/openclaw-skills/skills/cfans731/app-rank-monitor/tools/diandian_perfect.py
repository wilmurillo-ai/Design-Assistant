...
"""
点点数据安卓渠道榜单爬虫 - 完美版
使用正确的选择器：dd-data-table + dd-table-cell
"""
...
logger = logging.getLogger('DiandianPerfect')


class DiandianPerfectFetcher:
    """点点数据完美版爬虫"""
    ...
    async def fetch_channel_data(self, channel: str, app_type: str = 'new', limit: int = 100) -> Dict:
        ...
        async with async_playwright() as p:
            try:
                ...
                # Step 5: 截图
                screenshot_path = Path(__file__).parent.parent / f"debug_{channel}_{app_type}_perfect.png"
                await page.screenshot(path=str(screenshot_path), full_page=True)
                logger.info(f"📸 截图：{screenshot_path}")
                
                # Step 6: 保存 HTML
                html_path = Path(__file__).parent.parent / f"debug_{channel}_{app_type}_perfect.html"
                with open(html_path, 'w', encoding='utf-8') as f:
                    html = await page.content()
                    f.write(html)
                logger.info(f"📄 HTML: {html_path}")
                
                # Step 7: 提取应用数据（使用正确的选择器）
                logger.info("📊 提取应用数据...")
                apps = await self._extract_apps_perfect(page, limit, app_type)
    ...
    async def _extract_apps_perfect(self, page, limit: int = 100, app_type: str = 'new') -> List:
        """完美版数据提取 - 使用 dd-data-table"""
        try:
            apps_data = await page.evaluate('''() => {
                const apps = [];
                
                // 使用正确的选择器：dd-data-table
                const table = document.querySelector('.dd-data-table');
                if (!table) {
                    console.log('未找到 dd-data-table');
                    return apps;
                }
                ...
                for (let i = 1; i < rows.length && apps.length < 100; i++) {
                    const row = rows[i];
                    const cells = row.querySelectorAll('.dd-table-cell');
                    ...
                    if (cells.length >= 2) {
                        ...
                        const package_name = cells[5]?.textContent.trim() || '';
                        ...
                        if (name && 
                            ...
                            apps.push({
                                rank, name, developer, category, date, package: package_name
                            });
...
async def test_perfect():
    """测试完美版"""
    fetcher = DiandianPerfectFetcher()
