...
logger = logging.getLogger('DiandianFinalPerfect')


class DiandianFinalPerfectFetcher:
    """点点数据终极完美版爬虫"""
    ...
    async def fetch_channel_data(self, channel: str, app_type: str = 'new', limit: int = 100) -> Dict:
        ...
        async with async_playwright() as p:
            try:
                ...
                # Step 5: 截图
                screenshot_path = Path(__file__).parent.parent / f"debug_{channel}_{app_type}_final.png"
                await page.screenshot(path=str(screenshot_path), full_page=True)
                logger.info(f"📸 截图：{screenshot_path}")
                
                # Step 6: 提取应用数据（使用正确的选择器）
                logger.info("📊 提取应用数据...")
                apps = await self._extract_apps_final(page, limit)
    ...
    async def _extract_apps_final(self, page, limit: int = 100, app_type: str = 'new') -> List:
        """终极完美版数据提取 - 使用.app-name-text"""
        try:
            apps_data = await page.evaluate('''() => {
                const apps = [];
                
                // 使用正确的选择器：.app-name-text
                const appNames = document.querySelectorAll('.app-name-text');
                console.log(`找到 ${appNames.length} 个应用名称`);
                
                for (const nameEl of appNames) {
                    const name = nameEl.textContent.trim();
                    const row = nameEl.closest('tr, [class*="row"]');
...
async def test_final_perfect():
    """测试终极完美版"""
    fetcher = DiandianFinalPerfectFetcher()
...
if __name__ == "__main__":
    asyncio.run(test_final_perfect())
