...
logger = logging.getLogger('DiandianComplete')


class DiandianCompleteFetcher:
    """点点数据最终完成版爬虫"""
    ...
    async def fetch_channel_data(self, channel: str, app_type: str = 'new', limit: int = 100) -> Dict:
        ...
        async with async_playwright() as p:
            try:
                ...
                # Step 5: 滚动加载更多
                logger.info("📜 滚动加载更多应用...")
                for i in range(3):  # 滚动 3 次
                    await page.evaluate("window.scrollBy(0, 1000)")
                    await asyncio.sleep(2)
                logger.info(f"✅ 已滚动 {3} 次")
                
                # Step 6: 截图
                screenshot_path = Path(__file__).parent.parent / f"debug_{channel}_{app_type}_complete.png"
                await page.screenshot(path=str(screenshot_path), full_page=True)
                logger.info(f"📸 截图：{screenshot_path}")
                
                # Step 7: 提取应用数据
                logger.info("📊 提取应用数据...")
                apps = await self._extract_apps_complete(page, limit)
    ...
    async def _extract_apps_complete(self, page, limit: int = 100, app_type: str = 'new') -> List:
        """最终完成版数据提取"""
        try:
            apps_data = await page.evaluate('''() => {
                ...
                // 查找所有应用行
                const rows = document.querySelectorAll('[class*="dd-data-table-row"], [class*="table-row"]');
                ...
                for (const row of rows) {
                    ...
                    if (name.includes('监控') || name.includes('排名')) continue;
                    ...
                    if (!developer) {
                        ...
                        for (let i = 1; i < Math.min(4, lines.length); i++) {
                            ...
                            if (line && line !== name && !line.includes('热门地区') && !line.includes('分类')) {
                                developer = line;
                                break;
                            }
                        }
                    ...
                    console.log(`提取到：${name} - ${developer}`);
...
async def test_complete():
    """测试最终完成版"""
    fetcher = DiandianCompleteFetcher()
