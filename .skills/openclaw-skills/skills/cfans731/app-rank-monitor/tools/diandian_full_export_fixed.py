    ...
    try:
        ...
        await page.evaluate(f'''() => {{
            ...
            for (const div of allDivs) {{
                ...
                if (text && text.includes('{monitor_text}') && text.length < 200) {{
                    div.click();
                    return true;
                }}
            }}
        ...
        if exported:
            ...
            downloaded = await page.evaluate('''() => {
                ...
                if (dialog) {
                    ...
                    for (const btn of buttons) {
                        const text = btn.textContent.trim();
                        console.log(`弹窗按钮: ${text}`);
                        if (text.includes('下载') || text.includes('确定') || text.includes('导出') || text.includes('所有')) {
                            btn.click();
                            console.log('点击了下载按钮');
                            return true;
                        }
                ...
                for (const el of allElements) {
                    const text = el.textContent.trim();
                    if ((text.includes('所有') && text.includes('数据')) || 
                        (text.includes('全部') && text.includes('导出')) ||
                        text.includes('下载') || text.includes('导出')) {
                        if (el.tagName === 'BUTTON' || el.onclick || getComputedStyle(el).cursor === 'pointer') {
                            el.click();
                            return true;
                        }
                ...
                // 尝试点击任何包含下载的元素
                for (const el of allElements) {
                    if (el.textContent.trim().includes('下载')) {
                        if (el.tagName === 'BUTTON' || el.onclick) {
                            el.click();
                            return true;
                        }
            ...
            if downloaded:
                await asyncio.sleep(15)  # 等待下载完成（给更长时间）
...
async def merge_and_send_to_dingtalk(files: list, today_str: str):
    ...
    for src_file in files:
        # 保留原文件，复制到归档目录
        dest_file = export_dir / f"点点数据_{today_str}_{src_file.name}"
        shutil.copy2(src_file, dest_file)
        logger.info(f"📄 已归档: {dest_file.name}")
    
    # 检查是否已有汇总 Excel 文件
    summary_files = list(export_dir.glob(f"点点数据_上架下架榜_{today_str}*.xlsx"))
    if summary_files:
        final_file = summary_files[0]
    else:
        # 如果有多个文件，找最新的
        if files:
            final_file = export_dir / f"点点数据_上架下架榜_{today_str}.xlsx"
            shutil.copy2(files[0], final_file)
        else:
            final_file = None
    
    if final_file and final_file.exists():
        title = f"点点数据 {today_str} 全渠道上架/下架榜"
        message = f"今日（{today_str}）点点数据全渠道上架榜和下架榜已导出完成，请查看附件。"
