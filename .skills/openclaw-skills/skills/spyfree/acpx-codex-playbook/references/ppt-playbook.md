# PPT playbook for acpx + Codex

Use this pattern when the user wants Codex to generate a `.pptx` or similar structured artifact.

## Recommended flow

```bash
acpx codex sessions new --name ppt
acpx codex set-mode -s ppt full-access
acpx codex -s ppt -f prompt.txt
```
