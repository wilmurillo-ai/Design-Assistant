...
class MockPerformancePredictor {
    ...
    async predictPerformance(_task, _model) {
        return {
...
class MockLearningEngine {
    ...
    async recordDecision(_decision, _actualPerformance) {
        return Promise.resolve();
    }
    async getModelCorrections(_modelId) {
        return {
            latencyCorrection: 0,
            costCorrection: 0,
            qualityCorrection: 0,
...
export class TestDecisionEngine {
    ...
    async route(request) {
        ...
        const performance = await this.performancePredictor.predictPerformance(request.task, decision.selectedModel);
        return {
            decision,
            metrics: {
                decisionTime: 5,
...
// 工厂函数
export function createTestDecisionEngine(config) {
    return new TestDecisionEngine(config);
}
export default createTestDecisionEngine;
//# sourceMappingURL=decision-engine-mock.js.map