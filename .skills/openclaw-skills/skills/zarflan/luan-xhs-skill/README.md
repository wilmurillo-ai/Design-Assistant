...
- _meta.json now includes a config.useProxy boolean (default: false). When useProxy is false the skill will clear common proxy environment variables before making HTTP/browser requests to ensure calls do not go through a proxy. To change this behavior, edit _meta.json and set "config": { "useProxy": true }.

Usage note: the helper script scripts/disable_proxy.js is provided; core skill code should call require('./scripts/disable_proxy').apply(config.useProxy) at the start of any function that makes external requests.
