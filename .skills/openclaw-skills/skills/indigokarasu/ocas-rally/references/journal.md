...
Rally produces journals per spec-ocas-journal.md v1.3. Write a journal at the end of every run. Runs missing journals are invalid.

Journal path: `~/openclaw/journals/ocas-rally/YYYY-MM-DD/{run_id}.json`

Written atomically (write to `.tmp`, then rename). Never edit after writing.

## Journal type

Rally emits Observation Journals during research and scoring, and Action Journals during trade execution. Set journal_type to "observation" or "action" as appropriate.
