...
export function scoreForQuestionType(memories: Memory[], query: string): Memory[] {
  ...
  if (lowerQuery.includes('current') || lowerQuery.includes('now') || lowerQuery.includes('priority') || lowerQuery.includes('target') || lowerQuery.includes('revenue')) {
    ...
    return [...memories].sort((a, b) => {
      const timeA = a.timestamp ? new Date(a.timestamp).getTime() : new Date(a.created_at).getTime();
      const timeB = b.timestamp ? new Date(b.timestamp).getTime() : new Date(b.created_at).getTime();
