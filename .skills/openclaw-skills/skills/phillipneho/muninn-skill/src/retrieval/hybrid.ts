...
import { TemporalDecayScorer, applyTemporalDecayWithContradictions, getContradictionChain } from './temporal-decay.js';
...
export async function hybridSearch(
  ...
  if (options.enableTemporalDecay) {
    ...
    for (const mem of results) {
      baseScores.set(mem.id, (mem as any)._finalScore || (mem.salience || 0.5));
    }
    
    // Apply temporal decay with contradictions
    if (options.entityStore && options.relationshipStore) {
      results = applyTemporalDecayWithContradictions(
        results,
        baseScores,
        options.entityStore,
