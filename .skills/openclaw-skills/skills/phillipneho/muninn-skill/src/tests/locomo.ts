...
async function synthesizeAnswer(query: string, memories: Memory[]): Promise<string> {
  ...
  // Detect temporal queries - they need ALL history, not just recent
  const isTemporalQuery = /change|over time|history|evolve|previous|earlier|originally|started|reduced|increased|updated/i.test(query);
  const isContradictionQuery = /current|now|latest|was|changed|rebalanced|updated|previously/i.test(query);
  const isPriorityQuery = /priority|focus/i.test(query);
  
  // For temporal questions, we need ALL relevant memories sorted by time
  // First, extract the main entity from the query
  let mainEntity = '';
  ...
  // Filter to memories relevant to the main entity for temporal queries
  let relevantMemories = memories;
  if (mainEntity && (isTemporalQuery || isContradictionQuery)) {
    relevantMemories = memories.filter(m => 
      m.content.toLowerCase().includes(mainEntity.toLowerCase()) ||
      m.entities.some(e => e.toLowerCase().includes(mainEntity.toLowerCase()))
    );
  }
  
  if (isTemporalQuery) {
    // For temporal: sort OLDEST first (chronological order)
    ...
    const parts = sorted.map(m => {
      let text = m.content;
      if (m.sessionId) {
        const sessionNum = m.sessionId.replace('session_', '');
    ...
    // For contradiction: sort newest first but include ALL relevant
    const sorted = relevantMemories.sort((a, b) => {
      const ta = a.timestamp ? new Date(a.timestamp).getTime() : 0;
      const tb = b.timestamp ? new Date(b.timestamp).getTime() : 0;
    ...
    const parts = sorted.slice(0, 5).map(m => {
      let text = m.content;
      if (m.sessionId) {
        const sessionNum = m.sessionId.replace('session_', '');
        text += ` (session ${sessionNum})`;
...
async function runBenchmark(): Promise<void> {
  ...
  // Store key facts as semantic memories with temporal metadata
  const keyFacts = [
  ...
  for (const fact of keyFacts) {
    await store.remember(fact.content, 'semantic', {
      ...
      timestamp: (fact as any).timestamp || new Date().toISOString(),
      sessionId: (fact as any).sessionId
    });
  }
  ...
  for (const q of questions) {
    ...
    // Retrieve relevant memories - use higher limit for hard questions to capture full history
    const recallLimit = q.difficulty === 'hard' ? 15 : 8;
    const retrieved = await store.recall(q.query, { limit: recallLimit });
...
function scoreAnswer(q: Question, answer: string, retrieved: Memory[]): number {
  ...
  const isTemporalQuery = q.type === 'temporal';
  const isContradictionQuery = q.type === 'contradiction';
  
  // Check for key terms from expected answer
  let keyTerms = q.expectedAnswer
    .split(/[\s,]+/)
    .filter(t => t.length > 3)
  ...
  if (isTemporalQuery || isContradictionQuery) {
    const numericMatches = q.expectedAnswer.match(/\$[\d,]+/g) || [];
    for (const match of numericMatches) {
      const normalized = match.replace(/[^0-9]/g, '');
      if (normalized && !keyTerms.some(t => t.includes(normalized))) {
  ...
  const hasRelevantSession = retrieved.some(m => 
    q.requiresSessions.some(s => 
      (m.topics && m.topics.includes(s)) || 
      (m.sessionId === s)
    )
  );
  const sessionBonus = hasRelevantSession ? 0.2 : 0;
  
  // Additional bonus for temporal: check if we have multiple different values
  if (isTemporalQuery) {
    const uniqueValues = new Set();
    const valueMatches = answer.match(/\$[\d,]+/g) || [];
    for (const v of valueMatches) {
      uniqueValues.add(v);
    }
    // More values = more complete timeline = higher score
    const timelineBonus = Math.min(0.2, uniqueValues.size * 0.05);
