...
azure_login() {
    ...
    if az account show &> /dev/null; then
        ...
        if [ -n "$TENANT_ID" ] && [ "$TENANT_ID" != "null" ]; then
            echo -e "Current tenant: ${BLUE}$TENANT_ID${NC}"
        fi
    ...
    if [ -n "$TENANT_ID" ] && [ "$TENANT_ID" != "null" ]; then
        echo -e "Tenant: ${BLUE}$TENANT_ID${NC}"
    fi
}
...
authorize() {
    ...
    AUTH_TENANT="${TENANT_ID:-common}"
    AUTH_URL="https://login.microsoftonline.com/$AUTH_TENANT/oauth2/v2.0/authorize?client_id=$CLIENT_ID&response_type=code&redirect_uri=$REDIRECT_URI&scope=$(echo $SCOPES | sed 's/ /%20/g')&response_mode=query"
    ...
    TOKEN_RESPONSE=$(curl -s -X POST "https://login.microsoftonline.com/$AUTH_TENANT/oauth2/v2.0/token" \
