...
/**
 * OKKI 背调分析技能入口脚本
 * 用法：node run-okki.js <询盘单号> [开始时间] [结束时间]
 *
 * 功能：调用 OKKI 背调分析脚本（自包含模式）
...
function findMainScript() {
  const scriptName = 'okki-background.js';
  ...
  // 2. 同级目录：scripts/../okki-background.js
...
if (!targetInquiry) {
  console.error('用法: node run-okki.js <目标询盘号> [开始时间] [结束时间]');
  console.error('示例: node run-okki.js 50000126101155');
  console.error('      node run-okki.js 50000126101155 "2026-03-26T15:00:00" "2026-03-27T15:00:00"');
  process.exit(1);
}
...
if (!mainScript) {
  console.error('错误: 无法找到 okki-background.js');
  console.error('');
