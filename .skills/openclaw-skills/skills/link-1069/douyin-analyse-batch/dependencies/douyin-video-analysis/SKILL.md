...
description: 抖音视频分析流水线：输入抖音链接后，自动/半自动抓取页面信息与音频、转写口播、总结内容、提取有用信息与对应证据、进行批判性思维分析，把转写与分析结果合并写入一份本地 md 文档，并发送邮件到指定邮箱列表。 / Analyze a Douyin video from a shared URL by extracting page metadata, capturing audio when possible, transcribing speech into readable text, summarizing content, extracting useful information plus evidence, applying critical analysis, saving transcript and analysis into a single local markdown file, and emailing the result to a preset recipient list.
---
...
2. single combined markdown file (transcript + analysis) saved locally
3. email sent to preset recipient list with the markdown file attached
4. final chat summary with file path and email delivery status
...
### 6. Save into a single local markdown file

Default output directory: `~/Documents/douyin_analysis/`
(configurable via env var `DOUYIN_OUTPUT_DIR`)

File naming: `YYYY-MM-DD 抖音视频分析 - <topic>.md`

The file combines both sections:
- `## 口播转写` — raw machine transcript with quality warning
- `## 分析笔记` — full structured analysis (from workflow steps 4–5)

Include in the file header:
- source URL
- canonical URL
- account name if available
- transcript quality status (metadata-only / raw / cleaned)

### 7. Send by email

After the file is written, send it to the preset recipient list.

Default recipients: `3249331357@qq.com`  
(extend via env var `DOUYIN_EMAIL_RECIPIENTS`, comma-separated)

SMTP credentials must be set via environment variables (never hardcoded):
- `SMTP_USER` — sender email address
- `SMTP_PASS` — authorization code / app password
- `SMTP_HOST` — default `smtp.qq.com`
- `SMTP_PORT` — default `587`

Email format:
- Subject: `【抖音视频分析】<topic> <YYYY-MM-DD>`
- Body: first 500 chars of the analysis note as plain text preview
- Attachment: the full markdown file
...
#### B. Transcribe and create combined note
...
- combine transcript + analysis skeleton into a single markdown file
- save to `~/Documents/douyin_analysis/` (or `DOUYIN_OUTPUT_DIR`)
- return the note path as JSON

#### C. Send results by email

```bash
python3 /Users/bobzhong/.openclaw/workspace/skills/douyin-video-analysis/helpers/send_email.py \
  --note-path '<path-to-md-file>' \
  --topic '<short-topic>'
```

It will:
- read SMTP credentials from env vars (`SMTP_USER`, `SMTP_PASS`, `SMTP_HOST`, `SMTP_PORT`)
- send the markdown file as attachment to `DOUYIN_EMAIL_RECIPIENTS` (default: `3249331357@qq.com`)
- print delivery status as JSON
...
- where the combined markdown file was saved
- whether the email was sent successfully (and to which recipients)
