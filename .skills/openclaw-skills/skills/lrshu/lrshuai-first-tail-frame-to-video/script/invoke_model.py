...
def main():
    ...
    if args.image:
        ...
        if args.model in ["veo_3_1", "veo_3_1-fast", "veo_3_1-pro"]:
            if args.image_tail:
                # If image_tail is provided, Veo supports 'frames' mode with both first and last frame
                payload["input"]["generation_mode"] = "frames"
                payload["input"]["firstFrame"] = image_val
            else:
                # If only single image, Veo must use 'components' mode, and firstFrame must NOT be set
    ...
    if args.image_tail:
        ...
        if args.model in ["veo_3_1", "veo_3_1-fast", "veo_3_1-pro"]:
            # In frames mode, Veo needs lastFrame, but NOT images
