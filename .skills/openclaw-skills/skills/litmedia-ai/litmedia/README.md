...
1. **Log in** — run `python scripts/auth.py login` (one-time setup)
2. **Create** — tell the AI what you want in plain language. It picks the right tools, models, and parameters automatically.
...
| [Character Replace](references/video_mimic.md) | `scripts/video_mimic.py` | Character Replace in Videos with Scene Consistency using LitMedia Common Task APIs. |
| [User](references/user.md) | `scripts/user.py` | Credit balance and usage history |

## Configuration

Requires Python 3.8+ and a Litmedia AI account. Authenticate via the device flow:
...
This sets `LITMEDIA_UID` and `LITMEDIA_API_KEY` automatically. You can also set them manually:

```bash
export LITMEDIA_UID="your-uid"
