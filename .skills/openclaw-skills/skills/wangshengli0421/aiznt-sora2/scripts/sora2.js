...
async function main() {
  ...
  if (!cmd || cmd === 'help' || cmd === '-h') {
    console.log(`用法:
  submit --body '<json>'   # prompt、model 等
  fetch --task-id <id>`);
    process.exit(0);
  }
  ...
  if (cmd === 'submit') {
    const url = urls.v2_videos_generations;
    if (!url) throw new Error('AIZNT_PROXY_URLS 缺少 v2_videos_generations');
  ...
  if (cmd === 'fetch') {
    ...
    const tpl = urls.v2_videos_generations_fetch;
    if (!tpl) throw new Error('AIZNT_PROXY_URLS 缺少 v2_videos_generations_fetch');
