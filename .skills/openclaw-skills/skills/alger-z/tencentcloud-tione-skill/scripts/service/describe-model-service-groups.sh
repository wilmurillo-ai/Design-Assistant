...
选项:
  ...
  --limit <n>               返回数量 (默认: 20)
  --offset <n>              偏移量 (默认: 0)
  --order <asc|desc>        排序方向: ASC / DESC
  --order-field <field>     排序字段: CreateTime / UpdateTime
  --search-word <word>      按服务组名称搜索 (内部转换为 Filters ServiceGroupName)
...
示例:
  ...
  $(basename "$0") --region ap-shanghai --search-word "my-service"
  # 按状态过滤（多值用分号分隔）
  $(basename "$0") --region ap-shanghai --filters "Name=Status,Values=Normal;Abnormal;Waiting;Pending"

支持的 Status 值:
  Waiting | Pending | Normal | Abnormal | Stopping
EOF
    exit 0
}
...
main() {
    ...
    [[ -n "$search_word" ]] && filters+=("Name=ServiceGroupName,Values=${search_word}")
    ...
    log_info "查询服务组列表 (region=${region})"
    local result
    result=$(call_tione_api "DescribeModelServiceGroups" "$region" "${extra_args[@]}")
    format_json "$result"
}
