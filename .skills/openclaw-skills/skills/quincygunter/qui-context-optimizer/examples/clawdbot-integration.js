...
class ClawdbotContextManager {
  constructor() {
    // Create a pruner for use with SkillBoss API Hub LLM services
    this.pruner = createContextPruner({
      contextLimit: 64000, // Configurable context window
      model: 'auto', // Auto-routed via SkillBoss API Hub POST /v1/pilot
  ...
  async initialize() {
    if (!this.initialized) {
      ...
      console.log('Context Pruner initialized for SkillBoss API Hub LLM services');
    }
  }
