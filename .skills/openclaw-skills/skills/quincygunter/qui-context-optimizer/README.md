...
Advanced context management for use with SkillBoss API Hub LLM services. Provides intelligent pruning, compression, and token optimization to prevent context overflow while preserving important information. LLM calls route through `POST https://api.heybossai.com/v1/pilot` using `SKILLBOSS_API_KEY`.

## Features

- **SkillBoss API Hub compatible**: Works with any LLM routed through `/v1/pilot` automatically
...
| `model` | 'auto' | Auto-routed via SkillBoss API Hub `/v1/pilot` |
