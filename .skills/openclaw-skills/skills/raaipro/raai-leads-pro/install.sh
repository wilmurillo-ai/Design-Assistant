#!/usr/bin/env bash
# AI-Лиды PRO — автоустановщик
# Копирует коробку в ~/.openclaw/skills/ и создаёт дефолтные файлы.
#
# Usage:
# bash install.sh [--claude-home ~/.openclaw/skills/leads-pro]
#
# Поддержка: Linux / macOS / WSL. Windows — см. README Quick Start вариант ручной установки. set -euo pipefail SKILL_NAME="leads-pro"
DEFAULT_TARGET="${HOME}/.openclaw/skills/${SKILL_NAME}"
TARGET="${1:-$DEFAULT_TARGET}"
SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)" echo ""
echo "╔══════════════════════════════════════════════════╗"
echo "║ AI-Лиды PRO — установка ║"
echo "╚══════════════════════════════════════════════════╝"
echo ""
...
echo ""
echo "══════ СЛЕДУЮЩИЕ ШАГИ (5-10 минут) ══════"
echo ""
...
echo " • company.industry — ниша (B2B-услуги / e-commerce / стройка...)"
echo " • company.avg_deal_size — средний чек (руб)"
echo " • company.monthly_lead_goal — цель по лидам/мес"
echo " • sales.team_size — кол-во менеджеров"
echo ""
echo "3. Smoke-тест:"
echo " bash $TARGET/test/smoke-test.sh"
echo ""
echo "4. Первый запуск: напишите боту/агенту любой триггер"
echo " (аватар / где брать лидов / квалифицируй / отчёт лиды)"
echo ""
