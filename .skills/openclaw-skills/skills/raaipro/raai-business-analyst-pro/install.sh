#!/usr/bin/env bash
# AI-Аналитик бизнеса PRO — автоустановщик
# Копирует коробку в ~/.openclaw/skills/ и создаёт дефолтные файлы.
#
# Usage:
# bash install.sh [--claude-home ~/.openclaw/skills/business-analyst-pro]
#
# Поддержка: Linux / macOS / WSL. Windows — см. README Quick Start вариант ручной установки. set -euo pipefail SKILL_NAME="business-analyst-pro"
DEFAULT_TARGET="${HOME}/.openclaw/skills/${SKILL_NAME}"
TARGET="${1:-$DEFAULT_TARGET}"
SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)" echo ""
echo "╔══════════════════════════════════════════════════╗"
echo "║ AI-Аналитик бизнеса PRO — установка ║"
echo "╚══════════════════════════════════════════════════╝"
echo ""
...
echo " • leader.role — CEO / Собственник / CFO / Коммерческий директор"
echo " • company.name — название компании"
echo " • company.business_model — услуги / SaaS / продукт / marketplace"
echo " • finance.current_monthly_revenue — текущая выручка руб/мес"
echo ""
echo "3. Smoke-тест:"
echo " bash $TARGET/test/smoke-test.sh"
echo ""
echo "4. Первый запуск: напишите боту/агенту любой триггер"
echo " (дашборд / покажи KPI / unit экономика / финмодель / investor report)"
echo ""
