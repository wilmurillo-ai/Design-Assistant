#!/usr/bin/env bash
# build.sh — собирает AI-Аналитик бизнеса PRO в отгрузочный ZIP из исходника.
...
#
# Выход: ../zips-v<VER>/business-analyst-pro-v<VER>.zip set -euo pipefail SRC_DIR="$(cd "$(dirname "$0")" && pwd)"
SLUG="business-analyst-pro" # Версия из SKILL.md или переопределённая аргументом
if [[ -n "${1:-}" ]]; then VER="$1"
...
FILES=( SKILL.md README.md config.yaml .env.example install.sh test/smoke-test.sh examples/quick-start.md examples/full-library.md docs/onboarding.md docs/anti-fail.md docs/roi.md proof/case-01-chaos-to-dashboard.md proof/case-02-unit-economics-visible.md proof/case-03-funnel-losses-found.md proof/case-04-break-even-clarity.md proof/case-05-investor-ready-clarity.md
...
done # Marketing pack и competitors — если есть, включаются целиком
