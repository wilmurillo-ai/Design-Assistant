#!/usr/bin/env bash
# build.sh — собирает AI-Прораб PRO в отгрузочный ZIP из исходника.
# ЭТО ЕДИНСТВЕННЫЙ СПОСОБ СОБРАТЬ ZIP. Ручное редактирование ZIP запрещено.
#
# Использование:
# ./build.sh # собрать на основе version из SKILL.md
# ./build.sh 3.5.0 # собрать с конкретной версией (override)
#
# Выход: ../zips-v<VER>/foreman-pro-v<VER>.zip set -euo pipefail SRC_DIR="$(cd "$(dirname "$0")" && pwd)"
SLUG="foreman-pro" # Версия из SKILL.md или переопределённая аргументом
if [[ -n "${1:-}" ]]; then VER="$1"
...
FILES=( SKILL.md README.md config.yaml .env.example install.sh test/smoke-test.sh examples/quick-start.md examples/full-library.md docs/onboarding.md docs/anti-fail.md docs/roi.md proof/case-01-object-managed-by-memory.md proof/case-02-acceptance-dispute-fixed.md proof/case-03-hidden-works-not-recorded.md proof/case-04-supplies-and-crews-unsynced.md proof/case-05-docs-left-for-the-end.md
) # Опциональные (включаются если существуют)
OPTIONAL=( proof/dogfooding-RAAI.md
...
done # Marketing pack, competitors (если есть) — включаются целиком
for sub in marketing competitors; do if [[ -d "$SRC_DIR/$sub" ]] && [[ -n "$(ls -A "$SRC_DIR/$sub" 2>/dev/null)" ]]; then cp -R "$SRC_DIR/$sub" "$STAGE_DIR/$sub" echo " +dir: $sub/" fi
...
if [[ "${SMOKE_TEST:-1}" == "1" ]]; then if command -v unzip >/dev/null 2>&1; then echo "" echo "==> Running smoke-test on fresh ZIP..." TEST_DIR="$(mktemp -d)" unzip -q "$OUT_DIR/$ZIP_NAME" -d "$TEST_DIR" cd "$TEST_DIR/${SLUG}-v${VER}" if [[ -x test/smoke-test.sh ]]; then bash test/smoke-test.sh || { echo "SMOKE-TEST FAILED"; exit 1; } else echo " (smoke-test.sh не найден или не исполняемый — пропускаем)" fi rm -rf "$TEST_DIR" else echo " (unzip не найден — smoke-test на ZIP пропускаем)" fi
fi
