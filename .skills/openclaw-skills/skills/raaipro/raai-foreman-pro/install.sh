#!/usr/bin/env bash
# AI-Прораб PRO — автоустановщик
# Копирует коробку в ~/.openclaw/skills/ и создаёт дефолтные файлы.
#
# Usage:
# bash install.sh [--claude-home ~/.openclaw/skills/foreman-pro]
#
# Поддержка: Linux / macOS / WSL. Windows — см. README Quick Start вариант ручной установки. set -euo pipefail SKILL_NAME="foreman-pro"
DEFAULT_TARGET="${HOME}/.openclaw/skills/${SKILL_NAME}"
TARGET="${1:-$DEFAULT_TARGET}"
SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)" echo ""
echo "╔══════════════════════════════════════════════════╗"
echo "║ AI-Прораб PRO — установка ║"
echo "╚══════════════════════════════════════════════════╝"
echo ""
...
fi if [[ "$OPENCLAW_OK" == "no" && "$CLAUDE_OK" == "no" ]]; then echo "[!] Не найден ни openclaw, ни claude. Коробку всё равно установим, но запустить" echo " вы сможете только когда поставите один из движков." echo " OpenClaw: https://openclaw.ai/install" echo " Claude Code: https://claude.ai/download" echo ""
...
echo " • leader.role — Прораб / Начальник участка / Технадзор"
echo " • company.name — название компании"
echo " • object.name — название объекта"
echo " • team.foreman — ФИО прораба"
echo ""
echo "3. Smoke-тест:"
echo " bash $TARGET/test/smoke-test.sh"
echo ""
echo "4. Первый запуск: напишите боту/агенту любой триггер:"
echo " (план дня / приёмка: штукатурка / бригада: состав / журнал: запись)"
echo ""
