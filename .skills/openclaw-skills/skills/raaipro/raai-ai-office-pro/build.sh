#!/usr/bin/env bash
# build.sh — собирает AI-офис PRO в отгрузочный ZIP из исходника.
...
#
# Выход: ../zips-v<VER>/ai-office-pro-v<VER>.zip set -euo pipefail SRC_DIR="$(cd "$(dirname "$0")" && pwd)"
SLUG="ai-office-pro" # Версия из SKILL.md или переопределённая аргументом
if [[ -n "${1:-}" ]]; then VER="$1"
...
FILES=( SKILL.md README.md config.yaml .env.example install.sh test/smoke-test.sh examples/quick-start.md examples/full-library.md docs/onboarding.md docs/anti-fail.md docs/roi.md proof/case-01-chaos-to-briefing.md proof/case-02-no-kpi-control.md proof/case-03-overloaded-ceo.md proof/case-04-chaotic-decisions.md proof/case-05-no-quarter-focus.md
) # Опциональные (включаются если существуют): глубокий config + RAAI-промпт + dogfooding
OPTIONAL=( config/config-deep.yaml config/raai_system_prompt.md proof/dogfooding-RAAI.md docs/competitors-comparison.md docs/roi-calculator.md
...
done # Marketing pack (если есть) и demo (если есть) — включаются целиком
...
echo ""
echo "Contents:"
unzip -l "$OUT_DIR/$ZIP_NAME" | tail -30 # Smoke-test на свежераспакованном ZIP
if [[ -n "${SMOKE_TEST:-1}" ]]; then echo "" echo "==> Running smoke-test on fresh ZIP…" TEST_DIR="$(mktemp -d)" unzip -q "$OUT_DIR/$ZIP_NAME" -d "$TEST_DIR" cd "$TEST_DIR/${SLUG}-v${VER}" if [[ -x test/smoke-test.sh ]]; then bash test/smoke-test.sh || { echo "SMOKE-TEST FAILED"; exit 1; } else echo " (smoke-test.sh not executable or not found — skipped)" fi rm -rf "$TEST_DIR"
fi
