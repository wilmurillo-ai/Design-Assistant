#!/usr/bin/env bash
# AI-Монитор PRO — автоустановщик
# Копирует коробку в ~/.openclaw/skills/ и создаёт дефолтные файлы.
#
# Usage:
# bash install.sh [--claude-home ~/.openclaw/skills/ai-monitor-pro]
#
# Поддержка: Linux / macOS / WSL. Windows — см. README Quick Start вариант ручной установки. set -euo pipefail SKILL_NAME="ai-monitor-pro"
DEFAULT_TARGET="${HOME}/.openclaw/skills/${SKILL_NAME}"
TARGET="${1:-$DEFAULT_TARGET}"
SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)" echo ""
echo "╔══════════════════════════════════════════════════╗"
echo "║ AI-Монитор PRO — установка ║"
echo "╚══════════════════════════════════════════════════╝"
echo ""
...
echo ""
echo "══════ СЛЕДУЮЩИЕ ШАГИ (10 минут) ══════"
echo ""
echo "1. Откройте config.yaml:"
echo " $TARGET/config.yaml"
echo ""
echo "2. Заполните обязательные поля (OBLIGATORY):"
echo " • leader.name — Имя Отчество"
echo " • leader.role — CTO / Технический директор / Прораб"
echo " • company.name — название компании / объекта"
echo " • company.hourly_revenue_rub — выручка в час (для ROI)"
echo " • servers — список ваших серверов / объектов"
echo ""
echo "3. Smoke-тест:"
echo " bash $TARGET/test/smoke-test.sh"
echo ""
echo "4. Первый запуск: напишите боту/агенту любой триггер"
echo " (дашборд / мониторинг стройки / алерт / фото-отчёт / контроль качества)"
echo ""
