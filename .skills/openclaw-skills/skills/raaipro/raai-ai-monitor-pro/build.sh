#!/usr/bin/env bash
# build.sh — собирает AI-Монитор PRO в отгрузочный ZIP из исходника.
...
#
# Выход: ../zips-v<VER>/ai-monitor-pro-v<VER>.zip set -euo pipefail SRC_DIR="$(cd "$(dirname "$0")" && pwd)"
SLUG="ai-monitor-pro" # Версия из SKILL.md или переопределённая аргументом
if [[ -n "${1:-}" ]]; then VER="$1"
...
FILES=( SKILL.md README.md config.yaml .env.example install.sh test/smoke-test.sh examples/quick-start.md examples/full-library.md docs/onboarding.md docs/anti-fail.md docs/roi.md proof/case-01-alert-noise-to-control.md proof/case-02-sla-buffer-visible.md proof/case-03-incident-response-faster.md proof/case-04-ceo-visibility-on-risk.md proof/case-05-capacity-planning-grounded.md
) # Опциональные (включаются если существуют)
OPTIONAL=( proof/dogfooding-RAAI.md docs/competitors-comparison.md docs/roi-calculator.md
...
done # Упаковка — кросс-платформенная (zip / 7z / powershell; БЕЗ python3 — на Windows открывает MS Store)
...
fi # Smoke-test на свежераспакованном ZIP (если не отключён)
