#!/usr/bin/env bash
# AI-Сметчик PRO — автоустановщик
# Копирует коробку в ~/.openclaw/skills/ и создаёт дефолтные файлы.
#
# Usage:
# bash install.sh [--claude-home ~/.openclaw/skills/estimator-pro]
#
# Поддержка: Linux / macOS / WSL. Windows — см. README Quick Start вариант ручной установки. set -euo pipefail SKILL_NAME="estimator-pro"
DEFAULT_TARGET="${HOME}/.openclaw/skills/${SKILL_NAME}"
TARGET="${1:-$DEFAULT_TARGET}"
SRC_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)" echo ""
echo "╔══════════════════════════════════════════════════╗"
echo "║ AI-Сметчик PRO — установка ║"
echo "╚══════════════════════════════════════════════════╝"
echo ""
...
echo " • leader.role — Сметчик / Прораб / Генподрядчик / Заказчик"
echo " • company.name — название компании"
echo " • region.name — регион (Москва / СПб / Екатеринбург и т.д.)"
echo " • region.ter_code — код ТЕР вашего региона"
echo ""
echo "3. Smoke-тест:"
echo " bash $TARGET/test/smoke-test.sh"
echo ""
echo "4. Первый запуск: напишите боту/агенту любой триггер"
echo " (смета / материалы / проверь смету / тендер / дефектовка)"
echo ""
