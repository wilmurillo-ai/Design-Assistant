...
def verify_multi_witness(criterion: VerificationCriterion, proof: ProofItem) -> CriterionResult:
    """Verify N-of-M witness attestations.

    Params: required_witnesses (int), witness_threshold (int, default = required)
    Proof data: attestations (list of {witness_id: str, attested: bool})
    """
    required = int(criterion.params.get("required_witnesses", 2))
    threshold = int(criterion.params.get("witness_threshold", required))

    attestations = proof.data.get("attestations", [])
    valid_attestations = [a for a in attestations if a.get("attested", False)]
    unique_witnesses = set(a.get("witness_id", "") for a in valid_attestations if a.get("witness_id"))

    passed = len(unique_witnesses) >= threshold

    return CriterionResult(
        type="multi_witness",
        passed=passed,
        detail=f"Witnesses: {len(unique_witnesses)}/{threshold} required (from {len(attestations)} submitted)",
    )
...
_VERIFIERS = {
    ...
    "multi_witness": verify_multi_witness,
}
