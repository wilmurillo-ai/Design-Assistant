...
class VerificationCriterion(BaseModel):
    ...
    type: Literal["gps_proof", "photo_proof", "timestamp_proof", "signature_proof", "multi_witness"]
...
class Milestone(BaseModel):
    ...
    budget_pct: int = Field(..., gt=0, le=100, description="Percentage of total budget for this milestone.")
...
class CreateTaskRequest(BaseModel):
    ...
    milestones: list[Milestone] | None = Field(default=None, max_length=10)
...
class VerifyResponse(BaseModel):
    ...
    milestone_index: int | None = Field(default=None, description="Which milestone was verified (if applicable).")
    budget_released_usdc: Decimal | None = Field(default=None, description="Partial budget released for milestone.")
...
class VerifyMilestoneRequest(BaseModel):
    ...
    worker_id: str = Field(..., min_length=1, max_length=200)
    milestone_index: int = Field(..., ge=0, description="Which milestone to verify (0-indexed).")
    proofs: list[ProofItem] = Field(..., min_length=1, max_length=20)
...
class WorkerResponse(BaseModel):
    ...
    tasks_disputed: int = Field(..., ge=0)
    total_earned_usdc: Decimal = Field(..., ge=Decimal("0"))
