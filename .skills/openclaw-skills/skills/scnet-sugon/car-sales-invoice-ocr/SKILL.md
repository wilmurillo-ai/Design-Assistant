...
---
# Sugon-Scnet 机动车销售统一发票识别 OCR 技能

本技能封装了 Sugon-Scnet 机动车销售统一发票识别 OCR 服务，通过单一接口即可调用 1 种识别能力，高效提取文字及票据信息。

## 功能特性

- **机动车销售统一发票票据**：覆盖机动车销售统一发票自动提取关键字段。
...
```bash
   python .claude/skills/car_sales_invoice_ocr/scripts/main.py VEHICLE_SALE_INVOICE /path/to/invoice.jpg
```
