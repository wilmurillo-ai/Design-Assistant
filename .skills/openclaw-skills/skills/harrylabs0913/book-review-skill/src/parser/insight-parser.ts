...
export class InsightParser {
  ...
  async parse(text: string): Promise<ParsedInsight> {
    // 1. 文本预处理
  ...
  private simpleChineseKeywordExtraction(text: string): string[] {
  ...
  private identifyThemes(keywords: string[]): string[] {
  ...
  private assessComplexity(text: string, keywords: string[]): 'simple' | 'medium' | 'complex' {
    ...
    if (hasComplexStructure) score += 1;
