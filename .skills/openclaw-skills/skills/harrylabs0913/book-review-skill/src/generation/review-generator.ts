import { ParsedInsight, NoteSearchResult, ReviewResult, ReviewReference, GenerationContext } from '../types';
import axios from 'axios';

export class ReviewGenerator {
  private aiProvider: 'deepseek' | 'openai' | 'local';
  ...
  constructor(
    aiProvider: 'deepseek' | 'openai' | 'local' = 'deepseek',
  ...
  async generate(parsed: ParsedInsight, notes: NoteSearchResult[]): Promise<ReviewResult> {
    // 1. 准备生成上下文
  ...
  private prepareContext(parsed: ParsedInsight, notes: NoteSearchResult[]): GenerationContext {
    return {
      insight: parsed.text,
  ...
  private async generateWithDeepSeek(context: GenerationContext): Promise<string> {
    try {
  ...
  private async generateWithOpenAI(context: GenerationContext): Promise<string> {
    // 类似 DeepSeek 的实现，但使用 OpenAI API
    // 为简化，这里返回模板生成
    return this.generateWithTemplate(context);
  }

  private generateWithTemplate(context: GenerationContext): string {
...
Expanded review:`;
  ...
  private postProcess(text: string, context: GenerationContext): string {
  ...
  private extractReferences(text: string, notes: NoteSearchResult[]): ReviewReference[] {
  ...
  private generateSuggestions(parsed: ParsedInsight, notes: NoteSearchResult[]): string[] {
  ...
  private calculateConfidence(
    ...
    // 基于心得复杂度
    if (parsed.complexity === 'complex') confidence += 0.1;
    else if (parsed.complexity === 'simple') confidence -= 0.05;
    
    // 基于相关笔记数量
    if (notes.length >= 3) confidence += 0.1;
    else if (notes.length === 0) confidence -= 0.1;
    
    // 基于生成点评的质量（简单启发式）
    const reviewLength = review.length;
    if (reviewLength > 300 && reviewLength < 800) confidence += 0.1;
    if (reviewLength < 100) confidence -= 0.1;
