...
export class OutputFormatter {
  format(review: ReviewResult, format: OutputFormat = 'markdown'): SkillOutput {
  ...
  private formatMarkdown(review: ReviewResult): string {
    ...
    if (references.length > 0) {
      ...
      references.forEach(ref => {
        ...
        if (ref.page) output += ` (第${ref.page}页)`;
        output += `\n`;
      });
      output += `\n`;
    }
  ...
  private formatPlain(review: ReviewResult): string {
    const { original, expanded, references, suggestions, confidence } = review;
  ...
  private formatHtml(review: ReviewResult): string {
    const { original, expanded, references, suggestions, confidence } = review;
...
<body>
  ...
  private extractThemes(review: ReviewResult): string[] {
    const themes: string[] = [];
