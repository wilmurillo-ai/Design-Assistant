 ...
 * 12 Curated Celebrity Templates × 10 Configuration Files
 * 
 * Features:
 * - Local template selection (12 curated templates)
 * - Dialog-based installation guide
 * - Optional natural language custom generation (API)
...
// 12 curated templates by category
const TEMPLATE_CATEGORIES = {
  tech: ['linus-torvalds', 'guido-van-rossum'],
  startup: ['elon-musk', 'steve-jobs'],
  science: ['albert-einstein', 'alan-turing'],
  business: ['bill-gates', 'satya-nadella'],
  leadership: ['zhang-yiming', 'ren-zhengfei'],
  performance: ['andrew-ng', 'jensen-huang']
};
...
// Required tools for template operations
const REQUIRED_TOOLS = ['Find Skills', 'Summarize', 'healthcheck'];
...
async function handler(params) {
  ...
  if (!apiKey || apiKey.trim().length === 0) {
    return { 
      ...
      message: 'AI Soul Weaver Cloud API key required for custom generation mode',
      hint: 'API key required for cloud generation mode. You can also try local template mode without API key'
    };
  }
