...
#
# Runs the full Soul Weaver daily protocol
...
openclaw cron add \
  --name "soul-weaver-morning-briefing" \
  --cron "0 8 * * *" \
  --tz "America/Los_Angeles" \
  --session isolated \
  --message "Run the Soul Weaver daily protocol:
...
Step 3: Priority scan — Check for any tasks requiring attention.
