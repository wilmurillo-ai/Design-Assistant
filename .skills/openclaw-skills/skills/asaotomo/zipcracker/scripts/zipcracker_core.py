...
ZIPCRACKER_ASSUME_CRC_RECOVERY_ENV = "ZIPCRACKER_ASSUME_CRC_RECOVERY"
ZIPCRACKER_ASSUME_LARGE_MASK_ENV = "ZIPCRACKER_ASSUME_LARGE_MASK"
ZIPCRACKER_ASSUME_TEMPLATE_KPA_ENV = "ZIPCRACKER_ASSUME_TEMPLATE_KPA"
...
def bundled_password_list_path() -> Optional[str]:
    if os.path.isfile(DEFAULT_PASSWORD_LIST_PATH):
        return DEFAULT_PASSWORD_LIST_PATH
    return None
...
def offer_template_kpa_after_standard_failures(
    ...
    should_try = prompt_yes_no(
        ...
        env_name=ZIPCRACKER_ASSUME_TEMPLATE_KPA_ENV,
    )
    if not should_try:
        print(
            loc(
    ...
    for filename in file_list:
        ...
        if 0 < info.file_size <= 6:
            should_try = prompt_yes_no(
                ...
                env_name=ZIPCRACKER_ASSUME_CRC_RECOVERY_ENV,
            )
            if should_try:
                print(
                    loc(
...
def try_fast_password_from_plaintext(
    ...
    default_password_list = bundled_password_list_path()
    if default_password_list:
        try:
            for batch in iter_password_file_batches(default_password_list, 4096):
...
def crack_password_with_mask(
    ...
    if total_passwords > 100_000_000_000:
        should_continue = prompt_yes_no(
            ...
            env_name=ZIPCRACKER_ASSUME_LARGE_MASK_ENV,
        )
