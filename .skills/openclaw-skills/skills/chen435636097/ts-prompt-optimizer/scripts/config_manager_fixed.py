...
class TSConfigManager:
    ...
    def __init__(self, config_dir: str = None):
        ...
        self.default_config = self.config_dir / "model_config_new.json"
...
def main():
    ...
    if command == "status":
        ...
        for model_name, model_status in status['models'].items():
            if model_status['api_key_configured']:
                status_text = "[[EMOJI:5DF2][EMOJI:914D][EMOJI:7F6E]]"
            else:
                status_text = "[[EMOJI:672A][EMOJI:914D][EMOJI:7F6E]]"
            
            if model_status['enabled']:
                enabled_text = "[[EMOJI:542F][EMOJI:7528]]"
            else:
                enabled_text = "[[EMOJI:7981][EMOJI:7528]]"
            
            print(f"  {model_name}: {model_status['provider']}/{model_status['model']} {enabled_text} {status_text}")
    
    elif command == "show":
        config = config_manager.load_config()
        print("[EMOJI:5B8C][EMOJI:6574][EMOJI:914D][EMOJI:7F6E]:")
        ...
        if all_ok:
            ...
            print("\n[EMOJI:8BF7][EMOJI:8FD0][EMOJI:884C][EMOJI:914D][EMOJI:7F6E][EMOJI:5411][EMOJI:5BFC][EMOJI:8BBE][EMOJI:7F6E][EMOJI:7F3A][EMOJI:5931][EMOJI:7684]API[EMOJI:5BC6][EMOJI:94A5]")
    
    elif command == "get" and len(sys.argv) > 2:
        model_name = sys.argv[2]
        config = config_manager.get_model_config(model_name)
        if config:
            print(f"{model_name} [EMOJI:914D][EMOJI:7F6E]:")
            print(json.dumps(config, ensure_ascii=False, indent=2))
        else:
            print(f"[EMOJI:672A][EMOJI:627E][EMOJI:5230][EMOJI:6A21][EMOJI:578B] {model_name} [EMOJI:7684][EMOJI:914D][EMOJI:7F6E]")
    
    elif command == "test":
        print("[EMOJI:914D][EMOJI:7F6E][EMOJI:7CFB][EMOJI:7EDF][EMOJI:6D4B][EMOJI:8BD5]")
        print("=" * 60)
        
        # [EMOJI:6D4B][EMOJI:8BD5][EMOJI:914D][EMOJI:7F6E][EMOJI:52A0][EMOJI:8F7D]
        config_manager = TSConfigManager()
        config = config_manager.load_config()
        print(f"[EMOJI:914D][EMOJI:7F6E][EMOJI:52A0][EMOJI:8F7D][EMOJI:6210][EMOJI:529F][EMOJI:FF0C][EMOJI:5305][EMOJI:542B] {len(config.get('models', {}))} [EMOJI:4E2A][EMOJI:6A21][EMOJI:578B]")
        
        # [EMOJI:6D4B][EMOJI:8BD5][EMOJI:72B6][EMOJI:6001][EMOJI:68C0][EMOJI:67E5]
        status = config_manager.check_config_status()
        print(f"[EMOJI:72B6][EMOJI:6001][EMOJI:68C0][EMOJI:67E5][EMOJI:6210][EMOJI:529F][EMOJI:FF0C][EMOJI:53D1][EMOJI:73B0] {len(status['issues'])} [EMOJI:4E2A][EMOJI:95EE][EMOJI:9898]")
        ...
        print("\n[EMOJI:914D][EMOJI:7F6E][EMOJI:7CFB][EMOJI:7EDF][EMOJI:6D4B][EMOJI:8BD5][EMOJI:901A][EMOJI:8FC7][EMOJI:FF01]")
