...
async function testWithAppPassword() {
  ...
  if (config.appPassword === 'YOUR_APPLICATION_PASSWORD_HERE') {
    console.log('❌ 请先创建应用程序密码并更新配置');
    console.log('\n📋 创建步骤:');
    console.log('   1. 访问 https://your-site.com/wp-admin/profile.php');
    console.log('   2. 找到"应用程序密码"部分');
    console.log('   3. 输入名称: "OpenClaw-Auto-Publish"');
    console.log('   4. 点击"添加新应用程序密码"');
    console.log('   5. 复制生成的密码（24字符，有空格）');
    console.log('   6. 更新此脚本中的 appPassword');
    return;
  }
...
// 自动发布函数
async function autoPublish(title, content, options = {}) {
  const postData = {
    title: title,
    content: content,
...
// 使用
const result = await autoPublish('文章标题', '文章内容', {
  status: 'draft'
});
...
// 运行测试
testWithAppPassword().catch(error => {
