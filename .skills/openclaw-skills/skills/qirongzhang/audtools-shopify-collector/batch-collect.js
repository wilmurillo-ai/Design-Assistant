 ...
 * 
 * 使用方法：
 *   node batch-collect.js [csv-file-path]
 * 
 * 示例：
 *   node batch-collect.js C:\workspace\caiji\shop-futvortexstore-com-categories.csv
 */
...
async function batchCollect(csvFilePath) {
  ...
  // 提取完整链接列
  const links = results.map(row => row['完整链接']).filter(link => link && link.trim());
...
async function main() {
  ...
  if (!csvFilePath) {
    // 默认路径
    csvFilePath = 'C:\\workspace\\caiji\\shop-futvortexstore-com-categories.csv';
  }
