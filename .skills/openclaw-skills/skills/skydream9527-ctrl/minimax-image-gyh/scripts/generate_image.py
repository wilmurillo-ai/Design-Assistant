...
def build_request_body(prompt: str, image_size: str, aspect_ratio: str) -> bytes:
...
def make_api_request(api_key: str, request_body: bytes) -> dict:
    ...
    try:
        ...
        if e.code == 429:
            ...
            print("\nWhat to do:", file=sys.stderr)
            print("  1. Verify your API key", file=sys.stderr)
            print("  2. Ensure image generation is enabled for your plan", file=sys.stderr)
            if error_detail:
                print(f"\nAPI message: {error_detail}", file=sys.stderr)
        elif e.code == 400:
            print("=" * 60, file=sys.stderr)
            ...
            print("=" * 60, file=sys.stderr)
            print("ERROR: MiniMax API server error", file=sys.stderr)
            print("=" * 60, file=sys.stderr)
            print(f"\nThe API returned a server error (HTTP {e.code}).", file=sys.stderr)
            print("\nWhat to do:", file=sys.stderr)
            print("  1. Wait a few minutes and try again", file=sys.stderr)
            print("  2. Check MiniMax API status if the issue persists", file=sys.stderr)
            if error_detail:
                print(f"\nAPI message: {error_detail}", file=sys.stderr)
        else:
...
def extract_image_data(response: dict) -> str:
    ...
    try:
        ...
        if "data" in response and len(response["data"]) > 0:
            return response["data"][0].get("base64", "")
