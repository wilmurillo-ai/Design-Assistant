 ...
 * 生成周计划
 */
export function generateWeekPlan(selector, weekStart) {
...
function generateBeginnerPlan(weekStart, weeklyDays) {
    const days = [];
...
function generatePPLPlan(weekStart, weeklyDays) {
    const days = [];
    // 根据 weeklyDays 安排
...
function getPPLSchedule(weeklyDays) {
    // 返回 7 天的安排
    if (weeklyDays === 3)
...
function generateUpperLowerPlan(weekStart) {
    const days = [];
...
function getUpperLowerExercises(type) {
    if (type === 'upperA')
        return upperBodyA;
    if (type === 'lowerA')
        return lowerBodyA;
...
function getUpperLowerName(type) {
    const names = {
        upperA: '上肢 A',
        upperB: '上肢 B',
        lowerA: '下肢 A',
...
function getUpperLowerFocus(type) {
    if (type.startsWith('upper'))
        return '胸、背、肩、手臂';
    return '股四头、腘绳肌、臀、小腿';
}
function generateHomePlan(weekStart, weeklyDays) {
    const days = [];
...
function generateRunningPlan(weekStart, weeklyDays) {
    const days = [];
    const dayNames = ['周一', '周二', '周三', '周四', '周五', '周六', '周日'];
    for (let i = 0; i < 7; i++) {
        const date = addDays(weekStart, i);
        let plan;
        if (i === 0)
            plan = { ...runningBeginner, day: i + 1, date };
        else if (i === 2)
            plan = { ...runningIntervals, day: i + 1, date };
        else if (i === 4)
            plan = { ...runningLong, day: i + 1, date };
        else
            plan = { ...restDay, day: i + 1, date };
        days.push(plan);
    }
    return { weekStart, planType: 'running', days };
}
function generateHIITPlan(weekStart, weeklyDays) {
    const days = [];
