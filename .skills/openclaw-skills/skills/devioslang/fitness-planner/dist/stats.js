 ...
 * 更新统计数据（打卡后调用）
 */
export function updateStats(record) {
 ...
 * 计算当前连续天数
 */
function calculateCurrentStreak(lastDate) {
 ...
 * 生成周总结
 */
export function generateWeeklySummary(weekStart) {
    ...
    // 生成建议
    const recommendations = generateRecommendations(completedDays, totalDays, feelingDistribution, skippedDays);
    return {
        weekStart,
        weekEnd,
        totalDays,
 ...
 * 生成调整建议
 */
function generateRecommendations(completedDays, totalDays, feelings, skippedDays) {
