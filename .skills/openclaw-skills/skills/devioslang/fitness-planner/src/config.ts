 ...
 * 更新用户配置
 */
export function updateUserConfig(updates: Partial<UserConfig>): Config {
 ...
 * 获取缺失的配置项
 */
export function getMissingFields(config: Config): string[] {
  const missing: string[] = [];
  const { user } = config;
  
  if (!user.gender) missing.push('性别');
  if (!user.age) missing.push('年龄');
  if (!user.goal) missing.push('训练目标');
  if (!user.location) missing.push('训练场地');
  if (!user.weeklyDays) missing.push('每周训练天数');
  if (!user.sessionDuration) missing.push('每次训练时长');
  if (!user.experience) missing.push('健身经验');
...
export function parseExperience(input: string): Experience | null {
  const lower = input.toLowerCase();
  if (lower.includes('新') || lower.includes('beginner') || lower.includes('<6')) return 'beginner';
  if (lower.includes('高级') || lower.includes('advanced') || lower.includes('>24')) return 'advanced';
  if (lower.includes('中级') || lower.includes('intermediate') || lower.includes('6-24')) return 'intermediate';
  return null;
}
