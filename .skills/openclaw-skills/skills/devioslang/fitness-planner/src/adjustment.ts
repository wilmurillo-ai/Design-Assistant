...
import { WorkoutRecord, WeekPlan, Feeling, DayPlan } from './types.js';
import { loadMonthRecords, loadCurrentPlan, saveCurrentPlan, getWeekRecords } from './recorder.js';
...
const defaultState: AdjustmentState = {
 ...
 * 加载调整状态
 */
export function loadAdjustmentState(): AdjustmentState {
 ...
 * 保存调整状态
 */
export function saveAdjustmentState(state: AdjustmentState): void {
 ...
 * 更新调整状态（打卡时调用）
 */
export function updateAdjustmentState(record: WorkoutRecord): void {
  const state = loadAdjustmentState();
 ...
 * 调整计划强度
 */
export function adjustPlanIntensity(
  action: 'reduce' | 'increase'
): { success: boolean; message: string; plan?: WeekPlan } {
  const plan = loadCurrentPlan();
 ...
 * 生成下周计划（带调整）
 */
export function generateNextWeekPlan(): { success: boolean; message: string; plan?: WeekPlan } {
  const state = loadAdjustmentState();
  const stats = loadStats();
 ...
 * 检查是否需要主动询问
 */
export function checkProactiveInquiry(): string | null {
  const state = loadAdjustmentState();
  const plan = loadCurrentPlan();
