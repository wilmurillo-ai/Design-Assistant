...
import { loadCurrentPlan, saveCurrentPlan, getTodayPlan, getWeekRecords } from './recorder.js';
...
const defaultPeriodizationState: PeriodizationState = {
  currentPhase: {
    type: 'hypertrophy',
    weekNumber: 1,
    totalWeeks: 4,
...
function loadPeriodizationState(): PeriodizationState {
  try {
...
function savePeriodizationState(state: PeriodizationState): void {
  fs.writeFileSync(PERIODIZATION_FILE, JSON.stringify(state, null, 2), 'utf-8');
}

function calculatePhaseEndDate(phase: TrainingPhase): string {
 ...
 * 获取阶段描述
 */
function getPhaseDescription(type: PhaseType): string {
 ...
 * 获取阶段强度参数
 */
function getPhaseParameters(type: PhaseType): {
 ...
 * 根据阶段调整训练计划
 */
export function adjustPlanForPhase(plan: WeekPlan, phase: TrainingPhase): WeekPlan {
 ...
 * 推进到下一阶段
 */
export function advancePhase(): { message: string; newPhase: TrainingPhase } {
...
function getNextPhaseType(current: PhaseType, history: TrainingPhase[]): PhaseType {
  // 标准周期：力量 → 肌肥大 → 力量 → 减载
  const cycle: PhaseType[] = ['strength', 'hypertrophy', 'endurance', 'deload'];
 ...
 * 更新阶段周数
 */
export function incrementPhaseWeek(): void {
  const state = loadPeriodizationState();
...
function getDefaultMuscleProgress(): MuscleProgressState {
  const muscles: Record<MuscleGroup, MuscleProgress> = {} as any;
  
  const groups: MuscleGroup[] = ['chest', 'back', 'shoulder', 'biceps', 'triceps', 'leg', 'core', 'cardio'];
...
export function loadMuscleProgress(): MuscleProgressState {
  try {
...
function saveMuscleProgress(state: MuscleProgressState): void {
 ...
 * 获取动作涉及的目标肌群
 */
export function getExerciseMuscles(exerciseName: string): MuscleGroup[] {
 ...
 * 记录肌群训练
 */
export function recordMuscleTraining(exercises: Exercise[]): void {
 ...
 * 记录肌群酸痛
 */
export function recordSoreness(muscles: MuscleGroup[], level: number): void {
  const state = loadMuscleProgress();
  const today = new Date().toISOString().split('T')[0];
...
export function analyzeMuscleBalance(): {
  ...
  const recovered: MuscleGroup[] = [];
  const recommendations: string[] = [];
  
  for (const [muscle, progress] of Object.entries(state.muscles) as [MuscleGroup, MuscleProgress][]) {
    // 检查是否很久没训练
 ...
 * 根据肌群进展推荐动作调整
 */
export function recommendExerciseAdjustments(plan: DayPlan): {
...
function getDefaultFeedbackState(): FeedbackState {
  return {
    recentFeedback: [],
    averageSleep: 7,
    averageEnergy: 5,
 ...
 * 记录多维度反馈
 */
export function recordFeedback(feedback: MultiDimensionFeedback): void {
  ...
  state.recentFeedback = state.recentFeedback.filter(
    f => new Date(f.date) >= thirtyDaysAgo
  );
  ...
  if (state.recentFeedback.length > 0) {
    ...
    const energyMap: Record<EnergyLevel, number> = { low: 3, medium: 5, high: 8 };
    state.averageEnergy = state.recentFeedback.reduce((sum, f) => sum + energyMap[f.energy], 0) / state.recentFeedback.length;
    
    const stressMap: Record<StressLevel, number> = { low: 2, medium: 5, high: 8 };
 ...
 * 计算恢复指数
 */
function calculateRecoveryScore(state: FeedbackState): number {
  // 睡眠权重 40%
...
export function recordQuickFeedback(
  ...
  const feedback: MultiDimensionFeedback = {
