...
import { DayPlan, WeekPlan, WeeklySummary, Feeling, Exercise } from './types.js';
import { formatDate } from './templates.js';
import { formatDuration } from './stats.js';
import { findExerciseDetail, getExerciseBrief, ExerciseDetail } from './exercise-detail.js';
 ...
 * 搜索动作教学视频
 */
export async function searchExerciseVideo(exerciseName: string): Promise<{ url: string; title: string; author: string } | null> {
  // 先检查缓存
  videoCache = loadVideoCache();
  ...
  try {
    const { stdout } = await execAsync(
      `mcporter call 'exa.web_search_exa(query: "${searchKeyword} bilibili", numResults: 1)'`,
      { timeout: 30000 }
    );
 ...
 * 批量搜索动作视频
 */
export async function searchExerciseVideos(exercises: string[]): Promise<Map<string, { url: string; title: string; author: string }>> {
  const results = new Map<string, { url: string; title: string; author: string }>();
 ...
 */
export async function sendWecomMessage(message: string): Promise<boolean> {
 ...
 * 格式化训练提醒消息（包含动作讲解）
 */
export function formatWorkoutReminder(plan: DayPlan, includeDetails: boolean = true): string {
  const lines: string[] = [
    '🏋️ 今日训练提醒',
    '',
 ...
 */
export async function formatWorkoutReminderWithVideoLinks(plan: DayPlan): Promise<string> {
  const lines: string[] = [
    '🏋️ 今日训练提醒',
    '',
    `📋 ${plan.name} - ${plan.focus}`,
 ...
 * 格式化带视频链接的训练计划
 */
export function formatWorkoutReminderWithVideos(
  plan: DayPlan, 
 ...
 * 格式化单个动作的详细讲解
 */
export function formatExerciseDetail(name: string): string {
 ...
 * 格式化单个动作的详细讲解（包含视频链接）
 */
export async function formatExerciseDetailWithVideo(name: string): Promise<string> {
  const detail = findExerciseDetail(name);
  if (!detail) {
    // 尝试搜索视频
    const video = await searchExerciseVideo(name);
 ...
 * 格式化所有动作的简要讲解
 */
export function formatAllExerciseBriefs(plan: DayPlan): string {
  const lines: string[] = [
    `📖 ${plan.name} - 动作讲解`,
    ''
  ];
 ...
 * 格式化所有动作的完整讲解（包含视频链接）
 */
export async function formatAllExerciseBriefsWithVideos(plan: DayPlan): Promise<string> {
  const lines: string[] = [
    `📖 ${plan.name} - 动作讲解`,
    ''
  ];
 ...
 * 格式化早间总结消息
 */
export function formatMorningSummary(plan: DayPlan | null): string {
 ...
 * 格式化打卡成功消息
 */
export function formatCheckinSuccess(
  dayName: string,
  duration: number,
  feeling: Feeling | null
): string {
  const feelingEmoji: Record<Feeling, string> = {
    great: '💪',
    okay: '😐',
    tired: '😫'
  };
...
export function formatWeekProgress(
  ...
  records: Map<string, { feeling: Feeling | null; duration: number }>
): string {
...
export function formatStatsMessage(stats: {
  ...
  thisMonth: { workouts: number; minutes: number };
}): string {
  return [
    '📈 训练统计',
    '',
