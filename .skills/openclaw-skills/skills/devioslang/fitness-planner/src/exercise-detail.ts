...
export interface ExerciseDetail {
  ...
  videoSearchKeywords: string;  // 搜索视频的关键词
}
...
export function findExerciseDetail(name: string): ExerciseDetail | undefined {
  ...
  return exerciseDetails.find(ex => 
    ex.name === name ||
 ...
 * 获取动作的简要讲解（用于消息输出）
 */
export function getExerciseBrief(name: string): string {
 ...
 * 获取动作的完整讲解
 */
export function getExerciseFullDescription(name: string): string {
  const detail = findExerciseDetail(name);
  if (!detail) return `未找到 ${name} 的讲解`;
