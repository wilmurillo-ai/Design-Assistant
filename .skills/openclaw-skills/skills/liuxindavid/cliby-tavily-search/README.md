...
- Supports API key via CLI flag, environment variable, or secret file
...
```bash
mkdir -p skills/cliby-tavily-search/.secrets
chmod 700 skills/cliby-tavily-search/.secrets
```

Create the key file:

```bash
nano skills/cliby-tavily-search/.secrets/tavily.key
```
...
```bash
chmod 600 skills/cliby-tavily-search/.secrets/tavily.key
```
...
```bash
python3 skills/cliby-tavily-search/scripts/tavily_search.py --query "latest AI news"
```

News-focused search:

```bash
python3 skills/cliby-tavily-search/scripts/tavily_search.py --query "gold prices" --topic news
```

Advanced search:

```bash
python3 skills/cliby-tavily-search/scripts/tavily_search.py --query "raspberry pi ubuntu server optimization" --depth advanced
```

JSON output:

```bash
python3 skills/cliby-tavily-search/scripts/tavily_search.py --query "python asyncio" --json
```
...
4. The script reads the API key from `--api-key`, `TAVILY_API_KEY`, or `.secrets/tavily.key`
