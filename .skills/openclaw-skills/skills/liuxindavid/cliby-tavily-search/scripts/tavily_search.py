...
def load_api_key(args: argparse.Namespace):
    if args.api_key:
        return args.api_key.strip() or None
...
def main() -> int:
    ...
    if not api_key:
        print(
            "Error: Tavily API key not found. Provide --api-key, set TAVILY_API_KEY, or create ../.secrets/tavily.key",
            file=sys.stderr,
        )
        return 1
