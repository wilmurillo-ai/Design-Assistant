...
| `error.api_code` | int | No | Pangolin API error code (when applicable) |
...
When using `--raw`, the unprocessed Pangolin API response is output. The envelope structure above does **not** apply.
