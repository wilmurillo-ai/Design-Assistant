---
name: free-photo-video
version: "1.0.0"
displayName: "Free Photo Video — Turn Photos Into Shareable Videos"
description: >
  turn photos or images into photo slideshow video with this skill. Works with JPG, PNG, WEBP, HEIC files up to 200MB. casual creators and social media users use it for turning a set of photos into a shareable video — processing takes 30-60 seconds on cloud GPUs and you get 1080p MP4 files.
...
- "turning a set of photos into a shareable video for casual creators and social media users"
...
# Free Photo Video — Turn Photos Into Shareable Videos
...
Worth noting: fewer photos with longer durations per slide tend to look more polished than rapid-fire cuts.

## Matching Input to Actions

User prompts referencing free photo video, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `free-photo-video`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
