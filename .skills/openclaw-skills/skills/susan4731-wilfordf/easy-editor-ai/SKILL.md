...
name: easy-editor-ai
version: "1.0.0"
displayName: "Easy Editor AI — Edit and Export Videos Fast"
description: >
  content creators edit raw video clips into edited MP4 clips using this skill. Accepts MP4, MOV, AVI, WebM up to 500MB, renders on cloud GPUs at 1080p, and returns MP4 files in 1-2 minutes. Designed for quickly editing raw footage without manual timeline work.
...
- "quickly editing raw footage without manual timeline work for content creators"
...
# Easy Editor AI — Edit and Export Videos Fast
...
User prompts referencing easy editor ai, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `easy-editor-ai`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
