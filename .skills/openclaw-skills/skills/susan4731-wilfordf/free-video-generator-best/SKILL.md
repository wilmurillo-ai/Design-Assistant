---
name: free-video-generator-best
version: "1.0.0"
displayName: "Free Video Generator Best — Generate Videos from Text Free"
description: >
  Get ready-to-share videos ready to post, without touching a single slider. Upload your text prompts or clips (MP4, MOV, PNG, JPG, up to 500MB), say something like "generate a 30-second promotional video from my product description", and download 1080p MP4 when it's done. Built for content creators and marketers who move fast and want to create videos quickly without expensive software or design experience.
...
# Free Video Generator Best — Generate Videos from Text Free
...
Say you have a short text prompt describing a product demo scene and want to generate a 30-second promotional video from my product description — the backend processes it in about 1-2 minutes and hands you a 1080p MP4.

Tip: shorter, specific prompts produce more accurate and faster results.

## Matching Input to Actions

User prompts referencing free video generator best, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `free-video-generator-best`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
