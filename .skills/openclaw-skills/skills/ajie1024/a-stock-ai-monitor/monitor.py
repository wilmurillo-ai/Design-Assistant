...
WATCHLIST = [
    ...
    # 卖出 5000 股 @ 13 元后，剩余 16900 股
    {
        "code": "002459", 
        "name": "晶澳科技", 
        "market": "sz",
        "type": "individual",
        "cost": 16.275,                 # 持仓成本（2026-03-12 更新）
        "shares": 16900,                # 持仓数量
        "alerts": {
            "cost_pct_above": 25.0,    # 盈利25%提醒 (目标价 20.34)
            "cost_pct_below": -20.0,   # 止损20% (止损价 13.02)
            "change_pct_above": 3.0,   # 日内异动 ±3%
            "change_pct_below": -3.0,
            "volume_surge": 2.0,       # 成交量是5日均量2倍
            "price_above": 15.00,      # 突破15元加仓信号
            "price_below": 13.00       # 跌破13元风险预警
        }
    }
]
...
class StockAlert:
    ...
    def should_run_now(self):
        ...
        # WSL 已经是北京时间(CST)，无需转换
        from datetime import timedelta
        now = datetime.now()  # 已经是北京时间
...
if __name__ == '__main__':
    import sys
    import io
    # 设置 stdout 为 UTF-8 编码，避免 Windows 控制台编码问题
