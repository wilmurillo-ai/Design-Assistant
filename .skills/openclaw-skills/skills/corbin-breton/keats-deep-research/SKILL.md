...
- Two full research cycles per theme (with optional extension — see Adaptive Depth below)
...
#### Adaptive Depth: Budget-Gated Extension Cycles

The rigid 2-cycle minimum is a floor, not a ceiling. After completing both mandatory cycles for a theme, assess whether gaps remain:

1. **Gap assessment:** Are there significant unanswered questions, contradictions without resolution, or key entities mentioned but not investigated?
2. **Budget check:** Has the research consumed less than 70% of available context/time budget?
3. **Diminishing returns test:** Did Cycle 2 produce meaningful new findings beyond Cycle 1? (If Cycle 2 only confirmed Cycle 1 with no new insight, further cycles are unlikely to help.)

**If gaps remain AND budget allows AND Cycle 2 was productive:** run extension cycles (up to 2 additional, for 4 total maximum). Each extension cycle must target a specific identified gap — no broad re-searching.

**If any condition fails:** stop and document remaining gaps in the final report's Limitations section.

---

#### Topological Entity Expansion

After initial search results in each cycle, extract key entities — researchers, institutions, specific papers, tools, frameworks, or concepts — that appear across multiple sources:

1. **Entity extraction:** From Cycle 1 results, identify the 3-5 most-referenced entities not yet directly investigated.
2. **Targeted follow-up:** In Cycle 2 (or extension cycles), search specifically for those entities to gather primary-source depth. Use `web_fetch` on entity-specific URLs (author pages, project sites, original papers).
3. **Hop limit:** Follow entity links at most 1-2 hops from the original search results. Do not recursively expand — this prevents runaway scope.
4. **Synthesis:** Cross-reference entity-specific findings back to the theme's central questions.

This technique surfaces depth that broad keyword searches miss — the difference between finding "X is widely used" and understanding who built X, why, what its limitations are, and what the primary authors say about its future.
