...
async function cmdPair(args) {
  ...
  if (args.sendNetwork) params += `&sendNetwork=${args.sendNetwork}`;
  if (args.receiveNetwork) params += `&receiveNetwork=${args.receiveNetwork}`;
...
async function cmdOrder(args) {
  ...
  if (args.sendNetwork) orderData.sendNetwork = args.sendNetwork;
  if (args.receiveNetwork) orderData.receiveNetwork = args.receiveNetwork;
  ...
  if (!args.yes) {
    ...
    console.log(`To Address: ${args.address}`);
...
async function cmdUI(args) {
  ...
  server.listen(port, () => {
    ...
    console.log(`🚀 Crypto Exchange UI running at ${url}`);
    console.log('Press Ctrl+C to stop');
  });
...
function showHelp() {
  console.log(`
LightningEX Crypto Exchange CLI

Usage: crypto-exchange [command] [options]
    ...
    switch (args.command) {
      ...
      case 'pair':
        if (!args.options.send || !args.options.receive) {
          console.log('Usage: crypto-exchange pair --send <currency> --receive <currency>');
      ...
      case 'rate':
        if (!args.options.send || !args.options.receive || !args.options.amount) {
          console.log('Usage: crypto-exchange rate --send <currency> --receive <currency> --amount <amount>');
          console.log('       [--send-network <network>] [--receive-network <network>]');
          return;
        }
      ...
      case 'validate':
        if (!args.options.currency || !args.options.address) {
          console.log('Usage: crypto-exchange validate --currency <currency> --address <address>');
        ...
        break;
      case 'order':
        if (!args.options.send || !args.options.receive || !args.options.amount || !args.options.address) {
          console.log('Usage: crypto-exchange order --send <currency> --receive <currency>');
          console.log('       --amount <amount> --address <address>');
          console.log('       [--send-network <network>] [--receive-network <network>] [--yes]');
          return;
        }
        await cmdOrder(args.options);
        break;
      case 'status':
        if (!args.options.id) {
          console.log('Usage: crypto-exchange status --id <order-id>');
          return;
        }
      ...
      case 'monitor':
        if (!args.options.id) {
          console.log('Usage: crypto-exchange monitor --id <order-id>');
          return;
        }
