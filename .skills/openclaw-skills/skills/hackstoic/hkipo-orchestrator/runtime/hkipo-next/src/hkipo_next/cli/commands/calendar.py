"""Calendar/discovery command handler."""
...
from hkipo_next.adapters.legacy_discovery import LegacyDiscoveryAdapter
...
from hkipo_next.renderers.text_renderer import render_discovery_response, render_error_response
from hkipo_next.services.discovery_service import DiscoveryService
...
def execute(
    ...
    *,
    ...
    service = DiscoveryService(adapter=adapter or LegacyDiscoveryAdapter())

    try:
        result = service.discover(window=args.window, days=args.days)
...
def _render_success(response: DiscoveryResponse, output_format: str) -> str:
    if output_format == "json":
        return render_model_as_json(response)
    if output_format == "markdown":
        return render_discovery_markdown(response)
    return render_discovery_response(response)
