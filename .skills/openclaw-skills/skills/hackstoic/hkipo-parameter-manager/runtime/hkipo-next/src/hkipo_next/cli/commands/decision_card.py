...
from hkipo_next.contracts.decision_card import DecisionCardResponse
...
from hkipo_next.renderers.markdown_renderer import render_decision_card_markdown, render_error_markdown
from hkipo_next.renderers.text_renderer import render_decision_card_response, render_error_response
...
def execute(
    ...
    *,
    ...
    try:
        ...
        card = DecisionService(
            ScoringService(SnapshotService(adapter=LegacySnapshotAdapter()))
        ).build_for_symbol(
            symbol=args.symbol,
            parameter_version=parameter_version,
            profile=loaded_profile.to_view(),
        )
        response = DecisionCardResponse(
            data=card,
            meta=run_context.meta(
                degraded=card.source_issue_count > 0,
                data_status="partial" if card.source_issue_count > 0 else "complete",
            ),
        )
...
def _profile_overrides(args: Namespace) -> dict[str, object]:
    return {
        ...
        for field_name in PROFILE_DECISION_FIELDS
    }
...
def _persist_review_record(response: DecisionCardResponse, runtime_paths) -> ReviewRecord:
    try:
        return _build_review_service(runtime_paths).record_decision_card_response(response)
    except Exception as exc:  # pragma: no cover - exercised via CLI path
        raise AppException(
            AppError.system(
                "decision-card 结果已生成，但历史记录写入失败。",
                details={"reason": str(exc)},
            )
        ) from exc


def _render_success(response: DecisionCardResponse, output_format: str) -> str:
    if output_format == "json":
        return render_model_as_json(response)
    if output_format == "markdown":
        return render_decision_card_markdown(response)
    return render_decision_card_response(response)
