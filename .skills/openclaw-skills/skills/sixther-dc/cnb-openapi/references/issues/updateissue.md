...
## 原始 Swagger
https://api.cnb.cool/#/operations/UpdateIssue
...
**请求地址：** ${CNB_API_ENDPOINT}/{repo}/-/issues/{number}
...
{
  "body": "string", // Issue内容。
  "end_date": "string", // Issue结束日期。示例：`2026-12-30`
  "priority": "string", // Issue优先级。可选值：`-2P`,`-1P`,`P0`,`P1`,`P2`,`P3`
  "start_date": "string", // Issue开始日期。示例：`2026-01-01`
  "state": "string", // Issue状态。可选值：`open`,`closed`
  "state_reason": "string", // ISSUE状态变更原因。可选值：`completed`,`not_planned`,`reopened`
...
```bash
curl -X PATCH \
  "${CNB_API_ENDPOINT}/{repo}/-/issues/{number}" \
