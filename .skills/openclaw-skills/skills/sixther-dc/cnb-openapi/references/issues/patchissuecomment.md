...
## 原始 Swagger
https://api.cnb.cool/#/operations/PatchIssueComment
...
**请求地址：** ${CNB_API_ENDPOINT}/{repo}/-/issues/{number}/comments/{comment_id}
...
| comment_id | 字符串 | 是 | Issue评论唯一标识编号。|
...
```bash
curl -X PATCH \
  "${CNB_API_ENDPOINT}/{repo}/-/issues/{number}/comments/{comment_id}" \
