...
## 原始 Swagger
https://api.cnb.cool/#/operations/ListPackageTags
...
**请求地址：** ${CNB_API_ENDPOINT}/{slug}/-/packages/{type}/{name}/-/tags
...
| ordering | 字符串 | 否| 顺序。Ordering type.|
| tag_name | 字符串 | 否| 关键字。Key word to search tag name.|
## 响应信息


**响应类型：** git_woa_com_cnb_monorepo_platform_service_api_internal_models_artifactory_dto.Tag
...
```bash
curl -X GET \
  "${CNB_API_ENDPOINT}/{slug}/-/packages/{type}/{name}/-/tags" \
