...
## 原始 Swagger
https://api.cnb.cool/#/operations/GetUserStaredRepos
...
**请求地址：** ${CNB_API_ENDPOINT}/users/{username}/stared-repos
...
| 参数名 | 类型 | 必填 | 描述 |
|--------|------|------|------|
| search | 字符串 | 否| 过滤仓库。Filter by repositories|
...
```bash
curl -X GET \
  "${CNB_API_ENDPOINT}/users/{username}/stared-repos" \
