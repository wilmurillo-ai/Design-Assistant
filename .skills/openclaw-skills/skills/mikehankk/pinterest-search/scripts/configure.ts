#!/usr/bin/env bun
/**
 * Pinterest Search 配置工具
 * 用于输出环境变量设置命令、清理 Cache 等
 */

import { clearCache } from './pinterest_search';
import * as fs from 'fs';
import * as path from 'path';

// 当前环境变量值
function showCookie(): void {
    const cookie = process.env.PINTEREST_COOKIE;
    if (cookie) {
        console.log(`当前 Cookie: ${cookie.substring(0, 50)}...`);
    } else {
        console.log('未设置 PINTEREST_COOKIE 环境变量');
    }
}
...
// 输出设置命令
function printCookieCommand(cookie: string): void {
    console.log('\n请复制以下命令设置 Cookie:');
    console.log('\x1b[32m%s\x1b[0m', `export PINTEREST_COOKIE="${cookie}"`);
}
...
命令:
  cookie [value]          输出设置 Cookie 的环境变量命令
  cookie --show           查看当前环境变量中的 Cookie
...
示例:
  bun run configure.ts cookie "_auth=xxx;_pinterest_sess=yyy"
  bun run configure.ts proxy http://127.0.0.1:7890
  bun run configure.ts clearcache "中国女人"
  bun run configure.ts clearcache --all
  bun run configure.ts listcache

注意:
  Cookie 和 Proxy 只从环境变量读取，不会保存到文件。
    ...
    switch (command) {
        case 'cookie':
            if (args[1] === '--show') {
                ...
                printCookieCommand(args[1]);
            } else {
                console.log('请提供 Cookie 值或使用 --show 选项');
            }
            break;
