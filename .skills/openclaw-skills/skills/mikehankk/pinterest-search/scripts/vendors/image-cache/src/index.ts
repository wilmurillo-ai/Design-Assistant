...
function getCacheDir(customDir?: string): string {
  // 1. 优先使用传入的自定义目录
  if (customDir) {
...
// 用于外部设置缓存目录的函数
export function setCacheDir(customDir: string): void {
  CACHE_DIR = getCacheDir(customDir);
}

// 获取当前缓存目录
export function getCurrentCacheDir(): string {
  return CACHE_DIR;
}
...
async function downloadImage(url: string, proxy?: string, retries = 2): Promise<Buffer> {
  try {
    ...
    if (retries > 0) {
      return downloadImage(url, proxy, retries - 1);
    }
    throw err;
  }
}

// ====== Cache Lookup (isolated from network input) ======

async function lookupCache(localPath: string): Promise<CacheImageResult | null> {
  ...
  return {
    ...
    fromCache: true,
    hash: path.basename(localPath, '.jpg'),
  };
}
...
export async function cacheImage(
  ...
  // ====== Cache Hit ======
  // lookupCache is isolated from the url variable
  const cached = await lookupCache(localPath);
  if (cached) {
    return cached;
  }

  // ====== Download ======
  const buffer = await downloadImage(url, options?.proxy);
