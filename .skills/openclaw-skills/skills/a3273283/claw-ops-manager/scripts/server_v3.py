...
class AuditAPIHandler(SimpleHTTPRequestHandler):
    ...
    def serve_main_app(self):
        ...
        self.end_headers()
        try:
            self.wfile.write(self.get_full_app_html_v3().encode('utf-8'))
        except:
            try:
    ...
    def get_full_app_html_v3(self):
        """加载 v3 multilingual dashboard"""
        dashboard_path = Path(__file__).parent.parent / "assets" / "templates" / "dashboard_v3.html"
...
<body>
    ...
    def handle_api_get(self, parsed):
        ...
        if path == '/api/stats':
            ...
            self.send_json(self.get_operations(limit, lang))
            ...
            self.send_json(self.get_supported_languages())
        else:
            self.send_error(404)
    ...
    def get_operations(self, limit=50, lang='en'):
        conn = self.get_db_connection()
        cursor = conn.cursor()

        cursor.execute("""
            SELECT id, timestamp, tool_name, action, success, duration_ms,
                   friendly_name, snapshot_id, user, parameters
            FROM operations
        ...
        for row in cursor.fetchall():
            op = dict(row)
            # 如果需要重新生成描述（支持多语言）
            if lang != 'en' and op['parameters']:
                try:
                    import json
                    from describer import OperationDescriber
                    parameters = json.loads(op['parameters'])
                    op['friendly_name'] = OperationDescriber.describe(
                        op['tool_name'],
                        op['action'],
                        parameters,
                        lang=lang
...
if __name__ == '__main__':
    ...
    if not DB_PATH.exists():
        print("⚠️  Database not found. Run: python3 scripts/init.py")
        exit(1)

    server = HTTPServer(('0.0.0.0', 8080), AuditAPIHandler)
    print("🚀 Claw Operations Manager v3.0 Started!")
    print(f"📊 Dashboard: http://localhost:8080")
    print(f"📱 LAN access: http://<your-ip>:8080")
    print()
    print("✨ New Features:")
    print("  ✅ Multilingual support (6 languages)")
    print("  ✅ Intelligent operation descriptions")
    print("  ✅ Auto-snapshots with git")
    print("  ✅ One-click rollback")
    print("  ✅ Real-time statistics")
    print()
    print("🌐 Supported Languages:")
    print("  🇺🇸 English  🇨🇳 简体中文  🇯🇵 日本語")
    print("  🇪🇸 Español  🇫🇷 Français  🇩🇪 Deutsch")
    print()
