...
EPM_BASE_URL = "https://www.ebi.ac.uk/europepmc/webservices/rest"
...
def search_epmc(query, limit=10):
    """Search Europe PMC for articles with full text, citations, and date sorting."""
    try:
        url = f"{EPM_BASE_URL}/search"
        params = {
            "query": query,
            "resulttype": "lite",
            "format": "json",
        ...
        for item in data.get("resultList", {}).get("result", []):
            ...
            results.append({
                ...
                "abstract": None,  # Europe PMC lite doesn't include abstract
                "venue": item.get("journalTitle"),
                "citationCount": item.get("citedByCount"),
                "isOpenAccess": item.get("isOpenAccess") == "Y",
                "hasFullText": item.get("hasPDF") == "Y" or item.get("inPMC") == "Y",
                "source": "europe_pmc"
            })
        return {"total": data.get("hitCount"), "data": results}
    except requests.exceptions.Timeout:
        return {"error": "Request timed out", "data": []}
    except Exception as e:
        return {"error": str(e), "data": []}
...
def main():
    ...
    search_parser.add_argument("--source", choices=["s2", "oa", "cr", "pm", "epmc", "both", "all"], default="oa", 
                               help="Source: s2 (Semantic Scholar), oa (OpenAlex), cr (Crossref), pm (PubMed), epmc (Europe PMC)")
    ...
    if args.command == "search":
        ...
        if args.source in ["epmc", "both", "all"]: output["europe_pmc"] = search_epmc(args.query, args.limit)
