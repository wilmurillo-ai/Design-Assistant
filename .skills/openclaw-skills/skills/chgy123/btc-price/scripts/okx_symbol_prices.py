...
def _get_json(session: requests.Session, url: str, timeout_s: float) -> Dict[str, Any]:
    r = session.get(
        ...
        timeout=timeout_s,
        headers={
            "User-Agent": "okx-crypto-price/1.0",
...
def main() -> int:
    ...
    with requests.Session() as s:
        results: List[Tuple[str, Optional[Ticker], Optional[str]]] = []
        for base in symbols:
            ...
            for quote in ("USDT", "USDC", "USD"):
                ...
                if t is not None:
                    picked = t
                    break
            results.append((base, picked, last_err))

        if args.format == "json":
            payload: List[Dict[str, Any]] = []
            ts = _now_local_str()
            for base, t, err in results:
                if t is None:
                    payload.append(
                        {
                            "symbol": base,
        ...
        for base, t, err in results:
            if t is None:
                ...
                print(f"- {base}: N/A (no USDT/USDC/USD spot ticker found on OKX){suffix}")
            else:
