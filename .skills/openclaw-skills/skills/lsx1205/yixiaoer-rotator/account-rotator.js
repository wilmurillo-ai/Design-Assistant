...
function callYixiaoerAPI(payload) {
  const scriptPath = path.join(__dirname, '../yixiaoer-skill/scripts/api.ts');
  ...
  const payloadFile = path.join(__dirname, '.tmp-payload.json');
  
  try {
    // 写入 payload 到临时文件
    fs.writeFileSync(payloadFile, JSON.stringify(payload));
    
    // 通过 execSync 的 env 选项传递 API Key，避免 shell 注入
    const result = execSync(`node "${scriptPath}" --payload="$(cat '${payloadFile}')"`, {
      encoding: 'utf-8',
      shell: true,
      env: {
        ...process.env,
