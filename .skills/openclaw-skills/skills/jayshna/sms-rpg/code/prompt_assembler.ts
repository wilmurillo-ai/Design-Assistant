...
import { EpisodicMemoryAssembler } from './memory_assembler';
import { 
  WorldStateInput, 
  WorldContext, 
  PlayerState, 
...
export class PromptAssembler {
  private genesisEnginePrompt: string;
  private memoryAssembler: EpisodicMemoryAssembler;
  private currentTurnNumber: number = 1;
  
  constructor(genesisPrompt: string) {
    this.genesisEnginePrompt = genesisPrompt;
    this.memoryAssembler = new EpisodicMemoryAssembler({
  ...
  assemble(worldState: WorldStateInput): AssembledPrompt {
    // 1. 组装记忆层
    const memoryContext = this.memoryAssembler.assembleContext(
  ...
  private formatWorldContext(ctx: WorldContext): string {
    return `\n世界概况：\n` +
  ...
  private formatPlayerState(player: PlayerState): string {
  ...
  private formatLocationState(loc: LocationState): string {
    return `\n地点状态【${loc.name}】：\n` +
           `- 描述：${loc.description}\n` +
  ...
  private formatActiveNPCs(npcs: NPCState[]): string {
  ...
  private formatSystemRules(): string {
    return `\n系统硬规则：\n` +
  ...
  private formatActiveQuests(quests: QuestState[]): string {
    if (quests.length === 0) return "\n当前任务：无\n";
    
    const lines = quests.map(q => 
      `- [${q.status}] ${q.title}：${q.objective}`
    );
  ...
  private estimateTotalTokens(system: string, user: string): number {
    // 包含消息格式的开销
    const overhead = 10;
...
export type CanonCheckFn = (output: GenesisOutput) => Promise<{ verdict: "approved" | "rejected" | "needs_review"; violations?: unknown[] }>;
...
export async function generateTurn(
  ...
  // 解析并验证
  const output: GenesisOutput = JSON.parse(response.content);
...
export function createPromptAssembler(genesisPrompt: string): PromptAssembler {
