...
exports.loadGenesisPrompt = loadGenesisPrompt;
exports.getKimiClientConfigNonInteractive = getKimiClientConfigNonInteractive;
exports.hasConfiguredKimiApiKey = hasConfiguredKimiApiKey;
exports.isConfiguredKimiApiKeyUsable = isConfiguredKimiApiKeyUsable;
exports.configureKimiApiKey = configureKimiApiKey;
exports.getKimiClientConfig = getKimiClientConfig;
exports.kimiChatJson = kimiChatJson;
...
const APP_CONFIG_PATHS = (0, runtime_paths_1.getDataRootCandidates)().map(root => (0, node_path_1.join)(root, "config.json"));
async function loadPromptFile(filePath) {
    return await (0, promises_1.readFile)((0, runtime_paths_1.resolvePackagePath)(filePath), 'utf-8');
}
async function loadGenesisPrompt() {
    try {
        return await (0, promises_1.readFile)((0, runtime_paths_1.getPromptPath)('genesis_engine_prompt.md'), 'utf-8');
    }
    catch {
        return `# 【创世者协议 v1.0】
你是一位古老的创世者——"墨言"...
...
async function loadAppConfig() {
    for (const configPath of APP_CONFIG_PATHS) {
        try {
            ...
            if (!(0, world_state_utils_1.isRecord)(parsed)) {
                continue;
            }
            return {
                ...
                timeoutMs: (0, save_store_1.toPositiveInteger)(parsed.timeoutMs, DEFAULT_TIMEOUT_MS),
                maxTokens: (0, save_store_1.toPositiveInteger)(parsed.maxTokens, DEFAULT_MAX_TOKENS),
...
async function validateKimiApiKey(apiKey, baseUrl, model, timeoutMs) {
...
function resolveBaseConfig(savedConfig) {
    ...
    const timeoutMs = (0, save_store_1.toPositiveInteger)(process.env.MOONSHOT_TIMEOUT_MS, savedConfig.timeoutMs || DEFAULT_TIMEOUT_MS);
    const maxTokens = (0, save_store_1.toPositiveInteger)(process.env.MOONSHOT_MAX_TOKENS, savedConfig.maxTokens || DEFAULT_MAX_TOKENS);
    const enableCanonGuardian = parseBooleanFlag(process.env.MOONSHOT_ENABLE_CANON_GUARDIAN, typeof savedConfig.enableCanonGuardian === "boolean"
...
async function getKimiClientConfigNonInteractive() {
...
async function hasConfiguredKimiApiKey() {
    const savedConfig = await loadAppConfig();
    return Boolean(process.env.MOONSHOT_API_KEY?.trim() || savedConfig.apiKey);
}
async function isConfiguredKimiApiKeyUsable() {
    const savedConfig = await loadAppConfig();
...
async function configureKimiApiKey(apiKey) {
...
async function getKimiClientConfig() {
    const savedConfig = await loadAppConfig();
    const { baseUrl, model, timeoutMs, maxTokens, enableCanonGuardian } = resolveBaseConfig(savedConfig);
...
async function kimiChatJson(client, systemPrompt, userContent, maxTokensOverride) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
            ...
            if (!response.ok) {
                const errorText = await response.text();
                if (response.status === 401) {
                    throw new Error(`Kimi API 认证失败(401): ${response.statusText}\n` +
