...
exports.CUSTOM_ACTION_OPTION = void 0;
exports.ensureSuggestedOptions = ensureSuggestedOptions;
exports.ensureActionOptions = ensureActionOptions;
exports.isRecord = isRecord;
exports.toDisplayString = toDisplayString;
exports.normalizeDisplayStrings = normalizeDisplayStrings;
exports.uniqueStrings = uniqueStrings;
exports.clampNumber = clampNumber;
exports.previewJson = previewJson;
exports.normalizeWorldState = normalizeWorldState;
exports.formatCurrentStateSummary = formatCurrentStateSummary;
exports.updateWorldState = updateWorldState;
exports.CUSTOM_ACTION_OPTION = {
    type: "action",
    description: "自定义行动（告诉我你想做什么）"
};
function ensureSuggestedOptions(options) {
...
function ensureActionOptions(options) {
    return [...ensureSuggestedOptions(options), exports.CUSTOM_ACTION_OPTION];
}
...
function normalizeDisplayStrings(values) {
...
function normalizeInventoryItemKey(value) {
    return value
        .replace(/（[^）]*）/g, "")
        .replace(/\([^)]*\)/g, "")
        .replace(/\s+/g, "")
...
function mergeInventoryItems(values) {
    const orderedKeys = [];
...
function normalizeWorldState(state, playerName) {
    return {
        world_context: {
...
function formatCurrentStateSummary(state) {
...
function updateWorldState(state, output) {
...
function applyPlayerUpdates(state, output) {
...
function applyLocationUpdates(state, output) {
    if (output.state_changes.new_locations?.length) {
        ...
        state.location_state.connected_locations = Array.from(new Set([...state.location_state.connected_locations, ...newLocationIds]));
    }
    ...
    for (const updated of output.state_changes.updated_locations) {
        ...
        if (typeof changes.name === "string")
            state.location_state.name = changes.name;
        if (typeof changes.description === "string")
            state.location_state.description = changes.description;
        if (Array.isArray(changes.connected_locations)) {
            state.location_state.connected_locations = changes.connected_locations.filter((item) => typeof item === "string");
        }
        if (Array.isArray(changes.present_npcs)) {
            state.location_state.present_npcs = changes.present_npcs.filter((item) => typeof item === "string");
        }
...
function applyNpcUpdates(state, output) {
...
function createNewNpcs(state, output) {
...
function applyUpdatedNpcSnapshots(state, output) {
    ...
    for (const updated of output.state_changes.updated_npcs) {
        ...
        if (typeof changes.short_description === "string")
            target.short_description = changes.short_description;
        if (typeof changes.description === "string")
            target.short_description = changes.description;
        if (typeof changes.current_status === "string")
            target.current_status = changes.current_status;
        if (typeof changes.hp === "number")
            target.hp = changes.hp;
        if (typeof changes.max_hp === "number")
            target.max_hp = changes.max_hp;
        if (typeof changes.mp === "number")
            target.mp = changes.mp;
        if (typeof changes.max_mp === "number")
            target.max_mp = changes.max_mp;
        if (typeof changes.attitude_to_player === "number")
            target.attitude_to_player = changes.attitude_to_player;
        if (Array.isArray(changes.known_secrets)) {
            target.known_secrets = normalizeDisplayStrings(changes.known_secrets);
        }
        if (typeof changes.last_interaction === "string")
            target.last_interaction = changes.last_interaction;
    }
}
function applyNpcDeltaUpdates(state, output) {
...
function applyNpcDeltaUpdate(target, update) {
...
function applyNewItems(state, output) {
...
function applyQuestUpdates(state, output) {
    ...
    for (const updated of output.state_changes.updated_quests) {
        ...
        if (updated.progress)
            target.objective = updated.progress;
        if (updated.new_objectives?.length)
            target.objective = updated.new_objectives[0];
    }
}
function applyWorldMemoryUpdates(state, output) {
    ...
    state.world_memory.recent_events.push(`第${state.world_memory.recent_events.length + 1}回合：${output.narrative.substring(0, 40)}`);
