...
exports.exampleGenesisOutput = exports.exampleWorldState = void 0;
exports.exampleGameFlow = exampleGameFlow;
exports.exampleWithErrorHandling = exampleWithErrorHandling;
exports.exampleMemorySystem = exampleMemorySystem;
...
async function exampleGameFlow() {
    const client = await (0, kimi_client_1.getKimiClientConfig)();
    const genesisPrompt = await (0, kimi_client_1.loadGenesisPrompt)();
    const canonPrompt = await (0, kimi_client_1.loadPromptFile)('prompts/canon_guardian_prompt.md');
    const episodePrompt = await (0, kimi_client_1.loadPromptFile)('prompts/episode_summary_prompt.md');
    const assembler = (0, prompt_assembler_1.createPromptAssembler)(genesisPrompt);
    let currentTurn = 1;
    const maxTurns = 10;
    const allTurnRecords = [];
    let previousSummary = "";
    while (currentTurn <= maxTurns) {
        console.log(`\n========== 第 ${currentTurn} 回合 ==========`);
        // 1. 组装 Prompt
        const prompt = assembler.assemble(exports.exampleWorldState);
        console.log(`预估 Token: ${prompt.estimatedTokens}`);
        const outputObj = await (0, kimi_client_1.kimiChatJson)(client, prompt.systemPrompt, prompt.userMessage);
        if (client.enableCanonGuardian) {
            const canonResult = await (0, world_generation_1.runCanonGuardianCheck)(client, canonPrompt, exports.exampleWorldState, outputObj);
        ...
        (0, world_state_utils_1.updateWorldState)(exports.exampleWorldState, outputObj);
        const record = {
            turnNumber: currentTurn,
            timestamp: Date.now(),
            playerInput: exports.exampleWorldState.player_input,
        ...
        if (currentTurn % 5 === 0 && allTurnRecords.length >= 5) {
            ...
            const quests = exports.exampleWorldState.active_quests.filter(q => q.status === "active").map(q => q.title);
            const summary = await (0, world_generation_1.generateEpisodeSummary)(client, episodePrompt, currentTurn - 1, last5, previousSummary, quests);
...
async function exampleWithErrorHandling() {
    const assembler = (0, prompt_assembler_1.createPromptAssembler)(await (0, kimi_client_1.loadGenesisPrompt)());
    try {
        const prompt = assembler.assemble(exports.exampleWorldState);
        const client = await (0, kimi_client_1.getKimiClientConfig)();
        const output = await (0, kimi_client_1.kimiChatJson)(client, prompt.systemPrompt, prompt.userMessage);
        if (client.enableCanonGuardian) {
            const canonPrompt = await (0, kimi_client_1.loadPromptFile)('prompts/canon_guardian_prompt.md');
            const canonResult = await (0, world_generation_1.runCanonGuardianCheck)(client, canonPrompt, exports.exampleWorldState, output);
            if (canonResult.verdict === "rejected") {
                throw new prompt_assembler_1.CanonViolationError(canonResult.violations || []);
            }
        }
        return output;
    }
    catch (error) {
        if (error instanceof prompt_assembler_1.CanonViolationError) {
            console.error("正典违规:");
...
// 示例5：记忆系统使用
// ============================================================
function exampleMemorySystem() {
    const memoryAssembler = (0, memory_assembler_1.createMemoryAssembler)();
    // 添加回合记录
...
function displayLicenseBanner(status) {
    const versionLabel = status.isFullVersion ? "完整版" : "免费版";
    const slotCount = (0, license_service_1.getAccessibleSaveSlotCount)(status);
    console.log(`\n[授权] 当前为${versionLabel}，可用存档槽：${slotCount}`);
    if (!status.isFullVersion) {
        console.log("[授权] 输入“解锁”查看购买方式，输入“授权状态”查看当前授权。");
    }
}
function buildTurnPrompt(assembler, worldState, licenseStatus) {
...
async function handleRuntimeCommand(commandInput) {
    if (commandInput.command === "license_status") {
        const status = await (0, license_service_1.getLicenseStatus)();
        console.log(`\n${(0, license_service_1.formatLicenseStatus)(status)}`);
        return status;
    }
    if (!commandInput.code) {
        console.log(`\n${(0, license_service_1.formatUnlockInstructions)()}`);
        return (0, license_service_1.getLicenseStatus)();
    }
    const identity = (0, device_identity_1.getMachineIdentity)();
    const result = await (0, license_service_1.activateLicenseCode)(commandInput.code, identity.machineId);
    console.log(`\n[授权] ${result.message}`);
    if (result.ok) {
...
async function resolveTurnActionInput(rl, currentTurn, options, licenseStatus) {
    let currentStatus = licenseStatus;
    while (true) {
        const result = await (0, cli_helpers_1.promptTurnInput)(rl, currentTurn, options);
...
async function initializeGameRuntime(rl, client, assembler, licenseStatus) {
    const slotSelection = await (0, cli_helpers_1.chooseSaveSlot)(rl, (0, license_service_1.getAccessibleSaveSlotCount)(licenseStatus));
    if (!slotSelection) {
        return null;
    }
...
async function createNewGameRuntime(rl, client, slotId, licenseStatus) {
    const playerName = await (0, cli_helpers_1.promptPlayerName)(rl);
    const worldRequirement = await (0, cli_helpers_1.promptWorldRequirement)(rl);
    console.log("\n[正在生成初始世界，请稍候...]");
    const initialWorld = await (0, world_generation_1.generateInitialWorld)(client, worldRequirement, playerName);
    const currentOptions = (0, world_state_utils_1.ensureActionOptions)(initialWorld.player_options);
    console.log("\n【世界设定已生成】");
    console.log(initialWorld.story_overview);
    console.log("\n【开场引子】");
    console.log(initialWorld.opening_narrative);
    console.log("\n【当前状态】");
    console.log((0, world_state_utils_1.formatCurrentStateSummary)(initialWorld.world_state));
    console.log("\n【开局选择】");
    (0, cli_helpers_1.displayActionOptions)(currentOptions);
...
async function resumeGameRuntime(rl, assembler, slotId, loadedSave, licenseStatus) {
    const currentOptions = (0, world_state_utils_1.ensureActionOptions)(loadedSave.currentOptions || []);
    ...
    console.log("\n【当前状态】");
    console.log((0, world_state_utils_1.formatCurrentStateSummary)(loadedSave.worldState));
    console.log("\n【可选行动】");
    (0, cli_helpers_1.displayActionOptions)(currentOptions);
...
async function generateApprovedTurnOutput(client, canonPrompt, prompt, currentWorldState) {
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        ...
        (canonResult.violations || []).forEach((violation) => console.log(JSON.stringify(violation)));
        outputObj = null;
    }
...
async function persistRuntimeState(runtime) {
    const saveData = {
        version: "2.0",
    ...
    const savedFilePath = await (0, save_store_1.saveGame)(runtime.currentSlotId, saveData);
    if (savedFilePath) {
        console.log(`[系统] 游戏进度已自动保存至槽位：${runtime.currentSlotId}`);
    }
}
function applyNextTurnInput(runtime, nextTurnInput) {
...
async function runInteractiveGame() {
    ...
    const canonPrompt = client.enableCanonGuardian
        ? await (0, kimi_client_1.loadPromptFile)('prompts/canon_guardian_prompt.md')
        : "";
    const episodePrompt = await (0, kimi_client_1.loadPromptFile)('prompts/episode_summary_prompt.md');
    const assembler = (0, prompt_assembler_1.createPromptAssembler)(genesisPrompt);
    const rl = (0, promises_1.createInterface)({ input: node_process_1.stdin, output: node_process_1.stdout });
    try {
        const initialLicenseStatus = await (0, license_service_1.getLicenseStatus)();
        ...
        while (true) {
            ...
            console.log("\n【当前状态】");
            console.log((0, world_state_utils_1.formatCurrentStateSummary)(runtime.currentWorldState));
            console.log("\n【可选行动】");
            runtime.currentOptions = (0, world_state_utils_1.ensureActionOptions)(outputObj.player_options);
            (0, cli_helpers_1.displayActionOptions)(runtime.currentOptions);
