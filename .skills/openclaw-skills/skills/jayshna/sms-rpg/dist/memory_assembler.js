...
exports.EpisodicMemoryAssembler = void 0;
exports.createMemoryAssembler = createMemoryAssembler;
// ============================================================
// EpisodicMemoryAssembler 类
// ============================================================
class EpisodicMemoryAssembler {
    constructor(config) {
        this.recentMemory = [];
    ...
    // 核心组装方法
    // ============================================================
    assembleContext(currentTurn, worldState) {
    ...
    // 中景组装（剧情摘要）
    // ============================================================
    assembleEpisodeSummaries(budget) {
        // 按时间倒序，最新的摘要最详细
    ...
    // 远景组装（世界观时间线）
    // ============================================================
    assembleWorldTimeline(budget, retrievedTimelineEvents) {
    ...
    summarizeChanges(changes) {
        const parts = [];
        if (changes.new_npcs?.length)
            parts.push(`新增NPC：${changes.new_npcs.map(n => n.name).join(", ")}`);
        if (changes.new_locations?.length)
            parts.push(`发现地点：${changes.new_locations.map(l => l.name).join(", ")}`);
        if (changes.updated_npcs?.length)
            parts.push(`NPC状态变化：${changes.updated_npcs.length}人`);
        if (changes.world_events?.length)
            parts.push(`事件：${changes.world_events.map(e => e.description).join("; ")}`);
    ...
    getMajorFactions() {
        return this.factions.filter(f => f.playerRelation !== 0 ||
            this.worldTimeline.some(e => e.affectedFactions.includes(f.id)));
    }
    calculateCoverage(currentTurn) {
        const candidates = [
    ...
    buildMemoryQuery(currentTurn, state) {
    ...
    retrieveRelevantTimelineEvents(query, limit) {
    ...
    scoreTimelineEvent(event, query) {
        let relevance = 0;
...
exports.EpisodicMemoryAssembler = EpisodicMemoryAssembler;
// ============================================================
// 使用示例
// ============================================================
function createMemoryAssembler() {
