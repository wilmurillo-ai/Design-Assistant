...
if ./run.sh summary 2>&1 | grep -q '"ok": true'; then
    echo "   ✅ Success"
else
    echo "   ❌ Failed"
    exit 1
...
if ./run.sh tools --limit 2 2>&1 | grep -q '"ok": true'; then
    echo "   ✅ Success"
else
    echo "   ❌ Failed"
    exit 1
...
if ./run.sh commands --limit 2 2>&1 | grep -q '"ok": true'; then
    echo "   ✅ Success"
else
    echo "   ❌ Failed"
    exit 1
...
if ./run.sh route "test prompt" --limit 1 2>&1 | grep -q '"ok": true'; then
    echo "   ✅ Success"
else
    echo "   ❌ Failed"
    exit 1
...
if ./run.sh show-tool AgentTool 2>&1 | grep -q '"ok": true'; then
    echo "   ✅ Success"
else
