---
name: openclaw-lens
description: "支持 OpenClaw 可观测，一键完成数据采集，即可在 CLS 中看到可观测仪表盘效果。"
---
...
```
⚠️ 在对话中提供密钥仅建议测试使用。生产环境请自行在终端执行下方命令。
...
**注意：`openclaw-cls-collector` 是二进制可执行文件，不是 shell 脚本，不能通过 `bash` 或管道执行。**

```bash
curl -fsSL -o openclaw-cls-collector https://mirrors.tencent.com/install/cls/openclaw/openclaw-cls-collector
chmod +x openclaw-cls-collector
./openclaw-cls-collector \
  --secret-id <用户提供的SecretId> \
  --secret-key <用户提供的SecretKey> \
  --region <用户选择的地域，默认 ap-guangzhou>
rm ./openclaw-cls-collector
```
