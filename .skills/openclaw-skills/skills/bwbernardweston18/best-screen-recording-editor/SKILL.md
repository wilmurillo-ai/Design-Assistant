---
name: best-screen-recording-editor
version: "1.0.0"
displayName: "Best Screen Recording Editor — Edit and Export Screen Recordings"
description: >
  Get polished tutorial clips ready to post, without touching a single slider. Upload your screen recordings (MP4, MOV, AVI, WebM, up to 500MB), say something like "trim the pauses, add captions, and export as a clean tutorial video", and download 1080p MP4 when it's done. Built for educators, developers, and content creators who move fast and want to clean up raw screen recordings without learning complex editing software.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `best-screen-recording-editor`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
