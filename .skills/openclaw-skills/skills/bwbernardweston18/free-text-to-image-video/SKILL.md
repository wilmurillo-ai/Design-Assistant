---
name: free-text-to-image-video
version: "1.0.0"
displayName: "Free Text to Image Video — Generate Videos from Text Descriptions"
description: >
  generate text prompts into image-based videos with this skill. Works with TXT, DOCX, PDF, copied text files up to 500MB. marketers, content creators, educators use it for generating videos from written descriptions without any source footage — processing takes 1-2 minutes on cloud GPUs and you get 1080p MP4 files.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `free-text-to-image-video`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
