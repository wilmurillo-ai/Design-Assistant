---
name: video-compressor-ezgif
version: "1.0.0"
displayName: "Video Compressor Ezgif — Compress and Export Smaller Videos"
description: >
  compress video files into compressed video files with this skill. Works with MP4, GIF, MOV, AVI files up to 500MB. content creators, students, social media users use it for reducing video file size for sharing or uploading — processing takes 20-40 seconds on cloud GPUs and you get 720p MP4 files.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `video-compressor-ezgif`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
