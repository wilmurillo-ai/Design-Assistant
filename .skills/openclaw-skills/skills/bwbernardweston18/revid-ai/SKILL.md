...
name: revid-ai
version: "1.0.0"
displayName: "Revid AI — Create Videos from Text"
description: >
  content creators and marketers create text or video into AI-generated videos using this skill. Accepts MP4, MOV, WebM, TXT up to 500MB, renders on cloud GPUs at 1080p, and returns MP4 files in 1-2 minutes. Designed for converting text or scripts into short social media videos.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `revid-ai`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
