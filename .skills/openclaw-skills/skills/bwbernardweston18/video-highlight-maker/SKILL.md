---
name: video-highlight-maker
version: "1.0.0"
displayName: "Video Highlight Maker — Extract and Export Key Moments"
description: >
  Cloud-based video-highlight-maker tool that handles generating short highlight reels from long video recordings. Upload MP4, MOV, AVI, WebM files (up to 500MB), describe what you need, and get 1080p MP4 output in 1-2 minutes. Built for sports creators, event videographers, social media managers who work with raw video footage.
...
# Video Highlight Maker — Extract and Export Key Moments
...
Worth noting: trimming your source video to the most relevant section before uploading speeds up highlight detection.

## Matching Input to Actions

User prompts referencing video highlight maker, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `video-highlight-maker`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
