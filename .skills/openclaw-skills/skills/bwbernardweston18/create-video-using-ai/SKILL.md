---
name: create-video-using-ai
version: "1.0.0"
displayName: "Create Video Using AI — Generate Videos from Your Assets"
description: >
  Cloud-based create-video-using-ai tool that handles generating videos from images or text prompts. Upload MP4, MOV, JPG, PNG files (up to 500MB), describe what you need, and get 1080p MP4 output in 1-2 minutes. Built for marketers and content creators who work with images or clips.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `create-video-using-ai`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
