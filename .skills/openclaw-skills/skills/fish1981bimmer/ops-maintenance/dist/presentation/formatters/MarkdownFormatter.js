...
class MarkdownFormatter {
    ...
    getStatusDisplay(status) {
        switch (status) {
            case schemas_1.ServerStatus.HEALTHY:
                return '✅ 健康';
            case schemas_1.ServerStatus.WARNING:
                return '⚠️ 警告';
            case schemas_1.ServerStatus.OFFLINE:
                return '❌ 离线';
            default:
                return '❓ 未知';
        }
     ...
     * 生成优化建议
     */
    generateRecommendations(serverHealth) {
        const recommendations = [];
        for (const health of serverHealth) {
            const serverName = health.server.getDisplayName();
            if (health.status === schemas_1.ServerStatus.OFFLINE) {
        ...
        // 如果没有发现问题
        if (recommendations.length === 0 && report.serverHealth.every(h => h.status === schemas_1.ServerStatus.HEALTHY)) {
            recommendations.push({
                server: '所有服务器',
                category: '状态良好',
                severity: '🟢 低',
    ...
    formatPasswordReport(data) {
        ...
        // 优化建议
        lines.push('### 💡 优化建议\n');
        const hasExpired = results.some((r) => r.status.includes('已过期') || r.status.includes('即将过期') || r.status.includes('永不过期'));
        if (hasExpired) {
    ...
    formatServerHealthDetailed(health) {
        const lines = [];
     ...
     * 辅助：获取密码状态 emoji
     */
    getPasswordStatusEmoji(status) {
        if (status.includes('已过期'))
            return '❌';
        if (status.includes('即将过期'))
            return '⚠️';
        if (status.includes('永不过期'))
            return '⚠️';
