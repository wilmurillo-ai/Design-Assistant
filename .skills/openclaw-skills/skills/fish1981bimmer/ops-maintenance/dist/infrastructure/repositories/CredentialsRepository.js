...
exports.EnvironmentCredentialsProvider = exports.ConfigManagerCredentialsProvider = void 0;
 ...
 */
class ConfigManagerCredentialsProvider {
    ...
    getFromEnvironment(server) {
        ...
        if (keyFilePath && (0, fs_1.existsSync)(keyFilePath)) {
     ...
     * }
     */
    async getFromConfigFile(server) {
        // 读取原配置文件（包含敏感信息）
     ...
     * 保存凭据（运行时添加）
     */
    async setCredentials(server, credentials) {
        // 这里可以实现运行时凭据缓存
        // 但不持久化到文件（避免安全风险）
        console.warn('注意：运行时设置的凭据不会持久化，进程重启后需重新配置');
    }
}
exports.ConfigManagerCredentialsProvider = ConfigManagerCredentialsProvider;
/**
 * 环境变量凭据提供者（最安全）
...
exports.EnvironmentCredentialsProvider = EnvironmentCredentialsProvider;
// 辅助函数
function join(...paths) {
    return paths.join('/').replace(/\\/g, '/');
}
//# sourceMappingURL=CredentialsRepository.js.map