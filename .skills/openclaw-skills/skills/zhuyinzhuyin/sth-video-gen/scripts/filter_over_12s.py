...
def filter_csv_over_12s(input_file, output_file):
    with open(input_file, 'r') as f:
        reader = csv.DictReader(f)
        fieldnames = ['Names', 'Template ID', 'Duration', 'Status']
        rows = list(reader)
    ...
    print(f"Checking {len(rows)} templates for duration > 12s...")
    for row in rows:
        template_id = row.get('Template ID', row.get('Song Template ID', '')).strip()
        name = row.get('Names', row.get('Song', '')).strip()
        if not template_id:
            continue
        ...
        if duration is not None:
            if duration > 12.0:
                print(f"Template {template_id}: Duration {duration:.2f}s (>12s) ✓")
...
if __name__ == '__main__':
    filter_csv_over_12s(sys.argv[1], sys.argv[2])
