...
```bash
trends balance 11111111111111111111111111111111
```

Holdings:

```bash
trends holdings 11111111111111111111111111111111 --count 20
```

Created tokens:

```bash
trends created 11111111111111111111111111111111 --count 20
```

Transactions:

```bash
trends transactions 11111111111111111111111111111111 --count 20
```
...
```bash
trends quote buy <mint> --in-sol 0.1
```

Quote sell first:

```bash
trends quote sell <mint> --in-token 1
```
...
```bash
trends buy <mint> --in-sol 0.1
```

or

```bash
trends sell <mint> --in-token 1
```
...
Before `trends create`, complete missing parameters first. Do not jump directly to execution confirmation.
...
```bash
trends create --name "My Coin" --symbol "MYC"
```
...
```bash
trends create --name "My Coin" --symbol "MYC" --image-path ./logo.png
```
...
- `--slippage-bps` is not supported on `trends create`.
...
```bash
trends created <address> --count 20
```

Next page with cursor from previous response:

```bash
trends created <address> --count 20 --cursor <next_cursor>
```

Same for transactions:

```bash
trends transactions <address> --count 20 --cursor <next_cursor>
```
...
```bash
trends --json quote buy <mint> --in-sol 0.1
```
