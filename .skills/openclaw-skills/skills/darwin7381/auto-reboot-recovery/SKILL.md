...
3. **在工作目錄建立 CLAUDE.md**：空目錄會導致 Claude Code 彈 init 提示卡住
4. **確認正確的啟動指令**：從 `ps aux` 驗證真實指令，不要憑記憶寫
5. **確認 tmux socket**：預設 socket（人操作）還是 OpenClaw socket（agent 管）
6. **第一次手動啟動 + 確認 trust folder**：trust 是 per-directory 一次性的
7. **寫 wrapper script**：從 `references/wrapper-template.md` 改，PATH 必須包含所有依賴 binary
8. **建 LaunchAgent plist**：`KeepAlive: true`
9. **測試 1：kill session → wrapper 自動重建**
10. **測試 2：kill 程式但 session 殘留 → wrapper 偵測 pane process 是 shell → 自動重建**
11. **測試 3：確認 plugin subprocess 也在跑**（如 `ps aux | grep bun`）
12. **更新 service-registry.json**
13. **更新恢復文件**
