...
import {
  ...
  routeLabel,
  type Campus,
  type ZhuhaiStationKey,
} from "./routes.js";
import { printScheduleTable } from "./table.js";
import type { NormalizedSchedule } from "./types.js";

interface ParsedArgs {
  command: "list" | "link" | "routes" | "help";
...
async function listCommand(flags: Record<string, string | boolean>): Promise<void> {
  ...
  const groups: Array<{ label: string; items: NormalizedSchedule[] }> = [];
  const cacheItems: NormalizedSchedule[] = [];
  let nextCode = 1;
  for (const to of targets) {
    ...
    if (onlyAvailable) items = items.filter((item) => item.status === "available");
...
async function linkCommand(flags: Record<string, string | boolean>, positionals: string[]): Promise<void> {
  ...
  console.log(
    `Line ${available.lineTime}, board ${available.boardingTime}, arrive ${available.arrivalTime}, ${available.fromStation} -> ${available.toStation}`,
  );
  console.log(link);
}

async function linkFromCode(code: string, flags: Record<string, string | boolean>): Promise<void> {
  if (!/^\d+$/.test(code)) {
  ...
  const fresh = data.carLineList.find((raw) => {
    ...
    return (
      raw.shiftScheduleId === entry.shiftScheduleId &&
...
function getTargets(start: Campus, toFlag: string | undefined, all: boolean): Campus[] {
  if (toFlag) {
  ...
  if (all && start === "zhuhai") return ["south", "east"];
  return [defaultDestination(start)];
}

function validateRoute(start: Campus, to: Campus): void {
  if (to === start) throw new Error("--start and --to cannot be the same campus.");
  if (start !== "zhuhai" && to !== "zhuhai") {
    throw new Error("Unsupported route. One side must be zhuhai.");
  }
}

function getDate(flags: Record<string, string | boolean>): string {
...
function parseArgs(argv: string[]): ParsedArgs {
  ...
  let command: ParsedArgs["command"] = "list";
