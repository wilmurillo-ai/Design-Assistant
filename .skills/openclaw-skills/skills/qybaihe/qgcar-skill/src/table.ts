...
export function printScheduleTable(items: NormalizedSchedule[]): void {
  if (items.length === 0) {
...
function printRow(row: string[], widths: number[]): void {
  console.log(row.map((cell, index) => padEndDisplay(cell, widths[index])).join("  "));
}

function formatStatus(status: NormalizedSchedule["status"]): string {
