...
使用方法:
    python scripts/tianyancha_login.py
...
COOKIES_FILE = CONFIG_DIR / "tianyancha_cookies.json"
...
def check_login_status(page) -> bool:
    ...
    try:
        # 检查是否有用户头像或用户名元素
        page.wait_for_selector('.user-info, .tyc-header-user, [class*="user"]', timeout=5000)
        return True
    except:
        return False
...
def interactive_login():
    ...
    with sync_playwright() as p:
        ...
        # 访问天眼查
        print("正在打开天眼查...")
        page.goto(TIANYANCHA_URL, wait_until='networkidle')
        time.sleep(2)
...
def test_cookies():
    ...
    with sync_playwright() as p:
        ...
        try:
            page.goto(TIANYANCHA_URL, wait_until='networkidle', timeout=30000)
            time.sleep(2)
            ...
            if check_login_status(page):
                ...
                print("✗ 登录状态已过期")
                print("  请重新运行: python scripts/tianyancha_login.py")
                browser.close()
                return False
...
def main():
    ...
    parser = argparse.ArgumentParser(description="天眼查登录管理")
