...
CONFIG_FILE="${UNIFI_CONFIG_FILE:-$HOME/.openclaw/credentials/unifi.json}"

# Load config (support both formats)
if [ ! -f "$CONFIG_FILE" ]; then
  echo "Error: UniFi not configured. Create $CONFIG_FILE with {\"host\": \"...\", \"username\": \"...\", \"password\": \"...\"}" >&2
  exit 1
fi

# Support both {url: ...} and {host: ...} formats
UNIFI_HOST=$(jq -r '.host // empty' "$CONFIG_FILE" 2>/dev/null || echo "")
