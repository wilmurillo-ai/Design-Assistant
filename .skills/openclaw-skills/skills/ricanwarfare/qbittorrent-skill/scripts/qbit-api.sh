...
CONFIG_FILE="${QBIT_CONFIG:-$HOME/.openclaw/credentials/qbittorrent/config.json}"
...
Commands:
  ...
  add-trackers <hash> <urls>     Add trackers to torrent (newline-separated URLs)
  ...
  set-preferences <json>         Set app preferences (JSON string)
...
cmd_add_trackers() {
    local hash="$1"
    local urls="$2"
    api_call POST "/api/v2/torrents/addTrackers" -d "hash=$hash&urls=$urls"
    echo '{"status": "ok"}'
}
...
cmd_set_preferences() {
    local json="$1"
    api_call POST "/api/v2/app/setPreferences" -d "json=$json"
    echo '{"status": "ok"}'
}
...
case "${1:-}" in
    ...
    add-trackers) shift; cmd_add_trackers "$@" ;;
    ...
    set-preferences) shift; cmd_set_preferences "$@" ;;
    -h|--help|help|"") usage ;;
