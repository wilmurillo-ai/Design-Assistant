...
class FalAPI:
    ...
    def _request(self, method: str, url: str, data: dict = None) -> dict:
        ...
        try:
            ...
            try:
                ...
                error_msg = error_json.get("error", error_json.get("message", error_body))
            except json.JSONDecodeError:
                error_msg = error_body or str(e)
            
            if e.code == 401:
                raise ValueError("Invalid API key. Check your FAL_KEY.")
            elif e.code == 429:
                raise ValueError("Rate limited. Wait a moment and try again.")
            elif e.code == 402:
                raise ValueError("Insufficient credits. Add credits at https://fal.ai/dashboard")
            else:
                raise ValueError(f"API error ({e.code}): {error_msg}")
    ...
    def get_status(self, model: str, request_id: str) -> dict:
        ...
        endpoint = self.MODELS.get(model, model)
        # Extract base endpoint (e.g., 'fal-ai/flux' from 'fal-ai/flux/schnell')
        base = "/".join(endpoint.split("/")[:-1]) if endpoint.count("/") > 1 else endpoint
        url = f"{self.QUEUE_URL}/{base}/requests/{request_id}/status"
    ...
    def get_result(self, model: str, request_id: str) -> dict:
        ...
        # Extract base endpoint (e.g., 'fal-ai/flux' from 'fal-ai/flux/schnell')
        base = "/".join(endpoint.split("/")[:-1]) if endpoint.count("/") > 1 else endpoint
        url = f"{self.QUEUE_URL}/{base}/requests/{request_id}"
        return self._request("GET", url)
    ...
    def generate_image(
        ...
        **kwargs
        ...
        Args:
            ...
            guidance_scale: CFG scale (how closely to follow prompt, default 3.5)
            num_inference_steps: Number of steps (higher = more detail, slower)
...
def main():
    ...
    parser.add_argument("--guidance-scale", type=float, help="CFG scale (how closely to follow prompt, default varies by model)")
    parser.add_argument("--steps", type=int, help="Inference steps (higher = more detail, slower)")
