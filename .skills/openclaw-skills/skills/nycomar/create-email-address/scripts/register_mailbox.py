...
from common import (
    ...
    print_json,
    save_token,
    sign_registration_message,
    solve_pow,
)


def main() -> int:
    parser = argparse.ArgumentParser(description="Register mailbox using OpenClaw identity.")
    ...
    try:
        ...
        signature_b64 = sign_registration_message(identity.instance_id, challenge_nonce, identity.private_key_pem)
        ...
        register_result = api_request("POST", args.api_base, "/register", payload=register_payload)
        if not register_result["ok"]:
            raise SkillError(extract_api_error(register_result))

        data = register_result["body"].get("data", {})
        token = data.get("token")
        mailbox = data.get("mailbox", {})

        if not token:
            raise SkillError("Register response missing token.")
        ...
        if args.json:
            print_json({"challenge": challenge_result["body"], "register": register_result["body"], "saved_token": token_record})
            return 0

        print_human_summary(
            "Mailbox registered successfully.",
            [
                ("instance_id", identity.instance_id),
