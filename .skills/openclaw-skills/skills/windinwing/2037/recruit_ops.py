#!/usr/bin/env python3
"""
征兵 / 造兵 — 优先走 GameSkillAPI 桥接别名（无需手写 JSON 与时间）：

  LISTRECRUITQUEUE / RECRUITQLIST  — 对应 GETCONSCRIPTIONQUEUE（列表）
  RECRUITQUEUE / RECRUIT          — 对应 ADDCONSCRIPTIONQUEUE（args: troopId total [tileId]）

旧客户端协议（抓包）仍为 GETCONSCRIPTIONQUEUE + ADDCONSCRIPTIONQUEUE（JSON 数组）；无桥接时可用
子命令 raw-add、compose。
...
示例（需桥接服务已部署）：
  ...
  python3 recruit_ops.py raw-add '[{"cityID":293135,...}]'
  python3 recruit_ops.py compose --tile 293135 --troop 43 --total 8 --unit-training 141
"""
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Earth2037 征兵：桥接命令 LISTRECRUITQUEUE / RECRUITQUEUE（推荐）；或原始 ADDCONSCRIPTIONQUEUE JSON"
    )
    ...
    p_list = sub.add_parser(
        "list",
        aliases=["getconscriptionqueue"],
        help="列表征兵队列（LISTRECRUITQUEUE → GETCONSCRIPTIONQUEUE）",
    )
    p_list.add_argument("tile_id", nargs="?", default="", help="城市 tileID；省略则由服务端默认主城")

    p_rec = sub.add_parser("recruit", help="造兵入队（RECRUITQUEUE → ADDCONSCRIPTIONQUEUE），无需 JSON")
    p_rec.add_argument("troop_id", type=int, help="兵种 troopID")
    p_rec.add_argument("total", type=int, help="数量")
    p_rec.add_argument("tile_id", nargs="?", type=int, default=None, help="城市 tileID；省略=主城")

    p_raw = sub.add_parser(
        "raw-add",
        aliases=["addconscriptionqueue"],
        help="原始 ADDCONSCRIPTIONQUEUE：args 为 JSON 数组（无桥接时用）",
    )
    p_raw.add_argument("json", help="JSON 数组一行，或 @文件路径，或 - 从 stdin")

    p_comp = sub.add_parser(
        "compose",
        help="本地组 JSON 再 ADDCONSCRIPTIONQUEUE（无桥接 RECRUIT 时的退路）",
    )
    ...
    try:
        if args.cmd in ("list", "getconscriptionqueue"):
            ...
            if not js.lstrip().startswith("["):
                print("❌ ADDCONSCRIPTIONQUEUE 需要 JSON 数组，例如 [{...}]", file=sys.stderr)
                sys.exit(1)
            ...
            if ut is None or ut < 1:
                print(
                    "❌ 无法得到 unitTraining：请传 --unit-training，或先用桥接命令 recruit",
                    file=sys.stderr,
                )
                sys.exit(1)
