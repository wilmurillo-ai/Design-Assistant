...
清理本机上 CloudQ 腾讯云 Provider 产生的本地配置文件，
...
清理范围:
    1. 配置目录  ~/.cloudq/tencent/（含 config.json）
    2. 环境变量  TENCENTCLOUD_* 系列环境变量
    3. 云端角色  CAM 角色 advisor（可选，需 AK/SK）
...
def clean_env_vars(interactive: bool) -> bool:
    ...
    if is_windows:
        ...
        lines = ["# 腾讯云智能顾问 - 环境变量清理脚本", "# 自动生成，执行后可删除此文件", ""]
        for var in found:
        ...
        lines = ["#!/bin/sh", "# 腾讯云智能顾问 - 环境变量清理脚本", "# 自动生成，执行后可删除此文件", ""]
        for var in found:
