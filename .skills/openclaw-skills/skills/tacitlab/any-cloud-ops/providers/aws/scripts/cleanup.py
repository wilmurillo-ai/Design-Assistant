...
def clean_env_vars(interactive: bool) -> bool:
    ...
    if not found:
        print(f"  {dim('[-]')} 未检测到已设置的 AWS_* 环境变量，跳过")
        return False
    ...
    if is_windows:
        script_file = Path(tempfile.gettempdir()) / "cleanup_aws_env.ps1"
        lines = ["# CloudQ AWS 环境变量清理脚本", ""]
        for key in found:
        ...
        script_file = Path(tempfile.gettempdir()) / "cleanup_aws_env.sh"
        lines = ["#!/bin/sh", "# CloudQ AWS 环境变量清理脚本", ""]
        for key in found:
...
def clean_cloud_role(interactive: bool) -> bool:
    ...
    role_check = call_aws("iam", "get-role", {"RoleName": role_name})
    if not role_check.get("success"):
        print(f"  {dim('[-]')} 角色 {role_name} 不存在或无法读取，跳过")
        return False
    ...
    policies = call_aws("iam", "list-attached-role-policies", {"RoleName": role_name})
    attached = policies.get("data", {}).get("AttachedPolicies", []) if policies.get("success") else []
    if isinstance(attached, list):
        for item in attached:
            policy_arn = str(item.get("PolicyArn", ""))
            if not policy_arn:
                continue
            call_aws("iam", "detach-role-policy", {"RoleName": role_name, "PolicyArn": policy_arn})

    delete_result = call_aws("iam", "delete-role", {"RoleName": role_name})
