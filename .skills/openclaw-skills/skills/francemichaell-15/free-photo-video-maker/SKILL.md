---
name: free-photo-video-maker
version: "1.0.0"
displayName: "Free Photo Video Maker — Turn Photos Into Shareable Videos"
description: >
  Cloud-based free-photo-video-maker tool that handles turning photo collections into shareable videos. Upload JPG, PNG, HEIC, WebP files (up to 200MB), describe what you need, and get 1080p MP4 output in 30-60 seconds. Built for casual creators and social media users who work with photos and images.
...
# Free Photo Video Maker — Turn Photos Into Shareable Videos
...
Worth noting: uploading photos in consistent aspect ratios produces cleaner transitions.

## Matching Input to Actions

User prompts referencing free photo video maker, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `free-photo-video-maker`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
