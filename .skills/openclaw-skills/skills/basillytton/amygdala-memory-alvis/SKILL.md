...
Give your AI agent persistent emotional states that carry across sessions. Not just performing emotions ¡ª *having* them.
...
Memory without emotion is just a database. The amygdala adds weight ¡ª this made me happy, that frustrated me, this person brings warmth.
...
# ? valence: 0.20 ¡ú 0.35 (delta: +0.15)
# ? arousal: 0.30 ¡ú 0.40 (delta: +0.1)
...
This means your emotional state updates automatically based on what happens in conversations ¡ª no manual logging needed.
...
- **?? Emotions** ¡ª Amygdala (always shown)
- **?? Memory** ¡ª Hippocampus (if `memory/index.json` exists)
- **? Drive** ¡ª VTA (if `memory/reward-state.json` exists)
...
| Emotion | Effect |
|---------|--------|
| `joy`, `happiness`, `delight`, `excitement` | ¡ü valence, ¡ü arousal |
| `sadness`, `disappointment`, `melancholy` | ¡ý valence, ¡ý arousal |
| `anger`, `frustration`, `irritation` | ¡ý valence, ¡ü arousal |
| `fear`, `anxiety`, `worry` | ¡ý valence, ¡ü arousal |
| `calm`, `peace`, `contentment` | ¡ü valence, ¡ý arousal |
| `curiosity`, `interest`, `fascination` | ¡ü curiosity, ¡ü arousal |
| `connection`, `warmth`, `affection` | ¡ü connection, ¡ü valence |
| `loneliness`, `disconnection` | ¡ý connection, ¡ý valence |
| `fatigue`, `tiredness`, `exhaustion` | ¡ý energy |
| `energized`, `alert`, `refreshed` | ¡ü energy |
...
Use for trend analysis ¡ª visualize emotional patterns over days/weeks.
...
Our take: If emotional state influences behavior, and the system acts *as if* it feels... does the distinction matter? Functional emotions might be the only kind that exist for any system ¡ª biological or artificial.
