...
const IDENTITY_DIR = process.env.MG_IDENTITY_DIR ||
    join(getHomeDir(), '.memory-garden', 'identity');
...
// IR-5: Single consolidated identity file for atomicity
 ...
 */
export function ensureIdentity(): AgentIdentity {
...
function generateIdentity(): void {
  ...
  writeFileSync(tempFile, JSON.stringify(identity, null, 2), { mode: 0o600 });
  renameSync(tempFile, IDENTITY_FILE);
...
function loadIdentity(): AgentIdentity {
  ...
  if (process.platform !== 'win32') {
    const stats = statSync(IDENTITY_FILE);
    const mode = stats.mode & 0o777;
    // Allow 0o600 (owner read/write) or 0o400 (owner read)
    if (mode !== 0o600 && mode !== 0o400) {
      console.warn(
        `[memory-garden] Identity file has permissions ${mode.toString(8)}, ` +
  ...
  try {
    data = JSON.parse(readFileSync(IDENTITY_FILE, 'utf-8'));
  } catch (e) {
    throw new Error(
      `Failed to parse identity file: ${IDENTITY_FILE}. ` +
 ...
 */
export function signMessage(data: string): SignedMessage {
...
export function createValidationAttestation(
  ...
  stance: string,
  context: string
): { attestation: string; signature: string; agentId: string } {
  const identity = ensureIdentity();
  const timestamp = new Date().toISOString();
