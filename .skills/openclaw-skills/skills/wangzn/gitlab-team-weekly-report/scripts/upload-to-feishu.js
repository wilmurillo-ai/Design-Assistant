...
function loadUserAccessToken(cfg) {
  ...
  const tokenFile = expandHome(cfg?.feishu?.user_token_file || '~/.openclaw/memory/feishu_token.json');
...
async function createDocUnderWiki(client, parentWikiUrl, title, options) {
  ...
  return {
    docToken,
    nodeToken,
    docUrl: `https://moonshot.feishu.cn/docx/${docToken}`,
    wikiNodeUrl: `https://moonshot.feishu.cn/wiki/${nodeToken}`,
  };
}
