...
ETH staking yield (~4% APR) provides a fundamental price floor. This skill
uses the yield-implied fair value to identify mispriced ETH markets on Kalshi.
When markets price ETH below the staking-yield floor, buy YES.
...
CONFIG_SCHEMA = {
    "entry_edge":        {"env": "SIMMER_ETH_STAKE_ENTRY_EDGE",        "default": 0.08,  "type": float},
    "exit_threshold":    {"env": "SIMMER_ETH_STAKE_EXIT_THRESHOLD",    "default": 0.45,  "type": float},
    "max_position_usd":  {"env": "SIMMER_ETH_STAKE_MAX_POSITION_USD",  "default": 5.00,  "type": float},
    "max_trades_per_run":{"env": "SIMMER_ETH_STAKE_MAX_TRADES_PER_RUN","default": 3,     "type": int},
    "slippage_max":      {"env": "SIMMER_ETH_STAKE_SLIPPAGE_MAX",      "default": 0.15,  "type": float},
    "min_liquidity":     {"env": "SIMMER_ETH_STAKE_MIN_LIQUIDITY",     "default": 0.0,   "type": float},
    "staking_yield_pct": {"env": "SIMMER_ETH_STAKE_YIELD_PCT",         "default": 4.0,   "type": float},
    "base_eth_price":    {"env": "SIMMER_ETH_STAKE_BASE_PRICE",        "default": 3500.0,"type": float},
}

_config = load_config(CONFIG_SCHEMA, __file__, slug="kalshi-eth-staking-yield-trader")
...
_client = None
TRADE_SOURCE = "sdk:kalshi-eth-staking"
SKILL_SLUG = "kalshi-eth-staking-yield-trader"
...
STAKING_YIELD_PCT = _config["staking_yield_pct"]
...
def compute_yield_floor(base_price: float, yield_pct: float,
                        days_to_resolution: int) -> float:
    """Compute yield-implied price floor.

    Stakers earn yield_pct annually, which provides a fundamental floor.
    Over `days` until resolution, the minimum expected price assuming
    rational stakers is: base_price * (1 + yield_pct/100 * days/365).

    This is conservative -- it only accounts for yield, not price appreciation.
    """
    annual_fraction = days_to_resolution / 365.0
    yield_multiplier = 1.0 + (yield_pct / 100.0) * annual_fraction
    return base_price * yield_multiplier


def extract_price_target(question: str) -> float | None:
    """Extract price target from market question.

    Parses questions like "Will ETH be above $3000?" or "ETH close above $4000".
    Returns the price target as a float, or None.
    """
    ...
    for i, word in enumerate(words):
        if word.startswith("$"):
            try:
                ...
                if val > 100:  # Likely a price, not a percentage
                    return val
            except ValueError:
                continue

    # Try finding numbers after "above"/"below"/"over"/"under"
...
def compute_yield_fair_probability(price_target: float, floor_price: float,
                                    current_price: float) -> float:
    """Compute fair probability that ETH will be above price_target.

    Uses the yield floor as a minimum. If target < floor, probability is high.
    If target > floor, probability scales down based on how far above.
    """
    if price_target <= 0:
        return 0.5

    # If target is below the yield floor, very likely to be above
    if price_target <= floor_price * 0.9:
        return 0.85

    if price_target <= floor_price:
        return 0.70

    # Target above floor -- decreasing probability
    ratio = price_target / floor_price
    if ratio <= 1.1:
        return 0.55
    elif ratio <= 1.3:
        return 0.40
...
def check_exits(dry_run: bool, log=print) -> tuple:
    """Check open ETH staking positions for exit opportunities."""
    ...
    log(f"\n  Checking {len(eth_pos)} ETH staking positions for exit...")
    found, executed = 0, 0
...
def run_strategy(dry_run: bool = True, positions_only: bool = False,
    ...
    """Run the ETH staking yield floor trading strategy."""
    ...
    log(f"  Staking yield: {STAKING_YIELD_PCT:.1f}%  Base ETH price: ${BASE_ETH_PRICE:.0f}")
    log("")
    ...
    for m in markets:
        ...
        floor_price = compute_yield_floor(BASE_ETH_PRICE, STAKING_YIELD_PCT, days)

        # Compute fair probability
        fair_prob = compute_yield_fair_probability(price_target, floor_price, BASE_ETH_PRICE)

        log(f"  {question[:50]}  target=${price_target:.0f}  floor=${floor_price:.0f}  "
        ...
        if not dry_run:
            result = execute_trade(
                market_id, side, size,
                reasoning=reasoning,
                signal_data={"fair_prob": round(fair_prob, 4), "floor_price": round(floor_price, 2),
                             "price_target": price_target, "staking_yield": STAKING_YIELD_PCT},
            )
            if result.get("success"):
                trades_executed += 1
                total_usd += size
                ...
                if result.get("trade_id") and JOURNAL_AVAILABLE and not result.get("simulated"):
                    log_trade(trade_id=result["trade_id"], source=TRADE_SOURCE,
                              skill_slug=SKILL_SLUG, thesis="ETH staking yield floor", action="buy")
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kalshi ETH Staking Yield Trader")
