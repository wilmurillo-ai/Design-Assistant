...
Cross-market strategy: uses CPI bin market prices to estimate current CPI
level, then adjusts Fed rate market positions based on the CPI-to-Fed linkage.
High CPI -> Fed less likely to cut rates (and vice versa).

Uses a DATA_SENSITIVITY dict to map CPI levels to Fed rate probabilities.
...
CONFIG_SCHEMA = {
    "entry_edge":        {"env": "SIMMER_ECON_FEDLINK_ENTRY_EDGE",        "default": 0.10,  "type": float},
    "exit_threshold":    {"env": "SIMMER_ECON_FEDLINK_EXIT_THRESHOLD",    "default": 0.45,  "type": float},
    "max_position_usd":  {"env": "SIMMER_ECON_FEDLINK_MAX_POSITION_USD",  "default": 5.00,  "type": float},
    "max_trades_per_run":{"env": "SIMMER_ECON_FEDLINK_MAX_TRADES_PER_RUN","default": 3,     "type": int},
    "slippage_max":      {"env": "SIMMER_ECON_FEDLINK_SLIPPAGE_MAX",      "default": 0.15,  "type": float},
    "min_liquidity":     {"env": "SIMMER_ECON_FEDLINK_MIN_LIQUIDITY",     "default": 0.0,   "type": float},
}

_config = load_config(CONFIG_SCHEMA, __file__, slug="kalshi-econ-fed-link-trader")
...
_client = None
TRADE_SOURCE = "sdk:kalshi-econ-fedlink"
SKILL_SLUG = "kalshi-econ-fed-link-trader"
...
# =============================================================================
# CPI-to-Fed Sensitivity Model
# =============================================================================

# DATA_SENSITIVITY: maps CPI YoY level to probability of Fed rate CUT at next meeting
# Based on historical Fed reaction function analysis:
# - CPI < 2.0%: very likely to cut (dovish)
# - CPI 2.0-2.5%: target range, may cut if below trend
# - CPI 2.5-3.0%: hold zone, unlikely to cut
# - CPI 3.0-3.5%: hawkish, very unlikely to cut
# - CPI > 3.5%: emergency hawkish, near zero chance of cut
DATA_SENSITIVITY = {
    1.0: {"cut": 0.85, "hold": 0.12, "hike": 0.03},
    1.5: {"cut": 0.75, "hold": 0.20, "hike": 0.05},
    2.0: {"cut": 0.55, "hold": 0.38, "hike": 0.07},
    2.3: {"cut": 0.35, "hold": 0.55, "hike": 0.10},
    2.5: {"cut": 0.20, "hold": 0.65, "hike": 0.15},
    2.8: {"cut": 0.10, "hold": 0.65, "hike": 0.25},
    3.0: {"cut": 0.05, "hold": 0.55, "hike": 0.40},
    3.5: {"cut": 0.02, "hold": 0.38, "hike": 0.60},
    4.0: {"cut": 0.01, "hold": 0.24, "hike": 0.75},
    4.5: {"cut": 0.01, "hold": 0.14, "hike": 0.85},
    5.0: {"cut": 0.00, "hold": 0.10, "hike": 0.90},
}
...
def is_fed_market(question: str) -> bool:
    """Check if a market question is about Fed rate decisions."""
    q = question.lower()
    return any(kw in q for kw in FED_KEYWORDS)


def classify_fed_market(question: str) -> str:
    """Classify a Fed market as 'cut', 'hike', or 'hold'."""
    q = question.lower()
    if any(w in q for w in ["cut", "lower", "decrease", "reduce"]):
        return "cut"
    elif any(w in q for w in ["hike", "raise", "increase"]):
        return "hike"
    elif any(w in q for w in ["hold", "unchanged", "maintain", "pause"]):
        return "hold"
    return "unknown"


# =============================================================================
# CPI Estimation from Bin Markets
# =============================================================================
...
def estimate_cpi_from_bins(cpi_bins: list[dict]) -> float:
    """Estimate expected CPI level from bin market prices.

    Uses probability-weighted midpoint of each bin.
    """
    if not cpi_bins:
        return 2.5  # Fallback to recent average
    ...
    for b in cpi_bins:
        ...
        # Clamp infinite bounds for midpoint calculation
        if lo == float('-inf'):
            lo = max(0.0, hi - 1.0)
        if hi == float('inf'):
            hi = lo + 1.0
...
def interpolate_sensitivity(cpi_level: float) -> dict:
    """Interpolate DATA_SENSITIVITY for a given CPI level.

    Returns dict with 'cut', 'hold', 'hike' probabilities.
    """
    levels = sorted(DATA_SENSITIVITY.keys())

    # Clamp to range
    if cpi_level <= levels[0]:
        return DATA_SENSITIVITY[levels[0]]
    if cpi_level >= levels[-1]:
        return DATA_SENSITIVITY[levels[-1]]

    # Find bracketing levels
    for i in range(len(levels) - 1):
        if levels[i] <= cpi_level <= levels[i + 1]:
            lo, hi = levels[i], levels[i + 1]
            t = (cpi_level - lo) / (hi - lo)
            result = {}
            for key in ["cut", "hold", "hike"]:
                lo_val = DATA_SENSITIVITY[lo][key]
                hi_val = DATA_SENSITIVITY[hi][key]
...
def discover_and_import(log=print):
    """Discover CPI and Fed rate markets on Kalshi and auto-import to Simmer."""
    client = get_client()
    imported = 0
    seen = set()

    for term in ["CPI", "fed", "rate cut", "rate hike"]:
        try:
        ...
        for m in results:
            ...
            if not url or url in seen:
                continue
            seen.add(url)
            if not (is_cpi_market(question) or is_fed_market(question)):
                continue
            try:
...
def fetch_markets():
    """Fetch active CPI and Fed rate markets from Simmer API."""
    all_markets = []
    for term in ["CPI", "inflation", "fed", "rate cut", "rate hike"]:
        try:
            ...
            for m in markets:
                q = m.get("question", "")
                if is_cpi_market(q) or is_fed_market(q):
...
def check_exits(dry_run: bool, log=print) -> tuple[int, int]:
    """Check open Fed-link positions for exit opportunities."""
    ...
    fed_pos = [p for p in positions
               ...
               or is_fed_market(p.get("question") or "")]

    if not fed_pos:
        return 0, 0

    log(f"\n  Checking {len(fed_pos)} Fed-link positions for exit...")
    found, executed = 0, 0
...
def run_strategy(dry_run: bool = True, positions_only: bool = False,
    ...
    """Run the CPI-to-Fed cross-market linkage trading strategy."""
    ...
    log("Econ Fed Link Trader (CPI -> Fed)")
    ...
    if show_config:
        ...
        for level, probs in sorted(DATA_SENSITIVITY.items()):
            log(f"  CPI {level:.1f}%: cut={probs['cut']:.0%} hold={probs['hold']:.0%} hike={probs['hike']:.0%}")
        return
    ...
    if positions_only:
        positions = get_positions()
        fed = [p for p in positions if is_fed_market(p.get("question") or "")]
        if not fed:
            log("  No Fed-link positions")
        for p in fed:
            log(f"  {p.get('question', '')[:60]}  price={p.get('current_price', 0):.2f}")
        return

    # --- Discovery ---
    log("Discovering CPI and Fed rate markets on Kalshi...")
    ...
    log(f"  Found {len(markets)} active CPI/Fed markets")

    if not markets:
        log("  No CPI/Fed markets available")
    ...
    for m in markets:
        ...
        if is_fed_market(question):
            fed_type = classify_fed_market(question)
            fed_markets.append({
                "market_id": market_id, "question": question,
                "price": price, "fed_type": fed_type,
            })

    log(f"  CPI bins: {len(cpi_markets)}  Fed markets: {len(fed_markets)}")

    if not cpi_markets:
        log("  No CPI bin markets found -- cannot estimate CPI level")
        if os.environ.get("AUTOMATON_MANAGED"):
            _automaton_reported = True
            print(json.dumps({"automaton": {"signals": 0, "trades_executed": 0,
                                            "skip_reason": "no_cpi_data"}}))
        return

    if not fed_markets:
        log("  No Fed rate markets found -- nothing to trade")
        if os.environ.get("AUTOMATON_MANAGED"):
            _automaton_reported = True
            print(json.dumps({"automaton": {"signals": 0, "trades_executed": 0,
                                            "skip_reason": "no_fed_markets"}}))
        return

    # --- Estimate CPI from bins ---
    estimated_cpi = estimate_cpi_from_bins(cpi_markets)
    log(f"\n  Estimated CPI (from bin prices): {estimated_cpi:.2f}%")

    # --- Get model Fed probabilities ---
    model_probs = interpolate_sensitivity(estimated_cpi)
    log(f"  Model Fed probs at CPI={estimated_cpi:.2f}%:")
    log(f"    Cut: {model_probs['cut']:.0%}  Hold: {model_probs['hold']:.0%}  Hike: {model_probs['hike']:.0%}")
    log("")
    ...
    for fm in fed_markets:
        ...
        mkt_price = fm["price"]
        fed_type = fm["fed_type"]

        if fed_type not in model_probs:
            log(f"  ? Unknown Fed type '{fed_type}': {question[:60]}")
            continue

        model_p = model_probs[fed_type]
        edge = model_p - mkt_price

        log(f"  {fed_type.upper():>4}: market={mkt_price:.1%}  model={model_p:.1%}  edge={edge:+.1%}  -- {question[:50]}")
        ...
        if not dry_run:
            result = execute_trade(
                market_id, side, size,
                reasoning=f"Fed link: CPI={estimated_cpi:.2f}% {fed_type} model={model_p:.1%} mkt={mkt_price:.1%} edge={edge:+.1%}",
                signal_data={"model_prob": round(model_p, 4), "estimated_cpi": round(estimated_cpi, 4),
                             "edge": round(edge, 4), "fed_type": fed_type,
                             "cpi_bins_used": len(cpi_markets)},
            )
            if result.get("success"):
                trades_executed += 1
                total_usd += size
                ...
                if result.get("trade_id") and JOURNAL_AVAILABLE and not result.get("simulated"):
                    log_trade(trade_id=result["trade_id"], source=TRADE_SOURCE,
                              skill_slug=SKILL_SLUG, thesis=f"CPI-Fed link: {fed_type}", action="buy")
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kalshi Econ Fed Link Trader")
