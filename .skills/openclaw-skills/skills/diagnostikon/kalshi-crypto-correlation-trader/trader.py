...
Exploits BTC/ETH correlation (beta=1.3) to trade ETH price-level markets
when BTC makes a significant move. ETH tends to lag BTC moves by hours,
creating a window where ETH markets are mispriced relative to the BTC signal.
...
CONFIG_SCHEMA = {
    "entry_edge":        {"env": "SIMMER_CRYPTO_CORR_ENTRY_EDGE",        "default": 0.10,  "type": float},
    "exit_threshold":    {"env": "SIMMER_CRYPTO_CORR_EXIT_THRESHOLD",    "default": 0.45,  "type": float},
    "max_position_usd":  {"env": "SIMMER_CRYPTO_CORR_MAX_POSITION_USD",  "default": 5.00,  "type": float},
    "max_trades_per_run":{"env": "SIMMER_CRYPTO_CORR_MAX_TRADES_PER_RUN","default": 3,     "type": int},
    "slippage_max":      {"env": "SIMMER_CRYPTO_CORR_SLIPPAGE_MAX",      "default": 0.15,  "type": float},
    "min_liquidity":     {"env": "SIMMER_CRYPTO_CORR_MIN_LIQUIDITY",     "default": 0.0,   "type": float},
}

_config = load_config(CONFIG_SCHEMA, __file__, slug="kalshi-crypto-correlation-trader")
...
_client = None
TRADE_SOURCE = "sdk:kalshi-crypto-corr"
SKILL_SLUG = "kalshi-crypto-correlation-trader"
...
# Historical BTC/ETH beta: ETH moves ~1.3x BTC on average
BTC_ETH_BETA = 1.3

# Search terms for market discovery
BTC_SEARCH_TERMS = ["bitcoin", "BTC price", "BTC above"]
ETH_SEARCH_TERMS = ["ethereum", "ETH price", "ETH above"]
...
def compute_eth_fair_value(btc_markets: list[dict], eth_market: dict) -> float | None:
    """Compute fair ETH probability using BTC market prices and correlation beta.

    If BTC markets suggest BTC is more likely to hit targets than priced,
    ETH should move proportionally with beta=1.3.

    Uses the average BTC mispricing signal to adjust ETH fair probability.
    """
    if not btc_markets:
        return None

    eth_price = eth_market.get("external_price_yes") or eth_market.get("price_yes") or 0
    if eth_price <= 0:
        return None

    # Compute average BTC price level (as a sentiment indicator)
    btc_signals = []
    for bm in btc_markets:
        bp = bm.get("external_price_yes") or bm.get("price_yes") or 0
        if bp > 0:
            btc_signals.append(bp)

    if not btc_signals:
        return None

    # Average BTC probability across levels indicates bullish/bearish sentiment
    avg_btc_p = sum(btc_signals) / len(btc_signals)

    # If BTC average probability is high (>50%), ETH should also be higher
    # Adjustment = beta * (BTC_signal - 0.5) applied to ETH base price
    btc_sentiment = avg_btc_p - 0.5
    eth_adjustment = BTC_ETH_BETA * btc_sentiment * 0.2  # Scale factor for probability space

    fair_eth = min(0.95, max(0.05, eth_price + eth_adjustment))
    return fair_eth
...
def discover_and_import(log=print):
    """Discover crypto markets on Kalshi and auto-import to Simmer."""
    client = get_client()
    imported = 0
    seen = set()

    for term in BTC_SEARCH_TERMS + ETH_SEARCH_TERMS:
        try:
...
def run_strategy(dry_run: bool = True, positions_only: bool = False,
    ...
    """Run the BTC/ETH correlation trading strategy."""
    ...
    log("Crypto Correlation Trader (BTC/ETH beta)")
    ...
    log(f"  BTC/ETH beta: {BTC_ETH_BETA}")
    log("")
    ...
    log(f"  BTC markets: {len(btc_markets)}  ETH markets: {len(eth_markets)}")

    if not btc_markets or not eth_markets:
        log("  Need both BTC and ETH markets for correlation trading")
        if os.environ.get("AUTOMATON_MANAGED"):
            _automaton_reported = True
            print(json.dumps({"automaton": {"signals": 0, "trades_executed": 0,
    ...
    # --- Display BTC reference prices ---
    log("\nBTC Reference Markets:")
    for m in btc_markets:
        q = m.get("question", "")[:60]
        p = m.get("external_price_yes") or m.get("price_yes") or 0
        log(f"  {q}  P={p:.1%}")

    # --- Analyze ETH markets using BTC signal ---
    signals = 0
    trades_executed = 0
    total_usd = 0.0
    skip_reasons = []

    log("\nETH Correlation Signals:")
    for m in eth_markets:
        market_id = m.get("id")
        question = m.get("question", "")
        price = m.get("external_price_yes") or m.get("price_yes") or 0

        fair_p = compute_eth_fair_value(btc_markets, m)
        if fair_p is None:
            log(f"  ? Cannot compute fair value: {question[:60]}")
            continue

        edge = fair_p - price
        log(f"  {question[:50]}  market={price:.1%}  fair={fair_p:.1%}  edge={edge:+.1%}")
        ...
        if not dry_run:
            result = execute_trade(
                market_id, side, size,
                reasoning=f"BTC/ETH corr: ETH market={price:.1%} fair={fair_p:.1%} edge={edge:+.1%} beta={BTC_ETH_BETA}",
                signal_data={"fair_prob": round(fair_p, 4), "edge": round(edge, 4),
                             "beta": BTC_ETH_BETA, "btc_markets_count": len(btc_markets)},
            )
            if result.get("success"):
                trades_executed += 1
                total_usd += size
                ...
                if result.get("trade_id") and JOURNAL_AVAILABLE and not result.get("simulated"):
                    log_trade(trade_id=result["trade_id"], source=TRADE_SOURCE,
                              skill_slug=SKILL_SLUG, thesis="BTC/ETH correlation edge", action="buy")
            else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kalshi Crypto Correlation Trader")
