...
Trades CPI range bin markets on Kalshi using the constraint that all bins in
a mutually exclusive set must sum to ~100%. When the sum deviates beyond a
tolerance, normalize and trade the most mispriced bin.
...
CONFIG_SCHEMA = {
    "sum_tolerance":     {"env": "SIMMER_ECON_BINSUM_SUM_TOLERANCE",     "default": 0.05,  "type": float},
    "exit_threshold":    {"env": "SIMMER_ECON_BINSUM_EXIT_THRESHOLD",    "default": 0.45,  "type": float},
    "max_position_usd":  {"env": "SIMMER_ECON_BINSUM_MAX_POSITION_USD",  "default": 5.00,  "type": float},
    "max_trades_per_run":{"env": "SIMMER_ECON_BINSUM_MAX_TRADES_PER_RUN","default": 3,     "type": int},
    "slippage_max":      {"env": "SIMMER_ECON_BINSUM_SLIPPAGE_MAX",      "default": 0.15,  "type": float},
    "min_liquidity":     {"env": "SIMMER_ECON_BINSUM_MIN_LIQUIDITY",     "default": 0.0,   "type": float},
}

_config = load_config(CONFIG_SCHEMA, __file__, slug="kalshi-econ-bin-sum-trader")
...
_client = None
TRADE_SOURCE = "sdk:kalshi-econ-binsum"
SKILL_SLUG = "kalshi-econ-bin-sum-trader"
...
# Patterns for CPI / inflation bin markets
...
def parse_cpi_bin(question: str) -> tuple:
    ...
    - "CPI year-over-year above 3.5%?"
    - "CPI between 2.5% and 3.0%?"
    - "CPI below 2.0%?"
    ...
    # Try extracting two percentage-like numbers
    nums = re.findall(r'(-?\d+\.?\d*)\s*%', q)
    if len(nums) >= 2:
        vals = [float(n) for n in nums]
        return (min(vals), max(vals))
    elif len(nums) == 1:
        # Single number — context determines above/below
        val = float(nums[0])
...
def extract_event_group(question: str) -> str:
    """Extract the event group identifier from a CPI market question.

    Groups bins that belong to the same mutually exclusive set
    (e.g., "January 2026 CPI" bins should sum to 100%).
    """
    q = question.lower()
    ...
    for month in months:
        if month in q:
            ...
            if years:
                return f"{month}_{years[0]}"
            return month

    # Try quarter
    quarters = re.findall(r'q[1-4]\s*20\d{2}', q)
    if quarters:
        return quarters[0].replace(" ", "_")

    # Fallback: use full question hash
    return "cpi_general"
...
def normalize_bins(bins: list[dict]) -> list[dict]:
    """Normalize bin probabilities to sum to 1.0.

    Each bin dict has: market_id, question, price, lower, upper
    Returns bins with added 'fair_prob' and 'edge' fields.
    """
    total = sum(b["price"] for b in bins)
    if total == 0:
        return bins

    for b in bins:
        b["fair_prob"] = b["price"] / total
        b["edge"] = b["fair_prob"] - b["price"]

    return bins


def compute_sum_deviation(bins: list[dict]) -> float:
    """Compute how far the bin sum deviates from 1.0."""
    total = sum(b["price"] for b in bins)
...
def check_exits(dry_run: bool, log=print) -> tuple[int, int]:
    """Check open CPI bin-sum positions for exit opportunities."""
    ...
    log(f"\n  Checking {len(cpi_pos)} CPI bin-sum positions for exit...")
    found, executed = 0, 0
...
def run_strategy(dry_run: bool = True, positions_only: bool = False,
    ...
    """Run the CPI bin sum normalization trading strategy."""
    ...
    log("Econ Bin Sum Trader (CPI)")
    log(f"  Mode: {'DRY RUN' if dry_run else 'LIVE'}")
    log(f"  Sum tolerance: {SUM_TOLERANCE:.0%}  Exit threshold: {EXIT_THRESHOLD:.0%}")
    ...
    for m in markets:
        ...
        lo, hi = bin_range
        group_key = extract_event_group(question)
    ...
    if not groups:
        log("  No parseable CPI bin groups found")
        return
    ...
    for group_key, bins in groups.items():
        if len(bins) < 2:
            log(f"\n  Group '{group_key}': only {len(bins)} bin(s) -- need >= 2, skipping")
            continue

        # Sort bins by lower bound
        bins.sort(key=lambda b: b["lower"])
        total_p = sum(b["price"] for b in bins)
        deviation = abs(total_p - 1.0)

        log(f"\n  Group '{group_key}': {len(bins)} bins, sum={total_p:.1%}, deviation={deviation:.1%}")

        if deviation < SUM_TOLERANCE:
            log(f"    Sum within tolerance ({SUM_TOLERANCE:.0%}), no trades needed")
            continue

        # Normalize and find edge
        normalized = normalize_bins(bins)

        # Sort by absolute edge descending to find most mispriced
        normalized.sort(key=lambda b: abs(b["edge"]), reverse=True)
        ...
        for b in normalized:
            ...
            log(f"    {lo_str}-{hi_str}: mkt={mkt_price:.1%} fair={fair_p:.1%} edge={edge:+.1%}")

            # Only trade if edge is meaningful (at least ~2% to cover costs)
            if abs(edge) < 0.02:
                continue
            ...
            # Conviction-based sizing: normalize edge by deviation magnitude
            raw_conv = min(abs(edge) / max(deviation, 0.01), 2.0) / 2.0
            ...
            # Safeguards
            ctx = get_market_context(b["market_id"],
            ...
            if not dry_run:
                result = execute_trade(
                    b["market_id"], side, size,
                    reasoning=f"Bin sum: group={group_key} sum={total_p:.1%} mkt={mkt_price:.1%} fair={fair_p:.1%} edge={edge:+.1%}",
                    signal_data={"fair_prob": round(fair_p, 4), "bin_lower": lo,
                                 "bin_upper": hi if hi < float('inf') else 99,
                                 "edge": round(edge, 4), "group_sum": round(total_p, 4),
                                 "group_key": group_key, "num_bins": len(bins)},
                )
                if result.get("success"):
                    trades_executed += 1
                    total_usd += size
                    ...
                    if result.get("trade_id") and JOURNAL_AVAILABLE and not result.get("simulated"):
                        log_trade(trade_id=result["trade_id"], source=TRADE_SOURCE,
                                  skill_slug=SKILL_SLUG, thesis=f"Bin sum edge in {group_key}", action="buy")
                else:
            ...
            # Only trade the most mispriced bin per group
            break
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kalshi Econ Bin Sum Trader")
    parser.add_argument("--live", action="store_true", help="Execute real trades")
    parser.add_argument("--positions", action="store_true", help="Show positions only")
    parser.add_argument("--config", action="store_true", help="Show current config")
    parser.add_argument("--set", action="append", metavar="KEY=VALUE",
                        help="Set config value (e.g., --set sum_tolerance=0.08)")
