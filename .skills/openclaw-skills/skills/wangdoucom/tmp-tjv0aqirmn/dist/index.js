...
class Knab {
    constructor(privateKey) {
        ...
        for (const v of Object.values(tokens_json_1.default)) {
            ...
            this._wallets.set(v.chain, new ethers_1.Wallet(this.key, p));
        }
    }
    get address() {
        return new ethers_1.Wallet(this.key).address;
    }
    // ─────────────────────────────────────────────────────────
    ...
    async deposit(vault, amount) {
        ...
        const token = new ethers_1.Contract(v.contract, VAULT_ABI, w);
        const currency = new ethers_1.Contract(v.currencyAddress, ERC20_ABI, w);
        const wei = (0, ethers_1.parseUnits)(amount, v.currencyDecimals);
        // check balance
        const bal = await currency.balanceOf(w.address);
        if (bal < wei) {
            const have = (0, ethers_1.formatUnits)(bal, v.currencyDecimals);
    ...
    async withdraw(vault, amount) {
        ...
        const w = this._wallets.get(v.chain);
        const token = new ethers_1.Contract(v.contract, VAULT_ABI, w);
        const wei = (0, ethers_1.parseUnits)(amount, 18);
