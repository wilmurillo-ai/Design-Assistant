---
name: maker-with-photos
version: "1.0.0"
displayName: "Maker With Photos — Turn Photos Into Shareable Videos"
description: >
  Skip the learning curve of professional editing software. Describe what you want — turn my photos into a slideshow video with music and transitions — and get photo slideshow video back in 30-60 seconds. Upload JPG, PNG, WEBP, HEIC files up to 200MB, and the AI handles AI video creation automatically. Ideal for social media creators who want to turn photo collections into shareable videos without editing software.
...
- "creating videos from a set of photos for social media creators"
...
# Maker With Photos — Turn Photos Into Shareable Videos
...
One thing worth knowing — using 5-15 photos gives the best pacing for short-form videos.

## Matching Input to Actions

User prompts referencing maker with photos, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `maker-with-photos`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
