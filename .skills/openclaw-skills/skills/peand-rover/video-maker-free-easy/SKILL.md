---
name: video-maker-free-easy
version: "1.0.0"
displayName: "Video Maker Free Easy — Create and Export Videos Fast"
description: >
  Get finished MP4 video ready to post, without touching a single slider. Upload your images or clips (MP4, MOV, JPG, PNG, up to 500MB), say something like "turn these photos into a 30-second promo video with music and text", and download 1080p MP4 when it's done. Built for beginners and casual creators who move fast and want to make a video quickly without learning complex tools.
...
# Video Maker Free Easy — Create and Export Videos Fast
...
Tip: using 5-10 images keeps the process fast and the output tight.

## Matching Input to Actions

User prompts referencing video maker free easy, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `video-maker-free-easy`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
