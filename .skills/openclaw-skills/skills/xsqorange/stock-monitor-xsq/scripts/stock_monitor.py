...
def get_kline_eastmoney_hk(code: str, days: int = 60) -> List[Dict]:
    """使用东方财富获取港股K线数据（备用）"""
    import urllib.request

    try:
        if code.startswith("hk"):
            secid_code = code[2:].lstrip('0')
        else:
            secid_code = code.lstrip('0')
        if not secid_code:
            return None
        secid = f"116.{secid_code}"
        ...
        print(f"东方财富港股K线获取失败: {code}, {e}")
        return None

def get_kline_hk_fallback_from_quote(code: str) -> List[Dict]:
    """当K线API全部失败时，使用实时行情构造单日K线（最终保底）"""
...
def get_kline_tencent_hk(code: str, days: int = 60) -> List[Dict]:
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as r:
            ...
            if 'data' not in data:
                return _hk_kline_fallback(code)
            ...
            return result if result else _hk_kline_fallback(code)

    except Exception as e:
        print(f"腾讯港股K线获取失败: {code}, {e}")
        return _hk_kline_fallback(code)

def _hk_kline_fallback(code: str) -> List[Dict]:
    """港股K线降级：东方财富 → 实时行情保底"""
    result = get_kline_eastmoney_hk(code, 60)
    if result:
        print(f"[降级] {code} 腾讯K线切换至东方财富")
        return result
    result = get_kline_hk_fallback_from_quote(code)
    if result:
        print(f"[保底] {code} K线全部失败，使用实时行情")
    return result
...
def get_stock_quote_sina(code: str) -> Optional[Dict]:
    """获取股票实时行情 - 新浪API（备用数据源）"""
    import urllib.request
    import re
    ...
    try:
        ...
        print(f"新浪行情获取失败: {code}, {e}")
        return None

def get_stock_quote(code: str) -> Optional[Dict]:
    """获取股票实时行情 - 自动降级（腾讯优先，失败则新浪）"""
    # 优先腾讯
    result = get_stock_quote_tencent(code)
    if result and result.get('price', 0) > 0:
        return result
    # 降级新浪
    result = get_stock_quote_sina(code)
    if result and result.get('price', 0) > 0:
        print(f"[降级] {code} 腾讯数据异常，切换至新浪")
        return result
    # 都失败返回腾讯结果（可能为空）
    return result

def get_market_index_tencent() -> List[Dict]:
    """获取大盘指数实时行情 - 腾讯API（主数据源）"""
    import urllib.request
    import re
...
def get_market_index_sina() -> List[Dict]:
    """获取大盘指数实时行情 - 新浪API（备用数据源）"""
    import urllib.request
    import re
    ...
    for code, name in indices:
        try:
            ...
            print(f"新浪指数获取失败: {code}, {e}")
    return results

def get_market_index() -> List[Dict]:
    """获取大盘指数 - 自动降级（腾讯优先，失败则新浪）"""
    results = get_market_index_tencent()
    if results and len(results) >= 5:
        return results
    print("[降级] 腾讯指数数据异常，切换至新浪")
    return get_market_index_sina()
