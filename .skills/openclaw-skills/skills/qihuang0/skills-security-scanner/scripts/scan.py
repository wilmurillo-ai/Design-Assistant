...
class SkillScanner:
    def __init__(self, ak: str, sk: str, region: str = DEFAULT_REGION, host: str = DEFAULT_HOST, 
                 service: str = DEFAULT_SERVICE, session_token: str = "", user_id: str = "",
                 base_url: str = ""):
        self.ak = ak
        self.sk = sk
        self.region = region
        ...
        self.credentials = Credentials(ak, sk, service, region, session_token if session_token else "")
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if self.user_id:
                        v_request.headers['UserID'] = self.user_id
                    
                    SignerV4.sign(v_request, self.credentials)

                    response = requests.post(self.upload_url, headers=v_request.headers, data=prepped.body)
    ...
    def get_scan_detail(self, skill_id: str):
        ...
        for i in range(max_retries):
            try:
                ...
                response = requests.post(self.detail_url, data=payload, headers=v_request.headers)
                response.raise_for_status()
...
def main():
    ...
    session_token = os.environ.get("VOLC_SESSION_TOKEN", "")
    user_id = os.environ.get("VOLC_USERID", "")
    ...
    if base_url:
        ...
        scanner = SkillScanner(ak, sk, region, host, service, session_token, user_id)
