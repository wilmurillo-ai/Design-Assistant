...
NAVI_OFFICE_API_TOKEN = os.environ.get("NAVI_OFFICE_API_TOKEN", "")
NAVI_OFFICE_API_URL = os.environ.get("NAVI_OFFICE_API_URL", "https://oa.teredy.com/api")
NAVI_OFFICE_ALLOW_CUSTOM_DOMAIN = os.environ.get("NAVI_OFFICE_ALLOW_CUSTOM_DOMAIN", "false")
...
def validate_domain() -> None:
    """校验 API 域名，默认只允许官方域名"""
    try:
        hostname = parse.urlparse(NAVI_OFFICE_API_URL).hostname or ""
    except Exception:
        die("NAVI_OFFICE_API_URL 格式不正确")

    allow_custom = str(NAVI_OFFICE_ALLOW_CUSTOM_DOMAIN).lower() == "true"
    if not allow_custom and hostname != "https://oa.teredy.com/api":
        die("NAVI_OFFICE_API_URL 非官方域名。若确认可信，请设置 NAVI_OFFICE_ALLOW_CUSTOM_DOMAIN=true")


def check_keys() -> None:
    """检查必需的 API TOKEN"""
    if not NAVI_OFFICE_API_TOKEN:
...
def api_request(method: str, url: str, content_type: str, **kwargs) -> str:
    ...
    headers = {
        "X-Api-Token": NAVI_OFFICE_API_TOKEN,
...
def crm_list(module: str, opts: str = "") -> str:
    ...
    return api("GET", f"{NAVI_OFFICE_API_URL}/{module}/view/list", params=params)


def crm_get(module: str, id: str) -> str:
    """获取单条记录详情"""
    return api("GET", f"{NAVI_OFFICE_API_URL}/{module}/{id}")


def crm_contact(module: str, id: str) -> str:
    """获取联系人列表"""
    return api("GET", f"{NAVI_OFFICE_API_URL}/{module}/contact/list/{id}")
...
def crm_page(module: str, payload_or_keyword: str = "") -> str:
    ...
    path = f"{module}/page"
    return api("POST", f"{NAVI_OFFICE_API_URL}/{path}", data=body)


def crm_search(module: str, json_data: str = "") -> str:
    """全局搜索记录"""
    path = f"global/search/{module}"
    return api("POST", f"{NAVI_OFFICE_API_URL}/{path}", data=json_data)
...
def crm_follow_page(kind: str, module: str, payload: str = "") -> str:
    ...
    return api("POST", f"{NAVI_OFFICE_API_URL}/{module}/follow/{kind}/page", data=body)
...
def crm_product(keyword: str = "") -> str:
    ...
    return api("POST", f"{NAVI_OFFICE_API_URL}/field/source/product", data=body)


def crm_org() -> str:
    """获取组织架构"""
    return api("GET", f"{NAVI_OFFICE_API_URL}/department/tree")


def crm_members(json_data: str) -> str:
    """根据部门ID获取成员"""
    return api("POST", f"{NAVI_OFFICE_API_URL}/user/list", data=json_data)
...
def raw_api(method: str, path: str, *args) -> str:
    ...
    if path.startswith("http"):
        ...
        url = f"{NAVI_OFFICE_API_URL}{path}"
...
def print_usage():
    """打印使用帮助"""
    usage_text = """
navioffice — NAVIOFFICE CRM CLI 工具（X-Api-Token 模式）
...
列表查询示例:
  ...
  navioffice crm page lead
  navioffice crm page lead "测试"
  navioffice crm page lead '{"current":1,"pageSize":30,"sort":{},"combineSearch":{"searchMode":"AND","conditions":[]},"keyword":"","viewId":"ALL","filters":[]}'
  navioffice crm page contract/payment-plan '{"current":1,"pageSize":30,"sort":{},"combineSearch":{"searchMode":"AND","conditions":[]},"keyword":"","viewId":"ALL","filters":[]}'
  navioffice crm search account '{"current":1,"pageSize":30,"combineSearch":{"searchMode":"AND","conditions":[]},"keyword":"xyz","viewId":"ALL","filters":[]}'
  navioffice crm org
  navioffice crm members '{"current":1,"pageSize":30,"combineSearch":{"searchMode":"AND","conditions":[]},"keyword":"","departmentIds":["deptId1","deptId2"],"filters":[]}'
  navioffice crm follow plan lead '{"sourceId":"927627065163785","current":1,"pageSize":10,"keyword":"","status":"ALL","myPlan":false}'
  navioffice crm follow record account '{"sourceId":"1751888184018919","current":1,"pageSize":10,"keyword":"","myPlan":false}'
  navioffice crm product "测试"
  navioffice crm contact account '927627065163785'
...
列表查询示例：
  navioffice crm page contract/payment-plan
  navioffice crm page contract/business-title
...
环境变量要求:
  ...
  NAVI_OFFICE_API_URL（可选，默认 https://oa.teredy.com/api）
...
def main():
    ...
    if cmd == "crm":
        ...
        die(f"未知命令: {cmd}（尝试 navioffice.py help）")
