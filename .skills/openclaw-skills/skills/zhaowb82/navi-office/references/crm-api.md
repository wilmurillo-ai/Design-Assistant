# NAVIOFFICE CRM API 参考

此文档聚焦 NaviOffice CRM CLI 背后的原始 API，帮助 OpenClaw 理解请求结构、标准参数、模块定义、错误处理和最佳实践。
无论是让 OpenClaw 助手自动构建 `navioffice crm` 命令，还是自己发起 `navioffice raw` 请求，都能从这里快速查到细节。
...
## 2. 通用请求结构
NaviOffice CRM 的分页和搜索均遵循以下 JSON 模板：
...
> `navioffice raw {METHOD} {PATH}` 就是让你任意组合上述请求，并手动填写 body/headers。
...
### 分页列出商机（默认结构）
```bash
navioffice.sh crm page opportunity "{\"current\":1,\"pageSize\":20,\"keyword\":\"线索\"}"
```
会调用 `POST /opportunity/page`，body 同上。

### 二级模块支持
NaviOffice CRM 里有一些隐藏在 `contract`｜ `opportunity` 模块下的二级资源（比如回款计划、发票等），`navioffice` CLI 通过接受包含斜杠路径的模块名来访问它们。

- `navioffice crm page contract/payment-plan`：查询回款计划的分页列表，支持传入关键词/JSON body，实际上调用的是 `POST /contract/payment-plan/page`。
- `navioffice crm page invoice`：查询发票的分页列表，通过 `POST /invoice/page` 获取，每个条件都可以通过 `filters` 精细控制。
- `navioffice crm page contract/business-title`：检索工商抬头列表，同样支持关键词/filters。
- `navioffice crm page contract/payment-record`：查看回款记录列表，可结合关键词、`filters` 或 `viewId` 进行精细筛选。
- `navioffice crm page opportunity/quotation`：查看报价单列表，可结合关键词、`filters` 或 `viewId` 进行精细筛选。

对这些二级模块的查询依旧遵循 `page_payload` 结构（`current`/`pageSize`/`sort`/`filters`）和关键字补全，因此你只需提供想要筛选的字段，AI 会自动补上分页元数据。

需要更专业的筛选能力时，可以直接把完整 JSON body 透传给 `navioffice crm page contract/payment-plan '{…}'`，也可以用 `navioffice raw` 指定路径（例如 `navioffice raw POST /contract/payment-record/page '{...}'`）来跳过 CLI 结构化限制。

### 高级 search（带 filters + sort）
```bash
navioffice.sh crm search account '{
  "current":1,
  "pageSize":40,
  "keyword":"云",
...
### 获取某条记录
```
navioffice crm get lead 987654321
```
...
### 跟进计划/记录请求示例
```bash
navioffice.sh crm raw POST /lead/follow/record/page '{"sourceId":"927627065163785","current":1,"pageSize":10,"keyword":"回访"}'
navioffice.sh crm raw POST /account/follow/plan/page '{"sourceId":"1751888184018919","current":1,"pageSize":10,"status":"ALL","myPlan":false}'
```
...
## 6. 错误处理建议
1. **Token/密钥错误**：`INVALID_KEY`、`ACCESS_DENIED` → 检查 `NAVI_OFFICE_API_TOKEN` 是否正确、是否过期。
...
```
更多字段可以在 CLI 输出的 `moduleFields` 里查看或用 `navioffice raw GET /settings/fields?module={module}` 查询。
