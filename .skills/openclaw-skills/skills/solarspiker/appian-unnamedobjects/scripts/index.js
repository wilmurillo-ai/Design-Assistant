...
function parseEntry(xml, entryPath) {
    ...
    // Description — <description> or <a:description>, with optional CDATA.
    // Returns null when tag is absent, '' when present but empty.
    const descM = xml.match(
        /<(?:a:)?description(?:\s[^>]*)?>(?:<!\[CDATA\[([\s\S]*?)\]\]>|([^<]*))<\/(?:a:)?description>/i
    );
    const description = descM !== null ? (descM[1] ?? descM[2] ?? '').trim() : null;

    return { type, uuid, name, description };
}

// ── Main ───────────────────────────────────────────────────────────────────────

async function findUnnamedObjects(applicationUuid) {
    // Phase 1 — credentials
    ...
    process.stderr.write(`Scanning ${xmlFiles.length} XML file(s) for missing descriptions...\n`);

    const missing = [];
    for (const f of xmlFiles) {
        const obj = parseEntry(f.content, f.name);
        if (obj.description === null || obj.description === '') {
            missing.push({ type: obj.type, name: obj.name, uuid: obj.uuid });
        }
    }

    if (missing.length === 0) {
        console.log('✓ All objects have descriptions — nothing to report.');
    } else {
        console.log(`Found ${missing.length} object(s) missing a description:\n`);

        for (const o of missing) {
            console.log(`[${o.type}] ${o.name || '(no name)'} | ${o.uuid}`);
        }
...
findUnnamedObjects(applicationUuid)
    .then(() => {})
