...
# Auto-create empty ref.json files if missing (prevents Retriever crash)
_project_root = Path(__file__).parent
for _task in ["diagram", "plot"]:
    _ref_dir = _project_root / "data" / "PaperBananaBench" / _task
    _ref_file = _ref_dir / "ref.json"
    if not _ref_file.exists():
        _ref_dir.mkdir(parents=True, exist_ok=True)
        _ref_file.write_text("[]")
...
def main():
    ...
    with tab1:
        ...
        with st.sidebar:
            ...
            default_model = get_config_val("defaults", "main_model_name", "MAIN_MODEL_NAME", "qwen-vl-max")
            text_model_presets = [default_model] if default_model else ["qwen-vl-max"]
            if "qwen-vl-plus" not in text_model_presets:
                text_model_presets.append("qwen-vl-plus")
            if "qwen-vl-max" not in text_model_presets:
                text_model_presets.insert(0, "qwen-vl-max")
            ...
            default_image_model = get_config_val("defaults", "image_gen_model_name", "IMAGE_GEN_MODEL_NAME", "wanx2.1-t2i-turbo")
            image_model_presets = [default_image_model] if default_image_model else ["wanx2.1-t2i-turbo"]
            if "wanx2.1-t2i-plus" not in image_model_presets:
                image_model_presets.append("wanx2.1-t2i-plus")
            if "wanx2.1-t2i-turbo" not in image_model_presets:
                image_model_presets.insert(0, "wanx2.1-t2i-turbo")
            ...
            if image_model_selection == "Custom":
                image_gen_model_name = st.text_input(
                    ...
                    placeholder="e.g., wanx2.1-t2i-turbo"
                )
            else:
