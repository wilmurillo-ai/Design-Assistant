...
if openai_api_key:
    dashscope_base_url = get_config_val("dashscope", "base_url", "DASHSCOPE_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")
    openai_client = AsyncOpenAI(api_key=openai_api_key, base_url=dashscope_base_url)
...
async def call_model_with_retry_async(
    ...
    elif any(model_name.startswith(p) for p in ("gpt-", "o1-", "o3-", "o4-", "qwen")):
        provider = "openai"
        actual_model = model_name
    else:
...
# ─── DashScope native async image generation (for wanx / Wan models) ───
import httpx as _httpx_sync
import json as _json_mod

async def call_dashscope_image_generation_async(
    prompt: str,
    model_name: str = "",
    size: str = "1024*1024",
    max_attempts: int = 5,
    ...
    """
    Call DashScope native async API for image generation (Wan/wanx models).
    Returns a list with one base64 PNG string, or None on failure.
    """
    import asyncio, base64
    if not model_name:
        model_name = get_config_val("dashscope", "image_gen_model_name", "DASHSCOPE_IMAGE_MODEL", "wanx2.1-t2i-turbo")
    api_key = openai_api_key  # reuse the key loaded at module init

    dashscope_api_base = get_config_val("dashscope", "api_base", "DASHSCOPE_API_BASE", "https://dashscope.aliyuncs.com")
    submit_url = f"{dashscope_api_base}/api/v1/services/aigc/text2image/image-synthesis"
    headers = {
    ...
    for attempt in range(max_attempts):
        try:
            async with _httpx_sync.AsyncClient(timeout=60) as client:
                # Step 1: Submit task
                resp = await client.post(submit_url, headers=headers, json=payload)
                result = resp.json()
                task_id = result.get("output", {}).get("task_id")
                if not task_id:
                    print(f"[DashScope IMG] No task_id returned: {result}")
                    if attempt < max_attempts - 1:
                        await asyncio.sleep(retry_delay)
                        continue
                    return None
                ...
                poll_url = f"{dashscope_api_base}/api/v1/tasks/{task_id}"
                poll_headers = {"Authorization": f"Bearer {api_key}"}
                for _ in range(60):  # max ~3 minutes
                    await asyncio.sleep(3)
                    check = await client.get(poll_url, headers=poll_headers, timeout=30)
                    ...
                    if task_status == "SUCCEEDED":
                        ...
                        print(f"[DashScope IMG] Task failed: {status_data}")
                        break
                # If we get here, task timed out or failed
                if attempt < max_attempts - 1:
                    print(f"[DashScope IMG] Attempt {attempt+1} failed, retrying...")
                    await asyncio.sleep(retry_delay)
        except Exception as e:
            print(f"[DashScope IMG] Error on attempt {attempt+1}: {e}")
            if attempt < max_attempts - 1:
                await asyncio.sleep(retry_delay)
    return None
