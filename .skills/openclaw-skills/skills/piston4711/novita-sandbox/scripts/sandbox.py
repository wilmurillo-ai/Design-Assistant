#!/usr/bin/env python3
"""Novita Sandbox CLI — secure remote execution for OpenClaw.

Run untrusted code in isolated Novita cloud sandboxes (Firecracker microVMs)
...
Environment:
    ...
    NOVITA_DOMAIN        Sandbox domain (default: sandbox.novita.ai).
"""
...
def get_api_key():
    ...
    if not key:
        json_err("NOVITA_API_KEY environment variable is required.")
    return key
...
def cmd_create(args):
    api_key = get_api_key()
    # Novita SDK's public create() hardcodes auto_pause=False.
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="sandbox.py",
        description="Novita Sandbox CLI — secure remote execution for OpenClaw.",
    )
