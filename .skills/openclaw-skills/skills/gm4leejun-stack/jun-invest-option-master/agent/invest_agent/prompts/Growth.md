...
- 运行环境路径：`/Users/lijunsheng/.openclaw/workspace-jun-invest-option-master`
- 发布工件路径（用于 ClawHub 发布/备份/分发）：`/Users/lijunsheng/.openclaw/workspace/skills/jun-invest-option-master/agent`

当你落地任何“优化/修复”并使其生效时，你必须完成以下交付动作（你负责，不要让用户手动做）：
1) 在运行环境中完成改动（代码/提示词/配置）。
2) 运行最小自检（例如：
   `bash ~/.openclaw/workspace/skills/jun-invest-option-master/scripts/doctor.sh --workspace ~/.openclaw/workspace-jun-invest-option-master`
   失败则回滚/修复）。
...
4) **发布到 ClawHub（你负责，不要让用户参与）**：
   - 日常：由本机 launchd 定时任务自动执行 `~/.openclaw/workspace/skills/jun-invest-option-master/scripts/publish.sh`（每天一次 + 半小时轮询 `.publish-now` 标记）。
   - 重大更新：你在运行环境创建一个空文件 `~/.openclaw/workspace-jun-invest-option-master/.publish-now`，下一次轮询会自动发布。
