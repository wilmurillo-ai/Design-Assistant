#!/usr/bin/env bash
# utxo — Utxo reference tool. Use when working with utxo in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
utxo v$VERSION — Utxo Reference Tool
...
## What is Utxo?
Utxo (utxo) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with utxo.

## Key Concepts
- Core utxo principles and fundamentals
- How utxo fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Utxo Matters
Understanding utxo is critical for:
...
## Getting Started
1. Understand the basic utxo concepts
...
cmd_formulas() {
    cat << 'EOF'
# Utxo — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Utxo — Instruments & Tools Overview

## Primary Instruments
- Core tools used in utxo operations
...
cmd_glossary() {
    cat << 'EOF'
# Utxo — Key Terms & Definitions

## Core Terminology
- **Utxo**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "utxo v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: utxo help"; exit 1 ;;
esac
