#!/usr/bin/env bash
# policy — Policy reference tool. Use when working with policy in security contexts.
...
show_help() {
    cat << 'HELPEOF'
policy v$VERSION — Policy Reference Tool
...
## What is Policy?
Policy (policy) is a specialized tool/concept in the security domain.
It provides essential capabilities for professionals working with policy.

## Key Concepts
- Core policy principles and fundamentals
- How policy fits into the broader security ecosystem  
- Essential terminology every practitioner should know

## Why Policy Matters
Understanding policy is critical for:
...
## Getting Started
1. Understand the basic policy concepts
...
## Installation
1. Download or clone the policy package
...
cmd_patterns() {
    cat << 'EOF'
# Policy — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for policy
...
cmd_migration() {
    cat << 'EOF'
# Policy — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "policy v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: policy help"; exit 1 ;;
esac
