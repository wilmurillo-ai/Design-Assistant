#!/usr/bin/env bash
# iperf — Iperf reference tool. Use when working with iperf in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
iperf v$VERSION — Iperf Reference Tool
...
## What is Iperf?
Iperf (iperf) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with iperf.

## Key Concepts
- Core iperf principles and fundamentals
- How iperf fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Iperf Matters
Understanding iperf is critical for:
...
## Getting Started
1. Understand the basic iperf concepts
...
## Installation
1. Download or clone the iperf package
...
cmd_patterns() {
    cat << 'EOF'
# Iperf — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for iperf
...
cmd_performance() {
    cat << 'EOF'
# Iperf — Performance Optimization
...
case "$CMD" in
    ...
    version|--version|-v) echo "iperf v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: iperf help"; exit 1 ;;
esac
