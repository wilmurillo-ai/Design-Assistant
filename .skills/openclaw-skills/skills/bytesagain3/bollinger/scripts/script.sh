#!/usr/bin/env bash
# bollinger — Bollinger reference tool. Use when working with bollinger in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
bollinger v$VERSION — Bollinger Reference Tool

Usage: bollinger <command>
...
## What is Bollinger?
Bollinger (bollinger) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with bollinger.

## Key Concepts
- Core bollinger principles and fundamentals
- How bollinger fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Bollinger Matters
Understanding bollinger is critical for:
...
## Getting Started
1. Understand the basic bollinger concepts
...
cmd_formulas() {
    cat << 'EOF'
# Bollinger — Key Formulas & Calculations
...
cmd_regulations() {
    cat << 'EOF'
# Bollinger — Regulatory Framework
...
cmd_instruments() {
    cat << 'EOF'
# Bollinger — Instruments & Tools Overview

## Primary Instruments
- Core tools used in bollinger operations
...
cmd_glossary() {
    cat << 'EOF'
# Bollinger — Key Terms & Definitions

## Core Terminology
- **Bollinger**: The primary subject of this reference
...
cmd_checklist() {
    cat << 'EOF'
# Bollinger — Inspection Checklist
...
case "$CMD" in
    ...
    version|--version|-v) echo "bollinger v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: bollinger help"; exit 1 ;;
esac
