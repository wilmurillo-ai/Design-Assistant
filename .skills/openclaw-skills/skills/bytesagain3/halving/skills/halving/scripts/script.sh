#!/usr/bin/env bash
# halving — Halving reference tool. Use when working with halving in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
halving v$VERSION — Halving Reference Tool

Usage: halving <command>
...
## What is Halving?
Halving (halving) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with halving.

## Key Concepts
- Core halving principles and fundamentals
- How halving fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Halving Matters
Understanding halving is critical for:
...
## Getting Started
1. Understand the basic halving concepts
...
cmd_formulas() {
    cat << 'EOF'
# Halving — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Halving — Instruments & Tools Overview

## Primary Instruments
- Core tools used in halving operations
...
cmd_glossary() {
    cat << 'EOF'
# Halving — Key Terms & Definitions

## Core Terminology
- **Halving**: The primary subject of this reference
