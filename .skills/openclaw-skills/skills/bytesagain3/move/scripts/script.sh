#!/usr/bin/env bash
# move — Move reference tool. Use when working with move in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
move v$VERSION — Move Reference Tool
...
## What is Move?
Move (move) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with move.

## Key Concepts
- Core move principles and fundamentals
- How move fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Move Matters
Understanding move is critical for:
...
## Getting Started
1. Understand the basic move concepts
...
cmd_formulas() {
    cat << 'EOF'
# Move — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Move — Instruments & Tools Overview

## Primary Instruments
- Core tools used in move operations
...
cmd_glossary() {
    cat << 'EOF'
# Move — Key Terms & Definitions

## Core Terminology
- **Move**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "move v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: move help"; exit 1 ;;
esac
