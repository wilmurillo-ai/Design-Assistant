#!/usr/bin/env bash
# thesis-helper - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${THESIS_HELPER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/thesis-helper}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
thesis-helper v$VERSION

Multi-purpose utility tool

Usage: thesis-helper <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "thesis-helper v$VERSION" ;;
