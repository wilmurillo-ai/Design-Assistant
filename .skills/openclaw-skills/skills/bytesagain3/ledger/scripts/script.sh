#!/usr/bin/env bash
# ledger -- Calculate ledger financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LEDGER_DIR:-$HOME/.ledger}"
...
show_help() {
    cat << EOF
ledger v$VERSION -- Calculate ledger financial metrics and business data. Use when tracking expenses, analyzing investments, or generating financial reports.
...
cmd_add() {
    local value="${1:?Usage: ledger add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: ledger search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: ledger remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: ledger help"; exit 1 ;;
esac
