#!/usr/bin/env bash
# load-balancer — Load Balancer reference tool. Use when working with load balancer in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
load-balancer v$VERSION — Load Balancer Reference Tool

Usage: load-balancer <command>
...
Load Balancer (load-balancer) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with load balancer.

## Key Concepts
- Core load balancer principles and fundamentals
- How load balancer fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Load Balancer Matters
Understanding load balancer is critical for:
...
## Getting Started
1. Understand the basic load balancer concepts
...
## Installation
1. Download or clone the load balancer package
...
cmd_patterns() {
    cat << 'EOF'
# Load Balancer — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for load balancer
...
cmd_migration() {
    cat << 'EOF'
# Load Balancer — Migration & Upgrade Guide
