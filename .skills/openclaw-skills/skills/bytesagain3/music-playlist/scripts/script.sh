#!/usr/bin/env bash
# music-playlist - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${MUSIC_PLAYLIST_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/music-playlist}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
music-playlist v$VERSION

Multi-purpose utility tool

Usage: music-playlist <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "music-playlist v$VERSION" ;;
