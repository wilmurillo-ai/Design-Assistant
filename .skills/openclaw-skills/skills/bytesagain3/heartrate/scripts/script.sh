#!/usr/bin/env bash
# Heartrate — health tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

DATA_DIR="${HOME}/.local/share/heartrate"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "heartrate v2.0.0"; }

_help() {
    echo "Heartrate v2.0.0 — health toolkit"
    echo ""
    echo "Usage: heartrate <command> [args]"
    echo ""
    echo "Commands:"
    echo "  log                Log"
    echo "  track              Track"
...
case "${1:-help}" in
    log)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/log.log" 2>/dev/null || echo "  No entries yet. Use: heartrate log <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/log.log"
            local total=$(wc -l < "$DATA_DIR/log.log")
            echo "  [Heartrate] log: $input"
    ...
    track)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/track.log" 2>/dev/null || echo "  No entries yet. Use: heartrate track <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/track.log"
            local total=$(wc -l < "$DATA_DIR/track.log")
            echo "  [Heartrate] track: $input"
    ...
    chart)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/chart.log" 2>/dev/null || echo "  No entries yet. Use: heartrate chart <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/chart.log"
            local total=$(wc -l < "$DATA_DIR/chart.log")
            echo "  [Heartrate] chart: $input"
    ...
    goal)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/goal.log" 2>/dev/null || echo "  No entries yet. Use: heartrate goal <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/goal.log"
            local total=$(wc -l < "$DATA_DIR/goal.log")
            echo "  [Heartrate] goal: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: heartrate remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Heartrate] remind: $input"
    ...
    weekly)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/weekly.log" 2>/dev/null || echo "  No entries yet. Use: heartrate weekly <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/weekly.log"
            local total=$(wc -l < "$DATA_DIR/weekly.log")
            echo "  [Heartrate] weekly: $input"
    ...
    monthly)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/monthly.log" 2>/dev/null || echo "  No entries yet. Use: heartrate monthly <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/monthly.log"
            local total=$(wc -l < "$DATA_DIR/monthly.log")
            echo "  [Heartrate] monthly: $input"
    ...
    compare)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/compare.log" 2>/dev/null || echo "  No entries yet. Use: heartrate compare <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/compare.log"
            local total=$(wc -l < "$DATA_DIR/compare.log")
            echo "  [Heartrate] compare: $input"
    ...
    export)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export.log" 2>/dev/null || echo "  No entries yet. Use: heartrate export <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export.log"
            local total=$(wc -l < "$DATA_DIR/export.log")
            echo "  [Heartrate] export: $input"
    ...
    streak)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/streak.log" 2>/dev/null || echo "  No entries yet. Use: heartrate streak <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/streak.log"
            local total=$(wc -l < "$DATA_DIR/streak.log")
            echo "  [Heartrate] streak: $input"
    ...
    milestone)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/milestone.log" 2>/dev/null || echo "  No entries yet. Use: heartrate milestone <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/milestone.log"
            local total=$(wc -l < "$DATA_DIR/milestone.log")
            echo "  [Heartrate] milestone: $input"
    ...
    trend)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/trend.log" 2>/dev/null || echo "  No entries yet. Use: heartrate trend <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/trend.log"
            local total=$(wc -l < "$DATA_DIR/trend.log")
            echo "  [Heartrate] trend: $input"
    ...
    *)
        echo "Unknown: $1 — run 'heartrate help'"
        exit 1
        ;;
esac