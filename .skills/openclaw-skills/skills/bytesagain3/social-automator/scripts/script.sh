#!/usr/bin/env bash
# social-automator — Social Automator reference tool. Use when working with social automator in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
social-automator v$VERSION — Social Automator Reference Tool

Usage: social-automator <command>
...
cmd_intro() {
    cat << 'EOF'
# Social Automator — Overview

## What is Social Automator?
Social Automator (social-automator) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with social automator.

## Key Concepts
- Core social automator principles and fundamentals
- How social automator fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Social Automator Matters
Understanding social automator is critical for:
...
## Getting Started
1. Understand the basic social automator concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Social Automator — Quick Start Guide
...
## Installation
1. Download or clone the social automator package
...
cmd_patterns() {
    cat << 'EOF'
# Social Automator — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for social automator
...
cmd_debugging() {
    cat << 'EOF'
# Social Automator — Debugging Guide
...
cmd_performance() {
    cat << 'EOF'
# Social Automator — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Social Automator — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Social Automator — Migration & Upgrade Guide
...
cmd_cheatsheet() {
    cat << 'EOF'
# Social Automator — Quick Reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "social-automator v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: social-automator help"; exit 1 ;;
esac
