#!/usr/bin/env bash
# switchgear -- Switchgear specification manager
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${SWITCHGEAR_DIR:-$HOME/.switchgear}"
...
show_help() {
    cat << EOF
switchgear v$VERSION -- Switchgear specification manager

Usage: switchgear <command> [args]
...
cmd_add() {
    local value="${1:?Usage: switchgear add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: switchgear search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: switchgear remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="switchgear-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "switchgear v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: switchgear help"; exit 1 ;;
esac
