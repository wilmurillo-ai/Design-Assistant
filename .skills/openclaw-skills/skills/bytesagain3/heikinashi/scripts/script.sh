#!/usr/bin/env bash
# heikinashi — Heikinashi reference tool. Use when working with heikinashi in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
heikinashi v$VERSION — Heikinashi Reference Tool

Usage: heikinashi <command>
...
## What is Heikinashi?
Heikinashi (heikinashi) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with heikinashi.

## Key Concepts
- Core heikinashi principles and fundamentals
- How heikinashi fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Heikinashi Matters
Understanding heikinashi is critical for:
...
## Getting Started
1. Understand the basic heikinashi concepts
...
cmd_formulas() {
    cat << 'EOF'
# Heikinashi — Key Formulas & Calculations
...
cmd_regulations() {
    cat << 'EOF'
# Heikinashi — Regulatory Framework
...
cmd_instruments() {
    cat << 'EOF'
# Heikinashi — Instruments & Tools Overview

## Primary Instruments
- Core tools used in heikinashi operations
...
cmd_glossary() {
    cat << 'EOF'
# Heikinashi — Key Terms & Definitions

## Core Terminology
- **Heikinashi**: The primary subject of this reference
...
cmd_checklist() {
    cat << 'EOF'
# Heikinashi — Inspection Checklist
...
case "$CMD" in
    ...
    version|--version|-v) echo "heikinashi v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: heikinashi help"; exit 1 ;;
esac
