...
DATA_DIR="${HOME}/.local/share/dice"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "dice v2.0.0"; }

_help() {
    echo "Dice v2.0.0 — gaming toolkit"
    echo ""
    echo "Usage: dice <command> [args]"
    echo ""
    echo "Commands:"
    echo "  roll               Roll"
    echo "  score              Score"
...
case "${1:-help}" in
    roll)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/roll.log" 2>/dev/null || echo "  No entries yet. Use: dice roll <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/roll.log"
            local total=$(wc -l < "$DATA_DIR/roll.log")
            echo "  [Dice] roll: $input"
    ...
    score)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/score.log" 2>/dev/null || echo "  No entries yet. Use: dice score <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/score.log"
            local total=$(wc -l < "$DATA_DIR/score.log")
            echo "  [Dice] score: $input"
    ...
    rank)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/rank.log" 2>/dev/null || echo "  No entries yet. Use: dice rank <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/rank.log"
            local total=$(wc -l < "$DATA_DIR/rank.log")
            echo "  [Dice] rank: $input"
    ...
    history)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/history.log" 2>/dev/null || echo "  No entries yet. Use: dice history <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/history.log"
            local total=$(wc -l < "$DATA_DIR/history.log")
            echo "  [Dice] history: $input"
    ...
    stats)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/stats.log" 2>/dev/null || echo "  No entries yet. Use: dice stats <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/stats.log"
            local total=$(wc -l < "$DATA_DIR/stats.log")
            echo "  [Dice] stats: $input"
    ...
    challenge)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/challenge.log" 2>/dev/null || echo "  No entries yet. Use: dice challenge <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/challenge.log"
            local total=$(wc -l < "$DATA_DIR/challenge.log")
            echo "  [Dice] challenge: $input"
    ...
    create)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/create.log" 2>/dev/null || echo "  No entries yet. Use: dice create <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/create.log"
            local total=$(wc -l < "$DATA_DIR/create.log")
            echo "  [Dice] create: $input"
    ...
    join)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/join.log" 2>/dev/null || echo "  No entries yet. Use: dice join <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/join.log"
            local total=$(wc -l < "$DATA_DIR/join.log")
            echo "  [Dice] join: $input"
    ...
    track)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/track.log" 2>/dev/null || echo "  No entries yet. Use: dice track <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/track.log"
            local total=$(wc -l < "$DATA_DIR/track.log")
            echo "  [Dice] track: $input"
    ...
    leaderboard)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/leaderboard.log" 2>/dev/null || echo "  No entries yet. Use: dice leaderboard <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/leaderboard.log"
            local total=$(wc -l < "$DATA_DIR/leaderboard.log")
            echo "  [Dice] leaderboard: $input"
    ...
    reward)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/reward.log" 2>/dev/null || echo "  No entries yet. Use: dice reward <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/reward.log"
            local total=$(wc -l < "$DATA_DIR/reward.log")
            echo "  [Dice] reward: $input"
    ...
    reset)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/reset.log" 2>/dev/null || echo "  No entries yet. Use: dice reset <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/reset.log"
            local total=$(wc -l < "$DATA_DIR/reset.log")
            echo "  [Dice] reset: $input"
