#!/usr/bin/env bash
# ui-component - Design reference and helper tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${UI_COMPONENT_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/ui-component}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
ui-component v$VERSION

Design reference and helper tool

Usage: ui-component <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "ui-component v$VERSION" ;;
