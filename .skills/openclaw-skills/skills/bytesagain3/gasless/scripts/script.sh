#!/usr/bin/env bash
# gasless -- Analyze gasless operations. Use when you need to understand gasless mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${GASLESS_DIR:-$HOME/.gasless}"
...
show_help() {
    cat << EOF
gasless v$VERSION -- Analyze gasless operations. Use when you need to understand gasless mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: gasless <command> [args]
...
cmd_add() {
    local value="${1:?Usage: gasless add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: gasless search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: gasless remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "gasless v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: gasless help"; exit 1 ;;
esac
