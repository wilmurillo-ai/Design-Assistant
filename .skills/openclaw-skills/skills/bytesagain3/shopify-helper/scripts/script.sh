#!/usr/bin/env bash
# shopify-helper - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${SHOPIFY_HELPER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/shopify-helper}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
shopify-helper v$VERSION

Multi-purpose utility tool

Usage: shopify-helper <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "shopify-helper v$VERSION" ;;
