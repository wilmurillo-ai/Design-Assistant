...
DATA_DIR="${HOME}/.local/share/furniture"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "furniture v2.0.0"; }

_help() {
    echo "Furniture v2.0.0 — home toolkit"
    echo ""
    echo "Usage: furniture <command> [args]"
    echo ""
    echo "Commands:"
    echo "  add                Add"
...
case "${1:-help}" in
    add)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/add.log" 2>/dev/null || echo "  No entries yet. Use: furniture add <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/add.log"
            local total=$(wc -l < "$DATA_DIR/add.log")
            echo "  [Furniture] add: $input"
    ...
    inventory)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/inventory.log" 2>/dev/null || echo "  No entries yet. Use: furniture inventory <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/inventory.log"
            local total=$(wc -l < "$DATA_DIR/inventory.log")
            echo "  [Furniture] inventory: $input"
    ...
    schedule)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/schedule.log" 2>/dev/null || echo "  No entries yet. Use: furniture schedule <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/schedule.log"
            local total=$(wc -l < "$DATA_DIR/schedule.log")
            echo "  [Furniture] schedule: $input"
    ...
    remind)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/remind.log" 2>/dev/null || echo "  No entries yet. Use: furniture remind <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/remind.log"
            local total=$(wc -l < "$DATA_DIR/remind.log")
            echo "  [Furniture] remind: $input"
    ...
    checklist)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/checklist.log" 2>/dev/null || echo "  No entries yet. Use: furniture checklist <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/checklist.log"
            local total=$(wc -l < "$DATA_DIR/checklist.log")
            echo "  [Furniture] checklist: $input"
    ...
    usage)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/usage.log" 2>/dev/null || echo "  No entries yet. Use: furniture usage <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/usage.log"
            local total=$(wc -l < "$DATA_DIR/usage.log")
            echo "  [Furniture] usage: $input"
    ...
    cost)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cost.log" 2>/dev/null || echo "  No entries yet. Use: furniture cost <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/cost.log"
            local total=$(wc -l < "$DATA_DIR/cost.log")
            echo "  [Furniture] cost: $input"
    ...
    maintain)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/maintain.log" 2>/dev/null || echo "  No entries yet. Use: furniture maintain <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/maintain.log"
            local total=$(wc -l < "$DATA_DIR/maintain.log")
            echo "  [Furniture] maintain: $input"
    ...
    log)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/log.log" 2>/dev/null || echo "  No entries yet. Use: furniture log <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/log.log"
            local total=$(wc -l < "$DATA_DIR/log.log")
            echo "  [Furniture] log: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: furniture report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Furniture] report: $input"
    ...
    seasonal)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/seasonal.log" 2>/dev/null || echo "  No entries yet. Use: furniture seasonal <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/seasonal.log"
            local total=$(wc -l < "$DATA_DIR/seasonal.log")
            echo "  [Furniture] seasonal: $input"
    ...
    tips)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tips.log" 2>/dev/null || echo "  No entries yet. Use: furniture tips <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tips.log"
            local total=$(wc -l < "$DATA_DIR/tips.log")
            echo "  [Furniture] tips: $input"
    ...
    *)
        echo "Unknown: $1 — run 'furniture help'"
        exit 1
        ;;
esac