#!/usr/bin/env bash
# privacy — Privacy reference tool. Use when working with privacy in security contexts.
...
show_help() {
    cat << 'HELPEOF'
privacy v$VERSION — Privacy Reference Tool

Usage: privacy <command>
...
## What is Privacy?
Privacy (privacy) is a specialized tool/concept in the security domain.
It provides essential capabilities for professionals working with privacy.

## Key Concepts
- Core privacy principles and fundamentals
- How privacy fits into the broader security ecosystem  
- Essential terminology every practitioner should know

## Why Privacy Matters
Understanding privacy is critical for:
...
## Getting Started
1. Understand the basic privacy concepts
...
## Installation
1. Download or clone the privacy package
...
cmd_patterns() {
    cat << 'EOF'
# Privacy — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for privacy
...
cmd_migration() {
    cat << 'EOF'
# Privacy — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "privacy v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: privacy help"; exit 1 ;;
esac
