#!/usr/bin/env bash
# curl — Curl reference tool. Use when working with curl in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
curl v$VERSION — Curl Reference Tool
...
## What is Curl?
Curl (curl) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with curl.

## Key Concepts
- Core curl principles and fundamentals
- How curl fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Curl Matters
Understanding curl is critical for:
...
## Getting Started
1. Understand the basic curl concepts
...
## Installation
1. Download or clone the curl package
...
cmd_patterns() {
    cat << 'EOF'
# Curl — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for curl
