#!/usr/bin/env bash
# ppe -- Personal protective equipment tracker
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${PPE_DIR:-$HOME/.ppe}"
...
show_help() {
    cat << EOF
ppe v$VERSION -- Personal protective equipment tracker

Usage: ppe <command> [args]
...
cmd_add() {
    local value="${1:?Usage: ppe add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: ppe search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: ppe remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "ppe v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: ppe help"; exit 1 ;;
esac
