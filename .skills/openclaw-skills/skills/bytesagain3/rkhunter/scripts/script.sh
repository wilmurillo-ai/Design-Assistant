#!/usr/bin/env bash
# rkhunter — Rkhunter reference tool. Use when working with rkhunter in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
rkhunter v$VERSION — Rkhunter Reference Tool

Usage: rkhunter <command>
...
## What is Rkhunter?
Rkhunter (rkhunter) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with rkhunter.

## Key Concepts
- Core rkhunter principles and fundamentals
- How rkhunter fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Rkhunter Matters
Understanding rkhunter is critical for:
...
## Getting Started
1. Understand the basic rkhunter concepts
...
## Installation
1. Download or clone the rkhunter package
...
cmd_patterns() {
    cat << 'EOF'
# Rkhunter — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for rkhunter
...
cmd_performance() {
    cat << 'EOF'
# Rkhunter — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Rkhunter — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Rkhunter — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "rkhunter v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: rkhunter help"; exit 1 ;;
esac
