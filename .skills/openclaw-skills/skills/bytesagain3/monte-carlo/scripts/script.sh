#!/usr/bin/env bash
# monte-carlo — Monte Carlo reference tool. Use when working with monte carlo in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
monte-carlo v$VERSION — Monte Carlo Reference Tool

Usage: monte-carlo <command>
...
## What is Monte Carlo?
Monte Carlo (monte-carlo) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with monte carlo.

## Key Concepts
- Core monte carlo principles and fundamentals
- How monte carlo fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Monte Carlo Matters
Understanding monte carlo is critical for:
...
## Getting Started
1. Understand the basic monte carlo concepts
...
cmd_formulas() {
    cat << 'EOF'
# Monte Carlo — Key Formulas & Calculations
...
cmd_regulations() {
    cat << 'EOF'
# Monte Carlo — Regulatory Framework
...
cmd_instruments() {
    cat << 'EOF'
# Monte Carlo — Instruments & Tools Overview

## Primary Instruments
- Core tools used in monte carlo operations
...
cmd_glossary() {
    cat << 'EOF'
# Monte Carlo — Key Terms & Definitions

## Core Terminology
- **Monte Carlo**: The primary subject of this reference
...
cmd_checklist() {
    cat << 'EOF'
# Monte Carlo — Inspection Checklist
...
case "$CMD" in
    ...
    version|--version|-v) echo "monte-carlo v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: monte-carlo help"; exit 1 ;;
esac
