#!/usr/bin/env bash
# regex-builder — Regex Builder reference tool. Use when working with regex builder in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
regex-builder v$VERSION — Regex Builder Reference Tool

Usage: regex-builder <command>
...
cmd_intro() {
    cat << 'EOF'
# Regex Builder — Overview

## What is Regex Builder?
Regex Builder (regex-builder) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with regex builder.

## Key Concepts
- Core regex builder principles and fundamentals
- How regex builder fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Regex Builder Matters
Understanding regex builder is critical for:
...
## Getting Started
1. Understand the basic regex builder concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Regex Builder — Quick Start Guide
...
## Installation
1. Download or clone the regex builder package
...
cmd_patterns() {
    cat << 'EOF'
# Regex Builder — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for regex builder
...
cmd_performance() {
    cat << 'EOF'
# Regex Builder — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Regex Builder — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Regex Builder — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "regex-builder v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: regex-builder help"; exit 1 ;;
esac
