#!/usr/bin/env bash
# iptables — Iptables reference tool. Use when working with iptables in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
iptables v$VERSION — Iptables Reference Tool

Usage: iptables <command>
...
## What is Iptables?
Iptables (iptables) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with iptables.

## Key Concepts
- Core iptables principles and fundamentals
- How iptables fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Iptables Matters
Understanding iptables is critical for:
...
## Getting Started
1. Understand the basic iptables concepts
...
## Installation
1. Download or clone the iptables package
...
cmd_patterns() {
    cat << 'EOF'
# Iptables — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for iptables
...
cmd_security() {
    cat << 'EOF'
# Iptables — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Iptables — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "iptables v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: iptables help"; exit 1 ;;
esac
