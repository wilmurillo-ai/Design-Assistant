#!/usr/bin/env bash
# rectifier -- Rectifier circuit design calculator
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${RECTIFIER_DIR:-$HOME/.rectifier}"
...
show_help() {
    cat << EOF
rectifier v$VERSION -- Rectifier circuit design calculator

Usage: rectifier <command> [args]
...
cmd_add() {
    local value="${1:?Usage: rectifier add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: rectifier search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: rectifier remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="rectifier-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "rectifier v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: rectifier help"; exit 1 ;;
esac
