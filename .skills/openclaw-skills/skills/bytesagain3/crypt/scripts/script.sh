#!/usr/bin/env bash
# crypt — Crypt reference tool. Use when working with crypt in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
crypt v$VERSION — Crypt Reference Tool
...
## What is Crypt?
Crypt (crypt) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with crypt.

## Key Concepts
- Core crypt principles and fundamentals
- How crypt fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Crypt Matters
Understanding crypt is critical for:
...
## Getting Started
1. Understand the basic crypt concepts
...
## Installation
1. Download or clone the crypt package
...
cmd_patterns() {
    cat << 'EOF'
# Crypt — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for crypt
...
cmd_performance() {
    cat << 'EOF'
# Crypt — Performance Optimization
