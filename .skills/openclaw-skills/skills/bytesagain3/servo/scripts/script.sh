#!/usr/bin/env bash
# servo — Servo reference tool. Use when working with servo in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
servo v$VERSION — Servo Reference Tool
...
## What is Servo?
Servo (servo) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with servo.

## Key Concepts
- Core servo principles and fundamentals
- How servo fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Servo Matters
Understanding servo is critical for:
...
## Getting Started
1. Understand the basic servo concepts
...
## Installation
1. Download or clone the servo package
...
cmd_patterns() {
    cat << 'EOF'
# Servo — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for servo
...
cmd_performance() {
    cat << 'EOF'
# Servo — Performance Optimization
