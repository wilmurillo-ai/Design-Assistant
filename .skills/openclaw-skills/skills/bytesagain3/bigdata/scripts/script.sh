...
DATA_DIR="${HOME}/.local/share/bigdata"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }

_version() { echo "bigdata v2.0.0"; }

_help() {
    echo "Bigdata v2.0.0 — data toolkit"
    echo ""
    echo "Usage: bigdata <command> [args]"
    echo ""
    echo "Commands:"
    echo "  ingest             Ingest"
...
case "${1:-help}" in
    ingest)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/ingest.log" 2>/dev/null || echo "  No entries yet. Use: bigdata ingest <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/ingest.log"
            local total=$(wc -l < "$DATA_DIR/ingest.log")
            echo "  [Bigdata] ingest: $input"
    ...
    transform)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/transform.log" 2>/dev/null || echo "  No entries yet. Use: bigdata transform <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/transform.log"
            local total=$(wc -l < "$DATA_DIR/transform.log")
            echo "  [Bigdata] transform: $input"
    ...
    query)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/query.log" 2>/dev/null || echo "  No entries yet. Use: bigdata query <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/query.log"
            local total=$(wc -l < "$DATA_DIR/query.log")
            echo "  [Bigdata] query: $input"
    ...
    filter)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/filter.log" 2>/dev/null || echo "  No entries yet. Use: bigdata filter <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/filter.log"
            local total=$(wc -l < "$DATA_DIR/filter.log")
            echo "  [Bigdata] filter: $input"
    ...
    aggregate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/aggregate.log" 2>/dev/null || echo "  No entries yet. Use: bigdata aggregate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/aggregate.log"
            local total=$(wc -l < "$DATA_DIR/aggregate.log")
            echo "  [Bigdata] aggregate: $input"
    ...
    visualize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/visualize.log" 2>/dev/null || echo "  No entries yet. Use: bigdata visualize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/visualize.log"
            local total=$(wc -l < "$DATA_DIR/visualize.log")
            echo "  [Bigdata] visualize: $input"
    ...
    export)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/export.log" 2>/dev/null || echo "  No entries yet. Use: bigdata export <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/export.log"
            local total=$(wc -l < "$DATA_DIR/export.log")
            echo "  [Bigdata] export: $input"
    ...
    sample)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/sample.log" 2>/dev/null || echo "  No entries yet. Use: bigdata sample <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/sample.log"
            local total=$(wc -l < "$DATA_DIR/sample.log")
            echo "  [Bigdata] sample: $input"
    ...
    schema)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/schema.log" 2>/dev/null || echo "  No entries yet. Use: bigdata schema <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/schema.log"
            local total=$(wc -l < "$DATA_DIR/schema.log")
            echo "  [Bigdata] schema: $input"
    ...
    validate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/validate.log" 2>/dev/null || echo "  No entries yet. Use: bigdata validate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/validate.log"
            local total=$(wc -l < "$DATA_DIR/validate.log")
            echo "  [Bigdata] validate: $input"
    ...
    pipeline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/pipeline.log" 2>/dev/null || echo "  No entries yet. Use: bigdata pipeline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/pipeline.log"
            local total=$(wc -l < "$DATA_DIR/pipeline.log")
            echo "  [Bigdata] pipeline: $input"
    ...
    profile)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/profile.log" 2>/dev/null || echo "  No entries yet. Use: bigdata profile <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/profile.log"
            local total=$(wc -l < "$DATA_DIR/profile.log")
            echo "  [Bigdata] profile: $input"
    ...
    *)
        ...
        echo "Run 'bigdata help' for available commands."
        exit 1
        ;;
esac