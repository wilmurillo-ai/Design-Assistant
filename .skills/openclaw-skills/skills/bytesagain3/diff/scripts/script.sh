#!/usr/bin/env bash
# diff — Diff reference tool. Use when working with diff in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
diff v$VERSION — Diff Reference Tool
...
## What is Diff?
Diff (diff) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with diff.

## Key Concepts
- Core diff principles and fundamentals
- How diff fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Diff Matters
Understanding diff is critical for:
...
## Getting Started
1. Understand the basic diff concepts
...
## Installation
1. Download or clone the diff package
...
cmd_patterns() {
    cat << 'EOF'
# Diff — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for diff
