#!/usr/bin/env bash
# spend-tracker — Spend Tracker reference tool. Use when working with spend tracker in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
spend-tracker v$VERSION — Spend Tracker Reference Tool

Usage: spend-tracker <command>
...
cmd_intro() {
    cat << 'EOF'
# Spend Tracker — Overview

## What is Spend Tracker?
Spend Tracker (spend-tracker) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with spend tracker.

## Key Concepts
- Core spend tracker principles and fundamentals
- How spend tracker fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Spend Tracker Matters
Understanding spend tracker is critical for:
...
## Getting Started
1. Understand the basic spend tracker concepts
...
cmd_quickstart() {
    cat << 'EOF'
# Spend Tracker — Quick Start Guide
...
## Installation
1. Download or clone the spend tracker package
...
cmd_patterns() {
    cat << 'EOF'
# Spend Tracker — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for spend tracker
...
cmd_performance() {
    cat << 'EOF'
# Spend Tracker — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Spend Tracker — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Spend Tracker — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "spend-tracker v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: spend-tracker help"; exit 1 ;;
esac
