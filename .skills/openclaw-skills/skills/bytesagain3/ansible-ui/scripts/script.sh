#!/usr/bin/env bash
# ansible-ui - Developer workflow automation tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${ANSIBLE_UI_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/ansible-ui}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
ansible-ui v$VERSION

Developer workflow automation tool

Usage: ansible-ui <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "ansible-ui v$VERSION" ;;
