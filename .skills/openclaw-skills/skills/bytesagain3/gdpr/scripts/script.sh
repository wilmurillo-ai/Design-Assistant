...
DATA_DIR="${HOME}/.local/share/gdpr"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }
_version() { echo "gdpr v2.0.0"; }

_help() {
    echo "Gdpr v2.0.0 — security toolkit"
    echo ""
    echo "Usage: gdpr <command> [args]"
    echo ""
    echo "Commands:"
...
_search() {
    local term="${1:?Usage: gdpr search <term>}"
...
case "${1:-help}" in
    generate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/generate.log" 2>/dev/null || echo "  No entries yet. Use: gdpr generate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/generate.log"
            local total=$(wc -l < "$DATA_DIR/generate.log")
            echo "  [Gdpr] generate: $input"
    ...
    check-strength)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/check-strength.log" 2>/dev/null || echo "  No entries yet. Use: gdpr check-strength <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/check-strength.log"
            local total=$(wc -l < "$DATA_DIR/check-strength.log")
            echo "  [Gdpr] check-strength: $input"
    ...
    rotate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/rotate.log" 2>/dev/null || echo "  No entries yet. Use: gdpr rotate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/rotate.log"
            local total=$(wc -l < "$DATA_DIR/rotate.log")
            echo "  [Gdpr] rotate: $input"
    ...
    audit)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/audit.log" 2>/dev/null || echo "  No entries yet. Use: gdpr audit <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/audit.log"
            local total=$(wc -l < "$DATA_DIR/audit.log")
            echo "  [Gdpr] audit: $input"
    ...
    store)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/store.log" 2>/dev/null || echo "  No entries yet. Use: gdpr store <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/store.log"
            local total=$(wc -l < "$DATA_DIR/store.log")
            echo "  [Gdpr] store: $input"
    ...
    retrieve)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/retrieve.log" 2>/dev/null || echo "  No entries yet. Use: gdpr retrieve <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/retrieve.log"
            local total=$(wc -l < "$DATA_DIR/retrieve.log")
            echo "  [Gdpr] retrieve: $input"
    ...
    expire)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/expire.log" 2>/dev/null || echo "  No entries yet. Use: gdpr expire <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/expire.log"
            local total=$(wc -l < "$DATA_DIR/expire.log")
            echo "  [Gdpr] expire: $input"
    ...
    policy)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/policy.log" 2>/dev/null || echo "  No entries yet. Use: gdpr policy <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/policy.log"
            local total=$(wc -l < "$DATA_DIR/policy.log")
            echo "  [Gdpr] policy: $input"
    ...
    report)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/report.log" 2>/dev/null || echo "  No entries yet. Use: gdpr report <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/report.log"
            local total=$(wc -l < "$DATA_DIR/report.log")
            echo "  [Gdpr] report: $input"
    ...
    hash)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/hash.log" 2>/dev/null || echo "  No entries yet. Use: gdpr hash <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/hash.log"
            local total=$(wc -l < "$DATA_DIR/hash.log")
            echo "  [Gdpr] hash: $input"
    ...
    verify)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/verify.log" 2>/dev/null || echo "  No entries yet. Use: gdpr verify <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/verify.log"
            local total=$(wc -l < "$DATA_DIR/verify.log")
            echo "  [Gdpr] verify: $input"
    ...
    revoke)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/revoke.log" 2>/dev/null || echo "  No entries yet. Use: gdpr revoke <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/revoke.log"
            local total=$(wc -l < "$DATA_DIR/revoke.log")
            echo "  [Gdpr] revoke: $input"
