#!/usr/bin/env bash
# namespace — Namespace reference tool. Use when working with namespace in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
namespace v$VERSION — Namespace Reference Tool
...
## What is Namespace?
Namespace (namespace) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with namespace.

## Key Concepts
- Core namespace principles and fundamentals
- How namespace fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Namespace Matters
Understanding namespace is critical for:
...
## Getting Started
1. Understand the basic namespace concepts
...
## Installation
1. Download or clone the namespace package
...
cmd_patterns() {
    cat << 'EOF'
# Namespace — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for namespace
...
cmd_migration() {
    cat << 'EOF'
# Namespace — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "namespace v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: namespace help"; exit 1 ;;
esac
