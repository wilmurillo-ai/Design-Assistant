#!/usr/bin/env bash
# eam -- Enterprise asset management tracker
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${EAM_DIR:-$HOME/.eam}"
...
show_help() {
    cat << EOF
eam v$VERSION -- Enterprise asset management tracker

Usage: eam <command> [args]
...
cmd_add() {
    local value="${1:?Usage: eam add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: eam search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: eam remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "eam v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: eam help"; exit 1 ;;
esac
