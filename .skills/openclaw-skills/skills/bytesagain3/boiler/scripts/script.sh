#!/usr/bin/env bash
# boiler -- Boiler efficiency and sizing tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${BOILER_DIR:-$HOME/.boiler}"
...
show_help() {
    cat << EOF
boiler v$VERSION -- Boiler efficiency and sizing tool

Usage: boiler <command> [args]
...
cmd_add() {
    local value="${1:?Usage: boiler add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: boiler search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: boiler remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "boiler v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: boiler help"; exit 1 ;;
esac
