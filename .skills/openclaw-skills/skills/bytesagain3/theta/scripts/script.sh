#!/usr/bin/env bash
# theta — Theta reference tool. Use when working with theta in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
theta v$VERSION — Theta Reference Tool
...
## What is Theta?
Theta (theta) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with theta.

## Key Concepts
- Core theta principles and fundamentals
- How theta fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Theta Matters
Understanding theta is critical for:
...
## Getting Started
1. Understand the basic theta concepts
...
cmd_formulas() {
    cat << 'EOF'
# Theta — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Theta — Instruments & Tools Overview

## Primary Instruments
- Core tools used in theta operations
...
cmd_glossary() {
    cat << 'EOF'
# Theta — Key Terms & Definitions

## Core Terminology
- **Theta**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "theta v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: theta help"; exit 1 ;;
esac
