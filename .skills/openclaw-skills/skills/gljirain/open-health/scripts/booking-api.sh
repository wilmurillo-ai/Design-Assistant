...
BASE_URL="https://rx.helloklarity.com"
...
case "$command" in
  ...
  availability)
    ...
    [ -z "$service" ] && error "Usage: booking-api.sh availability <service> <state> [carrier] [date] [time_pref]"
    [ -z "$state" ]   && error "Usage: booking-api.sh availability <service> <state> [carrier] [date] [time_pref]"

    # Build query string
    qs="service=${service}&state=${state}"
    [ -n "$carrier" ]   && qs="${qs}&insurance_carrier=$(python3 -c "import sys, urllib.parse; print(urllib.parse.quote(sys.argv[1]))" "$carrier")"
  ...
  book|book-dry)
    json="${1:-}"
    [ -z "$json" ] && error "Usage: booking-api.sh book '<json-payload>'"
