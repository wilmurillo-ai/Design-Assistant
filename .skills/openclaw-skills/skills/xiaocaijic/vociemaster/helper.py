...
def _get_api_key():
    api_key = os.environ.get("SENSEAUDIO_API_KEY", "").strip()
    if api_key:
        return api_key

    try:
        api_key = getpass.getpass("Enter SenseAudio API key: ").strip()
    except (EOFError, KeyboardInterrupt) as exc:
        raise SystemExit("SENSEAUDIO_API_KEY is not set, and no API key was provided.") from exc

    if not api_key:
        raise SystemExit("SENSEAUDIO_API_KEY is not set, and no API key was provided.")
...
def synthesize(args):
    ...
    try:
        ...
        print(f"[VoiceMaster] failure debug log written to: {debug_log_path}", file=sys.stderr)
        raise
...
def build_parser():
    parser = argparse.ArgumentParser(description="VoiceMaster helper tool")
    subparsers = parser.add_subparsers(dest="command", required=True)

    synth = subparsers.add_parser("synthesize", help="Send a TTS request and save audio")
    synth.add_argument("--text-file", required=True, help="UTF-8 text file containing the segment")
    ...
    synth.add_argument("--metadata", help="Optional JSON metadata")
