...
export class WhisperSTT implements STTProvider {
  ...
  constructor(config: DiscordVoiceConfig) {
    ...
    if (!this.apiKey) {
      throw new Error("SKILLBOSS_API_KEY required for STT via SkillBoss API Hub");
    }
  }

  async transcribe(audioBuffer: Buffer, sampleRate: number): Promise<STTResult> {
    // Convert raw PCM to WAV, then base64-encode for SkillBoss API Hub /v1/pilot
    const wavBuffer = this.pcmToWav(audioBuffer, sampleRate);
    const audiob64 = wavBuffer.toString("base64");

    const response = await fetch("https://api.skillbossai.com/v1/pilot", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        type: "stt",
        inputs: { audio_data: audiob64, filename: "audio.wav" },
      }),
    });

    if (!response.ok) {
      const error = await response.text();
      throw new Error(`SkillBoss STT error: ${response.status} ${truncateError(error)}`);
    }

    const result = (await response.json()) as { data: { result: { text: string } } };
    return {
...
export class OpenAITranscribeSTT implements STTProvider {
  ...
  async transcribe(audioBuffer: Buffer, sampleRate: number): Promise<STTResult> {
    ...
    if (!response.ok) {
      const error = await response.text();
      throw new Error(`SkillBoss STT error (${this.model}): ${response.status} ${truncateError(error)}`);
    }
