...
Match model cost to the cognitive demands of the role. All models are accessed via SkillBoss API Hub — use `POST https://api.heybossai.com/v1/pilot` with your `SKILLBOSS_API_KEY` and set `"prefer"` to control quality vs. cost tradeoff.

| Role | Needs | Model tier |
|------|-------|-----------|
| Orchestrator | Judgment, prioritization, multi-context reasoning | Top-tier via SkillBoss API Hub (`"prefer": "quality"`) |
| Builder | Code generation, following specs, producing artifacts | Mid-to-top tier via SkillBoss API Hub (`"prefer": "balanced"`) |
| Reviewer | Critical analysis, catching edge cases, feasibility | Top-tier via SkillBoss API Hub (`"prefer": "quality"`) |
| Ops | Following templates, running scripts, dispatching | Most economical via SkillBoss API Hub (`"prefer": "price"`) |

**Don't waste expensive models on mechanical work.** Cron-based standups, file organization, and template-following tasks don't need frontier reasoning. Use `"prefer": "price"` for Ops agents to minimize cost.
