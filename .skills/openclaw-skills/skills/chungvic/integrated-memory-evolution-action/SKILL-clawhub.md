...
- [ ] 讀取 Layer 3（用戶習慣、核心規則）
  - 位置：`memory/layer3-permanent/`
  - 重點：溝通風格、工作習慣、禁忌清單
  
- [ ] 讀取 Layer 2（任務狀態、犯錯記錄）
  - 位置：`memory/layer2-events/tasks/`
  - 重點：當前任務、相關錯誤記錄
  
- [ ] 讀取 Layer 1（今日對話）
  - 位置：`memory/layer1-conversations/YYYY-MM-DD/`
  - 重點：今日相關對話總結
...
| 觸發情況 | 即時寫入邊層 | 寫入位置 |
|----------|-------------|----------|
| 用戶給出決策 | Layer 2 | `memory/layer2-events/tasks/` |
| 用戶表達偏好 | Layer 3 | `memory/layer3-permanent/` |
| 用戶分配任務 | Layer 2 | `memory/layer2-events/tasks/Txxx.md` |
| 用戶指出錯誤 | Layer 2 | `memory/layer2-events/errors/` |
| 用戶提供信息 | Layer 1 | `memory/layer1-conversations/YYYY-MM-DD/` |
| 任務完成 | Layer 2 | 更新任務狀態 |
| 技能使用 | Layer 2 | `memory/layer2-events/skills/` |
...
```bash
# 1. 更新 Layer 1（對話總結）
# 位置：memory/layer1-conversations/YYYY-MM-DD/<agent>.md

# 2. 更新 Layer 2（任務狀態）
# 位置：memory/layer2-events/tasks/Txxx.md

# 3. 更新 Layer 3（如有需要）
# 位置：memory/layer3-permanent/user-profile.md
```
...
┌─────────────────────────────────────────────────────────┐
│  2. 提煉到 Layer 2                                       │
│     - 任務 → memory/layer2-events/tasks/                │
│     - 錯誤 → memory/layer2-events/errors/               │
│     - 技能 → memory/layer2-events/skills/               │
└────────────────────┬────────────────────────────────────┘
                     ▼
┌─────────────────────────────────────────────────────────┐
│  3. 記錄到進化層                                         │
...
| 指標 | 目標 | 檢查頻率 | 驗證命令 |
|------|------|----------|----------|
| Layer 1 更新 | 每次對話 | 實時 | `ls -lht memory/layer1-conversations/` |
| Layer 2 分類 | 5 類完整 | 每日 | `ls memory/layer2-events/` |
| Layer 3 文件 | >0 | 每週 | `cat memory/layer3-permanent/*.md` |
...
| WAL Protocol | 100% 執行 | 每次對話 | `grep "WAL" memory/layer1-conversations/` |
| Resourcefulness | 嘗試>3 種 | 每次任務 | `grep "嘗試" memory/working-buffer.md` |
...
# 讀取 Layer 3
cat memory/layer3-permanent/user-profile.md

# 讀取 Layer 2 任務
cat memory/layer2-events/tasks/Txxx.md

# 讀取 Layer 1
cat memory/layer1-conversations/YYYY-MM-DD/<agent>.md
```
...
### 記憶層文件
- `memory/layer1-conversations/` - 對話總結
- `memory/layer2-events/` - 事件記錄
- `memory/layer3-permanent/` - 永久記憶
...
2. **讀取記憶層**
   ```bash
   cat memory/layer3-permanent/user-profile.md
   cat memory/layer2-events/tasks/
   cat memory/layer1-conversations/$(date +%Y-%m-%d)/
   ```
...
```bash
# 使用 ClawHub 安裝
clawhub install integrated-memory-evolution-action

# 或手動複製
cp -r skills/integrated-memory-evolution-action /your/workspace/skills/
```
...
**GitHub:** [clawhub.com/skills/integrated-memory-evolution-action](https://clawhub.com/skills/integrated-memory-evolution-action)

---

## 📄 許可證

MIT License - 免費使用，歡迎分享！
