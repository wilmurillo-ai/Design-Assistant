...
exports.createProductCommands = createProductCommands;
const commander_1 = require("commander");
const chalk_1 = __importDefault(require("chalk"));
const productDb = __importStar(require("../db/product"));
const merchantDb = __importStar(require("../db/merchant"));
function createProductCommands() {
    const product = new commander_1.Command('product').description('商品管理命令');
    // 添加新商品
    product
        .command('add')
        .description('添加新商品')
        ...
        try {
            ...
            if (isNaN(price) || price < 0) {
                console.log(chalk_1.default.red('❌ 价格必须是有效的正数'));
                process.exit(1);
            }
            ...
            if (originalPrice !== undefined && (isNaN(originalPrice) || originalPrice < 0)) {
                console.log(chalk_1.default.red('❌ 原价必须是有效的正数'));
                process.exit(1);
            }
            const deliveryTime = parseInt(options.time);
            if (isNaN(deliveryTime) || deliveryTime < 0) {
                console.log(chalk_1.default.red('❌ 配送时间必须是有效的正整数'));
                process.exit(1);
            }
            const stock = parseInt(options.stock);
            if (isNaN(stock) || stock < 0) {
                console.log(chalk_1.default.red('❌ 库存必须是有效的正整数'));
                process.exit(1);
            }
            ...
            console.log(chalk_1.default.green('✅ 商品添加成功！'));
            console.log(chalk_1.default.gray('─'.repeat(50)));
            console.log(`商品ID: ${chalk_1.default.cyan(prod.id)}`);
            console.log(`商品名称: ${chalk_1.default.bold(prod.name)}`);
            console.log(`所属商家: ${chalk_1.default.cyan(merchant.name)}`);
            console.log(`价格: ${chalk_1.default.green(`¥${prod.price.toFixed(2)}`)}`);
            if (prod.original_price) {
                console.log(`原价: ${chalk_1.default.strikethrough(`¥${prod.original_price.toFixed(2)}`)}`);
            }
            console.log(`配送时间: ${chalk_1.default.cyan(`${prod.delivery_time} 分钟`)}`);
            console.log(`库存: ${chalk_1.default.cyan(prod.stock)}`);
            console.log(`状态: ${chalk_1.default.yellow(prod.status)}`);
            console.log(chalk_1.default.gray('─'.repeat(50)));
        }
        catch (error) {
            console.error(chalk_1.default.red('❌ 添加失败:'), error instanceof Error ? error.message : error);
            process.exit(1);
        }
    });
    // 列出商品
    product
        ...
        try {
            ...
            if (products.length === 0) {
                console.log(chalk_1.default.yellow('暂无商品数据'));
                return;
            }
            ...
            products.forEach((p) => {
                const statusColors = {
                    ...
                    sold_out: chalk_1.default.red
                };
                const statusColor = statusColors[p.status] || chalk_1.default.white;
                const priceStr = p.original_price
                    ? `${chalk_1.default.green(`¥${p.price.toFixed(2)}`)} ${chalk_1.default.gray(`(原价 ¥${p.original_price.toFixed(2)})`)}`
                    : chalk_1.default.green(`¥${p.price.toFixed(2)}`);
                console.log(`${chalk_1.default.cyan(p.id)}. ${chalk_1.default.bold(p.name)} ${statusColor(`[${p.status}]`)}`);
    ...
    product
        ...
        try {
            ...
            if (!prod) {
                console.log(chalk_1.default.red('❌ 商品不存在'));
                process.exit(1);
            }
            ...
            console.log(`库存: ${chalk_1.default.cyan(prod.stock)}`);
            if (prod.category)
                console.log(`分类: ${chalk_1.default.cyan(prod.category)}`);
            console.log(chalk_1.default.gray('─'.repeat(50)));
            if (merchant)
                console.log(`所属商家: ${chalk_1.default.cyan(merchant.name)}`);
            if (prod.description)
                console.log(`描述: ${prod.description}`);
            if (prod.image_url)
                console.log(`图片: ${chalk_1.default.blue(prod.image_url)}`);
            console.log(chalk_1.default.gray('─'.repeat(50)));
            console.log(`创建时间: ${chalk_1.default.gray(prod.created_at)}`);
            console.log(`更新时间: ${chalk_1.default.gray(prod.updated_at)}`);
            console.log(chalk_1.default.gray('─'.repeat(50)));
        }
        catch (error) {
            console.error(chalk_1.default.red('❌ 查询失败:'), error instanceof Error ? error.message : error);
    ...
    product
        ...
        try {
            ...
            if (options.price)
                updateData.price = parseFloat(options.price);
            if (options.original !== undefined)
                updateData.original_price = parseFloat(options.original);
            if (options.image !== undefined)
                updateData.image_url = options.image;
            if (options.category !== undefined)
                updateData.category = options.category;
            if (options.stock)
                updateData.stock = parseInt(options.stock);
            if (Object.keys(updateData).length === 0) {
                console.log(chalk_1.default.yellow('⚠️ 没有提供要更新的字段'));
                process.exit(1);
            }
            const prod = productDb.updateProduct(productId, updateData);
            console.log(chalk_1.default.green('✅ 商品信息更新成功！'));
            console.log(chalk_1.default.gray('─'.repeat(50)));
            console.log(`商品ID: ${chalk_1.default.cyan(prod.id)}`);
            console.log(`商品名称: ${chalk_1.default.bold(prod.name)}`);
            console.log(`价格: ${chalk_1.default.green(`¥${prod.price.toFixed(2)}`)}`);
    ...
    product
        ...
        try {
            ...
            console.log(chalk_1.default.green('✅ 价格修改成功！'));
            console.log(chalk_1.default.gray('─'.repeat(50)));
            console.log(`商品: ${chalk_1.default.bold(prod.name)}`);
            console.log(`新价格: ${chalk_1.default.green(`¥${prod.price.toFixed(2)}`)}`);
            console.log(chalk_1.default.gray('─'.repeat(50)));
        }
        catch (error) {
            console.error(chalk_1.default.red('❌ 修改失败:'), error instanceof Error ? error.message : error);
            process.exit(1);
        }
    });
    // 修改配送时间
    product
        ...
        try {
            ...
            console.log(chalk_1.default.green('✅ 配送时间修改成功！'));
            console.log(chalk_1.default.gray('─'.repeat(50)));
            console.log(`商品: ${chalk_1.default.bold(prod.name)}`);
            console.log(`配送承诺: ${chalk_1.default.cyan(`${prod.delivery_time} 分钟`)}`);
            console.log(chalk_1.default.gray('─'.repeat(50)));
        }
        catch (error) {
            console.error(chalk_1.default.red('❌ 修改失败:'), error instanceof Error ? error.message : error);
    ...
    product
        ...
        try {
            ...
            console.log(chalk_1.default.green(`✅ 商品 "${prod.name}" 已上架`));
        }
        catch (error) {
    ...
    product
        ...
        try {
            ...
            console.log(chalk_1.default.yellow(`⏸️ 商品 "${prod.name}" 已下架`));
        }
        catch (error) {
            console.error(chalk_1.default.red('❌ 操作失败:'), error instanceof Error ? error.message : error);
            process.exit(1);
    ...
    product
        ...
        try {
            ...
            console.log(chalk_1.default.red(`🚫 商品 "${prod.name}" 已标记为售罄`));
        }
        catch (error) {
            console.error(chalk_1.default.red('❌ 操作失败:'), error instanceof Error ? error.message : error);
            process.exit(1);
    ...
    product
        ...
        try {
            ...
            console.log(chalk_1.default.green(`✅ 商品 "${prod.name}" 已删除`));
        }
        catch (error) {
    ...
    product
        ...
        try {
            ...
            if (products.length === 0) {
                console.log(chalk_1.default.yellow('未找到匹配的商品'));
                return;
            }
            console.log(chalk_1.default.bold(`\n🔍 搜索结果: "${keyword}"\n`));
            console.log(chalk_1.default.gray('─'.repeat(80)));
            products.forEach((p) => {
                console.log(`${chalk_1.default.cyan(p.id)}. ${chalk_1.default.bold(p.name)} ${chalk_1.default.green(`¥${p.price.toFixed(2)}`)}`);
                console.log(`   🏪 ${p.merchant_name}`);
                if (p.category)
                    console.log(`   🏷️ ${p.category}`);
                console.log(chalk_1.default.gray('─'.repeat(80)));
    ...
    product
        ...
        try {
            ...
            if (categories.length === 0) {
                console.log(chalk_1.default.yellow('暂无商品分类'));
                return;
            }
            console.log(chalk_1.default.bold('\n🏷️ 商品分类\n'));
            categories.forEach((cat, index) => {
                console.log(`${chalk_1.default.cyan(index + 1)}. ${cat}`);
            });
        }
        catch (error) {
            console.error(chalk_1.default.red('❌ 查询失败:'), error instanceof Error ? error.message : error);
