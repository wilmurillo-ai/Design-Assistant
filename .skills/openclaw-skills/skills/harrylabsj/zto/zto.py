...
# 配置
CONFIG_DIR = Path.home() / ".openclaw" / "data" / "zto"
DB_FILE = CONFIG_DIR / "zto.db"
CONFIG_DIR.mkdir(parents=True, exist_ok=True)


def get_secure_storage() -> SecureStorage:
    """按需初始化安全存储，避免普通查询场景提前创建密钥文件。"""
    return SecureStorage(app_name="zto")

# 中通产品类型
ZTO_PRODUCTS = {
    'standard': {'name': '中通标准快递', 'code': 'Z1', 'time': '2-4天', 'desc': '普通包裹、日常寄件'},
    'priority': {'name': '中通优先件', 'code': 'Z2', 'time': '1-3天', 'desc': '时效更高的急件'},
    'economy': {'name': '中通经济件', 'code': 'Z3', 'time': '3-5天', 'desc': '价格优先、非紧急件'},
    'large': {'name': '中通大件', 'code': 'Z4', 'time': '3-6天', 'desc': '大件与重货'},
    'cold_chain': {'name': '中通冷链', 'code': 'Z5', 'time': '2-4天', 'desc': '生鲜、冷藏物品'},
}

# 中通单号规则
ZTO_PATTERN = r'^(ZT)?\d{10,15}$'
...
class ZTOClient:
    ...
    def is_zto_number(self, tracking_number: str) -> bool:
        """检查是否为中通单号"""
        return bool(re.match(ZTO_PATTERN, tracking_number.upper()))
    
    async def query(self, tracking_number: str) -> TrackingResult:
        """查询物流信息"""
        if not self.is_zto_number(tracking_number):
            raise ValueError(f"{tracking_number} 不是有效的中通单号")
    ...
    def estimate_time(self, origin: str, destination: str, product: str = 'standard') -> TimeEstimate:
        """时效预估"""
        product_info = ZTO_PRODUCTS.get(product, ZTO_PRODUCTS['standard'])
    ...
    def estimate_price(self, origin: str, destination: str, weight: float, product: str = 'standard') -> PriceEstimate:
        ...
        if product == 'priority':
            ...
            base_price = max(base_price, weight * 2.8)
...
def print_products():
    ...
    print("-" * 60)
    for code, info in ZTO_PRODUCTS.items():
        print(f"{code:<15} {info['name']:<12} {info['time']:<10} {info['desc']}")


def print_history(client: ZTOClient, limit: int = 10, search: Optional[str] = None):
    """打印查询历史"""
...
def print_subscriptions(client: ZTOClient):
    """打印订阅列表"""
...
async def main():
    parser = argparse.ArgumentParser(description='中通速运助手')
    subparsers = parser.add_subparsers(dest='command', help='可用命令')
    
    # 单号查询
    query_parser = subparsers.add_parser('query', help='查询中通快递')
    query_parser.add_argument('tracking_number', help='中通单号')
    
    # 批量查询
    batch_parser = subparsers.add_parser('batch', help='批量查询')
    batch_parser.add_argument('tracking_numbers', nargs='+', help='中通单号列表')
    ...
    time_parser.add_argument('--product', '-p', default='standard', 
                            choices=list(ZTO_PRODUCTS.keys()),
                            help='产品类型')
    ...
    # 产品列表
    subparsers.add_parser('products', help='查看中通产品类型')
    ...
    sub_parser.add_argument('tracking_number', help='中通单号')
    
    # 取消订阅
    unsub_parser = subparsers.add_parser('unsubscribe', help='取消订阅')
    unsub_parser.add_argument('tracking_number', help='中通单号')
    ...
    # 初始化客户端
    async with ZTOClient() as client:
        # 单号查询
