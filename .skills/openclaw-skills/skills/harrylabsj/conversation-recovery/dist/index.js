...
import { createSession, getSession, updateSession, deleteSession, listSessions, createSnapshot, getSnapshot, getSessionSnapshots, deleteSnapshot, getLatestSnapshot, getStorageStats, initStorage } from './storage.js';
/**
 ...
 */
export async function startSession(title, channel) {
    await initStorage();
    const session = {
 ...
 */
export async function captureSnapshot(sessionId, options = {}) {
 ...
 */
export async function recoverSession(sessionId, snapshotId) {
    const session = await getSession(sessionId);
    if (!session) {
        throw new Error(`Session not found: ${sessionId}`);
    }
    ...
    const outstandingTasks = snapshot.tasks.filter(t => t.status === 'pending' || t.status === 'in_progress' || t.status === 'blocked');
 ...
 */
export async function cleanupSnapshots(sessionId, keepCount = 10) {
