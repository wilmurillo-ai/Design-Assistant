...
import type { Intent, Fact, Task, FactCategory, TaskPriority, TaskStatus } from './models.js';
...
export class ContextAnalyzer {
   ...
   */
  async analyze(messages: ConversationMessage[]): Promise<AnalysisResult> {
    const analyzedAt = new Date().toISOString();
    const tokenCount = this.estimateTokenCount(messages);

    // Extract intents
    const intentExtractions = await this.intentExtractor.extract(
      messages, 
      this.options.method,
      this.options.existingIntents
    );

    // Extract facts
    const factExtractions = await this.factExtractor.extract(
      messages,
      this.options.method,
      this.options.existingFacts
    );

    // Extract tasks
    const taskExtractions = await this.taskExtractor.extract(
      messages,
      this.options.method,
      this.options.existingTasks
    );
   ...
   */
  async analyzeQuick(messages: ConversationMessage[]): Promise<AnalysisResult> {
   ...
   */
  private convertToIntents(extractions: IntentExtraction[]): Intent[] {
    return extractions
   ...
   */
  private convertToFacts(extractions: FactExtraction[]): Fact[] {
    return extractions
      .filter(e => e.confidence >= (this.options.confidenceThreshold || 0.5))
      .map(e => ({
   ...
   */
  private convertToTasks(extractions: TaskExtraction[]): Task[] {
   ...
   */
  private mergeIntents(newIntents: Intent[], existing: Intent[]): Intent[] {
    const merged = [...existing];
    for (const newIntent of newIntents) {
      const isDuplicate = existing.some(e => 
        this.similarity(e.description, newIntent.description) > 0.8
      );
      if (!isDuplicate) {
   ...
   */
  private mergeFacts(newFacts: Fact[], existing: Fact[]): Fact[] {
    const merged = [...existing];
    for (const newFact of newFacts) {
      const isDuplicate = existing.some(e => 
        this.similarity(e.statement, newFact.statement) > 0.85
      );
      if (!isDuplicate) {
        merged.push(newFact);
      }
   ...
   */
  private mergeTasks(newTasks: Task[], existing: Task[]): Task[] {
    const merged = [...existing];
    for (const newTask of newTasks) {
      const isDuplicate = existing.some(e => 
        this.similarity(e.description, newTask.description) > 0.85
      );
      if (!isDuplicate) {
        merged.push(newTask);
      }
...
export class IntentExtractor {
  ...
  async extract(
    ...
    method: 'keyword' | 'llm' | 'hybrid' = 'hybrid',
    existingIntents?: Intent[]
  ): Promise<IntentExtraction[]> {
    const extractions: IntentExtraction[] = [];
   ...
   */
  private extractByKeywords(messages: ConversationMessage[]): IntentExtraction[] {
    const extractions: IntentExtraction[] = [];
   ...
   */
  private cleanIntentDescription(description: string, type: string): string {
   ...
   */
  private deduplicateIntents(extractions: IntentExtraction[]): IntentExtraction[] {
    const unique: IntentExtraction[] = [];
    
    for (const extraction of extractions) {
      const isDuplicate = unique.some(u => 
        this.similarity(u.description, extraction.description) > 0.75
      );
      if (!isDuplicate) {
...
export class FactExtractor {
  ...
  private factPatterns: Record<FactCategory, Array<{
    pattern: RegExp;
    weight: number;
  }>> = {
    preference: [
   ...
   */
  private extractByKeywords(messages: ConversationMessage[]): FactExtraction[] {
    const extractions: FactExtraction[] = [];
   ...
   */
  private validateFact(statement: string, context: string): {
    isValid: boolean;
    ...
    if (this.validationIndicators.strong.some(w => 
      lowerContext.includes(w) || lowerStatement.includes(w)
    )) {
      confidenceMultiplier = 1.0;
    }
    
    // Check for weak/uncertain indicators
    if (this.validationIndicators.weak.some(w => 
      lowerContext.includes(w) || lowerStatement.includes(w)
    )) {
    ...
    const hasNegation = this.validationIndicators.negative.some(w => 
      lowerContext.includes(w)
    );
    
    // If statement itself is positive but context has negation, reduce confidence
    if (hasNegation && !this.validationIndicators.negative.some(w => 
   ...
   */
  private validateAndDeduplicate(extractions: FactExtraction[]): FactExtraction[] {
    const unique: FactExtraction[] = [];
    ...
    for (const extraction of extractions) {
      ...
      const isDuplicate = unique.some(u => 
        u.category === extraction.category &&
...
export class TaskExtractor {
  ...
  // Priority indicators
  private priorityKeywords: Record<TaskPriority, string[]> = {
    critical: [
  ...
  // Due date patterns
  private datePatterns: Array<{
    pattern: RegExp;
    parser: (match: RegExpMatchArray) => string | undefined;
  }> = [
    {
  ...
  async extract(
    ...
    const extractions: TaskExtraction[] = [];
   ...
   */
  private extractByKeywords(messages: ConversationMessage[]): TaskExtraction[] {
    const extractions: TaskExtraction[] = [];
   ...
   */
  private assessPriority(description: string, context: string): TaskPriority {
   ...
   */
  private parseRelativeDay(day: string): string | undefined {
    const now = new Date();
   ...
   */
  private parseDate(month: string, day: string): string | undefined {
   ...
   */
  private parseDuration(amount: number, unit: string): string | undefined {
    const now = new Date();
   ...
   */
  private parseChineseDate(text: string): string | undefined {
    const now = new Date();
   ...
   */
  private deduplicateAndSort(extractions: TaskExtraction[]): TaskExtraction[] {
    const unique: TaskExtraction[] = [];
    
    for (const extraction of extractions) {
      const isDuplicate = unique.some(u => 
        this.similarity(u.description, extraction.description) > 0.8
      );
      if (!isDuplicate) {
        unique.push(extraction);
      }
