#!/usr/bin/env python3
"""
online-shopping.py - 全球在线购物指南 CLI

功能：
- recommend <product>: 根据商品推荐合适的电商平台
...
def recommend_platforms(product_query):
    ...
    if not matched:
        ...
        general_platforms = ["amazon", "ebay", "aliexpress", "temu"]
        ...
        print("\n✅ 最佳选择：Amazon（综合保障最好）")
        print("\n⚠️ 注意事项：")
        print("   • 高价值商品建议选择Amazon保障售后")
        print("   • 追求低价可考虑AliExpress或Temu")
        print("   • 注意查看卖家评分和商品评价")
        return
    ...
    # 最佳选择
    best = cat_info.get("best_choice", recommended[0] if recommended else "amazon")
...
用法：
  python3 online-shopping.py recommend <商品名称>   根据商品推荐平台
  python3 online-shopping.py categories              列出支持的类别
  python3 online-shopping.py compare <平台1> <平台2> 对比两个平台

示例：
  python3 online-shopping.py recommend 手机
  python3 online-shopping.py recommend iPhone case
  python3 online-shopping.py compare amazon ebay
  python3 online-shopping.py compare aliexpress temu

支持的平台：amazon, ebay, aliexpress, temu, shein, walmart, target
""")
...
def main():
    ...
    if command == "recommend":
        ...
        if len(sys.argv) < 4:
            ...
            print(f"   示例: python3 {sys.argv[0]} compare amazon ebay")
            sys.exit(1)
