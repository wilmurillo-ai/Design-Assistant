...
async def main():
    async with pw.async_playwright() as p:
        ...
        ctx = await browser.new_context(viewport={'width': 1080, 'height': 900})
        page = await ctx.new_page()
        with open(os.path.join(TEMPLATES, 'ref2_竖屏.html'), 'r', encoding='utf-8') as f:
            html = f.read()
        await page.set_content(html, timeout=15000)
        await page.wait_for_timeout(2000)
        await page.screenshot(path=os.path.join(OUTPUT, 'ref2_竖屏.png'), type='png', full_page=True)
        await ctx.close()
        print('OK')
