"""技能发布到 Cloudflare Pages（预览/生产）"""

import json, re, subprocess, time, os, shutil, tempfile
from pathlib import Path
from datetime import date

from lib.config import BASE_DIR, SITE_URL, _get_cfg_val, get_skill_cf_project, update_manifest_hashes
...
def publish_preview():
    ...
    project = get_skill_cf_project()

    # 1. Auto-bump patch version + update hashes
    ...
    print(f"\n✅ 验证无误后，运行以下命令发布到生产环境:")
    print(f"   python deploy.py skill-cf-publish-prod")
...
def publish_prod():
    ...
    # Sync version and regenerate index.html before deploying
    _sync_version(skill_dir, ver)
