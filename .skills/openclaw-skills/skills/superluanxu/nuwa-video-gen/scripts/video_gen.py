#!/usr/bin/env python3
"""
Video Generation Skill for MiniMax API (女娲)
Supports: text-to-video, image-to-video, start-end-to-video, subject-reference

运行后视频文件保存在当前工作目录（或 --output 指定路径）。
生成的视频文件由 Agent 发送给用户。
"""
...
if not API_KEY:
    ...
    print("请先设置环境变量: export MINIMAX_API_KEY='your-api-key'")
    sys.exit(1)
...
def invoke_subject_reference(prompt: str, subject_image: str, duration: int = 6, resolution: str = "1080P") -> str:
    """(模式四) 使用人物主体图片和文本描述发起视频生成任务（保持人物特征一致）。"""
    url = f"{API_HOST}/v1/video_generation"
    payload = {
        "prompt": prompt,
        "subject_reference": [
...
def fetch_video(file_id: str, output_path: str):
...
def main():
    parser = argparse.ArgumentParser(description="MiniMax 女娲视频生成")
    parser.add_argument("--mode", required=True,
                        choices=["text", "image", "start_end", "subject"],
                        help="生成模式: text(文生视频), image(图生视频), start_end(首尾帧), subject(主体参考)")
    parser.add_argument("--prompt", required=True, help="视频描述文本")
    parser.add_argument("--image", help="图生视频的首帧图片URL（image模式必填）")
    parser.add_argument("--first", help="首尾帧模式的首帧图片URL（start_end模式必填）")
    parser.add_argument("--last", help="首尾帧模式的尾帧图片URL（start_end模式必填）")
    parser.add_argument("--subject", help="主体参考模式的人脸图片URL（subject模式必填）")
    parser.add_argument("--duration", type=int, default=6, choices=[6, 10], help="视频时长: 6或10秒（默认6秒）")
    parser.add_argument("--resolution", default="1080P", choices=["720P", "1080P"], help="分辨率（默认1080P）")
    parser.add_argument("--output", default=None,
                        help="输出文件名（默认保存到当前目录 output.mp4）")
    ...
    # 下载视频
    fetch_video(file_id, output_path)
    print(f"\n✅ 完成！视频文件: {output_path}")
    print("请将此文件发送给用户。")
