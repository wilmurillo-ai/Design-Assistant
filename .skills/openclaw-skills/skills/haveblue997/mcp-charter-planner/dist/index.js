...
function getSeasonInfo(startDate) {
...
function generateItinerary(startDate, endDate, experience, interests) {
    ...
    const itinerary = [];
    const hasInterest = (keyword) => interests.some((i) => i.toLowerCase().includes(keyword));
...
function generateProvisioning(guests, numDays) {
    const items = [];
    const mealsPerDay = 3;
...
// ─── Tips Generator ──────────────────────────────────────────────────
function generateTips(experience, numDays, season) {
    const tips = [];
    // Universal tips
    tips.push("BVI customs: Clear in at Road Town, Jost Van Dyke, or Spanish Town. You need a cruising permit ($4/person/day). Have passports and boat documentation ready.");
    tips.push("Mooring balls: Most BVI anchorages use mooring balls ($25–35/night). Pick up with a boat hook from the bow — never tie to the pickup line, use your own line through the pennant.");
    tips.push("National park fees: The Baths, The Dogs, and other national parks charge $5/person. Have cash ready.");
    tips.push("VHF Channel 16 for emergencies, Channel 12 for BVI port authority. Monitor weather on Channel 2 (WX).");
    if (experience === "beginner") {
        tips.push("Keep passages short (under 10 nm) for your first few days. The BVI is compact — no need to rush.");
        tips.push("Practice picking up mooring balls before you need to in a crowded harbor. Assign crew roles: helm, bow, and line handler.");
        tips.push("Motor into the wind/current when picking up a mooring ball. Approach slowly — you can always go around.");
        tips.push("Reef the main before you think you need to. It's easier to shake out a reef than to put one in when overpowered.");
    }
    if (experience === "intermediate") {
        tips.push("Consider a night sail from Anegada back to Virgin Gorda — the stars are incredible with no light pollution.");
        tips.push("The passage to Anegada requires careful navigation around Horseshoe Reef. Use waypoints and follow the marked channel.");
    }
    if (experience === "expert") {
        tips.push("For the Anegada passage, depart Virgin Gorda at first light. The reef approach is best with the sun high and behind you for reef reading.");
        tips.push("Explore the less-visited south side of Norman Island — Benures Bay and Pelican Island have excellent diving.");
        tips.push("The roundabout race course around Tortola makes for an excellent day of competitive sailing if your crew is up for it.");
    }
    if (numDays >= 7) {
        tips.push("With a week or more, don't rush. Spend two nights at your favorite spots. The BVI rewards slow sailing.");
    }
    if (season.season.includes("Hurricane")) {
        tips.push("CRITICAL: Check NOAA hurricane forecasts every morning and evening. Know the location of hurricane holes: Paraquita Bay (Tortola), North Sound (Virgin Gorda).");
    }
    if (season.season.includes("Winter")) {
        tips.push("Christmas winds (Dec–Jan) can blow 25+ knots. Reef early and avoid exposed north-facing bays during northerly swells.");
    }
    tips.push("Provisioning tip: Rite Way and Bobby's Supermarket in Road Town are your best options. Prices are 2–3x US mainland — provision staples before arriving if possible.");
    tips.push("Leave no trace: BVI waters are pristine because sailors respect them. No discharge, no trash overboard, use reef-safe sunscreen, don't touch coral.");
    return tips;
}
// ─── MCP Server Setup ────────────────────────────────────────────────
...
server.tool("plan_charter", "Plan a BVI sailing charter with recommended itineraries, weather considerations, provisioning lists, and local tips. Covers real BVI anchorages including The Baths, Norman Island, Jost Van Dyke, Anegada, Cooper Island, and more.", {
    start_date: zod_1.z.string().describe("Charter start date (YYYY-MM-DD format)"),
    end_date: zod_1.z.string().describe("Charter end date (YYYY-MM-DD format)"),
    guests: zod_1.z.number().min(1).max(20).describe("Number of guests aboard"),
    experience: zod_1.z
    ...
    try {
        ...
        const itinerary = generateItinerary(startDate, endDate, experience, interests || []);
