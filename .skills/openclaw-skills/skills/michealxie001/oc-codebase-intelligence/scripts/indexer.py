...
class Indexer:
    ...
    def _parse_file(self, filepath: Path) -> Optional[FileIndex]:
        ...
        try:
            ...
            functions, classes, imports = self._extract_symbols(content, language, filepath)
    ...
    def _extract_symbols(self, content: str, language: str, filepath: Path = None) -> Tuple[List[str], List[str], List[str]]:
        ...
        if language == 'Python':
            ...
            functions, classes, imports = self._parse_c(filepath, content)
    ...
    def _parse_c(self, filepath: Path, content: str) -> Tuple[List[str], List[str], List[str]]:
        """Parse C/C++ file using c_parser"""
        functions = []
        classes = []  # structs, enums, unions, typedefs
        ...
        if self.c_parser:
            try:
                ...
                # Extract types (structs, enums, unions)
                for type_def in info.types:
                    classes.append(type_def.name)
                
                # Extract includes
                for inc in info.includes:
                    imports.append(inc.path)
        ...
        for match in re.finditer(r'^(?:typedef\s+)?(?:struct|union|enum)\s+(\w+)', content, re.MULTILINE):
            classes.append(match.group(1))
