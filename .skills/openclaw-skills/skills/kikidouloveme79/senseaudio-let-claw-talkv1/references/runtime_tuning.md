...
- Cuts off too early:
  - increase `--silence-seconds`
- Replies too slowly:
  - use `--tts-mode system` for quick local playback during debugging
...
- Windows sometimes stops hearing the user:
  - first open `~/.audioclaw/workspace/state/voiceclaw_runtime.log`
  - on Windows the equivalent path is:
    `C:\Users\<用户名>\.audioclaw\workspace\state\voiceclaw_runtime.log`
  - if you see repeated `capture_no_speech`, the capture/VAD side is not starting reliably
  - if you see repeated `asr_empty`, the mic captured audio but ASR returned empty text
  - if you see repeated `wake_blocked`, the assistant is still sleeping and waiting for the wake phrase
  - if you see `agent_error`, open `voiceclaw_agent_error.log` next
- Windows says `录入我的声音` and then appears to hang:
  - first check whether WeSpeaker is enabled on purpose
  - then open:
    `C:\Users\<用户名>\.audioclaw\workspace\state\wespeaker\service.log`
  - and also check `voiceclaw_runtime.log`
  - this usually means WeSpeaker startup or model warmup is blocking
- Windows minimal capture environment:
  - install `setuptools`, `numpy`, `sounddevice`, `webrtcvad`
  - recommend `py -3 -m pip install --upgrade pip setuptools`
  - then `py -3 -m pip install numpy sounddevice webrtcvad`
- Windows recommended Python:
  - prefer `Python 3.11` or `3.12`
  - avoid relying only on the `WindowsApps\python.exe` alias
- Shared credentials file:
  - put SenseAudio credentials in `senseaudio_credentials.json`
  - do not depend on a temporary terminal-only env var for long-term use
...
```bash
bash "{baseDir}/scripts/start_senseaudio_let_claw_talk_universal.sh" --skip-self-check
```
