...
WEBRTCVAD_IMPORT_ERROR = None
try:
    import webrtcvad
except Exception as exc:
    webrtcvad = None
    WEBRTCVAD_IMPORT_ERROR = exc
...
AGENT_ERROR_LOG_PATH = WORKSPACE_ROOT / "state" / "voiceclaw_agent_error.log"
RUNTIME_EVENT_LOG_PATH = WORKSPACE_ROOT / "state" / "voiceclaw_runtime.log"
if IS_WINDOWS:
    DEFAULT_WESPEAKER_PYTHON = str(WORKSPACE_ROOT / "tools" / "wespeaker" / ".venv" / "Scripts" / "python.exe")
else:
    DEFAULT_WESPEAKER_PYTHON = str(WORKSPACE_ROOT / "tools" / "wespeaker" / ".venv" / "bin" / "python")
WESPEAKER_GUIDE_PATH = CURRENT_FILE.parent.parent / "references" / "wespeaker_user_setup.md"
MAC_AGENT_CANDIDATES = [
    ...
    str((Path.home() / ".audioclaw" / "bin" / "audioclaw").resolve()),
]
WINDOWS_AGENT_CANDIDATES = [
    r"%ProgramFiles%\AudioClaw\resources\claws\picoclaw\audioclaw.exe",
    r"%ProgramFiles(x86)%\AudioClaw\resources\claws\picoclaw\audioclaw.exe",
    r"%LOCALAPPDATA%\Programs\AudioClaw\audioclaw.exe",
    r"%LOCALAPPDATA%\Programs\AudioClaw\bin\audioclaw.exe",
    r"%ProgramFiles%\AudioClaw\audioclaw.exe",
    r"%ProgramFiles(x86)%\AudioClaw\audioclaw.exe",
]
LINUX_AGENT_CANDIDATES = [
    str((Path.home() / ".local" / "bin" / "audioclaw").resolve()),
    str((Path.home() / ".audioclaw" / "bin" / "audioclaw").resolve()),
]
MAC_SYSTEM_SOUND_DIR = Path("/System/Library/Sounds")
STATUS_SOUNDS = {
    "activated": MAC_SYSTEM_SOUND_DIR / "Glass.aiff",
    "heard": MAC_SYSTEM_SOUND_DIR / "Pop.aiff",
    "listen": MAC_SYSTEM_SOUND_DIR / "Morse.aiff",
    "thinking": MAC_SYSTEM_SOUND_DIR / "Tink.aiff",
    "error": MAC_SYSTEM_SOUND_DIR / "Basso.aiff",
    "exit": MAC_SYSTEM_SOUND_DIR / "Submarine.aiff",
}
WINDOWS_BEEP_CODES = {
    "activated": getattr(winsound, "MB_ICONASTERISK", -1) if winsound else -1,
    "heard": getattr(winsound, "MB_OK", -1) if winsound else -1,
    "listen": getattr(winsound, "MB_OK", -1) if winsound else -1,
    "thinking": getattr(winsound, "MB_ICONASTERISK", -1) if winsound else -1,
    "error": getattr(winsound, "MB_ICONHAND", -1) if winsound else -1,
    "exit": getattr(winsound, "MB_ICONEXCLAMATION", -1) if winsound else -1,
}
...
def agent_candidates() -> list[str]:
    candidates: list[str] = []
    for env_name in ("VOICECLAW_AGENT_BIN", "AUDIOCLAW_AGENT_PATH", "AUDIOCLAW_AGENT_BIN", "OPENCLAW_AGENT_BIN"):
        value = str(os.getenv(env_name, "") or "").strip()
        if value:
            candidates.append(value)
    if IS_WINDOWS:
        candidates.extend(WINDOWS_AGENT_CANDIDATES)
        candidates.extend(["audioclaw.exe", "audioclaw"])
    elif IS_MACOS:
        candidates.extend(MAC_AGENT_CANDIDATES)
        candidates.append("audioclaw")
    else:
        candidates.extend(LINUX_AGENT_CANDIDATES)
        candidates.append("audioclaw")
    return candidates
...
def detect_default_agent() -> str:
    for candidate in agent_candidates():
        resolved = resolve_executable(candidate)
        if resolved:
            return resolved
    return "audioclaw.exe" if IS_WINDOWS else "audioclaw"


DEFAULT_AGENT = detect_default_agent()
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--wake-asr-model", default=os.getenv("SENSEAUDIO_WAKE_ASR_MODEL", "sense-asr-deepthink"))
    parser.add_argument("--voice-id", default="male_0004_a")
    parser.add_argument("--emotion", default="calm")
    parser.add_argument("--tts-mode", choices=["system", "say", "senseaudio", "none"], default="senseaudio")
    parser.add_argument("--tts-speed", type=float, default=1.25)
    parser.add_argument(
        "--senseaudio-streaming-tts",
        dest="senseaudio_streaming_tts",
        action="store_true",
        default=env_truthy("VOICECLAW_SENSEAUDIO_STREAMING_TTS", False),
    )
    ...
    parser.add_argument("--capture-backend", choices=["auto", "python", "native", "swift"], default="auto")
    ...
    parser.add_argument(
        "--wespeaker-python",
        default=os.getenv("VOICECLAW_WESPEAKER_PYTHON", DEFAULT_WESPEAKER_PYTHON),
    )
    parser.add_argument("--wespeaker-service-script", default=str(SCRIPT_DIR / "local_wespeaker_service.py"))
    parser.set_defaults(status_sounds=env_truthy("VOICECLAW_STATUS_SOUNDS", False), interrupt_playback=True)
...
def build_system_tts_command(text: str, args: argparse.Namespace) -> list[str]:
    ...
    if IS_WINDOWS:
        voice_filter = ""
        if args.say_voice:
            voice_name = powershell_quote(args.say_voice)
            voice_filter = (
                f"$voice = $speaker.GetVoices() | Where-Object {{$_.GetDescription() -like '*{voice_name}*'}} | Select-Object -First 1; "
                "if ($voice) { $speaker.Voice = $voice; } "
            )
        script = (
            "$speaker = New-Object -ComObject SAPI.SpVoice; "
            f"{voice_filter}"
            f"[void]$speaker.Speak('{powershell_quote(text)}');"
        )
    ...
    raise RuntimeError("No supported system TTS backend found on this platform.")


def build_audio_play_command(audio_path: Path) -> Optional[list[str]]:
    resolved = audio_path.expanduser().resolve()
    afplay = shutil.which("afplay")
    if IS_MACOS and afplay:
        return [afplay, str(resolved)]

    if IS_WINDOWS:
        uri = powershell_quote(resolved.as_uri())
        script = (
    ...
    if ffplay:
        return [ffplay, "-nodisp", "-autoexit", "-loglevel", "quiet", str(resolved)]
    mpv = shutil.which("mpv")
    if mpv:
        return [mpv, "--no-video", "--really-quiet", str(resolved)]
    paplay = shutil.which("paplay")
    if paplay:
        return [paplay, str(resolved)]
    play = shutil.which("play")
    if play:
        return [play, "-q", str(resolved)]
    return None
...
def play_status_sound(kind: str, args: argparse.Namespace) -> None:
    ...
    if IS_MACOS:
        ...
        if not sound_path or not sound_path.exists():
            return
        command = build_audio_play_command(sound_path)
        if not command:
            return
    ...
    if IS_WINDOWS and winsound is not None:
        code = WINDOWS_BEEP_CODES.get(kind, -1)
        try:
...
def decode_process_output(data: Optional[bytes]) -> str:
    if data is None:
        return ""
    if isinstance(data, str):
        return data
    for encoding in ("utf-8", sys.getdefaultencoding(), "gb18030", "gbk", "latin-1"):
        try:
...
def run_captured_command(command: list[str], *, timeout: Optional[float] = None) -> subprocess.CompletedProcess:
...
def resolve_api_key(purpose: str) -> str:
    ...
    for value in candidates:
        ...
        if value and not value.startswith("v2.public."):
            return value
    raise RuntimeError(
        f"Missing usable SenseAudio {purpose.upper()} API key. "
        f"Set SENSEAUDIO_API_KEY or write it to {SHARED_CREDENTIALS_PATH}."
    )
...
def preference_defaults() -> dict:
    return {
        ...
        "emotion": "calm",
        "tts_speed": 1.25,
        "senseaudio_streaming_tts": env_truthy("VOICECLAW_SENSEAUDIO_STREAMING_TTS", False),
        "senseaudio_streaming_backend": os.getenv("VOICECLAW_SENSEAUDIO_STREAMING_BACKEND", "auto"),
        "wake_asr_model": os.getenv("SENSEAUDIO_WAKE_ASR_MODEL", "sense-asr-deepthink"),
        ...
        "status_sounds": env_truthy("VOICECLAW_STATUS_SOUNDS", False),
...
def build_runtime_summary(args: argparse.Namespace, speaker_profile: dict) -> dict:
    ...
    return {
        ...
        "logs": {
            "runtime_state": str(get_runtime_state_path()),
            "runtime_log": str(RUNTIME_EVENT_LOG_PATH),
            "agent_error_log": str(AGENT_ERROR_LOG_PATH),
        },
...
def log_runtime_event(event: str, message: str, **fields) -> None:
    try:
        RUNTIME_EVENT_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
        line = f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {event}: {message}"
        if fields:
            line += " | " + json.dumps(fields, ensure_ascii=False, default=str)
        with RUNTIME_EVENT_LOG_PATH.open("a", encoding="utf-8") as fh:
...
def effective_capture_backend(args: argparse.Namespace) -> str:
    if args.capture_backend == "python":
        return "python"
    if args.capture_backend in {"native", "swift"}:
        return "native" if IS_MACOS else ("python" if sd is not None and webrtcvad is not None and np is not None else "unavailable")
...
def ensure_mic_binary(debug: bool) -> Path:
    if not IS_MACOS:
        raise SystemExit("Native mic capture is only available on macOS. Use --capture-backend python on this platform.")
    ...
    if needs_build:
        ...
        if not swiftc:
            raise SystemExit("swiftc not found. Install Xcode Command Line Tools or switch to --capture-backend python.")
        command = [
            swiftc,
            "-O",
            str(MIC_SOURCE),
...
def capture_utterance(args: argparse.Namespace, audio_path: Path) -> dict:
    backend = effective_capture_backend(args)
    if backend == "python":
        try:
            ...
            if args.capture_backend == "python":
                raise
            if args.debug:
                print_status(f"[listen] python backend failed, fallback to native: {exc}")

    if backend != "native":
        raise RuntimeError("No available capture backend. Install numpy, sounddevice, and webrtcvad, or run this skill on macOS.")
...
def summarize_agent_error(stderr: str, stdout: str) -> str:
    ...
    meaningful = [
        ...
        and not re.match(r"^Node\\.js v\\d", line)
        and not re.match(r"^node:internal/", line)
    ...
    if meaningful:
        for line in meaningful:
            lowered = line.lower()
            if any(token in lowered for token in ("module_not_found", "cannot find module", "error", "exception", "failed", "cannot", "can't", "missing", "syntaxerror", "referenceerror", "typeerror", "rangeerror")):
                return line
        return meaningful[-1]
    return lines[-1]


def log_agent_error(stderr: str, stdout: str) -> None:
    try:
        stderr = str(stderr or "")
        stdout = str(stdout or "")
        AGENT_ERROR_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
        body = []
        body.append(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] audioclaw agent failure")
        if stderr.strip():
            body.append("stderr:")
        ...
        body.append("")
        with AGENT_ERROR_LOG_PATH.open("a", encoding="utf-8") as fh:
            fh.write("\n".join(body))
    except Exception:
        pass
...
def resolve_agent_bin(value: str) -> Optional[str]:
...
def probe_agent_cli(agent_bin: str) -> Optional[str]:
    try:
        completed = run_captured_command([agent_bin, "agent", "--help"], timeout=15)
    except Exception as exc:
        return f"AudioClaw CLI probe failed: {exc}"
    ...
    if completed.returncode == 0 and ("help for agent" in lowered or "agent [flags]" in lowered):
        return None

    if "cannot find module" in lowered and re.search(r"[\\\\/]agent(['\"])?$", combined, flags=re.IGNORECASE):
        return (
            "The Windows audioclaw.exe exists, but it does not appear to expose the 'agent' CLI subcommand. "
            "It is being treated like a Node launcher that tries to open a local script named 'agent'."
        )

    if combined:
        return summarize_agent_error(stderr, stdout)
    return f"AudioClaw CLI probe failed with exit code {completed.returncode}."
...
def ask_agent(text: str, args: argparse.Namespace, conversation_state: Optional[dict] = None) -> str:
    ...
    if not agent_bin:
        raise RuntimeError(f"AudioClaw CLI not found: {args.agent_bin}")
    command = [
        agent_bin,
        "agent",
        "--session",
    ...
    for attempt in range(1, max(1, args.agent_retries) + 1):
        ...
        if completed.returncode == 0:
            ...
            log_agent_error(completed.stderr, completed.stdout)
...
def run_startup_self_check(args: argparse.Namespace) -> dict:
    ...
    resolved_agent = resolve_agent_bin(args.agent_bin)
    if not resolved_agent:
        report["fatal_errors"].append(f"AudioClaw CLI 不存在：{args.agent_bin}")
    else:
        args.agent_bin = resolved_agent
        probe_error = probe_agent_cli(resolved_agent)
        if probe_error:
            report["fatal_errors"].append(f"AudioClaw CLI 不可用：{probe_error}")

    if report["capture_backend"] == "python":
        missing = []
        if np is None:
            missing.append(describe_import_issue("numpy"))
        if sd is None:
            missing.append(describe_import_issue("sounddevice"))
        if webrtcvad is None:
            missing.append(describe_import_issue("webrtcvad"))
        if missing:
            report["fatal_errors"].append(f"Python 录音后端缺少依赖：{', '.join(missing)}")
    elif report["capture_backend"] == "native":
        if not MIC_SOURCE.exists():
            report["fatal_errors"].append(f"macOS 原生录音器源码不存在：{MIC_SOURCE}")
    else:
        report["fatal_errors"].append("当前平台没有可用录音后端。请安装 numpy、sounddevice、webrtcvad，或改在 macOS 上使用原生录音。")
...
def describe_import_issue(name: str) -> str:
    mapping = {
        "numpy": NP_IMPORT_ERROR,
        "sounddevice": SD_IMPORT_ERROR,
        "webrtcvad": WEBRTCVAD_IMPORT_ERROR,
        "miniaudio": MINIAUDIO_IMPORT_ERROR,
        "winsound": WINSOUND_IMPORT_ERROR,
    }
    exc = mapping.get(name)
    if not exc:
        return name
    return f"{name} ({exc})"
...
def speak(text: str, args: argparse.Namespace, turn_id: int) -> bool:
    ...
    if args.tts_mode == "none":
        return False
    if args.tts_mode in {"say", "system"}:
        command = build_system_tts_command(spoken_text, args)
    ...
    if not command:
        raise RuntimeError("No supported audio playback backend found on this platform.")
    proc = subprocess.Popen(command, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    return wait_for_playback_or_interrupt(proc, args)
...
def format_wespeaker_setup_error(exc: Exception) -> str:
    ...
    if "set_audio_backend" in detail or "torchaudio" in detail:
        return (
            "WeSpeaker 现在还没法启动：当前 torchaudio 版本和 WeSpeaker 依赖链不兼容。"
            f"请先按这个指南准备用户环境，并优先使用 WeSpeaker 推荐的 torch / torchaudio 版本：{WESPEAKER_GUIDE_PATH}"
        )
    return (
...
def wespeaker_start_timeout() -> float:
    return 20.0 if IS_WINDOWS else 45.0
...
def handle_wespeaker_command(
    ...
    if any(token in normalized for token in ("启动后台声纹", "预热声纹", "预热wespeaker", "启动wespeaker服务")):
        try:
            ensure_wespeaker_service(args, autostart=True, timeout=wespeaker_start_timeout())
        except Exception as exc:
...
def emit_reply(
    ...
    log_runtime_event("reply", "assistant reply emitted", preview=preview_text(reply), turn_id=turn_id)
    ...
    try:
        ...
        if interrupted:
            ...
            log_runtime_event("reply_interrupted", "assistant playback was interrupted by new speech", turn_id=turn_id)
    except Exception as exc:
        armed_until = extend_wake_window(args, armed_until)
        play_status_sound("error", args)
        print_status(f"[tts] 播报失败，已保留文字回复: {exc}")
        log_runtime_event("tts_error", "assistant playback failed", error=str(exc), turn_id=turn_id)
...
def main() -> int:
    ...
    if not args.skip_self_check:
        ...
        log_runtime_event(
            "startup_self_check",
            "startup self-check finished",
            python=self_check["python"],
            capture_backend=self_check["capture_backend"],
            asr_model=args.asr_model,
            wake_asr_model=args.wake_asr_model,
        ...
        for warning in self_check["warnings"]:
            ...
            log_runtime_event("startup_warning", warning)
        if self_check["fatal_errors"]:
            for message in self_check["fatal_errors"]:
                print_status(f"[startup] 错误：{message}")
                log_runtime_event("startup_fatal", message)
            raise SystemExit("SenseAudio-Let-Claw-Talk-Universal 启动自检失败，请先修复上面的错误。")
    ...
    print_status("SenseAudio-Let-Claw-Talk-Universal mode")
    ...
    while not stop_requested:
        ...
        with tempfile.TemporaryDirectory(prefix="voiceclaw_") as tempdir:
            ...
            if status == "no_speech":
                runtime_flags["consecutive_no_speech"] = int(runtime_flags.get("consecutive_no_speech") or 0) + 1
                no_speech_count = int(runtime_flags["consecutive_no_speech"])
                if no_speech_count in {1, 5, 10} or no_speech_count % 20 == 0:
                    log_runtime_event(
                        "capture_no_speech",
                        "no speech detected while listening",
                        consecutive=no_speech_count,
                        capture_backend=effective_capture_backend(args),
                    )
                if args.debug:
                ...
                continue
            runtime_flags["consecutive_no_speech"] = 0
            if status != "ok":
                error = str(capture.get("error") or "unknown capture error")
                log_runtime_event("capture_error", "microphone capture failed", error=error, capture=dict(capture))
            ...
            if not transcript:
                ...
                play_status_sound("error", args)
                log_runtime_event("asr_empty", "ASR returned an empty transcript", model=active_asr_model, source=capture.get("debug_source") or "mic")
                time.sleep(args.cooldown_seconds)
                continue
            if transcript_looks_like_recent_echo(transcript, conversation_state):
                print_status("[echo] 检测到与最近播报高度相似的回录，已忽略本轮语音。")
                play_status_sound("error", args)
                log_runtime_event("echo_ignored", "ignored transcript because it was too similar to the last spoken reply", preview=preview_text(transcript))
                time.sleep(args.cooldown_seconds)
                continue
            ...
            if should_exit(transcript):
                ...
                log_runtime_event("session_exit", "received exit request", transcript=preview_text(transcript))
            ...
            if should_sleep(transcript, args):
                ...
                armed_until = 0.0
                play_status_sound("heard", args)
                log_runtime_event("sleep_entered", "assistant entered sleep mode", transcript=preview_text(transcript))
                if args.wake_phrase:
            ...
            if not allowed:
                ...
                log_runtime_event("wake_blocked", "ignored speech because assistant was still sleeping", preview=preview_text(transcript), wake_phrase=args.wake_phrase)
                time.sleep(args.cooldown_seconds)
                continue
            if just_activated:
                play_status_sound("activated", args)
                log_runtime_event("wake_activated", "assistant woke up", preview=preview_text(transcript), wake_phrase=args.wake_phrase)
            ...
            if not transcript:
                ...
                continue
            print_status(f"[you] {transcript}")
            log_runtime_event("transcript", "recognized user speech", preview=preview_text(transcript), asr_model=active_asr_model)
            ...
            if int(runtime_flags.get("wespeaker_enroll_remaining") or 0) > 0:
                try:
                    ...
                    log_runtime_event("wespeaker_error", "voice enrollment failed", error=str(exc))
            ...
            if wespeaker_reply:
                log_runtime_event("local_command", "handled a local WeSpeaker command", preview=preview_text(transcript))
                conversation_state["last_user_text"] = transcript
            ...
            if preference_reply:
                log_runtime_event("local_command", "handled a local preference command", preview=preview_text(transcript))
                conversation_state["last_user_text"] = transcript
            ...
            if voice_reply:
                log_runtime_event("local_command", "handled a local voice command", preview=preview_text(transcript))
                conversation_state["last_user_text"] = transcript
            ...
            try:
                ...
                log_runtime_event("wespeaker_error", "voice verification failed and was bypassed", error=str(exc))
            ...
            if not allowed_by_wespeaker:
                ...
                if wespeaker_reason:
                    ...
                    log_runtime_event("wespeaker_rejected", "voice verification rejected the utterance", reason=wespeaker_reason)
                save_runtime_state(args, speaker_profile)
                time.sleep(args.cooldown_seconds)
                continue
            ...
            log_runtime_event("agent_request", "forwarding transcript to audioclaw agent", preview=preview_text(transcript), session=args.session)
            try:
                ...
                log_runtime_event("agent_error", "agent request failed but session stayed alive", error=str(exc))
                save_runtime_state(args, speaker_profile)
                time.sleep(args.cooldown_seconds)
                continue
            conversation_state["last_user_text"] = transcript
    ...
    print_status("SenseAudio-Let-Claw-Talk-Universal 已退出。")
    log_runtime_event("session_end", "voice assistant session ended")
    return 0
