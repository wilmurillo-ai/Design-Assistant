...
if CURRENT_PLATFORM == "nt":
    DEFAULT_VENV_PYTHON = WORKSPACE_ROOT / "tools" / "wespeaker" / ".venv" / "Scripts" / "python.exe"
else:
...
def tail_log(path: Path, max_lines: int = 8) -> str:
    if not path.exists():
        return ""
    try:
...
def launch_service(args, *, timeout: float = 60.0) -> dict:
    ...
    if not result or not result.get("ok"):
        ...
        if tail:
            raise RuntimeError(f"WeSpeaker 后台服务启动失败，请查看日志：{log_path}。最近日志：{tail}")
