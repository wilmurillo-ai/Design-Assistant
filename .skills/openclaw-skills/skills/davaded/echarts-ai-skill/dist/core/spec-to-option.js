function categoryValue(row, field) {
    if (!field) {
        return null;
    }
...
function buildCartesianSeries(spec, type) {
...
function buildScatterLikeSeries(spec, type) {
    return [
        {
...
function buildPieLikeData(spec) {
...
function buildRadarIndicators(spec) {
...
function buildRadarSeries(spec) {
...
function buildGaugeSeries(spec) {
...
function buildHeatmapOption(spec) {
    ...
    return {
        ...
        series: [{
                type: "heatmap",
                data: spec.dataset.map((row) => [
                    xValues.indexOf(categoryValue(row, spec.xField)),
                    yValues.indexOf(categoryValue(row, spec.yField)),
                    numberValue(row, spec.valueField) ?? 0
                ])
            }]
    };
}
function buildCandlestickOption(spec) {
...
function buildBoxplotOption(spec) {
    const axisData = spec.dataset.map((row) => categoryValue(row, spec.categoryField));
    return {
        title: titleBlock(spec),
...
function buildTreeSeries(spec, type) {
...
function buildSankeyOption(spec) {
    return {
        title: titleBlock(spec),
        tooltip: { trigger: "item" },
        series: [{
...
function buildGraphOption(spec) {
    return {
        title: titleBlock(spec),
        tooltip: { trigger: "item" },
        legend: spec.nodes.some((node) => node.category !== undefined)
            ? { data: uniqueValues(spec.nodes.map((node) => (node.category ?? null))) }
            : undefined,
        series: [{
                type: "graph",
...
function buildParallelOption(spec) {
    return {
        title: titleBlock(spec),
...
function buildLinesOption(spec) {
    return {
        title: titleBlock(spec),
        tooltip: { trigger: "item" },
        xAxis: { type: "value" },
...
function buildFunnelSeries(spec) {
    return [{
            type: "funnel",
            sort: "descending",
...
function buildRadarOption(spec) {
    return {
        title: titleBlock(spec),
        tooltip: { trigger: "item" },
        legend: { top: 28 },
...
function buildCartesianOption(spec, type) {
    const axisData = spec.dataset.map((row) => categoryValue(row, spec.categoryField));
    return {
        title: titleBlock(spec),
        tooltip: { trigger: "axis" },
...
function buildScatterOption(spec, type) {
    return {
        title: titleBlock(spec),
        tooltip: { trigger: "item" },
