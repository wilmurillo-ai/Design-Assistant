...
Note: `@types/chrome` provides types for all `chrome.*` APIs. They are automatically available via the `types` field.
...
Put message types, storage keys, constants, and utility functions in `src/shared/`. Import from any context. The bundler handles the rest.
