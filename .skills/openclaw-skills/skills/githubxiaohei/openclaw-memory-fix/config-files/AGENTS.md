...
### ⚡ 分层记忆与缓存机制
**热缓存 (memory/cache/hot/)**: 频繁访问的数据 → 优先检索
**冷存储 (memory/cache/cold/)**: 低频数据 → 按需检索
**预加载**: 预测用户意图 → 后台预加载资源
...
### 🧠 元认知与自我反思
**思维日志**：记录每次推理路径、使用的记忆、调用的工具
**自我调参**：根据反思动态调整参数（记忆阈值、超时时间等）
**策略抽象**：从多次反思中提炼通用策略

### 🎯 主动学习与知识缺口
**不确定性检测**：置信度<50%标记"待探索"
**主动提问**：信息不足时主动向用户澄清
**自动探索**：发现缺口后主动搜索资料更新记忆
