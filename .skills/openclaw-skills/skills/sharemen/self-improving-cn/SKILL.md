...
```
CONTEXT: 搭建Flutter界面
REFLECTION: 间距不对，需要重做
LESSON: 给用户看之前先检查视觉间距
```
...
| "你了解 X 的哪些内容？" | Search all tiers for X |
| "你都学到了什么？" | Show last 10 from `corrections.md` |
| "展示我的规则模式" | List `memory.md` (HOT) |
| "展示 [项目] 的规则模式" | Load `projects/{name}.md` |
| "热存储里有什么？" | List files in `projects/` + `domains/` |
| "忘记 X" | Show counts per tier |
| "导出记忆" | ZIP all files |
