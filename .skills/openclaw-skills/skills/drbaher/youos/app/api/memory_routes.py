"""Memory CRUD API routes — store and retrieve contextual facts for drafting."""
...
@router.get("")
def list_memory(request: Request, type: str | None = None) -> dict:
    """Return all memory facts, optionally filtered by type."""
...
@router.post("")
def create_memory(body: MemoryBody, request: Request) -> dict:
    """Create a new memory fact (or update if type+key+fact already exists)."""
...
def delete_memory(memory_id: int, request: Request) -> dict:
