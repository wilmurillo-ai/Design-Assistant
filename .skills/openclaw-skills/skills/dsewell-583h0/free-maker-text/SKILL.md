---
name: free-maker-text
version: "1.0.0"
displayName: "Free Maker Text — Convert Text Into Videos"
description: >
  Get text-based videos ready to post, without touching a single slider. Upload your text prompts (TXT, DOCX, PDF, MP4, up to 200MB), say something like "turn this text into a 30-second video with visuals and background music", and download 1080p MP4 when it's done. Built for marketers who move fast and want to create videos from scripts or copy without cameras or editing software.
...
# Free Maker Text — Convert Text Into Videos
...
Worth noting: shorter text inputs under 100 words produce the most focused and accurate video output.

## Matching Input to Actions

User prompts referencing free maker text, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `free-maker-text`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
...
Export as MP4 for widest compatibility across social platforms and presentation tools.
