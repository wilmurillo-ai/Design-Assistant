...
// Step 2: Sanitize (with div → section + new optimizations)
let result = content;
...
// NEW: Remove vertical-align:middle
result = result.replace(/;?vertical-align:middle;?/g, ';');

// NEW: Simplify <strong> with color:#1B1642
...
console.log('vertical-align count:', (compressed.match(/vertical-align/g) || []).length);
console.log('');
console.log('First 400 chars:', compressed.substring(0, 400));

fs.writeFileSync(path.join(__dirname, 'debug5-output.html'), compressed);
console.log('\nSaved to debug5-output.html');
