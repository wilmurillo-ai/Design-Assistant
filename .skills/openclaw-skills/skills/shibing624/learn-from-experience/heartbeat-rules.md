...
Use heartbeat to keep `~/learn-from-experience/` organized without creating churn or losing data.

## Source of Truth

Keep the workspace `HEARTBEAT.md` snippet minimal.
Treat this file as the stable contract for learn-from-experience heartbeat behavior.
Store mutable run state only in `~/learn-from-experience/heartbeat-state.md`.

## Start of Every Heartbeat

1. Ensure `~/learn-from-experience/heartbeat-state.md` exists.
2. Write `last_heartbeat_started_at` immediately in ISO 8601.
3. Read the previous `last_reviewed_change_at`.
4. Scan `~/learn-from-experience/` for files changed after that moment, excluding `heartbeat-state.md` itself.
...
- check cross-session sync status; if `stale`, run global config sync
...
- Never reorganize files outside `~/learn-from-experience/`
- If scope is ambiguous, leave files untouched and record a suggested follow-up instead

## State Fields

Keep `~/learn-from-experience/heartbeat-state.md` simple:
