...
`index.md` tracks all namespaces plus sync status:
```markdown
# Memory Index
...
## Sync Status
Last global config sync: 2026-02-15
memory.md last modified: 2026-02-15
Sync status: in_sync
```
...
### Sync Recovery
If global config sync fails or global config is corrupted:
1. memory.md is always the source of truth
2. Re-run sync from memory.md to rebuild ### Patterns block
3. If global config file is missing, skip sync (agent product may not be installed)
4. Never attempt to recover memory.md from global config (one-way sync only)
