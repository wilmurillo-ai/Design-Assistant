...
const SELECTORS: Record<string, {
  ...
  getStats: (post: Element) => Record<string, string | null>;
}> = {
  x: {
    name: "X/Twitter",
    url: (u) => `https://x.com/${u}`,
...
async function connectBrowser(): Promise<{ browser: Browser | null; context: BrowserContext; page: Page; mode: string }> {
  // Try CDP first (OpenClaw)
  try {
    const browser = await chromium.connectOverCDP(`http://localhost:${OPENCLAW_CDP_PORT}`);
...
async function extractPosts(page: Page, selectors: typeof SELECTORS['x'], max: number, withStats: boolean): Promise<PostData[]> {
...
async function extractProfile(page: Page, selectors: typeof SELECTORS['x']): Promise<ProfileData> {
...
async function detectLoginStatus(page: Page, accountBtnSelector: string): Promise<string> {
...
export async function braveBrowsePlatform(username: string, options: {
  platform?: string;
  maxPosts?: number;
  includeStats?: boolean;
