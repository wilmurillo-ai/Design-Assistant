...
**Description**: **需要使用用户token，根据用户id生成，详情参考上面生成的token文档**
...
**Description**: 默认请求地址：https://open.fangcloud.com/api/v2/knowledge/chatStream
...
--header 'Authorization: Bearer <TOKEN>' \
--header 'Content-Type: application/json' \
--data '{"files": [{"document_id": "1221900719926288472","book_id": "1221904085922373661"}],"document_id": "1221900719926288472","library_id": "8c77d9e93719031c70bfc1968a3ec8fd","item_type": "folder"}'
```
...
**Description**: 获取上传地址后，需要调用上传接口进行真正的上传(header中需要传x-file-name字段：文件的名称)，真正上传完成之后，需要再调用发布接口，才能在知识库中看到文件，上传文件示例：
...
**Description**: **需要使用企业token，根据企业id生成，详情参考上面生成的token文档**
