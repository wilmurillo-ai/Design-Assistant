...
def launch_app(
    ...
    Args:
        app_name: The app name alias or Android package name.
        device_id: Optional ADB device ID.
        delay: Delay in seconds after launching. If None, uses configured default.

    Returns:
        True if app was launched, False if the app name could not be resolved.
    """
    if delay is None:
        delay = TIMING_CONFIG.device.default_launch_delay

    package = APP_PACKAGES.get(app_name)
    if package is None and "." in app_name:
        package = app_name
