#!/bin/bash
# Auto Extract on Session Change - v1.0
...
LOG_FILE="${LOG_FILE:-$HOME/.openclaw/logs/auto-extract.log}"
...
write_to_daily_note() {
    ...
    # 追加内容
    echo "" >> "$DAILY_NOTE"
    echo "## [$timestamp] Auto-extracted" >> "$DAILY_NOTE"
...
# 主逻辑
main() {
    log "=== Auto Extract Check ==="
    
    local latest=$(extract_latest_content)
    [ -z "$latest" ] && log "No content to extract" && return
