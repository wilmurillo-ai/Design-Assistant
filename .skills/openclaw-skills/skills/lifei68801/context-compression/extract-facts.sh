#!/bin/bash
# Extract Facts Before Truncation - v1.1
...
# This script extracts conversation facts for memory persistence
...
ENABLE_CONFIG_EXTRACTION="${ENABLE_CONFIG_EXTRACTION:-false}"
...
detect_high_value_patterns() {
    ...
    # Users can opt-in by setting ENABLE_CONFIG_EXTRACTION=true
    if [ "$ENABLE_CONFIG_EXTRACTION" = "true" ]; then
...
extract_and_save_facts() {
    ...
    if [ -s "$temp_facts" ]; then
        ...
        log "📝 Extracted $facts_count facts from $session_name → MEMORY.md"
    fi
