...
def show_voice_list(limit: int = 10) -> str:
    ...
    lines.append("")
    lines.append(f"共 {result['count']} 个声音，说'用第 X 个声音'来选择")
...
def upload_audio(file_path: str, style_name: str = "") -> dict:
    """
    上传音频文件作为参考声音
    ...
    Returns:
        {"success": True, "audio_url": "...", "saved_path": "..."}
    """
    try:
        service = init_service()
        result = service.upload_audio(file_path, style_name)
        if result['success']:
...
def upload_and_add(file_path: str, style_name: str, audio_text: str) -> dict:
    ...
    """
    # 步骤 1：上传
    upload_result = upload_audio(file_path, style_name)
    if not upload_result["success"]:
        return upload_result

    # 步骤 2：添加记录
    add_result = add_custom_voice(
