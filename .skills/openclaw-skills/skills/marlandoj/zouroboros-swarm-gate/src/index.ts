...
export type Decision = "SWARM" | "DIRECT" | "SUGGEST" | "FORCE_SWARM";
export type Override = "force_swarm" | "bias_direct" | null;
...
export interface SwarmGateConfig {
...
function detectOverride(msg: string): Override {
...
function scoreDeliverableComplexity(msg: string): number {
  ...
  const artifacts = [
    /\b(report|pdf)\b/, /\b(email|send)\b/, /\b(code|implement|script)\b/,
    /\b(deploy|publish|site|page)\b/, /\b(diagram|chart|visual)\b/,
    /\b(database|schema|migration)\b/, /\b(config|configuration|setup)\b/,
    /\b(test|spec|benchmark)\b/, /\b(seed|yaml|spec)\b/,
    /\b(pr|pull\s+request|commit)\b/, /\b(blog\s+post|article|post|readme|documentation|guide)\b/,
    /\b(social|assets|media|banner|image)\b/, /\b(announcement|newsletter|campaign)\b/,
  ];
...
export function evaluate(msg: string, config?: SwarmGateConfig): SwarmDecision {
...
export { WEIGHTS, THRESHOLD_SWARM, THRESHOLD_SUGGEST };
