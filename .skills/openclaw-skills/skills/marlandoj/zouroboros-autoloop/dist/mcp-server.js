...
var SCRIPT_DIR = cliArgs["script-dir"];
var RESULTS_DIR = cliArgs["results-dir"];
function getAutoloopScript() {
  const candidates = [
...
function listRecentLoops(limit = 10) {
  const loops = [];
  ...
  scanDir(RESULTS_DIR);
  return loops.sort((a, b) => b.mtime - a.mtime).slice(0, limit).map(({ mtime, ...rest }) => rest);
}
function isLoopRunning(programPath) {
  const lockFile = join(dirname(programPath), ".autoloop.lock");
  return existsSync(lockFile);
}
function toolAutoloopStart(args) {
...
function toolAutoloopStop(args) {
  const programPath = args.program.startsWith("/") ? args.program : join(RESULTS_DIR, args.program);
  const lockFile = join(dirname(programPath), ".autoloop.lock");
...
var TOOLS = [
  { name: "autoloop_start", description: "Start an autoloop optimization campaign.", inputSchema: { type: "object", properties: { program: { type: "string", description: "Path to program.md" }, executor: { type: "string", description: "Shell command for LLM proposals (reads stdin)" }, resume: { type: "boolean" }, dryRun: { type: "boolean" } }, required: ["program"] } },
  { name: "autoloop_status", description: "Check autoloop status.", inputSchema: { type: "object", properties: { program: { type: "string" } }, required: ["program"] } },
  { name: "autoloop_results", description: "Get experiment results.", inputSchema: { type: "object", properties: { program: { type: "string" }, limit: { type: "number" } }, required: ["program"] } },
  { name: "autoloop_stop", description: "Stop a running autoloop.", inputSchema: { type: "object", properties: { program: { type: "string" } }, required: ["program"] } },
  { name: "autoloop_list", description: "List recent autoloop campaigns.", inputSchema: { type: "object", properties: { limit: { type: "number" } } } }
];
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      case "autoloop_start":
        result = toolAutoloopStart(args);
        break;
      case "autoloop_status":
        result = toolAutoloopStatus(args);
        break;
      case "autoloop_results":
        result = toolAutoloopResults(args);
        break;
      case "autoloop_stop":
        result = toolAutoloopStop(args);
        break;
      case "autoloop_list":
        result = toolAutoloopList(args);
        break;
      default:
        result = `Unknown: ${name}`;
    }
