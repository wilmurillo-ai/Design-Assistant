...
KEY_PREFIX = "agent:main:telegram:default:direct:"
...
def main() -> int:
    ...
    targeted = {
        ...
        if key.startswith(KEY_PREFIX) and should_prune(meta)
    }
    if not targeted:
        return 0

    archive_dir = SESSIONS_DIR / f"archive-budget-auto-{int(time.time())}"
    archive_dir.mkdir(parents=True, exist_ok=True)

    for key, meta in targeted.items():
        session_path = Path(str(meta.get("sessionFile") or ""))
