...
#### `maintainer.agentId`
- Type: `string`
- Meaning: the primary maintainer agent for centralized shared-memory coordination
...
Defines recommended schedule settings and is the source of truth for cron consistency checks.
...
- Type: `boolean`
- Meaning: whether subagents may directly write shared master files
...
#### `attachment.required`
- Type: `boolean`
- Meaning: whether local attachment is required for shared-memory reading
...
- Type: `string`
- Meaning: local attachment guide filename
- Recommended v1 value:
  ```json
  "SHARED_ATTACH.md"
  ```
...
- Type: `string`
- Meaning: local file used as the primary rule injection entrypoint
- Recommended v1 value:
  ```json
  "AGENTS.md"
  ```
...
```json
"sharing": {
  "shareAgentIdentity": false,
  "shareAgentPersona": false,
  "shareAgentTone": false,
...
- Type: `boolean`
- Recommended v1 value: `false`
- Meaning: whether agent identity may be shared

#### `shareAgentPersona`
- Type: `boolean`
- Recommended v1 value: `false`
- Meaning: whether persona framing may be shared

#### `shareAgentTone`
- Type: `boolean`
- Recommended v1 value: `false`
- Meaning: whether tone/style identity may be shared
...
"readOrder": {
  "privateFirst": true,
  "sharedSecond": true,
  "sharedCannotOverridePersona": true
}
```
...
- Type: `boolean`
- Meaning: read private long-term memory first

#### `readOrder.sharedSecond`
- Type: `boolean`
- Meaning: read shared long-term memory second

#### `readOrder.sharedCannotOverridePersona`
- Type: `boolean`
- Meaning: shared memory remains supplemental and cannot override agent identity/persona
...
- Type: `boolean`
- Meaning: allow re-promotion when a previously extracted private entry has materially changed
...
#### `maintenance.markPrivateExtractedEntries`
- Type: `boolean`
- Meaning: allow only minimal appended extraction markers on private source entries
- In v1, this means:
  - append extraction markers only
  - do not restructure private content

#### `maintenance.modifyPrivateMemoryDirectly`
- Type: `boolean`
- Recommended v1 value: `false`
- Meaning: whether shared maintenance may directly rewrite private long-term memory body content
- In v1, `false` means:
  - do not rewrite
  - do not restructure
  - do not delete private body content
...
If cron tasks are inconsistent with the updated config, report the mismatch.

Do not automatically rewrite all cron jobs in v1.
...
Every cron-related field present in JSON must be checked with no omissions.
...
- any other cron-related JSON-defined fields

### Where checks happen

#### Shared scan checks
Check all participating agents' private maintenance cron tasks for:
- existence
- enabled state
- consistency with current JSON config

#### Shared maintenance checks
Check shared scan and shared maintenance cron tasks for:
- existence
- enabled state
- consistency with current JSON config

#### Repair attachment checks
Check the target agent's private maintenance cron for:
- existence
- enabled state
- consistency with current JSON config
...
- `attachStatus` = local read-path readiness state
