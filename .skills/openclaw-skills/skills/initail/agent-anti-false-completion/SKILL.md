---
name: agent-anti-false-completion
description: "用于减少 AI Agent"没做却说做了""没验证却说完成了"等假完成行为的可靠性技能。通过任务约束、结果校验和执行规范，帮助 Agent 在复杂任务中保持真实执行、明确验证与可信交付。适用于代码、调试、研究、写作、规划、运维、API 集成、数据分析、部署等所有任务类型。"
---
...
- 未搜索直接假设 API 行为 → `[自动选择：⚫ 百度味 | 因为：没搜索就猜 | 改用：🟡 字节味/🟠 阿里味]`

## 搭配使用

- `systematic-debugging` — PUA 加动力层，systematic-debugging 提供方法论
- 任何需要验证的任务 — 在完成任务后主动使用本技能进行自检
