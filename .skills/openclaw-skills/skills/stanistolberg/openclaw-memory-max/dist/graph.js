...
exports.queryGraphForHook = queryGraphForHook;
...
function loadGraph() {
    ...
    try {
        const graph = JSON.parse(fs_1.default.readFileSync(gPath, 'utf8'));
        // Migrate v1 nodes: add missing fields
        for (const node of graph.nodes) {
            if (node.frequency === undefined)
                node.frequency = 1;
            if (node.lastAccessed === undefined)
                node.lastAccessed = node.timestamp;
        }
        return graph;
    }
    catch {
...
function textSimilarityQuick(a, b) {
...
async function scoreNodes(query, nodes, topK = 5) {
    if (nodes.length === 0)
        return [];
    ...
    try {
        const classifier = await (0, reranker_1.getClassifier)();
        const scored = [];
...
async function findDuplicate(graph, cause, action, effect, threshold = 0.85) {
...
async function queryGraphForHook(query, topK = 2) {
    const graph = loadGraph();
    if (graph.nodes.length === 0)
        return [];
    ...
    api.registerTool({
        ...
        async execute(_toolCallId, args) {
            ...
            for (const word of queryWords) {
                ...
                if (ids)
                    ids.forEach(id => tagMatchIds.add(id));
            }
