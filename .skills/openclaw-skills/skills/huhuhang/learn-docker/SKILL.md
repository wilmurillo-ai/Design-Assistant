---
name: learn-docker
description: "Learn Docker on LabEx via container fundamentals courses and image or compose labs. Docker courses and labs through LabEx MCP; return labex.io URLs for continued container learning."
---

# Learn Docker

This skill helps users find courses and hands-on labs for [Learn Docker](https://labex.io/learn/docker) on [LabEx](https://labex.io). Follow the public catalog MCP workflow: list courses, expand labs for a chosen course, and keep recommendations short and actionable. Stay within the Docker learning path and end with public LabEx lab URLs the user can open in a browser.

## Workflow

1. Clarify the Docker learning goal when the request is vague.
...
2. Start from the two public Docker catalog routes.
   Use `GET https://mcp.labex.io/learn/docker/courses` to list Docker courses.
   Use `GET https://mcp.labex.io/learn/<course-alias>/labs` to list labs for a selected Docker course.

3. Keep recommendations narrow and actionable.
   Recommend a small set of Docker labs or courses with a brief reason for each.
...
- User asks what Docker can be learned on LabEx: list `GET /learn/docker/courses`.
- User names a Docker course or picks one from the list: expand it with `GET /learn/<course-alias>/labs`.
