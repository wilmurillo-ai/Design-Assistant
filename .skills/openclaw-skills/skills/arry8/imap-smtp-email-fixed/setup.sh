...
case $PROVIDER_CHOICE in
  ...
  11)
    ...
    IMAP_TLS="true"
    echo ""
    echo "⚠️  Yahoo Mail requires an App Password — your regular Yahoo password will NOT work."
    echo "   1. Go to: https://login.yahoo.com/account/security"
    echo "   2. Click 'Generate and manage app passwords'"
    echo "   3. Select 'Other App', name it, and click Generate"
    echo "   4. Use the generated 16-character password below"
    echo ""
    ;;
  12)
...
# Build config vars (non-secret — skill config.env)
CONFIG_VARS="# ${ACCOUNT_NAME:-Default} account
...
# Build secret vars (passwords only — OC .env)
SECRET_VARS="# IMAP/SMTP ${ACCOUNT_NAME:-default} account
${ACCOUNT_PREFIX}IMAP_PASS=$PASSWORD
${ACCOUNT_PREFIX}SMTP_PASS=$PASSWORD"

# --- Write config to skill config.env ---
case $SETUP_MODE in
  "default")
    cat > "$CONFIG_FILE" << EOF
$CONFIG_VARS
...
# --- Append secrets to OC .env (skip if already present) ---
if [ -f "$OC_ENV" ]; then
  # Remove existing password lines for this account prefix before appending
  TEMP_FILE=$(mktemp)
  grep -v "^${ACCOUNT_PREFIX}IMAP_PASS=\|^${ACCOUNT_PREFIX}SMTP_PASS=" "$OC_ENV" | grep -vi "^# IMAP/SMTP ${ACCOUNT_NAME:-default} account" > "$TEMP_FILE" 2>/dev/null || true
  content=$(cat "$TEMP_FILE") && printf '%s\n' "$content" > "$TEMP_FILE"
  echo "" >> "$TEMP_FILE"
  echo "$SECRET_VARS" >> "$TEMP_FILE"
  mv "$TEMP_FILE" "$OC_ENV"
  chmod 600 "$OC_ENV"
  echo "✅ Secrets saved to $OC_ENV"
else
  echo "⚠️  OC .env not found at $OC_ENV — add credentials manually:"
  echo "$SECRET_VARS"
fi
echo ""
