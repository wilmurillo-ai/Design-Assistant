...
async function connect() {
  ...
  if (!imapConfig.user || !imapConfig.password) {
    throw new Error('Missing IMAP user or password. Check credentials in ~/.openclaw/.env and settings in config.env');
  }
...
async function checkEmails(mailbox = DEFAULT_MAILBOX, limit = 10, recentTime = null, unreadOnly = false) {
  ...
  try {
    ...
    // Get UIDs first, take only the newest N to avoid fetching entire mailbox
    const uids = await new Promise((resolve, reject) => {
      imap.search(searchCriteria, (err, results) => err ? reject(err) : resolve(results || []));
    });
    const recentUids = uids.slice(-limit);
    if (recentUids.length === 0) return [];

    const messages = await searchMessages(imap, [['UID', ...recentUids]], fetchOptions);
