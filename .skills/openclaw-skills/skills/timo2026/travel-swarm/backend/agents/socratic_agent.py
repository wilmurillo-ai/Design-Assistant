...
class SocraticAgent:
    ...
    def __init__(self, convergence_threshold: float = 0.8):  # 用户要求>80%
    ...
    def _extract_by_keywords(self, text: str):
        ...
        cities = ["杭州", "北京", "上海", "广州", "深圳", "成都", "重庆", "西安", "南京", "苏州", 
                  ...
                  "西藏", "新疆", "拉萨", "乌鲁木齐", "喀纳斯", "纳木错", "林芝", "阿勒泰"]  # ⭐ 添加西藏新疆
        matched_cities = [city for city in cities if city in text]
        
        if len(matched_cities) > 1:
            # ⭐ 边缘情况：目的地歧义 - 识别"去XX"后面的城市才是目的地
            print(f"[歧义检测] 发现多个城市: {matched_cities}")
            # 查找"去"+"城市名"的模式，这才是真正的目的地
            go_pattern = re.search(r'去([^\s]+)', text)
            if go_pattern:
                dest_hint = go_pattern.group(1)
                for city in matched_cities:
                    if city in dest_hint:
                        self.anchor.where = city
                        print(f"[目的地识别] 去XX模式 → 目的地: {city}")
                        break
            else:
                # 没有"去XX"模式，取最后一个城市（出发地在前，目的地在后）
                self.anchor.where = matched_cities[-1]
                print(f"[目的地识别] 无去XX模式 → 取最后城市: {matched_cities[-1]}")
        ...
        # === HOW: 交通（增强识别）===
        if "高铁" in text or "火车" in text or "12306" in text:
            self.anchor.how = "高铁"
            self.anchor.how_confirmed = True
        elif "飞机" in text or "航班" in text or "机票" in text:
            self.anchor.how = "飞机"
            self.anchor.how_confirmed = True
        elif "自驾" in text or "开车" in text or "租车" in text or "SUV" in text or "越野" in text:
            self.anchor.how = "自驾"
            self.anchor.how_confirmed = True
