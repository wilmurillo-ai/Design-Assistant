...
```bash
# 腾讯财经实时行情接口
curl -s "https://qt.gtimg.cn/q=sh600XXX,sz159XXX,sh510XXX"
```

**返回格式解析**：
```
v_sh600XXX="1~示例银行~600XXX~10.78~...";
# 字段顺序: 0=未知,1=名称,2=代码,3=当前价,4=昨收,5=今开...
```

**ETF代码前缀规则**：
- A股ETF: `sh` (上海) / `sz` (深圳)
- 港股: `hk` (如 hk00{持仓数量} 腾讯)
- 美股: 直接使用代码 (如 AAPL)
...
|------|------|------|------|------|--------|------|
| 示例ETF | 15200 | {成本价} | {现价} | +1656 | +11.5% | 盈利 |

# ✅ 树形格式（手机端友好）
**💼 持仓概览**

🔋 **示例ETF** | {持仓数量}份 | **+11.5%** (+¥1,656)
├─ 成本: ¥{成本价} → 现价: ¥{现价}
└─ 今日: +2.54% 📈
```
...
🎯 **示例ETF** | {持仓数量}份 | **+11.1%** (+¥1,594)
├─ 成本: ¥{成本价} → 现价: ¥1.051
└─ 今日: +2.54% 📈

📈 **示例指数ETF** | {持仓数量}份 | **+3.6%** (+¥704)
├─ 成本: ¥4.40 → 现价: ¥4.56
└─ 今日: +1.22% ✅

🏦 **示例银行** | {持仓数量}股 | **+20.2%** (+¥1,278)
├─ 成本: ¥{成本价} → 现价: ¥{现价}
└─ 距止盈¥{止盈线}还差6.3%
