...
echo -e "\n### Node Status ###" >&2
$CLI get nodes >&2
NOT_READY=$($CLI get nodes --no-headers 2>/dev/null | grep -cv " Ready" || echo 0)
...
$CLI get pods -n kube-system -l tier=control-plane 2>/dev/null || \
$CLI get pods -n kube-system | grep -E "kube-apiserver|kube-controller|kube-scheduler|etcd" >&2

# 4. Pods Not Running
echo -e "\n### Pods Not Running ###" >&2
NOT_RUNNING=$($CLI get pods -A --field-selector=status.phase!=Running,status.phase!=Succeeded --no-headers 2>/dev/null | wc -l | tr -d ' ')
...
DEPRECATED=$($CLI get --raw /metrics 2>/dev/null | grep -c "apiserver_requested_deprecated_apis" || echo 0)
if [ "$DEPRECATED" -gt 0 ]; then
    WARNINGS=$((WARNINGS + 1))
    echo "WARNING: Deprecated APIs may be in use" >&2
    echo "Check: $CLI get --raw /metrics | grep apiserver_requested_deprecated_apis" >&2
else
...
echo -e "\n### etcd Health ###" >&2
ETCD_PODS=$($CLI get pods -n kube-system -l component=etcd --no-headers 2>/dev/null | wc -l | tr -d ' ')
if [ "$ETCD_PODS" -gt 0 ]; then
    $CLI get pods -n kube-system -l component=etcd >&2
else
...
PRESSURE=$($CLI get nodes -o json 2>/dev/null | jq -r '.items[] | select(.status.conditions[] | select(.type | contains("Pressure")) | .status == "True") | .metadata.name' 2>/dev/null)
if [ -n "$PRESSURE" ]; then
    WARNINGS=$((WARNINGS + 1))
...
if command -v oc &> /dev/null && oc whoami &> /dev/null 2>&1; then
    ...
    echo -e "\n### MachineConfigPool Status ###" >&2
    MCP_UPDATING=$(oc get mcp --no-headers 2>/dev/null | grep -c "True" || echo 0)
    if [ "$MCP_UPDATING" -gt 0 ]; then
        BLOCKERS=$((BLOCKERS + 1))
        echo "BLOCKER: MachineConfigPool currently updating" >&2
        oc get mcp >&2
    else
        echo "✓ MachineConfigPools stable" >&2
    fi
fi
