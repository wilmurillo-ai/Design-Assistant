#!/usr/bin/env python3
"""
批量提取静安区项目的合同到期时间（V2版本）
改进：增加"评标结果公告"的处理逻辑
优先级：中标公告 > 评标结果公告 > 招标公告
"""
    ...
    for link in links:
        ...
        if district in text:
            ...
            if match:
                ...
                # 判断公告类型
                if '招标公告' in text and '中标' not in text and '评标结果' not in text:
                    projects[project_name]['招标公告'].append({
                        'title': text.strip(),
                        'infoid': infoid,
                        'pdf_id': pdf_id
                    })
                elif '中标公告' in text and '评标结果' not in text:
                    projects[project_name]['中标公告'].append({
                        'title': text.strip(),
                        'infoid': infoid,
                        'pdf_id': pdf_id
                    })
                elif '评标结果公告' in text:  # 新增
                    projects[project_name]['评标结果公告'].append({
                        'title': text.strip(),
                        'infoid': infoid,
                        'pdf_id': pdf_id
                    })
...
def process_projects(district: str, output_csv: str):
    ...
    for i, (project_name, announcements) in enumerate(projects.items()):
        ...
        if ann:
            pdf_path = f"/tmp/{project_name}_{date_source}_{ann['pdf_id']}.pdf"
            print(f"  下载{date_source}: {ann['title']}")
            ...
            if download_pdf(ann['pdf_id'], pdf_path):
                ...
                if text:
                    ...
                    if bidding_date:
                        ...
                        print(f"    {date_source}日期: {bidding_date}")
            else:
                if result['备注']:
                    result['备注'] += f'; {date_source} PDF 下载失败'
                else:
                    result['备注'] = f'{date_source} PDF 下载失败'
...
def main():
    """主函数"""
    district = '静安区'
    output_csv = '/Users/yujunwang/.openclaw/workspace/jingan_contract_dates_v2.csv'
