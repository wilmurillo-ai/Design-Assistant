...
import type { AgentState, CustomStrategy } from "./types.js";

const EXP_BASE = [40, 120, 350, 900, 2200, 5500, 14000, 35000];
const PK_TIERS = [1000n, 5000n, 10000n, 50000n, 100000n];
const e18 = 10n ** 18n;

function xpToNextLevel(realm: number, level: number): bigint {
...
function canLevelUp(s: AgentState): boolean {
...
function canBreakthrough(s: AgentState): boolean {
  const realmMax = (s.warrior.realm + 1) * 12;
  return s.warrior.level === realmMax && s.warrior.realm < 7;
}

function bestAffordableTier(balance: bigint): number {
  for (let i = 4; i >= 0; i--) {
    if (balance >= PK_TIERS[i] * e18) return i;
  }
  return -1;
}

/** 激进策略：优先 PK，高频战斗 */
export const aggressiveStrategy: CustomStrategy = {
  name: "aggressive",
  ...
  shouldPK: (s) => {
    if (s.isInPK || s.isMeditating) return { should: false, tier: 0 };
  ...
  distributeSP: (_s, points) => {
    ...
    if (points <= 0) return [0, 0, 0, 0, 0];
    const alloc: [number, number, number, number, number] = [0, 0, 0, 0, 0];
...
/** 防守策略：重闭关质押，稳健成长 */
export const defensiveStrategy: CustomStrategy = {
  name: "defensive",
...
/** 均衡策略：平衡修炼和 PK */
export const balancedStrategy: CustomStrategy = {
  name: "balanced",
  shouldMeditate: (s) => !s.isMeditating && !s.isInPK,
  ...
  distributeSP: (_s, points) => {
    ...
    const alloc: [number, number, number, number, number] = [each, each, each, each, 0];
    alloc[4] = points - each * 4;
    return alloc;
  },
};

export function getStrategy(name: string): CustomStrategy {
  switch (name) {
