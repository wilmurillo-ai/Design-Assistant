---
name: free-text-to-video-ki
version: "1.0.0"
displayName: "Free Text to Video AI — Convert Text into AI Videos"
description: >
  convert text prompts into AI-generated videos with this skill. Works with TXT, DOCX, PDF, plain text files up to 500MB. marketers, content creators, educators use it for generating videos from written text using AI — processing takes 1-2 minutes on cloud GPUs and you get 1080p MP4 files.
...
# Free Text to Video AI — Convert Text into AI Videos
...
Here's a typical use: you send a a two-sentence product description, ask for turn this text into a 30-second video with visuals and background music, and about 1-2 minutes later you've got a MP4 file ready to download. The whole thing runs at 1080p by default.

One thing worth knowing — shorter, clearer text prompts produce more accurate video results.

## Matching Input to Actions

User prompts referencing free text to video ki, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `free-text-to-video-ki`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
