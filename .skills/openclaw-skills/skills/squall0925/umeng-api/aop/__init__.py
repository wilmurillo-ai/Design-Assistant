...
def _load_umeng_config():
    """
    从 umeng-config.json 加载配置
    ...
    Returns:
        dict: 配置信息，包含 apiKey 和 apiSecurity，如果不存在则返回 None
    """
    config_paths = [
        ...
        if config_path.exists():
            try:
                ...
                if api_key and api_security:
                    return {'apiKey': api_key, 'apiSecurity': api_security}
            except Exception:
                pass
...
def _get_credentials_from_env():
    """从环境变量获取认证信息"""
    api_key = os.getenv("UMENG_API_KEY")
    api_security = os.getenv("UMENG_API_SECURITY")
    if api_key and api_security:
        return {'apiKey': api_key, 'apiSecurity': api_security}
    return None
...
def get_default_appinfo():
    ...
    优先级顺序：
    1. 已设置的默认值（通过 set_default_appinfo）
    2. umeng-config.json 配置文件
    3. 环境变量（UMENG_API_KEY, UMENG_API_SECURITY）
    
    Returns:
        AppInfo: 应用信息对象，如果未找到认证信息则返回 None
    """
    # 检查是否已设置默认值
    if hasattr(get_default_appinfo, '_appinfo') and get_default_appinfo._appinfo:
        return get_default_appinfo._appinfo
    
    # 尝试从配置文件加载
    config = _load_umeng_config()
    if config:
        appinfo = AppInfo(config['apiKey'], config['apiSecurity'])
        get_default_appinfo._appinfo = appinfo
        return appinfo
    
    # 尝试从环境变量加载
    env_creds = _get_credentials_from_env()
    if env_creds:
        appinfo = AppInfo(env_creds['apiKey'], env_creds['apiSecurity'])
        get_default_appinfo._appinfo = appinfo
        return appinfo
...
def set_default_appinfo(appkey, secret):
    ...
    get_default_appinfo._appinfo = AppInfo(appkey, secret)
