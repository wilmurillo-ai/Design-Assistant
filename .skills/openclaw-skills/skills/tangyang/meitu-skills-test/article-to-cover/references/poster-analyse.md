...
Extract comprehensive JSON covering:

### Style/Medium
Physical texture only (e.g., 3D render, Risograph, Matte Paper). Strictly forbidden to describe specific objects — only describe "visual feeling."

### Image Layout
Grid structure, composition logic, information hierarchy, reading path.

### Font Form
- **Case judgment**: ALL CAPS / Title Case / Lowercase
- **Arrangement logic**: Stacked / Curved / Scattered

### Brush Stroke Mimicry
Precisely describe medium (chalk texture, gouache dry brush, vector gradient) instead of generic "illustration."

### Detail Insight
- If no facial features → add "faceless character, blank face" to prompt; add "eyes, nose, mouth, facial features" to negative prompt
- Precisely capture all visual details

### Vector/Stroke Classification
Strictly distinguish between:
- `flat vector illustration, lineless, clean edges`
- `outlined illustration, ink stroke`

If no strokes in original → emphasize "no outlines" in prompt.
