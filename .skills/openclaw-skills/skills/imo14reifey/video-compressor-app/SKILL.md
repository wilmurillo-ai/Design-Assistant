...
version: "1.0.0"
displayName: "Video Compressor App — Compress and Export Smaller Videos"
description: >
  Turn a 500MB 4K travel video into 1080p compressed MP4 files just by typing what you need. Whether it's reducing video file size for sharing or uploading or quick social content, drop your large video files and describe the result you want. No timeline dragging, no export settings — 30-90 seconds from upload to download.
...
**Try saying:**
- "compress a 500MB 4K travel video into a 1080p MP4"
...
# Video Compressor App — Compress and Export Smaller Videos
...
Here's a typical use: you send a a 500MB 4K travel video, ask for compress this video to under 50MB without losing too much quality, and about 30-90 seconds later you've got a MP4 file ready to download. The whole thing runs at 1080p by default.

One thing worth knowing — shorter clips compress faster and give you more control over the quality-to-size ratio.

## Matching Input to Actions

User prompts referencing video compressor app, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `video-compressor-app`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
