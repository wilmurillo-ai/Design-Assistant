...
name: editor-ai
version: "1.0.0"
displayName: "Editor AI — Edit and Export Polished Videos"
description: >
  Cloud-based editor-ai tool that handles automatically editing raw footage into polished videos. Upload MP4, MOV, AVI, WebM files (up to 500MB), describe what you need, and get 1080p MP4 output in 1-2 minutes. Built for content creators who work with raw video footage.
...
# Editor AI — Edit and Export Polished Videos
...
User prompts referencing editor ai, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `editor-ai`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
