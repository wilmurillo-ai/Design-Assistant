...
HIGH_SIGNAL_PHRASES = {
    'vague_references': CONFIG['high_signal_patterns'].get('vague_references', {}).get('phrases', []) if CONFIG else [
        '有研究表明', '研究表明', '相关研究表明', '数据显示',
        '有研究显示', '专家表示', '业内认为', '普遍认为',
        '很多人觉得', '大多数人认为', '毫无疑问',
    ],
    ...
    'clickbait_intros': CONFIG['high_signal_patterns'].get('clickbait_intros', {}).get('phrases', []) if CONFIG else [
        '让我们一起', '走进', '揭秘', '你可能不知道',
        '有没有发现', '划重点', '敲黑板', '涨知识',
        '其实质是', '其实', '顾名思义', '说白了',
        '简单来说', '通俗地说', '99%的人都不知道',
...
def detect_patterns(text):
    ...
    # ── High: Vague references ──
    for phrase in HIGH_SIGNAL_PHRASES.get('vague_references', []):
        count = text.count(phrase)
        if count > 0:
            issues['vague_references'].append({
                'text': phrase,
                'count': count,
                'severity': 'high',
            })

    # ── High: Clickbait intros ──
    for phrase in HIGH_SIGNAL_PHRASES.get('clickbait_intros', []):
        count = text.count(phrase)
        if count > 0:
            issues['clickbait_intros'].append({
                'text': phrase,
                'count': count,
                'severity': 'high',
            })
...
CATEGORY_NAMES = {
    ...
    'clickbait_intros': ('🟠', '引流开场白'),
    'vague_references': ('🟠', '模糊概括/空洞引用'),
...
def format_output(issues, metrics, score, sentences=None, as_json=False, score_only=False, verbose=False):
    ...
    for cat in ['three_part_structure', 'mechanical_connectors', 'empty_grand_words',
                'clickbait_intros', 'vague_references', 'ai_high_freq_words', 'filler_phrases', 'balanced_arguments', 'template_sentences',
