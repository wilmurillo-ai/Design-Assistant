...
class HebbianGraphUpdater:
    ...
    def update_from_query_result(self, activated_node_ids: List[int]) -> int:
        ...
        try:
            ...
            for node_a, node_b in pairs:
                ...
                if row:
                    ...
                    new_w = min(
                        self.max_weight,
                        old_w + self.learning_rate * (1.0 - old_w),
                    )
                    conn.execute(
                        "UPDATE hebbian_edges"
                        " SET weight = ?, co_activations = ?, last_updated = ?"
                        " WHERE node_a = ? AND node_b = ?",
                        (new_w, old_count + 1, ts, node_a, node_b),
                    )
                else:
                    conn.execute(
                        "INSERT INTO hebbian_edges"
                        " (node_a, node_b, weight, co_activations, last_updated)"
                        " VALUES (?, ?, ?, ?, ?)",
                        (node_a, node_b, self.learning_rate, 1, ts),
                    )
                updated += 1
        ...
        if elapsed_ms > 50:
            logger.warning("[hebbian] SLOW: %.1f ms for %d edges", elapsed_ms, len(pairs))
        ...
        try:
            ...
            for cand in candidate_node_ids:
                ...
                for top in top_results:
                    ...
                    row = conn.execute(
                        "SELECT weight FROM hebbian_edges"
                        " WHERE node_a = ? AND node_b = ?",
                        (na, nb),
                    ).fetchone()
                    if row:
    ...
    def stats(self) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "max_weight":  max_w,
                "top_pairs": [
                    (r[0], r[1], r[2], r[3]) for r in top_pairs
                ],
            }
        finally:
            conn.close()
...
if __name__ == "__main__":
    ...
    if args.test:
        ...
        conn.execute(
            "CREATE TABLE memory_nodes (id INTEGER PRIMARY KEY, turn_id TEXT)"
        )
        conn.commit()
        conn.close()
        ...
        s = u.stats()
        print(f"📊 Hebbian Graph — {s['total_edges']} edges"
