...
export async function fetchDMInbox(cookies, count = 20) {
...
export async function fetchDMConversation(cookies, conversationId, maxId) {
    const params = new URLSearchParams({
        ...COMMON_PARAMS,
...
export function parseInbox(data) {
    ...
    const convList = Object.entries(conversations).map(([id, conv]) => {
        const participants = (conv.participants || []).map((p) => {
            const user = users[p.user_id];
            return user ? `@${user.screen_name}` : p.user_id;
        });
        // Find last message
        const lastEntry = entries.find((e) => e.message?.conversation_id === id);
        return {
            id,
            type: conv.type,
            name: conv.name,
...
export function parseConversation(data) {
    ...
    const entries = state.entries || [];
    return entries
        .filter((e) => e.message?.message_data?.text)
        .map((e) => {
