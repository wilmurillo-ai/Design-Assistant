...
def analyze_media(input_path=None, url=None, media_type=None, sensitivity=None, api_url=None, api_key=None, output_level=None):
    """调用API进行烟火检测"""
...
检测到烟雾: {'✅ 是' if detection.get('has_smoke') else '❌ 否'}
        ...
        regions = "\n".join([f"  📍 区域 {idx+1}: 火焰: {'✅ 有' if item.get('has_fire') else '❌ 无'}, 烟雾: {'✅ 有' if item.get('has_smoke') else '❌ 无'}, 置信度: {item.get('confidence', 0)}" for idx, item in enumerate(detection.get('regions', []))])
