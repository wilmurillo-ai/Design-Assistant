...
- 自动行为：
    ...
    2. **⚠️ 强制数据获取规则（次高优先级）**：如果用户触发任何历史报告查询关键词（如"查看所有识别报告"、"
       显示所有儿童安全报告"、"查看历史报告"等），**必须**：
        - 直接使用 `python -m scripts.child_dangerous_behavior_recognition_analysis --list --open-id {从消息上下文获取 open-id}`
          参数调用 API
          查询云端的历史报告数据
...
- **禁止**自行假设或生成 open-id 值（如 child123 等）
...
# 分析网络监控视频，调整预警阈值（OpenClaw UI 上下文，使用 metadata id 作为 open-id）
