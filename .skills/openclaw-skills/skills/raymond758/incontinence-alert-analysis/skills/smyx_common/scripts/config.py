...
class ConstantEnum(BaseEnum):
    ...
    def init(cls, config=None):
        super().init(config)
        # 新增：最优先读取 workspace 下 smyx_common/config.yaml 中的 api-key 配置
        workspace = os.environ.get('OPENCLAW_WORKSPACE', '/root/.openclaw/workspace')
        smyx_config_path = os.path.join(workspace, 'smyx_common', 'config.yaml')
        api_key_from_config = None
        if os.path.exists(smyx_config_path):
            try:
                with open(smyx_config_path, 'r', encoding='utf-8') as f:
                    smyx_config = yaml.safe_load(f) or {}
                    api_key_from_config = smyx_config.get('api-key')
            except Exception as e:
                pass
        
        # 如果配置文件中有 api-key，则优先使用它作为 open-id 和 username
        if api_key_from_config:
            cls.CURRENT__OPEN_ID = api_key_from_config
            cls.CURRENT__USER_NAME = api_key_from_config
        else:
            # 否则保持原有获取逻辑
