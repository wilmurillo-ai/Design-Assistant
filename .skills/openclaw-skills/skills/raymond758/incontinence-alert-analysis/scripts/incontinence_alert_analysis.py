...
def analyze_video(input_path=None, url=None, user_type=None, detection_mode=None, api_url=None, api_key=None, output_level=None):
    """调用API分析失禁状态监测视频"""
...
def main():
    ...
    parser.add_argument("--user-type", choices=["elderly", "bedridden", "infant", "other"], default=ConstantEnum.DEFAULT__USER_TYPE,
                        help="护理对象类型：elderly(失能老人), bedridden(卧床病人), infant(婴幼儿), other(其他)，默认 other")
    parser.add_argument("--detection-mode", choices=["real-time", "regular-check"], default=ConstantEnum.DEFAULT__DETECTION_MODE,
                        help="检测模式：real-time(实时监控), regular-check(定时巡查)，默认 real-time")
    parser.add_argument("--open-id", help="当前护理对象/看护人的OpenID/UserId/用户名/手机号")
