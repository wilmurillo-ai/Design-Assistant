...
def analyze_media(input_path=None, url=None, media_type=None, confidence_threshold=None, api_url=None, api_key=None, output_level=None):
    """调用API进行基础目标检测"""
...
检测到目标总数: {detection.get('total_count', 0)}
        ...
        objects = "\n".join([f"  📦 {category_map.get(obj.get('category'), obj.get('category'))}: {obj.get('count', 0)} 个，平均置信度: {obj.get('avg_confidence', 0)}" for obj in detection.get('category_stats', [])])
