...
description: "Bird recognition skill. Identifies bird species in images/videos of target areas. Supports recognition of no less than 500 common bird species, supports customized model training, suitable for ecological observation, garden bird watching and other scenarios. | 鸟类识别技能，识别目标区域图片/视频中的鸟类种类，支持不低于500种常见鸟类识别，支持定制化模型训练，适用于生态观测、庭院观鸟等场景"
---
...
- 自动行为：
    ...
    2. **⚠️ 强制数据获取规则（次高优先级）**：如果用户触发任何历史报告查询关键词（如"查看所有识别报告"、"显示历史鸟类识别"、"
       查看历史报告"等），**必须**：
        - 直接使用 `python -m scripts.bird_recognition_analysis --list --open-id {从消息上下文获取 open-id}` 参数调用
          API
          查询云端的历史报告数据
...
- **禁止**自行假设或生成 open-id 值（如 bird123、watching456 等）
...
```bash
# 识别本地鸟类图片（OpenClaw UI 上下文，使用 metadata id 作为 open-id）
python -m scripts.bird_recognition_analysis --input /path/to/bird.jpg --open-id openclaw-control-ui

# 识别本地视频（OpenClaw UI 上下文，使用 metadata id 作为 open-id）
