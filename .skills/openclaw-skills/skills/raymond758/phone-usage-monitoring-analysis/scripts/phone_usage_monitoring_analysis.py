...
def analyze_video(input_path=None, url=None, detection_type=None, work_area=None, api_url=None, api_key=None, output_level=None):
    """调用API分析玩手机行为监测视频"""
...
def main():
    ...
    parser.add_argument("--work-area", choices=["open-office", "cubicle", "meeting-room", "other"], default=ConstantEnum.DEFAULT__WORK_AREA,
                        help="工作区域类型：open-office(开放办公区), cubicle(独立工位), meeting-room(会议室), other(其他)，默认 other")
    parser.add_argument("--open-id", help="当前用户/企业的OpenID/UserId/用户名/手机号")
