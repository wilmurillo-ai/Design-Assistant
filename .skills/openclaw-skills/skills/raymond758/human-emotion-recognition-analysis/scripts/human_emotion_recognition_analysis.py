...
def analyze_media(input_path=None, url=None, media_type=None, threshold=None, api_url=None, api_key=None, output_level=None):
    """调用API进行人体情绪识别"""
...
情绪评分: {recognition.get('overall_score', '未知')}
        ...
        emotions = "\n".join([f"  {emotion_map.get(e.get('emotion'), e.get('emotion'))}: {e.get('probability'):.2f}" for e in recognition.get('emotions', [])])
        abnormal_emotions = "\n".join([f"  ⚠️  {emotion_map.get(e, e)} (强度超过阈值)" for e in recognition.get('abnormal_emotions', [])])
        dominant_emotion = recognition.get('dominant_emotion', '未知')
        dominant_emotion_cn = emotion_map.get(dominant_emotion, dominant_emotion)
