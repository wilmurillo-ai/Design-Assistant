---
name: video-editor-pc
version: "1.0.0"
displayName: "Video Editor PC — Edit and Export PC Videos"
description: >
  edit raw video footage into polished edited clips with this skill. Works with MP4, MOV, AVI, MKV files up to 500MB. PC video creators use it for editing raw PC footage into a clean final video — processing takes 1-2 minutes on cloud GPUs and you get 1080p MP4 files.
...
- "editing raw PC footage into a clean final video for PC video creators"
...
# Video Editor PC — Edit and Export PC Videos
...
A quick example: upload a 3-minute desktop screen recording, type "trim the pauses, add transitions, and export a clean final cut", and you'll get a 1080p MP4 back in roughly 1-2 minutes. All rendering happens server-side.

Worth noting: shorter clips under 2 minutes process significantly faster.

## Matching Input to Actions

User prompts referencing video editor pc, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `video-editor-pc`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
