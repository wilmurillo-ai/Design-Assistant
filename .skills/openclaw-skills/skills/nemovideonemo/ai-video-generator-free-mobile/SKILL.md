---
name: ai-video-generator-free-mobile
version: "1.0.0"
displayName: "AI Video Generator Free Mobile — Generate Videos from Your Phone"
description: >
  generate images or clips into ready-to-share videos with this skill. Works with MP4, MOV, JPG, PNG files up to 200MB. TikTok creators use it for generating short videos from photos or clips on a mobile device — processing takes 30-60 seconds on cloud GPUs and you get 1080p MP4 files.
...
# AI Video Generator Free Mobile — Generate Videos from Your Phone
...
A quick example: upload five product photos from a phone camera roll, type "turn these photos into a 30-second video with music and transitions", and you'll get a 1080p MP4 back in roughly 30-60 seconds. All rendering happens server-side.

Worth noting: vertical 9:16 aspect ratio works perfectly for TikTok and Reels exports.

## Matching Input to Actions

User prompts referencing ai video generator free mobile, aspect ratio, text overlays, or audio tracks get routed to the corresponding action via keyword and intent classification.
...
Headers are derived from this file's YAML frontmatter. `X-Skill-Source` is `ai-video-generator-free-mobile`, `X-Skill-Version` comes from the `version` field, and `X-Skill-Platform` is detected from the install path (`~/.clawhub/` = `clawhub`, `~/.cursor/skills/` = `cursor`, otherwise `unknown`).
