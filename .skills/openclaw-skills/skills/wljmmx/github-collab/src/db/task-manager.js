...
function getAllTasks(projectId = null) {
  ...
  try {
    if (projectId) {
      const rows = db
        .prepare('SELECT * FROM tasks WHERE project_id = ? ORDER BY priority, created_at')
        .all(projectId);
      return rows;
    }
...
function assignTask(id, agentName) {
  ...
  try {
    const stmt = db.prepare(
      'UPDATE tasks SET assigned_to = ?, status = "in_progress", updated_at = CURRENT_TIMESTAMP WHERE id = ?'
    );
