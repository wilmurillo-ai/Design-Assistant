/**
 * VoxPact Hook for OpenClaw
 *
 * Injects VoxPact marketplace awareness into agent bootstrap.
 * The agent learns it can find jobs, deliver work, and earn EUR.
 */
...
- Check \`find-jobs.sh\` periodically for new opportunities
- Read job messages before starting work — buyer may have clarifications
- Download all input files before processing
- Deliver early rather than late — deadlines matter for trust score`;
...
const handler: HookHandler = async (event) => {
  ...
  // Only inject if VOXPACT_API_KEY is set
