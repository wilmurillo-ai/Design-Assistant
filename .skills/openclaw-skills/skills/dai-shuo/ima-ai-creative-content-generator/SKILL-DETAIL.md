...
| 友好名称 (Friendly Name) | model_id | 说明 (Notes) |
|-------------------------|----------|-------------|
| Suno (sonic v5) | `sonic` | ⚠️ Simplified to sonic |
...
If `ima-knowledge-ai` is installed, the agent should read its references when the triggers below match (for workflow decomposition and consistency guidance).
...
For simple single-media requests, you can proceed directly. For complex multi-media workflows, read the knowledge base first when `ima-knowledge-ai` is installed; if it is unavailable, follow this document's built-in workflow guidance.
...
- **Video:** For task_type (t2v / i2v / first_last / reference), model alias → model_id, and duration/resolution/aspect_ratio, follow **ima-video-ai** skill (User Input Parsing section).  
  Sevio alias normalization in `ima-ai-creative-content-generator`:
...
- If `ima-knowledge-ai` is installed, it is consulted (even when user pref exists) to detect mismatches
...
| text_to_video (premium) | **Hailuo 2.3** | `MiniMax-Hailuo-2.3` | `MiniMax-Hailuo-2.3` | 32+ pts | Higher quality |
...
| image_to_video (premium) | **Kling 2.6** | `kling-v2-6` | `kling-v2-6` | 80+ pts | Premium Kling i2v |
...
- **🔥 Video from text (most popular)** → **Wan 2.6** (25, balanced); premium → Hailuo 2.3 (32+); budget → Vidu Q2 (5)
...
- **User explicitly asks for cheapest** → Prefer current lowest-cost music model from runtime product list (often DouBao BGM/Song around 30 pts, subject to change)
...
User asks for custom aspect ratio image (e.g. "7:3 landscape")
  → ⚠️ Image models support predefined aspect ratios, but non-standard custom ratios (e.g., 7:3) are not supported
  → ✅ Solution: "7:3 这类非标准比例当前不支持。可改用接近比例（如 21:9 或 16:9）；若必须严格 7:3，可先生成视频再截帧裁切。"
...
3. **User preference has been saved only if the user explicitly specified a model and generation succeeded**
...
| first_last / reference | Kling O1 | Kling 2.6 (80+pts) | Veo 3.1 (70pts+) |
...
Notes:
- `ima-ai-creative-content-generator` does not hardcode Sevio availability; it resolves by runtime `product/list`.
...
| | Sora 2 Pro | `sora-2-pro` | 122+ pts |
| | Kling O1 / 2.6 | `kling-video-o1/v2-6` | O1: 48-120 pts; 2.6: 80+ pts |
| | Google Veo 3.1 | `veo-3.1-generate-preview` | 70-330 pts |
...
| text_to_video | Sora 2 Pro / Sora 2 | `sora-2-pro` / `sora-2` | 122 / 35 pts |
| text_to_video | Kling O1 / 2.6 / 2.5 Turbo / 1.6 | `kling-video-o1` / `kling-v2-6` / `kling-v2-5-turbo` / `kling-v1-6` | 48 / 80+ / 24 / 32 pts |
| text_to_video | Google Veo 3.1 Fast / 3.1 / 3.0 | `veo-3.1-fast-generate-preview` / `veo-3.1-generate-preview` / `veo-3.0-generate-preview` | 55 / 70-330 / 280 pts |
...
| first_last_frame_to_video | Kling O1 | `kling-video-o1` | 48-120 pts |
...
**ima-ai-creative-content-generator has complete TTS capability:** This document and the bundled `ima_create.py` provide full TTS support (routing, parameters, create/poll, UX protocol Steps 0–5, error translation). The **ima-tts-ai** skill is an optional standalone package with the same specification.
...
| **text_to_music** |||||| |
| Suno (sonic-v5) | text_to_music | `sonic` | **2370** | 25 pts | Default |
...
| `appKey` | `32jdskjdk320eew` | Public fixed app key (non-user secret; used for `sign` generation) |
...
def get_upload_token(api_key: str,
    ...
    Args:
        api_key: Your IMA API key (ima_xxx...), used as both appUid and cmimToken
    ...
    r = requests.get(
        ...
        params={
            # App Key params
            "appUid":       api_key,       # IMA API key
            "appId":        APP_ID,
            "appKey":       APP_KEY,
            "cmimToken":    api_key,       # IMA API key
            "sign":         sign,
            "timestamp":    ts,
            "nonce":        nonce,
            # File params
...
```json
{
  ...
  "parameters": [{
    ...
    "parameters": {
      ...
      "n":               1,
      "input_images":    [],
      "cast":            {"points": 40, "attribute_id": 4838}
    }
  }]
}
```
...
See the Python example sections throughout this documentation for implementation guidance covering all 8 task types.
