...
const CONFIG_PATH = path.join(SKILL_DIR, 'projects.json');
...
function buildStatusText(_today, latestRun) {
...
async function main() {
  ...
  if (!feishu.appId || !feishu.appSecret || !feishu.notifyUserOpenId) {
    throw new Error('projects.json 缺少飞书告警所需配置（appId/appSecret/notifyUserOpenId）');
  }
  ...
  if (!latestRun) {
    console.log(JSON.stringify({ ok: true, notified: false, date: today, reason: 'no-finished-runs' }));
    return;
  }

  const accessToken = await getFeishuAccessToken(feishu.appId, feishu.appSecret);
  const text = buildStatusText(today, latestRun);
  await sendFeishuTextMessage(accessToken, feishu.notifyUserOpenId, text);
  console.log(JSON.stringify({ ok: true, notified: true, date: today, latestRunAt: latestRun.runAtMs || latestRun.ts || null, latestStatus: latestRun.status || 'unknown' }));
}
