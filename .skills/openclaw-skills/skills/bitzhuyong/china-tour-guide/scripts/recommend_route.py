...
def parse_markdown_file(file_path: str) -> Dict:
    ...
    try:
        ...
        if not data["spots"]:
            # 尝试从文件中提取一些内容作为景点
            content = ''.join(lines)
            if '故宫' in data["name"] or 'Forbidden' in data["name"]:
                data["spots"] = [
            ...
            elif '长城' in data["name"] or 'Great Wall' in data["name"]:
                data["spots"] = [
            ...
            elif '西湖' in data["name"] or 'West Lake' in data["name"]:
                data["spots"] = [
...
def recommend_route(attraction_data: Dict, profile_type: str, current_time: str = "14:00") -> Dict:
    ...
    for spot in recommended_spots:
        ...
        route.append({
            ...
            "stay_minutes": stay_minutes * stay_multiplier,
...
def format_output(result: Dict) -> str:
    ...
    for i, stop in enumerate(result['route'], 1):
        ...
        output.append(f"📷 {stop['photo_tip']}")
        output.append(f"📖 {stop['culture_highlight']}")
        output.append(f"➡️ {stop['next_direction']}")
        output.append("")
    
    output.append("=" * 40)
    output.append("")
    output.append("准备好出发了吗？")
    output.append("1️⃣ 开始导览")
    output.append("2️⃣ 调整路线")
    output.append("3️⃣ 查看拍照机位")
    output.append("")
...
def main():
    ...
    # 列出所有支持的景区
    if args.list:
        print("✅ ChinaTourGuide 支持 30 个核心 5A 景区：")
        print("=" * 50)
    ...
    if not attraction_data:
        ...
        print(f"✅ 支持的景区 ({len(SUPPORTED_ATTRACTIONS)}个): {', '.join(SUPPORTED_ATTRACTIONS[:10])} 等")
