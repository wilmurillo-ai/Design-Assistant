...
async function captureHomework(courseName, outputDir) {
  ...
  let noChangeCount = 0;
  const maxNoChange = 5; // 增加容错次数
  const scrollStep = 500;
  const maxScrolls = 20; // 最大滚动次数
  ...
  for (let scrollCount = 0; scrollCount < maxScrolls; scrollCount++) {
    ...
    console.log(`   当前顶部: ${currentTopStudent.name} (top: ${currentTopStudent.top})`);
    
    // 检查顶部学生是否变化
    if (prevTopStudent && prevTopStudent.name === currentTopStudent.name) {
      noChangeCount++;
      console.log(`   滚动未变化 (${noChangeCount}/${maxNoChange})`);
      
      // 只有当滚动多次无变化且已经截图过这个学生时才结束
      if (noChangeCount >= maxNoChange && screenshots.find(s => s.name === currentTopStudent.name)) {
        console.log('   滚动多次无变化且已截图，结束截图\n');
        break;
      }
    } else {
      noChangeCount = 0;
    ...
    // 检查是否已经截图过这个学生
    if (!screenshots.find(s => s.name === currentTopStudent.name)) {
      console.log(`\n[${screenshots.length + 1}] ${currentTopStudent.name}`);
