...
AI 在长对话和跨会话时会丢失上下文。我们的**三层反失忆架构**解决这个问题：

| 层级 | 引擎 | 作用 |
|------|------|------|
| **L1: MemOS** | 结构化记忆 | 每轮提取 BANT、承诺、异议，对话开始时注入 System Prompt |
| **L2: 主动摘要** | Token 监控 | 上下文占用 65% 时压缩历史，零信息丢失 — 数字、报价、承诺逐字保留 |
| **L3: ChromaDB** | 长期 RAG | 每轮存为向量，客户引用历史或 7 天后回访时自动检索 |

**效果**：AI 业务员记住每个客户、每份报价、每个承诺 — 即使 100+ 轮对话或沉默数周后。
