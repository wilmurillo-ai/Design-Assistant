...
| style | string | No | 500 | 3D风格描述（如"黄金材质"、"黏土风"、"毛茸茸"、"赛博朋克"、"玻璃质感"等） |
...
## Supported Chart Types (12 types)

| chartType | 中文名称 | 推荐场景 |
|-----------|---------|---------|
| `basic-line` | 基础折线图 | 时间序列/趋势数据 |
| `cascaded-area` | 层叠面积图 | 多系列趋势对比 |
| `stacked-area` | 堆叠面积图 | 累计趋势可视化 |
| `basic-pie` | 饼图 | 占比/分布数据 |
| `basic-column` | 基础柱状图 | 分类对比 |
| `basic-bar` | 基础条形图 | 水平分类对比 |
| `check-in-bubble` | 打卡气泡图 | 频次/热度数据 |
| `funnel` | 漏斗图 | 转化率/流程数据 |
| `donut-progress` | 圆环进度图 | 占比/完成度 |
| `bar-progress` | 条形进度图 | 单指标进度 |
| `word-cloud` | 词云图 | 关键词频率 |
| `liquid` | 水波图 | 单指标比率 |
