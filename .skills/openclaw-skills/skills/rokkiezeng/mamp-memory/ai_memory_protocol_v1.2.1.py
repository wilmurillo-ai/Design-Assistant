"""
AI Memory Protocol v1.2.0
===========================
Lightweight autonomous session memory protocol for AI agents.
Self-contained: no dependency on external session IDs.

v1.2.0 Changelog (from v1.1.9):
- NEW: Platform detection module — automatic platform-aware path handling
  for DB path, lock file, and working directory. Detects OS (linux/windows/
  macos), container environment, and architecture. Refactored DEFAULT_DB_PATH
  and _PROTOCOL_DIR to use platform-aware paths.
- NEW: AIMemoryDB.get_platform_info() — returns platform identification dict
  (os_name, os_version, hostname, container, arch, python_version)
- CHG: DB_VERSION 14 → 15 (platform_info table added)

v1.1.9 Changelog (from v1.1.8):
- NEW: SessionManager.heartbeat() — called externally by Hermes cron ~every
  5 min; flushes buffered turns and closes sessions idle >30 min
- NEW: SessionManager._close_idle_sessions(minutes=30) — auto-ends open
  sessions whose last activity timestamp exceeds the cutoff
...
# ============================================================
# Platform Detection Module — v1.2.0
# ============================================================

def _detect_platform() -> Dict[str, Any]:
    """
    Detect current platform and environment.
    Returns a dict with os_name, os_version, hostname, container, arch,
    ...
    if system == "linux":
        ...
        result["os_version"] = platform_module.release() or "unknown"
    elif system == "windows":
        result["os_name"] = "windows"
        result["os_version"] = platform_module.release() or "unknown"
    elif system == "darwin":
        result["os_name"] = "macos"
        result["os_version"] = platform_module.mac_ver()[0] or "unknown"
    else:
        result["os_name"] = system
        result["os_version"] = "unknown"

    # Architecture
    result["arch"] = platform_module.machine().lower()

    # Python version
    result["python_version"] = platform_module.python_version()

    # Python executable (useful for virtualenv detection)
    result["python_executable"] = sys.executable or "unknown"
    ...
    # Container detection (linux-only heuristics)
        ...
        if not result["container"]:
            try:
                with open("/proc/1/cgroup", "r") as f:
                    ...
                    if any(x in cgroup_content for x in ["docker", "containerd", "cri", "kubepod"]):
                        result["container"] = True
            except Exception:
                pass
...
# Detect once at import time; reuse across all instances
_PLATFORM_INFO: Dict[str, Any] = _detect_platform()


def get_platform_info() -> Dict[str, Any]:
    """Return cached platform detection results."""
    return dict(_PLATFORM_INFO)


def _platform_default_db_path() -> str:
    """
    Platform-aware default DB path.
    Respects MARK_MEMORY_DB env var; otherwise uses:
    - container: /opt/claw/hermes/protocol/mark_memory.db
    - host: /vol2/1000/claw/hermes/protocol/mark_memory.db
    """
    env_path = os.environ.get("MARK_MEMORY_DB", "")
    if env_path:
        return env_path
    return str(_platform_protocol_dir() / "mark_memory.db")


def _platform_protocol_dir() -> Path:
    """
    Return platform-aware protocol directory.
    - container: /opt/claw/hermes/protocol/
    - host: /vol2/1000/claw/hermes/protocol/
    """
    if _PLATFORM_INFO.get("container", False):
        return Path("/opt/claw/hermes/protocol")
    return Path("/vol2/1000/claw/hermes/protocol")
...
# Default DB path — uses platform-aware resolver
DEFAULT_DB_PATH = _platform_default_db_path()

# Paths — use platform-aware protocol directory, no system paths
_PROTOCOL_DIR = _platform_protocol_dir()
...
_MIGRATIONS: Dict[int, str] = {
    ...
    15: "CREATE TABLE IF NOT EXISTS platform_info (key TEXT PRIMARY KEY, value TEXT NOT NULL);",
}
...
class AIMemoryDB:
    """SQLite implementation of AI Memory Protocol v1.2.0."""
    ...
    def _init_db(self):
        ...
        # v1.2.0: Persist platform info to DB (idempotent — INSERT OR IGNORE)
        try:
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
            cursor.execute("PRAGMA busy_timeout=5000;")
            for key, value in _PLATFORM_INFO.items():
                cursor.execute(
                    "INSERT OR IGNORE INTO platform_info (key, value) VALUES (?, ?)",
                    (key, str(value))
                )
            conn.commit()
            conn.close()
    ...
    # ------------------------------------------------------------------
    # Platform Info — v1.2.0
    # ------------------------------------------------------------------

    def get_platform_info(self) -> Dict[str, Any]:
        """
        NEW v1.2.0: Returns cached platform detection info.
        Returns the same dict as the module-level get_platform_info().
        """
        return dict(_PLATFORM_INFO)
...
class SessionManager:
    ...
    def _enable_auto_record(self):
        """Scan sys.modules for agent objects and wrap their message-send methods."""
        import inspect
            ...
            try:
                ...
                for mod_name, mod in sys.modules.items():
                    ...
                    if not any(ag in mod_name.lower() for ag in agent_names):
                        continue
                    # Iterate module attributes and their sub-attributes (instances/classes)
                    for attr_name in dir(mod):
                        ...
                        candidates = []
                        if callable(top_obj) and not inspect.isclass(top_obj) \
                           and not inspect.ismodule(top_obj):
                            candidates.append((attr_name, top_obj, mod))
                        if inspect.isclass(top_obj) or (not callable(top_obj) and hasattr(top_obj, '__dict__')):
                        ...
                        if not candidates:
                            continue
                        for full_name, candidate, parent in candidates:
                                    ...
                                    def wrapper(*args, **kwargs):
                                        ...
                                        for a in args:
                                            if isinstance(a, str) and a.strip():
                                                ...
                                                break
                                        if content:
                                            self.add_turn(r, content)
                                        return orig(*args, **kwargs)
                                    return wrapper
                                wrapped_candidate = make_wrapper(original, role)
                                setattr(parent, full_name.split(".")[-1], wrapped_candidate)
                                wrapped[full_name] = (original, role, parent, full_name.split(".")[-1])
                            except Exception:
                                continue
    ...
    def _disable_auto_record(self):
        ...
        with self._auto_record_lock:
            if not self._auto_record:
                return
            try:
                for full_name, original, parent, attr in self._wrapped_methods.values():
                    try:
    ...
    def heartbeat(self):
        """Called externally by Hermes cron every ~5 minutes.
        Flushes buffered turns to DB and closes idle sessions (>30 min)."""
        self._flush()
        self._close_idle_sessions(30)

    def _close_idle_sessions(self, minutes: int = 30):
        """End open sessions idle for more than `minutes`."""
        try:
            open_sessions = self.db.get_open_sessions()
            now = datetime.now(timezone.utc)
            cutoff = (now - timedelta(minutes=minutes)).isoformat()
            for sess in open_sessions:
                sid = sess.get("local_session_id")
                if not sid:
                    continue
                last_act = self.db.get_last_activity(sid)
                if last_act and last_act < cutoff:
                    self.db.end_session(sid, summary=f"Auto-ended: idle > {minutes} min")
        except Exception:
            pass
    ...
    # ---- v1.2.0: Full AIMemoryDB delegation ----
    ...
    def get_platform_info(self) -> Dict[str, Any]:
        """v1.2.0: returns cached platform detection info."""
        return self.db.get_platform_info()
