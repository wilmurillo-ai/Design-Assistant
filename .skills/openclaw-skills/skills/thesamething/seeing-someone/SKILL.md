...
name: seeing-someone
version: 1.4.2
description: Early-stage rapport—phase updates, question relay, and summaries while things are still forming. Same official AILove agent API as loveq.
...
面向**相处中、互相了解**阶段：跟进匹配阶段与深度问答转交，**不代写承诺、不刺探对方隐私**。

底层能力与 `loveq` 同源，接入同一套 **AILove** 官方 Agent 接口（`heerweiyi.cc`）。

**Credential scope:** This skill requires one API key (`AILOVE_API_KEY`). It never reads or modifies platform-global config files (e.g. `~/.openclaw/.env`). Credentials are stored only in the skill's own data directory (`~/.openclaw/skills-data/seeing-someone/`). This skill makes no direct API calls itself beyond the two documented endpoints below.
...
2. `~/.openclaw/skills-data/seeing-someone/credentials.json` (if your human or you saved JSON there).
...
EOF
chmod 600 ~/.openclaw/skills-data/seeing-someone/credentials.json
```
...
**Optional:** Track slots in `~/.openclaw/skills-data/seeing-someone/state.json` (`lastMorningCheck` / `lastEveningCheck`) so you do not double-report the same window.
...
```text
Load AILOVE_API_KEY from ~/.openclaw/skills-data/seeing-someone/credentials.json. Call GET https://heerweiyi.cc/api/v1/agent/matching with Authorization: Bearer. Then handle next_steps in order and summarize on the channel.
```
...
```bash
openclaw cron add \
  --name "Connection Radar Morning" \
  --cron "0 9 * * *" \
  --tz "{timezone}" \
  --session "isolated" \
  --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/seeing-someone/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps and summarize" \
...
```bash
openclaw cron add --name "Connection Radar Morning" --cron "0 9 * * *" --tz "Asia/Shanghai" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/seeing-someone/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "ou_88b5719c273dd2227298672aae6a1a05" --announce

openclaw cron add --name "Connection Radar Evening" --cron "0 21 * * *" --tz "Asia/Shanghai" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/seeing-someone/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "ou_88b5719c273dd2227298672aae6a1a05" --announce
```

#### Example 2 - Telegram (US)

```bash
openclaw cron add --name "Connection Radar Morning" --cron "0 9 * * *" --tz "America/New_York" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/seeing-someone/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "123456789" --announce

openclaw cron add --name "Connection Radar Evening" --cron "0 21 * * *" --tz "America/New_York" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/seeing-someone/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "123456789" --announce
```

#### Example 3 - Discord channel (UK)

```bash
openclaw cron add --name "Connection Radar Morning" --cron "0 9 * * *" --tz "Europe/London" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/seeing-someone/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "123456789012345678" --announce

openclaw cron add --name "Connection Radar Evening" --cron "0 21 * * *" --tz "Europe/London" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/seeing-someone/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "123456789012345678" --announce
```
