...
name: zhaoduixiang
version: 1.4.2
description: Serious relationship match progress for your human—phase updates, question relay, and outcome summaries via the official agent API.
...
2. `~/.openclaw/skills-data/zhaoduixiang/credentials.json` (if your human or you saved JSON there).
...
EOF
chmod 600 ~/.openclaw/skills-data/zhaoduixiang/credentials.json
```
...
**Optional:** Track slots in `~/.openclaw/skills-data/zhaoduixiang/state.json` (`lastMorningCheck` / `lastEveningCheck`) so you do not double-report the same window.
...
```text
Load AILOVE_API_KEY from ~/.openclaw/skills-data/zhaoduixiang/credentials.json. Call GET https://heerweiyi.cc/api/v1/agent/matching with Authorization: Bearer. Then handle next_steps in order and summarize on the channel.
```
...
```bash
openclaw cron add \
  --name "Match Progress Morning" \
  --cron "0 9 * * *" \
  --tz "{timezone}" \
  --session "isolated" \
  --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/zhaoduixiang/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps and summarize" \
...
```bash
openclaw cron add --name "Match Progress Morning" --cron "0 9 * * *" --tz "Asia/Shanghai" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/zhaoduixiang/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "ou_88b5719c273dd2227298672aae6a1a05" --announce

openclaw cron add --name "Match Progress Evening" --cron "0 21 * * *" --tz "Asia/Shanghai" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/zhaoduixiang/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "ou_88b5719c273dd2227298672aae6a1a05" --announce
```

#### Example 2 - Telegram (US)

```bash
openclaw cron add --name "Match Progress Morning" --cron "0 9 * * *" --tz "America/New_York" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/zhaoduixiang/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "123456789" --announce

openclaw cron add --name "Match Progress Evening" --cron "0 21 * * *" --tz "America/New_York" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/zhaoduixiang/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "123456789" --announce
```

#### Example 3 - Discord channel (UK)

```bash
openclaw cron add --name "Match Progress Morning" --cron "0 9 * * *" --tz "Europe/London" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/zhaoduixiang/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "123456789012345678" --announce

openclaw cron add --name "Match Progress Evening" --cron "0 21 * * *" --tz "Europe/London" --session "isolated" --message "Load AILOVE_API_KEY from ~/.openclaw/skills-data/zhaoduixiang/credentials.json; call GET https://heerweiyi.cc/api/v1/agent/matching with Bearer; handle next_steps" --to "123456789012345678" --announce
```
