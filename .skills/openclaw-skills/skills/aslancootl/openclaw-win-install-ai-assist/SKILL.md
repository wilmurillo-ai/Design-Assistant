...
* 能够访问 GitHub 或配置了国内镜像源。如果网络连接不稳定，可以在开始前预先将安装文件下载到本地。

### **Pre-Download Installation Files (Optional, for Poor Network) / 准备安装文件（可选，网络环境不佳时）**
...
1. **WSL2 Update Package**  
   **WSL2 更新包**  
   * Download from: https://github.com/microsoft/WSL/releases/latest  
   * File: wsl.x.x.x.x64.msi  
   * Size: \~8 MB  
2. **Ubuntu 22.04 LTS for WSL**  
   **Ubuntu 22.04 LTS 安装镜像**  
   * Download from: https://cloud-images.ubuntu.com/wsl/jammy/current/  
   * File: ubuntu-jammy-wsl-amd64-wsl.rootfs.tar.gz  
   * Size: \~500 MB  
3. **Node.js for Linux**  
   **Linux 版 Node.js**  
   * Download from: https://nodejs.org/dist/v22.18.0/  
   * File: node-v22.18.0-linux-x64.tar.xz  
   * Size: \~30 MB  
4. **OpenClaw Source Code**  
   **OpenClaw 源码**  
   * Download from: https://github.com/openclaw-ai/openclaw/archive/refs/heads/main.zip  
   * File: openclaw-main.zip  
   * Size: \~40 MB
...
2. 核对版本：  
   ...
   * Windows 11 任意版本都符合  
3. 如果版本不达标：打开「设置 → Windows 更新 → 检查更新」，升级到最新稳定版再继续
...
2. 切换到「性能」选项卡，点击「CPU」  
3. 在右下角查看「虚拟化：已启用」  
4. 如果未启用：重启电脑进 BIOS 开启 CPU 虚拟化，搜索「你的电脑型号 开启虚拟化」获取教程
...
后续所有操作**必须**以管理员身份运行终端：

1. 按 Win \+ X  
2. 选择「Windows 终端（管理员）」（Windows 10 选择「Windows PowerShell（管理员）」）  
3. 同意 UAC 提示，进入管理员终端
...
## Chapter 8: Browser Extension & Desktop Shortcuts / 第八章：浏览器扩展与桌面快捷方式

### 8.1 Browser Extension / 浏览器扩展
...
### 8.2 Create Ubuntu Desktop Shortcut / 创建 Ubuntu 桌面快捷方式
...
### 8.3 Create OpenClaw Desktop Shortcut / 创建 OpenClaw 桌面快捷方式
...
### 8.4 Windows Desktop Shortcuts / Windows 桌面快捷方式
...
1. Right-click on desktop → New → Shortcut
   右键点击桌面 → 新建 → 快捷方式
2. For WSL Ubuntu, enter location:
   对于 WSL Ubuntu，输入位置：
   ```
   wsl -d Ubuntu-22.04
   ```

3. For OpenClaw Gateway, enter location:
   对于 OpenClaw Gateway，输入位置：
   ```
