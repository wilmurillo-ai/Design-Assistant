...
用法: python poll_task.py <HOST> <API_KEY> <TASK_ID> [INTERVAL_SECONDS]
"""
...
def poll_task(host: str, api_key: str, task_id: str, interval: int = 5, max_duration: int = 1200) -> dict:
    """
    轮询任务状态直到完成或失败
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python poll_task.py <HOST> <API_KEY> <TASK_ID> [INTERVAL_SECONDS]", file=sys.stderr)
        sys.exit(1)
