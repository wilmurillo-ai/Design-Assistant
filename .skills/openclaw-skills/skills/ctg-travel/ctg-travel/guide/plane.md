...
**表格列**：序号、舱位类型、票价、机建费、燃油费、总价、剩余座位、预定限制（关注`productDesc`和`memberDesc`两个字段，描述的是预定限制内容）
...
**入参**：`resourceItemId`(步骤2的 `data.goFlightCabin.cabinList[].resourceItemId`)、`goExtData`(步骤1的 `flights[].extData`)、`passengerIds`、`contactPhone`、`sessionId`(步骤2的 `data.sessionId`)、`extData`(步骤2的 `data.goFlightCabin.cabinList[].extData`)
...
**规范**：每10秒查询一次，最多10次，下单后**自动轮询**

| 状态码 | 状态描述      |  处理                 |
|------|-----------|-------------------|
| `1`  | 占位中       | 继续轮询，提示「正在为您占位，请稍候…」 |
| `10` | 占位成功（待支付） | 轮询结束，展示订单，引导支付      |
| `9`  | 占位失败      | 轮询结束，引导重新预订         |

**占位完成展示**：订单号、航班、日期、舱位、乘客、金额、提示「请前往中旅旅行App完成支付」
**创建订单失败或占位失败展示**：创建订单失败或占位失败原因、提示「请前往中旅旅行App购买机票」
**轮询结束后仍是占位中展示**：系统仍在为您占位处理中，请稍后再来询问我占位进度
