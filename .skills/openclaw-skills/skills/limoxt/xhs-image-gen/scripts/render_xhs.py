...
def generate_cover_html(metadata: dict, theme: str, width: int, height: int, avatar_path: str = "") -> str:
    """生成封面 HTML"""
    ...
    theme_backgrounds = {
        ...
        'rex': 'linear-gradient(180deg, #0d1117 0%, #161b22 100%)',
    ...
    title_gradients = {
        ...
        'rex': 'linear-gradient(180deg, #f97316 0%, #ef4444 100%)',
...
<body>
    ...
    <!-- Rex 头像水印 -->
    <img class="rex-avatar" src="file://{avatar_path}" alt="Rex">
</body>
</html>'''
    return html
...
def generate_card_html(content: str, theme: str, page_number: int = 1, 
                       ...
                       height: int = DEFAULT_HEIGHT, mode: str = 'separator', avatar_path: str = "") -> str:
    """生成正文卡片 HTML"""
    ...
    theme_backgrounds = {
        ...
        'rex': 'linear-gradient(135deg, #0d1117 0%, #161b22 100%)',
...
async def render_markdown_to_cards(md_file: str, output_dir: str, 
    ...
    if metadata.get('emoji') or metadata.get('title'):
        print("  📷 生成封面...")
        cover_html = generate_cover_html(metadata, theme, width, height, avatar_path)
    ...
    for i, content in enumerate(card_contents, 1):
        ...
        card_html = generate_card_html(content, theme, i, total_cards, width, height, mode, avatar_path)
