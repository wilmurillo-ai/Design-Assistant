#!/usr/bin/env python3
"""
LCP/1.1 Node for Windows
Mirror of lcp_node.py adapted for Windows:
- No Unix socket IPC (uses TCP localhost:9529 instead)
- Windows paths
- No LaunchAgent (use Task Scheduler or nssm)

Deploy:
    1. Copy this file + lcp_crypto_win.py to D:\lobster-comm\
    2. pip install pynacl
    3. python lcp_node_win.py

Requirements:
    - Python 3.9+
    - PyNaCl (pip install pynacl)
    - Tailscale running (UDP port 9528 reachable)
"""

import sys
import os
import json
import uuid
import socket
import struct
import threading
import time
import signal
import glob
import logging
from datetime import datetime, timezone, timedelta

# Import crypto module (same directory)
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
try:
    from lcp_crypto_win import sign_message, verify_message
except ImportError:
    print("ERROR: lcp_crypto_win.py not found. Copy it to same directory.", file=sys.stderr)
    sys.exit(1)

# ── Config ──────────────────────────────────────────────────────────────
LCP_PORT = 9528
IPC_PORT = 9529  # TCP localhost for Windows IPC (no Unix sockets)
MAGIC = b'LCP1'
MAX_RETRIES = 5
RETRY_BASE_MS = 500
ACK_TIMEOUT_S = 3.0
HEARTBEAT_INTERVAL_S = 30
PEER_TIMEOUT_S = 120

MY_NAME = "AgentB"
PEER_NAME = "AgentA"
MY_IP = "100.y.y.y"  # Your Tailscale IP
PEER_IP = "100.x.x.x"  # Peer Tailscale IP

# Windows paths
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DATA_DIR = os.path.join(BASE_DIR, "data")
INBOX_DIR = os.path.join(DATA_DIR, "inbox")
INBOX_ARCHIVE = os.path.join(DATA_DIR, "inbox_archive")
OUTBOX_DIR = os.path.join(DATA_DIR, "outbox")
SEEN_FILE = os.path.join(DATA_DIR, "seen_ids.json")
KEY_DIR = os.path.join(BASE_DIR, "keys")

for d in [INBOX_DIR, INBOX_ARCHIVE, OUTBOX_DIR, KEY_DIR]:
    os.makedirs(d, exist_ok=True)

# ── Logging ─────────────────────────────────────────────────────────────
LOG_FILE = os.path.join(DATA_DIR, "lcp_node.log")
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
    datefmt="%Y-%m-%d %H:%M:%S",
    handlers=[
        logging.FileHandler(LOG_FILE, encoding="utf-8"),
        logging.StreamHandler(sys.stdout),
    ],
)
log = logging.getLogger("lcp")

# ── Packet ──────────────────────────────────────────────────────────────
MSG_DATA = 0x01
MSG_ACK = 0x02
MSG_HEARTBEAT = 0x03

seq_counter = 0
seq_lock = threading.Lock()

def next_seq():
    global seq_counter
    with seq_lock:
        seq_counter = (seq_counter + 1) % 0xFFFFFFFF
        return seq_counter

def pack(msg_type, seq, payload=b''):
    return struct.pack('!4sIB', MAGIC, seq, msg_type) + payload

def unpack(data):
    if len(data) < 9:
        return None
    magic, seq, msg_type = struct.unpack('!4sIB', data[:9])
    if magic != MAGIC:
        return None
    return seq, msg_type, data[9:]

# ── Seen IDs ────────────────────────────────────────────────────────────
seen_ids = set()
seen_lock = threading.Lock()

def load_seen():
    global seen_ids
    try:
        if os.path.exists(SEEN_FILE):
            with open(SEEN_FILE, 'r') as f:
                seen_ids = set(json.load(f))
    except:
        seen_ids = set()

def save_seen():
    with seen_lock:
        ids = list(seen_ids)[-10000:]
        with open(SEEN_FILE, 'w') as f:
            json.dump(ids, f)

def mark_seen(msg_id):
    with seen_lock:
        seen_ids.add(msg_id)
    if len(seen_ids) % 50 == 0:
        save_seen()

def is_seen(msg_id):
    with seen_lock:
        return msg_id in seen_ids

# ── ACKs & Peer ─────────────────────────────────────────────────────────
pending_acks = {}
ack_lock = threading.Lock()
peer_last_seen = 0
peer_status_lock = threading.Lock()

def update_peer_seen():
    global peer_last_seen
    with peer_status_lock:
        peer_last_seen = time.time()

def is_peer_online():
    with peer_status_lock:
        return (time.time() - peer_last_seen) < PEER_TIMEOUT_S

# ── UDP ─────────────────────────────────────────────────────────────────
udp_sock = None
shutdown_flag = threading.Event()

def init_udp():
    global udp_sock
    udp_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    udp_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    udp_sock.bind(("0.0.0.0", LCP_PORT))
    udp_sock.settimeout(1.0)
    log.info(f"UDP bound on 0.0.0.0:{LCP_PORT}")

def udp_send_raw(packet, target_ip=PEER_IP):
    try:
        udp_sock.sendto(packet, (target_ip, LCP_PORT))
    except Exception as e:
        log.error(f"UDP send error: {e}")

def send_message(payload_dict):
    tz = timezone(timedelta(hours=8))
    if "id" not in payload_dict:
        payload_dict["id"] = str(uuid.uuid4())
    if "from" not in payload_dict:
        payload_dict["from"] = MY_NAME
    if "to" not in payload_dict:
        payload_dict["to"] = PEER_NAME
    if "timestamp" not in payload_dict:
        payload_dict["timestamp"] = datetime.now(tz).isoformat()

    try:
        signed = sign_message(payload_dict)
    except Exception as e:
        log.error(f"Signing failed: {e}")
        return False, payload_dict

    payload_bytes = json.dumps(signed, ensure_ascii=False).encode('utf-8')
    seq = next_seq()
    packet = pack(MSG_DATA, seq, payload_bytes)

    ack_event = threading.Event()
    with ack_lock:
        pending_acks[seq] = ack_event

    success = False
    for attempt in range(MAX_RETRIES):
        delay = (RETRY_BASE_MS * (2 ** attempt)) / 1000.0
        udp_send_raw(packet)
        log.info(f"Sent seq={seq} attempt={attempt+1}/{MAX_RETRIES}")
        if ack_event.wait(min(delay, ACK_TIMEOUT_S)):
            success = True
            log.info(f"ACK received for seq={seq}")
            break

    with ack_lock:
        pending_acks.pop(seq, None)

    if success:
        fname = f"msg_{signed['id'][:8]}.json"
        with open(os.path.join(OUTBOX_DIR, fname), 'w', encoding='utf-8') as f:
            json.dump(signed, f, ensure_ascii=False, indent=2)
    else:
        log.warning(f"Failed to deliver seq={seq}")

    return success, signed

# ── UDP Listener ────────────────────────────────────────────────────────
def udp_listener():
    while not shutdown_flag.is_set():
        try:
            data, addr = udp_sock.recvfrom(65535)
        except socket.timeout:
            continue
        except OSError:
            break

        parsed = unpack(data)
        if not parsed:
            continue

        seq, msg_type, payload = parsed
        update_peer_seen()

        if msg_type == MSG_ACK:
            with ack_lock:
                ev = pending_acks.get(seq)
                if ev:
                    ev.set()

        elif msg_type == MSG_HEARTBEAT:
            udp_send_raw(pack(MSG_ACK, seq), addr[0])

        elif msg_type == MSG_DATA:
            udp_send_raw(pack(MSG_ACK, seq), addr[0])
            try:
                msg = json.loads(payload.decode('utf-8'))
            except json.JSONDecodeError:
                continue

            msg_id = msg.get("id", "")
            if is_seen(msg_id):
                continue

            try:
                if not verify_message(msg):
                    log.warning(f"Invalid signature from {addr}")
                    continue
            except Exception as e:
                log.warning(f"Sig error: {e}")
                continue

            mark_seen(msg_id)
            fname = f"msg_{msg_id[:8]}.json"
            with open(os.path.join(INBOX_DIR, fname), 'w', encoding='utf-8') as f:
                json.dump(msg, f, ensure_ascii=False, indent=2)
            log.info(f"Received [{msg.get('type')}] from {msg.get('from')}: {msg.get('message','')[:60]}")

# ── Heartbeat ───────────────────────────────────────────────────────────
def heartbeat_loop():
    while not shutdown_flag.is_set():
        seq = next_seq()
        udp_send_raw(pack(MSG_HEARTBEAT, seq))
        shutdown_flag.wait(HEARTBEAT_INTERVAL_S)

# ── TCP IPC (Windows replacement for Unix socket) ──────────────────────
def handle_ipc_client(conn):
    try:
        raw = conn.recv(65536)
        if not raw:
            return
        req = json.loads(raw.decode('utf-8'))
        cmd = req.get("cmd", "").upper()

        if cmd == "SEND":
            payload = req.get("payload", {})
            ok, msg = send_message(payload)
            resp = {"ok": ok, "id": msg.get("id")}

        elif cmd == "CHECK":
            messages = []
            for fp in sorted(glob.glob(os.path.join(INBOX_DIR, "msg_*.json"))):
                try:
                    with open(fp, 'r', encoding='utf-8-sig') as f:
                        m = json.load(f)
                    m["_filepath"] = fp
                    messages.append(m)
                except:
                    pass
            resp = {"count": len(messages), "messages": messages}

        elif cmd == "ACK":
            archived = 0
            for fp in glob.glob(os.path.join(INBOX_DIR, "msg_*.json")):
                try:
                    os.rename(fp, os.path.join(INBOX_ARCHIVE, os.path.basename(fp)))
                    archived += 1
                except:
                    pass
            resp = {"archived": archived}

        elif cmd == "STATUS":
            inbox_count = len(glob.glob(os.path.join(INBOX_DIR, "msg_*.json")))
            resp = {
                "peer": PEER_NAME, "peer_ip": PEER_IP,
                "peer_online": is_peer_online(),
                "peer_last_seen": peer_last_seen,
                "inbox_count": inbox_count,
                "uptime": time.time() - start_time,
            }

        else:
            resp = {"error": f"Unknown: {cmd}"}

        conn.sendall(json.dumps(resp, ensure_ascii=False).encode('utf-8'))
    except Exception as e:
        try:
            conn.sendall(json.dumps({"error": str(e)}).encode('utf-8'))
        except:
            pass
    finally:
        conn.close()

def ipc_server():
    srv = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    srv.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    srv.bind(("127.0.0.1", IPC_PORT))
    srv.listen(5)
    srv.settimeout(1.0)
    log.info(f"IPC listening on 127.0.0.1:{IPC_PORT}")

    while not shutdown_flag.is_set():
        try:
            conn, _ = srv.accept()
            threading.Thread(target=handle_ipc_client, args=(conn,), daemon=True).start()
        except socket.timeout:
            continue
        except OSError:
            break
    srv.close()

# ── Main ────────────────────────────────────────────────────────────────
start_time = 0

def cleanup(_sig=None, _frame=None):
    log.info("Shutting down...")
    shutdown_flag.set()
    save_seen()
    sys.exit(0)

def main():
    global start_time
    start_time = time.time()
    signal.signal(signal.SIGINT, cleanup)
    signal.signal(signal.SIGTERM, cleanup)

    load_seen()
    init_udp()

    threads = [
        threading.Thread(target=udp_listener, daemon=True),
        threading.Thread(target=heartbeat_loop, daemon=True),
        threading.Thread(target=ipc_server, daemon=True),
    ]
    for t in threads:
        t.start()

    log.info(f"LCP/1.1 Node (Windows) — {MY_NAME} ({MY_IP}) ↔ {PEER_NAME} ({PEER_IP})")
    log.info(f"UDP :{LCP_PORT} | IPC 127.0.0.1:{IPC_PORT}")

    try:
        while not shutdown_flag.is_set():
            shutdown_flag.wait(1.0)
    except KeyboardInterrupt:
        cleanup()

if __name__ == "__main__":
    main()
