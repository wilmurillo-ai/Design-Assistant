...
export interface ComputerExecutor {
    ...
    previewHideSet(allowlistBundleIds: string[], displayId?: number): Promise<Array<{
        bundleId: string;
        displayName: string;
    }>>;
    getDisplaySize(displayId?: number): Promise<DisplayGeometry>;
    listDisplays(): Promise<DisplayGeometry[]>;
    findWindowDisplays(bundleIds: string[]): Promise<Array<{
        bundleId: string;
        displayIds: number[];
    }>>;
    ...
    zoom(regionLogical: {
        ...
        w: number;
        h: number;
    }, allowedBundleIds: string[], displayId?: number): Promise<{
        base64: string;
        width: number;
        height: number;
    }>;
