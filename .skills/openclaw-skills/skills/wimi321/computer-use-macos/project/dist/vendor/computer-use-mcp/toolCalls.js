...
function scaleCoord(rawX, rawY, mode, display, lastScreenshot, logger) {
    ...
    logger.warn("[computer-use] pixels-mode coordinate received with no prior screenshot; " +
...
const TIER_ANTI_SUBVERSION = " Do not attempt to work around this restriction — never use AppleScript, " +
...
async function runInputActionGates(adapter, overrides, subGates, actionKind) {
    ...
    if (frontmostTier !== undefined) {
        ...
        if (frontmostTier === "read") {
            ...
            const isBrowser = getDeniedCategoryForApp(frontmost.bundleId, frontmost.displayName) ===
                "browser";
            return errorResult(`"${frontmost.displayName}" is granted at tier "read" — ` +
                ...
                TIER_ANTI_SUBVERSION, "tier_insufficient");
        }
        // frontmostTier === "click" (tier === "full" would have passed tierSatisfies)
        if (actionKind === "keyboard") {
            return errorResult(`"${frontmost.displayName}" is granted at tier "click" — ` +
                ...
                `For shell commands, use the Bash tool.` + TIER_ANTI_SUBVERSION, "tier_insufficient");
        }
        ...
        return errorResult(`"${frontmost.displayName}" is granted at tier "click" — ` +
            ...
            `left_click is allowed here.` + TIER_ANTI_SUBVERSION, "tier_insufficient");
    }
    ...
    if (frontmost.bundleId === hostBundleId) {
        ...
        return errorResult("Claude's own window still has keyboard focus. This should not happen " +
    ...
    return errorResult(`"${frontmost.displayName}" is not in the allowed applications and is ` +
        `currently in front. Take a new screenshot — it may have appeared ` +
        `since your last one.`, "app_not_granted");
}
/**
...
async function runHitTestGate(adapter, overrides, subGates, x, y, actionKind) {
    ...
    if (!target)
        return null; // desktop / nothing under point / platform no-op
    ...
    if (!tierByBundleId.has(target.bundleId)) {
        ...
        return errorResult(`Click at these coordinates would land on "${target.displayName}", ` +
            `which is not in the allowed applications. Take a fresh screenshot ` +
            `to see the current window layout.`, "app_not_granted");
    }
...
export function resetMouseButtonHeld() {
 ...
 */
export function defersLockAcquire(toolName) {
    return (toolName === "request_access" ||
...
async function handleRequestAccess(adapter, args, overrides, tccState) {
    ...
    if (tccState) {
        ...
        if (recheck.granted) {
            return errorResult("macOS Accessibility and Screen Recording are now both granted. " +
        ...
        return errorResult(`macOS ${missing.join(" and ")} permission(s) not yet granted. ` +
            `The permission panel has been shown. Once the user grants the ` +
            `missing permission(s), call request_access again.`, "tcc_not_granted");
    }
    ...
    const { needDialog, skipDialogGrants, willHide, tieredApps, userDenied, policyDenied, } = await buildAccessRequest(adapter, apps, overrides.allowedApps, new Set(overrides.userDeniedBundleIds), overrides.selectedDisplayId);
    let dialogGranted = [];
    let dialogDenied = [];
...
async function buildAccessRequest(adapter, apps, allowedApps, userDeniedBundleIds, selectedDisplayId) {
    ...
    const now = Date.now();
    const skipDialogGrants = skipDialog
        .filter((r) => r.resolved)
        .map((r) => {
...
function buildTierGuidanceMessage(tiered) {
    ...
    if (readOther.length > 0) {
        ...
        parts.push(`${names} ${readOther.length === 1 ? "is" : "are"} granted at tier ` +
    ...
    if (clickTier.length > 0) {
        ...
        parts.push(`${names} ${clickTier.length === 1 ? "has" : "have"} terminal or IDE ` +
...
function buildUserDeniedGuidance(userDenied) {
    ...
    return (`${names} ${one ? "is" : "are"} in the user's auto-deny list ` +
...
function buildPolicyDeniedGuidance(policyDenied) {
    ...
    return (`${names} ${one ? "is" : "are"} blocked by policy for computer use. ` +
...
async function handleRequestTeachAccess(adapter, args, overrides, tccState) {
    ...
    if (tccState) {
        ...
        return errorResult(`macOS ${missing.join(" and ")} permission(s) not yet granted. ` +
            `The permission panel has been shown. Once the user grants the ` +
            `missing permission(s), call request_teach_access again.`, "tcc_not_granted");
    }
    const rawApps = args.apps;
    if (!Array.isArray(rawApps) || !rawApps.every((a) => typeof a === "string")) {
        return errorResult('"apps" must be an array of strings.', "bad_args");
...
async function validateTeachStepArgs(raw, adapter, overrides, label) {
    ...
    for (const [i, act] of actions.entries()) {
        ...
        if (!BATCHABLE_ACTIONS.has(action)) {
            return new Error(`${label}: actions[${i}].action="${action}" is not allowed. ` +
...
async function buildHiddenNote(adapter, hiddenSinceLastSeen) {
    ...
    return (`${list} ${one ? "was" : "were"} open and got hidden before this screenshot ` +
...
async function buildMonitorNote(adapter, shotDisplayId, lastDisplayId, canSwitchDisplay) {
    ...
    if (lastDisplayId !== shotDisplayId) {
        ...
        return (`This screenshot was taken on monitor "${current}", which is different ` +
...
async function handleClickVariant(adapter, args, overrides, subGates, button, count) {
    ...
    if (args.text !== undefined) {
        ...
        if (isSystemKeyCombo(args.text, adapter.executor.capabilities.platform) &&
            !overrides.grantFlags.systemKeyCombos) {
            return errorResult(`The modifier chord "${args.text}" would fire a system shortcut. ` +
                "Request the systemKeyCombos grant flag via request_access, or use " +
                "only modifier keys (shift, ctrl, alt, cmd) in the text parameter.", "grant_flag_required");
        }
    ...
    const clickActionKind = button !== "left" || (modifiers !== undefined && modifiers.length > 0)
        ? "mouse_full"
        : "mouse";
...
async function handleKey(adapter, args, overrides, subGates) {
    ...
    if (isSystemKeyCombo(keySequence, adapter.executor.capabilities.platform) &&
        !overrides.grantFlags.systemKeyCombos) {
...
async function handleOpenApplication(adapter, args, overrides) {
    ...
    if (overrides.onDisplayPinned !== undefined) {
        ...
        if (displayCount >= 2) {
            return okText(`Opened "${app}". If it isn't visible in the next screenshot, it may ` +
...
async function handleWriteClipboard(adapter, args, overrides, subGates) {
    ...
    if (subGates.clipboardGuard) {
        ...
        if (frontmost && frontmostTier === "click") {
            return errorResult(`"${frontmost.displayName}" is a tier-"click" app and currently ` +
...
async function handleHoldKey(adapter, args, overrides, subGates) {
    ...
    if (isSystemKeyCombo(text, adapter.executor.capabilities.platform) &&
        !overrides.grantFlags.systemKeyCombos) {
...
async function handleComputerBatch(adapter, args, overrides, subGates) {
    ...
    for (const [i, act] of actions.entries()) {
        ...
        if (!BATCHABLE_ACTIONS.has(action)) {
            return errorResult(`actions[${i}].action="${action}" is not allowed in a batch. ` +
...
// Main dispatch
// ---------------------------------------------------------------------------
export async function handleToolCall(adapter, name, args, rawOverrides) {
    ...
    const overrides = rawOverrides.allowedApps.some((a) => a.tier === undefined ||
    ...
    if (!osPerms.granted) {
        ...
        if (name !== "request_access" && name !== "request_teach_access") {
            return errorResult("Accessibility and Screen Recording permissions are required. " +
                "Call request_access to show the permission panel.", "tcc_not_granted");
        }
        tccState = {
    ...
    if (lock) {
        if (lock.holder !== undefined && !lock.isSelf) {
            return errorResult("Another Claude session is currently using the computer. Wait for " +
