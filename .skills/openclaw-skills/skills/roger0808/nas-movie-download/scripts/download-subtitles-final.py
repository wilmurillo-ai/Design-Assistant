...
def final_search_attempt(conn, folder_path, video_filename):
    """最终搜索尝试 - 使用多种策略"""
    ...
    try:
        ...
        # 策略2: 使用 subliminal 的所有 provider 和语言
        providers = [
        ...
        langs = ['zh', 'zho', 'chi', 'zh-cn', 'zh-tw', 'chs', 'cht', 'en', 'eng', 'cn']
...
def process_folder(conn, folder_name):
    ...
    for video in sorted(missing):
        success, status = final_search_attempt(conn, folder_path, video)
