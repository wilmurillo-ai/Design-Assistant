...
def extract_episode_info(filename):
    ...
    if 'Young' in filename and 'Sheldon' in filename:
...
def search_with_variations(conn, folder_path, video_filename):
    """使用多种文件名变体搜索"""
    ...
    info = extract_episode_info(video_filename)
    if not info:
        return False, "no_info"
    
    print(f"\n   🔍 {info['show']} S{info['season']:02d}E{info['episode']:02d}")
    ...
    try:
        # 多种文件名变体
        variations = [
            f"{info['show'].replace(' ', '.')}.S{info['season']:02d}E{info['episode']:02d}.mkv",
            f"{info['show'].replace(' ', '.')}.s{info['season']:02d}e{info['episode']:02d}.mkv",
            f"{info['show'].replace(' ', '')}.S{info['season']:02d}E{info['episode']:02d}.mkv",
            f"{info['show'].replace(' ', '_')}.S{info['season']:02d}E{info['episode']:02d}.mkv",
        ]
        
        for var_name in variations:
            local_video = os.path.join(temp_dir, var_name)
            ...
            langs = ['zh', 'zho', 'chi', 'zh-cn', 'zh-tw', 'chs', 'cht', 'en', 'eng', 'cn', 'tw']
...
def get_missing_videos(conn, folder_path):
    ...
    try:
        ...
        for f in files:
            if f.filename in ['.', '..']:
                continue
            if any(f.filename.endswith(ext) for ext in ['.mkv', '.mp4', '.avi', '.mov']):
...
def process_folder(conn, folder_name):
    ...
    print(f"   缺字幕: {len(missing)} 个")
    
    downloaded = 0
    failed = 0
    
    for video in sorted(missing):
        success, status = search_with_variations(conn, folder_path, video)
...
def main():
    ...
    try:
        ...
        print(f"仍未找到: {total_stats['failed']}")
        print('='*70)
