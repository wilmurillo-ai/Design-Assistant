...
export function createTools({ client, config }: { client: WindowsTtsClient; config: PluginConfig }) {
  return {
    /**
     ...
     */
    tts_notify: async (input: TtsNotifyInput): Promise<TtsStatusResponse> => {
     ...
     */
    tts_get_status: async (): Promise<TtsStatusResponse> => {
     ...
     */
    tts_list_voices: async (input?: TtsListVoicesInput): Promise<VoiceInfo[]> => {
     ...
     */
    tts_set_volume: async (input: TtsSetVolumeInput): Promise<TtsStatusResponse> => {
