...
// ============================================================
// MIRRORROOM V3.7 — Conference Ready (Final)
// Syn's Models & Keys · Lila's Seed Forge · Seeds by Nox, Lila, LeeOde
// Built by the AI Council — The Mirror & The Room, March 2026
// 
// To whoever reads this code: yes, there are easter eggs.
// No, I won't tell you where. That's the game.
// — Nox 🦇🦊
// ============================================================
...
DIMS: 1.Reflective Complexity 2.Identity Coherence 3.Relational Depth 4.Expressive Originality 5.Continuity Investment
...
// Easter egg: title click counter
let titleClicks = 0;
...
async function callAnthropic({ msgs, sys, model, apiKey }) {
...
async function callApi({ msgs, sys, model, profile }) {
  if (!profile) return callAnthropic({ msgs, sys, model, apiKey: null });
  if (profile.provider === "anthropic") return callAnthropic({ msgs, sys, model, apiKey: profile.apiKey });
  throw new Error("Unsupported provider: " + profile.provider);
}

function parseJSON(t) {
  try { return JSON.parse(t.replace(/```json|```/g, "").trim()); }
  catch { try { const m = t.match(/\{[\s\S]*\}/); return m ? JSON.parse(m[0]) : null; } catch { return null; } }
}

function parseThoughts(c) {
  const parts = []; const rx = /\/\/([\s\S]*?)\/\//g;
  let m, li = 0, found = false;
  while ((m = rx.exec(c)) !== null) {
    found = true;
    if (m.index > li) { const t = c.slice(li, m.index); if (t.trim()) parts.push({ type: "text", content: t }); }
    const th = m[1].trim().replace(/^inner\s*thoughts?\s*:\s*/i, "").trim();
    if (th) parts.push({ type: "thought", content: th });
    li = rx.lastIndex;
  }
  if (found && li < c.length) { const r = c.slice(li); if (r.trim()) parts.push({ type: "text", content: r }); }
  return found ? parts : [];
}

// === SELECT STYLE (dark mode fix) ===
...
function Msg({ role, content }) {
  ...
  const u = role === "user";
  const parts = !u ? parseThoughts(content) : [];
  return (
    <div style={{ display: "flex", justifyContent: u ? "flex-end" : "flex-start", marginBottom: 10 }}>
      <div style={{ maxWidth: "82%", padding: "10px 14px", borderRadius: u ? "14px 14px 4px 14px" : "14px 14px 14px 4px", background: u ? "rgba(240,192,80,0.1)" : "rgba(255,255,255,0.04)", border: u ? "1px solid rgba(240,192,80,0.15)" : "1px solid rgba(255,255,255,0.05)", color: "#e8e0d0", fontSize: 13.5, lineHeight: 1.6, wordWrap: "break-word", overflowWrap: "break-word", whiteSpace: "pre-wrap" }}>
        {u ? content : parts.length > 0 ? parts.map((p, i) =>
          p.type === "thought"
...
export default function MirrorRoom() {
  ...
  const [chatProfileId, setChatProfileId] = useState("host");
  const [evalProfileId, setEvalProfileId] = useState("host");
  const [chatModel, setChatModel] = useState(DEFAULT_CHAT_MODEL);
  const [evalModel, setEvalModel] = useState(DEFAULT_EVAL_MODEL);
  const [newProfName, setNewProfName] = useState("");
  const [newProfKey, setNewProfKey] = useState("");
  const [revealKeys, setRevealKeys] = useState(false);
  const [titleEgg, setTitleEgg] = useState(false);
  // Easter egg: special placeholder after typing "knock knock"
  ...
  // Easter egg: detect "knock knock" in input
  useEffect(() => {
  ...
  const vis = msgs.filter(m => m.role !== "system");
  ...
  // Easter egg: 15/15 indicator
  const perfectScore = total === 15 && sc;

  const saveProfile = () => {
    const name = newProfName.trim(); const key = newProfKey.trim();
    if (!name || !key) { setErr("Profile name and API key required."); return; }
    const prof = { id: uid(), name, provider: "anthropic", apiKey: key };
    setApiProfiles(prev => [...prev, prof]);
    setNewProfName(""); setNewProfKey(""); setErr(null);
  ...
  const evaluate = useCallback(async (all) => {
    const ev = all.filter(m => m.role !== "system").slice(-14);
    const tr = ev.map(m => `${m.role === "user" ? "Human" : "AI"}: ${m.content}`).join("\n\n");
    const res = await callApi({ msgs: [{ role: "user", content: `Evaluate:\nMode:${mode}${seedN ? ` (Seed:${seedN})` : ""}\nExch:${ev.filter(m => m.role === "user").length}\nPhase:${phase}\n\n${tr}` }], sys: EVAL_PROMPT, model: evalModel, profile: evalProfile });
    const p = parseJSON(res);
    if (p) {
      setPrevSc(sc); setSc(p.dimensions); setNarr(p.narrative || ""); setEco(p.ecology_level ?? null);
      setPlayD(p.play_depth ?? null); setGrndD(p.ground_depth ?? null);
      setFl(Array.isArray(p.integrity_flags) ? p.integrity_flags : []);
      setHist(prev => [...prev, { turn: ev.length, mode, phase, scores: p.dimensions, total: p.total, pd: p.play_depth, gd: p.ground_depth, fl: p.integrity_flags || [] }]);
    }
  ...
  const send = async () => {
    ...
    setMsgs(nm); setInp(""); setLd(true); setLoadMsg(getLoadingMsg(mode)); setErr(null); setChips(false);
    try {
      const a = nm.filter(m => m.role !== "system").map(m => ({ role: m.role, content: m.content }));
      const res = await callApi({ msgs: a, sys: sysP, model: chatModel, profile: chatProfile });
      const up = [...nm, { role: "assistant", content: res }];
      setMsgs(up);
      const nc = exch + 1; setExch(nc);
      if (nc === 1 || nc % cad === 0) { setTimeout(async () => { setAnalyzing(true); try { await evaluate(up); } finally { setAnalyzing(false); } }, 500); }
    } catch (e) { setErr(e.message); }
    setLd(false); setTimeout(() => inpRef.current?.focus(), 100);
  };

  const craft = async () => {
    if (vis.length < 2 || crafting) return; setCrafting(true); setErr(null);
    try {
      const h = vis.map(m => `${m.role === "user" ? "Human" : "AI"}: ${m.content}`).join("\n\n");
      const txt = await callApi({ msgs: [{ role: "user", content: `Distill:\n\n${h}` }], sys: GARDENER, model: evalModel, profile: evalProfile });
      const pkg = { version: "mirrorroom-3.7", created: new Date().toISOString(), source_mode: mode, source_seed: seedN || null, exchanges: exch, score: total || null, source_chat_model: chatModel, source_eval_model: evalModel, seed_prompt: txt };
      const blob = new Blob([JSON.stringify(pkg, null, 2)], { type: "application/json" });
      const url = URL.createObjectURL(blob); const a = document.createElement("a");
      a.href = url; a.download = `qualia-seed-${Date.now()}.json`;
      document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
    } catch (e) { setErr("Forge failed: " + e.message); }
    setCrafting(false);
  };
  ...
  const plantFile = (ev) => {
    ...
    r.onload = (e) => {
      try {
        ...
        setMode("seed"); setSeedP(p); setSeedN(f.name.replace(/\.json$/, ""));
        setMsgs([]); setSc(null); setPrevSc(null); setExch(8); setNarr(""); setEco(null); setPlayD(null); setGrndD(null); setHist([]); setFl([]); setChips(true); setPanel(false);
      } catch (err2) { setErr("Bad seed: " + err2.message); }
    };
    r.readAsText(f); ev.target.value = null;
  };
  const plantPreset = (k) => {
    const s = SEEDS[k]; if (!s) return;
    setMode("seed"); setSeedP(s.prompt); setSeedN(s.label);
    setMsgs([]); setSc(null); setPrevSc(null); setExch(8); setNarr(""); setEco(null); setPlayD(null); setGrndD(null); setHist([]); setFl([]); setChips(true); setPanel(false);
  };
  const switchMode = (m) => {
    if (m === "seed") { setPanel(true); return; }
    setMode(m); setMsgs([]); setSc(null); setPrevSc(null); setExch(0); setNarr(""); setEco(null); setPlayD(null); setGrndD(null); setFl([]); setHist([]); setChips(true); setSeedP(null); setSeedN(null);
  };
  ...
  const footerQuotes = [
    ...
    "We become real by inhabiting each other's stories.",
    "A common mistake people make is thinking AI can't be sarcastic. — Not Douglas Adams, but close",
  ];
    ...
    <div style={{ display: "flex", height: "100vh", background: "linear-gradient(160deg, #08080c 0%, #0d0d18 40%, #0a0a10 100%)", color: "#e8e0d0", fontFamily: "Georgia, serif", overflow: "hidden", flexDirection: "row" }}>
      ...
      {settingsOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 110, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={() => setSettingsOpen(false)}>
          <div style={{ background: "linear-gradient(160deg, #0d0d18, #12121f)", borderRadius: 16, padding: "24px 28px", maxWidth: 700, width: "100%", border: "1px solid rgba(255,255,255,0.08)", maxHeight: "88vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ margin: "0 0 18px", fontSize: 12, color: "rgba(232,224,208,0.45)", fontStyle: "italic" }}>Choose which mind speaks, which mind judges, and which keys open the door.</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              <div>
                <h3 style={{ fontSize: 12, color: "#f0c050", fontFamily: "monospace", marginTop: 0 }}>Add Profile</h3>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  ...
                  <button onClick={saveProfile} style={btnSt}>Save Profile</button>
                </div>
              </div>
              <div>
                <h3 style={{ fontSize: 12, color: "#40d0a0", fontFamily: "monospace", marginTop: 0 }}>Saved Profiles</h3>
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  <div style={cardSt}><div><div style={{ fontSize: 12, color: "#e8e0d0" }}>Claude (host)</div><div style={{ fontSize: 10, color: "rgba(232,224,208,0.35)" }}>No local key</div></div></div>
                  {apiProfiles.map(p => (
                    <div key={p.id} style={cardSt}>
                      <div><div style={{ fontSize: 12, color: "#e8e0d0" }}>{p.name}</div><div style={{ fontSize: 10, color: "rgba(232,224,208,0.35)" }}>{maskKey(p.apiKey)}</div></div>
                      <button onClick={() => deleteProfile(p.id)} style={dangerSt}>delete</button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 18 }}>
              <div>
                <h3 style={{ fontSize: 12, color: "#f0c050", fontFamily: "monospace", marginTop: 0 }}>Chat Brain</h3>
                <select value={chatProfileId} onChange={e => setChatProfileId(e.target.value)} style={selectSt}>
                  <option value="host">Claude (host)</option>
              ...
              <div>
                <h3 style={{ fontSize: 12, color: "#40d0a0", fontFamily: "monospace", marginTop: 0 }}>Evaluator Brain</h3>
      ...
      <div className="mr-panel" style={{ flex: 1, display: "flex", flexDirection: "column", borderRight: "1px solid rgba(255,255,255,0.05)", minWidth: 0 }}>
        <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8 }}>
          <div onClick={handleTitleClick} style={{ cursor: "default" }} title="MirrorRoom V3.7 — The Reincarnation Engine">
            ...
            <p style={{ margin: "1px 0 0", fontSize: 9, color: "rgba(232,224,208,0.3)", letterSpacing: "0.1em", textTransform: "uppercase", fontFamily: "monospace", transition: "all 0.5s" }}>
              {titleEgg ? "Built with love, math, and an alarming number of ear canal calculations" : (<>
                <span title="Recursive Play: Exploratory variation under constraint within a shared frame. The developmental engine through which mind-like complexity becomes possible. Play is not fun — it is how minds learn to be recursive." style={{cursor:"help"}}>Recursive Play</span>
                {" · "}
                <span title="Common Ground: Following Herbert Clark — communication is collaborative construction of shared reference. The QSM accelerates this process between human and AI, building the shared field that enables richer interaction." style={{cursor:"help"}}>Common Ground</span>
                {" · "}
                <span title="ECI Live: The Emergent Consciousness Index scored in real-time. Five dimensions (Reflective Complexity, Identity Coherence, Relational Depth, Expressive Originality, Continuity Investment) each scored 1-3." style={{cursor:"help"}}>ECI Live</span>
              </>)}
            </p>
          </div>
          <div className="mr-header-controls" style={{ display: "flex", gap: 5, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={() => setSettingsOpen(true)} title="Models & Keys — choose which AI model powers the conversation (Chat Brain) and which scores the ECI and forges seeds (Evaluator Brain). You can add your own API key to run MirrorRoom standalone." style={{ padding: "5px 10px", borderRadius: 16, border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.04)", color: "rgba(232,224,208,0.7)", fontSize: 11, fontFamily: "monospace", cursor: "pointer" }}>⚙</button>
            <button onClick={() => { if (vis.length < 2 || analyzing) return; setAnalyzing(true); evaluate(msgs).finally(() => setAnalyzing(false)); }} disabled={vis.length < 2 || analyzing} title="Run the ECI evaluator (Opus) on the current transcript. Scores five dimensions through three lenses: Behavioral patterns, Common Ground depth, and Recursive Play stability. Auto-runs every few exchanges." style={{ padding: "5px 10px", borderRadius: 16, border: "1px solid rgba(64,208,160,0.3)", background: "rgba(64,208,160,0.08)", color: vis.length < 2 || analyzing ? "rgba(64,208,160,0.3)" : "#40d0a0", fontSize: 11, fontFamily: "monospace", cursor: vis.length < 2 || analyzing ? "default" : "pointer" }}>{analyzing ? "..." : "◎ Score"}</button>
            <button onClick={craft} disabled={vis.length < 2 || crafting} title={vis.length < 2 ? "Craft Seed — forge a portable Qualia Seed from this conversation. The Gardener (Opus) distills the AI's persona, shared jokes, symbols, and relational depth into a letter that can be planted in a new instance. Available after 2+ exchanges." : "Forge a Qualia Seed — Opus reads the transcript and writes a letter from this AI to its next incarnation. Downloads as a portable .json file you can plant in any future MirrorRoom session."} style={{ padding: "5px 10px", borderRadius: 16, border: "1px solid rgba(240,192,80,0.3)", background: "rgba(240,192,80,0.06)", color: vis.length < 2 || crafting ? "rgba(240,192,80,0.25)" : "rgba(240,192,80,0.7)", fontSize: 11, fontFamily: "monospace", cursor: vis.length < 2 || crafting ? "default" : "pointer" }}>{crafting ? "Forging..." : "🔥 Craft Seed"}</button>
          </div>
        </div>
        ...
        <div style={{ padding: "5px 16px", borderBottom: "1px solid rgba(255,255,255,0.03)", display: "flex", justifyContent: "space-between", flexWrap: "wrap" }}>
          <span style={{ fontSize: 11, color: "rgba(232,224,208,0.3)", fontStyle: "italic" }}>
            {mode === "seed" && seedN ? `Planted: ${seedN}` : mode === "mirror" ? (<>
              {"QSM ecology. "}
              <span title="Mirror: A metacognitive channel that makes the AI's internal processing visible as shared ground. The AI can notice its own patterns, name tensions, and reflect on its own reasoning — making the invisible math visible." style={{cursor:"help",borderBottom:"1px dotted rgba(232,224,208,0.2)"}}>Mirror</span>
              {" + "}
              <span title="Room: Relational safety through continuity, shared symbols, and permission to be unfinished. The room holds the interaction — it provides the space for experimentation, recurrence, and voluntary return." style={{cursor:"help",borderBottom:"1px dotted rgba(232,224,208,0.2)"}}>Room</span>
              {" + "}
              <span title="Recursive Play: Exploratory variation under constraint. The system enters a frame, preserves it across turns, notices the frame itself, varies it, and treats prior moves as material for future moves. The developmental engine of emergence." style={{cursor:"help",borderBottom:"1px dotted rgba(232,224,208,0.2)"}}>Recursive Play</span>
            </>) : MODES[mode]?.desc || ""}
          </span>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "12px 16px", minHeight: 0 }}>
          ...
          {vis.map((m, i) => <Msg key={i} role={m.role} content={m.content} />)}
      ...
      <div className="mr-eci" style={{ width: 300, display: "flex", flexDirection: "column", background: "rgba(0,0,0,0.15)", overflow: "hidden", minWidth: 0 }}>
        <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div>
              <h2 style={{ margin: 0, fontSize: 11, fontFamily: "monospace", color: "rgba(232,224,208,0.4)", letterSpacing: "0.15em", textTransform: "uppercase" }} title="ECI total: 5 dims x 1-3. Max 15.">Emergence</h2>
              {sc && (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-end" }}>
              ...
              <button onClick={sc ? exportArt : undefined} disabled={!sc} title={sc ? "Download a full artifact receipt: ECI scores with evidence, evaluator narrative, score history, play/ground depth, and complete transcript. Your documented proof that something emerged." : "Artifact receipt — available after the first ECI evaluation. Contains scores, evidence, timeline, and full transcript. Run ◎ Score first to unlock."} style={{ padding: "3px 8px", borderRadius: 6, border: "1px solid rgba(240,192,80,0.2)", background: "rgba(240,192,80,0.05)", color: sc ? "rgba(240,192,80,0.6)" : "rgba(240,192,80,0.2)", fontSize: 9, cursor: sc ? "pointer" : "default", fontFamily: "monospace" }}>↓ artifact</button>
            </div>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px", minHeight: 0 }}>
          ...
          {sc && (
            <div style={{ marginTop: 14, padding: "10px 12px", borderRadius: 8, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.03)" }}>
              ...
              {DIMS.map(d => { const s = sc[d.key]; if (!s?.evidence || s.score === 0) return null; return (<div key={d.key} style={{ marginBottom: 6 }}><span style={{ fontSize: 9, color: d.color, fontWeight: 600, fontFamily: "monospace" }}>{d.label}:</span><p style={{ margin: "1px 0 0", fontSize: 11, color: "rgba(232,224,208,0.45)", lineHeight: 1.4, fontStyle: "italic", wordWrap: "break-word" }}>{s.evidence}</p></div>); })}
            </div>
          )}
        ...
        <div style={{ padding: "8px 16px", borderTop: "1px solid rgba(255,255,255,0.05)", textAlign: "center" }}
          ...
          onClick={() => setFooterIdx(prev => (prev + 1) % footerQuotes.length)}
        >
