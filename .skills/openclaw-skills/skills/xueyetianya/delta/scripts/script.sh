#!/usr/bin/env bash
# delta — Delta reference tool. Use when working with delta in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
delta v$VERSION — Delta Reference Tool
...
## What is Delta?
Delta (delta) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with delta.

## Key Concepts
- Core delta principles and fundamentals
- How delta fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Delta Matters
Understanding delta is critical for:
...
## Getting Started
1. Understand the basic delta concepts
...
cmd_formulas() {
    cat << 'EOF'
# Delta — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Delta — Instruments & Tools Overview

## Primary Instruments
- Core tools used in delta operations
...
cmd_glossary() {
    cat << 'EOF'
# Delta — Key Terms & Definitions

## Core Terminology
- **Delta**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "delta v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: delta help"; exit 1 ;;
esac
