#!/usr/bin/env bash
# schema — Schema reference tool. Use when working with schema in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
schema v$VERSION — Schema Reference Tool
...
## What is Schema?
Schema (schema) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with schema.

## Key Concepts
- Core schema principles and fundamentals
- How schema fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Schema Matters
Understanding schema is critical for:
...
## Getting Started
1. Understand the basic schema concepts
...
## Installation
1. Download or clone the schema package
...
cmd_patterns() {
    cat << 'EOF'
# Schema — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for schema
...
cmd_migration() {
    cat << 'EOF'
# Schema — Migration & Upgrade Guide
