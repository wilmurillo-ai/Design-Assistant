#!/usr/bin/env bash
# capistrano - Developer workflow automation tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${CAPISTRANO_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/capistrano}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
capistrano v$VERSION

Developer workflow automation tool

Usage: capistrano <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "capistrano v$VERSION" ;;
