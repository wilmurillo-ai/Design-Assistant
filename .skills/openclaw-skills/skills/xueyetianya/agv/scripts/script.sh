#!/usr/bin/env bash
# agv -- Automated guided vehicle route planner
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${AGV_DIR:-$HOME/.agv}"
...
show_help() {
    cat << EOF
agv v$VERSION -- Automated guided vehicle route planner

Usage: agv <command> [args]
...
cmd_add() {
    local value="${1:?Usage: agv add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: agv search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: agv remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "agv v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: agv help"; exit 1 ;;
esac
