#!/usr/bin/env bash
# renko — Renko reference tool. Use when working with renko in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
renko v$VERSION — Renko Reference Tool
...
## What is Renko?
Renko (renko) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with renko.

## Key Concepts
- Core renko principles and fundamentals
- How renko fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Renko Matters
Understanding renko is critical for:
...
## Getting Started
1. Understand the basic renko concepts
...
cmd_formulas() {
    cat << 'EOF'
# Renko — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Renko — Instruments & Tools Overview

## Primary Instruments
- Core tools used in renko operations
...
cmd_glossary() {
    cat << 'EOF'
# Renko — Key Terms & Definitions

## Core Terminology
- **Renko**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "renko v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: renko help"; exit 1 ;;
esac
