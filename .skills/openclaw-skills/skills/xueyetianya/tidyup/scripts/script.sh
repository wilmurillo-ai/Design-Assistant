#!/usr/bin/env bash
# tidyup — Tidyup reference tool. Use when working with tidyup in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
tidyup v$VERSION — Tidyup Reference Tool

Usage: tidyup <command>
...
## What is Tidyup?
Tidyup (tidyup) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with tidyup.

## Key Concepts
- Core tidyup principles and fundamentals
- How tidyup fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Tidyup Matters
Understanding tidyup is critical for:
...
## Getting Started
1. Understand the basic tidyup concepts
...
## Installation
1. Download or clone the tidyup package
...
cmd_patterns() {
    cat << 'EOF'
# Tidyup — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for tidyup
...
cmd_performance() {
    cat << 'EOF'
# Tidyup — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Tidyup — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Tidyup — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "tidyup v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: tidyup help"; exit 1 ;;
esac
