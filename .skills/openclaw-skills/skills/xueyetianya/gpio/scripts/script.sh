#!/usr/bin/env bash
# gpio — Gpio reference tool. Use when working with gpio in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
gpio v$VERSION — Gpio Reference Tool
...
## What is Gpio?
Gpio (gpio) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with gpio.

## Key Concepts
- Core gpio principles and fundamentals
- How gpio fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Gpio Matters
Understanding gpio is critical for:
...
## Getting Started
1. Understand the basic gpio concepts
...
## Installation
1. Download or clone the gpio package
...
cmd_patterns() {
    cat << 'EOF'
# Gpio — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for gpio
...
case "$CMD" in
    ...
    version|--version|-v) echo "gpio v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: gpio help"; exit 1 ;;
esac
