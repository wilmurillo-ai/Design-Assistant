...
DATA_DIR="${HOME}/.local/share/campaign"
mkdir -p "$DATA_DIR"

_log() { echo "$(date '+%m-%d %H:%M') $1: $2" >> "$DATA_DIR/history.log"; }

_version() { echo "campaign v2.0.0"; }

_help() {
    echo "Campaign v2.0.0 — content toolkit"
    echo ""
    echo "Usage: campaign <command> [args]"
    echo ""
    echo "Commands:"
    echo "  draft              Draft"
    echo "  edit               Edit"
...
_search() {
    local term="${1:?Usage: campaign search <term>}"
...
case "${1:-help}" in
    draft)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/draft.log" 2>/dev/null || echo "  No entries yet. Use: campaign draft <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/draft.log"
            local total=$(wc -l < "$DATA_DIR/draft.log")
            echo "  [Campaign] draft: $input"
    ...
    edit)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/edit.log" 2>/dev/null || echo "  No entries yet. Use: campaign edit <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/edit.log"
            local total=$(wc -l < "$DATA_DIR/edit.log")
            echo "  [Campaign] edit: $input"
    ...
    optimize)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/optimize.log" 2>/dev/null || echo "  No entries yet. Use: campaign optimize <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/optimize.log"
            local total=$(wc -l < "$DATA_DIR/optimize.log")
            echo "  [Campaign] optimize: $input"
    ...
    schedule)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/schedule.log" 2>/dev/null || echo "  No entries yet. Use: campaign schedule <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/schedule.log"
            local total=$(wc -l < "$DATA_DIR/schedule.log")
            echo "  [Campaign] schedule: $input"
    ...
    hashtags)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/hashtags.log" 2>/dev/null || echo "  No entries yet. Use: campaign hashtags <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/hashtags.log"
            local total=$(wc -l < "$DATA_DIR/hashtags.log")
            echo "  [Campaign] hashtags: $input"
    ...
    hooks)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/hooks.log" 2>/dev/null || echo "  No entries yet. Use: campaign hooks <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/hooks.log"
            local total=$(wc -l < "$DATA_DIR/hooks.log")
            echo "  [Campaign] hooks: $input"
    ...
    cta)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/cta.log" 2>/dev/null || echo "  No entries yet. Use: campaign cta <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/cta.log"
            local total=$(wc -l < "$DATA_DIR/cta.log")
            echo "  [Campaign] cta: $input"
    ...
    rewrite)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/rewrite.log" 2>/dev/null || echo "  No entries yet. Use: campaign rewrite <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/rewrite.log"
            local total=$(wc -l < "$DATA_DIR/rewrite.log")
            echo "  [Campaign] rewrite: $input"
    ...
    translate)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/translate.log" 2>/dev/null || echo "  No entries yet. Use: campaign translate <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/translate.log"
            local total=$(wc -l < "$DATA_DIR/translate.log")
            echo "  [Campaign] translate: $input"
    ...
    tone)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/tone.log" 2>/dev/null || echo "  No entries yet. Use: campaign tone <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/tone.log"
            local total=$(wc -l < "$DATA_DIR/tone.log")
            echo "  [Campaign] tone: $input"
    ...
    headline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/headline.log" 2>/dev/null || echo "  No entries yet. Use: campaign headline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/headline.log"
            local total=$(wc -l < "$DATA_DIR/headline.log")
            echo "  [Campaign] headline: $input"
    ...
    outline)
        ...
        if [ $# -eq 0 ]; then
            ...
            tail -20 "$DATA_DIR/outline.log" 2>/dev/null || echo "  No entries yet. Use: campaign outline <input>"
        else
            local input="$*"
            local ts=$(date '+%Y-%m-%d %H:%M')
            echo "$ts|$input" >> "$DATA_DIR/outline.log"
            local total=$(wc -l < "$DATA_DIR/outline.log")
            echo "  [Campaign] outline: $input"
    ...
    *)
        ...
        echo "Run 'campaign help' for available commands."
        exit 1
        ;;
esac