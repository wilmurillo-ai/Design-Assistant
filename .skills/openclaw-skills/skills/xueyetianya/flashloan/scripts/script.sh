#!/usr/bin/env bash
# flashloan -- Analyze flashloan operations. Use when you need to understand flashloan mechanisms, evaluate protocol security, or reference on-chain concepts.
...
show_help() {
    cat << EOF
flashloan v$VERSION -- Analyze flashloan operations. Use when you need to understand flashloan mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: flashloan <command> [args]
...
cmd_add() {
    local value="${1:?Usage: flashloan add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: flashloan search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: flashloan remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_export() {
    local fmt="${1:-json}"
    local out="flashloan-export.$fmt"
...
case "$CMD" in
    ...
    version|--version|-v) echo "flashloan v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: flashloan help"; exit 1 ;;
esac
