#!/usr/bin/env bash
# kaizen -- Kaizen continuous improvement tracker
...
show_help() {
    cat << EOF
kaizen v$VERSION -- Kaizen continuous improvement tracker
...
cmd_add() {
    local value="${1:?Usage: kaizen add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: kaizen search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: kaizen remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    *) echo "Unknown: $CMD"; echo "Run: kaizen help"; exit 1 ;;
esac
