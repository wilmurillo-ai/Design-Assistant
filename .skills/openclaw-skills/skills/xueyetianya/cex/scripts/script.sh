#!/usr/bin/env bash
# cex -- Analyze cex operations. Use when you need to understand cex mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${CEX_DIR:-$HOME/.cex}"
...
show_help() {
    cat << EOF
cex v$VERSION -- Analyze cex operations. Use when you need to understand cex mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: cex <command> [args]
...
cmd_add() {
    local value="${1:?Usage: cex add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: cex search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: cex remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "cex v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: cex help"; exit 1 ;;
esac
