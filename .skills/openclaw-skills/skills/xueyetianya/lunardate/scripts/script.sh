#!/usr/bin/env bash
# lunardate — Lunardate reference tool. Use when working with lunardate in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
lunardate v$VERSION — Lunardate Reference Tool

Usage: lunardate <command>
...
## What is Lunardate?
Lunardate (lunardate) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with lunardate.

## Key Concepts
- Core lunardate principles and fundamentals
- How lunardate fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Lunardate Matters
Understanding lunardate is critical for:
...
## Getting Started
1. Understand the basic lunardate concepts
...
## Installation
1. Download or clone the lunardate package
...
cmd_patterns() {
    cat << 'EOF'
# Lunardate — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for lunardate
...
cmd_performance() {
    cat << 'EOF'
# Lunardate — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Lunardate — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Lunardate — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "lunardate v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: lunardate help"; exit 1 ;;
esac
