#!/usr/bin/env bash
# piston — Piston reference tool. Use when working with piston in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
piston v$VERSION — Piston Reference Tool

Usage: piston <command>
...
## What is Piston?
Piston (piston) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with piston.

## Key Concepts
- Core piston principles and fundamentals
- How piston fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Piston Matters
Understanding piston is critical for:
...
## Getting Started
1. Understand the basic piston concepts
...
## Installation
1. Download or clone the piston package
...
cmd_patterns() {
    cat << 'EOF'
# Piston — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for piston
...
cmd_performance() {
    cat << 'EOF'
# Piston — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Piston — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Piston — Migration & Upgrade Guide
