#!/usr/bin/env bash
# csp — Csp reference tool. Use when working with csp in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
csp v$VERSION — Csp Reference Tool
...
## What is Csp?
Csp (csp) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with csp.

## Key Concepts
- Core csp principles and fundamentals
- How csp fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Csp Matters
Understanding csp is critical for:
...
## Getting Started
1. Understand the basic csp concepts
...
## Installation
1. Download or clone the csp package
...
cmd_patterns() {
    cat << 'EOF'
# Csp — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for csp
...
case "$CMD" in
    ...
    version|--version|-v) echo "csp v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: csp help"; exit 1 ;;
esac
