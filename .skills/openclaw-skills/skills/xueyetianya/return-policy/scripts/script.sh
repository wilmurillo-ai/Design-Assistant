#!/usr/bin/env bash
# return-policy - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${RETURN_POLICY_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/return-policy}"
...
Usage: return-policy <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "return-policy v$VERSION" ;;
