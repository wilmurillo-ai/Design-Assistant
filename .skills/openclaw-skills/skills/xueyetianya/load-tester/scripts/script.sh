#!/usr/bin/env bash
# load-tester — Load Tester reference tool. Use when working with load tester in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
load-tester v$VERSION — Load Tester Reference Tool

Usage: load-tester <command>
...
cmd_intro() {
    cat << 'EOF'
# Load Tester — Overview

## What is Load Tester?
Load Tester (load-tester) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with load tester.

## Key Concepts
- Core load tester principles and fundamentals
- How load tester fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Load Tester Matters
Understanding load tester is critical for:
...
## Getting Started
1. Understand the basic load tester concepts
...
## Installation
1. Download or clone the load tester package
...
cmd_patterns() {
    cat << 'EOF'
# Load Tester — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for load tester
...
cmd_performance() {
    cat << 'EOF'
# Load Tester — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Load Tester — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Load Tester — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "load-tester v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: load-tester help"; exit 1 ;;
esac
