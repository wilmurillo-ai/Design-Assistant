#!/usr/bin/env bash
# tax-calculator-cn - Financial tracking and analysis tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${TAX_CALCULATOR_CN_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/tax-calculator-cn}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
tax-calculator-cn v$VERSION

Financial tracking and analysis tool

Usage: tax-calculator-cn <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "tax-calculator-cn v$VERSION" ;;
