#!/usr/bin/env bash
# core — Core reference tool. Use when working with core in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
core v$VERSION — Core Reference Tool
...
## What is Core?
Core (core) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with core.

## Key Concepts
- Core core principles and fundamentals
- How core fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Core Matters
Understanding core is critical for:
...
## Getting Started
1. Understand the basic core concepts
...
## Installation
1. Download or clone the core package
...
cmd_patterns() {
    cat << 'EOF'
# Core — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for core
...
case "$CMD" in
    ...
    version|--version|-v) echo "core v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: core help"; exit 1 ;;
esac
