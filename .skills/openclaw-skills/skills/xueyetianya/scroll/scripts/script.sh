#!/usr/bin/env bash
# scroll — Scroll reference tool. Use when working with scroll in blockchain contexts.
...
show_help() {
    cat << 'HELPEOF'
scroll v$VERSION — Scroll Reference Tool

Usage: scroll <command>
...
## What is Scroll?
Scroll (scroll) is a specialized tool/concept in the blockchain domain.
It provides essential capabilities for professionals working with scroll.

## Key Concepts
- Core scroll principles and fundamentals
- How scroll fits into the broader blockchain ecosystem  
- Essential terminology every practitioner should know

## Why Scroll Matters
Understanding scroll is critical for:
...
## Getting Started
1. Understand the basic scroll concepts
...
cmd_formulas() {
    cat << 'EOF'
# Scroll — Key Formulas & Calculations
...
cmd_instruments() {
    cat << 'EOF'
# Scroll — Instruments & Tools Overview

## Primary Instruments
- Core tools used in scroll operations
...
cmd_glossary() {
    cat << 'EOF'
# Scroll — Key Terms & Definitions

## Core Terminology
- **Scroll**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "scroll v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: scroll help"; exit 1 ;;
esac
