#!/usr/bin/env bash
# rugpull -- Analyze rugpull operations. Use when you need to understand rugpull mechanisms, evaluate protocol security, or reference on-chain concepts.
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${RUGPULL_DIR:-$HOME/.rugpull}"
...
show_help() {
    cat << EOF
rugpull v$VERSION -- Analyze rugpull operations. Use when you need to understand rugpull mechanisms, evaluate protocol security, or reference on-chain concepts.

Usage: rugpull <command> [args]
...
cmd_add() {
    local value="${1:?Usage: rugpull add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: rugpull search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: rugpull remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "rugpull v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: rugpull help"; exit 1 ;;
esac
