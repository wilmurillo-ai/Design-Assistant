#!/usr/bin/env bash
# valve -- Valve sizing and selection tool
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${VALVE_DIR:-$HOME/.valve}"
...
show_help() {
    cat << EOF
valve v$VERSION -- Valve sizing and selection tool
...
cmd_add() {
    local value="${1:?Usage: valve add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: valve search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: valve remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "valve v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: valve help"; exit 1 ;;
esac
