#!/usr/bin/env bash
# journald — Journald reference tool. Use when working with journald in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
journald v$VERSION — Journald Reference Tool

Usage: journald <command>
...
## What is Journald?
Journald (journald) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with journald.

## Key Concepts
- Core journald principles and fundamentals
- How journald fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Journald Matters
Understanding journald is critical for:
...
## Getting Started
1. Understand the basic journald concepts
...
## Installation
1. Download or clone the journald package
...
cmd_patterns() {
    cat << 'EOF'
# Journald — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for journald
...
cmd_performance() {
    cat << 'EOF'
# Journald — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Journald — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Journald — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "journald v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: journald help"; exit 1 ;;
esac
