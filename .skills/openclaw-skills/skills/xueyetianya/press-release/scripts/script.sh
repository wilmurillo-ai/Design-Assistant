#!/usr/bin/env bash
# press-release - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${PRESS_RELEASE_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/press-release}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
press-release v$VERSION

Multi-purpose utility tool

Usage: press-release <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "press-release v$VERSION" ;;
