#!/usr/bin/env bash
# irr — Irr reference tool. Use when working with irr in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
irr v$VERSION — Irr Reference Tool
...
## What is Irr?
Irr (irr) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with irr.

## Key Concepts
- Core irr principles and fundamentals
- How irr fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Irr Matters
Understanding irr is critical for:
...
## Getting Started
1. Understand the basic irr concepts
...
cmd_instruments() {
    cat << 'EOF'
# Irr — Instruments & Tools Overview

## Primary Instruments
- Core tools used in irr operations
...
cmd_glossary() {
    cat << 'EOF'
# Irr — Key Terms & Definitions

## Core Terminology
- **Irr**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "irr v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: irr help"; exit 1 ;;
esac
