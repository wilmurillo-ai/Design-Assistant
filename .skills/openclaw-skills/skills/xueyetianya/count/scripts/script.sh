#!/usr/bin/env bash
# count — Count reference tool. Use when working with count in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
count v$VERSION — Count Reference Tool
...
## What is Count?
Count (count) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with count.

## Key Concepts
- Core count principles and fundamentals
- How count fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Count Matters
Understanding count is critical for:
...
## Getting Started
1. Understand the basic count concepts
...
## Installation
1. Download or clone the count package
...
cmd_patterns() {
    cat << 'EOF'
# Count — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for count
