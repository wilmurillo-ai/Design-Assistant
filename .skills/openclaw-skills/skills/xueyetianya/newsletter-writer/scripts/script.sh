#!/usr/bin/env bash
# newsletter-writer - Content creation and optimization assistant
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${NEWSLETTER_WRITER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/newsletter-writer}"
DB="$DATA_DIR/data.log"
mkdir -p "$DATA_DIR"

show_help() {
    cat << EOF
newsletter-writer v$VERSION

Content creation and optimization assistant

Usage: newsletter-writer <command> [args]
...
case "${1:-help}" in
    ...
    help|-h) show_help ;;
    version|-v) echo "newsletter-writer v$VERSION" ;;
