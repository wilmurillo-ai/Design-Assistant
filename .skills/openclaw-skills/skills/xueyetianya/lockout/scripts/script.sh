#!/usr/bin/env bash
# lockout -- Lockout tagout safety procedure manager
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${LOCKOUT_DIR:-$HOME/.lockout}"
...
show_help() {
    cat << EOF
lockout v$VERSION -- Lockout tagout safety procedure manager

Usage: lockout <command> [args]
...
cmd_add() {
    local value="${1:?Usage: lockout add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: lockout search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: lockout remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "lockout v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: lockout help"; exit 1 ;;
esac
