#!/usr/bin/env bash
# math-solver - Multi-purpose utility tool
set -euo pipefail
VERSION="2.0.0"
DATA_DIR="${MATH_SOLVER_DIR:-${XDG_DATA_HOME:-$HOME/.local/share}/math-solver}"
...
Usage: math-solver <command> [args]
...
case "${1:-help}" in
    ...
    info) shift; cmd_info "$@" ;;
    help|-h) show_help ;;
    version|-v) echo "math-solver v$VERSION" ;;
