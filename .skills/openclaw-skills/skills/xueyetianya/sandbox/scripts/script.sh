#!/usr/bin/env bash
# sandbox — Sandbox reference tool. Use when working with sandbox in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
sandbox v$VERSION — Sandbox Reference Tool
...
## What is Sandbox?
Sandbox (sandbox) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with sandbox.

## Key Concepts
- Core sandbox principles and fundamentals
- How sandbox fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Sandbox Matters
Understanding sandbox is critical for:
...
## Getting Started
1. Understand the basic sandbox concepts
...
## Installation
1. Download or clone the sandbox package
...
cmd_patterns() {
    cat << 'EOF'
# Sandbox — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for sandbox
...
cmd_migration() {
    cat << 'EOF'
# Sandbox — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "sandbox v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: sandbox help"; exit 1 ;;
esac
