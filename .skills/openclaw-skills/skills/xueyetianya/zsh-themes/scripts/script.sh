#!/usr/bin/env bash
# zsh-themes — Zsh Themes reference tool. Use when working with zsh themes in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
zsh-themes v$VERSION — Zsh Themes Reference Tool

Usage: zsh-themes <command>
...
## What is Zsh Themes?
Zsh Themes (zsh-themes) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with zsh themes.

## Key Concepts
- Core zsh themes principles and fundamentals
- How zsh themes fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Zsh Themes Matters
Understanding zsh themes is critical for:
...
## Getting Started
1. Understand the basic zsh themes concepts
...
## Installation
1. Download or clone the zsh themes package
...
cmd_patterns() {
    cat << 'EOF'
# Zsh Themes — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for zsh themes
...
cmd_performance() {
    cat << 'EOF'
# Zsh Themes — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Zsh Themes — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Zsh Themes — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "zsh-themes v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: zsh-themes help"; exit 1 ;;
esac
