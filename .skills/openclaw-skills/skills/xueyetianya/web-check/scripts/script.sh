#!/usr/bin/env bash
# web-check — Web Check reference tool. Use when working with web check in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
web-check v$VERSION — Web Check Reference Tool

Usage: web-check <command>
...
## What is Web Check?
Web Check (web-check) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with web check.

## Key Concepts
- Core web check principles and fundamentals
- How web check fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why Web Check Matters
Understanding web check is critical for:
...
## Getting Started
1. Understand the basic web check concepts
...
## Installation
1. Download or clone the web check package
...
cmd_patterns() {
    cat << 'EOF'
# Web Check — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for web check
...
cmd_performance() {
    cat << 'EOF'
# Web Check — Performance Optimization
...
cmd_security() {
    cat << 'EOF'
# Web Check — Security Considerations
...
cmd_migration() {
    cat << 'EOF'
# Web Check — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "web-check v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: web-check help"; exit 1 ;;
esac
