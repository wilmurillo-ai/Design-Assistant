#!/usr/bin/env bash
# andon -- Andon alert and production status board
# Powered by BytesAgain | bytesagain.com | hello@bytesagain.com
set -euo pipefail

VERSION="1.0.0"
DATA_DIR="${ANDON_DIR:-$HOME/.andon}"
...
show_help() {
    cat << EOF
andon v$VERSION -- Andon alert and production status board

Usage: andon <command> [args]
...
cmd_add() {
    local value="${1:?Usage: andon add <entry>}"
    shift || true
...
cmd_search() {
    local term="${1:?Usage: andon search <term>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
cmd_remove() {
    local num="${1:?Usage: andon remove <line-number>}"
    if [ -f "$DATA_DIR/data.jsonl" ]; then
...
case "$CMD" in
    ...
    version|--version|-v) echo "andon v$VERSION -- Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: andon help"; exit 1 ;;
esac
