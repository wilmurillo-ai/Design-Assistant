#!/usr/bin/env bash
# apparmor — Apparmor reference tool. Use when working with apparmor in sysops contexts.
...
show_help() {
    cat << 'HELPEOF'
apparmor v$VERSION — Apparmor Reference Tool

Usage: apparmor <command>
...
## What is Apparmor?
Apparmor (apparmor) is a specialized tool/concept in the sysops domain.
It provides essential capabilities for professionals working with apparmor.

## Key Concepts
- Core apparmor principles and fundamentals
- How apparmor fits into the broader sysops ecosystem  
- Essential terminology every practitioner should know

## Why Apparmor Matters
Understanding apparmor is critical for:
...
## Getting Started
1. Understand the basic apparmor concepts
...
## Installation
1. Download or clone the apparmor package
...
cmd_patterns() {
    cat << 'EOF'
# Apparmor — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for apparmor
...
cmd_migration() {
    cat << 'EOF'
# Apparmor — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "apparmor v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: apparmor help"; exit 1 ;;
esac
