#!/usr/bin/env bash
# license — License reference tool. Use when working with license in devtools contexts.
...
show_help() {
    cat << 'HELPEOF'
license v$VERSION — License Reference Tool
...
## What is License?
License (license) is a specialized tool/concept in the devtools domain.
It provides essential capabilities for professionals working with license.

## Key Concepts
- Core license principles and fundamentals
- How license fits into the broader devtools ecosystem  
- Essential terminology every practitioner should know

## Why License Matters
Understanding license is critical for:
...
## Getting Started
1. Understand the basic license concepts
...
## Installation
1. Download or clone the license package
...
cmd_patterns() {
    cat << 'EOF'
# License — Common Patterns & Best Practices

## Design Patterns
1. **Standard Pattern**: The most common approach for license
...
cmd_performance() {
    cat << 'EOF'
# License — Performance Optimization
...
cmd_migration() {
    cat << 'EOF'
# License — Migration & Upgrade Guide
...
case "$CMD" in
    ...
    version|--version|-v) echo "license v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: license help"; exit 1 ;;
esac
