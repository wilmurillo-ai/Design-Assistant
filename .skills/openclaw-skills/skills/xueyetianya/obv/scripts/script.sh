#!/usr/bin/env bash
# obv — Obv reference tool. Use when working with obv in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
obv v$VERSION — Obv Reference Tool
...
## What is Obv?
Obv (obv) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with obv.

## Key Concepts
- Core obv principles and fundamentals
- How obv fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Obv Matters
Understanding obv is critical for:
...
## Getting Started
1. Understand the basic obv concepts
...
cmd_instruments() {
    cat << 'EOF'
# Obv — Instruments & Tools Overview

## Primary Instruments
- Core tools used in obv operations
...
cmd_glossary() {
    cat << 'EOF'
# Obv — Key Terms & Definitions

## Core Terminology
- **Obv**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "obv v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: obv help"; exit 1 ;;
esac
