...
```bash
# 基于关键词找标签
pnpm start suggest_tags --keyword "同人二创" --size 15
# 输出示例：["崩坏星穹铁道", "原神", "明日方舟"]

# 或者用关键词建议辅助
pnpm start suggest_keywords --prefix "崩" --size 10
...
```bash
# 步骤 1：了解热门标签
pnpm start suggest_tags --keyword "角色塑造" --size 20
...
pnpm start suggest_categories --level 2 --parent_path "衍生创作类" &
pnpm start suggest_categories --level 2 --parent_path "数字艺术" &
pnpm start suggest_categories --level 2 --parent_path "生活方式" &
wait
```
...
```bash
# 并行验证多个路径
pnpm start validate_tax_path --tax_path "衍生创作类>同人二创>崩坏星穹铁道" &
pnpm start validate_tax_path --tax_path "衍生创作类>同人二创>原神" &
pnpm start validate_tax_path --tax_path "数字艺术>概念设计" &
wait
...
while read path; do
  pnpm start validate_tax_path --tax_path "$path"
done < /tmp/paths.txt
```
...
pnpm start suggest_content --intent search --search_keywords "关键词" --page_size 10
pnpm start suggest_content --intent exact --tax_paths "分类路径" --page_size 10
```
...
```bash
# keywords - 前缀匹配
pnpm start suggest_keywords --prefix "崩"  # 返回所有以"崩"开头的词

# tags - 相关性匹配
pnpm start suggest_tags --keyword "游戏"   # 返回与"游戏"相关的标签
```
