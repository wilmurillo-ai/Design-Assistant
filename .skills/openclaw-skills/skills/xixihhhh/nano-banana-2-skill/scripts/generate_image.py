...
def detect_provider():
    """Auto-detect provider based on available API keys. Prompts user if both are set."""
    has_atlas = bool(os.environ.get("ATLASCLOUD_API_KEY"))
    has_gemini = bool(os.environ.get("GEMINI_API_KEY"))
    if has_atlas and has_gemini:
        print("Both ATLASCLOUD_API_KEY and GEMINI_API_KEY are set.")
        print("  1) atlas  — Atlas Cloud (async, polling)")
        print("  2) google — Google AI Studio (synchronous)")
        choice = input("Select provider [1/2] (default: 1): ").strip()
        if choice == "2" or choice.lower() == "google":
            return "google"
        return "atlas"
    if has_atlas:
        return "atlas"
