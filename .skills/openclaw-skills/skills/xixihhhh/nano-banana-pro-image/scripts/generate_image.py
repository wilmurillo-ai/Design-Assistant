...
Network calls (all to api.atlascloud.ai):
  ...
  - POST /api/v1/model/uploadMedia   — upload local file (only via "upload" command, with user confirmation)
...
Usage:
  # Text-to-image
  python generate_image.py generate --model "google/nano-banana-pro/text-to-image" --prompt "A portrait"

  # Image editing (with remote image)
  python generate_image.py generate --model "google/nano-banana-pro/edit" --prompt "Change sky" --images "https://..."

  # Upload local file then edit
  python generate_image.py upload ./photo.jpg
  python generate_image.py generate --model "google/nano-banana-pro/edit" --prompt "Change sky" --images "https://...uploaded-url..."
...
def api_request(method, endpoint, data=None):
    ...
    headers = {"Authorization": f"Bearer {get_api_key()}", "User-Agent": UA}
    body = None
...
def main():
    ...
    gen.add_argument("--model", "-m", required=True, help="Model ID (e.g. google/nano-banana-pro/text-to-image)")
    ...
    saved = []
    for url in outputs:
        saved.append(download_file(url, args.output))
