"""
china-tools-sourcing - OpenClaw skill for China tools industry intelligence
...
def get_industry_overview() -> Dict:
    """Return overview of China's tools industry scale and targets."""
    return TOOLS_DATA.get("industry_overview", {})

def get_supply_chain_structure() -> Dict:
    """Return the complete tools supply chain structure."""
    return TOOLS_DATA.get("supply_chain_structure", {})
...
def get_regional_clusters(region: Optional[str] = None) -> Union[List[Dict], Dict]:
    ...
    """
    clusters = TOOLS_DATA.get("regional_clusters", [])
...
def find_clusters_by_specialization(specialization: str) -> List[Dict]:
    """Find clusters that specialize in a given tool type."""
...
def get_subsector_info(subsector: str) -> Dict:
    """Return detailed information about a specific tools subsector."""
    subsectors = TOOLS_DATA.get("subsector_details", {})
    ...
    # Keyword mapping for common tool terms
    keyword_mapping = {
        "hand": "hand_tools",
        "wrench": "hand_tools",
        "plier": "hand_tools",
        ...
        "tape": "measuring_tools",
        "level": "measuring_tools",
        "storage": "tool_storage",
        "box": "tool_storage",
        "automotive": "automotive_tools",
        "jack": "automotive_tools",
        "pneumatic": "pneumatic_tools",
        "air": "pneumatic_tools",
        "cutting": "cutting_tools",
        "drill bit": "cutting_tools"
    }
...
def get_sourcing_guide() -> Dict:
    ...
    guide = TOOLS_DATA.get("sourcing_guide", {})
...
def get_glossary(term: Optional[str] = None) -> Union[Dict, str]:
    ...
    glossary = TOOLS_DATA.get("glossary", {})
...
def search_data(query: str) -> List[Dict]:
    ...
    for cluster in TOOLS_DATA.get("regional_clusters", []):
    ...
    for subsector_name, subsector_data in TOOLS_DATA.get("subsector_details", {}).items():
...
def get_metadata() -> Dict:
    ...
    return TOOLS_DATA.get("metadata", {})

# --- If run directly, provide interactive example ---
if __name__ == "__main__":
    print("🔧 China Tools Sourcing Skill (CLI Test Mode)")
    print("=" * 60)
    ...
    print('  find_clusters_by_specialization("power tools")')
    print('  get_subsector_info("hand_tools")')