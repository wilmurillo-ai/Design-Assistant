"""
china-consumer-electronics-sourcing - OpenClaw skill for China consumer electronics industry intelligence
...
with open(_DATA_PATH, "r", encoding="utf-8") as f:
    CONSUMER_ELECTRONICS_DATA = json.load(f)
...
def get_industry_overview() -> Dict:
    """Return overview of China's consumer electronics industry scale and targets."""
    return CONSUMER_ELECTRONICS_DATA.get("industry_overview", {})

def get_supply_chain_structure() -> Dict:
    """Return the complete consumer electronics supply chain structure."""
    return CONSUMER_ELECTRONICS_DATA.get("supply_chain_structure", {})
...
def get_regional_clusters(region: Optional[str] = None) -> Union[List[Dict], Dict]:
    ...
    """
    clusters = CONSUMER_ELECTRONICS_DATA.get("regional_clusters", [])
...
def find_clusters_by_specialization(specialization: str) -> List[Dict]:
    """Find clusters that specialize in a given consumer electronics type."""
...
def get_subsector_info(subsector: str) -> Dict:
    """Return detailed information about a specific consumer electronics subsector."""
    subsectors = CONSUMER_ELECTRONICS_DATA.get("subsector_details", {})
    ...
    # Keyword mapping for common consumer electronics terms
    keyword_mapping = {
        "phone": "smartphones",
        ...
        "band": "wearables",
        "fitness": "wearables",
        "earbud": "audio_devices",
        "headphone": "audio_devices",
        "speaker": "audio_devices",
        "tws": "audio_devices",
        "smart home": "smart_home_devices",
        "vr": "AR_VR",
        "ar": "AR_VR",
        "headset": "AR_VR"
    }
...
def get_sourcing_guide() -> Dict:
    ...
    guide = CONSUMER_ELECTRONICS_DATA.get("sourcing_guide", {})
...
def get_faq(question: Optional[str] = None) -> Union[List[Dict], Dict]:
    ...
    faqs = CONSUMER_ELECTRONICS_DATA.get("faq", [])
...
def get_glossary(term: Optional[str] = None) -> Union[Dict, str]:
    ...
    glossary = CONSUMER_ELECTRONICS_DATA.get("glossary", {})
...
def search_data(query: str) -> List[Dict]:
    ...
    for cluster in CONSUMER_ELECTRONICS_DATA.get("regional_clusters", []):
    ...
    for subsector_name, subsector_data in CONSUMER_ELECTRONICS_DATA.get("subsector_details", {}).items():
...
def get_metadata() -> Dict:
    ...
    return CONSUMER_ELECTRONICS_DATA.get("metadata", {})

# --- If run directly, provide interactive example ---
if __name__ == "__main__":
    print("📱 China Consumer Electronics Sourcing Skill (CLI Test Mode)")
    print("=" * 60)
    ...
    print('  find_clusters_by_specialization("smartphone")')
    print('  get_subsector_info("audio_devices")')