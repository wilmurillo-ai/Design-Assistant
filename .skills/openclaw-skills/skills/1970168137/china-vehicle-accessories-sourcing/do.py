"""
china-vehicle-accessories-sourcing - OpenClaw skill for China vehicle accessories industry intelligence
Provides structured data access to manufacturing clusters, supply chain, and sourcing guidance for all vehicle types (cars, trucks, motorcycles, etc.).
"""
...
with open(_DATA_PATH, "r", encoding="utf-8") as f:
    VEHICLE_ACCESSORIES_DATA = json.load(f)
...
def get_industry_overview() -> Dict:
    """Return overview of China's vehicle accessories industry scale and targets."""
    return VEHICLE_ACCESSORIES_DATA.get("industry_overview", {})

def get_supply_chain_structure() -> Dict:
    """Return the complete vehicle accessories supply chain structure."""
    return VEHICLE_ACCESSORIES_DATA.get("supply_chain_structure", {})
...
def get_regional_clusters(region: Optional[str] = None) -> Union[List[Dict], Dict]:
    ...
    """
    clusters = VEHICLE_ACCESSORIES_DATA.get("regional_clusters", [])
...
def find_clusters_by_specialization(specialization: str) -> List[Dict]:
    """Find clusters that specialize in a given vehicle accessories type."""
...
def get_subsector_info(subsector: str) -> Dict:
    """Return detailed information about a specific vehicle accessories subsector."""
    subsectors = VEHICLE_ACCESSORIES_DATA.get("subsector_details", {})
    ...
    # Keyword mapping for common accessories terms
    keyword_mapping = {
        "dash cam": "electronic_accessories",
        "gps": "electronic_accessories",
        "tracker": "electronic_accessories",
        "sensor": "electronic_accessories",
        "multimedia": "electronic_accessories",
        "seat cover": "interior_accessories",
        "floor mat": "interior_accessories",
        "cargo liner": "interior_accessories",
        "steering wheel cover": "interior_accessories",
        "organizer": "interior_accessories",
        "window visor": "exterior_accessories",
        "roof rack": "exterior_accessories",
        "side step": "exterior_accessories",
        "spoiler": "exterior_accessories",
        "grille": "exterior_accessories",
        "truck bed": "exterior_accessories",
        "led": "lighting_accessories",
        "headlight": "lighting_accessories",
        "fog light": "lighting_accessories",
        "ambient light": "lighting_accessories",
        "light bar": "lighting_accessories",
        "wax": "vehicle_care_products",
        "polish": "vehicle_care_products",
        "cleaner": "vehicle_care_products",
        "air freshener": "vehicle_care_products",
        "roof box": "functional_accessories",
        "bike carrier": "functional_accessories",
        "tow bar": "functional_accessories",
        "cargo management": "functional_accessories",
        "child seat": "safety_accessories",
        "first aid": "safety_accessories",
        "fire extinguisher": "safety_accessories",
        "motorcycle": "interior_accessories",  # fallback, but could be more specific
        "commercial vehicle": "functional_accessories"
    }
...
def get_sourcing_guide() -> Dict:
    ...
    guide = VEHICLE_ACCESSORIES_DATA.get("sourcing_guide", {})
...
def get_faq(question: Optional[str] = None) -> Union[List[Dict], Dict]:
    ...
    faqs = VEHICLE_ACCESSORIES_DATA.get("faq", [])
...
def get_glossary(term: Optional[str] = None) -> Union[Dict, str]:
    ...
    glossary = VEHICLE_ACCESSORIES_DATA.get("glossary", {})
...
def search_data(query: str) -> List[Dict]:
    ...
    for cluster in VEHICLE_ACCESSORIES_DATA.get("regional_clusters", []):
    ...
    for subsector_name, subsector_data in VEHICLE_ACCESSORIES_DATA.get("subsector_details", {}).items():
...
def get_metadata() -> Dict:
    ...
    return VEHICLE_ACCESSORIES_DATA.get("metadata", {})

# --- If run directly, provide interactive example ---
if __name__ == "__main__":
    print("🚘 China Vehicle Accessories Sourcing Skill (CLI Test Mode)")
    print("=" * 60)
    ...
    print('  find_clusters_by_specialization("dash cam")')
    print('  find_clusters_by_specialization("truck accessories")')