"""
china-toys-sourcing - OpenClaw skill for China toys industry intelligence
...
def get_industry_overview() -> Dict:
    """Return overview of China's toys industry scale and targets."""
    return TOYS_DATA.get("industry_overview", {})

def get_supply_chain_structure() -> Dict:
    """Return the complete toys supply chain structure."""
    return TOYS_DATA.get("supply_chain_structure", {})
...
def get_regional_clusters(region: Optional[str] = None) -> Union[List[Dict], Dict]:
    ...
    """
    clusters = TOYS_DATA.get("regional_clusters", [])
...
def find_clusters_by_specialization(specialization: str) -> List[Dict]:
    """Find clusters that specialize in a given toy type."""
...
def get_subsector_info(subsector: str) -> Dict:
    """Return detailed information about a specific toy subsector."""
    subsectors = TOYS_DATA.get("subsector_details", {})
    ...
    # Keyword mapping for common toy terms
    keyword_mapping = {
        "plastic": "plastic_toys",
        "action figure": "plastic_toys",
        "electronic": "electronic_toys",
        "rc": "electronic_toys",
        "robot": "electronic_toys",
        "wooden": "wooden_toys",
        "plush": "plush_toys",
        "stuffed": "plush_toys",
        "model": "model_kits",
        "collectible": "model_kits",
        "educational": "educational_toys",
        "stem": "educational_toys"
    }
...
def get_sourcing_guide() -> Dict:
    ...
    guide = TOYS_DATA.get("sourcing_guide", {})
...
def get_glossary(term: Optional[str] = None) -> Union[Dict, str]:
    ...
    glossary = TOYS_DATA.get("glossary", {})
...
def search_data(query: str) -> List[Dict]:
    ...
    for cluster in TOYS_DATA.get("regional_clusters", []):
    ...
    for subsector_name, subsector_data in TOYS_DATA.get("subsector_details", {}).items():
...
def get_metadata() -> Dict:
    ...
    return TOYS_DATA.get("metadata", {})

# --- If run directly, provide interactive example ---
if __name__ == "__main__":
    print("🧸 China Toys Sourcing Skill (CLI Test Mode)")
    print("=" * 60)
    ...
    print('  find_clusters_by_specialization("electronic toys")')
    print('  get_subsector_info("plastic_toys")')