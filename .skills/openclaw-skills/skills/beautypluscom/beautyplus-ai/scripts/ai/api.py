...
def _replace_user_origin_image_url(obj, user_image_url: str):
...
def _default_connection_config():
    ...
    return {
        ...
        "token_policy_type": getattr(app_config, "TOKEN_POLICY_TYPE", ""),
        "token_policy_types": getattr(app_config, "TOKEN_POLICY_TYPES", None),
        "token_policy_app": getattr(app_config, "TOKEN_POLICY_APP", None),
    }
...
class AiApi:
    ...
    def __init__(self, key, secret, region="cn-north-4", oss_region=None, config=None):
        ...
        import config as app_config
        cache_file = getattr(app_config, "GID_CACHE_FILE", "~/.cache/beautyplus/gid_cache.json")
    ...
    def _prepare_algorithm_push_request(self, imageUrl, params, task, taskType, context=""):
        """Build signed POST to algorithm push_path (same body as ``run``); does not send."""
        import config as app_config
        ...
        return sign_request, policy

    def _prepare_algorithm_status_request(self, uri, policy):
        """Build signed GET for algorithm status_query (same as ``queryStatus``); does not send."""
        import config as app_config
    ...
    def run(
        ...
        """
        taskType = _default_task_type(taskType)
        sign_request, policy = self._prepare_algorithm_push_request(
            imageUrl, params, task, taskType, context
        )
        _progress_log(
