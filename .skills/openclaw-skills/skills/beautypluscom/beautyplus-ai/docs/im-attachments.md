...
How to turn chat media into a **local path** or URL for `beautyplus_ai.py run-task --input …`.
...
python3 {baseDir}/scripts/beautyplus_ai.py resolve-input \
  --file /tmp/user_upload.jpg \
  --output-dir /tmp

# Public URL (download first — useful if BeautyPlus cannot fetch the URL)
python3 {baseDir}/scripts/beautyplus_ai.py resolve-input \
...
```bash
TELEGRAM_BOT_TOKEN="$TELEGRAM_BOT_TOKEN" python3 {baseDir}/scripts/beautyplus_ai.py resolve-input \
...
```bash
FEISHU_APP_TOKEN="$FEISHU_APP_TOKEN" python3 {baseDir}/scripts/beautyplus_ai.py resolve-input \
...
Max download/copy size: **100 MB** (same guard as similar skills). For larger assets, use a direct URL if BeautyPlus can fetch it, or another upload path your tenant supports.
...
python3 {baseDir}/scripts/beautyplus_ai.py run-task \
  ...
  --input "/tmp/beautyplus_in_xxxxxxxxxx_user_upload.jpg"
```
