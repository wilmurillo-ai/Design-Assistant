...
```bash
curl -sL -o /tmp/beautyplus_result.mp4 "<primary_result_url>"
```
...
| `--video-url` | No | When set, always sends an extra **text** message with `Video Download: [Link](url)` style download text |
