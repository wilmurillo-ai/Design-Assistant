...
当用户不确定该用哪个 Sparki skill 时，用这个导航来帮他选：

- 泛视频编辑入口 → `AI Video Editor`
- Vlog / 旅行 / 生活方式内容 → `ai-vlog-editor`
- 长视频拆条 / 长切短 → `long-to-short`
- 字幕优先 / 上字视频 → `ai-caption`
- 解说 / 讲解 / reaction 风格 → `ai-commentary`
- 改比例 / 平台适配 → `video-resizer`
- 高光 / 精彩片段 / recap → `highlight-reels`
- 不露脸 / faceless 内容 → `faceless-video`
- TikTok 爆款节奏 → `tiktok-viral-editor`
- 口播 / presenter / talking-head → `talking-head-editor`

如果用户还是不确定，就先推荐装总入口 `AI Video Editor`。
