...
async function testConnection() {
    ...
    try {
        ...
        console.log('📊 Test: Tenant Information...');
        const tenant = await client.api('/organization').get();
        console.log('   Tenant:', tenant.value[0].displayName);
        console.log('   Domain:', tenant.value[0].verifiedDomains?.[0]?.name || 'N/A');
        console.log('');

        // Test: Get groups (Planner requires M365 Groups)
        console.log('📋 Test: M365 Groups...');
        const groups = await client.api('/groups').select('id,displayName,mail').top(5).get();
        if (groups.value.length > 0) {
            console.log(`   ${groups.value.length} Gruppen gefunden:`);
            groups.value.forEach(g => {
                console.log(`   - ${g.displayName} (${g.id})`);
            });
        } else {
        ...
        console.log('  1. Gruppe auswählen oder erstellen');
