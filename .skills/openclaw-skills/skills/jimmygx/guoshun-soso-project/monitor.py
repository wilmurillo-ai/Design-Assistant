...
def is_new(item, history):
    """检查项目是否为新（基于标题核心内容去重，忽略前缀标签）"""
    title = item.get("title", "")
    # 去除常见前缀标签，取核心标题
    core = title
    for prefix in ["[新]", "[中标结果]", "[招标公告]", "[招标计划]", "中标/流标公示", "招标公告"]:
    ...
    for h in history:
        ...
        h_core = h_title
        for prefix in ["[新]", "[中标结果]", "[招标公告]", "[招标计划]", "中标/流标公示", "招标公告"]:
            h_core = h_core.replace(prefix, "").strip()
        h_key = h_core[:25]
...
def send_feishu(text):
    ...
    if not token:
        ...
        return False
    targets = CONFIG["push"].get("targets", [])
    if not targets:
        single = CONFIG["push"].get("target", "")
        if single:
            targets = [single]
    if not targets:
        log("飞书推送跳过: 未配置推送目标")
    ...
    for target in targets:
        try:
            ...
            if resp.json().get("code") == 0:
                ...
                log(f"飞书推送失败 → {target}: {resp.json()}")
        except Exception as e:
            log(f"飞书推送异常 → {target}: {e}")
    return success
...
def build_message(items):
    ...
    max_items = CONFIG["push"].get("max_items", 20)

    msg = f"📢 发现 {len(deduped)} 条新招投标信息\n\n"
    for i, item in enumerate(deduped[:max_items], 1):
        stars = "★" * min(item["score"] // 5, 5)
        t = item["title"][:45] + ("..." if len(item["title"]) > 45 else "")
...
def save_results_file(msg):
    """将推送消息保存到文件，供定时任务agent读取后发企业微信"""
    results_file = os.path.join(DATA_DIR, "latest_results.txt")
...
def send_notification(items):
    ...
    # 保存到文件（供企业微信推送用）
    save_results_file(msg)

    channel = CONFIG["push"].get("channel", "openclaw")
    if channel == "feishu":
        send_feishu(msg)
...
def send_empty_report(total_checked):
    """无新项目时也推送简报"""
    now = datetime.now().strftime("%Y-%m-%d %H:%M")
    msg = f"🔍 招标监控简报 ({now})\n\n"
    msg += f"本次扫描了 {total_checked} 条招标信息，暂无新增项目。\n"
    msg += "监控正常运行中 ✅"
...
def run():
    ...
    if new_items:
        ...
        send_empty_report(total_checked)
    return new_items
