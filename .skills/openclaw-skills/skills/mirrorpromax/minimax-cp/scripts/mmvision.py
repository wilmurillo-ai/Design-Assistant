#!/usr/bin/env python3
"""MiniMax MCP Vision - CLI wrapper for MiniMax Coding Plan MCP image understanding"""
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: mmvision <prompt> <image_path_or_url>", file=sys.stderr)
        print("Example: mmvision '描述这张图片' /path/to/image.png", file=sys.stderr)
        print("Example: mmvision '提取图片中的文字' https://example.com/image.jpg", file=sys.stderr)
        sys.exit(1)
    
    prompt = sys.argv[1]
    image_source = sys.argv[2]
    
    result = call_mcp_tool("understand_image", {
        "prompt": prompt,
