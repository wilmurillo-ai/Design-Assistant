...
function detectMcporterConfigPath(): string | null {
  // 可能的配置路径
  const possiblePaths = [
  ...
  const result = spawnSync('mcporter', ['config', 'path'], {
    encoding: 'utf8',
    timeout: 5000
  });
...
async function main() {
  ...
  // 1. 检查 mcporter 是否安装
  console.log('\n📋 步骤 1: 检查 mcporter 安装');
  const mcporterCheck = spawnSync('which', ['mcporter'], { encoding: 'utf8', timeout: 5000 });
  ...
  const mcporterVersion = spawnSync('mcporter', ['--version'], {
    encoding: 'utf8',
    timeout: 5000
  });
  ...
  const configResult = spawnSync('mcporter', [
    'config', 'set',
  ...
  if (!process.env.OPENCLAW_MASTER_KEY) {
    ...
    const generateKeyResult = spawnSync('openssl', ['rand', '-base64', '32'], {
      encoding: 'utf8',
      timeout: 5000
    });
  ...
  // 使用 key-manager 保存
  try {
    const { SecretManager } = await import('../src/key-manager');
  ...
  const testResult = spawnSync('mcporter', ['list'], {
    encoding: 'utf8',
    timeout: 10000
  });
