...
exports.resultMerger = exports.ResultMerger = void 0;
/**
 * 引擎默认权重
 */
...
class ResultMerger {
     ...
     * 计算最终得分
     */
    calculateFinalScore(result, options) {
        // 基础分
        ...
        if (options.applyTimeDecay !== false && result.publishedDate) {
            const decayFactor = this.calculateTimeDecay(result.publishedDate, options.timeDecayHalfLife || 30);
            score *= decayFactor;
        }
        // 内容完整性加分
     ...
     * @returns 衰减因子 (0-1)
     */
    calculateTimeDecay(dateStr, halfLife) {
        try {
     ...
     * 获取每个引擎的最佳结果
     */
    getTopResultsByEngine(results, topN = 3) {
        const grouped = this.groupByEngine(results);
        const topResults = {};
        for (const [engine, engineResults] of Object.entries(grouped)) {
            // 按分数排序
            const sorted = [...engineResults].sort((a, b) => (b.finalScore || 0) - (a.finalScore || 0));
...
// 导出单例
exports.resultMerger = new ResultMerger();
