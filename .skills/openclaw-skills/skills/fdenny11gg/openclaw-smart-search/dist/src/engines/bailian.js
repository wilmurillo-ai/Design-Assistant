...
class BailianEngine {
    ...
    async search(query, options) {
        // 检查 mcporter 是否安装
        const mcporterCheck = (0, child_process_1.spawnSync)('which', ['mcporter'], { timeout: 5000 });
        if (mcporterCheck.status !== 0) {
            throw new Error('⚠️  警告：mcporter 未安装\n' +
        ...
        try {
            ...
            if (error.message.includes('not configured')) {
                throw new Error('⚠️  百炼 API Key 未配置\n' +
                    '请运行以下命令进行配置:\n' +
     ...
     */
    executeMcporter(apiKey, query, count) {
        return new Promise((resolve, reject) => {
            const timeout = 15000; // 15 秒超时
            let stdout = '';
            let stderr = '';
            // 使用 spawn，参数作为数组传递，避免 shell 注入
            const child = (0, child_process_1.spawn)('mcporter', [
                'call',
            ...
            const timer = setTimeout(() => {
                child.kill('SIGKILL');
                reject(new Error(`⏱️ 百炼搜索超时 (${timeout}ms)\n` +
                    '可能原因:\n' +
                    '  - 网络连接不稳定\n' +
                    '  - API 服务繁忙\n\n' +
                    '请稍后重试，或切换到其他引擎:\n' +
            ...
            child.on('error', (err) => {
                ...
                if (err.message.includes('ENOENT')) {
                    reject(new Error('⚠️  mcporter 未找到\n' +
            ...
            child.on('close', (code) => {
                ...
                if (code !== 0) {
                    ...
                    reject(new Error(`百炼搜索失败 (退出码: ${code})\n` +
                        '请检查:\n' +
...
exports.BailianEngine = BailianEngine;
