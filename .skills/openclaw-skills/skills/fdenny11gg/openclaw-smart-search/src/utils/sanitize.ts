 ...
 */
export function validateInputLength(input: string, maxLength: number = 500): string {
  if (input.length > maxLength) {
    throw new Error(
      `Input too long. Maximum length is ${maxLength} characters. ` +
 ...
 */
export function validateWithWhitelist(input: string): string {
  if (!ALLOWED_CHARS_REGEX.test(input)) {
    ...
    throw new Error(
      `Query contains invalid characters: ${invalidChars.slice(0, 5).map(c => `"${c}"`).join(', ')}${invalidChars.length > 5 ? '...' : ''}\n` +
 ...
 * @returns 是否检测到潜在攻击
 */
export function detectInjectionAttempt(input: string): { detected: boolean; patterns: string[] } {
...
export function validateSearchQuery(query: string): string {
  ...
  if (injectionCheck.detected) {
    console.warn(
      `[SECURITY WARNING] Potential injection pattern detected: ${injectionCheck.patterns.join(', ')}\n` +
