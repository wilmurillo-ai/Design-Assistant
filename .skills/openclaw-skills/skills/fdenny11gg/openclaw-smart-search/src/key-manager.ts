...
import { SecretConfig, EngineConfig, EngineType, ENGINE_PRIORITIES, ENGINE_QUOTAS } from './types';

export class SecretManager {
  private readonly keyFile: string;
  private masterKey: Buffer | null = null;
  private configCache: SecretConfig | null = null;
  
  constructor() {
    this.keyFile = path.join(
      os.homedir(),
      '.openclaw',
      'secrets',
      'smart-search.json.enc'
    );
  ...
  private async initMasterKey(): Promise<void> {
    ...
    if (!masterKeyStr) {
      throw new Error(
        'OPENCLAW_MASTER_KEY environment variable is required.\n' +
    ...
    if (masterKeyStr.length < 32) {
      throw new Error(
        'OPENCLAW_MASTER_KEY must be at least 32 characters long for security.\n' +
  ...
  // 临时存储当前盐值
  private _currentSalt: Buffer | null = null;
   ...
   * 读取并解密配置
   */
  async readConfig(): Promise<SecretConfig> {
    // 使用缓存
    if (this.configCache) {
   ...
   * 写入加密配置
   */
  async writeConfig(config: SecretConfig): Promise<void> {
    const masterKeyStr = process.env.OPENCLAW_MASTER_KEY;
   ...
   * 初始化配置（首次使用）
   */
  async initConfig(apiKeys: Record<string, string>): Promise<void> {
    ...
    for (const [engine, apiKey] of Object.entries(apiKeys)) {
      ...
      config.engines[engine] = {
        apiKey: apiKey.trim(),
        enabled: true,
        priority: ENGINE_PRIORITIES[engineType] || 99,
        quotaLimit: ENGINE_QUOTAS[engineType] || 0,
        quotaUsed: 0,
   ...
   * 获取单个引擎的 Key
   */
  async getEngineKey(engine: string): Promise<string> {
   ...
   * 设置单个引擎的 Key
   */
  async setEngineKey(engine: string, apiKey: string): Promise<void> {
   ...
   * 轮换 Key
   */
  async rotateKey(engine: string, newApiKey?: string): Promise<void> {
    const config = await this.readConfig();
    const engineConfig = config.engines[engine];
   ...
   * 列出所有引擎
   */
  async listEngines(): Promise<void> {
    const config = await this.readConfig();
   ...
   * 记录访问日志
   */
  private async logAccess(engine: string, action: string): Promise<void> {
    try {
      const config = await this.readConfig();
      config.metadata.accessLog.push({
        engine,
  ...
  try {
    switch (command) {
      case 'init':
        ...
        const engines: EngineType[] = ['bailian', 'tavily', 'serper', 'exa', 'firecrawl'];
