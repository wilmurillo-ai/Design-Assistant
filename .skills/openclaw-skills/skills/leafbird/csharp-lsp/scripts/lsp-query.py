...
class LSPClient:
    ...
    def _handle_server_request(self, msg):
        """Respond to server-to-client requests (e.g. client/registerCapability)."""
        rid = msg["id"]
        method = msg.get("method", "")
        with open("/tmp/lsp-query-debug.log", "a") as _dbg:
            _dbg.write(f"[srv-req] {method} id={rid}\n")
        response = {"jsonrpc": "2.0", "id": rid, "result": None}
        try:
            ...
            with open("/tmp/lsp-query-debug.log", "a") as _dbg:
                _dbg.write(f"[srv-req-err] {method}: {e}\n")

    def _handle_notification(self, msg):
        method = msg.get("method", "")
        with open("/tmp/lsp-query-debug.log", "a") as _dbg:
            _dbg.write(f"[notif] {method}\n")
            if method == "textDocument/publishDiagnostics":
                _dbg.write(f"  uri={msg.get('params',{}).get('uri','')}\n")
        if method == "textDocument/publishDiagnostics":
    ...
    def _open_file(self, filepath):
        ...
        # Wait for server to process the file. Roslyn-based servers (e.g. csharp-ls)
        # need time to load the project and compile. Wait until diagnostics arrive
        # for this file, or up to a timeout.
        deadline = time.time() + 30
        while time.time() < deadline:
            if furi in self._diagnostics:
                break
            time.sleep(0.5)
        else:
            # Even if no diagnostics, give a minimum settling time
            time.sleep(1)
        return furi
