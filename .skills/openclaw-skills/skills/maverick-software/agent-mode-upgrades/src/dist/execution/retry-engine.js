    ...
    timeout: (tool) => {
        if (tool.name === "exec" && tool.arguments) {
            return {
                ...
                modifiedTool: {
                    ...tool,
                    arguments: { ...tool.arguments, timeout: (tool.arguments.timeout || 30) * 2 },
                },
            };
        }
        return null;
...
export class RetryEngine {
    ...
    diagnose(tool, error) {
        ...
        const historicalMatch = this.errorHistory.find(h => h.tool === tool.name && error.includes(h.error.slice(0, 50)));
        if (historicalMatch) {
            return {
                category: "unknown",
                isRetryable: true,
     ...
     */
    async executeWithRetry(tool, executor) {
        const attempts = [];
     ...
     */
    async generateAlternative(tool, error, previousAttempts) {
...
Answer in JSON:
{
  ...
  "arguments": { ... }
     ...
     */
    recordSuccess(originalTool, successfulTool, error) {
...
// Factory
// ============================================================================
export function createRetryEngine(config, llmCaller) {
