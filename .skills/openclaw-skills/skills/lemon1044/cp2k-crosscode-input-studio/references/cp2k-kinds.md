...
- note: renderer may keep the generic `GTH-PBE` placeholder when no library-specific resolution step is available
...
## 9. Renderer output requirements

Before writing the final input, the renderer should ensure:
1. every explicitly parsed element in the structure has a `KIND` mapping or an explicit warning
2. if a kind label differs from the element symbol, `ELEMENT` is present when needed
3. all unresolved library-specific choices are surfaced as placeholders or warnings rather than invented exact names
4. if the task requests all-electron on selected atoms, the draft should not silently retain incompatible GTH pseudopotentials for those atoms
...
- do not assume installed CP2K data files are available

If no exact basis can be confirmed:
- fallback order:
  1. `DZVP-MOLOPT-SR-GTH`
  2. `DZVP-MOLOPT-GTH`
  3. raise a warning instead of silently inventing a name
ing a name
stead of silently inventing a name
