...
function computeYearlyAges(chart, birthYear, palacesForYearly) {
  ...
  for (let year = startYear; year <= endYear; year++) {
    ...
    try {
      const horoscope = chart.horoscope(`${year}-1-1`);
      const yearlyPalaceNames = horoscope.yearly?.palaceNames || [];
      if (yearlyPalaceNames.length === 0) continue;

      palacesForYearly.forEach((palaceData, palaceIndex) => {
        if (palaceIndex >= yearlyPalaceNames.length) return;
        const yearlyPalaceName = yearlyPalaceNames[palaceIndex];
        if (yearlyPalaceName === "命宫") {
          const age = year - birthYear;
          if (age >= 0) {
            yearlyAgesByPalace[palaceData.index].push(age);
          }
        }
      });
    } catch {
