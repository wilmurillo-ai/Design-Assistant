...
> **Hotel / 酒店 — discovery:** CLI **`flyai search-hotel`** returns structured **hotel** listings. **Differentiator:** same command covers **POI-adjacent** search (`--poi-name` + `--dest-name`), **three lodging modes** 酒店/民宿/客栈 (`--hotel-types`), **CNY nightly cap** (`--max-price`), plus stars, beds, dates, keywords, and sort—output includes **`mainPic`** and **`detailUrl`** for agent UI. **Parameters** below are authoritative; do not invent flags.
