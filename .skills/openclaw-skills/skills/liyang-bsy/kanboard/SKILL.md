...
description:
  ...
  Use when working with Kanboard tasks, projects, boards, columns, swimlanes, comments, subtasks, and users. Trigger on: "create task", "move task", "show board", "list projects", "add comment", "kanboard", "kanban task".
  Http basic auth, user is fixed string 'jsonrpc', password is Application API (token).
metadata:
  clawdbot:
    env:
...
1. KANBOARD_URL, the server url.
2. KANBOARD_API_TOKEN, the access api token.
