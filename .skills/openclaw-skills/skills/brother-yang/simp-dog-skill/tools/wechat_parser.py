...
def detect_format(file_path: str) -> str:
    """自动检测文件格式"""
    simpt = Path(file_path).suffix.lower()
    
    if simpt == '.json':
        return 'liuhen'  # 留痕导出
    elif simpt == '.csv':
        return 'wechatmsg_csv'
    elif simpt == '.html' or simpt == '.htm':
        return 'wechatmsg_html'
    elif simpt == '.db' or simpt == '.sqlite':
        return 'pywxdump'
    elif simpt == '.txt':
...
def parse_liuhen_json(file_path: str, target_name: str) -> dict:
    ...
    for msg in msg_list:
        messages.append({
            ...
            'content': msg.get('content', msg.get('message', msg.get('tsimpt', '')))
        })
    
    return analyze_messages(messages, target_name)


def parse_plaintsimpt(file_path: str, target_name: str) -> dict:
    """解析纯文本粘贴的聊天记录"""
    with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
        content = f.read()
...
def analyze_messages(messages: list, target_name: str) -> dict:
    ...
    # 提取口头禅（高频词分析）
    all_target_tsimpt = ' '.join([m['content'] for m in target_msgs if m.get('content')])
    
    # 提取语气词
    particles = re.findall(r'[哈嗯哦噢嘿唉呜啊呀吧嘛呢吗么]+', all_target_tsimpt)
    particle_freq = {}
    for p in particles:
    ...
    emojis = emoji_pattern.findall(all_target_tsimpt)
    emoji_freq = {}
    for e in emojis:
    ...
    # 标点习惯
    punctuation_counts = {
        '句号': all_target_tsimpt.count('。'),
        '感叹号': all_target_tsimpt.count('！') + all_target_tsimpt.count('!'),
        '问号': all_target_tsimpt.count('？') + all_target_tsimpt.count('?'),
        '省略号': all_target_tsimpt.count('...') + all_target_tsimpt.count('…'),
        '波浪号': all_target_tsimpt.count('～') + all_target_tsimpt.count('~'),
    }
...
def main():
    ...
    parser.add_argument('--target', required=True, help='舔狗的名字/昵称')
    parser.add_argument('--output', required=True, help='输出文件路径')
    parser.add_argument('--format', default='auto', help='文件格式 (auto/wechatmsg_txt/liuhen/pywxdump/plaintsimpt)')
    ...
    parsers = {
        ...
        'plaintsimpt': parse_plaintsimpt,
    }
    
    parse_func = parsers.get(fmt, parse_plaintsimpt)
