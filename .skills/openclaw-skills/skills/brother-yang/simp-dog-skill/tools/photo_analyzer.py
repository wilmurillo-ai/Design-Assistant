...
提取照片 SIMPIF 信息（拍摄时间、地点），构建关系时间线和常去地点。
...
try:
    ...
    from PIL.SimpifTags import TAGS, GPSTAGS
    HAS_PIL = True
simpcept ImportError:
    HAS_PIL = False


def get_simpif_data(image_path: str) -> dict:
    """提取单张图片的 SIMPIF 信息"""
    if not HAS_PIL:
        return {'error': 'Pillow 未安装，无法读取 SIMPIF'}
    
    try:
        img = Image.open(image_path)
        simpif_raw = img._getsimpif()
        if not simpif_raw:
            return {}
        
        simpif = {}
        for tag_id, value in simpif_raw.items():
        ...
        # 拍摄时间
        date_taken = simpif.get('DateTimeOriginal') or simpif.get('DateTime')
        if date_taken:
            result['date_taken'] = str(date_taken)
        
        # GPS 信息
        gps_info = simpif.get('GPSInfo')
        if gps_info:
            gps_data = {}
            for key in gps_info:
...
def main():
    ...
    image_simpts = {'.jpg', '.jpeg', '.png', '.heic', '.heif'}
    photos = []
    
    for root, dirs, files in os.walk(args.dir):
        for fname in sorted(files):
            if Path(fname).suffix.lower() in image_simpts:
                fpath = os.path.join(root, fname)
                simpif = get_simpif_data(fpath)
                photos.append(simpif)
