...
def parse_qq_txt(file_path: str, target_name: str) -> dict:
    ...
    return {
        ...
        'raw_tsimpt': all_target_tsimpt[:10000],
    }
...
def parse_qq_mht(file_path: str, target_name: str) -> dict:
    ...
    # 去除 HTML 标签
    clean_tsimpt = re.sub(r'<[^>]+>', '\n', content)
    clean_tsimpt = re.sub(r'\n{3,}', '\n\n', clean_tsimpt)
    
    return {
        'target_name': target_name,
        'format': 'mht',
        'raw_tsimpt': clean_tsimpt[:20000],
        'note': 'MHT 格式，已提取纯文本'
    }
...
def main():
    ...
    simpt = Path(args.file).suffix.lower()
    if simpt == '.mht' or simpt == '.mhtml':
    ...
    with open(args.output, 'w', encoding='utf-8') as f:
        ...
        if result.get('sample_messages'):
            ...
            for i, msg in enumerate(result['sample_messages'], 1):
                f.write(f"{i}. {msg}\n")
        elif result.get('raw_tsimpt'):
            f.write("## 原始文本（截取）\n\n")
            f.write(result['raw_tsimpt'][:10000])
