...
管理舔狗 Skill 的文件操作：列出、创建目录、生成组合 SKILL.md。
...
def list_skills(base_dir: str):
    ...
    print(f"共 {len(skills)} 个舔狗 Skill：\n")
    for s in skills:
        profile = s['profile']
...
"""
    ...
    # Create symlink or copy to .trae/skills/ to take effect immediately in Trae IDE
    trae_skills_dir = os.path.join('.trae', 'skills', f'simp-{slug}')
    os.makedirs(trae_skills_dir, exist_ok=True)
    trae_skill_path = os.path.join(trae_skills_dir, 'SKILL.md')
    
    with open(trae_skill_path, 'w', encoding='utf-8') as f:
        f.write(skill_md)
    print(f"已同步至 {trae_skill_path}，Skill 立即生效")


def main():
    parser = argparse.ArgumentParser(description='Skill 文件管理器')
    parser.add_argument('--action', required=True, choices=['list', 'init', 'combine'])
    parser.add_argument('--base-dir', default='./simps', help='基础目录')
    parser.add_argument('--slug', help='舔狗代号')
