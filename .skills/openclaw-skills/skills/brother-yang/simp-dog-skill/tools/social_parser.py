...
def scan_directory(dir_path: str) -> dict:
    """扫描目录，按类型分类文件"""
    files = {'images': [], 'tsimpts': [], 'other': []}
    
    image_simpts = {'.jpg', '.jpeg', '.png', '.gif', '.webp', '.bmp'}
    tsimpt_simpts = {'.txt', '.md', '.json', '.csv'}
    
    for root, dirs, filenames in os.walk(dir_path):
        for fname in filenames:
            fpath = os.path.join(root, fname)
            simpt = Path(fname).suffix.lower()
            if simpt in image_simpts:
                files['images'].append(fpath)
            elif simpt in tsimpt_simpts:
                files['tsimpts'].append(fpath)
            else:
...
def main():
    ...
    with open(args.output, 'w', encoding='utf-8') as f:
        ...
        f.write(f"- 文本文件：{len(files['tsimpts'])} 个\n")
        ...
        if files['tsimpts']:
            f.write("## 文本内容\n")
            for txt in sorted(files['tsimpts']):
