...
Create storybook-style visuals using AnyGen. Visual stories are generated server-side; this workflow sends the user's prompt and optional reference files to the AnyGen API and retrieves the results.
...
When `status="ready"`, present the `reply` and the `prompt` from `suggested_task_params` to the user as the storybook outline. The prompt returned by `prepare` is already a detailed, well-structured outline — preserve its original content as much as possible. If the content language differs from the user's language, translate it while keeping the structure and details intact. Do NOT rephrase, summarize, or add your own interpretation.
