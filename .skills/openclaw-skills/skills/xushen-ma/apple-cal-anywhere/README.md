...
Your Apple Calendar, on any platform. Manage events, attachments, and availability from macOS, Linux, and Windows.
...
#### Security Guardrails
To prevent accidental or malicious upload of sensitive system files, `attach add` enforces:
- **Extension Allowlist**: Only common document, image, and media types are permitted (`.pdf`, `.png`, `.docx`, `.txt`, etc.).
- **Sensitive Path Blocking**: Denies access to paths containing `.ssh`, `.gnupg`, `keychains`, `.aws`, etc.
- **Sensitive Name Blocking**: Denies files named `.env`, `id_rsa`, `*credentials*`, `*.pem`, `*.key`, etc.

#### Scoping to a Safe Directory
You can optionally restrict all uploads to a specific folder by setting the `APPLECAL_ATTACH_DIR` environment variable:
```bash
export APPLECAL_ATTACH_DIR="~/Documents/CalendarAttachments"
```
When set, the script will reject any file that does not resolve inside this directory.
