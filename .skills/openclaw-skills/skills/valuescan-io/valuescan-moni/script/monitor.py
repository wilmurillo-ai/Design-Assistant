...
def write_signal(payload: str, output_dir: str) -> tuple:
    """Returns (symbol, signal_type)."""
...
def notify_feishu(webhook: str, text: str) -> None:
    if not webhook:
        return
    try:
        requests.post(
...
def run_market(config: dict) -> None:
    ...
    webhook = config.get("feishuWebhook", "")
    ...
    for event in sseclient.SSEClient(resp).events():
        ...
        if event.event == "market":
            ...
            if webhook:
                try:
                    market_content = json.loads(event.data).get("content", event.data)
                except Exception:
                    market_content = event.data
                notify_feishu(webhook, f"【ValueScan 大盘分析】\n{market_content}")
...
def run_signal(config: dict, token_ids: list = None) -> None:
    ...
    for event in sseclient.SSEClient(resp).events():
        ...
        if event.event == "signal":
            symbol, signal_type = write_signal(event.data, output_dir)
            if webhook:
                type_zh = TYPE_ZH.get(signal_type, signal_type)
                try:
                    outer = json.loads(event.data)
                    inner = json.loads(outer.get("content", "{}"))
                    price = inner.get("price")
                    pct = inner.get("percentChange24h")
                    name = inner.get("name", "")
                    fmt = inner.get("fundsMovementType")
                    trade = inner.get("tradeType")
                    ts = outer.get("ts") or inner.get("updateTime")
                    lines = [f"【ValueScan 信号】{symbol} · {type_zh}"]
                    if ts:
                    ...
                    if price is not None:
                        lines.append(f"💰 价格：${float(price):.4f}")
                    if pct is not None:
                        pct_sign = "+" if pct >= 0 else ""
                        lines.append(f"📈 24h 涨幅：{pct_sign}{pct:.2f}%")
                    if trade is not None:
                        lines.append(f"📊 市场：{TRADE_TYPE_ZH.get(trade, trade)}")
                    if fmt is not None:
                        lines.append(f"🔔 信号类型：{FUNDS_MOVEMENT_TYPE_ZH.get(fmt, fmt)}")
                    notify_feishu(webhook, "\n".join(lines))
                except Exception:
                    notify_feishu(webhook, f"【ValueScan 信号】{symbol} · {type_zh}")
