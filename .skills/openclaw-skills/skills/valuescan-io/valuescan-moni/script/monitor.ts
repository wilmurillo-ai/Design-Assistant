...
function writeSignal(payload: string, outputDir: string): { symbol: string; type: string } {
    ...
    return { symbol, type: msg.type };
}

// ── Feishu ────────────────────────────────────────────────────────────────────

async function notifyFeishu(webhook: string, text: string): Promise<void> {
    if (!webhook) return;
    try {
        await fetch(webhook, {
            method: 'POST',
...
function runMarket(config: Config): void {
    ...
    es.addEventListener('market', (event: MessageEvent) => {
        ...
        if (config.feishuWebhook) {
            notifyFeishu(config.feishuWebhook, `【ValueScan 大盘分析】\n${event.data.slice(0, 300)}`);
        }
    });
...
async function runSignal(config: Config, tokens?: string): Promise<void> {
    ...
    es.addEventListener('signal', (event: MessageEvent) => {
        const { symbol, type } = writeSignal(event.data, config.outputDir);
        if (config.feishuWebhook) {
            const typeZh = TYPE_ZH[type] ?? type;
            let preview = '';
            try { preview = (JSON.parse(event.data).content ?? '').slice(0, 300); } catch { preview = event.data.slice(0, 300); }
            notifyFeishu(config.feishuWebhook, `【ValueScan 信号】${symbol} · ${typeZh}\n${preview}`);
        }
    });
