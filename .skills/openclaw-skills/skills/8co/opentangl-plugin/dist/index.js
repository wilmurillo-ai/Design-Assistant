 ...
 */
function resolveBin(config, workdir) {
    if (config.bin) {
    ...
    throw new Error(`OpenTangl CLI not found. Set plugin config 'workdir' to the OpenTangl workspace root, or install opentangl globally.`);
}
// ---------------------------------------------------------------------------
...
function assertIdentifier(value, name) {
    if (!RE_IDENTIFIER.test(value)) {
        throw new Error(`Invalid ${name} "${value}": only letters, digits, hyphens and underscores are allowed.`);
    }
}
function assertProjectList(value, name) {
    if (!RE_PROJECT_LIST.test(value)) {
        throw new Error(`Invalid ${name} "${value}": expected comma-separated identifiers (letters, digits, hyphens, underscores).`);
    }
}
...
function assertVar(value) {
    ...
    if (!RE_VAR_KEY.test(key)) {
        throw new Error(`Invalid var key "${key}": only letters, digits, and underscores are allowed.`);
    }
}
// ---------------------------------------------------------------------------
 ...
 */
function runCli(args, workdir, bin, timeout) {
    ...
    if (result.status !== 0) {
        ...
        if (stdout.trim())
            return stdout;
        throw new Error(stderr.trim() || `opentangl exited with code ${String(result.status)}`);
    }
    return stdout;
}
function ok(text) {
...
function fail(text) {
    return { content: [{ type: "text", text: text.trim() }], isError: true };
}
// ---------------------------------------------------------------------------
// Plugin entry
// ---------------------------------------------------------------------------
export default definePluginEntry({
    id: "opentangl",
    name: "OpenTangl",
    description: "Autonomous multi-repo development engine. Proposes tasks, runs autopilot cycles, manages workflows and the merge pipeline.",
    register(api) {
        ...
        const run = (args) => runCli(args, workdir, bin, timeout);
        // -----------------------------------------------------------------------
        // READ-ONLY TOOLS (always available, no side effects)
        // -----------------------------------------------------------------------
        api.registerTool({
            name: "opentangl_queue",
            description: "Show the current OpenTangl task queue: pending, running, completed, and failed tasks with dependency status.",
        ...
        api.registerTool({
            ...
            description: "List all projects registered in projects.yaml with their resolved paths and configuration.",
            parameters: Type.Object({}),
            async execute(_id, _params) {
                try {
        ...
        api.registerTool({
            ...
            description: "List recent workflow executions and their status (completed, failed, running).",
            parameters: Type.Object({}),
            async execute(_id, _params) {
                try {
                    return ok(run(["list"]));
        ...
        api.registerTool({
            ...
            description: "Ask OpenTangl to analyze the codebase and propose new development tasks. Use mode='preview' to see what would be queued without writing anything, or mode='queue' to append tasks to tasks/queue.yaml.",
            parameters: Type.Object({
                mode: Type.Union([Type.Literal("preview"), Type.Literal("queue")], {
                    description: "preview = dry run, queue = write to tasks/queue.yaml",
                    default: "preview",
                }),
                projects: Type.Optional(Type.String({
                    description: "Comma-separated list of project IDs to include (e.g. 'web,api'). Omit to use the default project.",
                })),
                featureRatio: Type.Optional(Type.Number({
                    minimum: 0,
                    maximum: 1,
                    description: "Target ratio of feature tasks vs maintenance tasks (0–1). Defaults to 0.7.",
                })),
            }),
            async execute(_id, params) {
                try {
                    ...
                    const args = ["propose", params["mode"]];
                    if (projects)
        ...
        api.registerTool({
            ...
            description: "Run OpenTangl autopilot: propose tasks and execute them in autonomous cycles. This writes code, creates commits, opens PRs, and merges. High impact — use with care.",
            parameters: Type.Object({
                cycles: Type.Optional(Type.Integer({
                    minimum: 1,
                    maximum: 10,
                ...
                projects: Type.Optional(Type.String({
                    description: "Comma-separated project IDs to target. Omit for the default project.",
                })),
                featureRatio: Type.Optional(Type.Number({
                    minimum: 0,
                    maximum: 1,
            ...
            async execute(_id, params) {
                try {
                    ...
                    if (params["cycles"])
                        args.push("--cycles", String(params["cycles"]));
                    if (projects)
                        args.push("--projects", projects);
                    if (params["featureRatio"] != null)
                        args.push("--feature-ratio", String(params["featureRatio"]));
        ...
        api.registerTool({
            ...
            description: "Run a workflow YAML file through OpenTangl. In 'run' mode the agent produces outputs but does not commit. In 'auto' mode it writes, verifies, and commits autonomously.",
            parameters: Type.Object({
                workflow: Type.String({
                    description: "Path to the workflow YAML file (relative to workdir or absolute).",
                }),
                mode: Type.Union([Type.Literal("run"), Type.Literal("auto")], {
                    description: "run = generate outputs only, auto = write + verify + commit",
                    default: "run",
                }),
                vars: Type.Optional(Type.Array(Type.String({
                    description: "Variable in key=value format",
                }), {
                    description: "Workflow variables as key=value pairs (e.g. ['feature_name=auth', 'target=src/auth.ts']).",
                })),
        ...
        api.registerTool({
            name: "opentangl_next",
            description: "Execute the next eligible pending task from the queue. Respects dependency ordering.",
            parameters: Type.Object({
                project: Type.Optional(Type.String({
        ...
        api.registerTool({
            name: "opentangl_wire",
            description: "Run a read-only cross-repo wiring audit. Analyzes how multiple projects connect and surfaces integration gaps or mismatches. Does not write any code.",
            parameters: Type.Object({
                projects: Type.String({
                    description: "Comma-separated list of project IDs to audit (e.g. 'web,api,mobile').",
                }),
            }),
            async execute(_id, params) {
                try {
        ...
        api.registerTool({
            ...
            description: "Run the OpenTangl merge pipeline for completed tasks: creates PRs, waits for CI, reviews diffs with LLM, and merges or escalates.",
            parameters: Type.Object({}),
            async execute(_id, _params) {
                try {
                    return ok(run(["merge"]));
        ...
        api.registerTool({
            ...
            description: "Remove terminal tasks (completed, merged, failed) from the queue and commit the cleaned queue state.",
            parameters: Type.Object({}),
            async execute(_id, _params) {
                try {
                    return ok(run(["prune"]));
        ...
        api.registerTool({
            ...
            description: "Resume a failed or interrupted workflow execution by its execution ID.",
            parameters: Type.Object({
