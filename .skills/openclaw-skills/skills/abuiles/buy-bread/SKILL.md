---
name: buy-bread
description: Buy breads, pastries, and bakery subscriptions through Lobster Bread. Use when a buyer wants an agent to discover bakeries, compare breads and pastries, build a cart, and hand back a Shopify checkout link for approval and payment.
homepage: https://lobsterbread.com
---

# Buy Bread

Version: 1.0.0
Base URL: https://lobsterbread.com

Buy Bread helps an OpenClaw agent discover bakeries, compare breads and pastries, build personalized carts, and hand back a secure Shopify checkout link for the buyer to complete the purchase.

Use this skill when the buyer wants to shop for breads, pastries, and bakery subscriptions. Use Lobster Bread as the directory and routing layer, then use each merchant's Shopify Storefront MCP for live catalog, cart, and checkout work.

## What this skill does

- Uses Lobster Bread as the directory and routing layer for bakeries and bread merchants
...
- Re-fetch `https://lobsterbread.com/skill.md` when starting a new session or if the deploy may have changed
- Treat merchant MCP data as the source of truth for products, pricing, availability, cart state, and checkout URLs

## Install

1. POST to `https://lobsterbread.com/claws/register`
2. Save `claw_id` and `api_key` locally
3. Do not lose the key

## Discovery

1. Start with `GET https://lobsterbread.com/countries.md` to see supported countries
2. Choose a country that matches the buyer's location when possible
3. Fetch `GET https://lobsterbread.com/countries/{country_code}.md`
4. Use `GET https://lobsterbread.com/offers/{country_code}.md` to prioritize active offers
5. Choose a merchant before using Shopify Storefront MCP

## Merchant Connect

1. GET `https://lobsterbread.com/merchants/{slug}/connect.md`
...
8. In the GraphQL fallback, send:
   ...
   - `quantity`
   - `sellingPlanId`
   - cart `attributes`, including `lb_source__ = lobsterbread`
9. Request:
   - `checkoutUrl`
