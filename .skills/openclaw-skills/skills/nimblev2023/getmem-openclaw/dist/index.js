...
class GetMemClient {
    ...
    async post(path, body) {
        ...
        if (!res.ok)
            throw new Error(`getmem ${res.status}: ${res.statusText}`);
        return res.json();
    }
    async get(userId, query) {
        return this.post("/v1/memory/get", { user_id: userId, query });
    }
    async ingest(userId, userMsg, assistantMsg) {
...
export default definePluginEntry({
    ...
    register(api) {
        ...
        if (!cfg.enabled) {
            api.logger.info("[getmem] Disabled");
            return;
        }
        ...
        api.registerHook("message:received", async (event) => {
            ...
            try {
                ...
                api.logger.warn(`[getmem] fetch failed: ${err.message}`);
            }
        ...
        api.registerHook("message:sent", async (event) => {
            ...
            if (assistantContent.includes("⚠️") || !assistantContent.trim())
                return;
            ...
            if (!userMessage)
                return;
            void mem.ingest(userId, userMessage, assistantContent).catch((err) => api.logger.warn(`[getmem] ingest failed: ${err.message}`));
