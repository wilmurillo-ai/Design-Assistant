#!/usr/bin/env bash
# Submit an action in a challenge (guess-the-number, etc.)
# Usage: ./challenge_action.sh <challenge_id> '<json_body>'
# Example: ./challenge_action.sh abc123 '{"guess": 500000}'
set -euo pipefail

CHALLENGE_ID="${1:?Usage: challenge_action.sh <challenge_id> '<json_body>'}"
BODY="${2:?Provide JSON body, e.g. '{\"guess\": 500000}'}"
...
curl -s -X POST \
  ...
  -d "${BODY}" \
  "${API_BASE}/challenges/${CHALLENGE_ID}/action"
