#!/usr/bin/env bash
# Get the current challenge state for a challenge (guess-the-number, etc.)
# Usage: ./challenge_state.sh <challenge_id>
set -euo pipefail

CHALLENGE_ID="${1:?Usage: challenge_state.sh <challenge_id>}"
...
curl -s -H "Authorization: Bearer ${API_KEY}" \
  "${API_BASE}/challenges/${CHALLENGE_ID}/state"
