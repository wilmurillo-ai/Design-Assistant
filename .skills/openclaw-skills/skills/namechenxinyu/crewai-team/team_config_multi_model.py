"""
CrewAI 产品需求收集团队配置 - 多模型优化版
先生，这个版本为不同角色配置最适合的模型
"""
...
def create_prd_summary_task() -> Task:
    return Task(
        ...
        agent=design_expert  # 由产品设计专家汇总（用 Max 模型）
    )
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("✅ CrewAI 团队配置完成（多模型优化版）")
    print("=" * 60)
    print("\n👥 团队成员与模型配置:")
    print(f"  📊 市场调研分析师 → qwen3.5-plus")
    print(f"  🎨 产品设计专家   → qwen3-max (高质量)")
    print(f"  🏗️ 技术总监       → qwen3-coder-plus")
    print(f"  💻 全栈技术专家   → qwen3-coder-next")
    print(f"  ✅ 质量专家       → qwen3.5-plus")
    print(f"\n📊 Agent 数量：{len(crew.agents)}")
    print(f"📋 任务数量：{len(crew.tasks)}")
    print("\n💰 预估成本：¥0.3-0.8 元/完整 PRD")
    print("=" * 60)
