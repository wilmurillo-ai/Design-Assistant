 ...
 * Replaces: pipeline-trace.js (332 lines) + metrics-collector.js (219 lines)
 * This file: ~220 lines
 */
...
class Diagnostics {
  ...
  // ═══ Phase API (trace-style) ═══
  start(phaseId, meta) {
    const ph = PHASES[phaseId];
    if (!ph) { console.log(`\x1b[33m[WARN] Unknown phase: ${phaseId}\x1b[0m`); return; }
  ...
  succeed(phaseId, meta) {
    ...
    if (!rec) { console.log(`\x1b[33m[WARN] succeed() unknown: ${phaseId}\x1b[0m`); return; }
    rec.status = 'succeeded'; rec.end = Date.now(); rec.duration_ms = rec.end - rec.start;
  ...
  fail(phaseId, err, meta) {
    ...
    if (rec) { rec.status = 'failed'; rec.end = Date.now(); rec.duration_ms = rec.end - rec.start; rec.error = msg; if (meta) Object.assign(rec.metadata, meta); }
  ...
  skip(phaseId, reason) {
    const rec = this.phases[phaseId];
    const ph = PHASES[phaseId];
    if (rec) { rec.status = 'skipped'; rec.end = Date.now(); rec.duration_ms = rec.end - rec.start; }
    console.log(`\x1b[33m⏭ [${ph?.label}]${RESET} Пропущен: ${ph?.label}${reason ? ` (${reason})` : ''}`);
  }

  // ═══ Memory + Counters (metrics-style) ═══
  snapMemory(label) {
  ...
  _emit(phaseId, status) {
    ...
    this.events.push(e);
    if (this.logDir) { try { fs.appendFileSync(this.logDir + `/trace-${this.traceId}.jsonl`, JSON.stringify(e) + '\n'); } catch (e2) {} }
  }

  _fmt(ms) { if (ms < 1000) return ms + 'ms'; if (ms < 60000) return (ms / 1000).toFixed(1) + 's'; return Math.floor(ms / 60000) + 'm' + ((ms % 60000) / 1000).toFixed(0) + 's'; }
  ...
  save() {
    ...
    const data = { traceId: this.traceId, phases: this.phases, memory: { peakMB: this.memorySnapshots.length > 0 ? Math.max(...this.memorySnapshots.map(s => s.rssMB)) : null }, counts: { retries: this.retryCount, continueRounds: this.continueRounds, apiRequests: this.apiRequests }, answerLength: this.charactersExtracted, answerComplete: this.answerComplete, incompleteReason: this.incompleteReason, error: this.error, totalMs: Date.now() - this.startTime, platform: os.platform(), nodeVersion: process.version };
    const file = this.logDir + '/metrics-' + this.traceId + '.json';
    try { fs.writeFileSync(file, JSON.stringify(data, null, 2)); return file; } catch (e) { return null; }
  }
