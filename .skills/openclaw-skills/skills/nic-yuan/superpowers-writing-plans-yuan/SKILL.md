...
description: >
  ...
  Guides writing comprehensive implementation plans with bite-sized tasks (2-5 min each), TDD cycle,
  and DRY/YAGNI principles. Every task must have exact file paths, actual code, and verification commands.
  Triggered by: approved design, user says "write a plan", "make a plan", "implementation plan",
  "break this down into tasks". Part of the superpowers suite.
version: 1.0.2
read_when:
  - User has an approved design or spec
  - User asks to plan implementation
  - Before writing any code on a multi-step task
  - "write a plan" / "make a plan" / "implementation plan" / "break this down"
  - Design has been approved and implementation needs to be decomposed
metadata:
  openclaw:
    emoji: "📋"
    requires:
