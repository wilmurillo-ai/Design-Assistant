...
description: >
  ...
  Use when: (1) building any new feature or app — triggers brainstorm → plan → subagent execution,
  (2) debugging a bug or test failure — triggers systematic root-cause process,
  (3) user says "let's build", "help me plan", "I want to add X", "this is broken",
  ...
  Requires: exec tool and sessions_spawn. Part of the superpowers suite.
version: 1.0.2
read_when:
  ...
  - User is unsure which superpowers skill to use — this is the entry point
metadata:
  openclaw:
    emoji: "⚡"
    requires:
