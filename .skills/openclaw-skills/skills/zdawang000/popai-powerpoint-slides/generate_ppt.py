...
def send_generate(api_key, channel_id, message, file_infos=None, output_file=None):
    """Trigger PPT generation via SSE, output parsed events as JSON lines."""
    payload = {"channelId": channel_id, "message": message}
    if file_infos:
    ...
    out = open(output_file, "a") if output_file else None
    try:
...
def main():
    ...
    parser.add_argument("--output", "-o", help="Write events to file (for background polling)")
    ...
    try:
        ...
        if args.file:
            if len(args.file) > 5:
                ...
                sys.exit(1)
            with ThreadPoolExecutor(max_workers=min(len(args.file), 5)) as pool:
                file_infos = list(pool.map(lambda fp: upload_file(api_key, fp), args.file))
            print(f"Uploaded {len(file_infos)} file(s).", file=sys.stderr)
        ...
        send_generate(api_key, channel_id, args.query, file_infos, args.output)
