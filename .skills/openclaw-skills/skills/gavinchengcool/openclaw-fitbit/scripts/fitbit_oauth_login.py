#!/usr/bin/env python3
"""Interactive Fitbit OAuth login.
...
from fitbit_token import AUTH_URL, exchange_code_for_token, save_token, token_path


DEFAULT_SCOPES = "activity sleep heartrate profile weight"
...
def listen_for_code(redirect_uri: str, timeout_s: int = 180) -> str:
    ...
    if u.scheme != "http" or u.hostname not in ("127.0.0.1", "localhost"):
        raise SystemExit("Loopback mode requires FITBIT_REDIRECT_URI like http://127.0.0.1:<port>/<path>")
    if not u.port:
        raise SystemExit("Loopback mode requires an explicit port in FITBIT_REDIRECT_URI")
    path = u.path or "/"
...
def main() -> None:
    ...
    redirect_uri = must_env("FITBIT_REDIRECT_URI")
    scopes = os.environ.get("FITBIT_SCOPES", DEFAULT_SCOPES)
