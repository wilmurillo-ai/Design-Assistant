...
class PromptOptimizer:
    ...
    def __init__(self, config_path: Optional[str] = None, use_cache: bool = True):
        ...
        self._detected_audience: str = ""
        self._detected_constraints: List[str] = []
        ...
        logger.info(f"   输出格式: {len(self.config['format_patterns'])} 种")
        logger.info(f"   目标受众: {len(self.config.get('audience_patterns', {}))} 种")
    ...
    def optimize(self, prompt: str, use_cache: Optional[bool] = None) -> OptimizationResult:
        ...
        try:
            ...
            # v3.2 新增：检测更多属性
            detected_format = self._detect_format(original)
            detected_style = self._detect_style(original)
            detected_audience = self._detect_audience(original)
            detected_constraints = self._detect_constraints(original)
            
            missing_info = self._detect_missing_info(original, task_type)
            optimized = self._apply_enhancements(
                original, task_type, missing_info,
                detected_format, detected_style, detected_audience, detected_constraints
            )
    ...
    def _detect_format(self, prompt: str) -> str:
        """v3.2 新增：检测输出格式"""
        prompt_lower = prompt.lower()
        for fmt_type, keywords in self.config.get("format_patterns", {}).items():
    ...
    def _detect_style(self, prompt: str) -> str:
        """v3.2 新增：检测风格"""
        prompt_lower = prompt.lower()
        for style_type, keywords in self.config.get("style_patterns", {}).items():
            if any(kw in prompt_lower for kw in keywords):
                return style_type
        return ""
    ...
    def _detect_audience(self, prompt: str) -> str:
        """v3.2 新增：检测目标受众"""
        prompt_lower = prompt.lower()
        audience_patterns = self.config.get("audience_patterns", {})
        for audience_type, keywords in audience_patterns.items():
            if any(kw in prompt_lower for kw in keywords):
                return audience_type
        return ""
    
    def _detect_constraints(self, prompt: str) -> List[str]:
        """v3.2 新增：检测约束条件"""
        constraints = []
        prompt_lower = prompt.lower()
        constraint_patterns = self.config.get("constraint_patterns", {})
        
        for constraint_type, levels in constraint_patterns.items():
            for level, keywords in levels.items():
                if any(kw in prompt_lower for kw in keywords):
                    constraints.append(f"{constraint_type}-{level}")
    ...
    def _apply_enhancements(
        ...
        detected_constraints: List[str] = None
    ) -> str:
        """应用增强 - v3.2 支持更多参数"""
        if detected_constraints is None:
        ...
        if detected_audience:
            audience_hints = {
                "技术人员": "请用技术语言，包含代码实现细节",
                "产品经理": "请从业务角度分析，突出价值和方案对比",
                "管理者": "请用简洁清晰的方式，突出决策要点",
                "运维人员": "请考虑部署和运维的便利性",
                "设计师": "请注重视觉和用户体验",
                "初学者": "请用通俗易懂的语言，逐步引导",
                "普通用户": "请用简单直白的表达",
            }
            if detected_audience in audience_hints:
                enhanced_parts.append(audience_hints[detected_audience])
        ...
        # v3.2: 根据检测到的输出格式添加要求
        format_hints = {
            "Markdown": "请用 Markdown 格式输出，包含适当的标题、列表和代码块",
            "JSON": "请输出标准的 JSON 格式，便于程序解析",
            "YAML": "请输出 YAML 格式配置",
            "XML": "请输出 XML 格式数据",
            "表格": "请用表格形式展示数据，包含表头",
            "代码块": "请用代码块输出，包含语法高亮标记",
            "PlantUML": "请用 PlantUML 语法输出",
            "SQL": "请输出可直接执行的 SQL 语句",
        }
        if detected_format and detected_format in format_hints:
            enhanced_parts.append(format_hints[detected_format])
        ...
        if detected_style and detected_style in style_hints:
            enhanced_parts.append(style_hints[detected_style])
        
        # 补全缺失信息（原有逻辑）
        if "输出格式" in missing_info and not detected_format:
        ...
        if "风格" in missing_info and not detected_style:
        ...
        for constraint in detected_constraints:
            if "字数" in constraint:
                if "简短" in constraint:
                    enhanced_parts.append("请控制在 100 字以内")
                elif "中等" in constraint:
                    enhanced_parts.append("请控制在 300 字以内")
                elif "详细" in constraint:
                    enhanced_parts.append("请详细展开，可超过 500 字")
            elif "条数" in constraint:
                if "少量" in constraint:
                    enhanced_parts.append("请提供 3-5 条")
                elif "中等" in constraint:
                    enhanced_parts.append("请提供 5-10 条")
                elif "大量" in constraint:
                    enhanced_parts.append("请提供 10 条以上")
    ...
    def format_result(self, result: OptimizationResult) -> str:
        ...
        if result.output_format:
            output.append(f"📄 输出格式: {result.output_format}")
        if result.style:
            output.append(f"🎨 风格: {result.style}")
        if result.audience:
            output.append(f"👥 目标受众: {result.audience}")
        if result.constraints:
            output.append(f"📏 约束: {', '.join(result.constraints)}")
    ...
    def format_result_compact(self, result: OptimizationResult) -> str:
        """简洁格式化（只输出优化后的 prompt）"""
        return result.optimized
    
    def format_result_json(self, result: OptimizationResult, indent: int = 2) -> str:
        """JSON 格式化"""
        return result.to_json(indent=indent)
    
    def format_result_yaml(self, result: OptimizationResult) -> str:
        """YAML 格式化"""
    ...
    def get_stats(self) -> dict:
        ...
        return {
            ...
            "audience_patterns": len(self.config.get("audience_patterns", {})),
            "constraint_patterns": len(self.config.get("constraint_patterns", {})),
