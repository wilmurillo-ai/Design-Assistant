...
### One-Command Disaster Recovery (when config is broken)

If `openclaw.json` is broken and OpenClaw cannot run, use this sequence:

```bash
# 1) Stop gateway first
openclaw gateway stop

# 2) Restore latest backup files
cd /path/to/soul-backup-skill
node scripts/restore.mjs

# 3) Recover config structure from sanitized backup
cp backups/<LATEST_OR_NAMED_BACKUP>/openclaw.sanitized.json ~/.openclaw/openclaw.json

# 4) Fill [REDACTED] values in ~/.openclaw/openclaw.json
...
✅ Recovery is complete only when the assistant can reply to a real `hello` message.
