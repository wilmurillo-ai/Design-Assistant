#!/usr/bin/env python3
"""
行业资讯查询脚本 (Industry Information)

基于 FEEDAX API 的行业资讯与舆情监测工具，提供全面的行业数据查询能力。

用法:
    python query_industry_information.py --api-key "your-api-key" --keyword "房地产" --sentiments 负面 --days 7
    python query_industry_information.py --keyword "人工智能" --page 0 --size 20

输出:
    - industry_information_<timestamp>.csv: 资讯结果 CSV 文件
    - industry_information_<timestamp>.md: 数据说明文件
...
def query_industry_information(
    api_key: str,
    keyword: str,
    ...
    Returns:
        dict: API 响应数据
    """
    url = f"{FEEDAX_BASE_URL}/data-service/v1/news/industry/external/query?apiKey={api_key}"
...
def generate_csv(data_list: list, output_path: str):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        # 写入表头
        headers = [
            "发布时间", "标题", "来源", "情感倾向", "行业正负面", "重要程度",
            "热度得分", "浏览数", "转发数", "证监会行业", "申万行业", "URL"
        ]
        ...
        for item in data_list:
            ...
            # 提取证监会行业
            csrc_industries = item.get('csrcIndustryVos', [])
            if csrc_industries:
                csrc_names = []
                for ind in csrc_industries[:2]:
                    category = ind.get('industryCategoryName', '')
                    specific = ind.get('industrySpecificName', '')
                    if category or specific:
                        csrc_names.append(f"{category}>{specific}" if specific else category)
                csrc_str = '; '.join(csrc_names)[:100]
            else:
                csrc_str = ''
            
            # 提取申万行业
            sw_industries = item.get('industrySwResultVos', [])
            if sw_industries:
                sw_names = []
                for ind in sw_industries[:2]:
                    sw1 = ind.get('industrySw1Name', '')
                    sw2 = ind.get('industrySw2Name', '')
                    if sw1 or sw2:
                        sw_names.append(f"{sw1}>{sw2}" if sw2 else sw1)
                sw_str = '; '.join(sw_names)[:100]
            else:
                sw_str = ''
            ...
            row = [
                ...
                item.get('industryAspectSentiment', '') or '',
...
def generate_description(response: dict, output_path: str, keyword: str, days: int):
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        ...
        for item in data:
            csrc_vos = item.get('csrcIndustryVos') or []
            for ind in csrc_vos:
                category = ind.get('industryCategoryName', '')
                specific = ind.get('industrySpecificName', '')
                if category:
            ...
            sw_vos = item.get('industrySwResultVos') or []
            for ind in sw_vos:
                sw1 = ind.get('industrySw1Name', '')
                sw2 = ind.get('industrySw2Name', '')
                if sw1:
        ...
        if csrc_industries:
            f.write("### 证监会行业\n\n")
            for name in list(csrc_industries)[:15]:
                f.write(f"- {name}\n")
            if len(csrc_industries) > 15:
                f.write(f"... 共 {len(csrc_industries)} 个行业\n")
        
        if sw_industries:
            f.write("\n### 申万行业\n\n")
            for name in list(sw_industries)[:15]:
                f.write(f"- {name}\n")
            if len(sw_industries) > 15:
                f.write(f"... 共 {len(sw_industries)} 个行业\n")
        
        # 数据来源说明
        f.write("\n## 数据来源\n\n")
        f.write("数据来自 FEEDAX 行业资讯监测平台，涵盖新闻、微信公众号、微博等多种信源。\n")
        f.write("\n---\n")
...
def format_output(data_list: list, verbose: bool = False):
    ...
    for i, item in enumerate(data_list, 1):
        ...
        industry_aspect = item.get('industryAspectSentiment', '')
        aspect_emoji = {'正面': '📈', '负面': '📉'}.get(industry_aspect, '')
        
        title = item.get('title', '')
        source = item.get('source', '')
        heat = item.get('heatScores', 0)
        
        print(f"{i}. [{date_str}] {sentiment_emoji} {sentiment} {aspect_emoji} {industry_aspect}")
        print(f"   标题：{title}")
        ...
        if csrc_industries:
            ...
            for ind in csrc_industries[:2]:
                ...
                specific = ind.get('industrySpecificName', '')
                if specific:
                    industries.append(f"{category}>{specific}")
                elif category:
...
def main():
    parser = argparse.ArgumentParser(
        description='行业资讯查询工具',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
示例:
  python query_industry_information.py --api-key "your-api-key" --keyword "房地产" --sentiments 负面 --days 7
  python query_industry_information.py --keyword "人工智能" --page 0 --size 20
  python query_industry_information.py --keyword "新能源" --sentiments 负面 中性 --days 30
        """
    )
    ...
    response = query_industry_information(
        api_key=api_key,
    ...
    if not args.no_output and data:
        ...
        csv_path = os.path.join(output_dir, f"industry_information_{timestamp}.csv")
        md_path = os.path.join(output_dir, f"industry_information_{timestamp}.md")
