"""
郑州科威莱步进电机控制器 Python 驱动
适用于 PC 端 (Windows/Linux/macOS)
...
class KeweilaiStepMotor:
    ...
    def __init__(self, port="COM5", baudrate=9600, device_addr=9, timeout=1):
        """
        初始化步进电机控制器
    ...
    def print_status(self):
        ...
        print("\n" + "=" * 50)
        print("郑州科威莱步进电机控制器 - 当前状态")
        print("=" * 50)
        
        limit_map = {0: "无限位", 1: "正转限位", 2: "反转限位"}
        
        print(f"位置：    {status.get('position', '?')}° ({self.degrees_to_mm(status.get('position', 0)):.2f}mm)")
        print(f"速度：    {status.get('speed', '?')} r/min")
        print(f"方向：    {'向右' if status.get('direction') == 1 else '向左'}")
        print(f"运行：    {'运行中' if status.get('run_cmd') == 1 else '停止'}")
        print(f"限位：    {limit_map.get(status.get('limit'), '?')}")
        print("=" * 50)
...
if __name__ == "__main__":
    print("=" * 50)
    print("郑州科威莱步进电机控制器 - Python 示例")
    print("河北雄安素水互联网科技有限公司")
    print("=" * 50)
    
    # 使用上下文管理器 (自动连接/断开)
    with KeweilaiStepMotor(port="COM5") as motor:
        # 测试连接
        print("\n[1] 测试连接...")
        ...
        # 示例：机械原点设定
        print("\n[3] 机械原点设定示例...")
        print("    注意：这将移动设备到反转限位")
        # if motor.mechanical_home():
        #     print("    [OK] 机械原点设定成功")
        # else:
        #     print("    [FAIL] 机械原点设定失败")
        ...
        # 读取所有寄存器
        print("\n[5] 读取所有关键寄存器...")
        for addr in [1, 2, 4, 7, 8, 10, 13, 14, 21, 22, 23]:
            value = motor.read_register(addr)
            print(f"    寄存器{addr:2d}: {value}")
