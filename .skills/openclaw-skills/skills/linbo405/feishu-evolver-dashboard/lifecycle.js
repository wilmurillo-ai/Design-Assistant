...
function ensureWatchdog() {
  ...
  // [FIX] Log watchdog invocations to a dedicated file for cron traceability
  const wdLogFile = path.resolve(__dirname, '../../logs/evolver_watchdog.log');
  const wdStamp = new Date().toISOString();
  // Use appendFileSync (always appends, no flag needed)
  fs.appendFileSync(wdLogFile, `[${wdStamp}] ensureWatchdog called\n`);
  ...
  try {
    ...
    if (!skipCheck) {
        ...
        try {
            ...
            fs.writeFileSync(cronStateFile, JSON.stringify({ lastChecked: Date.now(), exists: true }));
            // [FIX] Log success for cron traceability
            try { fs.appendFileSync(wdLogFile, `[${wdStamp}] OK\n`); } catch(e) {}
        } catch (e) {
            ...
            try {
                ...
                // [FIX] Log failure for cron traceability
                fs.appendFileSync(wdLogFile, `[${wdStamp}] FAIL: ${e.message}\n`);
            } catch (_) {}
        }
    }
  } catch (e) {
    console.error('[Lifecycle] Failed to ensure watchdog cron (outer):', e.message);
    try { fs.appendFileSync(wdLogFile, `[${wdStamp}] OUTER_FAIL: ${e.message}\n`); } catch(_) {}
  }
}
...
switch (action) {
  ...
  case 'daemon-loop':
    ...
    break;
  case 'ensure':{
    // [FIX] Log entry for cron traceability (Task Scheduler driven, not openclaw cron)
    const wdLogFile = path.resolve(__dirname, '../../logs/evolver_watchdog.log');
    const wdStamp = new Date().toISOString();
    fs.appendFileSync(wdLogFile, `[${wdStamp}] ensure started\n`);
    ...
    // Check if process is stuck (logs stale > 4h)
    let isStuck = false;
    try {
        const logFile = path.resolve(__dirname, '../../logs/wrapper_lifecycle.log');
        const outLog = path.resolve(__dirname, '../../logs/wrapper_out.log');
        ...
        let lifeStale = true, outStale = true;
        if (fs.existsSync(logFile)) lifeStale = (now - fs.statSync(logFile).mtimeMs) > threshold;
        if (fs.existsSync(outLog)) outStale = (now - fs.statSync(outLog).mtimeMs) > threshold;
        isStuck = lifeStale && outStale;
        if (isStuck) {
            console.warn('[Ensure] STUCK - logs stale > 4h. Sending rescue signal.');
            const pids = getAllRunningPids();
            for (const p of pids) { try { process.kill(p, 'SIGKILL'); } catch(e) {} }
            if (fs.existsSync(PID_FILE)) fs.unlinkSync(PID_FILE);
            if (fs.existsSync(DAEMON_PID_FILE)) fs.unlinkSync(DAEMON_PID_FILE);
            safeSendReport({ title: "🧬 Evolver STUCK Detected", status: "Status: STUCK - killed and restarted.", color: "red" });
        }
    } catch(e) { console.error('[Ensure] Stuck check error:', e.message); }
    ...
    try {
      if (fs.existsSync(ensureLock)) {
          const stats = fs.statSync(ensureLock);
          if (Date.now() - stats.mtimeMs < 300000) {
            fs.appendFileSync(wdLogFile, `[${wdStamp}] debounced\n`);
            return;
          }
      }
    ...
    // Skip ensureWatchdog() - it calls 'openclaw cron add' which times out on Windows
    // Task Scheduler handles the scheduling via schtasks every 30 minutes
    fs.appendFileSync(wdLogFile, `[${wdStamp}] skipping ensureWatchdog (Task Scheduler handles scheduling)\n`);

    // Think phase: idle-aware proactive thinking (NEW)
    (function thinkPhase() {
      const thinkStamp = new Date().toISOString();
      try {
        // Dynamically require idleScheduler to avoid breaking if module changes
        let idleRec = { intensity: 'normal', idle_seconds: -1, enabled: false };
        try {
          const idleMod = require(require('path').resolve(__dirname, '../../skills/evolver/src/gep/idleScheduler'));
          idleRec = idleMod.getScheduleRecommendation() || idleRec;
        } catch(e) { /* idleScheduler not available, skip */ }

        const isIdle = idleRec.enabled && (idleRec.intensity === 'aggressive' || idleRec.intensity === 'deep');

        // Read thinking queue
        const queuePath = require('path').resolve(__dirname, '../../memory/thinking-queue.md');
        let queueContent = '';
        let hasNewItems = false;
        try {
          if (require('fs').existsSync(queuePath)) {
            queueContent = require('fs').readFileSync(queuePath, 'utf8');
            hasNewItems = queueContent.includes('[TODO]');
          }
        } catch(e) {}

        // Always log the think phase (lightweight)
        fs.appendFileSync(wdLogFile, `[${thinkStamp}] think: idle=${idleRec.idle_seconds}s intensity=${idleRec.intensity} queue_has_todo=${hasNewItems}\n`);

        if (!isIdle || !hasNewItems) return; // Only think when idle AND queue has items

        // Extract first TODO item (must be at start of line, not inside blockquotes)
        const todoMatch = queueContent.match(/^## \[TODO\] (.+)$/m);
        if (!todoMatch) return;
        const taskTitle = todoMatch[1].trim().split('\n')[0].trim();

        fs.appendFileSync(wdLogFile, `[${thinkStamp}] THINKING: ${taskTitle.substring(0, 60)}\n`);

        // Update queue: mark as IN_PROGRESS
        const updated = queueContent.replace(
          '## [TODO] ' + taskTitle,
          `## [IN_PROGRESS] ${taskTitle} \`[${thinkStamp}]\``
        );
        require('fs').writeFileSync(queuePath, updated, 'utf8');

        // Build context: read research-topics and alpha-hypotheses
        const topicsPath = require('path').resolve(__dirname, '../../memory/research-topics.md');
        let context = '';
        try {
          if (require('fs').existsSync(topicsPath)) {
            context = require('fs').readFileSync(topicsPath, 'utf8');
          }
        } catch(e) {}

        // Dispatch thinking via a background node eval (non-blocking for ensure)
        const thinkScript = `
const fs = require('fs');
const path = require('path');
const Hyper = require('child_process').execFileSync || require('child_process').exec;

const TASK = ${JSON.stringify(taskTitle)};
const STAMP = ${JSON.stringify(thinkStamp)};

// Read full context
let ctx = '';
const topics = path.join(${JSON.stringify(path.dirname(queuePath))}, 'research-topics.md');
if (fs.existsSync(topics)) ctx += '## Research Topics\\n' + fs.readFileSync(topics, 'utf8') + '\\n';

// Simple LLM-free reflection: pattern match + insight generation
const insights = [];

// Topic 1: WeChat MP monitoring
if (TASK.includes('微信公众号')) {
  insights.push({ t: 'WeChat MP Monitoring', i: 'Use wechat-mp-cn skill with daily cron (not real-time, avoid rate limits). Push summary to Feishu via message tool.' });
}

// Topic 2: Stock data
if (TASK.includes('A股')) {
  insights.push({ t: 'Stock Fetcher', i: 'stock-browser-fetcher skill can run daily at market close (15:00-15:30). Parse market data and push index summary to Feishu.' });
}

// Topic 3: Three-bot collaboration
if (TASK.includes('三Bot') || TASK.includes('协作')) {
  insights.push({ t: 'Bot Auto-Relay', i: 'Use sessions_send in cron-inbox.md write hook. When小龙虾 writes to cron-inbox, meidou2 reads on next wake cycle.' });
}

// Fallback insight
if (insights.length === 0) {
  insights.push({ t: 'General', i: 'Queued task requires deeper context. Recommend adding more detail to research-topics.md.' });
}

// Write result to cron-inbox
const inboxPath = path.join(${JSON.stringify(path.dirname(queuePath))}, 'cron-inbox.md');
const result = `\\n\\n## [${STAMP}] AUTO_THINK -- ${TASK}\\nTask: ${TASK}\\nInsights: ${insights.map(x=>'**'+x.t+'**: '+x.i).join('\\n')}\\n`;

let inbox = '';
if (fs.existsSync(inboxPath)) {
  inbox = fs.readFileSync(inboxPath, 'utf8');
  // Remove '<!-- 新消息添加在顶部 -->' line and prepend
  inbox = inbox.replace(/^<!-- 新消息添加在顶部 -->\\n/m, '');
}
fs.writeFileSync(inboxPath, '<!-- 新消息添加在顶部 -->\\n' + result + '\\n---\\n' + inbox, 'utf8');

// Update thinking-queue: mark as DONE
const qPath = ${JSON.stringify(queuePath)};
let q = fs.readFileSync(qPath, 'utf8');
q = q.replace(
  '[IN_PROGRESS] ' + TASK + ' `\\\\[' + STAMP + ']\\`',
  '[DONE] ' + TASK + ' \\`[${STAMP}]\\`\\n  - Insight: ' + insights[0].i.substring(0, 100)
);
fs.writeFileSync(qPath, q, 'utf8');

console.log('Think complete:', insights[0].t);
`;

        const tmpFile = require('path').join(require('os').tmpdir(), 'evolver_think_' + Date.now() + '.js');
        require('fs').writeFileSync(tmpFile, thinkScript, 'utf8');
        require('child_process').spawn(process.execPath, [tmpFile], { detached: !IS_WIN, stdio: 'ignore', windowsHide: true }).unref();

      } catch(e) {
        try { fs.appendFileSync(wdLogFile, `[${thinkStamp}] think_error: ${e.message}\\n`); } catch(_) {}
      }
    })();

    // Ensure daemon is running
    if (!passArgs.includes('--daemon-check')) startDaemon();

    const runningPids = getAllRunningPids();
    if (runningPids.length > 1) {
        console.warn(`[Ensure] Multiple instances: ${runningPids.join(', ')}. Killing extras.`);
        runningPids.forEach(p => { try { process.kill(p, 'SIGKILL'); } catch(e) {} });
        if (fs.existsSync(PID_FILE)) fs.unlinkSync(PID_FILE);
        sleepSync(1000);
    }
    ...
    if (!getRunningPid()) {
      start(['--loop']);
      if (passArgs.includes('--report')) setTimeout(() => status(false), 2000);
      safeSendReport({ title: "🧬 Evolver Auto-Repair", status: "Status: [RESTARTED]", color: "orange" });
    } else {
      if (passArgs.includes('--json')) { setTimeout(() => status(true), 1000); return; }
      if (passArgs.includes('--report')) { status(false); return; }
      fs.appendFileSync(wdLogFile, `[${wdStamp}] healthy\n`);
      return;
    }

    if (!getRunningPid() || passArgs.includes('--json')) status(passArgs.includes('--json'));
    break;
}
